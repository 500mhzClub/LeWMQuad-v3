...
conversion, and Gazebo/Genesis parity.
...
audit, a smoke bag with a `/cmd_vel` drive, and the bag-to-raw-rollout
conversion. Each stage logs to `.generated/acceptance/<run>/stages/` and a
single `summary.json` records pass/fail. Use `--skip-build` /
