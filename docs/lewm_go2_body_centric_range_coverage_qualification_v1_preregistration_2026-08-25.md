...
## Prospective execution amendment and failed-attempt custody

The first execution frozen at commit `3ef985d7fcea8c26609fd6cc8a1d5e66507e9c1c` failed closed during state sensor materialisation at `wide-cal-0-05`: deployable applied-action identity had incorrectly been assumed to imply byte-identical replay geometry. The immutable failed namespace is `/home/andrewknowles/RecoveryStorage/LeWMQuad-v3/body_centric_range_coverage_qualification_v1__failed_3ef985d_copy_geometry_assertion`. Its `failed_attempt_receipt.json` SHA-256 is `277e76da379f7b992ae5b5df27d7cdf88b8f5b56d63720bb765e2074549f51e4`; its `FAILED_ATTEMPT_FILE_MANIFEST.jsonl` SHA-256 is `e6244e5d3284e29ad17a949a93b0d21e4d94e93557d57c3cafcda25feabae0a8`.

That attempt produced no `materialization_index.json`, no calibrated threshold, no development-held-out metric evaluation, no benchmark, no classification, and no result. Its partial shards are custody evidence only and are not reusable after refreeze. This amendment is prospective: the earlier freeze commit is not rewritten, the canonical output root remains unchanged, and reexecution starts from state zero with a fresh preexecution receipt.

Two identities are now frozen separately. `DECISION_ACTION_COPY_MAP` is the existing `AppliedActionCopyMap`; it defines the 13,385 unique deployable applied-action representatives used for calibration, two-ply safe-action counting, admission, and H3 routing. Its NPZ array is `action_representative_transition`; legacy `representative_transition` is retained only as a byte-identical alias. It does not authorize sensor-result reuse.

`EXACT_SENSOR_MATERIALIZATION_MAP` is the new `ExactGeometryMaterializationMap`; it is the only authority for reusing a rendered scan or copying sensor evidence. Two transitions share a geometry representative only when `qpos`, `link_transform`, `geom_transform`, `native_contact`, `exact_contact`, and `frozen_contact_label` are all exactly equal and the boundary snapshot digest is identical. No numerical tolerance is permitted. The frozen corpus contains 13,584 exact sensor-materialization representatives, 15,886 exactly reused transition pairs, and 199 applied-action-copy pairs that require independent materialisation across 81 states: 58 training, 12 internal-calibration, and 11 development-held-out.

Before any new scientific sensor result, preflight must audit both maps over all 176 states, reproduce those totals, prove both maps total and self-bound, prove the legacy action alias exact, and prove every sensor reuse satisfies every exact field plus boundary digest. Every one of the 29,470 frozen transition rows persists both representative identities and binds its sensor evidence to the exact geometry-source transition. Any mismatch fails closed before materialisation.
...
Two-ply admission requires predicted current-tick contact-free status and at least one predicted contact-free next action from the actual successor. Unique deployable applied actions and frozen H3 ordering are determined only by `DECISION_ACTION_COPY_MAP`; their sensor-derived evidence comes only from `EXACT_SENSOR_MATERIALIZATION_MAP`. The development-held-out gate is immutable:
