...
class JointFloorRegisteredSurfaceMemory(ConfirmedAuxiliaryFloorMemory):
    def observe(self, policy, depth, evidence, *, now_ns):
        ...
        try:
            ...
            p, R, pose = current_joint_floor_registered_pose(evidence, identity=self.identity, now_ns=now_ns)
...
class JointFloorRegisteredMap(ConfirmedAuxiliaryFloorMap):
    def __init__(self, *, identity=(0, 0, 0)):
        super().__init__(identity=identity)
        self.surface = JointFloorRegisteredSurfaceMemory(identity=identity)


class JointFloorRegisteredResidual(OnlineExecutedResidual):
    def observe(self, evidence, *, now_ns):
        p, R, pose = current_joint_floor_registered_pose(evidence, identity=(0, 0, 0), now_ns=now_ns)
        frame = pose['frame']
...
class JointFloorRegisteredRoundTripController(ConfirmedFloorRoundTripController):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.registration = JointFloorRegistration(identity=(0, 0, 0))
        self.mapper = JointFloorRegisteredMap(identity=(0, 0, 0))
        self.memory = self.mapper.surface
        self.residual = JointFloorRegisteredResidual()
    ...
    def _result(self, command, selection, distance):
        return super()._result(command, selection, distance) | dict(
            controller='joint_floor_registered_round_trip_controller_v1',
