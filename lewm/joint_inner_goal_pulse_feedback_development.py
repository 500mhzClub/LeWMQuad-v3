"""Separate joint-pose version of the frozen inner-goal feedback algorithm."""
import math
import numpy as np
from lewm.inner_goal_pulse_feedback_development import InnerGoalPulseServo
from lewm.joint_sensor_anchored_goal_development import current_joint_pose, JointAnchoredGoal
from lewm.causal_sensor_state import SensorContractError,_ns
from lewm.bounded_visual_servo_development import wrapped
from lewm.coupled_pulse_rollout_development import PulseTable,compose
from lewm.inner_goal_pulse_rollout_development import plan,INTERNAL_POSITION_TOLERANCE_M


class JointInnerGoalPulseServo(InnerGoalPulseServo):
    ...
    def __init__(self, goal, table):
        if not isinstance(goal, JointAnchoredGoal):
            raise SensorContractError('typed joint sensor goal required')
        super().__init__(goal, table)
    ...
    def step(self,evidence,*,now_ns):
        ...
        try:
            ...
            p,R,_=current_joint_pose(evidence,identity=self.identity,now_ns=now)
            yaw=math.atan2(R[1,0],R[0,0]);increment=wrapped(yaw-self.previous[1])
            if abs(increment)>.20:raise SensorContractError('joint-measured incremental yaw bound exceeded')
            ...
            in_goal=distance<=INTERNAL_POSITION_TOLERANCE_M and abs(error)<=.05;prior_in_goal=self.previous_in_goal
    ...
    def _result(self,now,command,diagnostic):
        return super()._result(now,command,diagnostic) | dict(
            controller='joint_inner_goal_empirical_pulse_feedback_v1',
            pose_mode='joint', gyro_role='consistency_monitor_only')
