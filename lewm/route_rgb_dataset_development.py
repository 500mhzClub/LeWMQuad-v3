"""Policy-only route capture reader; distinct schema from the 46-frame probe."""
import json
...
def load_route_observation(directory,index):
    ...
    if not isinstance(frames,list) or not 1<=len(frames)<=341:
