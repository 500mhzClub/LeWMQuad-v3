"""Distinct corner-feature joint observer; all registration/continuity gates inherited."""
import hashlib
import cv2
import numpy as np
...
class CornerSupportJointRGBDPose(JointTemporalAnchorRGBDPose):
    def observe(self, policy, depth, fast, *, now_ns):
        ...
        try:
            ...
            current = CornerSupportFeatureFrame(policy['image']['rgb'], depth)
            self.frame += 1
...
class CornerSupportVisualLedMotion(JointTemporalAnchorVisualLedMotion):
    def __init__(self, *, identity=(0, 0, 0)):
        super().__init__(identity=identity)
        self.model = CornerSupportJointRGBDPose()

    def snapshot(self, *, now_ns):
        row = super().snapshot(now_ns=now_ns)
        previous = self.model.previous
        return row | dict(feature_selection="corner_upright_sift_support_v1",
