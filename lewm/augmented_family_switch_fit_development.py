...
def verified_plan(view, indices, inputs):
    if not isinstance(view, AugmentedFamilySwitchView):
        raise ValueError('actual complete family view required')
    values, valid = zip(*((remaining_candidate(view.rows[i]['action'], view.rows[i]['offset_ticks']) if view.rows[i]['source']=='family'
        else timed_candidate(view.rows[i]['action'])) for i in indices), strict=True)
...
def score(view, arrays, *, role, head):
    ...
    for row, index in enumerate(indices):
        ...
        for h, target in enumerate(window['targets']):
            ...
            for scope in ('all', window['stratum'], *(['first_half_second',window['stratum']+'_first_half_second'] if h==0 else [])):
