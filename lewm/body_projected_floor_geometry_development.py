"""Original mapping geometry with an explicit observation-local body projection.

All depth, pose, floor, coverage, prefix and classification logic remains in its
original order. Only repeated optical/body projection expressions are routed.
"""
...
from lewm.current_primary_floor_plane_development import ROWS, COLUMNS
from lewm.frame_cached_floor_geometry_development import FloorFrameGeometry
from lewm.frame_body_projection_cache_development import FrameBodyProjectionCache

T = np.asarray(BODY_FROM_OPTICAL)


class BodyProjectedFloorGeometry(FloorFrameGeometry):
    def __init__(self):
        super().__init__()
        self.body_projection_cache = FrameBodyProjectionCache()

    def body_projection(self, depth):
        return self.body_projection_cache.body(depth)

    def close(self):
        try:
            self.body_projection_cache.close()
        finally:
            super().close()
    ...
    def floor_coverage(self, depth, valid, map_from_body, translation_map, floor_height, cells=GRID):
        ...
        body = self.body_projection(depth)
    ...
    def primary_floor_plane(self, depth, valid, rotation_map_from_body, position_map, floor_height):
        ...
        points = self.measured_points(depth, rotation_map_from_body, position_map)
    ...
    def confirm_auxiliary_floor(self, depth, valid, rotation_map_from_reference, position_map,
        ...
        points = self.measured_points(depth, R, position_map)
    ...
    def measured_points(self, depth, rotation_map_from_reference, position_map):
        ...
        T = np.asarray(BODY_FROM_OPTICAL)
        return self.body_projection(depth)@R.T+p
