"""Parameterized pulse execution of a sensor-anchored goal; no estimator reset."""
import math
import numpy as np
...
RULES=dict(target='sensor-anchored per-leg goal',approach_tolerance_m=.06,
    ...
    maximum_observed_translation_from_anchor_m=1.,maximum_target_direction_overshoot_m=.08,
    minimum_forward_response_m=.005)


class AnchoredPulseServo:
    def __init__(self,goal):
        if not isinstance(goal,AnchoredGoal): raise SensorContractError("typed sensor-anchored goal required")
        self.goal=goal
        identity=goal.identity
        self.identity=_identity(identity);self.stage='brake';self.task_stage='approach'
        self.last_ns=None;self.start_ns=goal.anchor_ns;self.previous=None;self.previous_in_goal=False
    ...
    def step(self,evidence,*,now_ns):
        ...
        try:
            ...
            yaw=math.atan2(R[1,0],R[0,0]);delta=np.array(self.goal.target_xy)-p[:2]
            distance=float(np.linalg.norm(delta));yaw_error=wrapped(self.goal.target_yaw_rad-yaw)
            speed=rate=None
            ...
            if self.terminal is None and self.stage=='choose':
                self.task_stage='orient' if distance<=.06 else 'approach'
    ...
    def _excursion(self,p):
        offset=p-np.asarray(self.goal.anchor_position)
        target=np.asarray(self.goal.target_xy)-np.asarray(self.goal.anchor_position)[:2]
        length=np.linalg.norm(target)
        return (np.linalg.norm(offset)>1. or (length>1e-12 and offset[:2]@target/length>length+.08))
    ...
    def _result(self,now,command,diagnostic):
        ...
        return dict(decision_ns=now,stage=self.stage,task_stage=self.task_stage,phase=phase,
            ...
            goal=self.goal.snapshot(),native_pose_used=False,learned_predictor_used=False,terrain_safety_qualified=False,navigation_qualified=False)
