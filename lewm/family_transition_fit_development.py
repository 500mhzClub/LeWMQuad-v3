"""Fresh family-plan fitting and policy-only inference; no experiment authority."""
...
from lewm.independent_pulse_input_ablation_development import transform_inputs, transform_training_batch
...
def verified_plan(view, indices, inputs):
    if not isinstance(view, FamilyWindowView):
        raise ValueError('actual complete family view required')
    values, valid = zip(*(remaining_candidate(view.windows[i]['action'], view.windows[i]['offset_ticks'])
        for i in indices), strict=True)
    ...
    return validate_timed_plan(inputs['known_action_blocks'], inputs['known_action_valid'], len(indices))


def train(trainer, stream, schedule, *, input_variant, on_update, benchmark=False):
    if (not isinstance(trainer, CumulativePulseTrainer) or trainer.failed or trainer.updates != 0
    ...
    try:
        expected = stream.view.schedule(updates=1200, batch_size=6, seed=2026091001)
...
def predict(trainer, stream, *, role, input_variant):
    if not isinstance(trainer, CumulativePulseTrainer) or trainer.failed or trainer.updates != 1200:
        raise ValueError('complete nonfailed final family fit required')
    indices = stream.view.indices(role)
    if len(indices) != (336 if role == 'train' else 348 if role == 'geometry_transfer' else -1):
        raise ValueError('complete exact actual role population required')
...
def score(view, arrays, *, role, head):
    ...
    for row, index in enumerate(indices):
        ...
        for h, target in enumerate(window['targets']):
            ...
            for scope in ('all', 'initial' if window['offset_ticks'] == 0 else 'moving'):
                cell = cells.setdefault((scope, window['cluster']), dict(position=[], yaw=[], brier=[],
    ...
    rows = []
    for (scope, cluster), c in sorted(cells.items()):
        rows.append(dict(scope=scope, cluster=cluster, **{k: c[k] for k in ('motion_targets', 'contact_targets', 'contact_positives', 'undefined_yaw')},
    ...
    return dict(role=role, primary_head=head, windows=len(indices), clusters=rows,
