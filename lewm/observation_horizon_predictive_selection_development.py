...
def score_candidates(prediction,*,goal_body_xy_m,contact_penalty_m):
    ...
    All six complete 0.8-second plans participate. Stable ties follow ACTIONS
    ...
    return dict(action=ACTIONS[chosen],action_index=chosen,
        ...
        prediction_horizon_ns=800_000_000,contact_probability_calibrated=False,
...
def candidate_inputs(observation_history):
    ...
    blocks,valid=zip(*(plan(a) for a in ACTIONS),strict=True)
...
def select(model,observation_history,*,head,input_variant,goal_body_xy_m,contact_penalty_m):
    ...
    """
    if not isinstance(model,ObservationHorizonRGBBodyJEPA) or model.training:
...
def require_short_forecast(selection):
    if (selection.get('first_prediction_horizon_ns')!=100_000_000
            or selection.get('prediction_horizon_ns')!=800_000_000
            or selection.get('target_offsets_ns')!=list(range(100_000_000,800_000_001,100_000_000))):
        raise ValueError('exact actual 100-ms forecast bank required')
