"""Non-predictive action rule over the same persistent observed floor route.
...
class ReactiveObservedRouteSelector:
    ...
    def choose(self, mapper, geometry, *, now_ns):
        ...
        if target is not None and np.linalg.norm(goal_body) > LOCAL_TARGET_RADIUS_M:
            ...
            elif connector['nominal_disk_connector_clear'] and not unknown:
                action = 'forward'
        else:
        ...
        return dict(action=action, requested_command=[0., 0., 0.] if action is None else candidate_commands(action)[0],
            ...
            current_geometry_checked=True, learned_model_used=False, candidate_future_outcomes_evaluated=False,
            ...
            rule='turn_to_observed_waypoint_then_forward_on_measured_clear_connector')
