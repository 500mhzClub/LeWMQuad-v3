"""Explicit fallback using the already scored causal 100ms position correction.

Original forecasts and all original veto receipts remain unchanged. Only the
first future XY point is corrected for a separate geometric evaluation. This
is a nominal-policy change, not a calibrated model-error or clearance bound.
"""
...
def causal_correction(receipt, *, now_ns):
    frame=receipt['frame']; rows=receipt['residuals']; count=receipt['observed_residual_samples']
    if (type(frame) is not int or frame<0 or type(now_ns) is not int
            or now_ns!=1_500_000_000+frame*100_000_000 or receipt['measured_ns']!=now_ns
            or receipt['window_ticks']!=WINDOW_TICKS or type(count) is not int
            or not 0<=count<=WINDOW_TICKS or len(rows)!=count
            or receipt['pending_forecast_tick'] is not None
            or any(receipt[k] is not False for k in (
                'native_outcomes_used','command_integrated_pose_used','model_weights_changed'))):
        raise ValueError('current pre-command public residual receipt required')
    ticks=[]; available=[]; residuals=[]
    for row in rows:
        tick=row['tick']; when=row['available_tick']
        if (type(tick) is not int or type(when) is not int or when!=tick+1
                or not max(0,frame-WINDOW_TICKS)<=tick<frame or when>frame
                or row['measured_ns']!=1_500_000_000+when*100_000_000
                or (ticks and tick<=ticks[-1])):
            raise ValueError('distinct ordered already observed one-step residuals required')
        predicted=np.asarray(row['predicted_body_xy_m'],float)
        observed=np.asarray(row['observed_body_xy_m'],float)
        residual=np.asarray(row['residual_xy_m'],float)
        if (any(x.shape!=(2,) or not np.isfinite(x).all() for x in (predicted,observed,residual))
                or not np.array_equal(predicted-observed,residual)):
            raise ValueError('original prediction minus measured public displacement required')
        ticks.append(tick);available.append(when);residuals.append(residual)
    bias=np.mean(residuals,axis=0) if rows else np.zeros(2)
    if (receipt['residual_source_ticks']!=ticks or receipt['residual_available_ticks']!=available
            or not np.array_equal(np.asarray(receipt['correction_xy_m'],float),bias)):
        raise ValueError('unchanged preceding-window correction arithmetic required')
    return bias


def correct_first_interval_feasibility(selection, receipt, mapper, geometry, *, now_ns):
    if (selection.get('action') is not None or 'prediction' not in selection
    ...
    result.update(action=None if chosen is None else ACTIONS[chosen],action_index=chosen,
        ...
        residual_first_interval_feasibility=dict(frame=receipt['frame'],measured_ns=now_ns,
