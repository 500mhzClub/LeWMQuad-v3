"""Keep the original first batch, then amortize longer historical searches."""
import numpy as np
...
def batch_ranges(count):
    if count:
        yield 0,min(FRAME_BATCH,count)
    for start in range(FRAME_BATCH,count,128):
        yield start,min(start+128,count)


class ProgressiveBatchedRetainedFloorPatches(BatchedRetainedFloorPatches):
    def coverage(self, centres, radius=.022):
        ...
        found = [None]*len(xy)
        for start, end in batch_ranges(len(self.frames)):
