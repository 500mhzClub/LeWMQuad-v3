"""Prospective constant-anchor correction; later-horizon calibration is unproven.

Retain the frozen first-point recovery before considering this distinct nominal
policy exception. Raw predictions remain the targets for future residuals.
"""
...
def _reconsider_anchored_hold(selection, receipt, mapper, geometry, *, now_ns):
    ...
    first=prediction[:,0,:2]-bias
    corrected[:,0,:2]=first
    corrected[:,1:,:2]=first[:,None,:]+(prediction[:,1:,:2]-prediction[:,:1,:2])
    ...
    result.update(action=None if chosen is None else ACTIONS[chosen],action_index=chosen,
        ...
        residual_anchored_continuation=dict(frame=receipt['frame'],measured_ns=now_ns,
            ...
            correction_horizon_ns=800_000_000,residual_measurement_horizon_ns=100_000_000,
            corrected_body_xy_m=corrected[:,:,:2].tolist(),
            anchoring_equation='corrected_first_xy + (raw_future_xy - raw_first_xy)',
            later_prediction_correction_calibrated=False,all_eight_segments_checked=True,
            later_predicted_points_unchanged=False,yaw_and_contact_forecasts_unchanged=True,
...
def reconsider_anchored_continuation(selection, receipt, mapper, geometry, *, now_ns):
    first = reconsider_hold_feasibility(selection, receipt, mapper, geometry, now_ns=now_ns)
    if first is not selection:
        return first
    return _reconsider_anchored_hold(selection, receipt, mapper, geometry, now_ns=now_ns)
