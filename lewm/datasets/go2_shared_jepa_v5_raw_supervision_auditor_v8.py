"""Standalone V8 auditor for the Shared-JEPA V5 raw-supervision dataset.
...
one fixed exact entry only after dual V8 source-review authorization.
"""
...
CANONICAL_AUDIT_REPORT = CANONICAL_DATASET.with_name(
    CANONICAL_DATASET.name + ".audit_v8.json"
)
CANONICAL_AUDIT_FAILURE = CANONICAL_DATASET.with_name(
    CANONICAL_DATASET.name + ".audit_v8.failed.json"
)
...
AUDIT_SCHEMA = "lewm_go2_shared_jepa_v5_raw_supervision_audit_v8"
AUDIT_FAILURE_SCHEMA = "lewm_go2_shared_jepa_v5_raw_supervision_audit_failure_v8"
...
# Builder V8 records this complete closure in the dataset provenance.
FROZEN_PARENT_FILE_SHA256 = dict(FROZEN_V8_PREDECESSOR_SHA256)
...
class ReviewBindingV8:
    ...
    file_sha256: str
    content_sha256: str
    candidate: tuple[SourceBindingV8, ...]


@dataclass(frozen=True)
class PhaseOneAuthorizationV8:
    authorization_file_sha256: str
    authorization_content_sha256: str
    source_map_sha256: str
    canonical_payload: bytes
    sources: tuple[SourceBindingV8, ...]
    builder_review: ReviewBindingV8
    auditor_review: ReviewBindingV8


@dataclass(frozen=True)
class AcceptedAuthorizationV8:
    authorization_file_sha256: str
    authorization_content_sha256: str
    source_map_sha256: str
    sources: tuple[SourceBindingV8, ...]
...
def _read_fixed_manifest_payload() -> tuple[bytes, str]:
    """Descriptor-read the fixed manifest after V8 authority acceptance."""
...
class _FailureReceiptPublisher:
    ...
    def publish(self, value: Mapping[str, Any]) -> None:
        ...
        if (
            ...
            raise PermissionError("only a terminal V8 failure receipt may publish")
...
@dataclass(frozen=True)
class _AuditPublicationContextV8:
    authorization: AcceptedAuthorizationV8
...
def _audit_transaction_bindings(
    context: _AuditPublicationContextV8,
) -> tuple[dict[Path, tuple[str, int | None]], set[Path]]:
...
class _ClosedAuditPublicationTransaction:
    ...
    def __init__(
        ...
        *,
        ...
        try:
            ...
            # Watching each canonical ancestor closes the V8 builder's
    ...
    def _validate_report_inventory_and_destination(
        ...
        try:
            ...
            named_path_before = self._candidate_path(after_rename=after_rename).stat(
                follow_symlinks=False
            )
            ...
            named_path_after = self._candidate_path(after_rename=after_rename).stat(
                follow_symlinks=False
            )
        except OSError:
...
def _source_binding(entry: object) -> SourceBindingV8:
...
def _candidate_bindings(
    ...
    *,
    roles: Sequence[str],
    source_by_role: Mapping[str, SourceBindingV8],
) -> tuple[SourceBindingV8, ...]:
...
def _validate_authorization_phase_one(
    ...
    *,
    authorization_file_sha256: str,
) -> PhaseOneAuthorizationV8:
    """Validate complete V8 authority without opening a mapped target."""
    ...
    if any(
        ...
        for role, digest in FROZEN_BUILDER_V8_ROLE_SHA256.items()
    ):
        raise PermissionError(
            "authorization does not bind the frozen Builder V8 candidate"
        )
    ...
    return PhaseOneAuthorizationV8(
...
def _review_candidate_value(
    candidate: Sequence[SourceBindingV8],
) -> list[dict[str, str]]:
    return [
...
def _validate_review_record(raw: bytes, binding: ReviewBindingV8) -> None:
...
def _validate_authorization_phase_two(
    phase_one: PhaseOneAuthorizationV8,
) -> AcceptedAuthorizationV8:
    """Rehash only the fixed V5 closure after structural acceptance."""

    if type(phase_one) is not PhaseOneAuthorizationV8:
        raise TypeError("phase two requires a completed V8 phase-one capsule")
    ...
    for relative, digest in {
        ...
        **REVIEWED_V4_SOURCE_SHA256,
        ...
        _read_absolute_bound_payload(
            ...
            digest,
            repository_root=ROOT,
            name=f"frozen V8 authority source {relative}",
        )
    return AcceptedAuthorizationV8(
        authorization_file_sha256=phase_one.authorization_file_sha256,
...
def _prepare_authorized_audit_v8(
    *,
    authorization_sha256: str,
    workers: int,
) -> _PreparedAuditV8:
    ...
    core["closed_publication_transaction_v8"] = {
...
def _final_revalidate_authorized_audit_v8(
    prepared: _PreparedAuditV8,
    *,
    authorization_sha256: str,
    workers: int,
) -> None:
    ...
    if type(prepared) is not _PreparedAuditV8:
        raise TypeError("final audit pass requires a prepared V8 audit")
    _require_exact_authority(authorization_sha256)
    ...
    try:
        for name in (CANONICAL_AUDIT_REPORT.name, CANONICAL_AUDIT_FAILURE.name):
            if _lstat_optional_at(retained.directory_fd, name) is not None:
                raise FileExistsError(f"immutable audit leaf already exists: {name}")
        prepared = _prepare_authorized_audit_v8(
            authorization_sha256=authorization_sha256,
            workers=worker_count,
        )
        (
