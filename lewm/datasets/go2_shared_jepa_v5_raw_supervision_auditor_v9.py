"""Standalone V9 auditor for the Shared-JEPA V5 raw-supervision dataset.
...
one fixed exact entry only after dual V9 source-review authorization.
"""
...
CANONICAL_AUDIT_FAILURE = CANONICAL_DATASET.with_name(
    CANONICAL_DATASET.name + ".audit_v9.failed.json"
)
...
AUDIT_SCHEMA = "lewm_go2_shared_jepa_v5_raw_supervision_audit_v9"
AUDIT_FAILURE_SCHEMA = "lewm_go2_shared_jepa_v5_raw_supervision_audit_failure_v9"
...
BUILDER_IMPLEMENTATION_AUTHOR = "/root/raw_v7_successor_author/auditor_v7_author"
AUDITOR_IMPLEMENTATION_AUTHOR = "/root/camera_v5_independent/camera_v7_pre_freeze_review/v7_review_artifact_schema"
MAX_WORKERS = 6
FROZEN_BUILDER_V9_ROLE_SHA256 = {
    "builder_source": (
...
class ReviewBindingV9:
    ...
    file_sha256: str
    content_sha256: str
    candidate: tuple[SourceBindingV9, ...]


@dataclass(frozen=True)
class PhaseOneAuthorizationV9:
    authorization_file_sha256: str
    authorization_content_sha256: str
    source_map_sha256: str
    canonical_payload: bytes
    sources: tuple[SourceBindingV9, ...]
    builder_review: ReviewBindingV9
    auditor_review: ReviewBindingV9


@dataclass(frozen=True)
class AcceptedAuthorizationV9:
    authorization_file_sha256: str
    authorization_content_sha256: str
    source_map_sha256: str
    sources: tuple[SourceBindingV9, ...]
...
def _read_fixed_manifest_payload() -> tuple[bytes, str]:
    """Descriptor-read the fixed manifest after V9 authority acceptance."""
...
class _FailureReceiptPublisher:
    ...
    def publish(self, value: Mapping[str, Any]) -> None:
        ...
        if (
            ...
            raise PermissionError("only a terminal V9 failure receipt may publish")
...
@dataclass(frozen=True)
class _AuditPublicationContextV9:
    authorization: AcceptedAuthorizationV9
...
def _audit_transaction_bindings(
    context: _AuditPublicationContextV9,
) -> tuple[dict[Path, tuple[str, int | None]], set[Path]]:
...
def _source_binding(entry: object) -> SourceBindingV9:
...
def _candidate_bindings(
    ...
    *,
    roles: Sequence[str],
    source_by_role: Mapping[str, SourceBindingV9],
) -> tuple[SourceBindingV9, ...]:
...
def _validate_authorization_phase_one(
    ...
    *,
    authorization_file_sha256: str,
) -> PhaseOneAuthorizationV9:
    """Validate complete V9 authority without opening a mapped target."""
    ...
    if any(
        ...
        for role, digest in FROZEN_BUILDER_V9_ROLE_SHA256.items()
    ):
        raise PermissionError(
            "authorization does not bind the frozen Builder V9 candidate"
        )
    ...
    return PhaseOneAuthorizationV9(
...
def _review_candidate_value(
    candidate: Sequence[SourceBindingV9],
) -> list[dict[str, str]]:
    return [
...
def _validate_review_record(raw: bytes, binding: ReviewBindingV9) -> None:
...
def _validate_authorization_phase_two(
    phase_one: PhaseOneAuthorizationV9,
) -> AcceptedAuthorizationV9:
    """Rehash only the fixed V5 closure after structural acceptance."""

    if type(phase_one) is not PhaseOneAuthorizationV9:
        raise TypeError("phase two requires a completed V9 phase-one capsule")
    ...
    return AcceptedAuthorizationV9(
        authorization_file_sha256=phase_one.authorization_file_sha256,
...
def _prepare_authorized_audit_v9(
    *,
    authorization_sha256: str,
    workers: int,
) -> _PreparedAuditV9:
    ...
    core["closed_publication_transaction_v9"] = {
...
def _final_revalidate_authorized_audit_v9(
    prepared: _PreparedAuditV9,
    *,
    authorization_sha256: str,
    workers: int,
) -> None:
    ...
    if type(prepared) is not _PreparedAuditV9:
        raise TypeError("final audit pass requires a prepared V9 audit")
    _require_exact_authority(authorization_sha256)
    ...
    try:
        for name in (CANONICAL_AUDIT_REPORT.name, CANONICAL_AUDIT_FAILURE.name):
            if _lstat_optional_at(retained.directory_fd, name) is not None:
                raise FileExistsError(f"immutable audit leaf already exists: {name}")
        prepared = _prepare_authorized_audit_v9(
            authorization_sha256=authorization_sha256,
            workers=worker_count,
        )
        (
