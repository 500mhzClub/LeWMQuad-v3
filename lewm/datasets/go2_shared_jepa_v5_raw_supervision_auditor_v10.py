"""Standalone V10 auditor for the Shared-JEPA V5 raw-supervision dataset.
...
one fixed exact entry only after a separate V10 audit authorization.
"""
...
AUTHORIZED_SUCCESS_REPORT_PATH = AUTHORIZED_DATASET_PATH + ".audit_v10.json"
AUTHORIZED_FAILURE_REPORT_PATH = AUTHORIZED_DATASET_PATH + ".audit_v10.failed.json"
...
AUDIT_SCHEMA = "lewm_go2_shared_jepa_v5_raw_supervision_audit_v10"
AUDIT_FAILURE_SCHEMA = "lewm_go2_shared_jepa_v5_raw_supervision_audit_failure_v10"
...
class ReviewBindingV10:
    ...
    file_sha256: str
    content_sha256: str
    candidate: tuple[SourceBindingV10, ...]


@dataclass(frozen=True)
class PhaseOneAuthorizationV10:
    authorization_file_sha256: str
    authorization_content_sha256: str
    source_map_sha256: str
    canonical_payload: bytes
    sources: tuple[SourceBindingV10, ...]
    auditor_review: ReviewBindingV10


@dataclass(frozen=True)
class AcceptedAuthorizationV10:
    # These three values describe the immutable V9 build provenance recorded
    # in the dataset manifest. V10 execution authority is kept separate below.
    authorization_file_sha256: str
    authorization_content_sha256: str
    source_map_sha256: str
...
def _read_fixed_manifest_payload() -> tuple[bytes, str]:
    """Descriptor-read the fixed manifest after V10 authority acceptance."""
...
class _FailureReceiptPublisher:
    ...
    def publish(self, value: Mapping[str, Any]) -> None:
        ...
        if (
            ...
            raise PermissionError("only a terminal V10 failure receipt may publish")
...
@dataclass(frozen=True)
class _AuditPublicationContextV10:
    authorization: AcceptedAuthorizationV10
...
def _audit_transaction_bindings(
    context: _AuditPublicationContextV10,
) -> tuple[dict[Path, tuple[str, int | None]], set[Path]]:
...
def _source_binding(entry: object) -> SourceBindingV10:
...
def _candidate_bindings(
    ...
    *,
    roles: Sequence[str],
    source_by_role: Mapping[str, SourceBindingV10],
) -> tuple[SourceBindingV10, ...]:
...
def _validate_authorization_phase_one(
    ...
    *,
    ...
    """Validate complete V10 authority without opening a mapped target."""
    ...
    if (
        ...
        or payload["exact_audit_v10_authorized"] is not True
        or any(payload[field] is not False for field in AUTHORIZATION_FALSE_FIELDS)
    ):
        raise PermissionError("raw-supervision exact V10 audit is not authorized")
    if (
        ...
        raise PermissionError("V10 input/output or terminal V9 binding changed")
    core = dict(payload)
    ...
    if any(
        ...
        raise PermissionError(
            "authorization does not bind the frozen V9/V10 predecessor closure"
        )
...
def _review_candidate_value(
    candidate: Sequence[SourceBindingV10],
) -> list[dict[str, str]]:
    return [
...
def _validate_review_record(raw: bytes, binding: ReviewBindingV10) -> None:
...
def _validate_authorization_phase_two(
    phase_one: PhaseOneAuthorizationV10,
) -> AcceptedAuthorizationV10:
    """Rehash only the fixed V10 closure after structural acceptance."""

    if type(phase_one) is not PhaseOneAuthorizationV10:
        raise TypeError("phase two requires a completed V10 phase-one capsule")
    ...
    return AcceptedAuthorizationV10(
...
def _require_exact_authority(
    ...
    if not AUDIT_AUTHORIZATION_PATH.is_file():
        raise PermissionError(
            "reviewed raw-supervision V10 audit authorization is absent"
        )
    raw = _read_absolute_bound_payload(
        AUDIT_AUTHORIZATION_PATH,
        authorization_sha256,
        repository_root=ROOT,
        name="V10 audit authorization",
    )
    payload = _strict_canonical_json_object(raw, name="V10 audit authorization")
...
def _prepare_authorized_audit_v10(
    *,
    authorization_sha256: str,
    workers: int,
) -> _PreparedAuditV10:
    ...
    core["closed_publication_transaction_v10"] = {
...
def _final_revalidate_authorized_audit_v10(
    prepared: _PreparedAuditV10,
    *,
    authorization_sha256: str,
    workers: int,
) -> None:
    ...
    if type(prepared) is not _PreparedAuditV10:
        raise TypeError("final audit pass requires a prepared V10 audit")
    _require_exact_authority(authorization_sha256)
    ...
    try:
        for name in (CANONICAL_AUDIT_REPORT.name, CANONICAL_AUDIT_FAILURE.name):
            if _lstat_optional_at(retained.directory_fd, name) is not None:
                raise FileExistsError(f"immutable audit leaf already exists: {name}")
        prepared = _prepare_authorized_audit_v10(
            authorization_sha256=authorization_sha256,
            workers=worker_count,
        )
        (
