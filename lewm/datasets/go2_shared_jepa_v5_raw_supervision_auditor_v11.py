"""Standalone V11 auditor for the Shared-JEPA V5 raw-supervision dataset.
...
one fixed exact entry only after a separate V11 audit authorization.
"""
...
CANONICAL_AUDIT_FAILURE = CANONICAL_DATASET.with_name(
    CANONICAL_DATASET.name + ".audit_v11.failed.json"
)
AUTHORIZED_DATASET_PATH = (
    ".generated/go2_shared_observable_camera_ray_jepa_v5/"
    "development_raw_supervision_v1"
)
AUTHORIZED_SUCCESS_REPORT_PATH = AUTHORIZED_DATASET_PATH + ".audit_v11.json"
AUTHORIZED_FAILURE_REPORT_PATH = AUTHORIZED_DATASET_PATH + ".audit_v11.failed.json"
...
AUDIT_SCHEMA = "lewm_go2_shared_jepa_v5_raw_supervision_audit_v11"
AUDIT_FAILURE_SCHEMA = "lewm_go2_shared_jepa_v5_raw_supervision_audit_failure_v11"
AUTHORIZATION_SCHEMA = (
    "lewm_go2_shared_jepa_v5_raw_supervision_audit_authorization_v11"
)
REVIEW_BINDING_SCHEMA = (
    "lewm_go2_shared_jepa_v5_raw_supervision_implementation_review_binding_v11"
)
AUDITOR_REVIEW_SCHEMA = (
    "lewm_go2_shared_jepa_v5_raw_supervision_auditor_v11_independent_review_v1"
)
V11_IMPLEMENTATION_AUTHOR = "/root/raw_v11_builder_auditor_diff"
...
AUDIT_AUTHORIZATION_PATH = (
    ROOT
    / "docs/lewm_go2_shared_jepa_v5_raw_supervision_audit_v11_"
...
class ReviewBindingV11:
    ...
    file_sha256: str
    content_sha256: str
    candidate: tuple[SourceBindingV11, ...]


@dataclass(frozen=True)
class PhaseOneAuthorizationV11:
    authorization_file_sha256: str
    authorization_content_sha256: str
    source_map_sha256: str
    canonical_payload: bytes
    sources: tuple[SourceBindingV11, ...]
    auditor_review: ReviewBindingV11


@dataclass(frozen=True)
class AcceptedAuthorizationV11:
    # These three values describe the immutable V9 build provenance recorded
    # in the dataset manifest. V11 execution authority is kept separate below.
    authorization_file_sha256: str
    authorization_content_sha256: str
    source_map_sha256: str
...
def _read_fixed_manifest_payload() -> tuple[bytes, str]:
    """Descriptor-read the fixed manifest after V11 authority acceptance."""
...
class _FailureReceiptPublisher:
    ...
    def publish(self, value: Mapping[str, Any]) -> None:
        ...
        if (
            ...
            raise PermissionError("only a terminal V11 failure receipt may publish")
...
@dataclass(frozen=True)
class _AuditPublicationContextV11:
    authorization: AcceptedAuthorizationV11
...
def _audit_transaction_bindings(
    context: _AuditPublicationContextV11,
) -> tuple[dict[Path, tuple[str, int | None]], set[Path]]:
...
def _source_binding(entry: object) -> SourceBindingV11:
...
def _candidate_bindings(
    ...
    *,
    roles: Sequence[str],
    source_by_role: Mapping[str, SourceBindingV11],
) -> tuple[SourceBindingV11, ...]:
...
def _validate_authorization_phase_one(
    ...
    *,
    ...
    """Validate complete V11 authority without opening a mapped target."""
    ...
    if (
        ...
        or payload["exact_audit_v11_authorized"] is not True
        or any(payload[field] is not False for field in AUTHORIZATION_FALSE_FIELDS)
    ):
        raise PermissionError("raw-supervision exact V11 audit is not authorized")
    if (
        ...
        raise PermissionError(
            "V11 input/output or predecessor terminal binding changed"
        )
    core = dict(payload)
    ...
    if any(
        ...
        raise PermissionError(
            "authorization does not bind the frozen V9/V10/V11 closure"
        )
...
def _review_candidate_value(
    candidate: Sequence[SourceBindingV11],
) -> list[dict[str, str]]:
    return [
...
def _validate_review_record(raw: bytes, binding: ReviewBindingV11) -> None:
...
def _require_v10_success_absent() -> None:
    try:
        os.lstat(CANONICAL_V10_AUDIT_REPORT)
    except FileNotFoundError:
        return
    raise PermissionError("a forbidden V10 audit success report exists")
...
def _validate_frozen_v10_authority_artifacts(
    ...
    if tuple(item.role for item in v10_sources) != tuple(
        role for role, _mapped in v10_to_v11_role
    ):
        raise PermissionError("frozen V10 source-map role order changed")
    for source, (_role, mapped_role) in zip(v10_sources, v10_to_v11_role):
        if (
...
def _validate_authorization_phase_two(
    phase_one: PhaseOneAuthorizationV11,
) -> AcceptedAuthorizationV11:
    """Rehash only the fixed V11 closure after structural acceptance."""

    if type(phase_one) is not PhaseOneAuthorizationV11:
        raise TypeError("phase two requires a completed V11 phase-one capsule")
    ...
    return AcceptedAuthorizationV11(
...
def _require_exact_authority(
    ...
    if not AUDIT_AUTHORIZATION_PATH.is_file():
        raise PermissionError(
            "reviewed raw-supervision V11 audit authorization is absent"
        )
    raw = _read_absolute_bound_payload(
        AUDIT_AUTHORIZATION_PATH,
        authorization_sha256,
        repository_root=ROOT,
        name="V11 audit authorization",
    )
    payload = _strict_canonical_json_object(raw, name="V11 audit authorization")
...
def _prepare_authorized_audit_v11(
    *,
    authorization_sha256: str,
    workers: int,
) -> _PreparedAuditV11:
    ...
    core["closed_publication_transaction_v11"] = {
...
def _final_revalidate_authorized_audit_v11(
    prepared: _PreparedAuditV11,
    *,
    authorization_sha256: str,
    workers: int,
) -> None:
    ...
    if type(prepared) is not _PreparedAuditV11:
        raise TypeError("final audit pass requires a prepared V11 audit")
    _require_exact_authority(authorization_sha256)
    ...
    try:
        for name in (CANONICAL_AUDIT_REPORT.name, CANONICAL_AUDIT_FAILURE.name):
            if _lstat_optional_at(retained.directory_fd, name) is not None:
                raise FileExistsError(f"immutable audit leaf already exists: {name}")
        prepared = _prepare_authorized_audit_v11(
            authorization_sha256=authorization_sha256,
            workers=worker_count,
        )
        (
