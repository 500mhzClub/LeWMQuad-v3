"""Standalone V4 auditor for the Shared-JEPA V5 raw-supervision dataset.
...
one fixed exact entry only after dual V4 source-review authorization.
"""
...
CANONICAL_AUDIT_REPORT = CANONICAL_DATASET.with_name(
    CANONICAL_DATASET.name + ".audit_v4.json"
)
CANONICAL_AUDIT_FAILURE = CANONICAL_DATASET.with_name(
    CANONICAL_DATASET.name + ".audit_v4.failed.json"
)
...
AUDIT_SCHEMA = "lewm_go2_shared_jepa_v5_raw_supervision_audit_v4"
AUDIT_FAILURE_SCHEMA = "lewm_go2_shared_jepa_v5_raw_supervision_audit_failure_v4"
...
def _read_fixed_manifest_payload() -> tuple[bytes, str]:
    """Descriptor-read the fixed manifest after V4 authority acceptance."""
...
def _validate_exact_input_provenance(
    ...
    *,
    ...
    authorization: AcceptedAuthorizationV4,
) -> None:
...
def _validate_authorization_phase_one(
    ...
    *,
    ...
    """Validate complete V4 authority without opening a mapped target."""
...
def _validate_authorization_phase_two(
    ...
    """Rehash only the fixed V4 closure after structural acceptance."""
    ...
    for relative, digest in {
        ...
        **REVIEWED_V4_SOURCE_SHA256,
        ...
        _read_absolute_bound_payload(
            ...
            digest,
            repository_root=ROOT,
            name=f"frozen V4 authority source {relative}",
        )
...
def _run_authorized_audit_v4(
    *,
    authorization_sha256: str,
    workers: int,
) -> dict[str, Any]:
    """Run the fixed owned audit after full V4 authority acceptance."""
...
def execute_exact_audit_v4(
    *,
    ...
    with _ExclusiveAuditPublisher(CANONICAL_AUDIT_REPORT.parent) as publisher:
        ...
        try:
            with _open_retained_directory_chain(CANONICAL_DATASET) as dataset:
                result = _run_authorized_audit_v4(
                    authorization_sha256=authorization_sha256,
                    workers=worker_count,
                )
                dataset.validate()
