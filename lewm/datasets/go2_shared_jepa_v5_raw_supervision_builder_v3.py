"""Standalone V3 raw-supervision builder for Shared-JEPA V5 development.

The reviewed V1 construction engine is owned directly by this module. Exact
mode uses a closed two-phase V3 authority, fixed worker functions, and native
...
AUTHORIZATION_SCHEMA = "lewm_go2_shared_jepa_v5_raw_supervision_build_authorization_v3"
...
FROZEN_PARENT_HASHES = {
    ...
    str(V3_SUCCESSOR_AMENDMENT_PATH.relative_to(ROOT)): (
...
@dataclass(frozen=True)
class ExactPrepublicationContextV3:
...
@dataclass(frozen=True)
class PreparedSceneJobV3:
    scene_id: str
    role: str
    family: str
    endpoints: tuple[PreparedEndpointV3, ...]
...
def _validate_jobs_and_pairs(
    jobs: Sequence[PreparedSceneJobV3],
    pairs: Sequence[Mapping[str, Any]],
) -> tuple[tuple[PreparedSceneJobV3, ...], tuple[Mapping[str, Any], ...]]:
    ...
    endpoints: dict[str, PreparedEndpointV3] = {}
...
def _build_exact_prepared_dataset_v3(
    jobs: Sequence[PreparedSceneJobV3],
    pairs: Sequence[Mapping[str, Any]],
    *,
    workers: int,
    ...
    prepublication_context: ExactPrepublicationContextV3,
) -> dict[str, Any]:
    """Build the one authorized exact artifact from prepared frames."""

    workers = _strict_workers(workers)
    if (
        type(prepublication_context) is not ExactPrepublicationContextV3
...
def _load_exact_scene_job(
    ...
    prepared: list[PreparedEndpointV3] = []
...
def _revalidate_exact_before_publication(
    context: ExactPrepublicationContextV3,
) -> None:
    if type(context) is not ExactPrepublicationContextV3:
        raise RawSupervisionBuildError("exact prepublication context changed")
...
def _source_binding(entry: object) -> SourceBindingV3:
    ...
    if not _is_sha256(digest):
        raise RawSupervisionBuildError("authorization source entry is malformed")
    return SourceBindingV3(role=role, path=path, sha256=digest)
...
def _validate_authorization_phase_one(
    ...
    *,
    ...
    """Validate the complete V3 authority without opening a target file."""
    ...
    if (
        ...
        raise AssertionError("V3 authorization policy is not nine unique rows")
...
def _validate_review_record(raw: bytes, binding: ReviewBindingV3) -> None:
...
def _validate_authorization_phase_two(
...
) -> AcceptedAuthorizationV3:
    """Rehash the fixed V3 closure after a complete structural phase."""

    if type(phase_one) is not PhaseOneAuthorizationV3:
        raise TypeError("phase two requires a completed V3 phase-one capsule")
    ...
    return AcceptedAuthorizationV3(
        authorization_file_sha256=phase_one.authorization_file_sha256,
...
def execute_exact_build_v3(*, authorization_sha256: str, workers: int) -> dict[str, Any]:
    ...
    try:
        ...
        return _build_exact_prepared_dataset_v3(
            jobs,
            plan.pairs,
            workers=workers,
