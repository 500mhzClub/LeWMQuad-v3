"""Standalone V12 auditor for the Shared-JEPA V5 raw-supervision dataset.
...
one fixed exact entry only after a separate V12 audit authorization.
"""
...
AUTHORIZED_SUCCESS_REPORT_PATH = AUTHORIZED_DATASET_PATH + ".audit_v12.json"
AUTHORIZED_FAILURE_REPORT_PATH = AUTHORIZED_DATASET_PATH + ".audit_v12.failed.json"
...
AUDIT_SCHEMA = "lewm_go2_shared_jepa_v5_raw_supervision_audit_v12"
AUDIT_FAILURE_SCHEMA = "lewm_go2_shared_jepa_v5_raw_supervision_audit_failure_v12"
...
V12_IMPLEMENTATION_AUTHOR = "/root/raw_v11_builder_auditor_diff"
...
class ReviewBindingV12:
    ...
    file_sha256: str
    content_sha256: str
    candidate: tuple[SourceBindingV12, ...]


@dataclass(frozen=True)
class PhaseOneAuthorizationV12:
    authorization_file_sha256: str
    authorization_content_sha256: str
    source_map_sha256: str
    canonical_payload: bytes
    sources: tuple[SourceBindingV12, ...]
    auditor_review: ReviewBindingV12


@dataclass(frozen=True)
class AcceptedAuthorizationV12:
    # These three values describe the immutable V9 build provenance recorded
    # in the dataset manifest. V12 execution authority is kept separate below.
    authorization_file_sha256: str
    authorization_content_sha256: str
    source_map_sha256: str
...
def _read_fixed_manifest_payload() -> tuple[bytes, str]:
    """Descriptor-read the fixed manifest after V12 authority acceptance."""
...
class _FailureReceiptPublisher:
    ...
    def publish(self, value: Mapping[str, Any]) -> None:
        ...
        if (
            ...
            raise PermissionError("only a terminal V12 failure receipt may publish")
...
@dataclass(frozen=True)
class _AuditPublicationContextV12:
    authorization: AcceptedAuthorizationV12
...
def _audit_transaction_bindings(
    context: _AuditPublicationContextV12,
) -> tuple[dict[Path, tuple[str, int | None]], set[Path]]:
...
def _source_binding(entry: object) -> SourceBindingV12:
...
def _candidate_bindings(
    ...
    *,
    roles: Sequence[str],
    source_by_role: Mapping[str, SourceBindingV12],
) -> tuple[SourceBindingV12, ...]:
...
def _validate_authorization_phase_one(
    ...
    *,
    ...
    """Validate complete V12 authority without opening a mapped target."""
    ...
    if (
        ...
        or payload["exact_audit_v12_authorized"] is not True
        or any(payload[field] is not False for field in AUTHORIZATION_FALSE_FIELDS)
    ):
        raise PermissionError("raw-supervision exact V12 audit is not authorized")
    if (
        ...
        or payload["v9_build_authorization_file_sha256"]
        != FROZEN_AUTHORITY_ROLE_SHA256["v9_build_authorization"]
        or payload["v9_build_authorization_content_sha256"]
        != FROZEN_V9_BUILD_AUTHORIZATION_CONTENT_SHA256
        or payload["v9_build_authorization_source_map_sha256"]
        != FROZEN_V9_BUILD_AUTHORIZATION_SOURCE_MAP_SHA256
        or payload["v9_dataset_manifest_file_sha256"]
        != FROZEN_AUTHORITY_ROLE_SHA256["v9_dataset_manifest"]
        or payload["v9_dataset_manifest_content_sha256"]
        != FROZEN_V9_DATASET_MANIFEST_CONTENT_SHA256
        or payload["v9_audit_failure_file_sha256"]
        != FROZEN_AUTHORITY_ROLE_SHA256["v9_terminal_failure"]
        or payload["v9_audit_failure_content_sha256"]
        != FROZEN_V9_AUDIT_FAILURE_CONTENT_SHA256
        or payload["v10_audit_authorization_file_sha256"]
        != FROZEN_AUTHORITY_ROLE_SHA256["v10_audit_authorization"]
        or payload["v10_audit_authorization_content_sha256"]
        != FROZEN_V10_AUDIT_AUTHORIZATION_CONTENT_SHA256
        or payload["v10_audit_authorization_source_map_sha256"]
        != FROZEN_V10_AUDIT_AUTHORIZATION_SOURCE_MAP_SHA256
        or payload["v10_audit_failure_file_sha256"]
        != FROZEN_AUTHORITY_ROLE_SHA256["v10_terminal_failure"]
        or payload["v10_audit_failure_content_sha256"]
        != FROZEN_V10_AUDIT_FAILURE_CONTENT_SHA256
        or payload["v11_auditor_block_file_sha256"]
        != FROZEN_AUTHORITY_ROLE_SHA256["v11_auditor_block"]
        or payload["v11_auditor_block_content_sha256"]
        != FROZEN_V11_AUDITOR_BLOCK_CONTENT_SHA256
    ):
        raise PermissionError(
            "V12 input/output or predecessor terminal binding changed"
        )
    core = dict(payload)
    ...
    if any(
        ...
        raise PermissionError(
            "authorization does not bind the frozen V9/V10/V11/V12 closure"
        )
...
def _review_candidate_value(
    candidate: Sequence[SourceBindingV12],
) -> list[dict[str, str]]:
    return [
...
def _validate_review_record(raw: bytes, binding: ReviewBindingV12) -> None:
...
def _validate_frozen_v9_authority_artifacts(
    ...
    if (
        ...
        != FROZEN_AUTHORITY_ROLE_SHA256["v9_builder_source"]
        or v9_by_role["builder_review"].sha256
        != FROZEN_AUTHORITY_ROLE_SHA256["v9_builder_review"]
    ):
...
def _require_predecessor_success_absent() -> None:
    for version, path in (
        ...
        try:
            ...
            continue
        raise PermissionError(f"a forbidden {version} audit success report exists")
...
def _validate_frozen_v10_authority_artifacts(
    ...
    if tuple(item.role for item in v10_sources) != tuple(
        role for role, _mapped in v10_to_v12_role
    ):
        raise PermissionError("frozen V10 source-map role order changed")
    for source, (_role, mapped_role) in zip(v10_sources, v10_to_v12_role):
        if (
            source.path != SOURCE_PATH_BY_ROLE[mapped_role]
            or source.sha256 != FROZEN_AUTHORITY_ROLE_SHA256[mapped_role]
        ):
    ...
    expected_candidate = [
        {
            "role": role,
            "path": SOURCE_PATH_BY_ROLE[mapped],
            "sha256": FROZEN_AUTHORITY_ROLE_SHA256[mapped],
        }
    ...
    if (
        ...
        or review_binding.get("path") != SOURCE_PATH_BY_ROLE["v10_auditor_review"]
        or review_binding.get("file_sha256")
        != FROZEN_AUTHORITY_ROLE_SHA256["v10_auditor_review"]
...
def _validate_frozen_v11_block_artifacts(
    ...
    candidate = [
        {
            "role": role,
            "path": SOURCE_PATH_BY_ROLE[f"v11_{role}"],
            "sha256": FROZEN_AUTHORITY_ROLE_SHA256[f"v11_{role}"],
        }
    ...
    if (
        ...
        or block.get("implementation_author") != V12_IMPLEMENTATION_AUTHOR
...
def _validate_authorization_phase_two(
    phase_one: PhaseOneAuthorizationV12,
) -> AcceptedAuthorizationV12:
    """Rehash only the fixed V12 closure after structural acceptance."""

    if type(phase_one) is not PhaseOneAuthorizationV12:
        raise TypeError("phase two requires a completed V12 phase-one capsule")
    ...
    _validate_frozen_v9_authority_artifacts(payload_by_role)
    _validate_frozen_v10_authority_artifacts(payload_by_role)
    _validate_frozen_v11_block_artifacts(payload_by_role)
    _require_predecessor_success_absent()
    for relative, digest in {
        **FROZEN_V9_PREDECESSOR_SHA256,
        **REVIEWED_V4_SOURCE_SHA256,
    ...
    return AcceptedAuthorizationV12(
        authorization_file_sha256=FROZEN_AUTHORITY_ROLE_SHA256[
            "v9_build_authorization"
        ],
...
def _require_exact_authority(
    ...
    if not AUDIT_AUTHORIZATION_PATH.is_file():
        raise PermissionError(
            "reviewed raw-supervision V12 audit authorization is absent"
        )
    raw = _read_absolute_bound_payload(
        AUDIT_AUTHORIZATION_PATH,
        authorization_sha256,
        repository_root=ROOT,
        name="V12 audit authorization",
    )
    payload = _strict_canonical_json_object(raw, name="V12 audit authorization")
...
def _prepare_authorized_audit_v12(
    *,
    authorization_sha256: str,
    workers: int,
) -> _PreparedAuditV12:
    ...
    core["authorization_v12"] = {
        ...
        "machine_pass_reviews_parsed": 1,
    }
    core["frozen_v9_terminal_artifacts"] = {
        "dataset_manifest_file_sha256": FROZEN_AUTHORITY_ROLE_SHA256[
            "v9_dataset_manifest"
        ],
        "dataset_manifest_content_sha256": (
            FROZEN_V9_DATASET_MANIFEST_CONTENT_SHA256
        ),
        "terminal_failure_file_sha256": FROZEN_AUTHORITY_ROLE_SHA256[
            "v9_terminal_failure"
        ],
    ...
    core["frozen_v11_terminal_artifacts"] = {
        "auditor_block_file_sha256": FROZEN_AUTHORITY_ROLE_SHA256[
            "v11_auditor_block"
        ],
    ...
    core["closed_publication_transaction_v12"] = {
...
def _final_revalidate_authorized_audit_v12(
    prepared: _PreparedAuditV12,
    *,
    authorization_sha256: str,
    workers: int,
) -> None:
    ...
    if type(prepared) is not _PreparedAuditV12:
        raise TypeError("final audit pass requires a prepared V12 audit")
    _require_exact_authority(authorization_sha256)
...
def _publish_terminal_audit_failure(
    *, authorization_sha256: str, error: BaseException
    ...
    failure_core = {
        ...
        "v9_dataset_manifest_file_sha256": FROZEN_AUTHORITY_ROLE_SHA256[
            "v9_dataset_manifest"
        ],
        "v9_dataset_manifest_content_sha256": (
            FROZEN_V9_DATASET_MANIFEST_CONTENT_SHA256
        ),
        "v9_terminal_failure_file_sha256": FROZEN_AUTHORITY_ROLE_SHA256[
            "v9_terminal_failure"
        ],
        "v9_terminal_failure_content_sha256": FROZEN_V9_AUDIT_FAILURE_CONTENT_SHA256,
        "v10_audit_authorization_file_sha256": FROZEN_AUTHORITY_ROLE_SHA256[
            "v10_audit_authorization"
        ],
        "v10_audit_authorization_content_sha256": (
            FROZEN_V10_AUDIT_AUTHORIZATION_CONTENT_SHA256
        ...
        "v10_terminal_failure_file_sha256": FROZEN_AUTHORITY_ROLE_SHA256[
            "v10_terminal_failure"
        ],
        "v10_terminal_failure_content_sha256": (
            FROZEN_V10_AUDIT_FAILURE_CONTENT_SHA256
        ),
        "v11_auditor_block_file_sha256": FROZEN_AUTHORITY_ROLE_SHA256[
            "v11_auditor_block"
        ],
        "v11_auditor_block_content_sha256": (
            FROZEN_V11_AUDITOR_BLOCK_CONTENT_SHA256
    ...
    try:
        for name in (CANONICAL_AUDIT_REPORT.name, CANONICAL_AUDIT_FAILURE.name):
            if _lstat_optional_at(retained.directory_fd, name) is not None:
                raise FileExistsError(f"immutable audit leaf already exists: {name}")
        prepared = _prepare_authorized_audit_v12(
            authorization_sha256=authorization_sha256,
            workers=worker_count,
        )
        (
