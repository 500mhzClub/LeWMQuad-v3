"""Standalone V7 auditor for the Shared-JEPA V5 raw-supervision dataset.
...
one fixed exact entry only after dual V7 source-review authorization.
"""
...
CANONICAL_AUDIT_REPORT = CANONICAL_DATASET.with_name(
    CANONICAL_DATASET.name + ".audit_v7.json"
)
CANONICAL_AUDIT_FAILURE = CANONICAL_DATASET.with_name(
    CANONICAL_DATASET.name + ".audit_v7.failed.json"
)
...
AUDIT_SCHEMA = "lewm_go2_shared_jepa_v5_raw_supervision_audit_v7"
AUDIT_FAILURE_SCHEMA = "lewm_go2_shared_jepa_v5_raw_supervision_audit_failure_v7"
...
# Builder V7 records this complete closure in the dataset provenance.
FROZEN_PARENT_FILE_SHA256 = dict(FROZEN_V7_PREDECESSOR_SHA256)
...
def _read_fixed_manifest_payload() -> tuple[bytes, str]:
    """Descriptor-read the fixed manifest after V7 authority acceptance."""
...
class _FailureReceiptPublisher:
    ...
    def publish(self, value: Mapping[str, Any]) -> None:
        ...
        if (
            ...
            raise PermissionError("only a terminal V7 failure receipt may publish")
...
@dataclass(frozen=True)
class _AuditPublicationContextV7:
    authorization: AcceptedAuthorizationV7
...
def _audit_transaction_bindings(
    context: _AuditPublicationContextV7,
) -> tuple[dict[Path, tuple[str, int | None]], set[Path]]:
    ...
    def __init__(
        ...
        *,
        ...
        try:
            ...
            # Watching each canonical ancestor closes the V7 builder's
...
def _validate_authorization_phase_one(
    ...
    *,
    ...
    """Validate complete V7 authority without opening a mapped target."""
    ...
    if any(
        ...
        for role, digest in FROZEN_BUILDER_V7_ROLE_SHA256.items()
    ):
        raise PermissionError(
    ...
    for relative, digest in {
        ...
        **REVIEWED_V4_SOURCE_SHA256,
        ...
        _read_absolute_bound_payload(
            ...
            digest,
            repository_root=ROOT,
            name=f"frozen V7 authority source {relative}",
        )
...
def _prepare_authorized_audit_v7(
    *,
    authorization_sha256: str,
    workers: int,
) -> _PreparedAuditV7:
    ...
    core["closed_publication_transaction_v7"] = {
...
def _final_revalidate_authorized_audit_v7(
    prepared: _PreparedAuditV7,
    *,
    authorization_sha256: str,
    workers: int,
) -> None:
    ...
    if type(prepared) is not _PreparedAuditV7:
        raise TypeError("final audit pass requires a prepared V7 audit")
    _require_exact_authority(authorization_sha256)
    ...
    try:
        for name in (CANONICAL_AUDIT_REPORT.name, CANONICAL_AUDIT_FAILURE.name):
            if _lstat_optional_at(retained.directory_fd, name) is not None:
                raise FileExistsError(f"immutable audit leaf already exists: {name}")
        prepared = _prepare_authorized_audit_v7(
            authorization_sha256=authorization_sha256,
            workers=worker_count,
        )
        (
