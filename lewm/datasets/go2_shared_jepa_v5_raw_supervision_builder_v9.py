"""Standalone V9 raw-supervision builder for Shared-JEPA V5 development.

The reviewed V1 construction engine is owned directly by this module. Exact
mode uses a closed two-phase V9 authority, fixed worker functions, and native
...
AUTHORIZATION_SCHEMA = "lewm_go2_shared_jepa_v5_raw_supervision_build_authorization_v9"
...
V9_LINEARIZATION_AMENDMENT_PATH = (
    ROOT
...
@dataclass(frozen=True)
class ExactPrepublicationContextV9:
...
def _exact_publication_source_hashes(
    context: ExactPrepublicationContextV9,
) -> dict[Path, str]:
    if type(context) is not ExactPrepublicationContextV9:
...
@dataclass(frozen=True)
class PreparedSceneJobV9:
    scene_id: str
    role: str
    family: str
    endpoints: tuple[PreparedEndpointV9, ...]
...
def _validate_jobs_and_pairs(
    jobs: Sequence[PreparedSceneJobV9],
    pairs: Sequence[Mapping[str, Any]],
) -> tuple[tuple[PreparedSceneJobV9, ...], tuple[Mapping[str, Any], ...]]:
    ...
    endpoints: dict[str, PreparedEndpointV9] = {}
...
def _build_exact_prepared_dataset_v9(
    jobs: Sequence[PreparedSceneJobV9],
    pairs: Sequence[Mapping[str, Any]],
    *,
    workers: int,
    ...
    prepublication_context: ExactPrepublicationContextV9,
) -> dict[str, Any]:
    """Build the one authorized exact artifact from prepared frames."""

    workers = _strict_workers(workers)
    if (
        type(prepublication_context) is not ExactPrepublicationContextV9
...
def _load_exact_scene_job(
    ...
    prepared: list[PreparedEndpointV9] = []
...
def _source_binding(entry: object) -> SourceBindingV9:
    ...
    if not _is_sha256(digest):
        raise RawSupervisionBuildError("authorization source entry is malformed")
    return SourceBindingV9(role=role, path=path, sha256=digest)
...
def _validate_authorization_phase_one(
    ...
    *,
    ...
    """Validate the complete V9 authority without opening a target file."""
    ...
    if (
        ...
        raise AssertionError("V9 authorization policy is not nine unique rows")
...
def _validate_authorization_phase_two(
    ...
    """Rehash the fixed V9 closure after a complete structural phase."""
...
def execute_exact_build_v9(*, authorization_sha256: str, workers: int) -> dict[str, Any]:
    ...
    try:
        ...
        return _build_exact_prepared_dataset_v9(
            jobs,
            plan.pairs,
            workers=workers,
