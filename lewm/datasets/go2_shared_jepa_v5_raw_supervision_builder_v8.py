"""Standalone V8 raw-supervision builder for Shared-JEPA V5 development.

The reviewed V1 construction engine is owned directly by this module. Exact
mode uses a closed two-phase V8 authority, fixed worker functions, and native
...
AUTHORIZATION_SCHEMA = "lewm_go2_shared_jepa_v5_raw_supervision_build_authorization_v8"
...
@dataclass(frozen=True)
class ExactPrepublicationContextV8:
...
def _exact_publication_source_hashes(
    context: ExactPrepublicationContextV8,
) -> dict[Path, str]:
    if type(context) is not ExactPrepublicationContextV8:
...
@dataclass(frozen=True)
class PreparedSceneJobV8:
    scene_id: str
    role: str
    family: str
    endpoints: tuple[PreparedEndpointV8, ...]
...
def _validate_jobs_and_pairs(
    jobs: Sequence[PreparedSceneJobV8],
    pairs: Sequence[Mapping[str, Any]],
) -> tuple[tuple[PreparedSceneJobV8, ...], tuple[Mapping[str, Any], ...]]:
    ...
    endpoints: dict[str, PreparedEndpointV8] = {}
...
def _build_exact_prepared_dataset_v8(
    jobs: Sequence[PreparedSceneJobV8],
    pairs: Sequence[Mapping[str, Any]],
    *,
    workers: int,
    ...
    prepublication_context: ExactPrepublicationContextV8,
) -> dict[str, Any]:
    """Build the one authorized exact artifact from prepared frames."""

    workers = _strict_workers(workers)
    if (
        type(prepublication_context) is not ExactPrepublicationContextV8
...
def _load_exact_scene_job(
    ...
    prepared: list[PreparedEndpointV8] = []
...
def _source_binding(entry: object) -> SourceBindingV8:
    ...
    if not _is_sha256(digest):
        raise RawSupervisionBuildError("authorization source entry is malformed")
    return SourceBindingV8(role=role, path=path, sha256=digest)
...
def _validate_authorization_phase_one(
    ...
    *,
    ...
    """Validate the complete V8 authority without opening a target file."""
    ...
    if (
        ...
        raise AssertionError("V8 authorization policy is not nine unique rows")
...
def _validate_authorization_phase_two(
    ...
    """Rehash the fixed V8 closure after a complete structural phase."""
...
def execute_exact_build_v8(*, authorization_sha256: str, workers: int) -> dict[str, Any]:
    ...
    try:
        ...
        return _build_exact_prepared_dataset_v8(
            jobs,
            plan.pairs,
            workers=workers,
