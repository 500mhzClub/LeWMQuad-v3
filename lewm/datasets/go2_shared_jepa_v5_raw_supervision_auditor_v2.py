"""Additive V2 auditor for the Shared-JEPA V5 raw-supervision artifact.
...
from lewm.datasets import go2_shared_jepa_v5_raw_supervision_auditor_v1 as _v1
from lewm.datasets import go2_shared_jepa_v5_raw_supervision_plan_v5 as plan_v5


ROOT = Path(__file__).resolve().parents[2]
CANONICAL_DATASET = _v1.CANONICAL_DATASET
CANONICAL_AUDIT_REPORT = CANONICAL_DATASET.with_name(
    CANONICAL_DATASET.name + ".audit_v2.json"
)
CANONICAL_AUDIT_FAILURE = CANONICAL_DATASET.with_name(
    CANONICAL_DATASET.name + ".audit_v2.failed.json"
)
...
AUDIT_SCHEMA = "lewm_go2_shared_jepa_v5_raw_supervision_audit_v2"
SYNTHETIC_AUDIT_SCHEMA = (
    "lewm_go2_shared_jepa_v5_raw_supervision_synthetic_audit_v2"
)
AUDIT_FAILURE_SCHEMA = "lewm_go2_shared_jepa_v5_raw_supervision_audit_failure_v2"
AUTHORIZATION_SCHEMA = (
    "lewm_go2_shared_jepa_v5_raw_supervision_build_authorization_v2"
)
...
AuditInputs = _v1.AuditInputs
RawSupervisionAuditError = _v1.RawSupervisionAuditError
...
def _validate_shard_cardinalities_v2(
    manifest: Mapping[str, Any], payloads: Mapping[str, bytes]
) -> None:
...
def _validate_frozen_population_v2(population: Mapping[str, Any]) -> None:
...
def _preflight_dataset_v2(
    ...
    *,
    ...
    root, manifest_payload, manifest = (
        ...
        else _read_manifest_bound_v2(
            dataset_root,
            expected_manifest_file_sha256=expected_manifest_file_sha256,
        )
    )
    payloads = _strict_file_inventory_v2(
        root, manifest, manifest_payload=manifest_payload
    )
    pairs, endpoints = _declared_index_payloads(manifest, payloads)
    _validate_shard_cardinalities_v2(manifest, payloads)
    population = _derive_population_v2(manifest, pairs, endpoints)
    if exact:
        _validate_frozen_population_v2(population)
    return DatasetPreflightV2(
        root=root,
        manifest=manifest,
...
def _source_binding_v2(entry: object) -> SourceBindingV2:
    ...
    return SourceBindingV2(role=role, path=relative, sha256=str(digest))


def _candidate_bindings_v2(
    value: object,
    *,
    roles: Sequence[str],
    ...
    candidate = tuple(_source_binding_v2(entry) for entry in value)
...
def _review_binding_v2(
    ...
    *,
    ...
    candidate = _candidate_bindings_v2(
        value.get("candidate"),
...
def _validate_authorization_phase_one_v2(
    ...
    *,
    ...
    """Validate complete V2 authority without opening any mapped target."""
    ...
    sources = tuple(_source_binding_v2(entry) for entry in source_map)
    ...
    builder_review = _review_binding_v2(
    ...
    auditor_review = _review_binding_v2(
...
def _review_candidate_value_v2(
...
def _expected_review_authority_v2(kind: str) -> dict[str, bool]:
    return {
...
def _validate_review_record_v2(raw: bytes, binding: ReviewBindingV2) -> None:
    ...
    if (
        ...
        or review.get("candidate") != _review_candidate_value_v2(binding.candidate)
        or review.get("authority") != _expected_review_authority_v2(binding.kind)
    ):
...
def _validate_authorization_phase_two_v2(
    ...
    *,
    ...
    revalidated = _validate_authorization_phase_one_v2(
        embedded,
...
def _require_exact_authorization_v2(
    expected_authorization_file_sha256: str,
) -> PhaseOneAuthorizationV2:
    """Validate fixed V2 authority before any dataset or metadata open."""
    ...
    phase_one = _validate_authorization_phase_one_v2(
        authorization,
    ...
    _validate_authorization_phase_two_v2(phase_one, repository_root=ROOT)
    return phase_one


def _validate_manifest_authorization_binding_v2(
    provenance: Mapping[str, Any], authorization: PhaseOneAuthorizationV2
) -> None:
    if (
...
def _validate_exact_authorization_v2(
    ...
    if not _is_sha256(authorization_sha256):
        raise RawSupervisionAuditError("build authorization file hash is malformed")
    phase_one = _require_exact_authorization_v2(str(authorization_sha256))
    _validate_manifest_authorization_binding_v2(provenance, phase_one)
    return phase_one


def _validate_exact_manifest_bindings_v2(
    manifest: Mapping[str, Any],
    *,
    inputs: AuditInputs,
    hashed_sources: Sequence[Mapping[str, Any]],
    parent_contracts: Sequence[Mapping[str, Any]],
    authorization: PhaseOneAuthorizationV2,
) -> None:
    frames_scanned = sum(
        _strict_int(
...
def audit_dataset_v2(
    ...
    *,
    ...
    if (
        ...
        raise RawSupervisionAuditError("dataset changed during V2 audit")
    return _v2_report(result, before.population, authoritative_exact=False)


audit_synthetic_dataset_v2 = audit_dataset_v2


def audit_exact_dataset_v2(
    repo_root: Path,
    dataset_root: Path,
    *,
    expected_manifest_file_sha256: str,
    ...
    authorization = _require_exact_authorization_v2(
        expected_authorization_file_sha256
    )

    initial = _read_manifest_bound_v2(
        dataset_root,
        expected_manifest_file_sha256=expected_manifest_file_sha256,
    )
    ...
    if type(provenance) is not dict or set(provenance) != _v1.EXACT_INPUT_PROVENANCE_FIELDS:
        raise RawSupervisionAuditError("exact input-provenance fields changed")
    _validate_manifest_authorization_binding_v2(provenance, authorization)
    preflight = _preflight_dataset_v2(
        dataset_root,
        expected_manifest_file_sha256=expected_manifest_file_sha256,
        exact=True,
        initial=initial,
    ...
    _validate_exact_manifest_bindings_v2(
        preflight.manifest,
        inputs=inputs,
    ...
    dataset_after = _preflight_dataset_v2(
        preflight.root,
        expected_manifest_file_sha256=expected_manifest_file_sha256,
        exact=True,
    )
    if (
        ...
        raise RawSupervisionAuditError("exact dataset changed during V2 audit")
    result = _v2_report(
...
def execute_exact_audit_v2(
    *,
    expected_manifest_file_sha256: str,
    expected_authorization_file_sha256: str,
    workers: int = MAX_WORKERS,
) -> dict[str, Any]:
    """Audit the canonical dataset and publish one immutable V2 terminal leaf."""
...
__all__ = [
    ...
    "SYNTHETIC_AUDIT_SCHEMA",
    "audit_dataset_v2",
    "audit_exact_dataset_v2",
    "audit_synthetic_dataset_v2",
