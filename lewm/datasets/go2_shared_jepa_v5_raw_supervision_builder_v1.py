"""Reviewed-V4 raw supervision builder for the three Shared-JEPA V5 dev roles.

The public synthetic path accepts already prepared V4 frame inputs.  The exact
path is separately gated and is intentionally unusable until a future
different-agent authorization record binds both this builder and an independent
auditor.  RGB and legacy label payloads are never opened by this module.
"""
...
from scripts import build_go2_observable_camera_ray_fit_v4 as v4_builder
...
AUTHORIZATION_SCHEMA = "lewm_go2_shared_jepa_v5_raw_supervision_build_authorization_v1"
...
class PreparedEndpointV1:
    plan_endpoint: Mapping[str, Any]
    family: str
    frame: v4_builder.FrameBuildInputV4
...
@dataclass(frozen=True)
class PreparedSceneJobV1:
    scene_id: str
    role: str
    family: str
    endpoints: tuple[PreparedEndpointV1, ...]

    def __post_init__(self) -> None:
        if (
            not self.scene_id
            or self.role not in plan_v5.DEVELOPMENT_ROLES
            or not self.family
...
def _write_prepared_scene_job(
    ...
    evidences = [v4_builder.build_frame_evidence_v4(item.frame) for item in ordered]
    rasters = [
...
def _validate_jobs_and_pairs(
    jobs: Sequence[PreparedSceneJobV1],
    pairs: Sequence[Mapping[str, Any]],
) -> tuple[tuple[PreparedSceneJobV1, ...], tuple[Mapping[str, Any], ...]]:
    ...
    endpoints: dict[str, PreparedEndpointV1] = {}
    ...
    ordered_pairs = tuple(
        sorted(
            pairs,
            key=lambda item: (
                plan_v5.DEVELOPMENT_ROLES.index(str(item["dataset_role"])),
                int(item["global_row"]),
            ),
        )
    )
...
def build_prepared_dataset_v1(
    ...
    *,
    ...
    exact_role_family_count: bool = False,
    prepublication_validator: Callable[[], None] | None = None,
) -> dict[str, Any]:
    """Build from prepared frames; this function performs no source-data reads."""
    ...
    try:
        ...
        try:
            ...
            with ProcessPoolExecutor(
                ...
                futures = [
                    executor.submit(_write_prepared_scene_job, job, str(staging))
                    for job in ordered_jobs
                ]
        ...
        endpoint_rows.sort(
            key=lambda item: (
                plan_v5.DEVELOPMENT_ROLES.index(item["dataset_role"]),
        ...
        if exact_role_family_count and len(sample["records"]) != 24:
            raise RawSupervisionBuildError(
        ...
        manifest = _with_content_sha256(
            {
                ...
                "evidence_schema": v4_builder.EVIDENCE_SCHEMA,
                "raster_schema": v4_builder.ray_v4.RASTER_SCHEMA,
...
def _load_exact_scene_job(
    ...
    prepared: list[PreparedEndpointV1] = []
    for digest in sorted(endpoint_by_digest):
        ...
        quaternion, stored_yaw = v4_builder._validated_sidecar_source_attitude(
            extracted, endpoint
        )
...
def _validate_exact_plan_result(
    ...
    if (
        ...
        or dict(inventory.hashes) != dict(plan_v5.SOURCE_INVENTORY_SHA256)
    ...
    *,
    ...
    try:
        ...
        with ProcessPoolExecutor(
            ...
            futures = [executor.submit(function, *arguments) for arguments in argument_rows]
...
def _validate_authorization_payload(
    ...
    *,
    ...
    if (
        ...
        or payload.get("builder_review", {}).get("verdict") != "PASS"
        or payload.get("auditor_review", {}).get("verdict") != "PASS"
    ):
        raise PermissionError("raw-supervision exact build lacks two reviewed implementations")
    core = dict(payload)
    declared = core.pop("content_sha256", None)
    if not _is_sha256(declared) or canonical_json_sha256(core) != declared:
    ...
    source_entries = payload.get("source_map")
    ...
    for entry in source_entries:
        ...
        if (
            ...
            raise RawSupervisionBuildError("authorization source entry changed")
...
def _require_exact_authority(authorization_sha256: str) -> dict[str, Any]:
    ...
    try:
        ...
        raise RawSupervisionBuildError("build authorization is invalid JSON") from error
    if not isinstance(payload, Mapping):
        raise RawSupervisionBuildError("build authorization is not an object")
    for relative, expected in {**FROZEN_PARENT_HASHES, **REVIEWED_V4_SOURCES}.items():
        _read_bound_regular_file(
            repository_root=ROOT,
...
def execute_exact_build_v1(*, authorization_sha256: str, workers: int) -> dict[str, Any]:
    ...
    try:
        ...
        def revalidate_before_publication() -> None:
            second_plan = plan_v5.load_frozen_development_metadata(ROOT)
            second_inventory = plan_v5.load_frozen_development_source_inventory(
                ROOT, second_plan
            )
            ...
            _load_parent_contracts(authorization_sha256)
            revalidated = _run_scene_pool(
