"""Standalone V6 raw-supervision builder for Shared-JEPA V5 development.

The reviewed V1 construction engine is owned directly by this module. Exact
mode uses a closed two-phase V6 authority, fixed worker functions, and native
...
AUTHORIZATION_SCHEMA = "lewm_go2_shared_jepa_v5_raw_supervision_build_authorization_v6"
...
@dataclass(frozen=True)
class ExactPrepublicationContextV6:
...
def _exact_publication_source_hashes(
    context: ExactPrepublicationContextV6,
) -> dict[Path, str]:
    if type(context) is not ExactPrepublicationContextV6:
...
class _ClosedPublicationTransaction:
    ...
    def _add_watch(self, path: Path, *, is_directory: bool, role: str) -> None:
        ...
        if existing is not None:
            ...
            return
        mask = _IN_DIRECTORY_MASK if is_directory else _IN_LEAF_MASK
...
@dataclass(frozen=True)
class PreparedSceneJobV6:
    scene_id: str
    role: str
    family: str
    endpoints: tuple[PreparedEndpointV6, ...]
...
def _validate_jobs_and_pairs(
    jobs: Sequence[PreparedSceneJobV6],
    pairs: Sequence[Mapping[str, Any]],
) -> tuple[tuple[PreparedSceneJobV6, ...], tuple[Mapping[str, Any], ...]]:
    ...
    endpoints: dict[str, PreparedEndpointV6] = {}
...
def _build_exact_prepared_dataset_v6(
    jobs: Sequence[PreparedSceneJobV6],
    pairs: Sequence[Mapping[str, Any]],
    *,
    workers: int,
    ...
    prepublication_context: ExactPrepublicationContextV6,
) -> dict[str, Any]:
    """Build the one authorized exact artifact from prepared frames."""

    workers = _strict_workers(workers)
    if (
        type(prepublication_context) is not ExactPrepublicationContextV6
...
def _load_exact_scene_job(
    ...
    prepared: list[PreparedEndpointV6] = []
...
def _source_binding(entry: object) -> SourceBindingV6:
    ...
    if not _is_sha256(digest):
        raise RawSupervisionBuildError("authorization source entry is malformed")
    return SourceBindingV6(role=role, path=path, sha256=digest)
...
def _validate_authorization_phase_one(
    ...
    *,
    ...
    """Validate the complete V6 authority without opening a target file."""
    ...
    if (
        ...
        raise AssertionError("V6 authorization policy is not nine unique rows")
...
def _validate_authorization_phase_two(
    ...
    """Rehash the fixed V6 closure after a complete structural phase."""
...
def execute_exact_build_v6(*, authorization_sha256: str, workers: int) -> dict[str, Any]:
    ...
    try:
        ...
        return _build_exact_prepared_dataset_v6(
            jobs,
            plan.pairs,
            workers=workers,
