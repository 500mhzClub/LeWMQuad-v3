"""Standalone V4 raw-supervision builder for Shared-JEPA V5 development.

The reviewed V1 construction engine is owned directly by this module. Exact
mode uses a closed two-phase V4 authority, fixed worker functions, and native
...
AUTHORIZATION_SCHEMA = "lewm_go2_shared_jepa_v5_raw_supervision_build_authorization_v4"
...
FROZEN_PARENT_HASHES = {
    ...
    str(V4_SUCCESSOR_AMENDMENT_PATH.relative_to(ROOT)): (
...
class ReviewBindingV4:
    ...
    file_sha256: str
    content_sha256: str
    candidate: tuple[SourceBindingV4, ...]


@dataclass(frozen=True)
class PhaseOneAuthorizationV4:
    authorization_file_sha256: str
    authorization_content_sha256: str
    source_map_sha256: str
    canonical_payload: bytes
    sources: tuple[SourceBindingV4, ...]
    builder_review: ReviewBindingV4
    auditor_review: ReviewBindingV4


@dataclass(frozen=True)
class AcceptedAuthorizationV4:
    authorization_file_sha256: str
    authorization_content_sha256: str
    source_map_sha256: str
...
@dataclass(frozen=True)
class ExactPrepublicationContextV4:
...
@dataclass(frozen=True)
class PreparedSceneJobV4:
    scene_id: str
    role: str
    family: str
    endpoints: tuple[PreparedEndpointV4, ...]
...
def _validate_jobs_and_pairs(
    jobs: Sequence[PreparedSceneJobV4],
    pairs: Sequence[Mapping[str, Any]],
) -> tuple[tuple[PreparedSceneJobV4, ...], tuple[Mapping[str, Any], ...]]:
    ...
    endpoints: dict[str, PreparedEndpointV4] = {}
...
def _build_exact_prepared_dataset_v4(
    jobs: Sequence[PreparedSceneJobV4],
    pairs: Sequence[Mapping[str, Any]],
    *,
    workers: int,
    ...
    prepublication_context: ExactPrepublicationContextV4,
) -> dict[str, Any]:
    """Build the one authorized exact artifact from prepared frames."""

    workers = _strict_workers(workers)
    if (
        type(prepublication_context) is not ExactPrepublicationContextV4
...
def _load_exact_scene_job(
    ...
    prepared: list[PreparedEndpointV4] = []
...
def _revalidate_exact_before_publication(
    context: ExactPrepublicationContextV4,
) -> None:
    if type(context) is not ExactPrepublicationContextV4:
        raise RawSupervisionBuildError("exact prepublication context changed")
...
def _source_binding(entry: object) -> SourceBindingV4:
    ...
    if not _is_sha256(digest):
        raise RawSupervisionBuildError("authorization source entry is malformed")
    return SourceBindingV4(role=role, path=path, sha256=digest)
...
def _candidate_bindings(
    ...
    *,
    roles: Sequence[str],
    source_by_role: Mapping[str, SourceBindingV4],
) -> tuple[SourceBindingV4, ...]:
...
def _validate_authorization_phase_one(
    ...
    *,
    authorization_file_sha256: str,
) -> PhaseOneAuthorizationV4:
    """Validate the complete V4 authority without opening a target file."""
    ...
    if (
        ...
        raise AssertionError("V4 authorization policy is not nine unique rows")
    ...
    return PhaseOneAuthorizationV4(
...
def _review_candidate_value(
    candidate: Sequence[SourceBindingV4],
) -> list[dict[str, str]]:
    return [
...
def _validate_review_record(raw: bytes, binding: ReviewBindingV4) -> None:
...
def _validate_authorization_phase_two(
    phase_one: PhaseOneAuthorizationV4,
) -> AcceptedAuthorizationV4:
    """Rehash the fixed V4 closure after a complete structural phase."""

    if type(phase_one) is not PhaseOneAuthorizationV4:
        raise TypeError("phase two requires a completed V4 phase-one capsule")
    ...
    return AcceptedAuthorizationV4(
        authorization_file_sha256=phase_one.authorization_file_sha256,
...
def execute_exact_build_v4(*, authorization_sha256: str, workers: int) -> dict[str, Any]:
    ...
    try:
        ...
        return _build_exact_prepared_dataset_v4(
            jobs,
            plan.pairs,
            workers=workers,
