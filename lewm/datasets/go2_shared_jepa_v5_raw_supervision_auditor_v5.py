"""Standalone V5 auditor for the Shared-JEPA V5 raw-supervision dataset.
...
one fixed exact entry only after dual V5 source-review authorization.
"""
...
CANONICAL_AUDIT_REPORT = CANONICAL_DATASET.with_name(
    CANONICAL_DATASET.name + ".audit_v5.json"
)
CANONICAL_AUDIT_FAILURE = CANONICAL_DATASET.with_name(
    CANONICAL_DATASET.name + ".audit_v5.failed.json"
)
...
AUDIT_SCHEMA = "lewm_go2_shared_jepa_v5_raw_supervision_audit_v5"
AUDIT_FAILURE_SCHEMA = "lewm_go2_shared_jepa_v5_raw_supervision_audit_failure_v5"
AUTHORIZATION_SCHEMA = (
    "lewm_go2_shared_jepa_v5_raw_supervision_build_authorization_v5"
)
...
def _read_fixed_manifest_payload() -> tuple[bytes, str]:
    """Descriptor-read the fixed manifest after V5 authority acceptance."""
...
def _validate_exact_input_provenance(
    ...
    *,
    ...
    authorization: AcceptedAuthorizationV5,
) -> None:
...
def _validate_authorization_phase_one(
    ...
    *,
    ...
    """Validate complete V5 authority without opening a mapped target."""
    ...
    if any(
        ...
        for role, digest in FROZEN_BUILDER_V5_ROLE_SHA256.items()
    ):
        raise PermissionError(
    ...
    for relative, digest in {
        ...
        **REVIEWED_V4_SOURCE_SHA256,
        ...
        _read_absolute_bound_payload(
            ...
            digest,
            repository_root=ROOT,
            name=f"frozen V5 authority source {relative}",
        )
...
def _run_authorized_audit_v5(
    *,
    authorization_sha256: str,
    workers: int,
) -> dict[str, Any]:
    """Run the fixed owned audit after full V5 authority acceptance."""
    ...
    with _ExclusiveAuditPublisher(CANONICAL_AUDIT_REPORT.parent) as publisher:
        ...
        try:
            with _open_retained_directory_chain(CANONICAL_DATASET) as dataset:
                result = _run_authorized_audit_v5(
                    authorization_sha256=authorization_sha256,
                    workers=worker_count,
                )
                dataset.validate()
