"""Matched cumulative-event objectives on actual 100-ms targets; fresh models only."""
from copy import deepcopy
import math
import torch
from lewm.cumulative_pulse_contact_development import cumulative_outcomes
...
def training_loss(model,batch,condition,*,outcome_transform=None):
    if not isinstance(model,ObservationHorizonRGBBodyJEPA):
    ...
    active,offsets=validate_plan(blocks,mask,len(z))
    ...
    if ((mv&~cv).any() or (fv&~mv).any() or not torch.isfinite(targets['motion'][mv]).all()
    ...
    if outcome_transform is not None:
        direct=outcome_transform(direct,blocks,mask)
    ...
    if condition!='direct':
        ...
        if outcome_transform is not None:
            rollout=outcome_transform(rollout,blocks,mask)
        rollout_loss=scaled_outcome_loss(rollout,*args)
...
class ObservationHorizonTrainer:
    def __init__(self,condition,*,seed,latent_dim=32,learning_rate=1e-3,ema_momentum=.99):
        ...
        with torch.random.fork_rng(devices=[]):
            torch.manual_seed(seed);self.model=ObservationHorizonRGBBodyJEPA(latent_dim).cpu()
    ...
    def checkpoint(self):
        return deepcopy(dict(schema='observation_horizon_training_development.v1',
