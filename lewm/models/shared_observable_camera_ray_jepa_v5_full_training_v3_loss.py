"""Full Training V3 Camera objective and exact B4 reduction adapter.
...
EXPECTED_MICROBATCH_SIZE_V3 = 4
CAMERA_MODEL_CONFIG_WEIGHT_V3 = 1.0


@dataclass(frozen=True)
class SharedObservableCameraRayV4FrameLossV3:
    """One separately computed real-B4 Camera V13 frame objective."""
...
@dataclass(frozen=True)
class SharedObservableCameraRayV4LossV3:
    """Exact 0.5/0.5 average of separately computed current and next B4 losses."""

    current: SharedObservableCameraRayV4FrameLossV3
    next: SharedObservableCameraRayV4FrameLossV3
    total: torch.Tensor


@dataclass(frozen=True)
class SharedJointLossV3:
    """Promoted JEPA plus corrected Camera V13 supervision."""

    total: torch.Tensor
    established_jepa: EstablishedJepaPackageV5
    observable_camera_ray_v4: SharedObservableCameraRayV4LossV3
...
def _require_real_b4_frame(
    ...
    *,
    ...
    if not isinstance(pair, SharedTrainingPairV5):
        raise TypeError("V3 pair must be SharedTrainingPairV5")
    if not isinstance(supervision, ObservableCameraRayV4FrameSupervisionV5):
        raise TypeError("V3 supervision type changed")
    ...
    if target_batch != observed:
        raise ValueError("V3 Camera supervision batch does not match its frame")
    if require_b4 and observed != EXPECTED_MICROBATCH_SIZE_V3:
        raise ValueError(
            "V3 nonlinear Camera loss requires one real B=4 frame microbatch; "
...
def _frame_loss_v3(
    ...
    *,
    frame_name: str,
    require_b4: bool,
) -> SharedObservableCameraRayV4FrameLossV3:
    ...
    if not bool(torch.isfinite(objective.total).item()):
        raise FloatingPointError("V3 frame Camera objective became nonfinite")
    return SharedObservableCameraRayV4FrameLossV3(
        hierarchical_first_hit_nll=first_hit.total,
...
def observable_camera_ray_v4_loss_v3(
    model: object,
    pair: SharedTrainingPairV5,
    ...
    *,
...
) -> SharedObservableCameraRayV4LossV3:
    ...
    if getattr(model_config, "observable_camera_ray_v4_weight", None) != (
        CAMERA_MODEL_CONFIG_WEIGHT_V3
    ):
        raise ValueError("V3 Camera model-config weight must remain exactly 1.0")
    current = _frame_loss_v3(
        model,
        pair,
        current_supervision,
        frame_name="current",
    ...
    if not bool(torch.isfinite(total).item()):
        raise FloatingPointError("V3 current/next Camera objective became nonfinite")
    return SharedObservableCameraRayV4LossV3(
        current=current,
        next=next_,
        total=total,
    )
...
def combine_joint_losses_v3(
    model: object,
    pair: SharedTrainingPairV5,
    current_supervision: ObservableCameraRayV4FrameSupervisionV5,
    next_supervision: ObservableCameraRayV4FrameSupervisionV5,
) -> SharedJointLossV3:
    """Combine established JEPA and the corrected V3 Camera pair scalar."""

    camera = observable_camera_ray_v4_loss_v3(
        model,
        pair,
        current_supervision,
        next_supervision,
    ...
    if not bool(torch.isfinite(total).item()):
        raise FloatingPointError("V3 promoted joint objective became nonfinite")
    return SharedJointLossV3(
        total=total,
        established_jepa=pair.jepa,
        observable_camera_ray_v4=camera,
        observable_camera_ray_v4_weight=CAMERA_MODEL_CONFIG_WEIGHT_V3,
    )


def average_four_microbatch_tensor_scalars_v3(
    values: Sequence[torch.Tensor],
) -> torch.Tensor:
    """Return the exact equal mean of four already-complete B4 scalars."""

    if isinstance(values, (str, bytes)) or len(values) != 4:
        raise ValueError("V3 update requires exactly four complete B4 scalars")
    if any(
        not isinstance(value, torch.Tensor) or value.ndim != 0
        for value in values
    ):
        raise TypeError("V3 microbatch losses must be scalar tensors")
    device = values[0].device
    if any(value.device != device for value in values):
        raise ValueError("V3 microbatch losses must share one device")
    result = sum(0.25 * value for value in values)
    if not bool(torch.isfinite(result).item()):
        raise FloatingPointError("V3 update scalar became nonfinite")
    return result


__all__ = [
    "CAMERA_MODEL_CONFIG_WEIGHT_V3",
    "EXPECTED_MICROBATCH_SIZE_V3",
    "SharedJointLossV3",
    "SharedObservableCameraRayV4FrameLossV3",
    "SharedObservableCameraRayV4LossV3",
    "average_four_microbatch_tensor_scalars_v3",
    "combine_joint_losses_v3",
    "observable_camera_ray_v4_loss_v3",
]
