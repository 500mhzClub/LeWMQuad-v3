"""Explicit weak-subspace inertial prediction alongside unchanged depth state.
...
SCHEMA='depth_inertial_weak_subspace_development.v1'
...
class WeakSubspaceIntegrator:
    ...
    def _acceleration(self,policy,rotation):
        ...
        force=force_in_current_frame(f['values'],g['values'])[-5:].mean(axis=0)
        return rotation@force-self.gravity
    ...
    def _observe(self,policy,state):
        ...
        if self.last_ns is None:
            ...
            if rank==3:
                ...
                self.velocity=observed/.1+.05*acceleration
                ...
                inertial=self.velocity*.1+.005*acceleration
                initial_delta=observed+weak_projector@inertial
                self.velocity=(np.eye(3)-weak_projector)@(observed/.1+.05*acceleration)+weak_projector@(self.velocity+.1*acceleration)
...
class DepthInertialState:
    """Live sensor observer with the original depth evidence kept verbatim."""
    def __init__(self):
        self.depth=DepthRelativeState(); self.integrator=WeakSubspaceIntegrator()
