"""Authority-explicit conversion of live G3 V2 paths to world waypoints.

This additive successor preserves the V1 geometry and exact-live path
validation while making both hardware and production-promotion denial part of
the typed, hashed controller-facing receipt.
"""
...
WORLD_WAYPOINT_ADAPTER_V2_SCHEMA = (
    "lewm_g3_v2_configuration_path_world_waypoint_adapter_v2"
)


class WorldWaypointV2BindingError(SnapshotBindingError):
    """Raised when a V2 waypoint receipt is stale, copied, or misbound."""
...
@dataclass(frozen=True)
class ConfigurationPathWorldWaypointReceiptV2:
    snapshot_sha256: str
    ...
    waypoints: tuple[WorldWaypointV2, ...]
    path_cost_configuration_steps: float
    path_cost_m: float
    exact_sim_tainted: bool
    _issuance_capability: object = field(repr=False, compare=False)
    hardware_execution_authorized: bool = field(init=False, default=False)
    production_promotion_authorized: bool = field(init=False, default=False)
    ...
    def __post_init__(self) -> None:
        ...
        if not waypoints or any(type(row) is not WorldWaypointV2 for row in waypoints):
            raise ValueError("waypoint receipt requires typed V2 waypoints")
        ...
        object.__setattr__(self, "hardware_execution_authorized", False)
        object.__setattr__(self, "production_promotion_authorized", False)
    ...
    def _core(self) -> dict[str, object]:
        return {
            "schema": "lewm_g3_v2_configuration_path_world_waypoint_receipt_v2",
            "adapter_schema": WORLD_WAYPOINT_ADAPTER_V2_SCHEMA,
    ...
    def assert_integrity(self) -> None:
        if (
            self.hardware_execution_authorized is not False
            or self.production_promotion_authorized is not False
        ):
            raise WorldWaypointV2BindingError(
                "world-waypoint receipt authority denial was mutated"
            )
        if self.content_sha256 != _sha256(self._core()):
            raise WorldWaypointV2BindingError("world-waypoint receipt was mutated")

    def __copy__(self) -> "ConfigurationPathWorldWaypointReceiptV2":
        raise TypeError("world-waypoint V2 receipts are non-copyable")

    def __deepcopy__(
        self,
        memo: object,
    ) -> "ConfigurationPathWorldWaypointReceiptV2":
        del memo
        raise TypeError("world-waypoint V2 receipts are non-copyable")


class ConfigurationPathWorldWaypointIssuerV2:
    """Issue single-use V2 world-centre receipts for exact live G3 paths."""
    ...
    def __init__(
        ...
        self._issued: dict[int, ConfigurationPathWorldWaypointReceiptV2] = {}
        self._consumed: set[int] = set()

    def __copy__(self) -> "ConfigurationPathWorldWaypointIssuerV2":
        raise TypeError("world-waypoint V2 issuers are non-copyable")
    ...
    def _build(
        ...
        return ConfigurationPathWorldWaypointReceiptV2(
    ...
    def validate(
        ...
        snapshot: TwoResolutionConfigurationSnapshotV2,
        path: ConfigurationPathV2,
        receipt: ConfigurationPathWorldWaypointReceiptV2,
        *,
        consume: bool = False,
    ) -> None:
        if type(receipt) is not ConfigurationPathWorldWaypointReceiptV2:
            raise TypeError("receipt must be ConfigurationPathWorldWaypointReceiptV2")
        self._projection.assert_current_snapshot(snapshot)
        self._planner.validate_path(snapshot, path)
        ...
        if identity in self._consumed:
            raise WorldWaypointV2BindingError("waypoint receipt was already consumed")
