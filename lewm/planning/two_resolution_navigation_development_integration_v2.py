"""Standalone atomic successor for two-resolution development integration V1.

V2 preserves V1 unchanged but does not retain or expose a V1 engine.  It calls
the passed G3/G4/G5/router/waypoint owners directly after an episode-bound claim
preflight.  A sealed owner-state digest rejects interference through any other
coordinator before that state can acquire V2 authority.
"""
...
PRODUCTION_TWO_RESOLUTION_NAVIGATION_INTEGRATION_V2 = None


class TwoResolutionNavigationIntegrationV2Error(ValueError):
    """Base error for the additive V2 coordinator."""


class TwoResolutionNavigationIntegrationV2BindingError(
    TwoResolutionNavigationIntegrationV2Error
):
    """An episode, controller, observer, owner, or state binding changed."""


class TwoResolutionNavigationIntegrationV2ReplayError(
    TwoResolutionNavigationIntegrationV2Error
):
    """An exact single-use V2 authority was replayed."""
...
def _validate_empty_ledger(value: object) -> dict[str, int]:
    ...
    if ledger != _empty_ledger() or any(
        ...
        raise TwoResolutionNavigationIntegrationV2BindingError(
...
class TwoResolutionNavigationTargetObjectBindingV2:
...
class TwoResolutionNavigationEpisodeAuthorityV2:
    ...
    target_object_bindings: tuple[TwoResolutionNavigationTargetObjectBindingV2, ...]
    snapshot_sha256: str
    ...
    def __post_init__(self) -> None:
        ...
        if (
            not bindings
            or any(
                type(binding) is not TwoResolutionNavigationTargetObjectBindingV2
    ...
    def to_dict(self, include_hash: bool = True) -> dict[str, object]:
        result: dict[str, object] = {
            "schema": "lewm_two_resolution_navigation_episode_authority_v2",
    ...
    def assert_integrity(self) -> None:
        if type(self._issuer) is not TwoResolutionNavigationEpisodeAuthorityIssuerV2:
            raise TwoResolutionNavigationIntegrationV2BindingError(
                "episode authority lost its exact issuer"
            )
        self._issuer.assert_episode_authority(self)

    def __copy__(self) -> "TwoResolutionNavigationEpisodeAuthorityV2":
        raise TypeError("navigation episode authorities are non-copyable")

    def __deepcopy__(
        self,
        memo: object,
    ) -> "TwoResolutionNavigationEpisodeAuthorityV2":
        del memo
        raise TypeError("navigation episode authorities are non-copyable")
...
class _EpisodeAuthorityRecord:
    authority: TwoResolutionNavigationEpisodeAuthorityV2
...
class _EpisodeIssuerState:
    ...
    task_ids: tuple[str, ...]
    task_hash: str
    bindings: tuple[TwoResolutionNavigationTargetObjectBindingV2, ...]
...
class TwoResolutionNavigationEpisodeAuthorityIssuerV2:
    ...
    def __init__(
        ...
        *,
        ...
        bindings = tuple(
            sorted(
                TwoResolutionNavigationTargetObjectBindingV2(semantic, task_object)
    ...
    def __copy__(self) -> "TwoResolutionNavigationEpisodeAuthorityIssuerV2":
    ...
    def issue(
        ...
        if state.record is not None:
            raise TwoResolutionNavigationIntegrationV2ReplayError(
    ...
    def _record(
        ...
        if type(authority) is not TwoResolutionNavigationEpisodeAuthorityV2:
...
@dataclass(frozen=True)
class TwoResolutionDevelopmentControllerClaimTraceV2:
    ...
    def __post_init__(self) -> None:
        ...
        if not all(
            ...
            raise TwoResolutionNavigationIntegrationV2BindingError(
                "V2 controller payload is structurally incomplete"
            )
        ...
        if (
            ...
            raise TwoResolutionNavigationIntegrationV2BindingError(
                "V2 controller payload differs from exact episode authority"
            )
        if self._integration is None:
            raise TypeError("V2 controller trace requires its exact integration")
    ...
    def raw_claim_trace(self) -> Mapping[str, object]:
        ...
        if type(value) is not dict:
            raise TwoResolutionNavigationIntegrationV2BindingError(
                "V2 controller payload lost its raw claim trace"
            )
        return value

    def to_dict(self, include_hash: bool = True) -> dict[str, object]:
        result: dict[str, object] = {
            "schema": "lewm_two_resolution_development_controller_claim_trace_v2",
    ...
    def _assert_structural_integrity(self) -> None:
        if self.content_sha256 != _sha256(self.to_dict(False)):
            raise TwoResolutionNavigationIntegrationV2BindingError(
                "V2 controller claim trace was mutated"
            )
    ...
    def assert_integrity(self) -> None:
        if type(self._integration) is not (
            ...
            raise TwoResolutionNavigationIntegrationV2BindingError(
                "V2 controller trace lost its exact integration"
            )
        self._integration.assert_controller_claim_trace(self)

    def __copy__(self) -> "TwoResolutionDevelopmentControllerClaimTraceV2":
        raise TypeError("V2 controller claim traces are non-copyable")

    def __deepcopy__(
        self,
        memo: object,
    ) -> "TwoResolutionDevelopmentControllerClaimTraceV2":
        del memo
        raise TypeError("V2 controller claim traces are non-copyable")

    def __reduce_ex__(self, protocol: int) -> object:
        del protocol
        raise TypeError("V2 controller claim traces are non-serializable")


@dataclass(frozen=True)
class TwoResolutionObserverClaimEvaluationV2:
    ...
    def __post_init__(self) -> None:
        ...
        if self._integration is None:
            raise TypeError("V2 observer result requires its exact integration")
    ...
    def to_dict(self, include_hash: bool = True) -> dict[str, object]:
        result: dict[str, object] = {
            "schema": "lewm_two_resolution_observer_claim_evaluation_v2",
    ...
    def _assert_structural_integrity(self) -> None:
        if self.content_sha256 != _sha256(self.to_dict(False)):
            raise TwoResolutionNavigationIntegrationV2BindingError(
                "V2 observer evaluation was mutated"
            )
    ...
    def assert_integrity(self) -> None:
        if type(self._integration) is not (
            ...
            raise TwoResolutionNavigationIntegrationV2BindingError(
                "V2 observer evaluation lost its exact integration"
            )
        self._integration.assert_observer_evaluation(self)

    def __copy__(self) -> "TwoResolutionObserverClaimEvaluationV2":
        raise TypeError("V2 observer claim evaluations are non-copyable")

    def __deepcopy__(
        self,
        memo: object,
    ) -> "TwoResolutionObserverClaimEvaluationV2":
        del memo
        raise TypeError("V2 observer claim evaluations are non-copyable")

    def __reduce_ex__(self, protocol: int) -> object:
        del protocol
        raise TypeError("V2 observer claim evaluations are non-serializable")


@dataclass(frozen=True)
class _ControllerRecordV2:
    artifact: TwoResolutionDevelopmentControllerClaimTraceV2
    original_content_sha256: str
    episode_authority: TwoResolutionNavigationEpisodeAuthorityV2


@dataclass(frozen=True)
class _ObserverRecordV2:
    result: TwoResolutionObserverClaimEvaluationV2
    original_content_sha256: str
...
class _IntegrationState:
    ...
    episode_issuer: TwoResolutionNavigationEpisodeAuthorityIssuerV2
    owner_state_sha256: str = ""
    issued: dict[int, _ControllerRecordV2] = field(default_factory=dict)
    observed_controller_ids: set[int] = field(default_factory=set)
    observer_results: dict[int, _ObserverRecordV2] = field(default_factory=dict)
...
def _integration_state_accessors():
    ...
    def resolve(owner: object) -> _IntegrationState:
        try:
            ...
            raise TwoResolutionNavigationIntegrationV2BindingError(
                "V2 integration has no exact live state"
            ) from exc
...
def _synthetic_transaction_fault(state: _IntegrationState, stage: str) -> None:
    if state.synthetic_fault_after_stage == stage:
        ...
        raise RuntimeError(f"synthetic V2 transaction fault after {stage}")
...
def _owner_state_sha256(state: _IntegrationState) -> str:
    ...
    return _sha256(
        {
            "schema": "lewm_two_resolution_navigation_owner_state_seal_v2",
            "g3_planner": {
...
def _transaction_owner_state_sha256(state: _IntegrationState) -> str:
    """Hash the complete mutable owner state, including allowed outcome ingress."""

    source = getattr(state.evidence_issuer, "_outcome_source")
    return _sha256(
        {
            "schema": "lewm_two_resolution_navigation_transaction_state_v2",
...
@dataclass(frozen=True)
class _OwnerTransactionSnapshotV2:
    ...
    @classmethod
    def capture(cls, state: _IntegrationState) -> "_OwnerTransactionSnapshotV2":
        evidence = state.evidence_issuer
    ...
    def restore(self, state: _IntegrationState) -> None:
        ...
        if restored != self.original_state_sha256:
            raise TwoResolutionNavigationIntegrationV2BindingError(
                "V2 transaction rollback did not restore exact owner state"
            )
...
def _assert_owner_state(state: _IntegrationState) -> None:
    ...
    if current != state.owner_state_sha256:
        raise TwoResolutionNavigationIntegrationV2BindingError(
            "shared downstream owner state changed outside this V2 coordinator"
        )


class TwoResolutionDevelopmentNavigationIntegrationV2:
    ...
    def __init__(
        ...
        *,
        ...
        episode_authority_issuer: TwoResolutionNavigationEpisodeAuthorityIssuerV2,
        _synthetic_development_fixture: bool = False,
        _synthetic_fault_after_stage: str | None = None,
    ) -> None:
        if _synthetic_development_fixture is not True:
            raise PermissionError(
                "no production two-resolution navigation integration V2 is configured"
            )
        ...
        if (
            ...
            raise ValueError("unknown synthetic V2 transaction fault stage")
        if (
    ...
    def __copy__(self) -> "TwoResolutionDevelopmentNavigationIntegrationV2":
        raise TypeError("V2 development integrations are non-copyable")
    ...
    def __reduce_ex__(self, protocol: int) -> object:
        del protocol
        raise TypeError("V2 development integrations are non-serializable")
    ...
    def _controller_record(
        ...
        state = _integration_state(self)
        _assert_owner_state(state)
        if type(artifact) is not TwoResolutionDevelopmentControllerClaimTraceV2:
        ...
        if (
            ...
            raise TwoResolutionNavigationIntegrationV2BindingError(
                "V2 controller claim trace is not the exact live object issued here"
            )
        artifact._assert_structural_integrity()
        if artifact.content_sha256 != row.original_content_sha256:
            raise TwoResolutionNavigationIntegrationV2BindingError(
                "V2 controller claim trace differs from its original issuance"
            )
        ...
        if row.episode_authority.content_sha256 != artifact.episode_authority_sha256:
            raise TwoResolutionNavigationIntegrationV2BindingError(
                "V2 episode authority changed after controller issuance"
            )
        return row
    ...
    def issue_controller_claim_trace(
        ...
        *,
        ...
        transaction = _OwnerTransactionSnapshotV2.capture(state)
        try:
            ...
            if selected is None:
                raise TwoResolutionNavigationIntegrationV2Error(
            ...
            payload = {
                "schema": "lewm_two_resolution_navigation_controller_payload_v2",
            ...
            try:
                ...
                raise TwoResolutionNavigationIntegrationV2BindingError(
                    "V2 transaction failed and exact rollback could not be proven"
                ) from rollback_error
            raise
        state.issued[id(artifact)] = _ControllerRecordV2(
            artifact=artifact,
    ...
    def evaluate_observer_only(
        ...
        *,
        ...
        if id(artifact) in state.observed_controller_ids:
            raise TwoResolutionNavigationIntegrationV2ReplayError(
                "V2 controller claim trace was already evaluated"
            )
        ...
        state.observer_results[id(result)] = _ObserverRecordV2(
            result=result,
    ...
    def _observer_record(
        ...
        *,
        ...
        state = _integration_state(self)
        _assert_owner_state(state)
        if type(result) is not TwoResolutionObserverClaimEvaluationV2:
        ...
        if row is None or row.result is not result or result._integration is not self:
            raise TwoResolutionNavigationIntegrationV2BindingError(
                "V2 observer result is not the exact live object issued here"
            )
        result._assert_structural_integrity()
        if result.content_sha256 != row.original_content_sha256:
            raise TwoResolutionNavigationIntegrationV2BindingError(
                "V2 observer result differs from its original issuance"
            )
        if id(result) in state.consumed_observer_result_ids:
            raise TwoResolutionNavigationIntegrationV2ReplayError(
                "V2 observer result was already consumed"
            )
        if consume:
...
def require_production_two_resolution_navigation_integration_v2() -> object:
    if PRODUCTION_TWO_RESOLUTION_NAVIGATION_INTEGRATION_V2 is None:
        raise PermissionError(
            "no production two-resolution navigation integration V2 is configured"
        )
    return PRODUCTION_TWO_RESOLUTION_NAVIGATION_INTEGRATION_V2
...
__all__ = [
    ...
    "TwoResolutionNavigationTargetObjectBindingV2",
    "TwoResolutionObserverClaimEvaluationV2",
