"""Standalone V5 development boundary for native learned physical evidence.

V5 owns its complete admission, issuance, commit, and retraction lifecycle.  It
...
PRODUCTION_NATIVE_V5_RUNNER = None
PRODUCTION_V5_CHECKPOINT_FILE_SHA256 = None
PRODUCTION_G2_REPORT_FILE_SHA256 = None
PRODUCTION_V5_CALIBRATION_SHA256 = None
PRODUCTION_NATIVE_LEARNED_PROJECTION_ADAPTER_V5 = None


@dataclass(frozen=True)
class QualifiedLearnedPhysicalDevelopmentAdmissionV5:
    admission_kind: str
    ...
    def __post_init__(self) -> None:
        if self.admission_kind not in {"projection", "retraction"}:
            raise ValueError("unsupported V5 development admission kind")
        for name in (
            "admission_id_sha256",
        ...
        expected_id = _sha256(
            {
                "schema": "lewm_g3_qualified_learned_admission_id_v5",
        ...
        if self.admission_id_sha256 != expected_id:
            raise NativeLearnedProjectionBindingError(
                "V5 development admission identity changed"
            )
    ...
    def to_dict(self, include_hash: bool = True) -> dict[str, object]:
        result: dict[str, object] = {
            "schema": "lewm_g3_qualified_learned_development_admission_v5",
    ...
    def assert_integrity(self) -> None:
        if self.content_sha256 != _sha256(self.to_dict(False)):
            raise NativeLearnedProjectionBindingError(
                "V5 development admission was mutated"
            )


@dataclass(frozen=True)
class QualifiedLearnedPhysicalDevelopmentTransactionV5:
    admission: QualifiedLearnedPhysicalDevelopmentAdmissionV5
    pose: PoseProvenance
    projection_receipt: NativeLearnedPhysicalProjectionReceiptV1
    retracts_observation_id: str | None
    _issuance_capability_v5: object = field(repr=False, compare=False)
    content_sha256: str = field(init=False)

    def __post_init__(self) -> None:
        if type(self.admission) is not QualifiedLearnedPhysicalDevelopmentAdmissionV5:
            raise TypeError("V5 transaction admission has the wrong type")
        if type(self.pose) is not PoseProvenance:
            raise TypeError("V5 transaction pose has the wrong type")
        if type(self.projection_receipt) is not NativeLearnedPhysicalProjectionReceiptV1:
            raise TypeError("V5 projection receipt has the wrong type")
        self._assert_bindings()
        if self._issuance_capability_v5 is None:
            raise TypeError("V5 transaction requires an issuance capability")
        object.__setattr__(self, "content_sha256", _sha256(self.to_dict(False)))
    ...
    def _assert_bindings(self) -> None:
        ...
        if self.pose.content_sha256 != self.admission.pose_sha256:
            raise NativeLearnedProjectionBindingError(
                "V5 transaction pose binding changed"
            )
        if (
            ...
            raise NativeLearnedProjectionBindingError(
                "V5 transaction projection receipt binding changed"
            )
        if self.retracts_observation_id != self.admission.retracts_observation_id:
            raise NativeLearnedProjectionBindingError(
                "V5 transaction retraction binding changed"
            )
    ...
    def to_dict(self, include_hash: bool = True) -> dict[str, object]:
        result: dict[str, object] = {
            "schema": "lewm_g3_qualified_learned_development_transaction_v5",
    ...
    def assert_integrity(self) -> None:
        ...
        if self.content_sha256 != _sha256(self.to_dict(False)):
            raise NativeLearnedProjectionBindingError(
                "V5 development transaction was mutated"
            )

    def __copy__(self) -> "QualifiedLearnedPhysicalDevelopmentTransactionV5":
        raise TypeError("V5 development transactions are non-copyable")
...
class _RetractionStateV5(str, Enum):
    LIVE = "live"
    STALE = "stale"
    CONSUMED = "consumed"
...
@dataclass
class _RetractionReservationV5:
    target_key: tuple[str, int]
    target_package: QualifiedLearnedPhysicalDevelopmentTransactionV5
    target_issued_content_sha256: str
    retraction_package: QualifiedLearnedPhysicalDevelopmentTransactionV5
    retraction_issued_content_sha256: str
    snapshot: TwoResolutionConfigurationSnapshotV2
    snapshot_issued_content_sha256: str
    state: _RetractionStateV5 = _RetractionStateV5.LIVE


class NativeLearnedPhysicalProjectionAdapterV5:
        ...
        *,
        ...
        if _synthetic_test_fixture is not True:
            raise PermissionError(
                "no production native learned physical projection V5 is configured"
            )
        ...
        if calibration is not FROZEN_SYNTHETIC_NATIVE_CALIBRATION_V1:
            raise NativeLearnedProjectionBindingError(
                "V5 requires the exact frozen synthetic calibration object"
            )
        ...
        if memory.config.promoted_runtime:
            raise PermissionError(
                "development learned projection V5 cannot target promoted memory"
            )
        if (
            not math.isclose(
        ...
        if runner_bindings != {
            ...
            raise NativeLearnedProjectionBindingError(
                "runner/checkpoint/G2/calibration identity differs from V5"
            )
        self.__memory_v5 = memory
        self.__projection_v5 = projection
        self.__runner_v5 = runner
        self.__calibration_v5 = calibration
        self.__expected_v5 = expected
        self.__contract_v5 = _sha256(
            {
                "schema": "lewm_g3_native_learned_projection_contract_v5",
                **expected,
                ...
                "v5_package_type_required": True,
                "permanent_rejection_identity_history": "seen_observation_ids_v1",
                "development_only": True,
                "hardware_execution_authorized": False,
                "production_promotion_authorized": False,
            }
        )
        self.__capability_v5 = object()
        self.__issued_v5: dict[
            int,
            tuple[QualifiedLearnedPhysicalDevelopmentTransactionV5, str],
        ] = {}
        self.__snapshots_v5: dict[
            int,
            tuple[TwoResolutionConfigurationSnapshotV2, str],
        ] = {}
        self.__consumed_v5: set[int] = set()
        self.__committed_v5: dict[
            str,
            QualifiedLearnedPhysicalDevelopmentTransactionV5,
        ] = {}
        self.__reservations_v5: dict[
            int,
            _RetractionReservationV5,
        ] = {}
        self.__live_by_target_v5: dict[tuple[str, int], int] = {}
        self.__sequence_v5 = 0

    def __copy__(self) -> "NativeLearnedPhysicalProjectionAdapterV5":
        raise TypeError("native learned projection V5 adapters are non-copyable")
    ...
    def __reduce_ex__(self, protocol: int) -> object:
        del protocol
        raise TypeError(
            "native learned projection V5 adapters are non-serializable"
        )

    @property
    def adapter_contract_sha256(self) -> str:
        return self.__contract_v5
    ...
    def _assert_outcome_bindings_v5(
        self,
        snapshot: TwoResolutionConfigurationSnapshotV2,
        outcome: SyntheticNativeV4RawOutcomeV1,
    ) -> None:
        self.__calibration_v5.assert_integrity()
        expected = self.__expected_v5
        if (
            ...
            != self.__calibration_v5.content_sha256
        ):
            raise NativeLearnedProjectionBindingError(
        ...
        if (
            ...
            or geometry.shape != self.__calibration_v5.source_shape
            or geometry.origin_forward_left_m
            != self.__calibration_v5.source_origin_forward_left_m
            or not math.isclose(
                geometry.cell_size_m,
                self.__calibration_v5.source_cell_size_m,
                rel_tol=0.0,
                abs_tol=1e-12,
            )
        ):
        ...
        if any(
            ...
            for value, limit in zip(
                diagonal,
                self.__calibration_v5.covariance_diagonal_max,
            )
        ):
            raise NativeLearnedProjectionRejectedError(
    ...
    def _uncertainty_transforms_v5(
        self,
        pose: PoseProvenance,
    ) -> tuple[Transform2, ...]:
        diagonal = tuple(
        ...
        for multiplier in self.__calibration_v5.pose_sigma_multipliers:
            pose_transform = (
            ...
            for camera_transform in (
                self.__calibration_v5.camera_local_uncertainty_transforms
            ):
                transforms.append(
    ...
    def _free_source_cells_v5(
        ...
        for row in outcome.ground_clear_query_tensor:
            ...
            known = tuple(
                ...
                >= self.__calibration_v5.known_probability_threshold
            ...
            if all(
                ...
                and is_known
                and probability
                >= self.__calibration_v5.free_given_known_probability_threshold
    ...
    def _selected_local_hits_v5(
        ...
        for row in outcome.ordered_ray_hit_depth_tensor:
            if any(
                depth < self.__calibration_v5.ray_depth_min_m - 1e-12
                or depth > self.__calibration_v5.ray_depth_max_m + 1e-12
            ...
            for logit, depth in zip(row.ordered_hit_logits, row.ordered_depth_m):
                ...
                if (
                    ...
                    >= self.__calibration_v5.known_probability_threshold
                    and first_hit
                    >= self.__calibration_v5.occupied_given_known_probability_threshold
                ):
    ...
    def _project_v5(
        ...
        transforms = self._uncertainty_transforms_v5(outcome.pose)
        free_source = self._free_source_cells_v5(outcome)
        ...
        for local_hit in self._selected_local_hits_v5(outcome):
            for transform in transforms:
                occupied.update(
                    _closed_point_supercover(
                        physical_frame,
        ...
        return NativeLearnedPhysicalProjectionReceiptV1(
            ...
            calibration_sha256=self.__calibration_v5.content_sha256,
    ...
    @staticmethod
    def _projection_payload_sha256_v5(
        receipt: NativeLearnedPhysicalProjectionReceiptV1,
    ) -> str:
        return _sha256(
            {
                "schema": "lewm_g3_native_learned_projection_payload_v5",
    ...
    def _build_physical_transaction_v5(
        self,
        *,
        admission_kind: str,
        observation_id: str,
        ...
        if admission_kind == "projection":
            ...
            if retracts_observation_id is None:
                raise NativeLearnedProjectionBindingError(
                    "V5 retraction transaction lost its target"
                )
            retractions = (retracts_observation_id,)
        else:
            raise NativeLearnedProjectionBindingError(
                "V5 development transaction kind changed"
            )
        return PhysicalEvidenceTransaction(
            ...
            map_frame=self.__memory_v5.map_frame,
            pose=pose,
    ...
    def _make_admission_v5(
        ...
        *,
        ...
        retracts_observation_id: str | None,
    ) -> QualifiedLearnedPhysicalDevelopmentAdmissionV5:
        observation = physical_transaction.observation
        core = {
            "schema": "lewm_g3_qualified_learned_admission_id_v5",
            "kind": admission_kind,
            "adapter_contract_sha256": self.__contract_v5,
            ...
            "memory_revision_before": self.__memory_v5.revision,
            "retracts_observation_id": retracts_observation_id,
        }
        return QualifiedLearnedPhysicalDevelopmentAdmissionV5(
            ...
            memory_config_sha256=self.__memory_v5.config.content_sha256,
            physical_map_frame_sha256=(
                self.__memory_v5.map_frame.content_sha256
            ),
            configuration_map_frame_sha256=(
                self.__projection_v5.configuration_map_frame.content_sha256
            ),
            physical_revision_before=self.__memory_v5.revision,
    ...
    def _register_v5(
        self,
        package: QualifiedLearnedPhysicalDevelopmentTransactionV5,
        snapshot: TwoResolutionConfigurationSnapshotV2,
    ) -> QualifiedLearnedPhysicalDevelopmentTransactionV5:
        self.__issued_v5[id(package)] = (package, package.content_sha256)
        self.__snapshots_v5[id(package)] = (
            snapshot,
    ...
    def issue(
        ...
        self.__projection_v5.assert_current_snapshot(snapshot)
        self.__runner_v5.assert_issued(outcome)
        self._assert_outcome_bindings_v5(snapshot, outcome)
        projection_receipt = self._project_v5(snapshot, outcome)
        physical_transaction = self._build_physical_transaction_v5(
            admission_kind="projection",
            observation_id=outcome.observation_id,
            observation_payload_sha256=(
                self._projection_payload_sha256_v5(projection_receipt)
            ),
            observation_producer_sha256=(
                outcome.runner_execution_identity_sha256
            ),
        ...
        admission = self._make_admission_v5(
            admission_kind="projection",
        ...
        package = QualifiedLearnedPhysicalDevelopmentTransactionV5(
            ...
            projection_receipt=projection_receipt,
            retracts_observation_id=None,
            _issuance_capability_v5=self.__capability_v5,
        )
        self.__runner_v5.assert_issued(outcome, consume=True)
        return self._register_v5(package, snapshot)

    def _assert_issued_digest_v5(
        self,
        package: QualifiedLearnedPhysicalDevelopmentTransactionV5,
    ) -> str:
        if type(package) is not QualifiedLearnedPhysicalDevelopmentTransactionV5:
            raise TypeError("package has the wrong V5 transaction type")
        issued = self.__issued_v5.get(id(package))
        if (
            package._issuance_capability_v5 is not self.__capability_v5
            or issued is None
            or issued[0] is not package
        ):
            raise NativeLearnedProjectionBindingError(
                "package is not the exact live object issued by this V5 adapter"
            )
        package.assert_integrity()
        if package.content_sha256 != issued[1]:
            raise NativeLearnedProjectionBindingError(
                "V5 package differs from its issued content"
            )
        return issued[1]

    def _assert_exact_package_v5(
        self,
        package: QualifiedLearnedPhysicalDevelopmentTransactionV5,
    ) -> TwoResolutionConfigurationSnapshotV2:
        self._assert_issued_digest_v5(package)
        if id(package) in self.__consumed_v5:
            raise NativeLearnedProjectionReplayError(
                "V5 development transaction was already consumed"
            )
        snapshot_record = self.__snapshots_v5[id(package)]
        snapshot, issued_snapshot_sha256 = snapshot_record
        if snapshot.content_sha256 != issued_snapshot_sha256:
            raise NativeLearnedProjectionBindingError(
                "V5 package snapshot differs from its issued content"
            )
        self.__projection_v5.assert_current_snapshot(snapshot)
        admission = package.admission
        if (
            admission.adapter_contract_sha256 != self.__contract_v5
            or admission.memory_config_sha256
            != self.__memory_v5.config.content_sha256
            or admission.physical_map_frame_sha256
            != self.__memory_v5.map_frame.content_sha256
            or admission.configuration_map_frame_sha256
            != self.__projection_v5.configuration_map_frame.content_sha256
            or admission.physical_revision_before != self.__memory_v5.revision
            ...
            raise NativeLearnedProjectionBindingError(
                "V5 transaction authority/frame/revision binding changed"
            )
        return snapshot
    ...
    def _commit_core_v5(
        ...
        self._assert_exact_package_v5(package)
        admission = package.admission
        reservation = self.__reservations_v5.get(id(package))
        if admission.admission_kind == "retraction":
            if reservation is None or reservation.retraction_package is not package:
                raise NativeLearnedProjectionBindingError(
                    "V5 retraction has no exact reservation"
                )
            self._assert_reservation_digests_v5(reservation, package)
            if reservation.state is _RetractionStateV5.STALE:
                raise NativeLearnedProjectionReplayError(
                    "V5 retraction package is terminally stale"
                )
            if reservation.state is _RetractionStateV5.CONSUMED:
                raise NativeLearnedProjectionReplayError(
                    "V5 retraction package was already consumed"
                )
            if reservation.state is not _RetractionStateV5.LIVE:
                raise NativeLearnedProjectionBindingError(
                    "V5 retraction reservation state is invalid"
                )
            self._assert_reservation_target_active_v5(reservation)
        elif reservation is not None:
            raise NativeLearnedProjectionBindingError(
                "V5 projection unexpectedly owns a retraction reservation"
            )
        physical_transaction = self._build_physical_transaction_v5(
        ...
        if (
            ...
            raise NativeLearnedProjectionBindingError(
                "V5 reconstructed physical transaction changed"
            )
        try:
            receipt = self.__memory_v5.apply_transaction(physical_transaction)
        except SnapshotBindingError:
            if reservation is not None:
                self._mark_reservation_terminal_v5(
                    reservation,
                    _RetractionStateV5.STALE,
                )
            raise
        except TransactionRejectedError:
            # Accepted identities remain indelible after their evidence is
            # retracted. History membership proves this exact package can never
            # succeed; unrelated failures preserve it for retry.
            if (
                reservation is not None
                and admission.observation_id
                in self.__memory_v5.seen_observation_ids
            ):
                self._mark_reservation_terminal_v5(
                    reservation,
                    _RetractionStateV5.STALE,
            ...
            raise
        self.__consumed_v5.add(id(package))
        if admission.admission_kind == "projection":
            self.__committed_v5[admission.observation_id] = package
        else:
            target = admission.retracts_observation_id
            if target is not None:
                self.__committed_v5.pop(target, None)
            if reservation is None:
                raise NativeLearnedProjectionBindingError(
                    "V5 committed retraction lost its reservation"
                )
            self._mark_reservation_terminal_v5(
                reservation,
                _RetractionStateV5.CONSUMED,
            )
        return receipt

    def _assert_exact_active_target_v5(
        self,
        package: QualifiedLearnedPhysicalDevelopmentTransactionV5,
    ) -> tuple[str, int]:
        self._assert_issued_digest_v5(package)
        ...
        if (
            ...
            or self.__committed_v5.get(observation_id) is not package
            or id(package) not in self.__consumed_v5
            or observation_id
            not in self.__memory_v5.learned_observation_ids
        ):
            raise NativeLearnedProjectionBindingError(
                "retraction target is not an exact active V5 projection"
            )
        return observation_id, id(package)

    def _assert_reservation_digests_v5(
        self,
        record: _RetractionReservationV5,
        package: QualifiedLearnedPhysicalDevelopmentTransactionV5,
    ) -> None:
        target_digest = self._assert_issued_digest_v5(record.target_package)
        if target_digest != record.target_issued_content_sha256:
            raise NativeLearnedProjectionBindingError(
                "V5 committed projection differs from its issued content"
            )
        if record.retraction_package is not package:
            raise NativeLearnedProjectionBindingError(
                "V5 reservation does not own this exact retraction package"
            )
        retraction_digest = self._assert_issued_digest_v5(package)
        if retraction_digest != record.retraction_issued_content_sha256:
            raise NativeLearnedProjectionBindingError(
                "V5 retraction differs from its issued content"
            )

    def _assert_reservation_target_active_v5(
        self,
        record: _RetractionReservationV5,
    ) -> None:
        observation_id = record.target_key[0]
        if (
            self.__committed_v5.get(observation_id)
            is not record.target_package
            or observation_id
            not in self.__memory_v5.learned_observation_ids
        ):
            raise NativeLearnedProjectionBindingError(
                "V5 retraction target is no longer exact active evidence"
            )

    def _reservation_snapshot_is_current_v5(
        self,
        record: _RetractionReservationV5,
    ) -> bool:
        if record.snapshot.content_sha256 != record.snapshot_issued_content_sha256:
            raise NativeLearnedProjectionBindingError(
                "V5 live reservation snapshot differs from issued content"
            )
        try:
            self.__projection_v5.assert_current_snapshot(record.snapshot)
        except SnapshotBindingError:
            return False
        self._assert_reservation_digests_v5(
            record,
            record.retraction_package,
        )
        self._assert_reservation_target_active_v5(record)
        return True

    def _mark_reservation_terminal_v5(
        self,
        record: _RetractionReservationV5,
        state: _RetractionStateV5,
    ) -> None:
        if state not in (_RetractionStateV5.STALE, _RetractionStateV5.CONSUMED):
            raise ValueError("V5 terminal state must be stale or consumed")
        if record.state is not _RetractionStateV5.LIVE:
            return
        record.state = state
        live_package_id = self.__live_by_target_v5.get(record.target_key)
        if live_package_id == id(record.retraction_package):
            self.__live_by_target_v5.pop(record.target_key, None)

    def issue_retraction(
        self,
        snapshot: TwoResolutionConfigurationSnapshotV2,
        committed_projection: QualifiedLearnedPhysicalDevelopmentTransactionV5,
    ) -> QualifiedLearnedPhysicalDevelopmentTransactionV5:
        self.__projection_v5.assert_current_snapshot(snapshot)
        target_key = self._assert_exact_active_target_v5(committed_projection)
        live_package_id = self.__live_by_target_v5.get(target_key)
        if live_package_id is not None:
            record = self.__reservations_v5[live_package_id]
            if record.state is not _RetractionStateV5.LIVE:
                raise NativeLearnedProjectionBindingError(
                    "V5 live index contains a terminal reservation"
                )
            if self._reservation_snapshot_is_current_v5(record):
                raise NativeLearnedProjectionReplayError(
                    "an exact live V5 retraction already exists for this target"
                )
            self._mark_reservation_terminal_v5(
                record,
                _RetractionStateV5.STALE,
        ...
        self.__sequence_v5 += 1
        observation_id = committed_projection.observation_id
        retraction_observation_id = (
            f"qualified-native-v4-v5-retract:{self.__sequence_v5}:"
            f"{observation_id}"
        )
        payload_sha256 = _sha256(
            {
                "schema": "lewm_g3_qualified_learned_retraction_payload_v5",
        ...
        physical_transaction = self._build_physical_transaction_v5(
            ...
            observation_producer_sha256=self.__contract_v5,
        ...
        self._register_v5(package, snapshot)
        record = _RetractionReservationV5(
            target_key=target_key,
            target_package=committed_projection,
            target_issued_content_sha256=(
                self.__issued_v5[id(committed_projection)][1]
            ),
            retraction_package=package,
            retraction_issued_content_sha256=(
                self.__issued_v5[id(package)][1]
            ),
            snapshot=snapshot,
            snapshot_issued_content_sha256=snapshot.content_sha256,
        )
        self.__reservations_v5[id(package)] = record
        self.__live_by_target_v5[target_key] = id(package)
        return package
    ...
    def commit(
        ...
        record = self.__reservations_v5.get(id(package))
        if record is None:
            if (
                type(package)
                is QualifiedLearnedPhysicalDevelopmentTransactionV5
                and package.admission.admission_kind == "retraction"
            ):
                raise NativeLearnedProjectionBindingError(
                    "V5 retraction has no exact reservation"
                )
            return self._commit_core_v5(package)
        self._assert_reservation_digests_v5(record, package)
        if record.state is _RetractionStateV5.STALE:
            raise NativeLearnedProjectionReplayError(
                "V5 retraction package is terminally stale"
            )
        if record.state is _RetractionStateV5.CONSUMED:
            raise NativeLearnedProjectionReplayError(
                "V5 retraction package was already consumed"
            )
        if record.state is not _RetractionStateV5.LIVE:
...
def require_production_native_learned_projection_adapter_v5() -> object:
    if (
        PRODUCTION_NATIVE_V5_RUNNER is None
        or PRODUCTION_V5_CHECKPOINT_FILE_SHA256 is None
        or PRODUCTION_G2_REPORT_FILE_SHA256 is None
        or PRODUCTION_V5_CALIBRATION_SHA256 is None
        or PRODUCTION_NATIVE_LEARNED_PROJECTION_ADAPTER_V5 is None
    ):
        raise PermissionError(
            "production native learned-projection V5 identities are unset"
        )
    return PRODUCTION_NATIVE_LEARNED_PROJECTION_ADAPTER_V5
