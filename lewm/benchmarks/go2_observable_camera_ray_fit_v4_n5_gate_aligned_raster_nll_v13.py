"""Static contract for the authority-free N5 gate-aligned raster-NLL V13.

V13 deliberately has no execution token, authority object, capability, issuer,
...
RETAINED_GATE_ADAPTER_VERSION = (
    "v13_native_diagnostics_excluded_then_hierarchical_to_ordered_key_v1"
)
THREAD_ENVIRONMENT = ("OMP_NUM_THREADS", "MKL_NUM_THREADS", "OPENBLAS_NUM_THREADS", "NUMEXPR_NUM_THREADS")

SOURCE_REVIEW_SCHEMA = "lewm_go2_observable_camera_ray_fit_v4_n5_gate_aligned_raster_nll_v13_source_review_v1"
RESERVATION_SCHEMA = "lewm_go2_observable_camera_ray_fit_v4_n5_gate_aligned_raster_nll_v13_reservation_v1"
RESULT_SCHEMA = "lewm_go2_observable_camera_ray_fit_v4_n5_gate_aligned_raster_nll_v13_result_v1"
COMPLETION_SCHEMA = "lewm_go2_observable_camera_ray_fit_v4_n5_gate_aligned_raster_nll_v13_completion_v1"
FAILURE_SCHEMA = "lewm_go2_observable_camera_ray_fit_v4_n5_gate_aligned_raster_nll_v13_failure_v1"
METRIC_RECEIPT_SCHEMA = "lewm_go2_observable_camera_ray_fit_v4_n5_gate_aligned_raster_nll_v13_metric_verification_v1"
GATE_SCHEMA = "lewm_go2_observable_camera_ray_fit_v4_n5_gate_aligned_raster_nll_v13_gate_v1"
VERIFICATION_REQUEST_SCHEMA = (
    "lewm_go2_observable_camera_ray_fit_v4_n5_gate_aligned_raster_nll_v13_"
    "verifier_request_v1"
)
...
ATTEMPT_SCOPE = "one_exclusive_fresh_gate_aligned_raster_nll_v13_attempt"
...
def owned_directory_transaction_contract() -> dict[str, Any]:
    """V8's reviewed mutation protocol, retained unchanged for V13."""

    return {
        "scope": "every_mutation_at_or_below_the_exclusive_v13_output_root",
...
def experiment_contract() -> dict[str, Any]:
    return {
        ...
        "additive_v13_loss_component": ADDITIVE_LOSS_COMPONENT,
...
def licenses() -> dict[str, bool]:
    return {
        ...
        "authorizes_later_v13_training": False,
        "authorizes_g2": False,
...
def expected_source_review_core(
    *,
    ...
    return {
        ...
        "status": (
            "different_agent_review_passed_n5_gate_aligned_raster_nll_v13_"
        ...
        "authority_boundary": {
            ...
            "scope": "one_exclusive_fresh_gate_aligned_raster_nll_v13_attempt",
            "retry_authorized": False,
            "scientific_retry_authorized": False,
...
def _validate_exact_successor_review_bindings(
        ...
        for relative in expected_paths:
            ...
            if type(binding) is not dict or set(binding) != expected_fields:
                raise PermissionError(
                    f"N5 gate-aligned raster-NLL V13 {role} binding fields changed"
                )
            path_value = binding["path"]
            digest_value = binding["file_sha256"]
            if type(path_value) is not str or type(digest_value) is not str:
                raise PermissionError(
                    f"N5 gate-aligned raster-NLL V13 {role} binding values changed"
                )
            if not is_sha256(digest_value):
                raise PermissionError(
                    f"N5 gate-aligned raster-NLL V13 {role} binding digest changed"
                )
    ...
    if len(set(bound_paths)) != len(bound_paths):
        raise PermissionError(
            "N5 gate-aligned raster-NLL V13 review contains duplicate binding paths"
        )
    ...
    for role, relative, binding in staged:
        ...
        if path_value != relative:
            raise PermissionError(
                f"N5 gate-aligned raster-NLL V13 {role} binding path or role changed"
            )
        checked = {
            "path": path_value,
...
def preflight_source_review(path: Path, file_sha256: str) -> tuple[dict[str, Any], bytes]:
    ...
    if supplied != canonical:
        raise PermissionError("N5 gate-aligned raster-NLL V13 review path is not canonical")
    review, raw = load_hashed_json(
        canonical,
        file_sha256,
        name="N5 gate-aligned raster-NLL V13 different-agent source review",
    )
    ...
    if (
        ...
        or reviewer
        in {"/root", AMENDMENT_AUTHOR, IMPLEMENTATION_AUTHOR, V12_REVIEWER}
        ...
        raise PermissionError(
            "N5 gate-aligned raster-NLL V13 review is not by an independent agent"
        )
    ...
    if core != expected or canonical_json_sha256(core) != declared:
        raise PermissionError("N5 gate-aligned raster-NLL V13 review contract changed")
    for relative, binding in checked.items():
        source_raw = read_regular_bytes(ROOT / relative, name=f"V13 source {relative}")
        if hashlib.sha256(source_raw).hexdigest() != binding["file_sha256"]:
            raise PermissionError(
                f"N5 gate-aligned raster-NLL V13 source changed: {relative}"
            )
    for relative, binding in checked_proofs.items():
        proof_raw = read_regular_bytes(ROOT / relative, name=f"V13 proof {relative}")
        if hashlib.sha256(proof_raw).hexdigest() != binding["file_sha256"]:
            raise PermissionError(
                f"N5 gate-aligned raster-NLL V13 proof changed: {relative}"
            )
    return review, raw
...
def source_review_binding(review: Mapping[str, Any], file_sha256: str) -> dict[str, str]:
    if not is_sha256(file_sha256) or not is_sha256(review.get("content_sha256")):
        raise ValueError("N5 gate-aligned raster-NLL V13 review binding is malformed")
...
def _validate_native_v13_objective(
    value: object,
    *,
    ...
    if not isinstance(value, Mapping) or set(value) != expected:
        raise ValueError(f"{name} native V13 objective fields changed")
    normalized: dict[str, float] = {}
    for key in ("derived_raster_cell_nll", "v11_base_total", "total"):
        item = value[key]
        if isinstance(item, bool) or not isinstance(item, (int, float)):
            raise ValueError(f"{name} native V13 objective {key} is not numeric")
        normalized[key] = float(item)
        if not math.isfinite(normalized[key]) or normalized[key] < 0.0:
            raise ValueError(f"{name} native V13 objective {key} is invalid")
    if not math.isclose(
    ...
    if not math.isclose(
        ...
        raise ValueError(f"{name} native V13 objective arithmetic is inconsistent")
    return normalized
...
def _validate_training_snapshot(value: object, *, name: str) -> dict[str, Any]:
    ...
    if set(components) != set(LOSS_COMPONENTS):
        raise ValueError(f"{name} V13 component fields changed")
    ...
    native = _validate_native_v13_objective(
        {
...
def _validate_training_record(value: object) -> dict[str, Any]:
    ...
    if not isinstance(value, Mapping) or set(value) != expected_fields:
        raise ValueError("N5 gate-aligned raster-NLL V13 training fields changed")
    required = {
        "steps": 4000,
        "batch_size": 5,
        "evaluation_batch_size": 1,
    ...
    if any(value.get(key) != expected for key, expected in required.items()):
        raise PermissionError(
            "N5 gate-aligned raster-NLL V13 optimizer/exposure contract changed"
        )
    trace = value.get("trace")
    if not isinstance(trace, list) or len(trace) != 41:
        raise ValueError("N5 gate-aligned raster-NLL V13 diagnostic trace changed")
    checked_trace = [
        _validate_training_snapshot(row, name=f"V13 training trace[{index}]")
        for index, row in enumerate(trace)
    ]
    if tuple(row["step"] for row in checked_trace) != _expected_diagnostic_updates():
        raise PermissionError("N5 gate-aligned raster-NLL V13 trace indices changed")
    initial = _validate_training_snapshot(value.get("initial"), name="V13 initial")
    final = _validate_training_snapshot(value.get("final"), name="V13 final")
    if initial != checked_trace[0] or final != checked_trace[-1] or final["step"] != 4000:
        raise PermissionError(
            "N5 gate-aligned raster-NLL V13 did not select only update 4000"
        )
    return dict(value)
...
def validate_evaluation_structure(value: object) -> dict[str, Any]:
    if not isinstance(value, Mapping) or set(value) != {
        ...
        raise ValueError("N5 gate-aligned raster-NLL V13 evaluation controls changed")
    expected = {
    ...
    for key, (control, mapping, degenerate) in expected.items():
        ...
        if not isinstance(row, Mapping) or set(row) != {
            ...
            raise ValueError(f"N5 gate-aligned raster-NLL V13 {key} fields changed")
        if (
            ...
            raise ValueError(f"N5 gate-aligned raster-NLL V13 {key} control changed")
        retained_losses = _validate_loss_record(
            row["losses"], name=f"V13 {key} retained V11"
        )
        native = _validate_native_v13_objective(
            row["native_v13_objective"],
            retained_losses=retained_losses,
            name=f"V13 {key}",
        )
        diagnostics = _validate_raster_nll_diagnostics(
            row["raster_nll_diagnostics"], name=f"V13 {key}"
        )
        metric_raster = row["metrics"].get("derived_raster")
        if not isinstance(metric_raster, Mapping):
            raise ValueError(f"V13 {key} retained raster metrics are malformed")
        ...
        if (
            ...
            raise ValueError(f"V13 {key} raster NLL recomputations disagree")
...
def adapt_native_v13_evaluation_for_retained_v4_gate(
    ...
    for control, gate_name in (
        ...
        for field in (
            ...
            if adapted_row[field] != original_row[field]:
                raise ValueError(f"V13 compatibility view changed {field}")
        from lewm.benchmarks import (
    ...
    if canonical_json_sha256(value) != original_sha256:
        raise ValueError("V13 retained-gate adapter mutated its caller")
    return adapted
...
def reconstruct_retained_v4_gate(
    ...
    *,
    ...
    if isinstance(fit_size, bool) or fit_size != 5:
        raise ValueError("V13 retained-gate reconstruction requires fit_size=5")
    compatibility_view = adapt_native_v13_evaluation_for_retained_v4_gate(
        evaluation
    )
    from lewm.benchmarks import (
        go2_observable_camera_ray_fit_v4_ladder_gate as retained_gate,
    ...
    if numeric.get("check_count") != 26 or len(numeric.get("checks", ())) != 26:
        raise ValueError("V13 retained N5 gate did not produce exactly 26 checks")
...
def validate_reservation_structure(reservation: Mapping[str, Any], *, expected_source_review: Mapping[str, str]) -> dict[str, Any]:
    ...
    declared = core.pop("content_sha256", None)
    if not is_sha256(declared) or canonical_json_sha256(core) != declared:
        raise ValueError("N5 gate-aligned raster-NLL V13 reservation hash changed")
    ...
    if not isinstance(recovery, list) or any(not isinstance(item, Mapping) for item in recovery):
        raise ValueError("N5 gate-aligned raster-NLL V13 recovery ledger is malformed")
    ...
    if (
        ...
        raise ValueError("N5 gate-aligned raster-NLL V13 reservation contract changed")
...
def validate_result_structure(result: Mapping[str, Any], *, expected_source_review: Mapping[str, str] | None = None) -> dict[str, Any]:
    ...
    if set(result) != expected_fields or result.get("schema") != RESULT_SCHEMA:
        raise ValueError("N5 gate-aligned raster-NLL V13 result schema changed")
    core = dict(result)
    declared = core.pop("content_sha256", None)
    if not is_sha256(declared) or canonical_json_sha256(core) != declared:
        raise ValueError("N5 gate-aligned raster-NLL V13 result hash changed")
    if (
        result.get("mode")
        != "exact_train_only_n5_gate_aligned_raster_nll_v13_development_fit"
        ...
        raise PermissionError("N5 gate-aligned raster-NLL V13 result crossed scope")
    ...
    if (
        ...
        raise PermissionError("N5 gate-aligned raster-NLL V13 review binding changed")
    if expected_source_review is not None and dict(review) != dict(
        expected_source_review
    ):
        raise PermissionError("N5 gate-aligned raster-NLL V13 binds another review")
    ...
    if (
        ...
        raise PermissionError("N5 gate-aligned raster-NLL V13 attempt scope changed")
    ...
    if (
        ...
        raise PermissionError("N5 gate-aligned raster-NLL V13 subset/target changed")
    if result.get("inputs") != _expected_exact_inputs(review):
        raise PermissionError("N5 gate-aligned raster-NLL V13 inputs changed")
    ...
    if (
        ...
        raise PermissionError("N5 gate-aligned raster-NLL V13 model binding changed")
    ...
    if (
        ...
        raise PermissionError("N5 gate-aligned raster-NLL V13 resource changed")
    ...
    if (
        ...
        raise PermissionError("N5 gate-aligned raster-NLL V13 determinism changed")
    ...
    if (
        ...
        raise PermissionError("N5 gate-aligned raster-NLL V13 access crossed scope")
    ...
    if licenses_value != expected_licenses:
        raise PermissionError("N5 gate-aligned raster-NLL V13 licenses changed")
    # The V13 validator above owns the complete hierarchical result schema.
    # A retained-V1 result shadow would leak the legacy-name compatibility view
    # outside the metric-gate stack, which the V13 amendment explicitly forbids.
    return dict(result)
