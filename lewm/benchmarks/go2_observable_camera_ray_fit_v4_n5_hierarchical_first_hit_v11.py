"""Static contract for the authority-free N5 hierarchical-first-hit V11.

V11 deliberately has no execution token, authority object, capability, issuer,
...
IMPLEMENTATION_AUTHOR = "/root/camera_v10_later_rung_plan"
AMENDMENT_AUTHOR = "/root/camera_v10_gate_loss_diagnosis"
...
RETAINED_GATE_ADAPTER_VERSION = (
    "hierarchical_first_hit_to_retained_ordered_key_only_v1"
)
THREAD_ENVIRONMENT = ("OMP_NUM_THREADS", "MKL_NUM_THREADS", "OPENBLAS_NUM_THREADS", "NUMEXPR_NUM_THREADS")

SOURCE_REVIEW_SCHEMA = "lewm_go2_observable_camera_ray_fit_v4_n5_hierarchical_first_hit_v11_source_review_v1"
RESERVATION_SCHEMA = "lewm_go2_observable_camera_ray_fit_v4_n5_hierarchical_first_hit_v11_reservation_v1"
RESULT_SCHEMA = "lewm_go2_observable_camera_ray_fit_v4_n5_hierarchical_first_hit_v11_result_v1"
COMPLETION_SCHEMA = "lewm_go2_observable_camera_ray_fit_v4_n5_hierarchical_first_hit_v11_completion_v1"
FAILURE_SCHEMA = "lewm_go2_observable_camera_ray_fit_v4_n5_hierarchical_first_hit_v11_failure_v1"
METRIC_RECEIPT_SCHEMA = "lewm_go2_observable_camera_ray_fit_v4_n5_hierarchical_first_hit_v11_metric_verification_v1"
GATE_SCHEMA = "lewm_go2_observable_camera_ray_fit_v4_n5_hierarchical_first_hit_v11_gate_v1"
VERIFICATION_REQUEST_SCHEMA = (
    "lewm_go2_observable_camera_ray_fit_v4_n5_hierarchical_first_hit_v11_"
    "verifier_request_v1"
)
...
ATTEMPT_SCOPE = "one_exclusive_fresh_hierarchical_first_hit_v11_attempt"
...
def owned_directory_transaction_contract() -> dict[str, Any]:
    """V8's reviewed mutation protocol, retained unchanged for V11."""

    return {
        "scope": "every_mutation_at_or_below_the_exclusive_v11_output_root",
...
def authority_bindings() -> dict[str, Any]:
    return {
        "v11_hierarchical_first_hit_amendment": {
            "path": V11_AMENDMENT_RELATIVE_PATH,
            "file_sha256": V11_AMENDMENT_FILE_SHA256,
        },
...
def licenses() -> dict[str, bool]:
    return {
        ...
        "authorizes_later_v11_training": False,
        "authorizes_g2": False,
...
def preflight_static_authority() -> dict[str, Any]:
    ...
    _hash_file(
        V11_AMENDMENT_RELATIVE_PATH,
        V11_AMENDMENT_FILE_SHA256,
        name="V11 hierarchical-first-hit amendment",
    )
...
def expected_source_review_core(
    *,
    ...
    return {
        ...
        "status": (
            "different_agent_review_passed_n5_hierarchical_first_hit_v11_"
        ...
        "authority_boundary": {
            ...
            "scope": "one_exclusive_fresh_hierarchical_first_hit_v11_attempt",
            "retry_authorized": False,
            "scientific_retry_authorized": False,
...
def preflight_source_review(path: Path, file_sha256: str) -> tuple[dict[str, Any], bytes]:
    ...
    if supplied != canonical:
        raise PermissionError("N5 hierarchical-first-hit V11 review path is not canonical")
    review, raw = load_hashed_json(
        canonical,
        file_sha256,
        name="N5 hierarchical-first-hit V11 different-agent source review",
    )
    ...
    if (
        ...
        raise PermissionError(
            "N5 hierarchical-first-hit V11 review is not by an independent agent"
        )
    ...
    for relative in SUCCESSOR_SOURCE_PATHS:
        ...
        if not isinstance(binding, Mapping) or binding.get("path") != relative or not is_sha256(binding.get("file_sha256")):
            raise PermissionError("N5 hierarchical-first-hit V11 source binding changed")
        source_raw = read_regular_bytes(ROOT / relative, name=f"V11 source {relative}")
        if hashlib.sha256(source_raw).hexdigest() != binding["file_sha256"]:
            raise PermissionError(f"N5 hierarchical-first-hit V11 source changed: {relative}")
    ...
    for relative in SUCCESSOR_PROOF_PATHS:
        ...
        if (
            ...
            raise PermissionError("N5 hierarchical-first-hit V11 proof binding changed")
        proof_raw = read_regular_bytes(ROOT / relative, name=f"V11 proof {relative}")
        if hashlib.sha256(proof_raw).hexdigest() != binding["file_sha256"]:
            raise PermissionError(f"N5 hierarchical-first-hit V11 proof changed: {relative}")
    ...
    if core != expected or canonical_json_sha256(core) != declared:
        raise PermissionError("N5 hierarchical-first-hit V11 review contract changed")
    return review, raw
...
def source_review_binding(review: Mapping[str, Any], file_sha256: str) -> dict[str, str]:
    if not is_sha256(file_sha256) or not is_sha256(review.get("content_sha256")):
        raise ValueError("N5 hierarchical-first-hit V11 review binding is malformed")
...
def _validate_training_record(value: object) -> dict[str, Any]:
    ...
    if not isinstance(value, Mapping) or set(value) != expected_fields:
        raise ValueError("N5 hierarchical-first-hit V11 training fields changed")
    required = {
        "steps": 4000,
        "batch_size": 5,
        "evaluation_batch_size": 1,
    ...
    if any(value.get(key) != expected for key, expected in required.items()):
        raise PermissionError(
            "N5 hierarchical-first-hit V11 optimizer/exposure contract changed"
        )
    trace = value.get("trace")
    if not isinstance(trace, list) or len(trace) != 41:
        raise ValueError("N5 hierarchical-first-hit V11 diagnostic trace changed")
    checked_trace = [
        _validate_training_snapshot(row, name=f"V11 training trace[{index}]")
        for index, row in enumerate(trace)
    ]
    if tuple(row["step"] for row in checked_trace) != _expected_diagnostic_updates():
        raise PermissionError("N5 hierarchical-first-hit V11 trace indices changed")
    initial = _validate_training_snapshot(value.get("initial"), name="V11 initial")
    final = _validate_training_snapshot(value.get("final"), name="V11 final")
    if initial != checked_trace[0] or final != checked_trace[-1] or final["step"] != 4000:
        raise PermissionError(
            "N5 hierarchical-first-hit V11 did not select only update 4000"
        )
    return dict(value)
...
def validate_evaluation_structure(value: object) -> dict[str, Any]:
    if not isinstance(value, Mapping) or set(value) != {
        ...
        raise ValueError("N5 hierarchical-first-hit V11 evaluation controls changed")
    expected = {
    ...
    for key, (control, mapping, degenerate) in expected.items():
        ...
        if not isinstance(row, Mapping) or set(row) != {
            ...
            raise ValueError(f"N5 hierarchical-first-hit V11 {key} fields changed")
        if (
            ...
            raise ValueError(f"N5 hierarchical-first-hit V11 {key} control changed")
        _validate_loss_record(row["losses"], name=f"V11 {key}")
...
def adapt_hierarchical_evaluation_for_retained_v4_gate(
    value: Mapping[str, Any],
) -> dict[str, Any]:
    """Return the sole private name-only view accepted by the retained gate."""
    ...
    for control, gate_name in (
        ...
        if "ordered_first_hit_nll" in adapted_losses:
            raise ValueError("V11 compatibility input already has the retired key")
        removed = adapted_losses.pop("hierarchical_first_hit_nll")
        if removed != source_value:
            raise ValueError("V11 compatibility copy changed the first-hit value")
        adapted_losses["ordered_first_hit_nll"] = source_value
        if canonical_json_sha256(original_row["metrics"]) != canonical_json_sha256(
            adapted_row["metrics"]
        ):
            raise ValueError("V11 compatibility view changed metric bytes")
        for field in (
            "control",
            "wrong_rgb_degenerate_singleton",
            "image_index_mapping",
            ...
            if adapted_row[field] != original_row[field]:
                raise ValueError(f"V11 compatibility view changed {field}")
        from lewm.benchmarks import (
    ...
    if canonical_json_sha256(value) != original_sha256:
        raise ValueError("V11 retained-gate adapter mutated its caller")
    return adapted
...
def reconstruct_retained_v4_gate(
    ...
    *,
    ...
    if isinstance(fit_size, bool) or fit_size != 5:
        raise ValueError("V11 retained-gate reconstruction requires fit_size=5")
    compatibility_view = adapt_hierarchical_evaluation_for_retained_v4_gate(
        evaluation
    )
    from lewm.benchmarks import (
        go2_observable_camera_ray_fit_v4_ladder_gate as retained_gate,
    ...
    if numeric.get("check_count") != 26 or len(numeric.get("checks", ())) != 26:
        raise ValueError("V11 retained N5 gate did not produce exactly 26 checks")
...
def validate_reservation_structure(reservation: Mapping[str, Any], *, expected_source_review: Mapping[str, str]) -> dict[str, Any]:
    ...
    declared = core.pop("content_sha256", None)
    if not is_sha256(declared) or canonical_json_sha256(core) != declared:
        raise ValueError("N5 hierarchical-first-hit V11 reservation hash changed")
    ...
    if not isinstance(recovery, list) or any(not isinstance(item, Mapping) for item in recovery):
        raise ValueError("N5 hierarchical-first-hit V11 recovery ledger is malformed")
    ...
    if (
        ...
        raise ValueError("N5 hierarchical-first-hit V11 reservation contract changed")
...
def validate_result_structure(result: Mapping[str, Any], *, expected_source_review: Mapping[str, str] | None = None) -> dict[str, Any]:
    ...
    if set(result) != expected_fields or result.get("schema") != RESULT_SCHEMA:
        raise ValueError("N5 hierarchical-first-hit V11 result schema changed")
    core = dict(result)
    declared = core.pop("content_sha256", None)
    if not is_sha256(declared) or canonical_json_sha256(core) != declared:
        raise ValueError("N5 hierarchical-first-hit V11 result hash changed")
    if (
        result.get("mode")
        != "exact_train_only_n5_hierarchical_first_hit_v11_development_fit"
        ...
        raise PermissionError("N5 hierarchical-first-hit V11 result crossed scope")
    ...
    if (
        ...
        raise PermissionError("N5 hierarchical-first-hit V11 review binding changed")
    if expected_source_review is not None and dict(review) != dict(
        expected_source_review
    ):
        raise PermissionError("N5 hierarchical-first-hit V11 binds another review")
    ...
    if (
        ...
        raise PermissionError("N5 hierarchical-first-hit V11 attempt scope changed")
    ...
    if (
        ...
        raise PermissionError("N5 hierarchical-first-hit V11 subset/target changed")
    if result.get("inputs") != _expected_exact_inputs(review):
        raise PermissionError("N5 hierarchical-first-hit V11 inputs changed")
    ...
    if (
        ...
        raise PermissionError("N5 hierarchical-first-hit V11 model binding changed")
    ...
    if (
        ...
        raise PermissionError("N5 hierarchical-first-hit V11 resource changed")
    ...
    if (
        ...
        raise PermissionError("N5 hierarchical-first-hit V11 determinism changed")
    ...
    if (
        ...
        raise PermissionError("N5 hierarchical-first-hit V11 access crossed scope")
    ...
    if licenses_value != expected_licenses:
        raise PermissionError("N5 hierarchical-first-hit V11 licenses changed")
    # The V11 validator above owns the complete hierarchical result schema.
    # A retained-V1 result shadow would leak the legacy-name compatibility view
    # outside the metric-gate stack, which the V11 amendment explicitly forbids.
    return dict(result)
