"""Static contract for the authority-free V5 N5 full-panel executor.

V5 deliberately has no execution token, authority object, capability, issuer,
...
SOURCE_REVIEW_SCHEMA = "lewm_go2_observable_camera_ray_fit_v4_n5_full_panel_v5_source_review_v1"
RESERVATION_SCHEMA = "lewm_go2_observable_camera_ray_fit_v4_n5_full_panel_v5_reservation_v1"
RESULT_SCHEMA = "lewm_go2_observable_camera_ray_fit_v4_n5_full_panel_v5_result_v1"
COMPLETION_SCHEMA = "lewm_go2_observable_camera_ray_fit_v4_n5_full_panel_v5_completion_v1"
FAILURE_SCHEMA = "lewm_go2_observable_camera_ray_fit_v4_n5_full_panel_v5_failure_v1"
METRIC_RECEIPT_SCHEMA = "lewm_go2_observable_camera_ray_fit_v4_n5_full_panel_v5_metric_verification_v1"
GATE_SCHEMA = "lewm_go2_observable_camera_ray_fit_v4_n5_full_panel_v5_gate_v1"
...
def expected_source_review_core(*, reviewer: str, successor_sources: Mapping[str, Mapping[str, str]]) -> dict[str, Any]:
    return {
        ...
        "status": "different_agent_review_passed_authority_free_exact_full_panel_v5",
...
def preflight_source_review(path: Path, file_sha256: str) -> tuple[dict[str, Any], bytes]:
    ...
    if supplied != canonical:
        raise PermissionError("N5 full-panel V5 source review path is not canonical")
    review, raw = load_hashed_json(
        canonical,
        file_sha256,
        name="N5 full-panel V5 different-agent source review",
    )
    ...
    if not isinstance(reviewer, str) or not reviewer.startswith("/root/") or reviewer == IMPLEMENTATION_AUTHOR or not isinstance(sources, Mapping) or set(sources) != set(SUCCESSOR_SOURCE_PATHS):
        raise PermissionError("N5 full-panel V5 review is not by a different agent")
    ...
    for relative in SUCCESSOR_SOURCE_PATHS:
        ...
        if not isinstance(binding, Mapping) or binding.get("path") != relative or not is_sha256(binding.get("file_sha256")):
            raise PermissionError("N5 full-panel V5 source binding changed")
        source_raw = read_regular_bytes(ROOT / relative, name=f"V5 source {relative}")
        if hashlib.sha256(source_raw).hexdigest() != binding["file_sha256"]:
            raise PermissionError(f"N5 full-panel V5 source changed: {relative}")
    ...
    if core != expected or canonical_json_sha256(core) != declared:
        raise PermissionError("N5 full-panel V5 source review contract changed")
    return review, raw
...
def source_review_binding(review: Mapping[str, Any], file_sha256: str) -> dict[str, str]:
    if not is_sha256(file_sha256) or not is_sha256(review.get("content_sha256")):
        raise ValueError("N5 full-panel V5 review binding is malformed")
...
def validate_reservation_structure(reservation: Mapping[str, Any], *, expected_source_review: Mapping[str, str]) -> dict[str, Any]:
    ...
    declared = core.pop("content_sha256", None)
    if not is_sha256(declared) or canonical_json_sha256(core) != declared:
        raise ValueError("N5 full-panel V5 reservation content SHA-256 changed")
    ...
    if not isinstance(recovery, list) or any(not isinstance(item, Mapping) for item in recovery):
        raise ValueError("N5 full-panel V5 recovery ledger is malformed")
    ...
    if reservation.get("schema") != RESERVATION_SCHEMA:
        raise ValueError("N5 full-panel V5 reservation schema changed")
...
def validate_result_structure(result: Mapping[str, Any], *, expected_source_review: Mapping[str, str] | None = None) -> dict[str, Any]:
    ...
    declared = core.pop("content_sha256", None)
    if not is_sha256(declared) or canonical_json_sha256(core) != declared:
        raise ValueError("N5 full-panel V5 result content SHA-256 changed")
    ...
    if result.get("schema") != RESULT_SCHEMA:
        raise ValueError("N5 full-panel V5 result schema changed")
    return dict(result)
