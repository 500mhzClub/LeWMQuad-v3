"""Source-only contract for the science-identical V3 operational recovery.
...
IMPLEMENTATION_AUTHOR = "/root/terminal_failure_audit"
SCHEMA_PREFIX = "lewm_go2_shared_jepa_v5_multires_probe_v3"
...
TEST_RELATIVE_PATH = (
    "lewm/tests/test_go2_shared_jepa_v5_multires_probe_v3.py"
)
...
SOURCE_CLOSURE_TEST_RELATIVE_PATH = (
    "lewm/tests/test_go2_multires_probe_source_closure_v3.py"
)
SCIENCE_IDENTITY_CHECKER_RELATIVE_PATH = (
    "scripts/check_go2_multires_probe_v3_science_identity.py"
)
SCIENCE_IDENTITY_TEST_RELATIVE_PATH = (
    "lewm/tests/test_go2_multires_probe_v3_science_identity.py"
)
COMPATIBILITY_CONTRACT_RELATIVE_PATH = (
    "lewm/benchmarks/go2_rgb_multiresolution_perception_v3_"
    "r9700_strict_compatibility_v1.py"
)
COMPATIBILITY_LAUNCHER_RELATIVE_PATH = (
    "scripts/launch_go2_rgb_multiresolution_perception_v3_"
    "r9700_strict_compatibility_v1.py"
)
COMPATIBILITY_RUNNER_RELATIVE_PATH = (
...
PREREGISTRATION_COMMIT = "7e6e539370c8f9d9d228da5ef4bc9ea4d10569a2"
...
def validate_source_manifest(raw: bytes) -> dict[str, Any]:
    ...
    if (
        value["schema"]
        != "lewm_go2_rgb_multiresolution_perception_v3_source_manifest"
...
def validate_pre_ledger_failure_receipt(
    ...
    *,
    ...
    """Validate the distinct V3 receipt for failure before ledger acceptance."""
    fields = {
        "schema",
        "status",
        "attempt_identity",
    ...
    if type(value) is not dict or set(value) != fields:
        raise PermissionError("V3 pre-ledger failure fields changed")
    core = dict(value)
    declared = core.pop("content_sha256")
    ...
    if type(ledger_state) is not dict or set(ledger_state) != {
        ...
        raise PermissionError("V3 pre-ledger state changed")
    ...
    if header_status == "NOT_PUBLISHED":
        if header is not None or header_prefix is not None:
            raise PermissionError("V3 absent pre-ledger header changed")
        ...
        if (
            ...
            raise PermissionError("V3 unaccepted pre-ledger prefix changed")
        ...
        if (
            ...
            raise PermissionError("V3 durable pre-ledger header changed")
        expected_prefix_paths = ["partial_access.jsonl", "reservation.json"]
        expected_boundary = (
            "after_durable_header_before_constructor_acceptance"
        )
    else:
        raise PermissionError("V3 pre-ledger status changed")
    prefix_fields = {"path", "file_sha256", "byte_count", "mode"}
    prefix_index = (
        {
            row["path"]: row
            for row in prefix
            if type(row) is dict and set(row) == prefix_fields
        }
        if type(prefix) is list
        else {}
    )
    if (
        value["schema"] != PRE_LEDGER_FAILURE_SCHEMA
        or value["status"]
        != "TERMINAL_V3_POST_RESERVATION_PRE_LEDGER_FAILURE_NO_RETRY"
        ...
        or not is_sha256(prefix_index["reservation.json"]["file_sha256"])
        or prefix_index["reservation.json"]["byte_count"] <= 0
        ...
        or value["v2_runtime_output_open_count"] != 0
        ...
        raise PermissionError("V3 pre-ledger failure receipt changed")
    if header is not None and (
        ...
        raise PermissionError("V3 pre-ledger header prefix changed")
    if header_prefix is not None and (
        ...
        raise PermissionError("V3 unaccepted header prefix binding changed")
    return dict(value)
...
def validate_failure_receipt(
    ...
    *,
    ...
    if type(value) is not dict or set(value) != fields:
        raise PermissionError("V3 failure-receipt fields changed")
    core = dict(value)
    declared = core.pop("content_sha256")
    reservation = validate_binding(value["reservation"], path="reservation.json")
    ...
    if (
        ...
        or value["status"]
        != "TERMINAL_V3_OPERATIONAL_OR_INTEGRITY_FAILURE_NO_RETRY"
        or not is_sha256(value["attempt_identity"])
        or (
            reservation_binding is not None
            and reservation != dict(reservation_binding)
        ...
        raise PermissionError("V3 failure receipt changed")
    return dict(value)
