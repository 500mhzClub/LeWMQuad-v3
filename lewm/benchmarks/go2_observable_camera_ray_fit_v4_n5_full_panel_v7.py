"""Static contract for the authority-free V7 N5 full-panel executor.

V7 deliberately has no execution token, authority object, capability, issuer,
...
OWNED_DIRECTORY_TRANSACTION_AMENDMENT_RELATIVE_PATH = (
    "docs/lewm_go2_observable_camera_ray_fit_v4_n5_full_panel_v7_"
    "owned_directory_transaction_amendment_2026-07-13.md"
)
OWNED_DIRECTORY_TRANSACTION_AMENDMENT_FILE_SHA256 = (
    "17ca6b726d1eaa25662a1823b4c153d496f1e51502b764350ddd6a3a34f249da"
)

# Retained aliases keep the executor-facing policy API stable while binding the
# actual V7 amendment rather than the mechanically copied V6 filename/hash pair.
RECOVERY_AMENDMENT_RELATIVE_PATH = (
    OWNED_DIRECTORY_TRANSACTION_AMENDMENT_RELATIVE_PATH
)
RECOVERY_AMENDMENT_FILE_SHA256 = OWNED_DIRECTORY_TRANSACTION_AMENDMENT_FILE_SHA256
...
SOURCE_REVIEW_SCHEMA = "lewm_go2_observable_camera_ray_fit_v4_n5_full_panel_v7_source_review_v1"
RESERVATION_SCHEMA = "lewm_go2_observable_camera_ray_fit_v4_n5_full_panel_v7_reservation_v1"
RESULT_SCHEMA = "lewm_go2_observable_camera_ray_fit_v4_n5_full_panel_v7_result_v1"
COMPLETION_SCHEMA = "lewm_go2_observable_camera_ray_fit_v4_n5_full_panel_v7_completion_v1"
FAILURE_SCHEMA = "lewm_go2_observable_camera_ray_fit_v4_n5_full_panel_v7_failure_v1"
METRIC_RECEIPT_SCHEMA = "lewm_go2_observable_camera_ray_fit_v4_n5_full_panel_v7_metric_verification_v1"
GATE_SCHEMA = "lewm_go2_observable_camera_ray_fit_v4_n5_full_panel_v7_gate_v1"
...
def owned_directory_transaction_contract() -> dict[str, Any]:
    """Frozen V7 mutation protocol attested by a passing source review."""

    return {
        "scope": "every_mutation_at_or_below_the_exclusive_v7_output_root",
...
def authority_bindings() -> dict[str, Any]:
    return {
        ...
        "v7_owned_directory_transaction_amendment": {
            "path": OWNED_DIRECTORY_TRANSACTION_AMENDMENT_RELATIVE_PATH,
            "file_sha256": OWNED_DIRECTORY_TRANSACTION_AMENDMENT_FILE_SHA256,
        },
...
def licenses() -> dict[str, bool]:
    return {
        ...
        "authorizes_later_v7_training": False,
        "authorizes_g2": False,
...
def _validate_v6_terminal_block() -> dict[str, Any]:
    ...
    if (
        ...
        or value.get("implementation_author") != IMPLEMENTATION_AUTHOR
...
def preflight_static_authority() -> dict[str, Any]:
    ...
    _hash_file(
        OWNED_DIRECTORY_TRANSACTION_AMENDMENT_RELATIVE_PATH,
        OWNED_DIRECTORY_TRANSACTION_AMENDMENT_FILE_SHA256,
        name="N5 V7 owned-directory transaction amendment",
    )
...
def expected_source_review_core(*, reviewer: str, successor_sources: Mapping[str, Mapping[str, str]]) -> dict[str, Any]:
    return {
        ...
        "status": "different_agent_review_passed_n5_full_panel_v7_owned_directory_transactions",
        ...
        "authority_boundary": {
            ...
            "retry_authorized": False,
            "scientific_retry_authorized": False,
            "v5_or_v6_numeric_state_authorized": False,
...
def preflight_source_review(path: Path, file_sha256: str) -> tuple[dict[str, Any], bytes]:
    ...
    if supplied != canonical:
        raise PermissionError("N5 full-panel V7 source review path is not canonical")
    review, raw = load_hashed_json(
        canonical,
        file_sha256,
        name="N5 full-panel V7 different-agent source review",
    )
    ...
    if not isinstance(reviewer, str) or not reviewer.startswith("/root/") or reviewer == IMPLEMENTATION_AUTHOR or not isinstance(sources, Mapping) or set(sources) != set(SUCCESSOR_SOURCE_PATHS):
        raise PermissionError("N5 full-panel V7 review is not by a different agent")
    ...
    for relative in SUCCESSOR_SOURCE_PATHS:
        ...
        if not isinstance(binding, Mapping) or binding.get("path") != relative or not is_sha256(binding.get("file_sha256")):
            raise PermissionError("N5 full-panel V7 source binding changed")
        source_raw = read_regular_bytes(ROOT / relative, name=f"V7 source {relative}")
        if hashlib.sha256(source_raw).hexdigest() != binding["file_sha256"]:
            raise PermissionError(f"N5 full-panel V7 source changed: {relative}")
    ...
    if core != expected or canonical_json_sha256(core) != declared:
        raise PermissionError("N5 full-panel V7 source review contract changed")
    return review, raw
...
def source_review_binding(review: Mapping[str, Any], file_sha256: str) -> dict[str, str]:
    if not is_sha256(file_sha256) or not is_sha256(review.get("content_sha256")):
        raise ValueError("N5 full-panel V7 review binding is malformed")
...
def validate_reservation_structure(reservation: Mapping[str, Any], *, expected_source_review: Mapping[str, str]) -> dict[str, Any]:
    ...
    declared = core.pop("content_sha256", None)
    if not is_sha256(declared) or canonical_json_sha256(core) != declared:
        raise ValueError("N5 full-panel V7 reservation content SHA-256 changed")
    ...
    if not isinstance(recovery, list) or any(not isinstance(item, Mapping) for item in recovery):
        raise ValueError("N5 full-panel V7 recovery ledger is malformed")
    ...
    if (
        ...
        raise ValueError("N5 full-panel V7 reservation contract changed")
...
def validate_result_structure(result: Mapping[str, Any], *, expected_source_review: Mapping[str, str] | None = None) -> dict[str, Any]:
    ...
    declared = core.pop("content_sha256", None)
    if not is_sha256(declared) or canonical_json_sha256(core) != declared:
        raise ValueError("N5 full-panel V7 result content SHA-256 changed")
    if (
        ...
        raise ValueError("N5 full-panel V7 result recovery binding changed")
    ...
    if isinstance(result_licenses, dict) and "v7_training_authorized" in result_licenses:
        result_licenses["v5_training_authorized"] = result_licenses.pop(
            "v7_training_authorized"
        )
    shadow_core = dict(shadow)
    shadow_core.pop("content_sha256")
    shadow["content_sha256"] = canonical_json_sha256(shadow_core)
    ...
    if result.get("schema") != RESULT_SCHEMA:
        raise ValueError("N5 full-panel V7 result schema changed")
    return dict(result)
