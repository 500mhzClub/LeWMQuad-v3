"""Static contract for the authority-free V8 N5 full-panel executor.

V8 deliberately has no execution token, authority object, capability, issuer,
...
SOURCE_REVIEW_SCHEMA = "lewm_go2_observable_camera_ray_fit_v4_n5_full_panel_v8_source_review_v1"
RESERVATION_SCHEMA = "lewm_go2_observable_camera_ray_fit_v4_n5_full_panel_v8_reservation_v1"
RESULT_SCHEMA = "lewm_go2_observable_camera_ray_fit_v4_n5_full_panel_v8_result_v1"
COMPLETION_SCHEMA = "lewm_go2_observable_camera_ray_fit_v4_n5_full_panel_v8_completion_v1"
FAILURE_SCHEMA = "lewm_go2_observable_camera_ray_fit_v4_n5_full_panel_v8_failure_v1"
METRIC_RECEIPT_SCHEMA = "lewm_go2_observable_camera_ray_fit_v4_n5_full_panel_v8_metric_verification_v1"
GATE_SCHEMA = "lewm_go2_observable_camera_ray_fit_v4_n5_full_panel_v8_gate_v1"
VERIFICATION_REQUEST_SCHEMA = (
    "lewm_go2_observable_camera_ray_fit_v4_n5_full_panel_v8_verifier_request_v1"
)
VERIFICATION_RESPONSE_SCHEMA = (
    "lewm_go2_observable_camera_ray_fit_v4_n5_full_panel_v8_verifier_response_v1"
)
...
def owned_directory_transaction_contract() -> dict[str, Any]:
    """Frozen V8 mutation protocol attested by a passing source review."""

    return {
        "scope": "every_mutation_at_or_below_the_exclusive_v8_output_root",
...
def licenses() -> dict[str, bool]:
    return {
        ...
        "authorizes_later_v8_training": False,
        "authorizes_g2": False,
...
def expected_source_review_core(*, reviewer: str, successor_sources: Mapping[str, Mapping[str, str]]) -> dict[str, Any]:
    return {
        ...
        "status": "different_agent_review_passed_n5_full_panel_v8_isolated_verifier",
        ...
        "authority_boundary": {
            ...
            "retry_authorized": False,
            "scientific_retry_authorized": False,
            "v5_v6_or_v7_numeric_state_authorized": False,
...
def preflight_source_review(path: Path, file_sha256: str) -> tuple[dict[str, Any], bytes]:
    ...
    if supplied != canonical:
        raise PermissionError("N5 full-panel V8 source review path is not canonical")
    review, raw = load_hashed_json(
        canonical,
        file_sha256,
        name="N5 full-panel V8 different-agent source review",
    )
    ...
    if not isinstance(reviewer, str) or not reviewer.startswith("/root/") or reviewer == IMPLEMENTATION_AUTHOR or not isinstance(sources, Mapping) or set(sources) != set(SUCCESSOR_SOURCE_PATHS):
        raise PermissionError("N5 full-panel V8 review is not by a different agent")
    ...
    for relative in SUCCESSOR_SOURCE_PATHS:
        ...
        if not isinstance(binding, Mapping) or binding.get("path") != relative or not is_sha256(binding.get("file_sha256")):
            raise PermissionError("N5 full-panel V8 source binding changed")
        source_raw = read_regular_bytes(ROOT / relative, name=f"V8 source {relative}")
        if hashlib.sha256(source_raw).hexdigest() != binding["file_sha256"]:
            raise PermissionError(f"N5 full-panel V8 source changed: {relative}")
    ...
    if core != expected or canonical_json_sha256(core) != declared:
        raise PermissionError("N5 full-panel V8 source review contract changed")
    return review, raw
...
def source_review_binding(review: Mapping[str, Any], file_sha256: str) -> dict[str, str]:
    if not is_sha256(file_sha256) or not is_sha256(review.get("content_sha256")):
        raise ValueError("N5 full-panel V8 review binding is malformed")
...
def validate_reservation_structure(reservation: Mapping[str, Any], *, expected_source_review: Mapping[str, str]) -> dict[str, Any]:
    ...
    declared = core.pop("content_sha256", None)
    if not is_sha256(declared) or canonical_json_sha256(core) != declared:
        raise ValueError("N5 full-panel V8 reservation content SHA-256 changed")
    ...
    if not isinstance(recovery, list) or any(not isinstance(item, Mapping) for item in recovery):
        raise ValueError("N5 full-panel V8 recovery ledger is malformed")
    ...
    if (
        ...
        raise ValueError("N5 full-panel V8 reservation contract changed")
...
def validate_result_structure(result: Mapping[str, Any], *, expected_source_review: Mapping[str, str] | None = None) -> dict[str, Any]:
    ...
    declared = core.pop("content_sha256", None)
    if not is_sha256(declared) or canonical_json_sha256(core) != declared:
        raise ValueError("N5 full-panel V8 result content SHA-256 changed")
    if (
        ...
        raise ValueError("N5 full-panel V8 result recovery binding changed")
    ...
    if isinstance(result_licenses, dict) and "v8_training_authorized" in result_licenses:
        result_licenses["v5_training_authorized"] = result_licenses.pop(
            "v8_training_authorized"
        )
    shadow_core = dict(shadow)
    shadow_core.pop("content_sha256")
    shadow["content_sha256"] = canonical_json_sha256(shadow_core)
    ...
    if result.get("schema") != RESULT_SCHEMA:
        raise ValueError("N5 full-panel V8 result schema changed")
    return dict(result)
