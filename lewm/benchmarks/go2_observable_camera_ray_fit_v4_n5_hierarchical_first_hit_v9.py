"""Static contract for the authority-free N5 hierarchical-first-hit V9.

V9 deliberately has no execution token, authority object, capability, issuer,
...
SOURCE_REVIEW_SCHEMA = "lewm_go2_observable_camera_ray_fit_v4_n5_hierarchical_first_hit_v9_source_review_v1"
RESERVATION_SCHEMA = "lewm_go2_observable_camera_ray_fit_v4_n5_hierarchical_first_hit_v9_reservation_v1"
RESULT_SCHEMA = "lewm_go2_observable_camera_ray_fit_v4_n5_hierarchical_first_hit_v9_result_v1"
COMPLETION_SCHEMA = "lewm_go2_observable_camera_ray_fit_v4_n5_hierarchical_first_hit_v9_completion_v1"
FAILURE_SCHEMA = "lewm_go2_observable_camera_ray_fit_v4_n5_hierarchical_first_hit_v9_failure_v1"
METRIC_RECEIPT_SCHEMA = "lewm_go2_observable_camera_ray_fit_v4_n5_hierarchical_first_hit_v9_metric_verification_v1"
GATE_SCHEMA = "lewm_go2_observable_camera_ray_fit_v4_n5_hierarchical_first_hit_v9_gate_v1"
VERIFICATION_REQUEST_SCHEMA = (
    "lewm_go2_observable_camera_ray_fit_v4_n5_hierarchical_first_hit_v9_"
    "verifier_request_v1"
)
...
ATTEMPT_SCOPE = "one_exclusive_fresh_hierarchical_first_hit_v9_attempt"
...
def owned_directory_transaction_contract() -> dict[str, Any]:
    """V8's reviewed mutation protocol, retained unchanged for V9."""

    return {
        "scope": "every_mutation_at_or_below_the_exclusive_v9_output_root",
...
def authority_bindings() -> dict[str, Any]:
    return {
        "v9_hierarchical_first_hit_amendment": {
            "path": V9_AMENDMENT_RELATIVE_PATH,
            "file_sha256": V9_AMENDMENT_FILE_SHA256,
        },
...
def licenses() -> dict[str, bool]:
    return {
        ...
        "authorizes_later_v9_training": False,
        "authorizes_g2": False,
...
def preflight_static_authority() -> dict[str, Any]:
    ...
    _hash_file(
        V9_AMENDMENT_RELATIVE_PATH,
        V9_AMENDMENT_FILE_SHA256,
        name="V9 hierarchical-first-hit amendment",
    )
    _hash_file(
        V8_DIAGNOSIS_RELATIVE_PATH,
        V8_DIAGNOSIS_FILE_SHA256,
...
def expected_source_review_core(
    *,
    ...
    return {
        ...
        "status": (
            "different_agent_review_passed_n5_hierarchical_first_hit_v9_"
...
def preflight_source_review(path: Path, file_sha256: str) -> tuple[dict[str, Any], bytes]:
    ...
    if supplied != canonical:
        raise PermissionError("N5 hierarchical-first-hit V9 review path is not canonical")
    review, raw = load_hashed_json(
        canonical,
        file_sha256,
        name="N5 hierarchical-first-hit V9 different-agent source review",
    )
    ...
    if (
        ...
        raise PermissionError(
            "N5 hierarchical-first-hit V9 review is not by an independent agent"
        )
    ...
    for relative in SUCCESSOR_SOURCE_PATHS:
        ...
        if not isinstance(binding, Mapping) or binding.get("path") != relative or not is_sha256(binding.get("file_sha256")):
            raise PermissionError("N5 hierarchical-first-hit V9 source binding changed")
        source_raw = read_regular_bytes(ROOT / relative, name=f"V9 source {relative}")
        if hashlib.sha256(source_raw).hexdigest() != binding["file_sha256"]:
            raise PermissionError(f"N5 hierarchical-first-hit V9 source changed: {relative}")
    ...
    for relative in SUCCESSOR_PROOF_PATHS:
        ...
        if (
            ...
            raise PermissionError("N5 hierarchical-first-hit V9 proof binding changed")
        proof_raw = read_regular_bytes(ROOT / relative, name=f"V9 proof {relative}")
        if hashlib.sha256(proof_raw).hexdigest() != binding["file_sha256"]:
            raise PermissionError(f"N5 hierarchical-first-hit V9 proof changed: {relative}")
    ...
    if core != expected or canonical_json_sha256(core) != declared:
        raise PermissionError("N5 hierarchical-first-hit V9 review contract changed")
    return review, raw
...
def source_review_binding(review: Mapping[str, Any], file_sha256: str) -> dict[str, str]:
    if not is_sha256(file_sha256) or not is_sha256(review.get("content_sha256")):
        raise ValueError("N5 hierarchical-first-hit V9 review binding is malformed")
...
def _validate_training_record(value: object) -> dict[str, Any]:
    ...
    if not isinstance(value, Mapping) or set(value) != expected_fields:
        raise ValueError("N5 hierarchical-first-hit V9 training fields changed")
    required = {
        "steps": 4000,
        "batch_size": 5,
        "evaluation_batch_size": 1,
    ...
    if any(value.get(key) != expected for key, expected in required.items()):
        raise PermissionError(
            "N5 hierarchical-first-hit V9 optimizer/exposure contract changed"
        )
    trace = value.get("trace")
    if not isinstance(trace, list) or len(trace) != 41:
        raise ValueError("N5 hierarchical-first-hit V9 diagnostic trace changed")
    checked_trace = [
        _validate_training_snapshot(row, name=f"V9 training trace[{index}]")
        for index, row in enumerate(trace)
    ]
    if tuple(row["step"] for row in checked_trace) != _expected_diagnostic_updates():
        raise PermissionError("N5 hierarchical-first-hit V9 trace indices changed")
    initial = _validate_training_snapshot(value.get("initial"), name="V9 initial")
    final = _validate_training_snapshot(value.get("final"), name="V9 final")
    if initial != checked_trace[0] or final != checked_trace[-1] or final["step"] != 4000:
        raise PermissionError(
            "N5 hierarchical-first-hit V9 did not select only update 4000"
        )
    return dict(value)
...
def validate_evaluation_structure(value: object) -> dict[str, Any]:
    if not isinstance(value, Mapping) or set(value) != {
        ...
        raise ValueError("N5 hierarchical-first-hit V9 evaluation controls changed")
    expected = {
    ...
    for key, (control, mapping, degenerate) in expected.items():
        ...
        if not isinstance(row, Mapping) or set(row) != {
            ...
            raise ValueError(f"N5 hierarchical-first-hit V9 {key} fields changed")
        if (
            ...
            raise ValueError(f"N5 hierarchical-first-hit V9 {key} control changed")
        _validate_loss_record(row["losses"], name=f"V9 {key}")
...
def _shadow_evaluation(value: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for row in shadow.values():
        ...
        if "hierarchical_first_hit_nll" in losses:
            ...
            raise ValueError("V9 evaluation shadow lost its first-hit component")
    return shadow
...
def validate_reservation_structure(reservation: Mapping[str, Any], *, expected_source_review: Mapping[str, str]) -> dict[str, Any]:
    ...
    declared = core.pop("content_sha256", None)
    if not is_sha256(declared) or canonical_json_sha256(core) != declared:
        raise ValueError("N5 hierarchical-first-hit V9 reservation hash changed")
    ...
    if not isinstance(recovery, list) or any(not isinstance(item, Mapping) for item in recovery):
        raise ValueError("N5 hierarchical-first-hit V9 recovery ledger is malformed")
    ...
    if (
        ...
        raise ValueError("N5 hierarchical-first-hit V9 reservation contract changed")
...
def validate_result_structure(result: Mapping[str, Any], *, expected_source_review: Mapping[str, str] | None = None) -> dict[str, Any]:
    ...
    declared = core.pop("content_sha256", None)
    if not is_sha256(declared) or canonical_json_sha256(core) != declared:
        raise ValueError("N5 hierarchical-first-hit V9 result hash changed")
    if (
        result.get("schema") != RESULT_SCHEMA
        or result.get("mode")
        != "exact_train_only_n5_hierarchical_first_hit_v9_development_fit"
        ...
        raise ValueError("N5 hierarchical-first-hit V9 result binding changed")
    attempt = result.get("attempt")
    if not isinstance(attempt, Mapping) or attempt.get("scope") != ATTEMPT_SCOPE:
        raise PermissionError("N5 hierarchical-first-hit V9 attempt scope changed")
    ...
    if licenses_value != expected_licenses:
        raise PermissionError("N5 hierarchical-first-hit V9 licenses changed")
    ...
    if result.get("schema") != RESULT_SCHEMA:
        raise ValueError("N5 hierarchical-first-hit V9 result schema changed")
    return dict(result)
