"""Static contract for the authority-free V4 N5 full-panel V4 executor.

V4 deliberately has no execution token, authority object, capability, issuer,
...
IMPLEMENTATION_AUTHOR = "/root/raw_plan_v2_qa"
...
OUTPUT_ROOT_RELATIVE_PATH = ".generated/go2_observable_camera_ray_fit_v4/n5_full_panel_v1"
...
SOURCE_REVIEW_SCHEMA = "lewm_go2_observable_camera_ray_fit_v4_n5_full_panel_v4_source_review_v1"
RESERVATION_SCHEMA = "lewm_go2_observable_camera_ray_fit_v4_n5_full_panel_v4_reservation_v1"
RESULT_SCHEMA = "lewm_go2_observable_camera_ray_fit_v4_n5_full_panel_v4_result_v1"
COMPLETION_SCHEMA = "lewm_go2_observable_camera_ray_fit_v4_n5_full_panel_v4_completion_v1"
FAILURE_SCHEMA = "lewm_go2_observable_camera_ray_fit_v4_n5_full_panel_v4_failure_v1"
METRIC_RECEIPT_SCHEMA = "lewm_go2_observable_camera_ray_fit_v4_n5_full_panel_v4_metric_verification_v1"
GATE_SCHEMA = "lewm_go2_observable_camera_ray_fit_v4_n5_full_panel_v4_gate_v1"
...
def read_regular_bytes_at(root: Path, relative: str | Path, *, name: str) -> bytes:
    """Read a leaf while retaining every no-follow directory descriptor."""
    ...
    try:
        ...
        if not stat.S_ISDIR(root_metadata.st_mode):
            raise PermissionError(f"{name} root is not a real directory")
        descriptors.append((root_fd, _stable_fingerprint(root_metadata)))
        parent_fd = root_fd
        for component in parts[:-1]:
            ...
            descriptors.append((child_fd, _stable_fingerprint(metadata)))
        ...
        for descriptor, fingerprint in descriptors:
            if _stable_fingerprint(os.fstat(descriptor)) != fingerprint:
                raise RuntimeError(f"{name} parent identity changed while read")
        return b"".join(chunks)
    finally:
        if leaf_descriptor >= 0:
            os.close(leaf_descriptor)
        for descriptor, _fingerprint in reversed(descriptors):
            os.close(descriptor)
...
def expected_source_review_core(*, reviewer: str, successor_sources: Mapping[str, Mapping[str, str]]) -> dict[str, Any]:
    return {
        ...
        "status": "different_agent_review_passed_authority_free_exact_full_panel_v4",
...
def preflight_source_review(path: Path, file_sha256: str) -> tuple[dict[str, Any], bytes]:
    ...
    if supplied != canonical:
        raise PermissionError("N5 full-panel V4 source review path is not canonical")
    review, raw = load_hashed_json(
        canonical,
        file_sha256,
        name="N5 full-panel V4 different-agent source review",
    )
    ...
    if not isinstance(reviewer, str) or not reviewer.startswith("/root/") or reviewer == IMPLEMENTATION_AUTHOR or not isinstance(sources, Mapping) or set(sources) != set(SUCCESSOR_SOURCE_PATHS):
        raise PermissionError("N5 full-panel V4 review is not by a different agent")
    ...
    for relative in SUCCESSOR_SOURCE_PATHS:
        ...
        if not isinstance(binding, Mapping) or binding.get("path") != relative or not is_sha256(binding.get("file_sha256")):
            raise PermissionError("N5 full-panel V4 source binding changed")
        source_raw = read_regular_bytes(ROOT / relative, name=f"V4 source {relative}")
        if hashlib.sha256(source_raw).hexdigest() != binding["file_sha256"]:
            raise PermissionError(f"N5 full-panel V4 source changed: {relative}")
    ...
    if core != expected or canonical_json_sha256(core) != declared:
        raise PermissionError("N5 full-panel V4 source review contract changed")
    return review, raw
...
def source_review_binding(review: Mapping[str, Any], file_sha256: str) -> dict[str, str]:
    if not is_sha256(file_sha256) or not is_sha256(review.get("content_sha256")):
        raise ValueError("N5 full-panel V4 review binding is malformed")
...
def validate_reservation_structure(reservation: Mapping[str, Any], *, expected_source_review: Mapping[str, str]) -> dict[str, Any]:
    ...
    declared = core.pop("content_sha256", None)
    if not is_sha256(declared) or canonical_json_sha256(core) != declared:
        raise ValueError("N5 full-panel V4 reservation content SHA-256 changed")
    ...
    if not isinstance(recovery, list) or any(not isinstance(item, Mapping) for item in recovery):
        raise ValueError("N5 full-panel V4 recovery ledger is malformed")
    ...
    if reservation.get("schema") != RESERVATION_SCHEMA:
        raise ValueError("N5 full-panel V4 reservation schema changed")
...
def validate_result_structure(result: Mapping[str, Any], *, expected_source_review: Mapping[str, str] | None = None) -> dict[str, Any]:
    ...
    declared = core.pop("content_sha256", None)
    if not is_sha256(declared) or canonical_json_sha256(core) != declared:
        raise ValueError("N5 full-panel V4 result content SHA-256 changed")
    ...
    if result.get("schema") != RESULT_SCHEMA:
        raise ValueError("N5 full-panel V4 result schema changed")
    return dict(result)
