"""CPU-only author and adversarial tests for Camera-ray N5 V15.
...
def test_v15_hierarchical_first_hit_matches_hand_computation() -> None:
...
def test_v15_presence_does_not_retain_old_one_over_groups_weight() -> None:
    breakdown = _direct_breakdown(
        hit_probabilities=[
...
def test_v15_presence_and_conditional_depth_invariances() -> None:
...
def test_v15_extreme_logits_have_finite_gradients_for_every_state_and_bin() -> None:
...
def test_v15_plus_minus_ten_thousand_logits_remain_finite() -> None:
    logits = torch.tensor(
        [
...
def test_v15_empty_groups_keep_a_finite_zero_gradient_term(
...
def test_v15_rejects_unnormalized_or_nonfinite_log_probabilities() -> None:
...
def test_v15_frozen_full_panel_schedule_is_exact_and_deterministic() -> None:
...
def test_v15_trainer_and_verifier_preserve_all_non_first_hit_branches(
    ...
    for compute in (
        trainer.compute_gate_aligned_v15_losses,
        verifier.compute_gate_aligned_v15_losses_for_verification,
    ):
...
def test_v15_standalone_verifier_is_compute_only() -> None:
    from scripts import (
        verify_go2_observable_camera_ray_fit_v4_n5_gate_aligned_raster_nll_v15
        as verifier,
    )
...
def test_v15_exact_cell_nll_formula_gradients_and_nonmutation() -> None:
    logits = torch.tensor(
        [
            [
                [[2.0, -1.0], [0.5, 0.25]],
...
def test_v15_cell_nll_rejects_schema_crossing(
    probabilities: torch.Tensor,
    labels: torch.Tensor,
    message: str,
) -> None:
...
def test_v15_batch_five_five_by_one_and_retained_accumulator_parity() -> None:
...
def test_v15_objective_keeps_all_four_retained_coefficients_and_adds_one() -> None:
    values = {
...
def test_v15_rouf_frozen_counts_and_missing_class_cases() -> None:
    labels = torch.cat(
        (
...
def test_v15_diagnostics_partition_merge_nonmutation_and_rejection() -> None:
    probabilities = torch.tensor(
        [
...
def test_v15_native_record_adapter_and_actual_26_check_gate() -> None:
    ...
    adapted = policy.adapt_native_v15_evaluation_for_retained_v4_gate(evaluation)
    assert evaluation == before
    assert "native_v15_objective" not in adapted["matched_rgb"]
    ...
    row["native_v15_objective"]["derived_raster_cell_nll"] = 0.07
    row["native_v15_objective"]["total"] = (
        row["native_v15_objective"]["v11_base_total"] + 0.25 * 0.07
    )
...
def test_v15_verifier_recomputes_g_without_trainer_scalar(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    from scripts import (
        train_go2_observable_camera_ray_fit_v4_n5_gate_aligned_raster_nll_v15
    ...
    monkeypatch.setattr(
        trainer,
        "compute_gate_aligned_v15_losses",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
            AssertionError("trainer scalar reused")
        ),
    )
    actual = verifier.derived_raster_cell_nll_for_verification_v15(
        probabilities, labels
    )
    assert actual.item() == pytest.approx(expected)


def test_v15_sources_forbid_v11_checkpoint_input_and_exact_root_is_absent() -> None:
    from lewm.benchmarks import (
        go2_observable_camera_ray_fit_v4_n5_gate_aligned_raster_nll_v15 as policy,
    )
    ...
    sources = (
        root
        / "scripts/train_go2_observable_camera_ray_fit_v4_n5_gate_aligned_raster_nll_v15.py",
        root
        / "scripts/verify_go2_observable_camera_ray_fit_v4_n5_gate_aligned_raster_nll_v15.py",
        root
    ...
    assert policy.OUTPUT_ROOT_RELATIVE_PATH == (
