"""Source-free author tests for the standalone raw-supervision Builder V5."""
...
from lewm.tests import go2_shared_jepa_v5_raw_supervision_builder_v5_test_support as support
...
def _synthetic_job_and_pair() -> tuple[
    ...
    job = builder.PreparedSceneJobV5(
        scene_id=scene_id,
        role="train",
        family=family,
...
def test_v5_amendment_v4_block_and_v3_invalidation_are_frozen() -> None:
    assert _sha(ROOT / AMENDMENT) == AMENDMENT_SHA256
    assert {relative: _sha(ROOT / relative) for relative in V4_BLOCK} == V4_BLOCK
    assert {
    ...
    assert all(
        ...
        for relative, digest in V4_BLOCK.items()
    )
    assert all(
        builder.FROZEN_PARENT_HASHES[relative] == digest
    ...
    assert review["content_sha256"] == (
        "34cfd6b139434f3b09b98d4bb44a339b7c355b68027d7f1e192112191c404ea6"
    )
    assert not any(review["authority"].values())


def test_v4_block_is_reproduced_and_v5_second_pass_is_final() -> None:
    v4_calls = _ordered_calls(
        ROOT / "lewm/datasets/go2_shared_jepa_v5_raw_supervision_builder_v4.py",
        "_build_exact_prepared_dataset_v4",
    )
    v5_calls = _ordered_calls(Path(builder.__file__), "_build_exact_prepared_dataset_v5")
    forbidden = {
    ...
    def bounds(calls: list[tuple[int, str]]) -> tuple[int, int]:
        revalidations = [
            line
        ...
        publications = [
            line for line, name in calls if name == "_libc_renameat2"
        ]
        assert len(revalidations) == 1
        assert len(publications) == 1
        assert revalidations[0] < publications[0]
        return revalidations[0], publications[0]

    v4_revalidation, v4_publication = bounds(v4_calls)
    v4_intervening = [
        (line, name)
        for line, name in v4_calls
        if v4_revalidation < line < v4_publication and name in forbidden
    ]
    assert [name for _line, name in v4_intervening] == [
        "_precommitted_audit_sample",
        "_with_content_sha256",
        "_validate_staging_inventory",
        "_write_json_exclusive",
    ...
    v5_revalidation, v5_publication = bounds(v5_calls)
    v5_intervening = [
        (line, name)
        for line, name in v5_calls
        if v5_revalidation < line < v5_publication and name in forbidden
    ]
    assert v5_intervening == []
    permitted_after_second_pass = {
        "validate",
        "_named_directory_identity",
        "RawSupervisionBuildError",
    }
    observed_after_second_pass = {
        name
        for line, name in v5_calls
        if v5_revalidation < line < v5_publication
    }
    assert observed_after_second_pass <= permitted_after_second_pass
    ...
    for name in required_before_second_pass:
        prepublication_occurrences = [
            line
            for line, observed_name in v5_calls
            if line < v5_publication and observed_name == name
        ]
        assert prepublication_occurrences
        assert max(prepublication_occurrences) < v5_revalidation


def test_canonical_v5_roles_schemas_and_authors_are_exact() -> None:
    ...
    assert builder.AUTHORIZATION_SCHEMA.endswith("build_authorization_v5")
    assert builder.REVIEW_BINDING_SCHEMA.endswith("review_binding_v5")
    assert builder.BUILDER_REVIEW_SCHEMA.endswith(
        "builder_v5_independent_review_v1"
    )
    assert builder.AUDITOR_REVIEW_SCHEMA.endswith(
        "auditor_v5_independent_review_v1"
    )
...
def test_production_signatures_have_no_injection_seams() -> None:
    ...
    exact = inspect.signature(builder.execute_exact_build_v5)
    ...
    build = inspect.signature(builder._build_exact_prepared_dataset_v5)
    forbidden = {
        "output_directory",
    ...
    assert tuple(field.name for field in fields(builder.AcceptedAuthorizationV5)) == (
        "authorization_file_sha256",
        "authorization_content_sha256",
        "source_map_sha256",
    )
    receipt = builder.AcceptedAuthorizationV5("1" * 64, "2" * 64, "3" * 64)
...
def test_absent_authority_reaches_no_byte_or_metadata_opener(
    ...
    with pytest.raises(PermissionError, match="authorization is absent"):
        builder.execute_exact_build_v5(
...
def test_deterministic_shard_science_and_layout_without_exact_data(
    ...
    reversed_job = builder.PreparedSceneJobV5(
...
def test_join_rejects_duplicate_endpoint_and_cross_role() -> None:
    job, pair = _synthetic_job_and_pair()
    duplicate = builder.PreparedSceneJobV5(
        scene_id=job.scene_id,
        role=job.role,
        family=job.family,
