...
from lewm import visibility_batched_footprint_controller_development as copied
...
def fixture(monkeypatch,tmp_path,fault=None):
    ...
    monkeypatch.setattr(run,'ReceiptCopiedFootprintController',Baseline)
    monkeypatch.setattr(run,'VisibilityBatchedFootprintController',Candidate)
...
def test_original_loop_and_only_declared_composition_changes():
    ...
    assert new.__globals__['FrozenFootprintAnchoredController'] is run.ReceiptCopiedFootprintController
    assert new.__globals__['ScopedFootprintAnchoredController'] is run.VisibilityBatchedFootprintController
...
def test_full_history_and_seven_original_state_checks(monkeypatch,tmp_path):
    ...
    assert result['baseline']=='ReceiptCopiedFootprintController'
    assert result['candidate']=='VisibilityBatchedFootprintController'
