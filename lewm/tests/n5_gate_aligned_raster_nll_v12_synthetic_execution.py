"""Production-ineligible V12 filesystem lifecycle used only by tests.
...
STAGING_PREFIX = ".n5.synthetic-v12-"
LOCK_NAME = ".n5.synthetic-v12.lock"
SERIALIZER_SUFFIX = ".n5.synthetic-v12.serializer"
SCHEMA = "lewm_go2_n5_gate_aligned_raster_nll_v12_synthetic_reservation_v1"
MANIFEST_SCHEMA = "lewm_go2_n5_gate_aligned_raster_nll_v12_synthetic_staging_v1"
...
def complete_retained_gate_evaluation_v12() -> dict[str, Any]:
    ...
    def control(name: str, mapping: list[int], *, perfect: bool) -> dict[str, Any]:
        ...
        if family_count * len(family_names) != raster["count"]:
            raise ValueError("V12 synthetic family partition is not even")
        diagnostics = {
            "overall": {
...
def _directory_flags() -> int:
    ...
    if not nofollow or not directory:
        raise PermissionError("synthetic V12 requires no-follow directory opens")
...
class _WatchV12:
    ...
    label: str
    generation: int
    snapshot: _SnapshotV12


class _DeclaredJournalV12:
    """A deterministic test journal for declared filesystem state deltas."""

    def __init__(self) -> None:
        self._watches: dict[tuple[int, int], _WatchV12] = {}
        self._generation = 0
    ...
    def _fail(self, reason: str) -> None:
        ...
        raise PermissionError(f"synthetic V12 journal rejected: {reason}")
    ...
    @staticmethod
    def _snapshot(directory_fd: int) -> _SnapshotV12:
        ...
        return _SnapshotV12(_fingerprint(opened), rows)
    ...
    def watch(self, directory_fd: int, *, label: str) -> None:
        ...
        self._generation += 1
        self._watches[identity] = _WatchV12(
            retained,
            identity,
            label,
            self._generation,
    ...
    def _snapshots(self) -> dict[tuple[int, int], _SnapshotV12]:
        return {
    ...
    ) -> tuple[_WatchV12, dict[tuple[int, int], _SnapshotV12]]:
        self.assert_clean()
    ...
    def _finish(
        self,
        before: Mapping[tuple[int, int], _SnapshotV12],
        *,
...
@dataclass
class _AncestorChainV12:
    descriptors: list[int]
    entries: list[_AncestorEntryV12]
    closed: bool = False


def _open_ancestor_chain(path: Path) -> _AncestorChainV12:
    filesystem_root = Path(path.anchor)
    anchor_fd = os.open(filesystem_root, _directory_flags())
    chain = _AncestorChainV12([anchor_fd], [])
    parent_fd = anchor_fd
    try:
        for component in path.parts[1:]:
            ...
            chain.entries.append(
                _AncestorEntryV12(parent_fd, component, child_fd, identity)
            )
            parent_fd = child_fd
        return chain
...
def _assert_ancestor_chain(chain: _AncestorChainV12) -> None:
    if chain.closed:
...
def _close_ancestor_chain(chain: _AncestorChainV12) -> None:
    if chain.closed:
        return
    chain.closed = True
...
class SyntheticReservationV12:
    ...
    seed_root_fd: int
    derived_fd: int
    journal: _DeclaredJournalV12
    ancestor_chain: _AncestorChainV12
    owned_directory_fds: tuple[int, ...]
    owned_claim_artifacts: dict[str, _OwnedArtifactV12] = field(
        default_factory=dict, compare=False
    )
    owned_derived_artifacts: dict[str, _OwnedArtifactV12] = field(
        default_factory=dict, compare=False
    )


class SyntheticExecutionV12:
    ...
    def __copy__(self) -> "SyntheticExecutionV12":
        return type(self)(self.root)

    def __deepcopy__(self, memo: object) -> "SyntheticExecutionV12":
        del memo
        return type(self)(self.root)
    ...
    def _new_staging(
        self, seed_fd: int, journal: _DeclaredJournalV12
    ) -> tuple[str, int]:
        for _ in range(128):
    ...
    @classmethod
    def _assert_claim_fd(cls, reservation: SyntheticReservationV12) -> None:
    ...
    @classmethod
    def _assert_claim(cls, reservation: SyntheticReservationV12) -> None:
    ...
    def _write_claim(
        cls,
        reservation: SyntheticReservationV12,
        name: str,
        raw: bytes,
        *,
        terminal: bool = False,
        ...
        reservation.owned_claim_artifacts[name] = _OwnedArtifactV12(
    ...
    @classmethod
    def _write_derived(
        cls, reservation: SyntheticReservationV12, name: str, raw: bytes
    ) -> None:
        cls._assert_claim(reservation)
        fingerprint = reservation.journal.create(
            reservation.derived_fd, name, raw
        )
        reservation.owned_derived_artifacts[name] = _OwnedArtifactV12(
    ...
    def claim(
        ...
        with self._serializer():
            ...
            claimed: SyntheticReservationV12 | None = None
            ...
            try:
                ...
                claimed = SyntheticReservationV12(
                    self.attempt,
                    value,
                    raw,
                    active_fd,
    ...
    def _cleanup_owned(
        reservation: SyntheticReservationV12,
        registry: dict[str, _OwnedArtifactV12],
        *,
        ...
        *,
        ...
        if not reservation.journal.poisoned:
            try:
                ...
                pass
        core = {
            "schema": "lewm_go2_n5_gate_aligned_raster_nll_v12_synthetic_failure_v1",
            "status": "failed",
            "failure_stage": stage,
    ...
    def publish(self, reservation: SyntheticReservationV12, raw: bytes) -> None:
    ...
    @staticmethod
    def close(reservation: SyntheticReservationV12) -> None:
        try:
