...
from lewm.longer_motion_collection_development import schedule
from scripts.audit_go2_longer_observed_floor_motion_development_v1 import audit_tape


def fixture():
    commands = schedule()+[dict(segment='terminal_zero_tail',phase=10,requested_command=[0.,0.,0.]) for _ in range(5)]
    requested = np.zeros((30000,3)); phase = np.zeros(30000); tape = []
    ...
    return dict(timestamp_s=np.arange(1,30001)*.002,requested_command=requested,
