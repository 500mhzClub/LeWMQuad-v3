"""Source-free author tests for the standalone raw-supervision Builder V7."""
...
from lewm.tests import go2_shared_jepa_v5_raw_supervision_builder_v7_test_support as support
...
def _synthetic_job_and_pair() -> tuple[
    ...
    job = builder.PreparedSceneJobV7(
        scene_id=scene_id,
        role="train",
        family=family,
...
def test_v7_amendment_and_v6_block_are_frozen() -> None:
...
def test_v5_race_is_reproduced_and_v7_transaction_spans_publication() -> None:
    ...
    v7_calls = _ordered_calls(Path(builder.__file__), "_build_exact_prepared_dataset_v7")
    ...
    v7_names = [name for _line, name in v7_calls]
    assert v7_names.count("_ClosedPublicationTransaction") == 1
    assert v7_names.count("_revalidate_exact_before_publication") == 1
    assert v7_names.count("validate_before_rename") == 1
    assert v7_names.count("rename_owned") == 1
    assert v7_names.count("validate_after_rename") == 1
    assert v7_names.count("require_final_quiet") == 1
    assert "_libc_renameat2" not in v7_names
    transaction_index = v7_names.index("_ClosedPublicationTransaction")
    revalidate_index = v7_names.index("_revalidate_exact_before_publication")
    validate_index = v7_names.index("validate_before_rename")
    rename_index = v7_names.index("rename_owned")
    post_index = v7_names.index("validate_after_rename")
    quiet_index = v7_names.index("require_final_quiet")
    ...
    for name in required_before_second_pass:
        prepublication_occurrences = [
            line
            for line, observed_name in v7_calls
...
def test_v6_ancestor_gap_is_reproduced_and_v7_rejects_it(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    transaction, retained, paths = _open_synthetic_transaction(v7_root, monkeypatch)
...
def test_canonical_v7_roles_schemas_and_authors_are_exact() -> None:
    ...
    assert builder.AUTHORIZATION_SCHEMA.endswith("build_authorization_v7")
    assert builder.REVIEW_BINDING_SCHEMA.endswith("review_binding_v7")
    assert builder.BUILDER_REVIEW_SCHEMA.endswith(
        "builder_v7_independent_review_v1"
    )
    assert builder.AUDITOR_REVIEW_SCHEMA.endswith(
        "auditor_v7_independent_review_v1"
    )
    assert builder.BUILDER_IMPLEMENTATION_AUTHOR == "/root/raw_v7_successor_author"
    assert builder.AUDITOR_IMPLEMENTATION_AUTHOR == "/root/raw_v7_successor_author/auditor_v7_author"
...
def test_production_signatures_have_no_injection_seams() -> None:
    ...
    exact = inspect.signature(builder.execute_exact_build_v7)
    ...
    build = inspect.signature(builder._build_exact_prepared_dataset_v7)
    forbidden = {
        "output_directory",
    ...
    assert tuple(field.name for field in fields(builder.AcceptedAuthorizationV7)) == (
        "authorization_file_sha256",
        "authorization_content_sha256",
        "source_map_sha256",
    )
    receipt = builder.AcceptedAuthorizationV7("1" * 64, "2" * 64, "3" * 64)
...
def test_absent_authority_reaches_no_byte_or_metadata_opener(
    ...
    with pytest.raises(PermissionError, match="authorization is absent"):
        builder.execute_exact_build_v7(
...
def test_deterministic_shard_science_and_layout_without_exact_data(
    ...
    reversed_job = builder.PreparedSceneJobV7(
...
def test_join_rejects_duplicate_endpoint_and_cross_role() -> None:
    job, pair = _synthetic_job_and_pair()
    duplicate = builder.PreparedSceneJobV7(
        scene_id=job.scene_id,
        role=job.role,
        family=job.family,
...
def test_full_build_rejects_mutation_during_final_source_pass(
    ...
    job, pair = _synthetic_job_and_pair()
    context = builder.ExactPrepublicationContextV7(
    ...
    with pytest.raises(builder.RawSupervisionBuildError, match="poisoned"):
        builder._build_exact_prepared_dataset_v7(
            (job,),
            (pair,),
            workers=1,
            input_provenance={},
