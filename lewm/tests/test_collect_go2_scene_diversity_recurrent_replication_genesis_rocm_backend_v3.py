...
from scripts import (
    collect_go2_scene_diversity_recurrent_replication_genesis_rocm_backend_v3
    as collector,
)
...
def test_collector_is_fresh_v3_layer_on_corrected_v2() -> None:
...
def test_v2_review_binding_is_semantically_exact() -> None:
    valid = {
        "predecessor_v2_qualification_terminal_review_binding": (
            collector._standard_v2_review_binding()  # noqa: SLF001
        )
    }
    collector._require_v2_review_binding(valid)  # noqa: SLF001
    changed = copy.deepcopy(valid)
    changed["predecessor_v2_qualification_terminal_review_binding"][
        "sha256"
    ] = "0" * 64
    with pytest.raises(
        ...
        collector._require_v2_review_binding(changed)  # noqa: SLF001


def test_scoped_overlay_routes_v3_and_restores_v2() -> None:
