...
from scripts import (
    execute_go2_rgb_object_space_explicit_plan_terminal_successor_state_joint_jepa_v28
    as executor,
)
...
def test_authority_and_one_shot_reservation_are_fail_closed(tmp_path: Path) -> None:
    assert executor.SCHEMA_PREFIX.endswith("joint_jepa_v28")
    assert executor.CERTIFIED_SOURCE_ROOT.endswith(
        "v28-explicit-plan-terminal-successor-source"
    )
    authority = _authority()
    assert executor.validate_future_execution_prerequisites_v28(authority) == authority
    ...
    assert executor.validate_attempt_reservation_v28(reservation) == reservation
    assert reservation["maximum_updates"] == 400
    assert reservation["maximum_presentations"] == 12_800
    with pytest.raises(FileExistsError):
        executor.reserve_attempt_v28(
    ...
    with pytest.raises(PermissionError):
        executor.validate_future_execution_prerequisites_v28(changed)
...
def test_exception_terminalizer_truthfully_quarantines_partial_checkpoint(
    ...
    failure = executor.terminalize_failure_v28(
        output,
        reservation,
...
def test_hard_gate_boundaries_are_exact(
    ...
    plan = _plan_metrics()
    mutation(plan)
    result = executor.evaluate_gate_v28(
        update100_physical={},
        update400_physical={},
        update400_controls={},
        plan_metrics=plan,
...
def test_update_integrity_accepts_only_exact_mixed_accounting() -> None:
    ...
    result = SimpleNamespace(
        ...
        mean_losses={"C": 1.0, "N28": 1.0, "J24": 1.0, "P28": 1.0, "L28": 4.0},
        plan_diagnostics={
            "mechanism": "explicit_plan_terminal_successor_state",
    ...
    receipt = executor.validate_update_integrity_v28(
        runtime, model, result, update=update
    )
    assert receipt["passed"] is True
    assert receipt["p25_evaluation_count"] == 0

    result.plan_diagnostics = {**result.plan_diagnostics, "gamma": 0.9}
    with pytest.raises(RuntimeError, match="plan diagnostics"):
        executor.validate_update_integrity_v28(runtime, model, result, update=update)
    result.plan_diagnostics.pop("gamma")
