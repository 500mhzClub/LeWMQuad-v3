...
def test_report_retains_original_state_and_negative_evidence_without_mutation():
    prior = dict(incremental_tiled_dense_floor_comparison=True,
        both_controllers_use_progressive_retained_floor_patch_batching=True, observed_state_checks=[{'frame': 1173, 'sha': 'old'}],
    ...
    assert expected['candidate'] == 'BodyProjectedTiledController'
    assert 'incremental_tiled_dense_floor_comparison' not in expected


def test_wrong_predecessor_scope_rejected():
    with pytest.raises(ValueError, match='tiled'):
        check.expected_report(dict(incremental_tiled_dense_floor_comparison=False), {})
