...
def test_all_eight_persistent_indices_are_empty_distinct_and_explicit():
    c=SinglePassLaterFloorController(None,None,**kwargs());m=c.memory
    ...
    assert all(type(v) is SinglePassMeasuredSampleBoundsIndex and not v.cells for v in indices)
...
def test_actual_public_packet_path_preserves_every_decision_and_bound(monkeypatch):
    ...
    old,new=[cls(None,None,**kwargs()) for cls in (LaterFloorResolutionRoundTripController,SinglePassLaterFloorController)]
    previous=None
    for i in range(2):
