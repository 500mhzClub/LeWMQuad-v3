...
def _manifest(scene_id: str = "world-waypoint-v2-unit") -> SceneManifest:
...
def _constructor_fields(
    ...
    return {
        ...
        for name, definition in (
            ConfigurationPathWorldWaypointReceiptV2.__dataclass_fields__.items()
        )
        if definition.init
    }


def test_v2_hashes_and_serializes_both_explicit_authority_denials(
    shared_bundle,
) -> None:
    _memory, projection, snapshot, planner, path = shared_bundle
    receipt = ConfigurationPathWorldWaypointIssuerV2(projection, planner).issue(
        snapshot,
        path,
    )
    ...
    assert encoded["development_execution_eligible"] is True
    receipt.assert_integrity()


def test_v2_authority_denial_tampering_is_integrity_checked(
    shared_bundle,
) -> None:
    _memory, projection, snapshot, planner, path = shared_bundle
    issuer = ConfigurationPathWorldWaypointIssuerV2(projection, planner)
    for field_name in (
        ...
        object.__setattr__(receipt, field_name, True)
        assert receipt.content_sha256 == original_hash
        with pytest.raises(
            WorldWaypointV2BindingError,
...
def test_v2_exact_high_index_world_centres_and_metric_receipt(
    shared_bundle,
) -> None:
    _memory, projection, snapshot, planner, path = shared_bundle
    issuer = ConfigurationPathWorldWaypointIssuerV2(projection, planner)
...
def test_v2_receipt_is_exact_live_single_use_noncopyable_and_nonforgeable(
    shared_bundle,
) -> None:
    _memory, projection, snapshot, planner, path = shared_bundle
    issuer = ConfigurationPathWorldWaypointIssuerV2(projection, planner)
    ...
    forged = ConfigurationPathWorldWaypointReceiptV2(
        **_constructor_fields(receipt)
    )
    assert forged.content_sha256 == receipt.content_sha256
    with pytest.raises(WorldWaypointV2BindingError, match="exact live"):
        issuer.validate(snapshot, path, forged)

    issuer.validate(snapshot, path, receipt, consume=True)
    with pytest.raises(WorldWaypointV2BindingError, match="already consumed"):
        issuer.validate(snapshot, path, receipt)


def test_v2_wrong_projection_planner_pair_and_stale_snapshot_reject() -> None:
    _memory, projection, snapshot, planner, path = _bundle()
    ...
    with pytest.raises(SnapshotBindingError, match="instances differ"):
        ConfigurationPathWorldWaypointIssuerV2(projection, second_planner)
...
def test_v2_receipt_rejects_wrong_ratio_or_cost(shared_bundle) -> None:
    _memory, projection, snapshot, planner, path = shared_bundle
    receipt = ConfigurationPathWorldWaypointIssuerV2(projection, planner).issue(
        snapshot,
        path,
    )
    fields = _constructor_fields(receipt)
