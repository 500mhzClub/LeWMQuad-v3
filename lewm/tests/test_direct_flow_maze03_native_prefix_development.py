...
from lewm.tests.test_direct_flow_maze03_prefix_development import rows as decisions, boundary
from scripts import direct_flow_maze03_native_prefix_development as prefix
...
def evidence():
    ...
    report=dict(case='full_jepa_novel_maze_03',frames=265,maximum_frames=265,boundary_frame=264,
    ...
    return dict(status='DIRECT_FLOW_MAZE03_PREFIX_V1_COMPLETE',model_loaded=True,
...
def test_complete_corrected_replay_required_before_native_use(monkeypatch,fault):
    ...
    elif fault=='early_recovery': rows[263]['comparison']['controller_recovered']=True
    elif fault=='failed_v1': result['status']='TERMINAL_DIRECT_FLOW_MAZE03_PREFIX_FAILURE'
...
def native_fixture(monkeypatch,tmp_path):
    ...
    data=dict(timestamp_s=np.arange(14000)*.002,base_pose_world=np.zeros((14000,7)))
    ...
    def read_rows(p):
        ...
        pytest.fail('read beyond the265-observation intervention')
    monkeypatch.setattr(prefix,'read_rows',read_rows)
    monkeypatch.setattr(prefix,'read_json',lambda p,n:tapes[p] if n=='command_tape.json' else [{}]*265)
    def reader(p):
        def packet(i):
            packets.append((p,i))
...
def test_exact_native_past_with_no_borrowed_changed_command_outcomes(monkeypatch,tmp_path,fault):
    ...
    if fault=='physical': data['base_pose_world'][13949,0]=.1; np.savez(new/'physics_trace.npz',**data)
    elif fault=='new_future': data['base_pose_world'][13950:,0]=100.; np.savez(new/'physics_trace.npz',**data)
    elif fault=='public': changes[(new,264)]=1
    elif fault=='original_failure': rows[old][264]['decision']['original_visual_evidence']['reference_selection']={}
    elif fault=='decision': rows[new][264]['decision']['new_selection']['prediction']=[[2.]]
    elif fault=='prior_command': tapes[new][263]['requested_command']=[.2,0.,0.]
    elif fault=='new_command': tapes[new][264]['requested_command']=[0.,0.,0.]
    elif fault=='short': np.savez(new/'physics_trace.npz',**{k:v[:13949] for k,v in data.items()})
    elif fault=='partial_new_command': tapes[new][264]['completed']=False
    if fault in (None,'new_future','partial_new_command'):
        r=prefix.compare(old,new,saved,report)
        assert r['physical_prefix_samples']==13950 and r['common_prefix_frames']==265
        assert r['raw_model_forecast_comparisons']==261 and r['complete_candidate_decisions_match_prospective_prefix']
        assert r['candidate_intervention_command_completed']==(fault!='partial_new_command')
        assert len(reads)==795 and len(packets)==530 and not r['following_physical_outcomes_compared']
    else:
