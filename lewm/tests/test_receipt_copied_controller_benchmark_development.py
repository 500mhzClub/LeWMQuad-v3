...
import pytest
from scripts import benchmark_go2_receipt_copied_controller_v1 as runner
...
def test_timing_comparison_separates_warmup_terminal_and_order_groups():
    rows=[dict(frame=i,warmup=i<3,terminal='DONE' if i==5 else None,
        ...
        controller_wall_ms={'original':200.+i,'receipt_copied':150.+i}) for i in range(6)]
...
def test_complete_replay_or_immediate_stop_without_following_observation(monkeypatch,tmp_path,mismatch):
    ...
    if mismatch:
        with pytest.raises(ValueError,match='complete receipt_copied decision'): runner.replay({'correction_admission':{}})
        assert reads==[0,1,2]
    else:
