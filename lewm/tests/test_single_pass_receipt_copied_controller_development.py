...
def test_new_controller_owns_empty_indices_and_preserves_dual_camera_and_mission_types():
    old, new = [cls(None, None, **kwargs()) for cls in (ReceiptCopiedMeasuredFloorController, SinglePassReceiptCopiedController)]
...
def test_composition_preserves_current_observe_advance_and_selector():
    from lewm.measured_floor_transport_controller_development import MeasuredFloorTransportController
    from lewm.receipt_copied_selector_development import ReceiptCopiedViewReentrySelector
    for method in ('observe','advance','_result'):
        assert getattr(SinglePassReceiptCopiedController,method) is getattr(ReceiptCopiedMeasuredFloorController,method)
    new=SinglePassReceiptCopiedController(None,None,**kwargs())
    original=MeasuredFloorTransportController(None,None,**kwargs())
    assert type(new.selector) is ReceiptCopiedViewReentrySelector
    for name in ('motion','registration','mission','residual'):
        assert type(getattr(new,name)) is type(getattr(original,name))
