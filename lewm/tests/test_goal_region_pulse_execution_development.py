...
from lewm.goal_region_pulse_scene_development import TRIALS,specification,pack
...
def test_complete_feedback_on_synthetic_command_responsive_plant():
    model=GoalRegionPulseServo();x=y=yaw=0.;rows=[]
...
def test_five_drive_ticks_then_twenty_zero_intervals_before_next_choice():
    model=GoalRegionPulseServo();rows=[step(model,i) for i in range(46)]
...
def test_motion_rejects_brake_settling_and_elapsed_time_remains_bounded():
    ...
    other=GoalRegionPulseServo();other.start_ns=-100_000_000_000
...
def test_input_faults_stop_and_latch(fault):
    model=GoalRegionPulseServo();step(model,0);t=1_600_000_000;e=evidence(t)
