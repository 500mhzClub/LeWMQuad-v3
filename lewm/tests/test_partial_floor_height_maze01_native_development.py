...
from scripts import run_go2_partial_floor_height_maze01_pilot_v1 as runner
...
def test_original_native_and_evaluation_calculations_are_preserved():
    class Normalize(ast.NodeTransformer):
        def visit_Name(self, n):
            if n.id == 'PartialHeightDirectFlowController': n.id = 'DirectFlowFloorTransportController'
            return n
        def visit_Constant(self, n):
            if isinstance(n.value, str): n.value = n.value.replace('PARTIAL_FLOOR_HEIGHT_MAZE01', 'DIRECT_FLOW_MAZE01')
            return n
    for kind, names in [('episode', ('collect', 'artifacts')), ('audit', ('audit',))]:
        for name in names:
            old = function('scripts/direct_flow_maze01_'+kind+'_development.py', name)
            new = function('scripts/partial_floor_height_maze01_'+kind+'_development.py', name)
            assert ast.dump(old) == ast.dump(Normalize().visit(new)), (kind, name)


def test_fixed_original_tracking_maze_and_exclusive_height_prefix():
    assert runner.CASE == ('full_jepa_partial_floor_height_maze_01',1,'full','jepa','seed_2026091001_full_jepa')
    assert runner.PRIOR_CASE[0] == 'full_jepa_direct_flow_maze_01'
    assert runner.PREFIX.name == 'go2_partial_floor_height_prefix_v1_attempt_001'
    assert runner.PRIOR_SHA == 'd6774bae22cb9effeb0cd85ae255de203de1539541f701d57788b58ab00769de'
    assert runner.OUTPUT.name == 'go2_partial_floor_height_maze01_pilot_v1_attempt_001'
...
def test_worker_uses_fresh_models_and_retains_evidence_after_failure(monkeypatch,tmp_path,failed_stage):
    ...
    if failed_stage is None:
        assert record['status']=='PARTIAL_FLOOR_HEIGHT_MAZE01_COLLECTED_AND_RAW_AUDITED'
        ...
        assert record['status']=='PARTIAL_FLOOR_HEIGHT_MAZE01_WORKER_FAILED'
