...
import pytest
from scripts import benchmark_go2_single_pass_receipt_copied_v1 as runner
...
def test_timing_comparison_separates_warmup_terminal_and_order_groups():
    rows=[dict(frame=i,warmup=i<3,terminal='DONE' if i==5 else None,
        ...
        controller_wall_ms={'receipt_copied':200.+i,'single_pass_receipt_copied':150.+i}) for i in range(6)]
...
def test_complete_replay_or_immediate_stop_without_following_observation(monkeypatch,tmp_path,mismatch):
    ...
    class Controller:
        ...
        def observe(self,policy,*a,**k):
            ...
            return result
    monkeypatch.setattr(runner,'ReceiptCopiedMeasuredFloorController',Controller)
    ...
    if mismatch:
        with pytest.raises(ValueError,match='complete single_pass_receipt_copied decision'): runner.replay({'correction_admission':{}})
        assert reads==[0,1,2]
    else:
...
@pytest.mark.parametrize('fault',[None,'source_conflict','original_verifier_failure'])
def test_predecessors_use_their_original_distinct_launch_schemas(monkeypatch,fault):
    roots=list(runner.PREDECESSORS); checked=[]
    monkeypatch.setattr(runner,'verify_artifacts',lambda *a:None)
    def read(root,name):
        index=roots.index(root.name)
        if name=='result.json':
            return dict(artifact_sha256={},source_sha256={'shared':'b'*64 if fault=='source_conflict' and index==1 else 'a'*64})
        if index==0: return dict(input_sha256={'original':'same'})
        return dict(prefix_artifact_sha256={'prefix':'same'},native_artifact_sha256={'native':'same'},
            performance_artifact_sha256={'performance':'same'})
    monkeypatch.setattr(runner,'read_json',read)
    def verify_current(launch):
        assert launch==dict(input_sha256={'original':'same'}); checked.append('current')
    def verify_prior(launch):
        assert 'input_sha256' not in launch and set(launch)=={
            'prefix_artifact_sha256','native_artifact_sha256','performance_artifact_sha256'}
        checked.append('prior')
        if fault=='original_verifier_failure': raise ValueError('predecessor authentication failed')
    monkeypatch.setattr(runner,'verify_inputs',verify_current)
    monkeypatch.setattr(runner,'verify_single_pass',verify_prior)
    if fault is None: assert runner.verify_predecessors()=={'shared':'a'*64}
    else:
        with pytest.raises(ValueError): runner.verify_predecessors()
    assert checked==['current','prior']
