...
from scripts import run_go2_independent_round_trip_population_eight_diagnostics_v1 as run
from lewm.tests.test_independent_round_trip_eight_diagnostic_final_admission_development import admissions
...
def bundle():
    ...
    return dict(schema='independent_round_trip_eight_diagnostic_completed_input_bundle.v1',source_sha256={'frozen.py':'a'*64},
        input_admission=inp,eight_stage_queue_admission=queue,final_policy_review_completed=False,
...
def test_bundle_preparation_joins_all_eight_actual_admission_outputs(monkeypatch):
    inp,q=admissions();calls=[]
    monkeypatch.setattr(run.admission.queue,'owners_ended',lambda:calls.append('owners'))
    ids={k:str(i+1)*64 for i,k in enumerate(('frontier','hold','contact','tracking','budget',
        'sustained_turn','contact_flow','chained_anchor'))}
    def old(batch,waiters,*,sources):
        assert batch==inp['adapter_batch_result_sha256']
        assert set(waiters)=={'frontier','hold','contact','tracking','budget'}
        calls.append('original_five')
        return dict(input_admission=inp,five_stage_queue_admission=q['original_five_stage_admission'])
    def later(five,waiters,*,sources):
        assert five is q['original_five_stage_admission']
        assert waiters=={k:ids[k] for k in ('sustained_turn','contact_flow','chained_anchor')}
        calls.append('later_three');return q
    monkeypatch.setattr(run.original,'prepare_input_bundle',old)
    monkeypatch.setattr(run.admission.queue,'admit',later)
    actual=run.prepare_input_bundle(inp['adapter_batch_result_sha256'],ids,sources={})
    assert actual['eight_stage_queue_admission'] is q
    assert actual['final_policy_review_completed'] is False
    assert actual['population_execution_permitted'] is False
    assert calls==['owners','original_five','later_three']
