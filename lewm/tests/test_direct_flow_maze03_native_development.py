...
from scripts import run_go2_direct_flow_maze03_pilot_v1 as runner
...
def test_original_native_and_evaluation_calculations_are_preserved():
    class Normalize(ast.NodeTransformer):
        ...
        def visit_Constant(self,n):
            if isinstance(n.value,str): n.value=n.value.replace('DIRECT_FLOW_MAZE03','MEASURED_FLOOR_TRANSPORT_MAZE')
            return n
        ...
        def visit_Assert(self,n):
            ...
            return self.generic_visit(n)
    for kind,names in [('episode',('collect','artifacts'))]:
        for name in names:
            old=function('scripts/measured_floor_transport_maze_'+kind+'_development.py',name)
            new=function('scripts/direct_flow_maze03_'+kind+'_development.py',name)
            assert ast.dump(old)==ast.dump(Normalize().visit(new)),(kind,name)


def test_fixed_original_maze3_and_completed_corrected_tracking_prefix():
    from scripts.direct_flow_maze01_audit_development import audit
    assert runner.audit is audit
    assert runner.CASE==('full_jepa_direct_flow_maze_03',3,'full','jepa','seed_2026091001_full_jepa')
    assert runner.LEARNED_CASE[0]=='full_jepa_novel_maze_03'
    assert runner.PREFIX.name=='go2_direct_flow_maze03_prefix_v1_attempt_001'
    assert runner.PREFIX_SHA=='104edd70824e7d4e7b909be924eb68a918542efd3464ff3247f9570ddc28669e'
    assert runner.OUTPUT.name=='go2_direct_flow_maze03_pilot_v1_attempt_001'
...
def test_worker_uses_fresh_models_and_retains_evidence_after_failure(monkeypatch,tmp_path,failed_stage):
    ...
    if failed_stage is None:
        assert record['status']=='DIRECT_FLOW_MAZE03_COLLECTED_AND_RAW_AUDITED'
        ...
        assert record['status']=='DIRECT_FLOW_MAZE03_WORKER_FAILED'
