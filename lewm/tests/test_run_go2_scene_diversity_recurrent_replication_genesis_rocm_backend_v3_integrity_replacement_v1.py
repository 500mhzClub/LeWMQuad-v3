...
from scripts import (
    run_go2_scene_diversity_recurrent_replication_genesis_rocm_backend_v3_integrity_replacement_v1
    as runner,
)
...
def scientific_plan() -> dict:
    ...
    frozen = copy.deepcopy(
        runner.plan_builder.predecessor.predecessor.predecessor._IMMUTABLE_FROZEN_VULKAN_PLAN  # noqa: SLF001
    )
...
def test_source_closure_freezes_v3_and_adds_review_only_custody() -> None:
    ...
    assert len(runner.FROZEN_V3_SOURCE_PATHS) == 220
    assert len(runner.SOURCE_PATHS) == 236
    assert runner.SOURCE_PATHS["v2_rocm_preregistration_source"] == (
        runner.V2_PREREGISTRATION_SOURCE
    )
    assert runner.SOURCE_PATHS[
        "predecessor_v3_qualification_terminal_review"
    ] == runner.plan_builder.V3_QUALIFICATION_TERMINAL_REVIEW
...
def test_failure_evidence_adds_only_exact_v3_review_and_literal_source() -> None:
    evidence = runner.predecessor_failure_bindings_rocm()
    assert evidence["predecessor_v3_qualification_terminal_review"] == (
        runner._standard_binding(  # noqa: SLF001
            runner.plan_builder.v3_qualification_terminal_review_binding()
        )
    )
...
def test_execute_uses_frozen_science_and_v3_home_guard(
    ...
    authority = {
        ...
        "predecessor_v3_qualification_terminal_review_binding": (
            runner.collector._standard_v3_review_binding()  # noqa: SLF001
        ),
    }
