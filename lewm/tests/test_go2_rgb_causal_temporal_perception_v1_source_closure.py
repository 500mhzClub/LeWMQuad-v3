...
SPEC = importlib.util.spec_from_file_location(
    "_test_go2_rgb_causal_temporal_perception_v1_source_closure",
    CHECKER_PATH,
)
...
def test_dynamic_edges_tests_and_temporal_mechanism_are_in_closure() -> None:
    ...
    assert not any(
        path.startswith(checker._BASE.FORBIDDEN_RUNNER_PREFIX)
        for path in paths
    )
