"""Focused standalone-authority and lifecycle tests for projection V5."""
...
def _stack_v5(*, origin: tuple[float, float] = (-1.6, -1.6)):
    physical_frame = MapFrameIdentity(
        session_id=f"native-v5-physical:{origin}",
        origin_xy_m=origin,
        cell_size_m=0.05,
        frame_id="native_physical",
    )
    configuration_frame = MapFrameIdentity(
        session_id=f"native-v5-configuration:{origin}",
        origin_xy_m=origin,
        cell_size_m=0.10,
    ...
    adapter = NativeLearnedPhysicalProjectionAdapterV5(
        memory=memory,
...
def _mutate_and_rehash_v5(package, *, timestamp: int) -> None:
...
def _seed_retraction_identity_collision_v5(
    memory: RevisionedPhysicalMemory,
    observation_id: str,
) -> None:
    payload_sha256 = v1_module._sha256(
        {
            "schema": "v5-retraction-identity-collision",
...
def _retract_seeded_identity_v5(
    memory: RevisionedPhysicalMemory,
    observation_id: str,
) -> str:
    retraction_id = f"seed-retraction:{observation_id}"
    memory.apply_transaction(
        PhysicalEvidenceTransaction(
            observation=ObservationIdentity(
                observation_id=retraction_id,
                payload_sha256=v1_module._sha256(
                    {
                        "schema": "v5-seeded-identity-retraction",
            ...
            map_frame=memory.map_frame,
            pose=_pose(timestamp=701),
            retract_learned_observation_ids=(observation_id,),
            projection_contract_sha256=PROFILE_SHA256,
        )
    )
...
def _advance_v5(projection, snapshot, runner, adapter, *, sequence: int):
    package = adapter.issue(
        snapshot,
        _outcome(
            runner,
...
def test_v5_is_standalone_and_old_bound_or_unbound_methods_reject() -> None:
    memory, projection, snapshot, runner, adapter = _stack_v5()
    package = adapter.issue(
        snapshot,
        _outcome(runner, snapshot, ground=(_ground_row(),)),
    ...
    with pytest.raises((AttributeError, TypeError)):
        NativeLearnedPhysicalProjectionAdapterV4.commit(adapter, package)
    with pytest.raises((AttributeError, TypeError)):
        NativeLearnedPhysicalProjectionAdapterV4.issue_retraction(
            adapter,
            current,
            package,
        )
    ...
    with pytest.raises((AttributeError, TypeError)):
        NativeLearnedPhysicalProjectionAdapterV4.issue(
            adapter,
            current,
            unused_outcome,
        )
...
def test_v5_closes_reachable_engine_mutated_target_bypass() -> None:
    memory, projection, _runner, adapter, target = _committed_target_v5()
    _mutate_and_rehash_v5(target, timestamp=101)
    current = projection.project()
...
def test_v5_target_digest_is_checked_at_issue_and_final_commit() -> None:
    memory, projection, _runner, adapter, target = _committed_target_v5()
    _mutate_and_rehash_v5(target, timestamp=102)
    with pytest.raises(
        NativeLearnedProjectionBindingError,
        match="issued content",
    ):
    ...
    memory2, projection2, _runner2, adapter2, target2 = _committed_target_v5()
    retraction = adapter2.issue_retraction(projection2.project(), target2)
    _mutate_and_rehash_v5(target2, timestamp=103)
    with pytest.raises(
        NativeLearnedProjectionBindingError,
        match="issued content",
    ):
...
def test_v5_copy_forgery_transfer_mutation_and_replay_cannot_remove_target() -> None:
    memory, projection, runner, adapter, target = _committed_target_v5()
    ...
    foreign = NativeLearnedPhysicalProjectionAdapterV5(
        memory=memory,
        projection=projection,
        runner=runner,
        calibration=CALIBRATION,
    ...
    _mutate_and_rehash_v5(mutated, timestamp=104)
    with pytest.raises(NativeLearnedProjectionBindingError):
        adapter2.commit(mutated)
    assert target2.observation_id in memory2.learned_observation_ids


def test_v5_failed_stale_commit_releases_for_retry_and_old_stays_terminal() -> None:
    memory, projection, runner, adapter, target = _committed_target_v5()
    shared = projection.project()
    stale = adapter.issue_retraction(shared, target)
    _advance_v5(projection, shared, runner, adapter, sequence=2)
...
def test_v5_proactive_stale_retry_allows_exactly_one_live_retraction() -> None:
    memory, projection, _runner, adapter, target = _committed_target_v5()
    ...
    with pytest.raises(
        NativeLearnedProjectionReplayError,
        match="exact live V5 retraction",
    ):
        adapter.issue_retraction(current, target)
    with pytest.raises(
        NativeLearnedProjectionReplayError,
...
def test_v5_removal_is_atomic_and_preserves_other_active_identity() -> None:
    memory, projection, runner, adapter, target = _committed_target_v5()
    other = _advance_v5(
        projection,
        projection.project(),
...
def test_v5_preserves_free_rotation_boundary_and_occupied_precedence() -> None:
    memory, _projection, snapshot, runner, adapter = _stack_v5()
    free = adapter.issue(
        snapshot,
...
def test_v5_preserves_translation_covariance_and_native_resolution_rules() -> None:
    _memory, _projection, snapshot, runner, adapter = _stack_v5()
...
def test_v5_contradiction_is_unknown_and_exact_retraction_restores_free() -> None:
    memory, projection, snapshot, runner, adapter = _stack_v5()
    free = adapter.issue(
        snapshot,
        _outcome(runner, snapshot, ground=(_ground_row(),), sequence=1),
...
def test_v5_every_callable_commit_path_rechecks_final_target_binding() -> None:
    memory, projection, _runner, adapter, target = _committed_projection_v5()
    retraction = adapter.issue_retraction(projection.project(), target)
    _mutate_and_rehash_v5(target, timestamp=703)
    ...
    with pytest.raises(NativeLearnedProjectionBindingError, match="issued content"):
        adapter._commit_core_v5(retraction)
    assert target.observation_id in memory.learned_observation_ids


def test_v5_permanent_late_rejection_releases_target_slot() -> None:
    memory, projection, _runner, adapter, target = _committed_projection_v5()
    colliding_id = (
        "qualified-native-v4-v5-retract:1:" f"{target.observation_id}"
    )
    _seed_retraction_identity_collision_v5(memory, colliding_id)
...
def test_v5_retracted_duplicate_identity_is_permanent_and_releases_target() -> None:
    memory, projection, _runner, adapter, target = _committed_projection_v5()
    colliding_id = (
        "qualified-native-v4-v5-retract:1:" f"{target.observation_id}"
    )
    _seed_retraction_identity_collision_v5(memory, colliding_id)
    seed_retraction_id = _retract_seeded_identity_v5(memory, colliding_id)

    assert colliding_id not in memory.learned_observation_ids
    assert colliding_id in memory.seen_observation_ids
    assert seed_retraction_id in memory.seen_observation_ids
...
def test_v5_transient_late_rejection_keeps_exact_package_retryable(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    memory, projection, _runner, adapter, target = _committed_projection_v5()
    retraction_snapshot = projection.project()
    retraction = adapter.issue_retraction(retraction_snapshot, target)
    revision_before = memory.revision
    ...
    with pytest.raises(
        ...
        adapter.issue_retraction(retraction_snapshot, target)
...
def test_v5_contract_authority_copy_serialization_and_production_fail_closed() -> None:
    _memory, _projection, snapshot, runner, adapter = _stack_v5()
    package = adapter.issue(
        snapshot,
        _outcome(runner, snapshot, ground=(_ground_row(),)),
...
def test_v5_source_has_no_old_adapter_or_runtime_input_surface() -> None:
    source = Path(v5_module.__file__).read_text(encoding="utf-8").lower()
    for forbidden in (
        "nativelearnedphysicalprojectionadapterv1",
        "nativelearnedphysicalprojectionadapterv2",
        "nativelearnedphysicalprojectionadapterv3",
