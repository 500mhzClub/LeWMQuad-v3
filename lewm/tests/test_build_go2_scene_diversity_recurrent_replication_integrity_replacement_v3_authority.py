...
from scripts import (
    build_go2_scene_diversity_recurrent_replication_integrity_replacement_v3_authority
    as authority,
)
...
def reviewed_inputs(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> dict:
    ...
    preregistration_path.write_text("# replacement V3 preregistration\n")
    ...
    monkeypatch.setattr(
        authority.runner, "predecessor_failure_bindings_v2", lambda: {}
    )
    source_bindings = authority.source_bindings_v3()

    preregistration_binding = authority.file_binding_v3(preregistration_path)
    panel_binding = authority.file_binding_v3(authority.SCENE_PANEL)
    plan_binding = authority.file_binding_v3(plan_path)
    review = {
    ...
    review_binding = authority.file_binding_v3(review_path)
    ...
    dino = {
        ...
        "checkpoint_binding": authority.file_binding_v3(dino_checkpoint),
    }
    monkeypatch.setattr(
...
def _build(reviewed_inputs: dict) -> dict:
    return authority.build_authority_v3(
...
def test_authority_binds_fresh_v3_without_creating_attempt(
...
def test_authority_rejects_missing_or_changed_audits(
...
def test_authority_rejects_scientific_drift_or_nonfresh_root(
    ...
    drift = copy.deepcopy(reviewed_inputs["plan"])
    drift["execution_contract"]["seed"] += 1
    with pytest.raises(
        authority.SceneDiversityReplacementAuthorityError,
        match="science-identical",
    ):
        authority._validate_science_identical_plan_v3(drift)  # noqa: SLF001
...
def test_scoped_authority_overlay_is_restored() -> None:
    overrides = authority._configuration_overrides_v3()  # noqa: SLF001
    original = {
        name: getattr(authority.predecessor_authority, name)
        for name in overrides
    }

    with authority._configured_predecessor_authority_v3():  # noqa: SLF001
        assert all(
