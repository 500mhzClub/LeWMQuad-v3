...
from lewm.tests.test_progressive_batched_floor_controller_completion_development import histories


def fixture(monkeypatch,tmp_path):
    rows,prior_rows=histories()
    sources={'source.py':'a'*64};admission={'original_raw_and_model':'unchanged'}
    prior_report=dict(incremental_density_routed_floor_comparison=True,
        both_controllers_use_visibility_batched_receipt_copied_packed_fused_queries=True,
    ...
    result=dict(status='PROGRESSIVE_BATCHED_FLOOR_LATE_HISTORY_V1_COMPLETE',source_sha256=sources.copy(),
