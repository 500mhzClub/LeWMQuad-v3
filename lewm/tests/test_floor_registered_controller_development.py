...
from lewm.floor_registered_evidence_development import FloorRegistration, current_registered_pose, depth_hash
...
def test_all_online_consumers_share_registered_pose_and_raw_witness_survives():
    c = FloorRegisteredRoundTripController(None, None, public_mission=dict(
    ...
    for frame in range(2):
        ...
        e = result['evidence']; position, R, _ = current_registered_pose(e, identity=(0, 0, 0), now_ns=now)
...
def test_composition_and_causal_witness_tampering_rejected(fault):
    ...
    e = FloorRegistration().observe(p, d, a, raw, now_ns=now)
    ...
    elif fault == 'pair': e['floor_registration']['paired_plane']['offset_body_m'] += .01
    elif fault == 'reference': e['floor_registration']['reference']['frame'] = 1
    elif fault == 'clock': e['decision_ns'] += 1
    else: e['identity'] = (0, False, 0)
    with pytest.raises(ValueError): current_registered_pose(e, identity=(0, 0, 0), now_ns=now)
...
def test_three_pose_consumer_derivatives_only_change_admission_accessor():
    ...
    class Normalize(ast.NodeTransformer):
        def visit_Name(self, node):
            if node.id == 'current_registered_pose': node.id = 'current_joint_pose'
            return node
    for path, old, new, name in (
        ('joint_visual_surface_memory', 'JointVisualSurfaceMemory', 'FloorRegisteredSurfaceMemory', 'observe'),
        ('online_executed_residual', 'OnlineExecutedResidual', 'FloorRegisteredResidual', 'observe'),
        ('observed_round_trip_controller', 'ObservedRoundTripController', 'FloorRegisteredRoundTripController', 'advance')):
        a = method('lewm/'+path+'_development.py', old, name)
        b = Normalize().visit(method('lewm/floor_registered_controller_development.py', new, name))
