...
from scripts import verify_go2_visibility_batched_footprint_completion_v1 as check
...
def test_report_retains_original_state_and_negative_evidence_without_mutation():
    prior = dict(incremental_footprint_evidence_copy_comparison=True,
        both_controllers_use_packed_fused_scoped_queries=True, observed_state_checks=[{'frame': 1173, 'sha': 'old'}],
    ...
    assert expected['candidate'] == 'VisibilityBatchedFootprintController'
    assert 'incremental_footprint_evidence_copy_comparison' not in expected


def test_wrong_predecessor_scope_rejected():
    with pytest.raises(ValueError, match='receipt-copy'):
        check.expected_report(dict(incremental_footprint_evidence_copy_comparison=False), {})
