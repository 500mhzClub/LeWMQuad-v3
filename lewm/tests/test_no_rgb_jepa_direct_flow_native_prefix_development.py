...
from lewm.tests.test_no_rgb_jepa_direct_flow_controller_prefix_development import early, boundary
...
def fixture():
    ...
    for frame in range(prefix.FRAMES):
        ...
        saved.append(dict(tick=frame,decision=new,comparison=check,original_requested_command=old['requested_command'],
            public_input_arrays_unchanged=True,public_input_sha256='packet_'+str(frame)))
    ...
    report=dict(frames=860,exact_original_decisions=859,original_forecasts_compared=856,
...
def test_complete_controller_prefix_reconstructs_and_allows_an_honest_hold():
    ...
    assert r==dict(frames=860,first_intervention_frame=859,original_forecasts_compared=856,complete_saved_comparisons_reconstructed=True)
...
def test_invalid_saved_prefix_is_rejected(fault):
    ...
    elif fault=='forecast':saved[800]['decision']['new_selection']['prediction']['x'][0]+=1
    ...
    elif fault=='comparison':saved[800]['comparison']['stop']=True
    elif fault=='command':tape[800]['requested_command']=[.2,0.,0.]
...
@pytest.mark.parametrize('key,value',[
    ('frames',True),('frames',859),('exact_original_decisions',858),('original_forecasts_compared',True),
...
def test_candidate_command_must_actually_complete(fault):
    ...
    elif fault=='earlier':new[850]['requested_command']=[0.,0.,.45]
...
def physical_fixture(monkeypatch,tmp_path,fault=None):
    ...
    def packets(p):
        for i in range(prefix.FRAMES):
            yield 'wrong' if fault=='public' and p==current and i==859 else 'packet_'+str(i)
    monkeypatch.setattr(prefix,'public_packets',packets)
    return prior,current,root,report


def test_exact_43700_samples_and_completed_hold_are_checked(monkeypatch,tmp_path):
    result=prefix.compare(*physical_fixture(monkeypatch,tmp_path))
    assert result['physical_prefix_samples']==43700 and result['common_prefix_frames']==860
...
@pytest.mark.parametrize('fault',['physics','truncated_physics','decision','completion','row_clock','short','public'])
...
def admission_fixture(monkeypatch,tmp_path,fault=None):
    ...
    result=dict(status='NO_RGB_JEPA_DIRECT_FLOW_CONTROLLER_PREFIX_V1_COMPLETE',native_execution=False,
    ...
    launch=dict(source_sha256=sources,case=list(prefix.replay.CASE),model_state_sha256=prefix.replay.MODEL_SHA,
        boundary_frame=859,implementation_class='DirectFlowResidualAnchoredController',
        full_original_input_admission_reexecuted=True,observer_artifact_sha256=observer_ids,input_artifact_sha256=original_inputs)
    ...
    elif fault=='admission':launch['full_original_input_admission_reexecuted']=False
    ...
    prior=prefix.replay.batch.OUTPUT/prefix.replay.CASE[0]
    def read(p,name):
...
def test_admission_reconstructs_every_row_and_requests_all_bindings(monkeypatch,tmp_path):
    ...
    assert (root,{'result.json':'e'*64}) in checked
    assert (prefix.replay.batch.OUTPUT,original_inputs) in checked
