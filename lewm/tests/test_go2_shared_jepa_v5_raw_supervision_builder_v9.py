"""Source-free author tests for the standalone raw-supervision Builder V9."""
...
def validate_phase_two_for_tests(
    phase_one: builder.PhaseOneAuthorizationV9,
    *,
    repository_root: Path,
    reader: Reader,
) -> tuple[builder.AcceptedAuthorizationV9, tuple[str, ...]]:
    ...
    if type(phase_one) is not builder.PhaseOneAuthorizationV9:
        raise TypeError("test phase two requires a completed V9 phase-one capsule")
    ...
    return (
        builder.AcceptedAuthorizationV9(
            authorization_file_sha256=phase_one.authorization_file_sha256,
...
def write_prepared_scene_job_for_tests(
    job: builder.PreparedSceneJobV9,
    staging_root: Path,
) -> dict[str, Any]:
    ...
    receipt = builder.AcceptedAuthorizationV9("a" * 64, "b" * 64, "c" * 64)
...
def test_v9_amendment_and_v8_evidence_are_frozen() -> None:
    assert _sha(ROOT / AMENDMENT) == AMENDMENT_SHA256
    assert {
        relative: _sha(ROOT / relative) for relative in V8_PREDECESSOR_EVIDENCE
    } == V8_PREDECESSOR_EVIDENCE
    ...
    assert all(
        ...
        for relative, digest in V8_PREDECESSOR_EVIDENCE.items()
    )
    assert all(
        builder.FROZEN_PARENT_HASHES[relative] == digest
        for relative, digest in V6_BLOCK.items()
    )
    assert len(builder.FROZEN_PARENT_HASHES) == 83
    assert (
    ...
    assert builder_v8_review["verdict"] == "PASS"
    assert builder_v8_review["content_sha256"] == (
        "c12c037bd2d6c3cbe099c1b0b94cc13924ef134c0c025161722df0c0f5944122"
    )
    assert auditor_v8_review["verdict"] == "BLOCK"
    assert auditor_v8_review["content_sha256"] == (
        "fdc52fe997c7e83df0971b1b37626d771ffaa7b2dd9676b4868db4ee24844f15"
    )


def test_v9_is_mechanical_v8_definition_successor() -> None:
    v8 = _normalized_top_level_definitions(
        ROOT / "lewm/datasets/go2_shared_jepa_v5_raw_supervision_builder_v8.py",
        normalize_v8=True,
    )
    v9 = _normalized_top_level_definitions(Path(builder.__file__), normalize_v8=False)
    assert len(v8) == 80
    assert v9 == v8


def test_builder_publication_linearization_drain_is_last_observation() -> None:
    tree = ast.parse(Path(builder.__file__).read_text(encoding="utf-8"))
    transaction = next(
        node
        for node in tree.body
        if isinstance(node, ast.ClassDef)
        and node.name == "_ClosedPublicationTransaction"
    )
    final = next(
        node
        for node in transaction.body
        if isinstance(node, ast.FunctionDef) and node.name == "require_final_quiet"
    )
    calls = sorted(
        (node for node in ast.walk(final) if isinstance(node, ast.Call)),
        key=lambda node: (node.lineno, node.col_offset),
    )
    assert _call_name(calls[-1]) == "_require_no_events"
    assert len(calls[-1].args) == 1
    assert isinstance(calls[-1].args[0], ast.Constant)
    assert calls[-1].args[0].value == "final publication identity"


def test_v5_race_is_reproduced_and_v9_transaction_spans_publication() -> None:
    ...
    v9_calls = _ordered_calls(Path(builder.__file__), "_build_exact_prepared_dataset_v9")
    ...
    v9_names = [name for _line, name in v9_calls]
    assert v9_names.count("_ClosedPublicationTransaction") == 1
    assert v9_names.count("_revalidate_exact_before_publication") == 1
    assert v9_names.count("validate_before_rename") == 1
    assert v9_names.count("rename_owned") == 1
    assert v9_names.count("validate_after_rename") == 1
    assert v9_names.count("require_final_quiet") == 1
    assert "_libc_renameat2" not in v9_names
    transaction_index = v9_names.index("_ClosedPublicationTransaction")
    revalidate_index = v9_names.index("_revalidate_exact_before_publication")
    validate_index = v9_names.index("validate_before_rename")
    rename_index = v9_names.index("rename_owned")
    post_index = v9_names.index("validate_after_rename")
    quiet_index = v9_names.index("require_final_quiet")
    ...
    for name in required_before_second_pass:
        prepublication_occurrences = [
            line
            for line, observed_name in v9_calls
...
def test_v6_ancestor_gap_is_reproduced_and_v9_rejects_it(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    v9_root = tmp_path / "v9" / "canonical"
    v9_root.mkdir(parents=True)
    transaction, retained, paths = _open_synthetic_transaction(v9_root, monkeypatch)
    moved = tmp_path / "v9-moved"
    try:
        transaction.validate_before_rename()
        transaction.rename_owned()
        retained.refresh_after_owned_mutation()
        transaction.validate_after_rename()
        os.fsync(retained.parent_fd)
        v9_root.rename(moved)
        v9_root.mkdir()
        (v9_root / "publication").mkdir()
...
def test_canonical_v9_roles_schemas_and_authors_are_exact() -> None:
    ...
    assert builder.AUTHORIZATION_SCHEMA.endswith("build_authorization_v9")
    assert builder.REVIEW_BINDING_SCHEMA.endswith("review_binding_v9")
    assert builder.BUILDER_REVIEW_SCHEMA.endswith(
        "builder_v9_independent_review_v1"
    )
    assert builder.AUDITOR_REVIEW_SCHEMA.endswith(
        "auditor_v9_independent_review_v1"
    )
    assert (
...
def test_v9_cli_imports_only_v9_exact_entry() -> None:
    ...
    assert "go2_shared_jepa_v5_raw_supervision_builder_v9" in cli
    assert "execute_exact_build_v9" in cli
    assert "execute_exact_build_v8" not in cli
    assert "execute_exact_build_v7" not in cli
    assert "go2_shared_jepa_v5_raw_supervision_builder_v8 import" not in cli
...
def test_import_surface_exposes_no_legacy_builder_or_nonpure_loader() -> None:
    ...
    legacy_modules = {
        ...
        and (
            ...
            or "raw_supervision_builder_v8" in value.__name__
...
def test_production_signatures_have_no_injection_seams() -> None:
    ...
    exact = inspect.signature(builder.execute_exact_build_v9)
    ...
    build = inspect.signature(builder._build_exact_prepared_dataset_v9)
    forbidden = {
        "output_directory",
    ...
    assert tuple(field.name for field in fields(builder.AcceptedAuthorizationV9)) == (
        "authorization_file_sha256",
        "authorization_content_sha256",
        "source_map_sha256",
    )
    receipt = builder.AcceptedAuthorizationV9("1" * 64, "2" * 64, "3" * 64)
...
def test_source_uses_only_fixed_worker_targets_and_authorized_initializers() -> None:
    ...
    assert not any(
        ...
        or "raw_supervision_builder_v8" in module
...
def test_absent_authority_reaches_no_byte_or_metadata_opener(
    ...
    with pytest.raises(PermissionError, match="authorization is absent"):
        builder.execute_exact_build_v9(
...
def test_deterministic_shard_science_and_layout_without_exact_data(
    ...
    reversed_job = builder.PreparedSceneJobV9(
...
def test_one_and_six_worker_full_synthetic_build_bytes_are_identical(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    source = tmp_path / "source.bin"
    source.write_bytes(b"frozen synthetic source\n")
    source_sha256 = _sha(source)
    authorization_sha256 = "d" * 64
    job, pair = _synthetic_job_and_pair()
    observed_workers: list[int] = []

    def inline_executor_factory(
        *,
        max_workers: int,
        initializer: Any = None,
        initargs: tuple[Any, ...] = (),
    ...
    monkeypatch.setattr(builder, "ProcessPoolExecutor", inline_executor_factory)
    monkeypatch.setattr(
        builder,
        "_precommitted_audit_sample",
    ...
    def build(*, workers: int, parent: Path) -> tuple[dict[str, Any], dict[str, str]]:
        output = parent / "dataset"
        ...
        return manifest, _tree_hashes(output)

    one_manifest, one_hashes = build(workers=1, parent=tmp_path / "one")
    six_manifest, six_hashes = build(workers=6, parent=tmp_path / "six")
    assert observed_workers == [1, 6]
    assert builder.canonical_json_bytes(one_manifest) == builder.canonical_json_bytes(
        six_manifest
    )
    assert one_hashes == six_hashes


def test_join_rejects_duplicate_endpoint_and_cross_role() -> None:
    job, pair = _synthetic_job_and_pair()
    duplicate = builder.PreparedSceneJobV9(
        scene_id=job.scene_id,
        role=job.role,
        family=job.family,
...
def test_bound_reader_rejects_intermediate_canonical_symlink(tmp_path: Path) -> None:
    real = tmp_path / "real"
    real.mkdir()
    payload = real / "source.json"
...
def test_full_build_rejects_mutation_during_final_source_pass(
    ...
    with pytest.raises(builder.RawSupervisionBuildError, match="poisoned"):
        builder._build_exact_prepared_dataset_v9(
            (job,),
            (pair,),
            workers=1,
            input_provenance={},
