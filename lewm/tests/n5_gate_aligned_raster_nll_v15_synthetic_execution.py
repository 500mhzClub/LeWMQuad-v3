"""Production-ineligible V15 filesystem lifecycle used only by tests.
...
STAGING_PREFIX = ".n5.synthetic-v15-"
LOCK_NAME = ".n5.synthetic-v15.lock"
SERIALIZER_SUFFIX = ".n5.synthetic-v15.serializer"
SCHEMA = "lewm_go2_n5_gate_aligned_raster_nll_v15_synthetic_reservation_v1"
MANIFEST_SCHEMA = "lewm_go2_n5_gate_aligned_raster_nll_v15_synthetic_staging_v1"


def _gate_binary_metrics(
    negative: int,
    positive: int,
    *,
    perfect: bool,
    ...
    present = [
        value for value in (negative_recall, positive_recall) if value is not None
    ]
    return {
...
def _gate_raster_metrics(
    ...
    *,
    ...
    present: list[float] = []
    for index, (class_name, count) in enumerate(
        zip(gate.RASTER_CLASSES, target_counts, strict=True)
    ):
        correct = count if perfect else count // 2
        matrix[index][index] = correct
        matrix[index][(index + 1) % 3] = count - correct
        recall = None if count == 0 else correct / count
        recalls[class_name] = recall
        if recall is not None:
            present.append(recall)
    nll = 0.01 if perfect else 1.0
    count = sum(target_counts)
    return {
        "nll": nll,
        "nll_sum": nll * count,
        "confusion_target_rows_predicted_columns": matrix,
...
def _gate_metric_bundle(*, perfect: bool) -> dict[str, Any]:
    from lewm.benchmarks import (
        go2_observable_camera_ray_fit_v4_ladder_gate as gate,
    )

    signature = gate.EXPECTED_TARGET_PARTITION_SIGNATURES[5]
    depth_count = int(signature["depth_count"])
    ...
    return {
        "frame_count": 5,
        "pixel_hit_no_hit": _gate_binary_metrics(
            *signature["pixel"],
            perfect=perfect,
        ),
        "pixel_hit_depth": {
        ...
        "ground_clear": {
            "overall": _gate_binary_metrics(
                *signature["ground_overall"],
                perfect=perfect,
            ),
            "by_distance_m": {
                group: _gate_binary_metrics(*counts, perfect=perfect)
                for group, counts in signature["ground_by_distance"].items()
            },
            "by_family": {
                family: _gate_binary_metrics(*counts, perfect=perfect)
                for family, counts in signature["ground_by_family"].items()
            },
        },
        "derived_raster": _gate_raster_metrics(
            signature["raster_target_counts"],
            perfect=perfect,
        ),
    }


def complete_retained_gate_evaluation_v15() -> dict[str, Any]:
    """Fixed production-ineligible hierarchical evaluation for gate proofs."""
    ...
    def control(name: str, mapping: list[int], *, perfect: bool) -> dict[str, Any]:
        metrics = _gate_metric_bundle(perfect=perfect)
        raster = metrics["derived_raster"]
        nll = float(raster["nll"])
        target_counts = [sum(row) for row in raster["confusion_target_rows_predicted_columns"]]
        class_names = ("UNKNOWN", "FREE", "OCCUPIED")
        family_names = sorted(metrics["ground_clear"]["by_family"])
        family_count = int(raster["count"]) // len(family_names)
        if family_count * len(family_names) != raster["count"]:
            raise ValueError("V15 synthetic family partition is not even")
        diagnostics = {
            "overall": {
                "count": int(raster["count"]),
                "nll_sum": float(raster["nll_sum"]),
                "mean": nll,
            },
            "by_target_class": {
                class_name: {
                    "count": count,
                    "nll_sum": nll * count,
                    "mean": None if count == 0 else nll,
                }
                for class_name, count in zip(
                    class_names, target_counts, strict=True
                )
            },
            "by_family": {
                family: {
                    "count": family_count,
                    "nll_sum": nll * family_count,
                    "mean": nll,
                }
        ...
        return {
            ...
            "native_v15_objective": {
                "derived_raster_cell_nll": nll,
                "v11_base_total": losses["total"],
                "total": losses["total"] + 0.25 * nll,
            },
    ...
    return {
        ...
        "wrong_rgb_with_target_calibration": control(
...
def _directory_flags() -> int:
    ...
    if not nofollow or not directory:
        raise PermissionError("synthetic V15 requires no-follow directory opens")
...
class _WatchV15:
    ...
    label: str
    generation: int
    snapshot: _SnapshotV15


class _DeclaredJournalV15:
    """A deterministic test journal for declared filesystem state deltas."""

    def __init__(self) -> None:
        self._watches: dict[tuple[int, int], _WatchV15] = {}
        self._generation = 0
    ...
    def _fail(self, reason: str) -> None:
        ...
        raise PermissionError(f"synthetic V15 journal rejected: {reason}")
    ...
    @staticmethod
    def _snapshot(directory_fd: int) -> _SnapshotV15:
        ...
        return _SnapshotV15(_fingerprint(opened), rows)
    ...
    def watch(self, directory_fd: int, *, label: str) -> None:
        ...
        self._generation += 1
        self._watches[identity] = _WatchV15(
            retained,
            identity,
            label,
            self._generation,
    ...
    def _snapshots(self) -> dict[tuple[int, int], _SnapshotV15]:
        return {
    ...
    ) -> tuple[_WatchV15, dict[tuple[int, int], _SnapshotV15]]:
        self.assert_clean()
    ...
    def _finish(
        self,
        before: Mapping[tuple[int, int], _SnapshotV15],
        *,
...
@dataclass
class _AncestorChainV15:
    descriptors: list[int]
    entries: list[_AncestorEntryV15]
    closed: bool = False


def _open_ancestor_chain(path: Path) -> _AncestorChainV15:
    filesystem_root = Path(path.anchor)
    anchor_fd = os.open(filesystem_root, _directory_flags())
    chain = _AncestorChainV15([anchor_fd], [])
    parent_fd = anchor_fd
    try:
        for component in path.parts[1:]:
            ...
            chain.entries.append(
                _AncestorEntryV15(parent_fd, component, child_fd, identity)
            )
            parent_fd = child_fd
        return chain
...
def _assert_ancestor_chain(chain: _AncestorChainV15) -> None:
    if chain.closed:
...
def _close_ancestor_chain(chain: _AncestorChainV15) -> None:
    if chain.closed:
        return
    chain.closed = True
...
class SyntheticReservationV15:
    ...
    seed_root_fd: int
    derived_fd: int
    journal: _DeclaredJournalV15
    ancestor_chain: _AncestorChainV15
    owned_directory_fds: tuple[int, ...]
    owned_claim_artifacts: dict[str, _OwnedArtifactV15] = field(
        default_factory=dict, compare=False
    )
    owned_derived_artifacts: dict[str, _OwnedArtifactV15] = field(
        default_factory=dict, compare=False
    )


class SyntheticExecutionV15:
    ...
    def __copy__(self) -> "SyntheticExecutionV15":
        return type(self)(self.root)

    def __deepcopy__(self, memo: object) -> "SyntheticExecutionV15":
        del memo
        return type(self)(self.root)
    ...
    def _new_staging(
        self, seed_fd: int, journal: _DeclaredJournalV15
    ) -> tuple[str, int]:
        for _ in range(128):
    ...
    @classmethod
    def _assert_claim_fd(cls, reservation: SyntheticReservationV15) -> None:
    ...
    @classmethod
    def _assert_claim(cls, reservation: SyntheticReservationV15) -> None:
    ...
    def _write_claim(
        cls,
        reservation: SyntheticReservationV15,
        name: str,
        raw: bytes,
        *,
        terminal: bool = False,
        ...
        reservation.owned_claim_artifacts[name] = _OwnedArtifactV15(
    ...
    @classmethod
    def _write_derived(
        cls, reservation: SyntheticReservationV15, name: str, raw: bytes
    ) -> None:
        cls._assert_claim(reservation)
        fingerprint = reservation.journal.create(
            reservation.derived_fd, name, raw
        )
        reservation.owned_derived_artifacts[name] = _OwnedArtifactV15(
    ...
    def claim(
        ...
        with self._serializer():
            ...
            claimed: SyntheticReservationV15 | None = None
            ...
            try:
                ...
                claimed = SyntheticReservationV15(
                    self.attempt,
                    value,
                    raw,
                    active_fd,
    ...
    def _cleanup_owned(
        reservation: SyntheticReservationV15,
        registry: dict[str, _OwnedArtifactV15],
        *,
        ...
        *,
        ...
        if not reservation.journal.poisoned:
            try:
                ...
                pass
        core = {
            "schema": "lewm_go2_n5_gate_aligned_raster_nll_v15_synthetic_failure_v1",
            "status": "failed",
            "failure_stage": stage,
    ...
    def publish(self, reservation: SyntheticReservationV15, raw: bytes) -> None:
    ...
    @staticmethod
    def close(reservation: SyntheticReservationV15) -> None:
        try:
