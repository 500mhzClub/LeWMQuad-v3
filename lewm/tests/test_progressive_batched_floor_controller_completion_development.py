...
from scripts import verify_go2_progressive_batched_floor_controller_completion_v1 as check
...
def test_report_retains_original_state_and_negative_evidence_without_mutation():
    prior = dict(incremental_density_routed_floor_comparison=True,
        both_controllers_use_visibility_batched_receipt_copied_packed_fused_queries=True, observed_state_checks=[{'frame': 1173, 'sha': 'old'}],
    ...
    assert expected['candidate'] == 'ProgressiveBatchedFloorController'
    assert 'incremental_density_routed_floor_comparison' not in expected


def test_wrong_predecessor_scope_rejected():
    with pytest.raises(ValueError, match='density-routed'):
        check.expected_report(dict(incremental_density_routed_floor_comparison=False), {})
