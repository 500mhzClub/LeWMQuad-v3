...
def fixture():
    prefix = dict(frames=4, first_changed_command_frame=3, raw_model_forecast_comparisons=1,
        original_requested_command=[0., 0., .45], candidate_requested_command=[.2, 0., 0.])
    audit = dict(layout_index=2, ordinary_waypoint_commitment_contact_enabled=True,
    ...
    receipt = dict(common_prefix_frames=4, first_intervention_frame=3, physical_prefix_samples=900,
        raw_model_forecast_comparisons=1, physical_and_public_prefix_exact=True,
        ...
        original_intervention_command=[0., 0., .45], candidate_intervention_command=[.2, 0., 0.],
    ...
    record = dict(status=native.WORKER_STATUS, case=native.CASE[0], layout_index=2,
        ...
        collection=dict(status='COMMITMENT_CONTACT_ANCHORED_MAZE02_TERMINAL_AUDIT_REQUIRED', ordinary_waypoint_commitment_contact_enabled=True),
...
def test_incomplete_changed_or_overclaimed_native_result_is_rejected(fault):
    ...
    if fault == 'collection': record['collection']['ordinary_waypoint_commitment_contact_enabled'] = False
    ...
    if fault == 'frames': record['prefix_comparison']['common_prefix_frames'] = 2
    if fault == 'physics': record['prefix_comparison']['physical_prefix_samples'] = 899
    if fault == 'forecasts': record['prefix_comparison']['raw_model_forecast_comparisons'] = 0
...
def test_both_exact_prior_waiters_required_without_retries():
    raw = dict(status='COMMITMENT_CONTACT_ANCHORED_RAW_PREFIX_WAIT_V1_COMPLETE', native_execution=False, automatic_retry=False)
    frontier = dict(status='HOLD_REORIENTATION_MAZE02_NATIVE_WAIT_V1_COMPLETE', automatic_retry=False)
...
def admission_fixture(monkeypatch, fault=None):
    raw_root, front_root = inputs.raw_wait.OUTPUT, inputs.prior_wait.OUTPUT
    native_root, prefix_root, batch_root = inputs.prior_wait.native.OUTPUT, inputs.replay.OUTPUT, inputs.replay.original.OUTPUT
    ...
    raw = dict(status='COMMITMENT_CONTACT_ANCHORED_RAW_PREFIX_WAIT_V1_COMPLETE', native_execution=False,
        automatic_retry=False, report=deepcopy(raw_report))
    frontier = dict(status='HOLD_REORIENTATION_MAZE02_NATIVE_WAIT_V1_COMPLETE', automatic_retry=False, report=deepcopy(front_report))
    native_result = dict(adapter_batch_result_sha256='batch-result')
    native_launch = dict(input_admission=dict(adapter_batch_result_sha256='batch-result', all_six_adapter_cases_completed=True))
    ...
    results = {raw_root:(raw, {}, {'result.json':'raw-wait-result'}),
        ...
        batch_root:({'status':'ALL_PHASE_ADAPTER_MAZE02_MATCHED_NATIVE_V1_COMPLETE', 'all_fixed_cases_executed':True}, {'input_admission':{'correction_admission':{'same':'model'}}}, batch_ids)}
    ...
    def completed(root, sha, launch_sha, sources):
        completed_calls.append((root,sha,launch_sha))
        if root == batch_root and sha != 'batch-result': raise ValueError('different completed batch identity')
        return deepcopy(results[root])
    ...
    monkeypatch.setattr(inputs.prior_wait, 'authenticate_completed', lambda *a: deepcopy(front_report))
    monkeypatch.setattr(inputs.raw_wait, 'authenticate_replay', lambda *a: deepcopy(raw_report))
    monkeypatch.setattr(inputs.prior_wait.native, 'verify_inputs', lambda launch, **kwargs: full.append(kwargs))
...
def test_input_admission_links_same_second_worker_to_both_completed_chains(monkeypatch):
    full, calls = admission_fixture(monkeypatch)
    result = inputs.admit('raw-wait-result', 'front-wait-result', {})
    assert full == [{'full':True}]
    assert [r for r,_,_ in calls] == [inputs.prior_wait.OUTPUT, inputs.raw_wait.OUTPUT,
        inputs.prior_wait.native.OUTPUT, inputs.replay.OUTPUT, inputs.replay.original.OUTPUT]
    assert result['original_worker_terminal_sha256'] == 'terminal-sha'
    assert result['scheduled_hold_native_completed_before_this_experiment'] is True
