"""Focused standalone-authority and lifecycle tests for projection V3."""
...
def _stack_v3(*, origin: tuple[float, float] = (-1.6, -1.6)):
    physical_frame = MapFrameIdentity(
        session_id=f"native-v3-physical:{origin}",
        origin_xy_m=origin,
        cell_size_m=0.05,
        frame_id="native_physical",
    )
    configuration_frame = MapFrameIdentity(
        session_id=f"native-v3-configuration:{origin}",
        origin_xy_m=origin,
        cell_size_m=0.10,
    ...
    adapter = NativeLearnedPhysicalProjectionAdapterV3(
        memory=memory,
...
def _mutate_and_rehash_v3(package, *, timestamp: int) -> None:
...
def _advance_v3(projection, snapshot, runner, adapter, *, sequence: int):
...
def test_v3_is_standalone_and_old_bound_or_unbound_methods_reject() -> None:
    memory, projection, snapshot, runner, adapter = _stack_v3()
    package = adapter.issue(
        snapshot,
        _outcome(runner, snapshot, ground=(_ground_row(),)),
    ...
    v1_adapter, v2_adapter = _older_adapters(memory, projection, runner)
    for older in (v1_adapter, v2_adapter):
...
def test_v3_closes_reachable_engine_mutated_target_bypass() -> None:
    memory, projection, _runner, adapter, target = _committed_target_v3()
    _mutate_and_rehash_v3(target, timestamp=101)
    current = projection.project()
...
def test_v3_target_digest_is_checked_at_issue_and_final_commit() -> None:
    memory, projection, _runner, adapter, target = _committed_target_v3()
    _mutate_and_rehash_v3(target, timestamp=102)
    with pytest.raises(
        NativeLearnedProjectionBindingError,
        match="issued content",
    ):
    ...
    memory2, projection2, _runner2, adapter2, target2 = _committed_target_v3()
    retraction = adapter2.issue_retraction(projection2.project(), target2)
    _mutate_and_rehash_v3(target2, timestamp=103)
    with pytest.raises(
        NativeLearnedProjectionBindingError,
        match="issued content",
    ):
...
def test_v3_copy_forgery_transfer_mutation_and_replay_cannot_remove_target() -> None:
    memory, projection, runner, adapter, target = _committed_target_v3()
    ...
    _mutate_and_rehash_v3(mutated, timestamp=104)
    with pytest.raises(NativeLearnedProjectionBindingError):
        adapter2.commit(mutated)
    assert target2.observation_id in memory2.learned_observation_ids


def test_v3_failed_stale_commit_releases_for_retry_and_old_stays_terminal() -> None:
    memory, projection, runner, adapter, target = _committed_target_v3()
    shared = projection.project()
    stale = adapter.issue_retraction(shared, target)
    _advance_v3(projection, shared, runner, adapter, sequence=2)
...
def test_v3_proactive_stale_retry_allows_exactly_one_live_retraction() -> None:
    memory, projection, _runner, adapter, target = _committed_target_v3()
    ...
    with pytest.raises(
        NativeLearnedProjectionReplayError,
        match="exact live V3 retraction",
    ):
        adapter.issue_retraction(current, target)
    with pytest.raises(
        NativeLearnedProjectionReplayError,
...
def test_v3_removal_is_atomic_and_preserves_other_active_identity() -> None:
    memory, projection, runner, adapter, target = _committed_target_v3()
    other = _advance_v3(
        projection,
        projection.project(),
...
def test_v3_preserves_free_rotation_boundary_and_occupied_precedence() -> None:
    memory, _projection, snapshot, runner, adapter = _stack_v3()
    free = adapter.issue(
        snapshot,
...
def test_v3_preserves_translation_covariance_and_native_resolution_rules() -> None:
...
def test_v3_contradiction_is_unknown_and_exact_retraction_restores_free() -> None:
    memory, projection, snapshot, runner, adapter = _stack_v3()
    free = adapter.issue(
        snapshot,
        _outcome(runner, snapshot, ground=(_ground_row(),), sequence=1),
...
def test_v3_contract_authority_copy_serialization_and_production_fail_closed() -> None:
    _memory, _projection, snapshot, runner, adapter = _stack_v3()
    package = adapter.issue(
        snapshot,
        _outcome(runner, snapshot, ground=(_ground_row(),)),
    )
    assert type(package) is QualifiedLearnedPhysicalDevelopmentTransactionV3
...
def test_v3_source_has_no_old_adapter_or_runtime_input_surface() -> None:
    source = Path(v3_module.__file__).read_text(encoding="utf-8").lower()
    for forbidden in (
        "nativelearnedphysicalprojectionadapterv1",
        "nativelearnedphysicalprojectionadapterv2",
