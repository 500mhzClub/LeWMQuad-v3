...
from scripts import (
    build_go2_scene_diversity_recurrent_replication_genesis_rocm_backend_v2_authority
    as authority,
)
...
def test_scientific_authority_binds_exact_pass_and_no_probe_payloads(
    ...
    assert result["predecessor_v1_qualification_terminal_review_binding"] == (
        authority.qualification_authority._v1_terminal_review_binding()  # noqa: SLF001
    )
...
def test_scientific_authority_is_impossible_before_exact_v2_pass(
    tmp_path: Path, monkeypatch,
) -> None:
    science, bindings = _prepare(
...
def test_no_v2_authority_or_attempt_exists() -> None:
