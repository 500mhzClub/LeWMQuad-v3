...
import pytest
from scripts.run_go2_augmented_family_switch_fits_v1 import ROSTER,OPTIMIZATION_SEEDS,benchmark_decision


def phase(name,wall):
    return dict(wall_s=wall,records=[dict(name=f'{name}_{i}',status='AUGMENTED_FAMILY_SWITCH_WORKER_COMPLETE',
        actual_updates=20,fit=dict(seed=2026091410+i,model_sha256=str(i)*64),ledger_sha256=str(i)*64,
