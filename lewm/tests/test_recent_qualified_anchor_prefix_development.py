...
import numpy as np
import pytest
from lewm.recent_qualified_anchor_prefix_development import PrefixComparison
...
def row(frame,*,previous=None,attempt=False,qualified=True):
    ...
    old=dict(controller='partial_height_direct_flow_controller_v1',partial_floor_height_constraint_enabled=True,
    ...
    new=deepcopy(old);new.update(controller='recent_qualified_anchor_controller_v1',recent_qualified_anchor_enabled=True)
