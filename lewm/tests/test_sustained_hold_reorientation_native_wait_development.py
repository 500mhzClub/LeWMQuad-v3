...
from scripts import await_go2_sustained_hold_reorientation_maze02_native_v1 as waiter
...
def test_fixed_roster_contains_exact_raw_owner_and_original_final_waiter():
    assert [s[0] for s in waiter.PREREQUISITES] == ['raw', 'budget']
    assert [s[1]['pid'] for s in waiter.PREREQUISITES] == [2813368, 2793505]
    assert waiter.PREREQUISITES[0][3] == '6f68017be0f68198ab08af97fb4d90f33ebbdc2b1b56f8da123e8b34b4dd89df'
    assert waiter.PREREQUISITES[1][3] == '0d767f341c8f0e28cd61107b198aa6eb76445f45cf13cc6e8602565c529fdf4f'
...
def test_command_forwards_every_exact_completion_once():
    ...
    assert dict(zip(cmd[3::2], cmd[4::2], strict=True)) == dict(
        zip(['--raw-prefix-result-sha256', '--budget-wait-result-sha256'], ids.values(), strict=True))
...
def test_incomplete_or_invented_command_roster_rejected(fault):
    ids = identities()
    if fault == 'missing': ids.pop('budget')
    elif fault == 'extra': ids['replacement'] = 'a'*64
    elif fault == 'invalid': ids['budget'] = 'not-a-hash'
    else: ids['budget'] = True
...
def loop_fixture(monkeypatch):
    ...
    ids = identities()
    def owner(owner):
        return state['tick'] == 0 or (state['tick'] == 1 and owner['pid'] == 2793505)
...
def test_no_identity_is_inferred_while_its_original_owner_is_live(monkeypatch):
    ...
    assert (1, 'budget') not in checked and (2, 'budget') in checked
...
def completion_fixture(tmp_path, monkeypatch, key='raw'):
    ...
    result = dict(status=spec[4], source_sha256=sources.copy(),
        ...
        report=dict(frames=407, first_changed_command_frame=406, raw_model_forecast_comparisons=404,
            original_requested_command=[0.,0.,0.], candidate_requested_command=[0.,0.,.45], native_execution=False))
...
@pytest.mark.parametrize('key', ['raw', 'budget'])
...
def test_incomplete_or_changed_original_completion_rejected(tmp_path, monkeypatch, fault):
    spec, sources, result, launch, verified = completion_fixture(tmp_path, monkeypatch,
        'budget' if fault == 'retry' else 'raw')
...
def terminal_fixture(tmp_path, monkeypatch):
    ...
    collection = {'observations':407}; readout = {'verified_round_trip':False}; prefix = {'actual':True}
    ...
    launch = dict(source_sha256={'src':'sha'}, input_admission=dict(raw_prefix_result_sha256=ids['raw'],
        budget_wait_result_sha256=ids['budget'], prefix_report={'raw':True}))
    result = dict(status='SUSTAINED_HOLD_REORIENTATION_MAZE02_PILOT_V1_COMPLETE',
        artifact_sha256=bindings, source_sha256={'src':'sha'}, conditions=[record], measured_round_trip_successes=0,
        raw_prefix_result_sha256=ids['raw'], prospective_prefix_result_sha256=ids['raw'], budget_wait_result_sha256=ids['budget'])
...
@pytest.mark.parametrize('fault', ['raw', 'budget', 'admission', 'missing_raw', 'worker', 'collection',
...
def test_changed_native_evidence_cannot_be_certified(tmp_path, monkeypatch, fault):
    ...
    if fault == 'raw': result['raw_prefix_result_sha256'] = 'b'*64
    if fault == 'budget': result['budget_wait_result_sha256'] = 'b'*64
    if fault == 'admission': records['launch.json']['input_admission']['raw_prefix_result_sha256'] = 'b'*64
