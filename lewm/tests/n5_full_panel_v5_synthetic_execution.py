"""Production-ineligible filesystem model for V5 lifecycle tests.
...
STAGING_PREFIX = ".n5.synthetic-v5-"
LOCK_NAME = ".n5.synthetic-v5.lock"
SCHEMA = "lewm_go2_n5_full_panel_v5_synthetic_reservation_v1"
MANIFEST_SCHEMA = "lewm_go2_n5_full_panel_v5_synthetic_staging_v1"
...
@dataclass
class _DirectoryChainV5:
    anchor_fd: int
    anchor_fingerprint: tuple[int, ...]
    descriptors: list[int]
    entries: list[_DirectoryEntryV5]
    path_fds: dict[Path, int]
    closed: bool = False


@dataclass(frozen=True)
class _OwnedArtifactV5:
    role: str
    parent_fd: int
    name: str
    fingerprint: tuple[int, ...]
...
def _assert_chain(chain: _DirectoryChainV5) -> None:
    if chain.closed:
        raise PermissionError("synthetic V5 canonical directory chain is closed")
    if _fingerprint(os.fstat(chain.anchor_fd)) != chain.anchor_fingerprint:
        raise PermissionError("synthetic V5 filesystem-root descriptor changed")
    for entry in chain.entries:
        ...
        if (
            ...
            or _fingerprint(named) != entry.fingerprint
            or _fingerprint(opened) != entry.fingerprint
        ):
            raise PermissionError("synthetic V5 canonical directory identity changed")


def _refresh_chain(chain: _DirectoryChainV5, mutable_fds: set[int]) -> None:
    if chain.closed:
        raise PermissionError("synthetic V5 canonical directory chain is closed")
    if _fingerprint(os.fstat(chain.anchor_fd)) != chain.anchor_fingerprint:
        raise PermissionError("synthetic V5 filesystem-root descriptor changed")
    for entry in chain.entries:
        ...
        if entry.child_fd in mutable_fds:
            ...
            raise PermissionError("synthetic V5 unexpected canonical directory mutation")


def _open_chain(final_path: Path) -> _DirectoryChainV5:
    final_path = Path(final_path)
    if not final_path.is_absolute() or any(
        component in {"", ".", ".."} for component in final_path.parts[1:]
    ):
        raise PermissionError("synthetic V5 canonical path is malformed")
    ...
    try:
        if _fingerprint(os.fstat(anchor_fd)) != anchor_fingerprint:
            raise PermissionError("synthetic V5 filesystem root changed during open")
        parent_fd = anchor_fd
        ...
        for component in final_path.parts[1:]:
            ...
            if stat.S_ISLNK(before.st_mode) or not stat.S_ISDIR(before.st_mode):
                raise PermissionError("synthetic V5 canonical component is not a directory")
            child_fd = os.open(component, _directory_flags(), dir_fd=parent_fd)
            chain.descriptors.append(child_fd)
            opened = os.fstat(child_fd)
            if _fingerprint(opened) != fingerprint:
                raise PermissionError("synthetic V5 canonical component changed during open")
            chain.entries.append(
                _DirectoryEntryV5(parent_fd, component, child_fd, fingerprint)
            )
...
def _open_chain_child(
    ...
    try:
        ...
        before = _entry_metadata(parent_fd, name)
    if stat.S_ISLNK(before.st_mode) or not stat.S_ISDIR(before.st_mode):
        raise PermissionError("synthetic V5 derived parent is not a directory")
    fingerprint = _fingerprint(before)
    child_fd = os.open(name, _directory_flags(), dir_fd=parent_fd)
    chain.descriptors.append(child_fd)
    if _fingerprint(os.fstat(child_fd)) != fingerprint:
        raise PermissionError("synthetic V5 derived parent changed during open")
    child_path = parent_path / name
    chain.entries.append(_DirectoryEntryV5(parent_fd, name, child_fd, fingerprint))
    chain.path_fds[child_path] = child_fd
    _assert_chain(chain)
    return child_fd


def _close_chain(chain: _DirectoryChainV5) -> None:
    if chain.closed:
        return
    chain.closed = True
...
@dataclass(frozen=True)
class SyntheticReservationV5:
    directory: Path
    value: Mapping[str, Any]
    raw: bytes
    directory_fd: int = -1
    directory_identity: tuple[int, int] | None = None
    directory_chain: _DirectoryChainV5 | None = None
    owned_claim_artifacts: dict[str, _OwnedArtifactV5] = field(
        default_factory=dict,
        compare=False,
    )
    owned_derived_artifacts: dict[str, _OwnedArtifactV5] = field(
        default_factory=dict,
        compare=False,
    )
...
class SyntheticExecutionV5:
    ...
    def __copy__(self) -> "SyntheticExecutionV5":
        return type(self)(self.root)

    def __deepcopy__(self, memo: object) -> "SyntheticExecutionV5":
        del memo
        return type(self)(self.root)
    ...
    def _reservation(self, recovery: Sequence[Mapping[str, Any]]) -> SyntheticReservationV5:
        core = self._core(recovery)
        value = {**core, "content_sha256": _digest(core)}
        return SyntheticReservationV5(self.attempt, value, _json(value) + b"\n")

    @staticmethod
    def _assert_claim_fd(reservation: SyntheticReservationV5) -> None:
        if reservation.directory_fd < 0 or reservation.directory_identity is None:
            raise PermissionError("synthetic V5 reservation has no open claim")
        opened = os.fstat(reservation.directory_fd)
        if (
            not stat.S_ISDIR(opened.st_mode)
            or (opened.st_dev, opened.st_ino) != reservation.directory_identity
        ):
            raise PermissionError("synthetic V5 claimed descriptor identity changed")

    @classmethod
    def _assert_claim(cls, reservation: SyntheticReservationV5) -> None:
        cls._assert_claim_fd(reservation)
        if reservation.directory_chain is None or reservation.seed_root_fd < 0:
            raise PermissionError("synthetic V5 reservation lacks canonical ancestry")
        ...
        if (
            ...
            raise PermissionError("synthetic V5 canonical claim identity changed")

    @classmethod
    def _write_claim(
        cls,
        reservation: SyntheticReservationV5,
        name: str,
        raw: bytes,
        *,
        ...
        try:
            ...
            if not stat.S_ISREG(metadata.st_mode) or metadata.st_nlink != 1:
                raise PermissionError("synthetic V5 claim leaf is not singly linked")
        ...
        if (
            ...
            raise PermissionError("synthetic V5 committed claim leaf changed")
        reservation.owned_claim_artifacts[name] = _OwnedArtifactV5(
            role=role or name,
    ...
    def _write_derived(
        ...
        if Path(name).name != name or reservation.derived_fd < 0:
            raise PermissionError("synthetic V5 derived artifact escaped")
        descriptor = os.open(
            name,
            os.O_WRONLY
            | os.O_CREAT
        ...
        try:
            metadata = os.fstat(descriptor)
            if not stat.S_ISREG(metadata.st_mode) or metadata.st_nlink != 1:
                raise PermissionError("synthetic V5 derived leaf is not singly linked")
            identity = (metadata.st_dev, metadata.st_ino)
            with os.fdopen(descriptor, "wb", closefd=True) as stream:
                descriptor = -1
                stream.write(raw)
        ...
        if (
            ...
            raise PermissionError("synthetic V5 committed derived leaf changed")
        reservation.owned_derived_artifacts[name] = _OwnedArtifactV5(
            role="derived",
    ...
    @staticmethod
    def _cleanup_owned(
        registry: dict[str, _OwnedArtifactV5],
        *,
    ...
    def terminate(
        ...
        *,
        ...
        core = {
            "schema": "lewm_go2_n5_full_panel_v5_synthetic_failure_v1",
            "status": "failed",
            "failure_stage": stage,
    ...
    def publish(self, reservation: SyntheticReservationV5, raw: bytes) -> None:
        self._assert_claim(reservation)
        if reservation.directory != self.attempt:
            raise PermissionError("synthetic V5 reservation belongs to another operation")
        self._write_claim(
            reservation,
            "completed.json",
            raw,
    ...
    @staticmethod
    def close(reservation: SyntheticReservationV5) -> None:
        os.close(reservation.directory_fd)
        if reservation.directory_chain is not None:
            _close_chain(reservation.directory_chain)

    def _manifest(self, staging: Path, reservation: SyntheticReservationV5) -> bytes:
        core = {
    ...
    def _classify(self, staging: Path) -> tuple[dict[str, str], SyntheticReservationV5 | None]:
        evidence = {
            "staging_name": staging.name,
        ...
        return evidence, SyntheticReservationV5(self.attempt, reservation, reservation_raw)
    ...
    def claim(self, *, failure_injection: str | None = None) -> SyntheticReservationV5:
        active: Path | None = None
        identity: tuple[int, int] | None = None
        directory_fd: int | None = None
        directory_chain: _DirectoryChainV5 | None = None
        claimed: SyntheticReservationV5 | None = None
        try:
            with self._lock():
                ...
                complete: list[tuple[Path, SyntheticReservationV5, dict[str, str]]] = []
                ...
                _fsync(active)
                directory_chain = _open_chain(self.attempt.parent)
                ...
                if _fingerprint(named) != _fingerprint(metadata):
                    raise PermissionError("synthetic V5 staging identity changed")
                identity = (metadata.st_dev, metadata.st_ino)
                claimed = SyntheticReservationV5(
...
__all__ = ["SyntheticExecutionV5", "SyntheticReservationV5"]
