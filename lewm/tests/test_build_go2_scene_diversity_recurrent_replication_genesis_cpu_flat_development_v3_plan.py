...
from scripts import build_go2_scene_diversity_recurrent_replication_genesis_cpu_flat_development_v3_plan as builder
...
def test_recursive_delta_is_only_identity_root_contract_and_path() -> None:
    predecessor = json.loads(builder.V2_QUALIFICATION_PLAN.read_bytes())
...
def test_validator_rejects_any_additional_environment_or_science_delta() -> None:
    ...
    with pytest.raises(
        builder.CpuFlatDevelopmentV3PlanError,
        match="changed beyond fresh identity/root/contracts and fixed PATH",
    ):
        builder.validate_qualification_plan(changed)

    changed = copy.deepcopy(candidate)
    changed["expected_counts"]["states"] += 1
    with pytest.raises(builder.CpuFlatDevelopmentV3PlanError):
        builder.validate_qualification_plan(changed)
...
def test_existing_attempt_root_fails_closed(
    ...
    with pytest.raises(
        builder.CpuFlatDevelopmentV3PlanError,
        match="fresh V3 qualification/scientific roots changed",
    ):
