...
import pytest
from scripts import tiled_density_progressive_floor_replay_development as run
from lewm import tiled_density_progressive_floor_controller_development as copied
from lewm import progressive_batched_floor_controller_development as visible
...
def fixture(monkeypatch,tmp_path,fault=None):
    ...
    class Baseline(fake):
        ...
        def observe(self,*args,**kwargs):
            return super().observe(*args,**kwargs)|{'controller':visible.CONTROLLER,packed.FLAG:True,visible.FLAG:True,density.FLAG:True,visibility.FLAG:True}
    class Candidate(Baseline):
        index=1
        def observe(self,*args,**kwargs):
            return super().observe(*args,**kwargs)|{'controller':copied.CONTROLLER,copied.FLAG:True}
    monkeypatch.setattr(run,'ProgressiveBatchedFloorController',Baseline)
    monkeypatch.setattr(run,'TiledDensityProgressiveFloorController',Candidate)
    ...
    for i,row in enumerate(islice(run.original.profile.read_rows(None),1428)):
        decision=row['decision']|{run.original.FLAG:True,'controller':visible.CONTROLLER,visible.FLAG:True,density.FLAG:True,visibility.FLAG:True,
...
def test_original_loop_and_only_declared_composition_changes():
    ...
    assert new.__globals__['FrozenFootprintAnchoredController'] is run.ProgressiveBatchedFloorController
    assert new.__globals__['ScopedFootprintAnchoredController'] is run.TiledDensityProgressiveFloorController
...
def test_full_history_and_seven_original_state_checks(monkeypatch,tmp_path):
    ...
    assert result['baseline']=='ProgressiveBatchedFloorController'
    assert result['candidate']=='TiledDensityProgressiveFloorController'
