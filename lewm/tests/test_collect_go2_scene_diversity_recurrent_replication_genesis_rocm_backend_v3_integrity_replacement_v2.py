...
from scripts import (
    collect_go2_scene_diversity_recurrent_replication_genesis_rocm_backend_v3_integrity_replacement_v2
    as collector,
)
...
def test_collector_is_fresh_closed_interface_layer_on_replacement_v1() -> None:
    ...
    assert len(collector.AUTHORITY_FIELDS) == 19
    ...
    assert audit["replacement_v1_runtime_payload_reuse_authorized"] is False
...
def test_replacement_v1_review_binding_is_semantically_exact() -> None:
    valid = {
        "predecessor_replacement_v1_qualification_terminal_review_binding": (
            collector._standard_replacement_v1_review_binding()  # noqa: SLF001
        )
    }
    collector._require_replacement_v1_review_binding(valid)  # noqa: SLF001
    ...
    with pytest.raises(
        ...
        collector._require_replacement_v1_review_binding(changed)  # noqa: SLF001
...
def test_collector_facade_and_exact_override_matrix_pass() -> None:
    audit = collector.validate_collector_compatibility_facade()
    assert audit["passed"] is True
    assert len(audit["layers"]) == 7
    assert set(collector._configuration_overrides_v3()) == set(  # noqa: SLF001
        collector.COLLECTOR_COMPATIBILITY_MATRIX[
