...
@pytest.mark.parametrize('live_index', [0, 1, None])
def test_replay_and_completion_watcher_must_both_have_ended(monkeypatch,tmp_path,live_index):
    boot=run.Path('/proc/sys/kernel/random/boot_id').read_text().strip()
    monkeypatch.setattr(run,'ROOT',tmp_path)
    calls=[]
    for i,name in enumerate(run.SLOT_EXECUTIONS):
        path=tmp_path/name;path.parent.mkdir(parents=True,exist_ok=True)
        path.write_text(json.dumps(dict(boot_id=boot,owner=dict(pid=i))))
    monkeypatch.setattr(run,'verify',lambda ids:calls.append('verified fixed execution identities'))
    monkeypatch.setattr(run,'owner_live',lambda owner:calls.append(owner['pid']) or owner['pid']==live_index)
    if live_index is None:
        run.full_replay_slot_available()
        assert calls==['verified fixed execution identities',0,1]
    else:
        with pytest.raises(ValueError,match='still occupies CPU slot'):run.full_replay_slot_available()
        assert calls==['verified fixed execution identities']+list(range(live_index+1))


def test_changed_boot_cannot_release_slot(monkeypatch,tmp_path):
    monkeypatch.setattr(run,'ROOT',tmp_path)
    name=next(iter(run.SLOT_EXECUTIONS));path=tmp_path/name
    path.parent.mkdir(parents=True);path.write_text(json.dumps(dict(boot_id='changed',owner={})))
    monkeypatch.setattr(run,'verify',lambda ids:None)
    monkeypatch.setattr(run,'owner_live',lambda owner:pytest.fail('boot identity must be checked first'))
    with pytest.raises(ValueError,match='execution boot'):run.full_replay_slot_available()
