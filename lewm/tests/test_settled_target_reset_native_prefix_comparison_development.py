...
import numpy as np
import pytest
from scripts import settled_target_reset_native_prefix_comparison_development as module
...
def fixture(tmp_path,monkeypatch):
    ...
    for i in range(2):
        ...
        for d in (a,b):
            ...
            d['mission_receipt'].update(frame=i,measured_ns=1_500_000_000+i*100_000_000)
        b['mission_receipt']['observed_settling'].update(current_frame=i,measured_ns=1_500_000_000+i*100_000_000)
        if i==1:
            a['planner_mode']='NEW'
...
@pytest.mark.parametrize('fault',['physics','public','map','pose','forecast','command','last_command',
    'clock','missing','earlier_phase','terminal','prospective','mode','previous_mode'])
def test_mismatch_rejects_even_when_prospective_candidate_is_changed_to_match(fixture,fault,monkeypatch):
    ...
    elif fault=='forecast':d.update(new_selection={'prediction':[.1]})
    elif fault=='command':tapes[new][0]['requested_command'][0]=.2
    elif fault=='last_command':tapes[new][1]['requested_command'][2]=.45
    elif fault=='mode':d['planner_mode']='VIEW_ACQUISITION'
    elif fault=='previous_mode':
        rows[new][0]['decision']['planner_mode']='VIEW_ACQUISITION'
        rows[old][0]['decision']['planner_mode']='VIEW_ACQUISITION'
    ...
    if fault in ('map','pose','forecast','terminal','mode','previous_mode'):rows[bound]=deepcopy(rows[new])
