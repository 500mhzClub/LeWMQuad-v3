...
import pytest
from scripts.run_go2_settled_boundary_maze_pilot_v2 import admit_prefix


def prefix():
    return dict(status='SETTLED_BOUNDARY_CONTROLLER_PREFIX_V2_COMPLETE',frames=1867,
        ...
        navigation_qualified=False,final_target_reset_mode_difference=True,
        original_failed_attempt_preserved=True,controller_implementation_unchanged=True,final_mission_receipt=dict(frame=1866,phase='OUTBOUND',hold_required=True,
...
@pytest.mark.parametrize('field,value',[('status','RUNNING'),('frames',1866),
    ...
    ('complete_decisions_exact_outside_declared_mission_and_target_reset_fields',False),('actual_requested_commands_exact',False),
    ('stopped_before_later_decisions',False),('model_state_unchanged',False),
    ('final_target_reset_mode_difference',False),('original_failed_attempt_preserved',False),
    ('controller_implementation_unchanged',False),('status','SETTLED_BOUNDARY_CONTROLLER_PREFIX_COMPLETE'),
