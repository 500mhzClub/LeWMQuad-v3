...
from scripts import verify_go2_tiled_density_progressive_floor_controller_completion_v1 as check
...
def test_report_retains_original_state_and_negative_evidence_without_mutation():
    prior = dict(incremental_progressive_patch_batching_comparison=True,
        both_controllers_use_density_routed_floor_registration_and_mapping=True, observed_state_checks=[{'frame': 1173, 'sha': 'old'}],
    ...
    assert expected['candidate'] == 'TiledDensityProgressiveFloorController'
    assert 'incremental_progressive_patch_batching_comparison' not in expected


def test_wrong_predecessor_scope_rejected():
    with pytest.raises(ValueError, match='progressive'):
        check.expected_report(dict(incremental_progressive_patch_batching_comparison=False), {})
