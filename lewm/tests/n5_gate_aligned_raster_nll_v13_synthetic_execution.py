"""Production-ineligible V13 filesystem lifecycle used only by tests.
...
STAGING_PREFIX = ".n5.synthetic-v13-"
LOCK_NAME = ".n5.synthetic-v13.lock"
SERIALIZER_SUFFIX = ".n5.synthetic-v13.serializer"
SCHEMA = "lewm_go2_n5_gate_aligned_raster_nll_v13_synthetic_reservation_v1"
MANIFEST_SCHEMA = "lewm_go2_n5_gate_aligned_raster_nll_v13_synthetic_staging_v1"
...
def complete_retained_gate_evaluation_v13() -> dict[str, Any]:
    ...
    def control(name: str, mapping: list[int], *, perfect: bool) -> dict[str, Any]:
        ...
        if family_count * len(family_names) != raster["count"]:
            raise ValueError("V13 synthetic family partition is not even")
        diagnostics = {
            "overall": {
...
def _directory_flags() -> int:
    ...
    if not nofollow or not directory:
        raise PermissionError("synthetic V13 requires no-follow directory opens")
...
class _WatchV13:
    ...
    label: str
    generation: int
    snapshot: _SnapshotV13


class _DeclaredJournalV13:
    """A deterministic test journal for declared filesystem state deltas."""

    def __init__(self) -> None:
        self._watches: dict[tuple[int, int], _WatchV13] = {}
        self._generation = 0
    ...
    def _fail(self, reason: str) -> None:
        ...
        raise PermissionError(f"synthetic V13 journal rejected: {reason}")
    ...
    @staticmethod
    def _snapshot(directory_fd: int) -> _SnapshotV13:
        ...
        return _SnapshotV13(_fingerprint(opened), rows)
    ...
    def watch(self, directory_fd: int, *, label: str) -> None:
        ...
        self._generation += 1
        self._watches[identity] = _WatchV13(
            retained,
            identity,
            label,
            self._generation,
    ...
    def _snapshots(self) -> dict[tuple[int, int], _SnapshotV13]:
        return {
    ...
    ) -> tuple[_WatchV13, dict[tuple[int, int], _SnapshotV13]]:
        self.assert_clean()
    ...
    def _finish(
        self,
        before: Mapping[tuple[int, int], _SnapshotV13],
        *,
...
@dataclass
class _AncestorChainV13:
    descriptors: list[int]
    entries: list[_AncestorEntryV13]
    closed: bool = False


def _open_ancestor_chain(path: Path) -> _AncestorChainV13:
    filesystem_root = Path(path.anchor)
    anchor_fd = os.open(filesystem_root, _directory_flags())
    chain = _AncestorChainV13([anchor_fd], [])
    parent_fd = anchor_fd
    try:
        for component in path.parts[1:]:
            ...
            chain.entries.append(
                _AncestorEntryV13(parent_fd, component, child_fd, identity)
            )
            parent_fd = child_fd
        return chain
...
def _assert_ancestor_chain(chain: _AncestorChainV13) -> None:
    if chain.closed:
...
def _close_ancestor_chain(chain: _AncestorChainV13) -> None:
    if chain.closed:
        return
    chain.closed = True
...
class SyntheticReservationV13:
    ...
    seed_root_fd: int
    derived_fd: int
    journal: _DeclaredJournalV13
    ancestor_chain: _AncestorChainV13
    owned_directory_fds: tuple[int, ...]
    owned_claim_artifacts: dict[str, _OwnedArtifactV13] = field(
        default_factory=dict, compare=False
    )
    owned_derived_artifacts: dict[str, _OwnedArtifactV13] = field(
        default_factory=dict, compare=False
    )


class SyntheticExecutionV13:
    ...
    def __copy__(self) -> "SyntheticExecutionV13":
        return type(self)(self.root)

    def __deepcopy__(self, memo: object) -> "SyntheticExecutionV13":
        del memo
        return type(self)(self.root)
    ...
    def _new_staging(
        self, seed_fd: int, journal: _DeclaredJournalV13
    ) -> tuple[str, int]:
        for _ in range(128):
    ...
    @classmethod
    def _assert_claim_fd(cls, reservation: SyntheticReservationV13) -> None:
    ...
    @classmethod
    def _assert_claim(cls, reservation: SyntheticReservationV13) -> None:
    ...
    def _write_claim(
        cls,
        reservation: SyntheticReservationV13,
        name: str,
        raw: bytes,
        *,
        terminal: bool = False,
        ...
        reservation.owned_claim_artifacts[name] = _OwnedArtifactV13(
    ...
    @classmethod
    def _write_derived(
        cls, reservation: SyntheticReservationV13, name: str, raw: bytes
    ) -> None:
        cls._assert_claim(reservation)
        fingerprint = reservation.journal.create(
            reservation.derived_fd, name, raw
        )
        reservation.owned_derived_artifacts[name] = _OwnedArtifactV13(
    ...
    def claim(
        ...
        with self._serializer():
            ...
            claimed: SyntheticReservationV13 | None = None
            ...
            try:
                ...
                claimed = SyntheticReservationV13(
                    self.attempt,
                    value,
                    raw,
                    active_fd,
    ...
    def _cleanup_owned(
        reservation: SyntheticReservationV13,
        registry: dict[str, _OwnedArtifactV13],
        *,
        ...
        *,
        ...
        if not reservation.journal.poisoned:
            try:
                ...
                pass
        core = {
            "schema": "lewm_go2_n5_gate_aligned_raster_nll_v13_synthetic_failure_v1",
            "status": "failed",
            "failure_stage": stage,
    ...
    def publish(self, reservation: SyntheticReservationV13, raw: bytes) -> None:
    ...
    @staticmethod
    def close(reservation: SyntheticReservationV13) -> None:
        try:
