"""Synthetic and adversarial tests for raw-supervision auditor V2.
...
from lewm.datasets import go2_shared_jepa_v5_raw_supervision_auditor_v2 as auditor
from lewm.datasets import go2_shared_jepa_v5_raw_supervision_builder_v2 as builder_v2
...
from scripts import audit_go2_shared_jepa_v5_raw_supervision_v2 as audit_cli
...
def _authority_fixture(
    ...
    *,
...
) -> tuple[Path, Path, dict[str, str], tuple[auditor.SourceBindingV2, ...]]:
        ...
        record = _with_hash(
            {
                ...
                "candidate": candidate,
                "authority": auditor._expected_review_authority_v2(kind),
            }
        )
    ...
    sources = tuple(auditor._source_binding_v2(row) for row in source_rows)
...
def test_contract_matches_builder_v2_without_importing_it_in_production() -> None:
    assert {relative: _sha(ROOT / relative) for relative in FROZEN_BUILDER_V2} == (
        FROZEN_BUILDER_V2
    )
    assert auditor.AUTHORIZATION_SCHEMA == builder_v2.AUTHORIZATION_SCHEMA
    assert auditor.REVIEW_BINDING_SCHEMA == builder_v2.REVIEW_BINDING_SCHEMA
    assert auditor.BUILDER_REVIEW_SCHEMA == builder_v2.BUILDER_REVIEW_SCHEMA
    assert auditor.AUDITOR_REVIEW_SCHEMA == builder_v2.AUDITOR_REVIEW_SCHEMA
    assert (
        auditor.BUILDER_IMPLEMENTATION_AUTHOR
        == builder_v2.BUILDER_IMPLEMENTATION_AUTHOR
    )
    assert (
        auditor.AUDITOR_IMPLEMENTATION_AUTHOR
        == builder_v2.AUDITOR_IMPLEMENTATION_AUTHOR
    )
    assert auditor.SOURCE_ROLE_PATHS == builder_v2.AUTHORIZED_ROLE_PATHS
    assert auditor.AUTHORIZATION_FIELDS == builder_v2.AUTHORIZATION_FIELDS
    assert auditor.REVIEW_BINDING_FIELDS == builder_v2.REVIEW_BINDING_FIELDS
    assert auditor.REVIEW_RECORD_FIELDS == builder_v2.REVIEW_RECORD_FIELDS
...
def test_synthetic_callback_path_passes_and_has_no_exact_switch(tmp_path: Path) -> None:
    ...
    assert "exact" not in inspect.signature(auditor.audit_dataset_v2).parameters
    assert "sample_recomputer" not in inspect.signature(
        auditor.audit_exact_dataset_v2
    ).parameters
    result = auditor.audit_dataset_v2(
        root,
...
def test_phase_one_is_zero_open_and_phase_two_opens_exactly_nine(
    ...
    phase_one = auditor._validate_authorization_phase_one_v2(
        payload,
    ...
    auditor._validate_authorization_phase_two_v2(
...
def test_exact_api_rejects_authority_before_dataset_or_metadata_open(
    ...
    monkeypatch.setattr(auditor, "_read_manifest_bound_v2", forbidden("manifest"))
    monkeypatch.setattr(
        auditor.plan_v5,
...
def test_incomplete_authority_opens_only_fixed_authorization(
    ...
    with pytest.raises(auditor.RawSupervisionAuditError, match="incomplete"):
        auditor._validate_exact_authorization_v2(provenance)
...
def test_report_leaf_names_and_schema_are_additive_v2() -> None:
    assert auditor.CANONICAL_AUDIT_REPORT.name.endswith(".audit_v2.json")
    assert auditor.CANONICAL_AUDIT_FAILURE.name.endswith(".audit_v2.failed.json")
    assert auditor.AUDIT_SCHEMA == (
        "lewm_go2_shared_jepa_v5_raw_supervision_audit_v2"
    )
    assert auditor.SYNTHETIC_AUDIT_SCHEMA != auditor.AUDIT_SCHEMA


def test_v2_exact_failure_is_terminal_and_retry_is_refused(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    dataset = tmp_path / "development_raw_supervision_v1"
    report = tmp_path / "development_raw_supervision_v1.audit_v2.json"
    failure = tmp_path / "development_raw_supervision_v1.audit_v2.failed.json"
    calls = 0
    ...
    monkeypatch.setattr(auditor, "audit_exact_dataset_v2", fail)
    with pytest.raises(auditor.RawSupervisionAuditError, match="terminal failure"):
        auditor.execute_exact_audit_v2(
            expected_manifest_file_sha256="a" * 64,
            expected_authorization_file_sha256="b" * 64,
            workers=1,
        )
