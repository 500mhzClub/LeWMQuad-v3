"""CPU-hidden author closure for gate-aligned raster-NLL V12."""
...
from lewm.tests.n5_gate_aligned_raster_nll_v12_synthetic_execution import (
    ...
    SyntheticExecutionV12,
    complete_retained_gate_evaluation_v12,
)
from scripts import execute_go2_observable_camera_ray_fit_v4_n5_gate_aligned_raster_nll_v12 as executor
...
def _load_nested_executor() -> ModuleType:
    ...
    for node in tree.body:
        if node is entry:
            ...
            body.append(node)
    module_name = f"_lewm_camera_v12_author_{uuid.uuid4().hex}"
...
def _runtime_reservation(
    ...
    reservation = runtime.AttemptReservationV12(
        directory=attempt,
...
def test_v12_claim_transaction_rejects_frozen_v6_create_delete_blocker(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_v12_derived_transaction_rejects_frozen_v6_create_delete_blocker(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime, reservation, _generated = _runtime_reservation(tmp_path, monkeypatch)
...
def test_v12_owned_claim_and_derived_transactions_pass(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime, reservation, _generated = _runtime_reservation(tmp_path, monkeypatch)
...
def test_v12_production_cleanup_and_failure_receipt_are_journaled(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime, reservation, _generated = _runtime_reservation(tmp_path, monkeypatch)
...
def test_v12_claim_commit_is_registered_before_postcommit_stat_failure(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime, reservation, _generated = _runtime_reservation(tmp_path, monkeypatch)
...
def test_v12_derived_commit_is_registered_before_postcommit_fsync_failure(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime, reservation, _generated = _runtime_reservation(tmp_path, monkeypatch)
...
def test_v12_poisoned_journal_writes_identity_only_failure_and_never_success(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime, reservation, _generated = _runtime_reservation(tmp_path, monkeypatch)
...
def test_v12_poison_during_failure_receipt_preserves_invalid_and_returns_no_binding(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime, reservation, _generated = _runtime_reservation(tmp_path, monkeypatch)
...
def test_v12_post_training_ownership_failure_terminalizes_and_closes(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime, reservation, _generated = _runtime_reservation(tmp_path, monkeypatch)
...
def test_v12_move_in_out_restoration_is_rejected(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime, reservation, _generated = _runtime_reservation(tmp_path, monkeypatch)
...
def test_v12_restoration_history_rejects_at_every_transaction_hook(
    tmp_path: Path,
    history: str,
    hook: str,
) -> None:
    ...
    outside_fd = os.open(outside, runtime._directory_flags())
    journal = runtime.OwnedDirectoryJournalV12()
...
def test_v12_kernel_exact_sequences_cover_all_journal_primitives(
    tmp_path: Path,
) -> None:
    runtime = _load_nested_executor()
...
def test_v12_no_replace_claim_race_preserves_foreign_destination(
    tmp_path: Path,
) -> None:
    runtime = _load_nested_executor()
    parent_fd = os.open(tmp_path, runtime._directory_flags())
...
def test_v12_directory_rename_binds_source_to_retained_descriptor(
    tmp_path: Path,
) -> None:
    runtime = _load_nested_executor()
...
def test_v12_unlink_syscall_boundary_race_poisons_success(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime = _load_nested_executor()
...
def test_v12_replace_syscall_boundary_race_poisons_success(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime = _load_nested_executor()
...
def test_v12_rmdir_syscall_boundary_race_poisons_success(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime = _load_nested_executor()
...
def test_v12_watch_rejects_mutation_before_installation(
    tmp_path: Path,
) -> None:
    runtime = _load_nested_executor()
    directory_fd = os.open(tmp_path, runtime._directory_flags())
...
def test_v12_watch_rejects_mutation_during_installation(
    tmp_path: Path,
) -> None:
    runtime = _load_nested_executor()
    directory_fd = os.open(tmp_path, runtime._directory_flags())
...
def test_v12_exclusive_root_and_descendants_are_created_by_transactions(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime = _load_nested_executor()
...
def test_v12_exclusive_root_allows_unrelated_named_shared_churn(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime = _load_nested_executor()
    ...
    try:
        fingerprint = journal.create_exclusive_root(parent_fd, "exclusive-v12")
        assert runtime._stable_fingerprint(
            os.stat("exclusive-v12", dir_fd=parent_fd, follow_symlinks=False)
        ) == fingerprint
        journal.assert_clean()
    finally:
        journal.close()
...
def test_v12_exclusive_root_rejects_shared_parent_self_event(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime = _load_nested_executor()
    ...
    try:
        with pytest.raises(PermissionError, match="shared parent self"):
            journal.create_exclusive_root(parent_fd, "exclusive-v12")
        assert journal.poisoned is True
    finally:
        journal.close()
        os.close(parent_fd)
...
def test_v12_exclusive_root_rejects_unknown_shared_event_mask(
    tmp_path: Path,
) -> None:
    runtime = _load_nested_executor()
    parent_fd = os.open(tmp_path, runtime._directory_flags())
    ...
    def events_with_unknown_mask() -> list[Any]:
        ...
        if any(event.name == "exclusive-v12" for event in events):
            events.append(
                runtime.JournalEventV12(
                    events[0].watch,
                    0x00000001,
                    0,
                    "unrelated",
...
def _patch_temporary_reservation_authority(
    ...
    root.mkdir()
    output = root / ".generated/camera-v12"
    seed_root = output / "attempts/seed_20260710"
    attempt = seed_root / "n5"
    review_binding = {
        "path": policy.SOURCE_REVIEW_RELATIVE_PATH,
...
def test_v12_verifier_child_environment_removes_all_conflicting_selectors(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime = _load_nested_executor()
    for name in (
...
def test_v12_exact_relaunch_removes_all_conflicting_device_selectors(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime = _load_nested_executor()
    for name in (
...
def test_v12_verifier_request_binds_every_authority_surface(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    mutation: str,
) -> None:
...
def test_v12_verifier_response_rejects_binding_or_publication_tamper(
    monkeypatch: pytest.MonkeyPatch,
    mutation: str,
) -> None:
    runtime = _load_nested_executor()
...
def test_v12_parent_spawns_exact_compute_child_then_publishes_once(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime = _load_nested_executor()
    request = {
    ...
    def fake_observe(command: list[str], **kwargs: Any) -> Any:
        ...
        return runtime.ChildProcessObservationV12(
            returncode=0,
            signal=None,
            timed_out=False,
...
def test_v12_verifier_child_failures_never_publish_or_fallback(
    monkeypatch: pytest.MonkeyPatch,
    failure: str,
) -> None:
    runtime = _load_nested_executor()
...
def test_v12_verifier_child_is_the_only_frozen_interop_setter_path() -> None:
...
def _synthetic_child_failure(runtime: ModuleType) -> Any:
    ...
    observation = runtime.ChildProcessObservationV12(
        returncode=70,
        signal=None,
        timed_out=False,
        stdout_raw=raw,
    ...
    return runtime.VerifierChildFailureV12(
...
def test_v12_verification_diagnostic_is_fsynced_before_cleanup_and_survives(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime, reservation, _generated = _runtime_reservation(tmp_path, monkeypatch)
...
def test_v12_diagnostic_publication_failure_preserves_scientific_artifacts(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime, reservation, _generated = _runtime_reservation(tmp_path, monkeypatch)
...
def test_v12_actual_script_runs_real_cpu_verifier_contract_smoke() -> None:
...
def test_v12_normalized_science_ast_matches_frozen_v9() -> None:
    ...
    assert "derived_raster_cell_nll_for_verification_v12(" in verifier_source
...
def test_v12_full_temporary_reservation_journals_lock_staging_and_claim(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime = _load_nested_executor()
...
def test_v12_transferred_claim_fd_cleanup_failure_still_closes_chain(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime = _load_nested_executor()
    ...
    monkeypatch.setattr(
        runtime.OwnedDirectoryJournalV12,
        "rename_directory",
        fail_after_transfer,
    )
    with pytest.raises(
...
def test_v12_complete_staging_recovers_by_rehash_then_atomic_claim(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime = _load_nested_executor()
...
def test_v12_existing_exact_scaffold_without_staging_blocks_claim(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime = _load_nested_executor()
...
def test_v12_missing_derived_scaffold_blocks_valid_complete_recovery(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    missing: str,
) -> None:
...
def test_v12_invalid_lock_blocks_valid_complete_recovery(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    lock_state: str,
) -> None:
...
def test_v12_insecure_recovered_leaf_is_preserved_and_blocks_claim(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    mutation: str,
) -> None:
...
def test_v12_claim_manifest_alias_is_preserved_invalid(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime = _load_nested_executor()
...
def test_v12_multiple_equivalent_complete_stagings_resume_lexical_first(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime = _load_nested_executor()
...
def test_v12_conflicting_authority_core_cannot_pass_complete_classification(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime = _load_nested_executor()
    ...
    conflicting_reservation = runtime.AttemptReservationV12(
        directory=attempt,
        value=value,
        raw=raw,
        file_sha256=hashlib.sha256(raw).hexdigest(),
...
def test_v12_exact_name_invalid_staging_closes_opened_descriptor(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime = _load_nested_executor()
...
def test_v12_foreign_staging_prefix_is_preserved_and_blocks_claim(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime = _load_nested_executor()
...
def test_v12_historical_foreign_inventory_blocks_claim(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    location: str,
) -> None:
...
def test_v12_existing_nonprivate_exclusive_root_blocks_claim(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime = _load_nested_executor()
...
def test_v12_recovered_staging_rejects_leaf_rewrite_before_watch(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime = _load_nested_executor()
...
def test_v12_all_production_child_watches_bind_expected_snapshots() -> None:
    tree = ast.parse((ROOT / policy.EXECUTOR_RELATIVE_PATH).read_text())
    calls = [
        node
        for node in ast.walk(tree)
...
def test_v12_special_or_unknown_event_permanently_poisons_success(
    tmp_path: Path,
    kind: str,
    mask: int,
) -> None:
    ...
    journal._events = lambda: [
        runtime.JournalEventV12(event_watch, mask, 0, "")
    ]
    try:
        with pytest.raises(PermissionError):
            journal.assert_clean()
...
def test_v12_unknown_watch_and_watch_descriptor_reuse_poison(
    tmp_path: Path,
) -> None:
    runtime = _load_nested_executor()
    ...
    journal._events = lambda: [
        runtime.JournalEventV12(state.watch + 1000, journal.IN_CREATE, 0, "x")
    ]
    try:
...
def test_v12_directory_move_cookie_and_event_order_are_exact(
    tmp_path: Path,
    corruption: str,
) -> None:
    runtime = _load_nested_executor()
    ...
    def corrupted() -> list[Any]:
        ...
        if len(events) == 3 and events[0].mask & journal.IN_MOVED_FROM:
            if corruption == "cookie":
                events[1] = runtime.JournalEventV12(
                    events[1].watch,
                    events[1].mask,
                    events[1].cookie + 1,
                    events[1].name,
...
def test_v12_source_only_predecessor_bindings_are_frozen() -> None:
    for relative, expected in FROZEN_V4.items():
        assert _sha(ROOT / relative) == expected
    for relative, expected in FROZEN_V5.items():
        assert _sha(ROOT / relative) == expected
    assert _sha(ROOT / policy.V12_AMENDMENT_RELATIVE_PATH) == (
        policy.V12_AMENDMENT_FILE_SHA256
    )
...
def test_v12_public_exports_include_isolated_verifier_and_v7_closure() -> None:
    assert {
...
def test_v12_reservation_metadata_constructs_without_module_aliases() -> None:
    runtime = _load_nested_executor()
    from scripts import (
        train_go2_observable_camera_ray_fit_v4_n5_gate_aligned_raster_nll_v12
        as trainer,
...
def test_v12_import_exposes_no_callable_operation_and_performs_no_lifecycle_work() -> None:
    defined_functions = {
        name
...
def test_v12_production_source_defines_partial_stages_only_inside_script_entry() -> None:
    tree = ast.parse((ROOT / policy.EXECUTOR_RELATIVE_PATH).read_text())
...
def test_v12_no_constructible_stage_evidence_class_is_importable() -> None:
    exposed = {
        name
        for name, value in vars(executor).items()
...
def test_v12_reservation_copy_reconstruction_and_mutation_surface_is_absent() -> None:
    assert not hasattr(executor, "AttemptReservationV12")
    assert not any("reservation" in name.casefold() for name in vars(executor))


def test_v12_completion_writer_is_not_importable() -> None:
...
def test_v12_metric_and_gate_writers_are_not_importable(name: str) -> None:
    assert not hasattr(executor, name)


def test_v12_publication_rejects_replaced_claim_directory(tmp_path: Path) -> None:
    operation = SyntheticExecutionV12(tmp_path / "synthetic")
...
def test_v12_shared_generated_direct_child_churn_does_not_abort_claim(
    tmp_path: Path,
) -> None:
    generated = tmp_path / "repo/.generated"
    operation = SyntheticExecutionV12(generated / "exclusive-v12")
    reservation = operation.claim()
...
def test_v12_real_chain_allows_shared_churn_and_blocks_alias_and_owned_churn(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime = _load_nested_executor()
    root = tmp_path / "repo"
    generated = root / ".generated"
    output = generated / "camera-output-v12"
    seed_root = output / "attempts/seed_20260710"
    seed_root.mkdir(parents=True)
    for directory in (output, seed_root.parent, seed_root):
        directory.chmod(0o700)
...
def test_v12_shared_generated_alias_swap_is_rejected(tmp_path: Path) -> None:
    generated = tmp_path / "repo/.generated"
    operation = SyntheticExecutionV12(generated / "exclusive-v12")
    reservation = operation.claim()
    ...
    try:
        ...
        assert (moved / "exclusive-v12/attempts/seed_20260710/n5/failed.json").is_file()
    finally:
        operation.close(reservation)


def test_v12_shared_generated_permission_change_is_rejected(tmp_path: Path) -> None:
    generated = tmp_path / "repo/.generated"
    operation = SyntheticExecutionV12(generated / "exclusive-v12")
    reservation = operation.claim()
...
def test_v12_unowned_exclusive_subtree_mutation_is_rejected(tmp_path: Path) -> None:
    operation = SyntheticExecutionV12(tmp_path / "repo/.generated/exclusive-v12")
    reservation = operation.claim()
...
def test_v12_unowned_claim_child_is_rejected_and_terminalized(tmp_path: Path) -> None:
    operation = SyntheticExecutionV12(tmp_path / "repo/.generated/exclusive-v12")
    reservation = operation.claim()
...
def test_v12_review_preflight_rejects_canonical_leaf_alias(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_v12_review_preflight_accepts_only_the_regular_canonical_entry(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    temporary_root = tmp_path / "repo"
...
def test_v12_review_preflight_rejects_ineligible_authors(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    reviewer: str,
) -> None:
...
def test_v12_source_reader_rejects_parent_identity_change(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    root = tmp_path / "root"
...
def test_v12_source_reader_walks_from_filesystem_root_without_absolute_repo_open(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_v12_source_reader_rejects_transient_root_component_alias_before_any_read(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    root = tmp_path / "canonical-root"
...
def test_v12_source_reader_rejects_restored_transient_ancestor_alias(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    anchor = tmp_path / "v12-root-anchor"
    root = anchor / "repo"
    ...
    def replace_ancestor_during_component_open(
        ...
        *,
        ...
        if not replaced and path == anchor.name and dir_fd is not None:
            replaced = True
            moved = tmp_path / "v12-moved-root-anchor"
            anchor.rename(moved)
...
def test_v12_source_reader_rejects_same_inode_parent_metadata_change(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    root = tmp_path / "root"
...
def test_v12_source_reader_rejects_symlink_hardlink_and_fifo(tmp_path: Path) -> None:
    root = tmp_path / "root"
    root.mkdir()
...
def test_v12_synthetic_operation_rejects_production_namespaces(unsafe: Path) -> None:
    with pytest.raises(PermissionError, match="production|repository"):
        SyntheticExecutionV12(unsafe)


def test_v12_synthetic_recovery_is_single_use_and_descriptor_bound(tmp_path: Path) -> None:
    operation = SyntheticExecutionV12(tmp_path / "safe")
...
def test_v12_synthetic_existing_scaffold_without_staging_blocks_claim(
    tmp_path: Path,
) -> None:
    operation = SyntheticExecutionV12(tmp_path / "safe")
...
def test_v12_synthetic_multiple_equivalent_stagings_resume_one(
    tmp_path: Path,
) -> None:
    operation = SyntheticExecutionV12(tmp_path / "safe")
...
def test_v12_synthetic_foreign_staging_is_preserved_and_blocks_claim(
    tmp_path: Path,
) -> None:
    operation = SyntheticExecutionV12(tmp_path / "safe")
    complete = operation.prepare_complete_staging()
    foreign = operation.attempt.parent / ".n5.synthetic-v12-000-foreign"
...
def test_v12_synthetic_historical_foreign_inventory_blocks_claim(
    tmp_path: Path,
) -> None:
    operation = SyntheticExecutionV12(tmp_path / "safe")
...
def test_v12_success_rejects_replaced_output_ancestor_but_fd_failure_terminalizes(
    tmp_path: Path,
) -> None:
    operation = SyntheticExecutionV12(tmp_path / "safe")
    reservation = operation.claim()
...
def test_v12_post_training_failure_removes_only_owned_partials_and_is_no_retry(
    tmp_path: Path,
    stage: str,
    derived_names: tuple[str, ...],
) -> None:
    operation = SyntheticExecutionV12(tmp_path / stage)
    reservation = operation.claim()
    try:
        ...
        with pytest.raises(FileExistsError, match="already claimed"):
            SyntheticExecutionV12(operation.root).claim()
    finally:
        operation.close(reservation)


def test_v12_terminalization_preserves_mutated_artifacts_as_invalid(
    tmp_path: Path,
) -> None:
    operation = SyntheticExecutionV12(tmp_path / "safe")
    reservation = operation.claim()
...
def test_v12_concurrent_synthetic_claims_choose_exactly_one(tmp_path: Path) -> None:
    operations = [SyntheticExecutionV12(tmp_path / "safe") for _ in range(2)]

    def claim(operation: SyntheticExecutionV12) -> object:
        try:
    ...
    SyntheticExecutionV12.close(reservations[0])  # type: ignore[arg-type]


def test_v12_preclaim_cleanup_and_postclaim_failure_are_durable(tmp_path: Path) -> None:
    before = SyntheticExecutionV12(tmp_path / "before")
    ...
    after = SyntheticExecutionV12(tmp_path / "after")
    ...
    with pytest.raises(FileExistsError, match="already claimed"):
        SyntheticExecutionV12(after.root).claim()


def test_v12_retained_source_output_and_claim_chains_are_present_in_production() -> None:
    tree = ast.parse((ROOT / policy.EXECUTOR_RELATIVE_PATH).read_text())
    functions = {
        node.name: ast.unparse(node)
        for node in ast.walk(tree)
...
def test_v12_post_training_exception_terminalizes_before_all_descriptors_close() -> None:
    tree = ast.parse((ROOT / policy.EXECUTOR_RELATIVE_PATH).read_text())
    functions = {
        node.name: ast.unparse(node)
        for node in ast.walk(tree)
...
def test_v12_exact_entry_rejects_a_claim_but_preserves_preclaim_recovery() -> None:
    tree = ast.parse((ROOT / policy.EXECUTOR_RELATIVE_PATH).read_text())
    functions = {
        node.name: ast.unparse(node)
        for node in ast.walk(tree)
...
def test_v12_frozen_science_and_gpu0_launcher_are_unchanged() -> None:
    from lewm.benchmarks import (
...
def test_v12_review_contract_binds_end_to_end_sources_and_v4_block_closures() -> None:
    sources = {
...
def _synthetic_evaluation(loss_name: str) -> dict[str, Any]:
    value = complete_retained_gate_evaluation_v12()
...
def test_v12_evaluation_accepts_only_the_new_loss_name() -> None:
...
def test_v12_training_record_is_4000_updates_and_final_only() -> None:
...
def test_v12_full_result_schema_accepts_only_new_loss_and_schedule() -> None:
    review = {
        "path": policy.SOURCE_REVIEW_RELATIVE_PATH,
        "file_sha256": "a" * 64,
        "content_sha256": "b" * 64,
    ...
    core = {
        ...
        "mode": "exact_train_only_n5_gate_aligned_raster_nll_v12_development_fit",
        "authoritative": False,
...
def test_v12_changed_predecessor_bytes_fail_closed(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    relative = policy.V12_AMENDMENT_RELATIVE_PATH
...
def test_v12_static_preflight_consumes_new_v10_and_retained_bindings() -> None:
    source = ast.unparse(
        next(
            node
...
def test_v12_preserves_metric_accumulation_wrong_rgb_and_all_26_thresholds() -> None:
...
def test_v12_adapter_reproduces_failure_is_nonmutating_and_changes_only_keys() -> None:
    from lewm.benchmarks import (
    ...
    adapted = policy.adapt_native_v12_evaluation_for_retained_v4_gate(
        evaluation
    )
...
def test_v12_adapter_negative_cases_fail_closed(mutation: str) -> None:
    value = complete_retained_gate_evaluation_v12()
    ...
    if mutation == "missing_hierarchical":
        ...
        value["extra_control"] = {}
    with pytest.raises((PermissionError, ValueError)):
        policy.adapt_native_v12_evaluation_for_retained_v4_gate(value)


def test_v12_shared_reconstruction_is_actual_26_check_gate_and_schema_stays_native(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    evaluation = complete_retained_gate_evaluation_v12()
    original = copy.deepcopy(evaluation)
    real_adapter = policy.adapt_native_v12_evaluation_for_retained_v4_gate
    calls = 0
    ...
    malformed = complete_retained_gate_evaluation_v12()
...
def test_v12_adapter_cannot_change_a_passing_or_failing_gate_decision() -> None:
    from lewm.benchmarks import (
        go2_observable_camera_ray_fit_v4_ladder_gate as frozen,
    )
...
def test_v12_raw_retained_gate_bypasses_exist_only_in_shared_policy_helper() -> None:
...
def test_v12_child_parent_and_finalizer_each_runtime_invoke_one_adapter(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    from scripts import (
