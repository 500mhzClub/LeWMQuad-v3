"""Production-ineligible synthetic authority support for Builder V7 tests only."""
...
def validate_phase_two_for_tests(
    phase_one: builder.PhaseOneAuthorizationV7,
    *,
    repository_root: Path,
    reader: Reader,
) -> tuple[builder.AcceptedAuthorizationV7, tuple[str, ...]]:
    ...
    if type(phase_one) is not builder.PhaseOneAuthorizationV7:
        raise TypeError("test phase two requires a completed V7 phase-one capsule")
    ...
    return (
        builder.AcceptedAuthorizationV7(
            authorization_file_sha256=phase_one.authorization_file_sha256,
...
def write_prepared_scene_job_for_tests(
    ...
    receipt = builder.AcceptedAuthorizationV7("a" * 64, "b" * 64, "c" * 64)
