"""Production-ineligible V9 filesystem lifecycle used only by tests.
...
STAGING_PREFIX = ".n5.synthetic-v9-"
LOCK_NAME = ".n5.synthetic-v9.lock"
SERIALIZER_SUFFIX = ".n5.synthetic-v9.serializer"
SCHEMA = "lewm_go2_n5_hierarchical_first_hit_v9_synthetic_reservation_v1"
MANIFEST_SCHEMA = "lewm_go2_n5_hierarchical_first_hit_v9_synthetic_staging_v1"
...
def _directory_flags() -> int:
    ...
    if not nofollow or not directory:
        raise PermissionError("synthetic V9 requires no-follow directory opens")
...
class _WatchV9:
    ...
    label: str
    generation: int
    snapshot: _SnapshotV9


class _DeclaredJournalV9:
    """A deterministic test journal for declared filesystem state deltas."""

    def __init__(self) -> None:
        self._watches: dict[tuple[int, int], _WatchV9] = {}
        self._generation = 0
    ...
    def _fail(self, reason: str) -> None:
        ...
        raise PermissionError(f"synthetic V9 journal rejected: {reason}")
    ...
    @staticmethod
    def _snapshot(directory_fd: int) -> _SnapshotV9:
        ...
        return _SnapshotV9(_fingerprint(opened), rows)
    ...
    def watch(self, directory_fd: int, *, label: str) -> None:
        ...
        self._generation += 1
        self._watches[identity] = _WatchV9(
            retained,
            identity,
            label,
            self._generation,
    ...
    def _snapshots(self) -> dict[tuple[int, int], _SnapshotV9]:
        return {
    ...
    ) -> tuple[_WatchV9, dict[tuple[int, int], _SnapshotV9]]:
        self.assert_clean()
    ...
    def _finish(
        self,
        before: Mapping[tuple[int, int], _SnapshotV9],
        *,
...
@dataclass
class _AncestorChainV9:
    descriptors: list[int]
    entries: list[_AncestorEntryV9]
    closed: bool = False


def _open_ancestor_chain(path: Path) -> _AncestorChainV9:
    filesystem_root = Path(path.anchor)
    anchor_fd = os.open(filesystem_root, _directory_flags())
    chain = _AncestorChainV9([anchor_fd], [])
    parent_fd = anchor_fd
    try:
        for component in path.parts[1:]:
            ...
            chain.entries.append(
                _AncestorEntryV9(parent_fd, component, child_fd, identity)
            )
            parent_fd = child_fd
        return chain
...
def _assert_ancestor_chain(chain: _AncestorChainV9) -> None:
    if chain.closed:
...
def _close_ancestor_chain(chain: _AncestorChainV9) -> None:
    if chain.closed:
        return
    chain.closed = True
...
class SyntheticReservationV9:
    ...
    seed_root_fd: int
    derived_fd: int
    journal: _DeclaredJournalV9
    ancestor_chain: _AncestorChainV9
    owned_directory_fds: tuple[int, ...]
    owned_claim_artifacts: dict[str, _OwnedArtifactV9] = field(
        default_factory=dict, compare=False
    )
    owned_derived_artifacts: dict[str, _OwnedArtifactV9] = field(
        default_factory=dict, compare=False
    )


class SyntheticExecutionV9:
    ...
    def __copy__(self) -> "SyntheticExecutionV9":
        return type(self)(self.root)

    def __deepcopy__(self, memo: object) -> "SyntheticExecutionV9":
        del memo
        return type(self)(self.root)
    ...
    def _new_staging(
        self, seed_fd: int, journal: _DeclaredJournalV9
    ) -> tuple[str, int]:
        for _ in range(128):
    ...
    @classmethod
    def _assert_claim_fd(cls, reservation: SyntheticReservationV9) -> None:
    ...
    @classmethod
    def _assert_claim(cls, reservation: SyntheticReservationV9) -> None:
    ...
    def _write_claim(
        cls,
        reservation: SyntheticReservationV9,
        name: str,
        raw: bytes,
        *,
        terminal: bool = False,
        ...
        reservation.owned_claim_artifacts[name] = _OwnedArtifactV9(
    ...
    @classmethod
    def _write_derived(
        cls, reservation: SyntheticReservationV9, name: str, raw: bytes
    ) -> None:
        cls._assert_claim(reservation)
        fingerprint = reservation.journal.create(
            reservation.derived_fd, name, raw
        )
        reservation.owned_derived_artifacts[name] = _OwnedArtifactV9(
    ...
    def claim(
        ...
        with self._serializer():
            ...
            claimed: SyntheticReservationV9 | None = None
            ...
            try:
                ...
                claimed = SyntheticReservationV9(
                    self.attempt,
                    value,
                    raw,
                    active_fd,
    ...
    def _cleanup_owned(
        reservation: SyntheticReservationV9,
        registry: dict[str, _OwnedArtifactV9],
        *,
        ...
        *,
        ...
        if not reservation.journal.poisoned:
            try:
                ...
                pass
        core = {
            "schema": "lewm_go2_n5_hierarchical_first_hit_v9_synthetic_failure_v1",
            "status": "failed",
            "failure_stage": stage,
    ...
    def publish(self, reservation: SyntheticReservationV9, raw: bytes) -> None:
    ...
    @staticmethod
    def close(reservation: SyntheticReservationV9) -> None:
        try:
