...
import pytest
from scripts.benchmark_go2_packed_owned_controller_pair_v1 import admit, order, measure, FRAMES


def valid():
    return dict(status='PACKED_OWNED_MAZE_CONTROLLER_REPLAY_COMPLETE',frames=1881,
...
def test_order_balances_first_position_without_changing_frame_population():
    ...
    assert all(set(order(i))=={'original','packed_owned'} for i in range(FRAMES))
