...
from scripts import (
    build_go2_scene_diversity_recurrent_replication_genesis_rocm_backend_v3_integrity_replacement_v1_plan
    as builder,
)
...
def plans(runtime_bindings: dict) -> tuple[dict, dict]:
    frozen = copy.deepcopy(
        builder.predecessor.predecessor.predecessor._IMMUTABLE_FROZEN_VULKAN_PLAN  # noqa: SLF001
    )
    return (
...
def test_exact_v3_terminal_review_is_the_only_v3_failure_witness() -> None:
    binding = builder.v3_qualification_terminal_review_binding()
    assert binding == {
        "path": str(builder.V3_QUALIFICATION_TERMINAL_REVIEW.resolve()),
        "file_sha256": (
...
def test_v3_to_replacement_plan_delta_is_exact_identity_cache_and_witness_only(
    ...
    assert "integrity_replacement_v1" in replacement_environment[
        "GS_CACHE_FILE_PATH"
    ]
    ...
    added = {
        ...
        "v3_authority_or_command_reuse_authorized",
        "v3_runtime_payload_reuse_authorized",
        "v3_runtime_metadata_as_identity_evidence_authorized",
    }
    ...
    assert replacement_successor["v3_runtime_payload_reuse_authorized"] is False
    assert (
    ...
    assert replacement_successor["v3_qualification_terminal_review_binding"] == (
        builder.v3_qualification_terminal_review_binding()
    )
...
def test_plan_validator_rejects_missing_or_mutated_home(
    ...
    for mutation in (None, "/tmp/wrong-home"):
        ...
        with pytest.raises(
            builder.SceneDiversityGenesisRocmV3IntegrityReplacementV1PlanError,
