"""Production-ineligible V14 filesystem lifecycle used only by tests.
...
STAGING_PREFIX = ".n5.synthetic-v14-"
LOCK_NAME = ".n5.synthetic-v14.lock"
SERIALIZER_SUFFIX = ".n5.synthetic-v14.serializer"
SCHEMA = "lewm_go2_n5_gate_aligned_raster_nll_v14_synthetic_reservation_v1"
MANIFEST_SCHEMA = "lewm_go2_n5_gate_aligned_raster_nll_v14_synthetic_staging_v1"
...
def complete_retained_gate_evaluation_v14() -> dict[str, Any]:
    ...
    def control(name: str, mapping: list[int], *, perfect: bool) -> dict[str, Any]:
        ...
        if family_count * len(family_names) != raster["count"]:
            raise ValueError("V14 synthetic family partition is not even")
        diagnostics = {
            "overall": {
...
def _directory_flags() -> int:
    ...
    if not nofollow or not directory:
        raise PermissionError("synthetic V14 requires no-follow directory opens")
...
class _WatchV14:
    ...
    label: str
    generation: int
    snapshot: _SnapshotV14


class _DeclaredJournalV14:
    """A deterministic test journal for declared filesystem state deltas."""

    def __init__(self) -> None:
        self._watches: dict[tuple[int, int], _WatchV14] = {}
        self._generation = 0
    ...
    def _fail(self, reason: str) -> None:
        ...
        raise PermissionError(f"synthetic V14 journal rejected: {reason}")
    ...
    @staticmethod
    def _snapshot(directory_fd: int) -> _SnapshotV14:
        ...
        return _SnapshotV14(_fingerprint(opened), rows)
    ...
    def watch(self, directory_fd: int, *, label: str) -> None:
        ...
        self._generation += 1
        self._watches[identity] = _WatchV14(
            retained,
            identity,
            label,
            self._generation,
    ...
    def _snapshots(self) -> dict[tuple[int, int], _SnapshotV14]:
        return {
    ...
    ) -> tuple[_WatchV14, dict[tuple[int, int], _SnapshotV14]]:
        self.assert_clean()
    ...
    def _finish(
        self,
        before: Mapping[tuple[int, int], _SnapshotV14],
        *,
...
@dataclass
class _AncestorChainV14:
    descriptors: list[int]
    entries: list[_AncestorEntryV14]
    closed: bool = False


def _open_ancestor_chain(path: Path) -> _AncestorChainV14:
    filesystem_root = Path(path.anchor)
    anchor_fd = os.open(filesystem_root, _directory_flags())
    chain = _AncestorChainV14([anchor_fd], [])
    parent_fd = anchor_fd
    try:
        for component in path.parts[1:]:
            ...
            chain.entries.append(
                _AncestorEntryV14(parent_fd, component, child_fd, identity)
            )
            parent_fd = child_fd
        return chain
...
def _assert_ancestor_chain(chain: _AncestorChainV14) -> None:
    if chain.closed:
...
def _close_ancestor_chain(chain: _AncestorChainV14) -> None:
    if chain.closed:
        return
    chain.closed = True
...
class SyntheticReservationV14:
    ...
    seed_root_fd: int
    derived_fd: int
    journal: _DeclaredJournalV14
    ancestor_chain: _AncestorChainV14
    owned_directory_fds: tuple[int, ...]
    owned_claim_artifacts: dict[str, _OwnedArtifactV14] = field(
        default_factory=dict, compare=False
    )
    owned_derived_artifacts: dict[str, _OwnedArtifactV14] = field(
        default_factory=dict, compare=False
    )


class SyntheticExecutionV14:
    ...
    def __copy__(self) -> "SyntheticExecutionV14":
        return type(self)(self.root)

    def __deepcopy__(self, memo: object) -> "SyntheticExecutionV14":
        del memo
        return type(self)(self.root)
    ...
    def _new_staging(
        self, seed_fd: int, journal: _DeclaredJournalV14
    ) -> tuple[str, int]:
        for _ in range(128):
    ...
    @classmethod
    def _assert_claim_fd(cls, reservation: SyntheticReservationV14) -> None:
    ...
    @classmethod
    def _assert_claim(cls, reservation: SyntheticReservationV14) -> None:
    ...
    def _write_claim(
        cls,
        reservation: SyntheticReservationV14,
        name: str,
        raw: bytes,
        *,
        terminal: bool = False,
        ...
        reservation.owned_claim_artifacts[name] = _OwnedArtifactV14(
    ...
    @classmethod
    def _write_derived(
        cls, reservation: SyntheticReservationV14, name: str, raw: bytes
    ) -> None:
        cls._assert_claim(reservation)
        fingerprint = reservation.journal.create(
            reservation.derived_fd, name, raw
        )
        reservation.owned_derived_artifacts[name] = _OwnedArtifactV14(
    ...
    def claim(
        ...
        with self._serializer():
            ...
            claimed: SyntheticReservationV14 | None = None
            ...
            try:
                ...
                claimed = SyntheticReservationV14(
                    self.attempt,
                    value,
                    raw,
                    active_fd,
    ...
    def _cleanup_owned(
        reservation: SyntheticReservationV14,
        registry: dict[str, _OwnedArtifactV14],
        *,
        ...
        *,
        ...
        if not reservation.journal.poisoned:
            try:
                ...
                pass
        core = {
            "schema": "lewm_go2_n5_gate_aligned_raster_nll_v14_synthetic_failure_v1",
            "status": "failed",
            "failure_stage": stage,
    ...
    def publish(self, reservation: SyntheticReservationV14, raw: bytes) -> None:
    ...
    @staticmethod
    def close(reservation: SyntheticReservationV14) -> None:
        try:
