...
from lewm.benchmarks.phase2r_geometry_affordance import (
...
def test_phase2r_geometry_examples_join_pose_and_materialize_batch(
    tmp_path: Path,
) -> None:
    scene_id = "test_scene"
    ...
    feature_names = phase2r_geometry_feature_names(8)

    examples = build_phase2r_geometry_affordance_examples(
        rows,
        primitive_names=("forward_slow", "backward"),
        ray_count=8,
        max_ray_m=4.0,
    )
    batch = materialize_phase2r_geometry_batch(examples, (0,))
    audit = phase2r_geometry_dataset_audit(
        examples,
        split_name="train",
    ...
    assert len(examples[0].geometry_features) == len(feature_names)
    assert batch.geometry_features.shape == (1, len(feature_names))
    assert batch.factor_targets.shape == (1, 2, 6)
    assert audit["finite_features"]
    assert audit["goal_pose_found"] == 1
    assert batch.geometry_features[0, 0] <= 1.0


def test_geometry_primitive_affordance_model_backpropagates() -> None:
