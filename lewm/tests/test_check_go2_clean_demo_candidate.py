...
SUPPORT_SPEC = importlib.util.spec_from_file_location(
    "claim_checker_test_support_clean",
...
def test_clean_checker_accepts_external_manifest_recomputation(tmp_path: Path) -> None:
    manifest = SUPPORT._manifest(task_count=4)
    code, report = SUPPORT._run_checker(
        tmp_path, "clean", SUPPORT._canonical_result(manifest), manifest
    )
...
def test_clean_checker_rejects_each_canonical_claim_mutation(
    tmp_path: Path,
    mutation: str,
) -> None:
    manifest = SUPPORT._manifest(task_count=4)
    ...
    code, report = SUPPORT._run_checker(
        tmp_path, "clean", result, supplied_manifest
    )
