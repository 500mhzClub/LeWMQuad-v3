...
import pytest
from scripts import verify_go2_receipt_copied_profile_completion_v1 as check
...
def fixture():
    ...
    report=dict(frames=1428,raw_model_forecast_comparisons=1425,model_state_sha256=check.run.original.reference.MODEL_SHA,
        last_replayed_observation=1427,controller='ReceiptCopiedFootprintController',windows=windows,
        ...
        complete_normalized_candidate_decisions_exact=True,receipt_copied_footprint_controller_profiled=True,
        all_profiled_decisions_equal_completed_copied_replay=True)
...
def test_explicit_result_identity_required_before_io(monkeypatch,value):
    ...
    with pytest.raises(ValueError,match='SHA-256'):check.verify_completed(value)
...
def test_live_owner_rejects_before_result_or_profile_access(monkeypatch,tmp_path):
    ...
    with pytest.raises(ValueError,match='owner still live'):check.verify_completed('a'*64)
