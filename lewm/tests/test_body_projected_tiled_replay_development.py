...
import pytest
from scripts import body_projected_tiled_replay_development as run
from lewm import body_projected_tiled_controller_development as copied
...
def fixture(monkeypatch,tmp_path,fault=None):
    ...
    monkeypatch.setattr(run,'BodyProjectedTiledController',Candidate)
...
def test_original_loop_and_only_declared_composition_changes():
    ...
    assert new.__globals__['ScopedFootprintAnchoredController'] is run.BodyProjectedTiledController
...
def test_full_history_and_seven_original_state_checks(monkeypatch,tmp_path):
    ...
    assert result['candidate']=='BodyProjectedTiledController'
