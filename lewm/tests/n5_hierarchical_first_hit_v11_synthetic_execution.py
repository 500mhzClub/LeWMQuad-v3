"""Production-ineligible V11 filesystem lifecycle used only by tests.
...
STAGING_PREFIX = ".n5.synthetic-v11-"
LOCK_NAME = ".n5.synthetic-v11.lock"
SERIALIZER_SUFFIX = ".n5.synthetic-v11.serializer"
SCHEMA = "lewm_go2_n5_hierarchical_first_hit_v11_synthetic_reservation_v1"
MANIFEST_SCHEMA = "lewm_go2_n5_hierarchical_first_hit_v11_synthetic_staging_v1"
...
def complete_retained_gate_evaluation_v11() -> dict[str, Any]:
...
def _directory_flags() -> int:
    ...
    if not nofollow or not directory:
        raise PermissionError("synthetic V11 requires no-follow directory opens")
...
class _WatchV11:
    ...
    label: str
    generation: int
    snapshot: _SnapshotV11


class _DeclaredJournalV11:
    """A deterministic test journal for declared filesystem state deltas."""

    def __init__(self) -> None:
        self._watches: dict[tuple[int, int], _WatchV11] = {}
        self._generation = 0
    ...
    def _fail(self, reason: str) -> None:
        ...
        raise PermissionError(f"synthetic V11 journal rejected: {reason}")
    ...
    @staticmethod
    def _snapshot(directory_fd: int) -> _SnapshotV11:
        ...
        return _SnapshotV11(_fingerprint(opened), rows)
    ...
    def watch(self, directory_fd: int, *, label: str) -> None:
        ...
        self._generation += 1
        self._watches[identity] = _WatchV11(
            retained,
            identity,
            label,
            self._generation,
    ...
    def _snapshots(self) -> dict[tuple[int, int], _SnapshotV11]:
        return {
    ...
    ) -> tuple[_WatchV11, dict[tuple[int, int], _SnapshotV11]]:
        self.assert_clean()
    ...
    def _finish(
        self,
        before: Mapping[tuple[int, int], _SnapshotV11],
        *,
...
@dataclass
class _AncestorChainV11:
    descriptors: list[int]
    entries: list[_AncestorEntryV11]
    closed: bool = False


def _open_ancestor_chain(path: Path) -> _AncestorChainV11:
    filesystem_root = Path(path.anchor)
    anchor_fd = os.open(filesystem_root, _directory_flags())
    chain = _AncestorChainV11([anchor_fd], [])
    parent_fd = anchor_fd
    try:
        for component in path.parts[1:]:
            ...
            chain.entries.append(
                _AncestorEntryV11(parent_fd, component, child_fd, identity)
            )
            parent_fd = child_fd
        return chain
...
def _assert_ancestor_chain(chain: _AncestorChainV11) -> None:
    if chain.closed:
...
def _close_ancestor_chain(chain: _AncestorChainV11) -> None:
    if chain.closed:
        return
    chain.closed = True
...
class SyntheticReservationV11:
    ...
    seed_root_fd: int
    derived_fd: int
    journal: _DeclaredJournalV11
    ancestor_chain: _AncestorChainV11
    owned_directory_fds: tuple[int, ...]
    owned_claim_artifacts: dict[str, _OwnedArtifactV11] = field(
        default_factory=dict, compare=False
    )
    owned_derived_artifacts: dict[str, _OwnedArtifactV11] = field(
        default_factory=dict, compare=False
    )


class SyntheticExecutionV11:
    ...
    def __copy__(self) -> "SyntheticExecutionV11":
        return type(self)(self.root)

    def __deepcopy__(self, memo: object) -> "SyntheticExecutionV11":
        del memo
        return type(self)(self.root)
    ...
    def _new_staging(
        self, seed_fd: int, journal: _DeclaredJournalV11
    ) -> tuple[str, int]:
        for _ in range(128):
    ...
    @classmethod
    def _assert_claim_fd(cls, reservation: SyntheticReservationV11) -> None:
    ...
    @classmethod
    def _assert_claim(cls, reservation: SyntheticReservationV11) -> None:
    ...
    def _write_claim(
        cls,
        reservation: SyntheticReservationV11,
        name: str,
        raw: bytes,
        *,
        terminal: bool = False,
        ...
        reservation.owned_claim_artifacts[name] = _OwnedArtifactV11(
    ...
    @classmethod
    def _write_derived(
        cls, reservation: SyntheticReservationV11, name: str, raw: bytes
    ) -> None:
        cls._assert_claim(reservation)
        fingerprint = reservation.journal.create(
            reservation.derived_fd, name, raw
        )
        reservation.owned_derived_artifacts[name] = _OwnedArtifactV11(
    ...
    def claim(
        ...
        with self._serializer():
            ...
            claimed: SyntheticReservationV11 | None = None
            ...
            try:
                ...
                claimed = SyntheticReservationV11(
                    self.attempt,
                    value,
                    raw,
                    active_fd,
    ...
    def _cleanup_owned(
        reservation: SyntheticReservationV11,
        registry: dict[str, _OwnedArtifactV11],
        *,
        ...
        *,
        ...
        if not reservation.journal.poisoned:
            try:
                ...
                pass
        core = {
            "schema": "lewm_go2_n5_hierarchical_first_hit_v11_synthetic_failure_v1",
            "status": "failed",
            "failure_stage": stage,
    ...
    def publish(self, reservation: SyntheticReservationV11, raw: bytes) -> None:
    ...
    @staticmethod
    def close(reservation: SyntheticReservationV11) -> None:
        try:
