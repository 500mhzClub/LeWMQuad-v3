...
import scripts.replay_go2_gyro_seeded_correspondence_v1 as replay
...
def test_complete_paired_stream_precedes_native_and_scores_orientation(synthetic):
    ...
    assert result['status'] == 'GYRO_SEEDED_PAIRED_DEVELOPMENT_REPLAY_COMPLETE'
    ...
    assert e['availability'] == dict(both=3, original_only=0, gyro_seeded_only=0, neither=0)
...
def test_candidate_missingness_keeps_original_and_common_denominators(monkeypatch, synthetic):
    original = replay.GyroSeededVisualLedMotion
    class Stops(original):
        def observe(self, *args, **kwargs):
            row = super().observe(*args, **kwargs)
            if self.model.frame >= 1: row['current_pose'] = None
            return row
    monkeypatch.setitem(replay.MODELS, 'gyro_seeded', Stops)
    replay.run(); r = replay.read_json(replay.OUTPUT, 'result.json'); e = r['evaluations']['inner_synthetic']
    assert e['availability'] == dict(both=1, original_only=2, gyro_seeded_only=0, neither=0)
    assert e['arms']['original']['position_m']['count'] == 3
    assert e['arms']['original']['paired_position_m']['count'] == 1
    assert e['arms']['gyro_seeded']['paired_position_m']['count'] == 1
    assert r['reports']['inner_synthetic']['arms']['gyro_seeded']['first_failure'] == 1
