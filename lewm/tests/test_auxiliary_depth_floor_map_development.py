...
from lewm.auxiliary_tilted_depth_geometry_development import body_from_optical,reference_pose
from lewm.auxiliary_tilted_depth_observation_development import from_native_depth,FOCAL
...
def memory():
    policy,_,now=frame();m=AuxiliaryDepthSurfaceMemory(identity=(0,0,0));w=dict(frame=0,measured_ns=now)
