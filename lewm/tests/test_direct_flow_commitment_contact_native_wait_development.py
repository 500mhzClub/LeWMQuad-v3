...
from scripts import await_go2_direct_flow_commitment_contact_maze02_native_v1 as waiter
...
def test_fixed_roster_contains_exact_raw_owner_and_original_final_waiter():
    assert [s[0] for s in waiter.PREREQUISITES] == ['raw', 'sustained']
    assert [s[1]['pid'] for s in waiter.PREREQUISITES] == [2822758, 2817601]
    assert waiter.PREREQUISITES[0][3] == '6c577445f7bff2e58aad960c6d26908c683a20b0d7a4fbb2344967b5072da9ad'
    assert waiter.PREREQUISITES[1][3] == '679d0519e3eb5ddacf5cc3708254de12a117551725c43c0136141e43ce64263e'
...
def test_command_forwards_every_exact_completion_once():
    ...
    assert dict(zip(cmd[3::2], cmd[4::2], strict=True)) == dict(
        zip(['--raw-prefix-result-sha256', '--sustained-wait-result-sha256'], ids.values(), strict=True))
...
def test_incomplete_or_invented_command_roster_rejected(fault):
    ids = identities()
    if fault == 'missing': ids.pop('sustained')
    elif fault == 'extra': ids['replacement'] = 'a'*64
    elif fault == 'invalid': ids['sustained'] = 'not-a-hash'
    else: ids['sustained'] = True
...
def loop_fixture(monkeypatch):
    ...
    ids = identities()
    def owner(owner):
        return state['tick'] == 0 or (state['tick'] == 1 and owner['pid'] == 2817601)
...
def test_no_identity_is_inferred_while_its_original_owner_is_live(monkeypatch):
    ...
    assert (1, 'sustained') not in checked and (2, 'sustained') in checked
...
def completion_fixture(tmp_path, monkeypatch, key='raw'):
    ...
    result = dict(status=spec[4], source_sha256=sources.copy(),
        ...
        report=__import__('lewm.tests.test_direct_flow_commitment_contact_native_prefix_development',fromlist=['fixture']).fixture()[0])
...
@pytest.mark.parametrize('key', ['raw', 'sustained'])
...
def test_incomplete_or_changed_original_completion_rejected(tmp_path, monkeypatch, fault):
    spec, sources, result, launch, verified = completion_fixture(tmp_path, monkeypatch,
        'sustained' if fault == 'retry' else 'raw')
...
def terminal_fixture(tmp_path, monkeypatch):
    ...
    launch = dict(source_sha256={'src':'sha'}, input_admission=dict(raw_prefix_result_sha256=ids['raw'],
        sustained_wait_result_sha256=ids['sustained'], prefix_report={'raw':True}))
    result = dict(status='DIRECT_FLOW_COMMITMENT_CONTACT_MAZE02_PILOT_V1_COMPLETE',
        artifact_sha256=bindings, source_sha256={'src':'sha'}, conditions=[record], measured_round_trip_successes=0,
        raw_prefix_result_sha256=ids['raw'], prospective_prefix_result_sha256=ids['raw'], sustained_wait_result_sha256=ids['sustained'])
...
@pytest.mark.parametrize('fault', ['raw', 'sustained', 'admission', 'missing_raw', 'worker', 'collection',
...
def test_changed_native_evidence_cannot_be_certified(tmp_path, monkeypatch, fault):
    ...
    if fault == 'sustained': result['sustained_wait_result_sha256'] = 'b'*64
