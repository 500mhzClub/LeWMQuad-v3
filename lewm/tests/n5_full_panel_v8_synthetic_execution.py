"""Production-ineligible V8 filesystem lifecycle used only by tests.
...
STAGING_PREFIX = ".n5.synthetic-v8-"
LOCK_NAME = ".n5.synthetic-v8.lock"
SERIALIZER_SUFFIX = ".n5.synthetic-v8.serializer"
SCHEMA = "lewm_go2_n5_full_panel_v8_synthetic_reservation_v1"
MANIFEST_SCHEMA = "lewm_go2_n5_full_panel_v8_synthetic_staging_v1"
...
def _directory_flags() -> int:
    ...
    if not nofollow or not directory:
        raise PermissionError("synthetic V8 requires no-follow directory opens")
...
class _WatchV8:
    ...
    label: str
    generation: int
    snapshot: _SnapshotV8


class _DeclaredJournalV8:
    """A deterministic test journal for declared filesystem state deltas."""

    def __init__(self) -> None:
        self._watches: dict[tuple[int, int], _WatchV8] = {}
        self._generation = 0
    ...
    def _fail(self, reason: str) -> None:
        ...
        raise PermissionError(f"synthetic V8 journal rejected: {reason}")
    ...
    @staticmethod
    def _snapshot(directory_fd: int) -> _SnapshotV8:
        ...
        return _SnapshotV8(_fingerprint(opened), rows)
    ...
    def watch(self, directory_fd: int, *, label: str) -> None:
        ...
        self._generation += 1
        self._watches[identity] = _WatchV8(
            retained,
            identity,
            label,
            self._generation,
    ...
    def _snapshots(self) -> dict[tuple[int, int], _SnapshotV8]:
        return {
    ...
    ) -> tuple[_WatchV8, dict[tuple[int, int], _SnapshotV8]]:
        self.assert_clean()
    ...
    def _finish(
        self,
        before: Mapping[tuple[int, int], _SnapshotV8],
        *,
...
@dataclass
class _AncestorChainV8:
    descriptors: list[int]
    entries: list[_AncestorEntryV8]
    closed: bool = False


def _open_ancestor_chain(path: Path) -> _AncestorChainV8:
    filesystem_root = Path(path.anchor)
    anchor_fd = os.open(filesystem_root, _directory_flags())
    chain = _AncestorChainV8([anchor_fd], [])
    parent_fd = anchor_fd
    try:
        for component in path.parts[1:]:
            ...
            chain.entries.append(
                _AncestorEntryV8(parent_fd, component, child_fd, identity)
            )
            parent_fd = child_fd
        return chain
...
def _assert_ancestor_chain(chain: _AncestorChainV8) -> None:
    if chain.closed:
...
def _close_ancestor_chain(chain: _AncestorChainV8) -> None:
    if chain.closed:
        return
    chain.closed = True
...
class SyntheticReservationV8:
    ...
    seed_root_fd: int
    derived_fd: int
    journal: _DeclaredJournalV8
    ancestor_chain: _AncestorChainV8
    owned_directory_fds: tuple[int, ...]
    owned_claim_artifacts: dict[str, _OwnedArtifactV8] = field(
        default_factory=dict, compare=False
    )
    owned_derived_artifacts: dict[str, _OwnedArtifactV8] = field(
        default_factory=dict, compare=False
    )


class SyntheticExecutionV8:
    ...
    def __copy__(self) -> "SyntheticExecutionV8":
        return type(self)(self.root)

    def __deepcopy__(self, memo: object) -> "SyntheticExecutionV8":
        del memo
        return type(self)(self.root)
    ...
    def _new_staging(
        self, seed_fd: int, journal: _DeclaredJournalV8
    ) -> tuple[str, int]:
        for _ in range(128):
    ...
    @classmethod
    def _assert_claim_fd(cls, reservation: SyntheticReservationV8) -> None:
    ...
    @classmethod
    def _assert_claim(cls, reservation: SyntheticReservationV8) -> None:
    ...
    def _write_claim(
        cls,
        reservation: SyntheticReservationV8,
        name: str,
        raw: bytes,
        *,
        terminal: bool = False,
        ...
        reservation.owned_claim_artifacts[name] = _OwnedArtifactV8(
    ...
    @classmethod
    def _write_derived(
        cls, reservation: SyntheticReservationV8, name: str, raw: bytes
    ) -> None:
        cls._assert_claim(reservation)
        fingerprint = reservation.journal.create(
            reservation.derived_fd, name, raw
        )
        reservation.owned_derived_artifacts[name] = _OwnedArtifactV8(
    ...
    def claim(
        ...
        with self._serializer():
            ...
            claimed: SyntheticReservationV8 | None = None
            ...
            try:
                ...
                claimed = SyntheticReservationV8(
                    self.attempt,
                    value,
                    raw,
                    active_fd,
    ...
    def _cleanup_owned(
        reservation: SyntheticReservationV8,
        registry: dict[str, _OwnedArtifactV8],
        *,
        ...
        *,
        ...
        if not reservation.journal.poisoned:
            try:
                ...
                pass
        core = {
            "schema": "lewm_go2_n5_full_panel_v8_synthetic_failure_v1",
            "status": "failed",
            "failure_stage": stage,
    ...
    def publish(self, reservation: SyntheticReservationV8, raw: bytes) -> None:
    ...
    @staticmethod
    def close(reservation: SyntheticReservationV8) -> None:
        try:
