"""CPU-hidden author closure for gate-aligned raster-NLL V15."""
...
from lewm.tests.n5_gate_aligned_raster_nll_v15_synthetic_execution import (
    LOCK_NAME as SYNTHETIC_LOCK_NAME,
    SyntheticExecutionV15,
    complete_retained_gate_evaluation_v15,
)
from scripts import execute_go2_observable_camera_ray_fit_v4_n5_gate_aligned_raster_nll_v15 as executor
...
def _gpu_visibility_binding() -> dict[str, Any]:
    return {
        "receipt_path": str(policy.GPU_VISIBILITY_RECEIPT_PATH),
        "receipt_schema": policy.GPU_VISIBILITY_RECEIPT_SCHEMA,
        "receipt_file_sha256": "d" * 64,
        "receipt_content_sha256": "e" * 64,
        "receipt_disposition": "pass_exactly_one_r9700",
        "source_review_file_sha256": "a" * 64,
        "source_review_content_sha256": "b" * 64,
        "repository_commit": "f" * 64,
        "live_recheck_disposition": "pass_exactly_one_r9700",
        "live_recheck_completed_before_reservation": True,
    }
...
def _load_nested_executor() -> ModuleType:
    ...
    for node in tree.body:
        if node is entry:
            ...
            body.append(node)
    module_name = f"_lewm_camera_v15_author_{uuid.uuid4().hex}"
...
def _runtime_reservation(
    ...
    reservation = runtime.AttemptReservationV15(
        directory=attempt,
...
def test_v15_claim_transaction_rejects_frozen_v6_create_delete_blocker(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_v15_derived_transaction_rejects_frozen_v6_create_delete_blocker(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime, reservation, _generated = _runtime_reservation(tmp_path, monkeypatch)
...
def test_v15_owned_claim_and_derived_transactions_pass(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime, reservation, _generated = _runtime_reservation(tmp_path, monkeypatch)
...
def test_v15_production_cleanup_and_failure_receipt_are_journaled(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime, reservation, _generated = _runtime_reservation(tmp_path, monkeypatch)
...
def test_v15_claim_commit_is_registered_before_postcommit_stat_failure(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime, reservation, _generated = _runtime_reservation(tmp_path, monkeypatch)
...
def test_v15_derived_commit_is_registered_before_postcommit_fsync_failure(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime, reservation, _generated = _runtime_reservation(tmp_path, monkeypatch)
...
def test_v15_poisoned_journal_writes_identity_only_failure_and_never_success(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime, reservation, _generated = _runtime_reservation(tmp_path, monkeypatch)
...
def test_v15_poison_during_failure_receipt_preserves_invalid_and_returns_no_binding(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime, reservation, _generated = _runtime_reservation(tmp_path, monkeypatch)
...
def test_v15_post_training_ownership_failure_terminalizes_and_closes(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime, reservation, _generated = _runtime_reservation(tmp_path, monkeypatch)
    from scripts import (
        train_go2_observable_camera_ray_fit_v4_n5_gate_aligned_raster_nll_v15
        as retained,
    )
    ...
    monkeypatch.setattr(
        ...
        lambda _source_review_sha256, _gpu_visibility: reservation,
    )
    ...
    with pytest.raises(
        ...
        runtime._run_frozen_training(
            "a" * 64,
            _gpu_visibility_binding(),
            rgb_workers=1,
        )
    failed = policy.parse_json(
...
def test_v15_move_in_out_restoration_is_rejected(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime, reservation, _generated = _runtime_reservation(tmp_path, monkeypatch)
...
def test_v15_restoration_history_rejects_at_every_transaction_hook(
    tmp_path: Path,
    history: str,
    hook: str,
) -> None:
    ...
    outside_fd = os.open(outside, runtime._directory_flags())
    journal = runtime.OwnedDirectoryJournalV15()
...
def test_v15_kernel_exact_sequences_cover_all_journal_primitives(
    tmp_path: Path,
) -> None:
    runtime = _load_nested_executor()
...
def test_v15_no_replace_claim_race_preserves_foreign_destination(
    tmp_path: Path,
) -> None:
    runtime = _load_nested_executor()
    parent_fd = os.open(tmp_path, runtime._directory_flags())
...
def test_v15_directory_rename_binds_source_to_retained_descriptor(
    tmp_path: Path,
) -> None:
    runtime = _load_nested_executor()
...
def test_v15_unlink_syscall_boundary_race_poisons_success(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime = _load_nested_executor()
...
def test_v15_replace_syscall_boundary_race_poisons_success(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime = _load_nested_executor()
...
def test_v15_rmdir_syscall_boundary_race_poisons_success(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime = _load_nested_executor()
...
def test_v15_watch_rejects_mutation_before_installation(
    tmp_path: Path,
) -> None:
    runtime = _load_nested_executor()
    directory_fd = os.open(tmp_path, runtime._directory_flags())
...
def test_v15_watch_rejects_mutation_during_installation(
    tmp_path: Path,
) -> None:
    runtime = _load_nested_executor()
    directory_fd = os.open(tmp_path, runtime._directory_flags())
...
def test_v15_exclusive_root_and_descendants_are_created_by_transactions(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime = _load_nested_executor()
...
def test_v15_exclusive_root_allows_unrelated_named_shared_churn(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime = _load_nested_executor()
    ...
    try:
        fingerprint = journal.create_exclusive_root(parent_fd, "exclusive-v15")
        assert runtime._stable_fingerprint(
            os.stat("exclusive-v15", dir_fd=parent_fd, follow_symlinks=False)
        ) == fingerprint
        journal.assert_clean()
    finally:
        journal.close()
...
def test_v15_exclusive_root_rejects_shared_parent_self_event(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime = _load_nested_executor()
    ...
    try:
        with pytest.raises(PermissionError, match="shared parent self"):
            journal.create_exclusive_root(parent_fd, "exclusive-v15")
        assert journal.poisoned is True
    finally:
        journal.close()
        os.close(parent_fd)
...
def test_v15_exclusive_root_rejects_unknown_shared_event_mask(
    tmp_path: Path,
) -> None:
    runtime = _load_nested_executor()
    parent_fd = os.open(tmp_path, runtime._directory_flags())
    ...
    def events_with_unknown_mask() -> list[Any]:
        ...
        if any(event.name == "exclusive-v15" for event in events):
            events.append(
                runtime.JournalEventV15(
                    events[0].watch,
                    0x00000001,
                    0,
                    "unrelated",
...
def _patch_temporary_reservation_authority(
    ...
    root.mkdir()
    output = root / ".generated/camera-v15"
    seed_root = output / "attempts/seed_20260710"
    attempt = seed_root / "n5"
    review_binding = {
        "path": policy.SOURCE_REVIEW_RELATIVE_PATH,
...
def test_v15_verifier_child_environment_removes_all_conflicting_selectors(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime = _load_nested_executor()
    for name in (
        *policy.GPU_VISIBILITY_UNSET_SELECTORS,
...
def test_v15_exact_relaunch_removes_all_conflicting_device_selectors(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime = _load_nested_executor()
    for name in (
    ...
    argv = [
        ...
        "a" * 64,
        "--gpu-visibility-receipt-sha256",
        "d" * 64,
        "--gpu-visibility-receipt-content-sha256",
        "e" * 64,
    ]
...
def test_v15_verifier_request_binds_every_authority_surface(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    mutation: str,
) -> None:
...
def test_v15_verifier_response_rejects_binding_or_publication_tamper(
    monkeypatch: pytest.MonkeyPatch,
    mutation: str,
) -> None:
    runtime = _load_nested_executor()
...
def test_v15_parent_spawns_exact_compute_child_then_publishes_once(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime = _load_nested_executor()
    request = {
    ...
    def fake_observe(command: list[str], **kwargs: Any) -> Any:
        ...
        return runtime.ChildProcessObservationV15(
            returncode=0,
            signal=None,
            timed_out=False,
...
def test_v15_verifier_child_failures_never_publish_or_fallback(
    monkeypatch: pytest.MonkeyPatch,
    failure: str,
) -> None:
    runtime = _load_nested_executor()
...
def test_v15_verifier_child_is_the_only_frozen_interop_setter_path() -> None:
...
def _synthetic_child_failure(runtime: ModuleType) -> Any:
    ...
    observation = runtime.ChildProcessObservationV15(
        returncode=70,
        signal=None,
        timed_out=False,
        stdout_raw=raw,
    ...
    return runtime.VerifierChildFailureV15(
...
def test_v15_verification_diagnostic_is_fsynced_before_cleanup_and_survives(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime, reservation, _generated = _runtime_reservation(tmp_path, monkeypatch)
...
def test_v15_diagnostic_publication_failure_preserves_scientific_artifacts(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime, reservation, _generated = _runtime_reservation(tmp_path, monkeypatch)
...
def test_v15_actual_script_runs_real_cpu_verifier_contract_smoke() -> None:
...
def test_v15_normalized_science_ast_matches_frozen_v9() -> None:
    ...
    assert "derived_raster_cell_nll_for_verification_v15(" in verifier_source
...
def test_v15_full_temporary_reservation_journals_lock_staging_and_claim(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime = _load_nested_executor()
    ...
    reservation = runtime._reserve_exact_attempt("a" * 64, _gpu_visibility_binding())
    try:
        runtime._assert_owned_claim(reservation)
...
def test_v15_transferred_claim_fd_cleanup_failure_still_closes_chain(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime = _load_nested_executor()
    ...
    monkeypatch.setattr(
        runtime.OwnedDirectoryJournalV15,
        "rename_directory",
        fail_after_transfer,
    )
    with pytest.raises(
        ...
        runtime._reserve_exact_attempt("a" * 64, _gpu_visibility_binding())
...
def test_v15_complete_staging_recovers_by_rehash_then_atomic_claim(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime = _load_nested_executor()
...
def test_v15_existing_exact_scaffold_without_staging_blocks_claim(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime = _load_nested_executor()
...
def test_v15_missing_derived_scaffold_blocks_valid_complete_recovery(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    missing: str,
) -> None:
...
def test_v15_invalid_lock_blocks_valid_complete_recovery(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    lock_state: str,
) -> None:
...
def test_v15_insecure_recovered_leaf_is_preserved_and_blocks_claim(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    mutation: str,
) -> None:
...
def test_v15_claim_manifest_alias_is_preserved_invalid(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime = _load_nested_executor()
...
def test_v15_multiple_equivalent_complete_stagings_resume_lexical_first(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime = _load_nested_executor()
...
def test_v15_conflicting_authority_core_cannot_pass_complete_classification(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime = _load_nested_executor()
    ...
    conflicting_reservation = runtime.AttemptReservationV15(
        directory=attempt,
        value=value,
        raw=raw,
        file_sha256=hashlib.sha256(raw).hexdigest(),
...
def test_v15_exact_name_invalid_staging_closes_opened_descriptor(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime = _load_nested_executor()
...
def test_v15_foreign_staging_prefix_is_preserved_and_blocks_claim(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime = _load_nested_executor()
...
def test_v15_historical_foreign_inventory_blocks_claim(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    location: str,
) -> None:
...
def test_v15_existing_nonprivate_exclusive_root_blocks_claim(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime = _load_nested_executor()
...
def test_v15_recovered_staging_rejects_leaf_rewrite_before_watch(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime = _load_nested_executor()
...
def test_v15_all_production_child_watches_bind_expected_snapshots() -> None:
    tree = ast.parse((ROOT / policy.EXECUTOR_RELATIVE_PATH).read_text())
    calls = [
        node
        for node in ast.walk(tree)
...
def test_v15_special_or_unknown_event_permanently_poisons_success(
    tmp_path: Path,
    kind: str,
    mask: int,
) -> None:
    ...
    journal._events = lambda: [
        runtime.JournalEventV15(event_watch, mask, 0, "")
    ]
    try:
        with pytest.raises(PermissionError):
            journal.assert_clean()
...
def test_v15_unknown_watch_and_watch_descriptor_reuse_poison(
    tmp_path: Path,
) -> None:
    runtime = _load_nested_executor()
    ...
    journal._events = lambda: [
        runtime.JournalEventV15(state.watch + 1000, journal.IN_CREATE, 0, "x")
    ]
    try:
...
def test_v15_directory_move_cookie_and_event_order_are_exact(
    tmp_path: Path,
    corruption: str,
) -> None:
    runtime = _load_nested_executor()
    ...
    def corrupted() -> list[Any]:
        ...
        if len(events) == 3 and events[0].mask & journal.IN_MOVED_FROM:
            if corruption == "cookie":
                events[1] = runtime.JournalEventV15(
                    events[1].watch,
                    events[1].mask,
                    events[1].cookie + 1,
                    events[1].name,
...
def test_v15_source_only_predecessor_bindings_are_frozen() -> None:
    for relative, expected in FROZEN_V4.items():
        assert _sha(ROOT / relative) == expected
    for relative, expected in FROZEN_V5.items():
        assert _sha(ROOT / relative) == expected
    assert _sha(ROOT / policy.V15_AMENDMENT_RELATIVE_PATH) == (
        policy.V15_AMENDMENT_FILE_SHA256
    )
    assert _sha(
        ROOT / policy.V15_TERMINAL_V14_PROOF_CLARIFICATION_RELATIVE_PATH
    ) == policy.V15_TERMINAL_V14_PROOF_CLARIFICATION_FILE_SHA256
    assert policy.frozen_source_bindings()[
        policy.V15_TERMINAL_V14_PROOF_CLARIFICATION_RELATIVE_PATH
    ] == policy.V15_TERMINAL_V14_PROOF_CLARIFICATION_FILE_SHA256
    assert _sha(ROOT / policy.V14_AMENDMENT_RELATIVE_PATH) == (
        policy.V14_AMENDMENT_FILE_SHA256
    )
    for relative, digest in policy.V14_RETAINED_SOURCE_AND_PROOF_BINDINGS:
        assert _sha(ROOT / relative) == digest
    assert _sha(ROOT / policy.V14_REVIEW_RELATIVE_PATH) == (
        policy.V14_REVIEW_FILE_SHA256
    )
...
def test_v15_terminal_v14_state_replaces_obsolete_root_absence_proof() -> None:
    forbidden_v11_attempt = (
        b".generated/go2_observable_camera_ray_fit_v4/"
        b"n5_hierarchical_first_hit_v11/attempts"
    )
    for relative in (
        ...
        assert forbidden_v11_attempt not in policy.read_regular_bytes_at(
            ROOT,
            relative,
            name=f"frozen V14 source {relative}",
        )

    v14_root_relative = Path(
        ".generated/go2_observable_camera_ray_fit_v4/"
        "n5_gate_aligned_raster_nll_v14"
    )
    v14_root = ROOT / v14_root_relative
    directory_flags = (
        os.O_RDONLY
    ...
    root_fd = os.open(v14_root, directory_flags)
    ...
    def walk(directory_fd: int, prefix: Path) -> None:
        for name in sorted(os.listdir(directory_fd)):
            ...
            if stat.S_ISDIR(metadata.st_mode):
                directory_inventory.append(relative.as_posix())
                ...
                file_inventory.append(relative.as_posix())

    try:
        walk(root_fd, Path())
    finally:
        os.close(root_fd)
    assert directory_inventory == [
        "attempts",
        "attempts/seed_20260710",
        "attempts/seed_20260710/n5",
        "gates",
        "metric_verifications",
    ]
    expected_files = [
        "attempts/seed_20260710/.n5.reservation-v14.lock",
        "attempts/seed_20260710/n5/failed.json",
        "attempts/seed_20260710/n5/reservation.json",
    ]
    assert file_inventory == expected_files

    bindings = {
        "attempts/seed_20260710/n5/reservation.json": (
            policy.V14_TERMINAL_BINDINGS["reservation"][1],
            policy.V14_TERMINAL_BINDINGS["reservation"][2],
            policy.V14_TERMINAL_BINDINGS["reservation"][3],
        ),
        "attempts/seed_20260710/n5/failed.json": (
            policy.V14_TERMINAL_BINDINGS["failure"][1],
            policy.V14_TERMINAL_BINDINGS["failure"][2],
            policy.V14_TERMINAL_BINDINGS["failure"][3],
        ),
    }
    values: dict[str, dict[str, Any]] = {}
    for relative, (file_sha256, content_sha256, byte_count) in bindings.items():
        raw = policy.read_regular_bytes_at(
            ROOT,
            str(v14_root_relative / relative),
            name=f"terminal V14 {relative}",
        )
        assert len(raw) == byte_count
        assert hashlib.sha256(raw).hexdigest() == file_sha256
        value = policy.parse_json(raw, name=f"terminal V14 {relative}")
        assert raw == policy.canonical_json_bytes(value) + b"\n"
        core = dict(value)
        assert core.pop("content_sha256") == content_sha256
        assert policy.canonical_json_sha256(core) == content_sha256
        values[relative] = value
    lock = policy.read_regular_bytes_at(
        ROOT,
        policy.V14_TERMINAL_LOCK_RELATIVE_PATH,
        name="terminal V14 reservation lock",
    )
    assert lock == b""
    assert hashlib.sha256(lock).hexdigest() == policy.V14_TERMINAL_LOCK_FILE_SHA256

    reservation = values["attempts/seed_20260710/n5/reservation.json"]
    assert {
        key: reservation[key]
        for key in ("attempt_index", "maximum_attempts", "seed", "fit_size")
    } == {
        "attempt_index": 1,
        "maximum_attempts": 1,
        "seed": 20260710,
    ...
    assert reservation["source_review"] == {
        "path": policy.V14_REVIEW_RELATIVE_PATH,
        "file_sha256": policy.V14_REVIEW_FILE_SHA256,
        "content_sha256": policy.V14_REVIEW_CONTENT_SHA256,
    }

    failure = values["attempts/seed_20260710/n5/failed.json"]
    assert failure["status"] == "failed"
    assert failure["failure_stage"] == "training"
    assert failure["failure"] == {"class": "runtime", "code": "execution_failure"}
    assert failure["partial_artifacts_removed"] is True
    assert failure["owned_directory_journal"] == {
        "integrity": "intact",
        "poison_reason": None,
        "success_eligibility_restored": False,
    }
    assert failure["retry_authorized"] is False
    assert failure["diagnostic_publication_succeeded"] is False
    assert failure["verification_failure"] is None
    assert failure["licenses"] and all(
        authorized is False for authorized in failure["licenses"].values()
    )
    for authority in (
        "checkpoint_use_authorized",
        "development_checkpoint_use_authorized",
        "metric_verification_checkpoint_use_authorized",
        "new_model_output_authorized",
        "later_rung_execution_authorized",
        "shared_jepa_training_authorized",
        "selection_authorized",
        "calibration_change_authorized",
        "g2_authorized",
        ...
        assert failure["licenses"].get(authority, False) is False

    absence_fd = os.open(v14_root, directory_flags)
    try:
        for relative in (
            "attempts/seed_20260710/n5/checkpoint.pt",
            "attempts/seed_20260710/n5/result.json",
            "attempts/seed_20260710/n5/completed.json",
            "metric_verifications/seed_20260710_n5.json",
            "gates/seed_20260710_n5.json",
        ):
            with pytest.raises(FileNotFoundError):
                os.stat(relative, dir_fd=absence_fd, follow_symlinks=False)
    finally:
        os.close(absence_fd)


def test_v15_public_exports_include_isolated_verifier_and_v7_closure() -> None:
    assert {
...
def test_v15_reservation_metadata_constructs_without_module_aliases() -> None:
    runtime = _load_nested_executor()
    from scripts import (
        train_go2_observable_camera_ray_fit_v4_n5_gate_aligned_raster_nll_v15
        as trainer,
    ...
    try:
        core = runtime._reservation_core(
            review,
            _gpu_visibility_binding(),
            recovery_events=[],
        )
...
def test_v15_import_exposes_no_callable_operation_and_performs_no_lifecycle_work() -> None:
    defined_functions = {
        name
...
def test_v15_production_source_defines_partial_stages_only_inside_script_entry() -> None:
    tree = ast.parse((ROOT / policy.EXECUTOR_RELATIVE_PATH).read_text())
...
def test_v15_no_constructible_stage_evidence_class_is_importable() -> None:
    exposed = {
        name
        for name, value in vars(executor).items()
...
def test_v15_reservation_copy_reconstruction_and_mutation_surface_is_absent() -> None:
    assert not hasattr(executor, "AttemptReservationV15")
    assert not any("reservation" in name.casefold() for name in vars(executor))


def test_v15_completion_writer_is_not_importable() -> None:
...
def test_v15_metric_and_gate_writers_are_not_importable(name: str) -> None:
    assert not hasattr(executor, name)


def test_v15_publication_rejects_replaced_claim_directory(tmp_path: Path) -> None:
    operation = SyntheticExecutionV15(tmp_path / "synthetic")
...
def test_v15_shared_generated_direct_child_churn_does_not_abort_claim(
    tmp_path: Path,
) -> None:
    generated = tmp_path / "repo/.generated"
    operation = SyntheticExecutionV15(generated / "exclusive-v15")
    reservation = operation.claim()
...
def test_v15_real_chain_allows_shared_churn_and_blocks_alias_and_owned_churn(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    runtime = _load_nested_executor()
    root = tmp_path / "repo"
    generated = root / ".generated"
    output = generated / "camera-output-v15"
    seed_root = output / "attempts/seed_20260710"
    seed_root.mkdir(parents=True)
    for directory in (output, seed_root.parent, seed_root):
        directory.chmod(0o700)
...
def test_v15_shared_generated_alias_swap_is_rejected(tmp_path: Path) -> None:
    generated = tmp_path / "repo/.generated"
    operation = SyntheticExecutionV15(generated / "exclusive-v15")
    reservation = operation.claim()
    ...
    try:
        ...
        assert failure["retry_authorized"] is False
        assert (moved / "exclusive-v15/attempts/seed_20260710/n5/failed.json").is_file()
    finally:
        operation.close(reservation)


def test_v15_shared_generated_permission_change_is_rejected(tmp_path: Path) -> None:
    generated = tmp_path / "repo/.generated"
    operation = SyntheticExecutionV15(generated / "exclusive-v15")
    reservation = operation.claim()
...
def test_v15_unowned_exclusive_subtree_mutation_is_rejected(tmp_path: Path) -> None:
    operation = SyntheticExecutionV15(tmp_path / "repo/.generated/exclusive-v15")
    reservation = operation.claim()
...
def test_v15_unowned_claim_child_is_rejected_and_terminalized(tmp_path: Path) -> None:
    operation = SyntheticExecutionV15(tmp_path / "repo/.generated/exclusive-v15")
    reservation = operation.claim()
...
def test_v15_review_preflight_rejects_canonical_leaf_alias(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_v15_review_preflight_accepts_only_the_regular_canonical_entry(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    temporary_root = tmp_path / "repo"
...
def test_v15_review_preflight_rejects_every_nested_binding_schema_attack_before_open(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    mutation: str,
    role: str,
...
def test_v15_wrong_digest_without_enclosing_rehash_rejects_before_candidate_open(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    role: str,
) -> None:
    ...
    selected[paths[0]]["file_sha256"] = ("f" if role == "source" else "e") * 64
    # Deliberately retain the valid review's old content_sha256. The caller file
    # hash below is current, so the stale enclosing content hash is the failure.
    raw = policy.canonical_json_bytes(value) + b"\n"
    digest = hashlib.sha256(raw).hexdigest()
    canonical = temporary_root / policy.SOURCE_REVIEW_RELATIVE_PATH
    canonical.parent.mkdir(parents=True, exist_ok=True)
    ...
    monkeypatch.setattr(policy, "ROOT", temporary_root)
    monkeypatch.setattr(policy, "CANONICAL_SOURCE_REVIEW_PATH", canonical)
    real_read = policy.read_regular_bytes
    candidate_paths = {
        temporary_root / relative
        for relative in (*policy.SUCCESSOR_SOURCE_PATHS, *policy.SUCCESSOR_PROOF_PATHS)
    }
    review_opens: list[Path] = []
    candidate_opens: list[Path] = []
    forbidden_opens: list[Path] = []

    def counted_read(path: Path, *, name: str) -> bytes:
        if path == canonical:
            review_opens.append(path)
        elif path in candidate_paths:
            candidate_opens.append(path)
        else:
            forbidden_opens.append(path)
        return real_read(path, name=name)

    monkeypatch.setattr(policy, "read_regular_bytes", counted_read)

    with pytest.raises(ValueError, match="content SHA-256 changed"):
        policy.preflight_source_review(canonical, digest)
    assert review_opens == [canonical]
...
def test_v15_self_consistent_wrong_digest_reads_exact_frozen_prefix(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    role: str,
    position: str,
    ...
    index = {
        ...
        "final": len(relatives) - 1,
    }[position]
    selected[relatives[index]]["file_sha256"] = (
        "f" if role == "source" else "e"
    ) * 64
    value = _rehash(value)
    raw = policy.canonical_json_bytes(value) + b"\n"
    digest = hashlib.sha256(raw).hexdigest()
    ...
    monkeypatch.setattr(policy, "CANONICAL_SOURCE_REVIEW_PATH", canonical)
    real_read = policy.read_regular_bytes
    frozen_sources = [temporary_root / item for item in policy.SUCCESSOR_SOURCE_PATHS]
    frozen_proofs = [temporary_root / item for item in policy.SUCCESSOR_PROOF_PATHS]
    frozen_candidates = set((*frozen_sources, *frozen_proofs))
    opened_candidates: list[Path] = []
    forbidden_opens: list[Path] = []
    ...
    def counted_candidate_read(path: Path, *, name: str) -> bytes:
        if path in frozen_candidates:
    ...
    monkeypatch.setattr(policy, "load_hashed_json", in_memory_review)
    monkeypatch.setattr(policy, "read_regular_bytes", counted_candidate_read)

    with pytest.raises(PermissionError, match=f"{role} changed"):
        policy.preflight_source_review(canonical, digest)
    expected_prefix = (
        frozen_sources[: index + 1]
        if role == "source"
        else [*frozen_sources, *frozen_proofs[: index + 1]]
    )
    assert opened_candidates == expected_prefix
    assert forbidden_opens == []


def test_v15_self_consistent_changed_authority_core_opens_no_candidate(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    value = _temporary_review_value(tmp_path / "review-targets")
    value["exact_attempt_authorized"] = False
    value = _rehash(value)
    raw = policy.canonical_json_bytes(value) + b"\n"
    digest = hashlib.sha256(raw).hexdigest()
    opened: list[Path] = []
    ...
    monkeypatch.setattr(
        policy,
        "load_hashed_json",
        lambda *_args, **_kwargs: (value, raw),
    )
    monkeypatch.setattr(
        policy,
        "read_regular_bytes",
        lambda path, *, name: opened.append(path) or b"",
    )

    with pytest.raises(PermissionError, match="review contract changed"):
        policy.preflight_source_review(policy.CANONICAL_SOURCE_REVIEW_PATH, digest)
    assert opened == []
...
def test_v15_review_preflight_rejects_ineligible_authors(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    reviewer: str,
) -> None:
...
def test_v15_source_reader_rejects_parent_identity_change(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    root = tmp_path / "root"
...
def test_v15_source_reader_walks_from_filesystem_root_without_absolute_repo_open(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
def test_v15_source_reader_rejects_transient_root_component_alias_before_any_read(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    root = tmp_path / "canonical-root"
...
def test_v15_source_reader_rejects_restored_transient_ancestor_alias(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    anchor = tmp_path / "v15-root-anchor"
    root = anchor / "repo"
    ...
    def replace_ancestor_during_component_open(
        ...
        *,
        ...
        if not replaced and path == anchor.name and dir_fd is not None:
            replaced = True
            moved = tmp_path / "v15-moved-root-anchor"
            anchor.rename(moved)
...
def test_v15_source_reader_rejects_same_inode_parent_metadata_change(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    root = tmp_path / "root"
...
def test_v15_source_reader_rejects_symlink_hardlink_and_fifo(tmp_path: Path) -> None:
    root = tmp_path / "root"
    root.mkdir()
...
def test_v15_synthetic_operation_rejects_production_namespaces(unsafe: Path) -> None:
    with pytest.raises(PermissionError, match="production|repository"):
        SyntheticExecutionV15(unsafe)


def test_v15_synthetic_recovery_is_single_use_and_descriptor_bound(tmp_path: Path) -> None:
    operation = SyntheticExecutionV15(tmp_path / "safe")
...
def test_v15_synthetic_existing_scaffold_without_staging_blocks_claim(
    tmp_path: Path,
) -> None:
    operation = SyntheticExecutionV15(tmp_path / "safe")
...
def test_v15_synthetic_multiple_equivalent_stagings_resume_one(
    tmp_path: Path,
) -> None:
    operation = SyntheticExecutionV15(tmp_path / "safe")
...
def test_v15_synthetic_foreign_staging_is_preserved_and_blocks_claim(
    tmp_path: Path,
) -> None:
    operation = SyntheticExecutionV15(tmp_path / "safe")
    complete = operation.prepare_complete_staging()
    foreign = operation.attempt.parent / ".n5.synthetic-v15-000-foreign"
...
def test_v15_synthetic_historical_foreign_inventory_blocks_claim(
    tmp_path: Path,
) -> None:
    operation = SyntheticExecutionV15(tmp_path / "safe")
...
def test_v15_success_rejects_replaced_output_ancestor_but_fd_failure_terminalizes(
    tmp_path: Path,
) -> None:
    operation = SyntheticExecutionV15(tmp_path / "safe")
    reservation = operation.claim()
...
def test_v15_post_training_failure_removes_only_owned_partials_and_is_no_retry(
    tmp_path: Path,
    stage: str,
    derived_names: tuple[str, ...],
) -> None:
    operation = SyntheticExecutionV15(tmp_path / stage)
    reservation = operation.claim()
    try:
        ...
        with pytest.raises(FileExistsError, match="already claimed"):
            SyntheticExecutionV15(operation.root).claim()
    finally:
        operation.close(reservation)


def test_v15_terminalization_preserves_mutated_artifacts_as_invalid(
    tmp_path: Path,
) -> None:
    operation = SyntheticExecutionV15(tmp_path / "safe")
    reservation = operation.claim()
...
def test_v15_concurrent_synthetic_claims_choose_exactly_one(tmp_path: Path) -> None:
    operations = [SyntheticExecutionV15(tmp_path / "safe") for _ in range(2)]

    def claim(operation: SyntheticExecutionV15) -> object:
        try:
    ...
    SyntheticExecutionV15.close(reservations[0])  # type: ignore[arg-type]


def test_v15_preclaim_cleanup_and_postclaim_failure_are_durable(tmp_path: Path) -> None:
    before = SyntheticExecutionV15(tmp_path / "before")
    ...
    after = SyntheticExecutionV15(tmp_path / "after")
    ...
    with pytest.raises(FileExistsError, match="already claimed"):
        SyntheticExecutionV15(after.root).claim()


def test_v15_retained_source_output_and_claim_chains_are_present_in_production() -> None:
    tree = ast.parse((ROOT / policy.EXECUTOR_RELATIVE_PATH).read_text())
    functions = {
        node.name: ast.unparse(node)
        for node in ast.walk(tree)
...
def test_v15_post_training_exception_terminalizes_before_all_descriptors_close() -> None:
    tree = ast.parse((ROOT / policy.EXECUTOR_RELATIVE_PATH).read_text())
    functions = {
        node.name: ast.unparse(node)
        for node in ast.walk(tree)
...
def test_v15_exact_entry_rejects_a_claim_but_preserves_preclaim_recovery() -> None:
    tree = ast.parse((ROOT / policy.EXECUTOR_RELATIVE_PATH).read_text())
    functions = {
        node.name: ast.unparse(node)
        for node in ast.walk(tree)
    ...
    execute = functions["execute_exact"]
    freshness = functions["_preflight_exact_output_freshness"]
    reservation = functions["_reservation_core"]
    assert execute.index("gpu_visibility.validate_fixed_receipt") < execute.index(
        "_preflight_exact_output_freshness()"
    )
    assert execute.index("gpu_visibility.require_passing_observation") < execute.index(
        "_preflight_exact_output_freshness()"
    )
    assert "os.lstat(path)" in freshness
    assert "policy.CANONICAL_OUTPUT_ROOT" in freshness
    assert "policy.CANONICAL_ATTEMPT_PATH" in freshness
    assert "not fresh" in freshness
    assert "gpu_visibility" in reservation
...
def test_v15_generated_mutator_probe_distinguishes_writable_descriptors(
    tmp_path: Path,
) -> None:
    runtime = _load_nested_executor()
    proc = tmp_path / "proc"
    descriptor = proc / "4321/fd/7"
    descriptor.parent.mkdir(parents=True)
    fdinfo = proc / "4321/fdinfo/7"
    fdinfo.parent.mkdir(parents=True)
    fake_generated = tmp_path / "repo/.generated"
    descriptor.symlink_to(fake_generated / "active-output.json")
    fdinfo.write_text("pos:\t0\nflags:\t0100002\n", encoding="ascii")
    try:
        assert runtime._active_generated_mutator_pids(
            proc_root=proc,
            current_pid=9999,
            generated_root=fake_generated,
        ) == [4321]
        fdinfo.write_text("pos:\t0\nflags:\t0100000\n", encoding="ascii")
        assert runtime._active_generated_mutator_pids(
            proc_root=proc,
...
def test_v15_frozen_science_and_gpu0_launcher_are_unchanged() -> None:
    from lewm.benchmarks import (
...
def test_v15_review_contract_binds_end_to_end_sources_and_v4_block_closures() -> None:
    sources = {
    ...
    assert core["scientific_successor_authorized"] is False
    assert core["runtime_visibility_successor_authorized"] is True
    ...
    assert core["v14_terminal_source_review_inspected"] is True
    assert core["v14_terminal_lifecycle_evidence_inspected"] is True
    assert core["v14_numeric_payload_inspected"] is False
    assert core["v14_checkpoint_inspected"] is False
    assert core["retained_v14_source_commit"] == policy.V14_SOURCE_COMMIT
    assert core["terminal_v14_proof_clarification"] == {
        "path": policy.V15_TERMINAL_V14_PROOF_CLARIFICATION_RELATIVE_PATH,
        "file_sha256": (
            policy.V15_TERMINAL_V14_PROOF_CLARIFICATION_FILE_SHA256
        ),
    ...
    assert core["retained_v14_sources_and_proofs"] == dict(
        policy.V14_RETAINED_SOURCE_AND_PROOF_BINDINGS
    )
    assert core["v14_passing_source_review"]["reviewer"] == policy.V14_REVIEWER
    assert core["v14_terminal_runtime_failure"] == policy.authority_bindings()[
        "v14_consumed_terminal_runtime_failure"
    ]
    assert core["v13_terminal_source_review_inspected"] is True
    assert core["v13_checkpoint_inspected"] is False
    assert core["retained_v13_amendment"] == {
        "path": policy.V13_AMENDMENT_RELATIVE_PATH,
        "file_sha256": policy.V13_AMENDMENT_FILE_SHA256,
    }
    assert core["retained_v13_sources_and_proofs"] == dict(
        policy.V13_RETAINED_SOURCE_AND_PROOF_BINDINGS
    )
    assert core["v13_terminal_source_review_block"] == (
        policy.authority_bindings()["v13_terminal_source_review_block"]
    )
    assert core["v13_terminal_source_review_block"]["status"] == (
    ...
    assert core["authority_boundary"]["v13_checkpoint_input_authorized"] is False
    assert core["authority_boundary"]["v14_checkpoint_input_authorized"] is False
    assert core["authority_boundary"]["v14_retry_authorized"] is False
    assert core["gpu_visibility_contract"] == policy.gpu_visibility_contract()
...
def _synthetic_evaluation(loss_name: str) -> dict[str, Any]:
    value = complete_retained_gate_evaluation_v15()
...
def test_v15_evaluation_accepts_only_the_new_loss_name() -> None:
...
def test_v15_training_record_is_4000_updates_and_final_only() -> None:
...
def test_v15_full_result_schema_accepts_only_new_loss_and_schedule() -> None:
    review = {
        "path": policy.SOURCE_REVIEW_RELATIVE_PATH,
        "file_sha256": "a" * 64,
        "content_sha256": "b" * 64,
    ...
    core = {
        ...
        "mode": "exact_train_only_n5_gate_aligned_raster_nll_v15_development_fit",
        "authoritative": False,
...
def test_v15_changed_predecessor_bytes_fail_closed(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    relative = policy.V15_AMENDMENT_RELATIVE_PATH
...
def test_v15_static_preflight_consumes_v14_terminal_and_retained_bindings() -> None:
    source = ast.unparse(
        next(
            node
...
def test_v15_normalized_ast_delta_from_v14_is_exactly_runtime_visibility() -> None:
    production_pairs = (
        (
            "scripts/train_go2_observable_camera_ray_fit_v4_n5_"
            "gate_aligned_raster_nll_v14.py",
        ...
        (
            ...
            policy.EXECUTOR_RELATIVE_PATH,
            {
                ...
                "_reserve_exact_attempt",
                "_run_frozen_training",
                "_update_complete_staging",
                "_validate_recoverable_reservation",
                "_verification_child_environment",
                "dispatch",
                "execute_exact",
                "parse_args",
                "reserve",
            ...
            {
                "_gpu_visibility_reservation_binding",
                "_active_generated_mutator_pids",
        ...
        (
            ...
            policy.SYNTHETIC_RELATIVE_PATH,
            set(),
            set(),
        ),
        (
            ...
            "gate_aligned_raster_nll_v14.py",
            policy.LOSS_TEST_RELATIVE_PATH,
            {"test_v14_sources_forbid_v11_checkpoint_input_and_exact_root_is_absent"},
            set(),
        ),
    )
    for v14_relative, v15_relative, expected_changed, expected_added in production_pairs:
        v14_tree = ast.parse((ROOT / v14_relative).read_text())
        normalized_v15 = (
            (ROOT / v15_relative)
            .read_text()
            .replace("V15", "V14")
        ...
        v15_tree = ast.parse(normalized_v15)

        def all_function_asts(tree: ast.Module) -> dict[str, list[str]]:
            values: dict[str, list[str]] = {}
            for node in ast.walk(tree):
                if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
                    values.setdefault(node.name, []).append(
                        ast.dump(node, include_attributes=False)
                    )
            return values

        v14_functions = all_function_asts(v14_tree)
        v15_functions = all_function_asts(v15_tree)
        assert set(v15_functions) - set(v14_functions) == expected_added
        assert not (set(v14_functions) - set(v15_functions))
        assert {
            name
            for name in set(v14_functions) & set(v15_functions)
            if v14_functions[name] != v15_functions[name]
        } == expected_changed

    v14_policy = ast.parse(
        (
            ROOT
    ...
    normalized_v15_policy = ast.parse(
    ...
    v14_functions = function_asts(v14_policy)
    v15_functions = function_asts(normalized_v15_policy)
    assert set(v15_functions) - set(v14_functions) == {
        "gpu_visibility_contract",
        "preflight_v14_source_authority_documents",
        "validate_gpu_visibility_reservation_binding",
    }
    assert not (set(v14_functions) - set(v15_functions))
    permitted_runtime_deltas = {
        "authority_bindings",
...
def test_v15_nested_binding_validator_has_no_permissive_value_access() -> None:
...
def test_v15_preflight_core_precedes_fixed_tuple_candidate_opens() -> None:
    tree = ast.parse((ROOT / policy.POLICY_RELATIVE_PATH).read_text())
    preflight = next(
        node
        for node in tree.body
        if isinstance(node, ast.FunctionDef) and node.name == "preflight_source_review"
    )
    source = ast.unparse(preflight)
    assert source.count("read_regular_bytes(ROOT / relative") == 2
    assert "read_regular_bytes(binding['path']" not in source
    assert "read_regular_bytes(Path(binding['path'])" not in source
    assert source.index("core != expected") < source.index(
        "read_regular_bytes(ROOT / relative"
    )


def test_v15_preserves_metric_accumulation_wrong_rgb_and_all_26_thresholds() -> None:
...
def test_v15_adapter_reproduces_failure_is_nonmutating_and_changes_only_keys() -> None:
    from lewm.benchmarks import (
        go2_observable_camera_ray_fit_v4_ladder_gate as frozen,
    )

    evaluation = complete_retained_gate_evaluation_v15()
    ...
    adapted = policy.adapt_native_v15_evaluation_for_retained_v4_gate(
        evaluation
    )
...
def test_v15_adapter_negative_cases_fail_closed(mutation: str) -> None:
    value = complete_retained_gate_evaluation_v15()
    ...
    if mutation == "missing_hierarchical":
        ...
        value["extra_control"] = {}
    with pytest.raises((PermissionError, ValueError)):
        policy.adapt_native_v15_evaluation_for_retained_v4_gate(value)


def test_v15_shared_reconstruction_is_actual_26_check_gate_and_schema_stays_native(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    evaluation = complete_retained_gate_evaluation_v15()
    original = copy.deepcopy(evaluation)
    real_adapter = policy.adapt_native_v15_evaluation_for_retained_v4_gate
    calls = 0
    ...
    malformed = complete_retained_gate_evaluation_v15()
...
def test_v15_adapter_cannot_change_a_passing_or_failing_gate_decision() -> None:
    from lewm.benchmarks import (
        go2_observable_camera_ray_fit_v4_ladder_gate as frozen,
    )
...
def test_v15_raw_retained_gate_bypasses_exist_only_in_shared_policy_helper() -> None:
...
def test_v15_child_parent_and_finalizer_each_runtime_invoke_one_adapter(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    from scripts import (
    ...
    from scripts import (
        verify_go2_observable_camera_ray_fit_v4_n5_gate_aligned_raster_nll_v15
        as verifier,
    )
