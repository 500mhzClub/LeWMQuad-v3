...
from lewm.visibility_batched_footprint_controller_development import (
    VisibilityBatchedFootprintController, install_empty_visibility_patches, normalize_to_receipt_copied)
...
def controllers(geometry=None):
    ...
    return [kind(None,geometry,**kwargs()) for kind in (ReceiptCopiedFootprintController,VisibilityBatchedFootprintController)]
...
def test_only_two_empty_patch_stores_change_and_memory_aliases_remain():
    ...
    for field in ('patches','auxiliary_patches'):
        assert type(getattr(old.memory,field)) is BatchedRetainedFloorPatches
        assert type(getattr(new.memory,field)) is VisibilityBatchedRetainedFloorPatches
...
def test_incompatible_storage_is_rejected_before_any_replacement(fault):
    ...
    if fault=='custom':m.auxiliary_patches=type('Custom',(BatchedRetainedFloorPatches,),{})()
    if fault=='mixed':m.auxiliary_patches=VisibilityBatchedRetainedFloorPatches()
    before=(m.patches,m.auxiliary_patches)
    with pytest.raises(ValueError):install_empty_visibility_patches(m)
...
def test_state_normalization_cannot_hide_changed_witness_or_pixel_bytes():
    ...
    assert fingerprint(state(old))!=fingerprint(state(new))
    new.memory.auxiliary_patches=BatchedRetainedFloorPatches()
...
test_actual_public_packets_maps_and_robot_footprints=FunctionType(_function.__code__,
    _function.__globals__|dict(controllers=controllers,state=state,normalize_to_scoped=normalize_to_receipt_copied,
...
def test_original_failure_stop_and_metadata_are_retained():
    ...
    assert normalize_to_receipt_copied(actual)==expected and actual['terminal']=='SENSOR_OR_MODEL_FAILURE'
    with pytest.raises(ValueError):normalize_to_receipt_copied(expected)
