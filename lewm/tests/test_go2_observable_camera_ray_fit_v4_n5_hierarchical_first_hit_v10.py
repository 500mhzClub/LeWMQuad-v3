"""CPU-only author and adversarial tests for Camera-ray N5 V10.
...
def test_v10_hierarchical_first_hit_matches_hand_computation() -> None:
...
def test_v10_presence_does_not_retain_old_one_over_groups_weight() -> None:
    breakdown = _direct_breakdown(
        hit_probabilities=[
...
def test_v10_presence_and_conditional_depth_invariances() -> None:
...
def test_v10_extreme_logits_have_finite_gradients_for_every_state_and_bin() -> None:
...
def test_v10_plus_minus_ten_thousand_logits_remain_finite() -> None:
    logits = torch.tensor(
        [
...
def test_v10_empty_groups_keep_a_finite_zero_gradient_term(
...
def test_v10_rejects_unnormalized_or_nonfinite_log_probabilities() -> None:
...
def test_v10_frozen_full_panel_schedule_is_exact_and_deterministic() -> None:
...
def test_v10_trainer_and_verifier_preserve_all_non_first_hit_branches(
...
def test_v10_standalone_verifier_is_compute_only() -> None:
    from scripts import (
        verify_go2_observable_camera_ray_fit_v4_n5_hierarchical_first_hit_v10
        as verifier,
    )
