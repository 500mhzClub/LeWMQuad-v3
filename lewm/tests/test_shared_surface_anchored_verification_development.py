...
import sys
import pytest
from scripts import verify_go2_shared_surface_anchored_prefix_v1 as verifier
...
def reports():
    ...
    result = dict(status='SHARED_SURFACE_ANCHORED_PREFIX_V1_COMPLETE', native_execution=False,
    ...
    launch = dict(preceding_result_sha256=replay.PRECEDING_SHA, profile_result_sha256=replay.preceding.PROFILE_SHA,
        ...
        invocation_local_surface_receipt_sharing=True, normalized_state_type_paths=[], source_sha256={})
...
def test_missing_final_result_rejects_before_reading_any_packets(monkeypatch, tmp_path):
    ...
    with pytest.raises(ValueError, match='missing final'): verifier.verified_result('f'*64)
...
def test_main_binds_its_sources_before_and_after_result_checks(monkeypatch, tmp_path, changed_sources):
    ...
    monkeypatch.setattr(sys, 'argv', [verifier.SOURCE, '--result-sha256', 'f'*64])
