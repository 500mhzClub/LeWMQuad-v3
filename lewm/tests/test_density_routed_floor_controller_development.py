...
import numpy as np
import pytest
from lewm import density_routed_floor_controller_development as routed
...
from scripts.visibility_batched_footprint_state_development import normalized_state_tree
...
def controllers(geometry=None):
    ...
    return [kind(None,geometry,**kwargs()) for kind in (VisibilityBatchedFootprintController,routed.DensityRoutedFloorController)]
...
def test_only_private_floor_kernel_bindings_change():
    pairs=((FrameFloorIndexCache.index,routed.DensityRoutedRecordingFloorGeometry.index,'observed_floor_cell_index'),
           (LaterResolvedFloorMap.observe,routed.DensityRoutedFloorMap.observe,'RecordingFloorGeometry'))
...
def test_constructor_retains_all_existing_packed_memory_and_patch_fields():
    old,new=controllers()
    assert type(old.mapper) is MeasuredFloorTransportMap
    assert type(new.mapper) is routed.DensityRoutedFloorMap
...
def test_live_or_incompatible_consumer_cannot_be_replaced(fault):
    ...
    with pytest.raises(ValueError):routed.install_fresh_routed_consumers(old)
...
def test_cache_keeps_exact_key_readonly_bytes_and_closed_lifetime():
    ...
    context=routed.DensityRoutedRecordingFloorGeometry({}, {},0,1_500_000_000)
...
test_actual_public_packets_registration_map_cache_and_robot_footprints=FunctionType(_function.__code__,
    _function.__globals__|dict(controllers=controllers,state=state,
        normalize_to_scoped=routed.normalize_to_visibility_batched,
        BatchedRetainedFloorPatches=VisibilityBatchedRetainedFloorPatches),
...
def test_original_sensor_failure_and_metadata_retained():
    ...
    assert routed.normalize_to_visibility_batched(actual)==expected
    assert actual['terminal']=='SENSOR_OR_MODEL_FAILURE'
    with pytest.raises(ValueError):routed.normalize_to_visibility_batched(expected)
