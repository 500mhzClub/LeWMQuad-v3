...
def evidence(monkeypatch,inputs,queue):
    ...
    monkeypatch.setattr(final,'diagnostic_readout',diagnostic)
    return final.review_evidence(inputs,queue)


def review(inputs,queue,proof):
    return dict(schema='independent_round_trip_final_policy_review.v1',
        ...
        input_admission_sha256=final.fingerprint(inputs),five_stage_queue_admission_sha256=final.fingerprint(queue),
...
def test_all_negative_results_remain_valid_review_evidence(monkeypatch):
    ...
    assert len(proof['original_three_diagnostics'])==3
    assert proof['tracking']['readout']['verified_round_trip'] is False
    assert proof['extended_budget']['prefix_comparison']['budget_only_preboundary_execution_supported'] is False
    assert (inp,q)==before
...
def test_mixed_or_incomplete_diagnostic_chain_rejected(monkeypatch,fault):
    ...
    elif fault=='stage':q['original_four_stage_admission']['original_queue_admission']['completed'].pop()
    elif fault=='tracking':q['original_four_stage_admission']['tracking_wait_result_sha256']='f'*64
    else:q['budget_inputs'].pop('hold')
...
def test_review_cannot_substitute_policy_outcomes_or_qualification(monkeypatch,fault):
    ...
    elif fault=='queue':r['five_stage_queue_admission_sha256']='f'*64
    elif fault=='outcome':r['development_evidence']['six_case_development_summary']['measured_round_trip_successes']=1
    elif fault=='prefix':r['development_evidence']['extended_budget']['prefix_comparison']['budget_only_preboundary_execution_supported']=True
...
def test_full_queue_reconstruction_chains_all_actual_admission_outputs(monkeypatch):
    _,q=admissions();calls=[];first={'first':'receipt'};second={'second':'receipt'}
    def three(ids,**kwargs):calls.append(('three',ids,kwargs));return first
    def four(old,sha,**kwargs):assert old is first;calls.append(('four',sha,kwargs));return second
    def five(old,sha,**kwargs):assert old is second;calls.append(('five',sha,kwargs));return q
    monkeypatch.setattr(final.queue.original.original,'admit',three)
    monkeypatch.setattr(final.queue.original,'admit',four)
    monkeypatch.setattr(final.queue,'admit',five)
    assert final.reconstruct_full_queue(q,{}) is q
    assert [r[0] for r in calls]==['three','four','five']
    assert all(r[-1]['full'] is True for r in calls)
...
def prepared_launch(monkeypatch,tmp_path):
    ...
    launch=dict(source_sha256=sources,runtime_verifier=dict(source=final.SOURCE,function='verify_population'),
        ...
        five_stage_queue_admission=q,final_policy_review_completed=True,
