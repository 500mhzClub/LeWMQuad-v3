"""Distinct joint-mode transcription of the frozen continuity checker.
...
causes. Only the accepted mode/gyro-role checks differ from the frozen checker.
The separate joint rotation checker verifies the additional matrix witnesses.
"""
...
def verify_joint_history(rows, saved_summary):
    """Fixed challenge clocks and at most443 rows; retain every terminal row.

    This history checker reconstructs position disagreement and range-checks
    rotation disagreement. The separate joint rotation checker consumes the
    additional matrix witnesses to independently reconstruct that angle.
    """
    ...
    for row in rows:
        ...
        require(isinstance(e, dict) and e['status'] in modes and pose['frame'] == frame
            ...
            and pose['navigation_qualified'] is False and pose['mode'] == 'joint'
            and pose['gyro_role'] == 'consistency_monitor_only', 'same-frame unpromoted joint-RGB-D pose required')
    ...
    return dict(available_bridge_and_rejoin_history_verified=True, summary=summary,
        ...
        incremental_rotation_witness_checked_by_this_function=False, rotation_disagreements_independently_recomputed=False,
