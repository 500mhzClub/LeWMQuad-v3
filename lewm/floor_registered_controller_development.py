"""One registered pose for mapping, residuals, mission tracking and contact checks.

Three narrow method derivatives retain predecessor behavior except the explicit
pose accessor. Old modules and their witnesses are never patched in place.
"""
...
from lewm.floor_registered_evidence_development import FloorRegistration, current_registered_pose


class FloorRegisteredSurfaceMemory(ConfirmedAuxiliaryFloorMemory):
    def observe(self, policy, depth, evidence, *, now_ns):
        ...
        try:
            ...
            p, R, pose = current_registered_pose(evidence, identity=self.identity, now_ns=now_ns)
...
class FloorRegisteredMap(ConfirmedAuxiliaryFloorMap):
    def __init__(self, *, identity=(0, 0, 0)):
        super().__init__(identity=identity)
        self.surface = FloorRegisteredSurfaceMemory(identity=identity)


class FloorRegisteredResidual(OnlineExecutedResidual):
    def observe(self, evidence, *, now_ns):
        p, R, pose = current_registered_pose(evidence, identity=(0, 0, 0), now_ns=now_ns)
        frame = pose['frame']
...
class FloorRegisteredRoundTripController(ConfirmedFloorRoundTripController):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.registration = FloorRegistration(identity=(0, 0, 0))
        self.mapper = FloorRegisteredMap(identity=(0, 0, 0))
        self.memory = self.mapper.surface
        self.residual = FloorRegisteredResidual()
    ...
    def advance(self, policy, evidence, *, now_ns):
        ...
        try:
            ...
            p, _, pose = current_registered_pose(evidence, identity=(0, 0, 0), now_ns=now_ns)
    ...
    def _result(self, command, selection, distance):
        return super()._result(command, selection, distance) | dict(
            controller='floor_registered_round_trip_controller_v1',
            floor_registration_enabled=True, registered_pose_used_by_all_online_pose_consumers=True,
            original_visual_pose_retained=True, observation_grounded_static_floor_hypothesis=True,
