...
def specification(trial):
    ...
    return base|dict(scene_id='intent-room-return-v1-'+trial,family='CONTROLLED_ROOM_RETURN',trial=trial,
                     condition=condition,turn_sign=-1 if right else 1,friction_mu=.15 if condition=='lower_friction' else 1.,
                     procedural_seed=2026090701+int(right),appearance_seed=2026090705+int(right),
                     geometry=base['geometry']|dict(spawn_se2_world=[-.05,.35,-.065] if right else [.05,-.38,.055],wall_boxes=walls))
