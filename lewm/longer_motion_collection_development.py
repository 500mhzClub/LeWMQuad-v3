...
DEFINITIONS = {
    'fit': dict(physics_seed=2026090633, appearance_seed=2026090635, spawn=(-.5, -.3, 0.)),
    'validation': dict(physics_seed=2026090634, appearance_seed=2026090636, spawn=(-.5, .3, .04)),
}
SEGMENTS = (
    ('initial_weak_observation', 10, (0., 0., 0.)),
    ('sustained_forward', 400, (.12, 0., 0.)),
...
def specification(trial):
    ...
    return spec | dict(scene_id='longer-observed-floor-v1-'+trial,
        family='SUPERVISED_LONGER_MOTION_DEVELOPMENT', trial=trial,
...
def pack(spec):
    ...
    if trial not in TRIALS or spec != specification(trial):
        raise ValueError('exact frozen longer-motion specification required')
