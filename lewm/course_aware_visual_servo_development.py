"""Course-aware engineering successor; same full local target sequence."""
import math
import numpy as np
    ...
    def step(self,evidence,*,now_ns):
        ...
        try:
            ...
            if (now-self.start_ns)//100_000_000>=350: self._fail('TIME_LIMIT')
                ...
                if abs(yaw_error)<=.03: self.stage='final_brake'; self.brake_ticks=self.quiet=0
                else: command=[0.,0.,float(np.clip(1.5*yaw_error,-.25,.25))]
            else:
                self.brake_ticks+=1; self.quiet=self.quiet+1 if speed is not None and speed<=.02 and rate<=.05 else 0
                if self.quiet>=10:
