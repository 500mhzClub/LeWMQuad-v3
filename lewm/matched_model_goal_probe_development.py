...
All six family models remain unqualified for arbitrary moving action switches.
...
class MatchedModelGoalProbe:
    def __init__(self, model, geometry, *, condition, variant, persistent):
        ...
        self.selector = MatchedModelWaypointSelector(condition=condition, variant=variant)
    ...
    def _result(self, command, selection, distance):
        return dict(controller='matched_model_goal_probe_v1', planner_mode=self.selector.mode,
            model_condition=self.selector.condition, input_variant=self.selector.variant,
