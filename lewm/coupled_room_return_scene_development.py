...
def specification(trial):
    ...
    return base|dict(scene_id='coupled-room-return-v1-'+trial,family='CONTROLLED_ROOM_RETURN',trial=trial,
                     condition=condition,turn_sign=-1 if right else 1,friction_mu=.15 if condition=='lower_friction' else 1.,
                     procedural_seed=2026090691+int(right),appearance_seed=2026090695+int(right),
                     geometry=base['geometry']|dict(spawn_se2_world=[-.12,.28,-.055] if right else [-.18,-.32,.045],wall_boxes=walls))
