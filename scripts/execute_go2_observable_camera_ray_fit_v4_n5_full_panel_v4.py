#!/usr/bin/env python3
"""Authority-free, one-process V4 executor for the frozen N5 experiment.
...
if __name__ == "__main__":
    ...
    STAGING_PREFIX = ".n5.reservation-v4-"
    LEGACY_STAGING_NAME = ".n5.reservation-staging"
    V2_STAGING_PREFIX = ".n5.reservation-v2-"
    LOCK_NAME = ".n5.reservation-v4.lock"
    STAGING_SCHEMA = "lewm_go2_n5_full_panel_v4_preclaim_staging_v1"
    
    
    @dataclass(frozen=True)
    class AttemptReservationV4:
        directory: Path
    ...
    def _assert_owned_claim(reservation: AttemptReservationV4) -> None:
        """Bind every post-claim stage to the still-canonical open directory."""

        if reservation.directory_fd < 0 or reservation.directory_identity is None:
            raise PermissionError("N5 full-panel V4 lacks an open claimed directory")
        opened = os.fstat(reservation.directory_fd)
        if not stat.S_ISDIR(opened.st_mode):
            raise PermissionError("N5 full-panel V4 claimed descriptor is not a directory")
        if (opened.st_dev, opened.st_ino) != reservation.directory_identity:
            raise PermissionError("N5 full-panel V4 claimed descriptor identity changed")
        canonical = os.stat(reservation.directory, follow_symlinks=False)
        if (
            not stat.S_ISDIR(canonical.st_mode)
            or (canonical.st_dev, canonical.st_ino) != reservation.directory_identity
        ):
            raise PermissionError("N5 full-panel V4 canonical claim identity changed")


    def _write_claim_file_exclusive(
        reservation: AttemptReservationV4,
        name: str,
        payload: bytes,
        *,
        mode: int = 0o600,
        ...
        if Path(name).name != name or name in {"", ".", ".."}:
            raise PermissionError("N5 full-panel V4 claim filename escaped")
        ...
        try:
            ...
            if not stat.S_ISREG(metadata.st_mode) or metadata.st_nlink != 1:
                raise PermissionError("N5 full-panel V4 claim leaf is not singly linked")
            with os.fdopen(descriptor, "wb", closefd=True) as stream:
                descriptor = -1
                stream.write(payload)
                stream.flush()
    ...
    def _read_claim_file(reservation: AttemptReservationV4, name: str) -> bytes:
        if Path(name).name != name or name in {"", ".", ".."}:
            raise PermissionError("N5 full-panel V4 claim filename escaped")
        _assert_owned_claim(reservation)
        descriptor = os.open(
        ...
        try:
            ...
            if fingerprint(before) != fingerprint(after):
                raise RuntimeError("N5 full-panel V4 claim leaf changed while read")
        finally:
            os.close(descriptor)
        _assert_owned_claim(reservation)
    ...
    def _classify_staging(
        ...
        *,
        attempt_path: Path,
        review_binding: Mapping[str, str],
    ) -> tuple[dict[str, Any], AttemptReservationV4 | None]:
        ...
        return evidence, AttemptReservationV4(
            directory=attempt_path,
    ...
    def _new_staging(seed_root: Path, attempt_name: str) -> Path:
        for _ in range(128):
            staging = seed_root / f".{attempt_name}.reservation-v4-{secrets.token_hex(16)}"
            try:
    ...
    def _reserve_exact_attempt(source_review_file_sha256: str) -> AttemptReservationV4:
        ...
        claimed_reservation: AttemptReservationV4 | None = None
        try:
            with _locked_seed_root(seed_root):
                ...
                complete: list[tuple[Path, AttemptReservationV4, dict[str, Any]]] = []
                ...
                if (
                    ...
                    raise PermissionError("N5 full-panel V4 staging identity changed")
                owned_claim_identity = (staging_metadata.st_dev, staging_metadata.st_ino)
                claimed_reservation = AttemptReservationV4(
                    directory=attempt_path,
    ...
    def _terminate_failure(
        ...
        value = {**core, "content_sha256": policy.canonical_json_sha256(core)}
        raw = policy.canonical_json_bytes(value) + b"\n"
        _write_claim_file_exclusive(reservation, "failed.json", raw)
        os.fsync(reservation.directory_fd)
    ...
    def _publish_success(
        ...
        *,
        ...
        if not policy.is_sha256(checkpoint_content_sha256):
            raise ValueError("N5 full-panel V4 checkpoint content hash is malformed")
        ...
        _write_claim_file_exclusive(reservation, "checkpoint.pt", checkpoint_raw)
        _write_claim_file_exclusive(reservation, "result.json", result_raw)
        _write_claim_file_exclusive(reservation, "completed.json", completion_raw)
        os.fsync(reservation.directory_fd)
        _fsync_directory(reservation.directory.parent)
        _assert_owned_claim(reservation)
        return {
    ...
    def _write_canonical_json(
        ...
        if path not in {policy.CANONICAL_METRIC_RECEIPT_PATH, policy.CANONICAL_GATE_PATH}:
            raise PermissionError("N5 full-panel V4 publication path is not canonical")
        expected_schema = (
        ...
        if (
            ...
            raise ValueError("N5 full-panel V4 derived artifact is malformed")
        raw = policy.canonical_json_bytes(value) + b"\n"
        relative = path.relative_to(policy.CANONICAL_OUTPUT_ROOT)
        if len(relative.parts) != 2:
            raise PermissionError("N5 full-panel V4 publication depth changed")
        output_fd = os.open(
        ...
        try:
            try:
                os.mkdir(relative.parts[0], 0o700, dir_fd=output_fd)
            except FileExistsError:
                pass
            parent_fd = os.open(
                relative.parts[0],
            ...
            if not stat.S_ISREG(metadata.st_mode) or metadata.st_nlink != 1:
                raise PermissionError("N5 full-panel V4 derived leaf is not singly linked")
    ...
    def _run_frozen_training(
        ...
        *,
        rgb_workers: int,
    ) -> tuple[dict[str, Any], AttemptReservationV4]:
        ...
        def require(value: object) -> object:
            if value is not token:
                raise PermissionError("retained science lacks its local V4 execution token")
        ...
        def reserve(value: object) -> AttemptReservationV4:
            nonlocal claimed_reservation
            require(value)
            if claimed_reservation is not None:
                raise FileExistsError("the sole V4 attempt was already reserved")
        ...
        if claimed_reservation is None:
            raise RuntimeError("frozen V4 training returned without claiming its attempt")
        _assert_owned_claim(claimed_reservation)
        return {
            **summary,
            "schema": "lewm_go2_observable_camera_ray_fit_v4_n5_full_panel_v4_launch_summary_v1",
        }, claimed_reservation
    ...
    def _run_independent_verification(
        ...
        def require(value: object) -> object:
            if value is not token:
                raise PermissionError("retained verifier lacks its local V4 execution token")
            policy.preflight_static_authority()
            policy.preflight_source_review(
                policy.CANONICAL_SOURCE_REVIEW_PATH,
                source_review_file_sha256,
        ...
        def write_metric(path: Path, value: Mapping[str, Any]) -> dict[str, Any]:
            if Path(path) != policy.CANONICAL_METRIC_RECEIPT_PATH:
                raise PermissionError("retained verifier changed its V4 output path")
    ...
    def _run_finalization(
        ...
        def require(value: object) -> object:
            if value is not token:
                raise PermissionError("retained finalizer lacks its local V4 execution token")
            policy.preflight_static_authority()
            policy.preflight_source_review(
                policy.CANONICAL_SOURCE_REVIEW_PATH,
                source_review_file_sha256,
        ...
        def write_gate(path: Path, value: Mapping[str, Any]) -> dict[str, Any]:
            if Path(path) != policy.CANONICAL_GATE_PATH:
                raise PermissionError("retained finalizer changed its V4 output path")
            return _write_canonical_json(reservation, Path(path), value)
    ...
    def execute_exact(
        ...
        *,
        ...
        if not sys.flags.isolated:
            raise PermissionError("N5 full-panel V4 exact execution requires isolation")
        ...
        try:
            ...
            _assert_owned_claim(reservation)
            return {
                "schema": "lewm_go2_observable_camera_ray_fit_v4_n5_full_panel_v4_end_to_end_summary_v1",
                "seed": 20260710,
                "fit_size": 5,
                "training": training,
    ...
    def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
        ...
        if not policy.is_sha256(args.source_review_sha256):
            raise ValueError("N5 full-panel V4 source review SHA-256 is malformed")
        return args
