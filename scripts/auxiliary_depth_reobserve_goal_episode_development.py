...
from scripts.auxiliary_depth_goal_session_development import AuxiliaryDepthGoalSession
...
def collect(trial, definition, *, output, model, geometry, persistent, episode_name, condition, variant):
    ...
    try:
        session = AuxiliaryDepthGoalSession(spec, directory)
        session._runtime['high_level'] = 'auxiliary_depth_reobserve_goal_probe_v1; online model plus measured surfaces'
        ...
        controller = AuxiliaryDepthReobserveGoalProbe(model, geometry, persistent=persistent, condition=condition, variant=variant)
        try:
        ...
        result = dict(status='AUXILIARY_DEPTH_REOBSERVE_GOAL_PROBE_TERMINAL_AUDIT_REQUIRED', trial=trial,
