...
def collect(trial, definition, *, output, model, geometry, persistent, episode_name, condition, variant):
    ...
    try:
        ...
        session._runtime['high_level'] = 'causal_residual_final_goal_probe_v1; online model plus measured surfaces'
        ...
        controller = CausalResidualFinalGoalProbe(model, geometry, persistent=persistent, condition=condition, variant=variant)
        try:
        ...
        result = dict(status='CAUSAL_RESIDUAL_FINAL_GOAL_PROBE_TERMINAL_AUDIT_REQUIRED', trial=trial,
