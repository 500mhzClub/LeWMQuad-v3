...
from lewm.auxiliary_tilted_depth_geometry_development import CALIBRATION_ID as CAPTURE_CALIBRATION
...
def packet(directory,index,policy,acquisition,*,now_ns):
    if type(index) is not int or not 0<=index<=253:raise ValueError('bounded actual auxiliary frame required')
