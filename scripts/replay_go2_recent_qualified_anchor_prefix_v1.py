"""Prospective retention replay, stopping before any changed-command future."""
import argparse
...
from lewm.partial_floor_height_development import current_partial_height_pose
...
from scripts.run_go2_partial_floor_height_maze01_scoped_verification_pilot_v1 import OUTPUT as INPUT,CASE
from scripts.run_go2_partial_floor_height_maze01_pilot_v1 import verify_inputs as verify_native_context
...
OUTPUT=BASE/'go2_recent_qualified_anchor_prefix_v1_attempt_001'
PROTOCOL='docs/go2_recent_qualified_anchor_prefix_v1_2026-09-09.md'
INPUT_SHA='32edbb748e04e18816e0b0fb265f465706ac8c8684849d73a091321f79a3b07f'
MAX_OUTPUT_BYTES=2*1024**3
MEMORY_BYTES=8*1024**3


def admit_native(result,launch,audit):
    if (result['status']!='PARTIAL_FLOOR_HEIGHT_MAZE01_SCOPED_VERIFICATION_PILOT_V1_COMPLETE'
            or len(result['conditions'])!=1 or launch['planned_case']!=list(CASE)
            or launch['implementation_class']!='PartialHeightDirectFlowController'
            or launch['scene_specification']!=specification(1) or launch['public_mission']!=public_mission(1)
            or launch['verification_benchmark_result_sha256']!=BENCHMARK_SHA
            or launch['prefix_report']['model_state_sha256']!=MODEL_STATE):
        raise ValueError('completed exact height-native input required')
    record=result['conditions'][0]
    if (record['case']!=CASE[0] or record['layout_index']!=1 or audit['layout_index']!=1
            or record['status']!='PARTIAL_FLOOR_HEIGHT_MAZE01_SCOPED_VERIFICATION_COLLECTED_AND_RAW_AUDITED'
            or record['model_state_unchanged'] is not True):
        raise ValueError('complete original maze1 raw audit required')
    require_raw_audit(record,audit,learned=True)
    expected=dict(common_prefix_frames=505,first_intervention_frame=504,physical_prefix_samples=25950,
        physical_and_public_prefix_exact=True,all_preintervention_requested_commands_exact=True,
        complete_candidate_decisions_match_prospective_prefix=True,raw_model_forecast_comparisons=501,
        all_compared_raw_model_forecasts_exact=True,following_physical_outcomes_compared=False,
        unexecuted_outcomes_inferred=False)
    for key,value in expected.items():
        if record['prefix_comparison'][key]!=value:raise ValueError('complete original physical intervention required: '+key)
...
def verify_inputs(launch):
    if (launch['native_result_sha256']!=INPUT_SHA
            or launch['replay_input_bindings'].get('result.json')!=INPUT_SHA
            or launch['verification_benchmark_result_sha256']!=BENCHMARK_SHA):
        raise ValueError('fixed completed native and scoped benchmark identities required')
    ...
    print('RECENT_QUALIFIED_ANCHOR_INPUTS_VERIFIED',counters,flush=True)
...
def replay(launch):
    ...
    controller=RecentQualifiedAnchorController(model,ArticulatedCollisionGeometry(URDF),
    ...
    if len(reader.frames)!=656 or len(acquisitions)!=656 or len(tape)!=655:
        raise ValueError('complete original height-native population required')
    ...
    with writer(OUTPUT) as append:
        for i,original in enumerate(islice(read_rows(directory),MAX_FRAMES)):
            ...
            try:
                ...
                if live['terminal'] is None:current_partial_height_pose(live['evidence'],identity=(0,0,0),now_ns=now)
            ...
            if i%32==0:print('RECENT_QUALIFIED_ANCHOR_PREFIX_FRAME',i,flush=True)
            if check['stop']:break
    ...
    return dict(case=CASE[0],layout_index=1,frames=frames,maximum_frames=MAX_FRAMES,exact_original_decisions=exact,
...
def main():
    ...
    if args.native_result_sha256!=INPUT_SHA:raise ValueError('exact completed height-native result required')
    ...
    for name,sha in benchmark['source_sha256'].items():
        ...
        inherited[name]=sha
    sources=discover_sources((PROTOCOL,'scripts/replay_go2_recent_qualified_anchor_prefix_v1.py',
        'lewm/tests/test_recent_qualified_anchor_development.py','lewm/tests/test_recent_qualified_anchor_prefix_development.py',
        'lewm/tests/test_recent_qualified_anchor_replay_development.py',
        'docs/go2_partial_floor_height_maze01_anchor_loss_readout_2026-09-09.md'),inherited)
    ...
    launch.update(protocol=PROTOCOL,source_sha256=sources,output_root=str(OUTPUT),replay_input_bindings=ids,
        ...
        implementation_class='RecentQualifiedAnchorController',native_execution=False,model_loaded=True,model_training=False,
        ...
        input_scope='completed height maze1; stop before any changed-command future or either terminal',
    ...
    if args.preflight_only:
        print('RECENT_QUALIFIED_ANCHOR_PREFIX_PREFLIGHT',json.dumps(dict(source_count=len(sources),hardware=resources,
    ...
    print('RECENT_QUALIFIED_ANCHOR_PREFIX_LAUNCHED',digest(OUTPUT/'launch.json'),flush=True)
    try:
        ...
        write_json(OUTPUT/'result.json',dict(status='RECENT_QUALIFIED_ANCHOR_PREFIX_V1_COMPLETE',source_sha256=sources,
        ...
        print('RECENT_QUALIFIED_ANCHOR_PREFIX_COMPLETE',digest(OUTPUT/'result.json'),report,flush=True)
    except Exception as error:
        write_json(OUTPUT/'failure.json',dict(status='TERMINAL_RECENT_QUALIFIED_ANCHOR_PREFIX_FAILURE',reason=repr(error)));raise
