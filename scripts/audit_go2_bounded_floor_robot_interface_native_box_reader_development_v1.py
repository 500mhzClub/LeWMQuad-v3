...
def check_native_box_size(data, dimensions):
    """Native seven-slot BOX data: three dimensions plus exact zero padding."""
    values = np.asarray(data, float)
    expected = np.asarray(dimensions, float)
    require(values.shape == (7,) and expected.shape == (3,) and np.isfinite(values).all()
            and np.isfinite(expected).all() and np.array_equal(values[3:], np.zeros(4)),
            'native BOX dimensions and exact reserved-zero slots')
    np.testing.assert_allclose(values[:3], expected, atol=2e-7, rtol=0)
...
def main():
    ...
    names = ('scripts/audit_go2_bounded_floor_robot_interface_native_box_reader_development_v1.py',
             'lewm/tests/test_bounded_floor_robot_native_box_reader_development.py')
    correction = {p: digest(ROOT / p) for p in names}
    verify_bindings(correction)
    report = audit()
    verify_bindings(correction)
    output = {'status': 'NATIVE_BOX_READER_CORRECTION_COMPLETE', 'reader_source_sha256': correction,
        'original_reader_failure': '85679 EXIT1 native_box_size shape7 versus expected3',
        'scope': 'read-only representation correction; same saved physics and scientific checks',
        'result': report}
    write_json(OUTPUT / 'native_box_reader_correction.json', output)
