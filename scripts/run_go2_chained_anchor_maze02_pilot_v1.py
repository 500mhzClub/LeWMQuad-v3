"""Fresh chained-anchor physics after completed raw replay and the completed contact-plus-flow queue."""
import argparse
import contextlib
...
from scripts.chained_anchor_native_prefix_development import compare, boundary
from scripts.chained_anchor_maze02_episode_development import collect, artifacts
from scripts.chained_anchor_maze02_audit_development import audit
...
OUTPUT = BASE/'go2_chained_anchor_maze02_pilot_v1_attempt_001'
SOURCE = 'scripts/run_go2_chained_anchor_maze02_pilot_v1.py'
PROTOCOL = 'docs/go2_chained_anchor_maze02_pilot_v1_2026-09-11.md'
TESTS = ('lewm/tests/test_chained_anchor_native_launcher_development.py',
    'lewm/tests/test_chained_anchor_native_prefix_development.py',
    'lewm/tests/test_chained_anchor_native_source_development.py')
CASE = ('no_rgb_jepa_chained_anchor_maze_02', *inputs.replay.native.CASE[1:])
WORKER_STATUS = 'CHAINED_ANCHOR_MAZE02_COLLECTED_AND_RAW_AUDITED'


def prepared_sources():
    return inputs.prepared_sources((SOURCE, PROTOCOL, *TESTS))
...
def verify_inputs(launch, *, full=False):
    ...
    expected = dict(planned_case=list(CASE), output_root=str(OUTPUT),
        ...
        chained_retained_anchor_reacquisition_enabled=True, direct_corner_flow_missingness_fallback_enabled=True, planner_interface_adapter_enabled=True,
    ...
    if any(launch[k] != v or type(launch[k]) is not type(v) for k, v in expected.items()):
        raise ValueError('exact prospective chained-anchor native definition required')
    admission = launch['input_admission']; boundary(admission['prefix_report'])
    inputs.verify_bound(admission, launch['source_sha256'])
    if full and inputs.admit(admission['controller_completion_sha256'], admission['flow_wait_result_sha256'],
            launch['source_sha256']) != admission:
        raise ValueError('complete original chained-anchor input admission changed')
...
def assigned_model(launch):
    ...
    if (type(model) is not AllPhasePlannerModel or model.training
        ...
        raise ValueError('original assigned chained-anchor model state required')
    return model
...
def require_worker(record, report, prefix_report):
    if (record['status'] != WORKER_STATUS or 'failure' in record or record['case'] != CASE[0]
            ...
            or report['chained_retained_anchor_reacquisition_enabled'] is not True
            or report['direct_corner_flow_missingness_fallback_enabled'] is not True
            or record['collection']['status'] != 'CHAINED_ANCHOR_MAZE02_TERMINAL_AUDIT_REQUIRED'
            or record['collection']['chained_retained_anchor_reacquisition_enabled'] is not True
            or record['collection']['direct_corner_flow_missingness_fallback_enabled'] is not True):
        raise ValueError('complete exact chained-anchor collection and raw audit required')
    ...
    receipt = record['prefix_comparison']; boundary(prefix_report); frames, changed = 854, 853
    for key in ('physical_and_public_prefix_exact', 'all_preintervention_requested_commands_exact',
            ...
            'anchor_evidence_intervention'):
        if receipt[key] is not True: raise ValueError('complete physical chained-anchor intervention required: '+key)
    expected_receipt = dict(common_prefix_frames=frames, first_intervention_frame=changed,
        ...
        original_controller_failed_at_intervention=False, boundary_command_samples_present=50,
...
def worker(launch_sha):
    ...
    record = dict(case=name, layout_index=index, variant=variant, condition=condition, model_name=model_name,
        status='CHAINED_ANCHOR_MAZE02_WORKER_FAILED', artifact_sha256={}); started = time.perf_counter()
    with (OUTPUT/(name+'_worker.log')).open('x') as log, contextlib.redirect_stdout(log), contextlib.redirect_stderr(log):
        try:
            ...
            if state_digest(model.state_dict()) != before: raise ValueError('collection changed assigned chained-anchor model')
            ...
            record.update(status=WORKER_STATUS, artifact_sha256=ids, prefix_comparison=receipt, readout=readout,
                model_state_sha256=before, model_state_unchanged=True, chained_retained_anchor_reacquisition_enabled=True, direct_corner_flow_missingness_fallback_enabled=True,
            ...
            import traceback
            traceback.print_exc(); record.update(status='CHAINED_ANCHOR_MAZE02_WORKER_FAILED', failure=repr(error))
...
def main():
    ...
    parser.add_argument('--controller-completion-sha256'); parser.add_argument('--flow-wait-result-sha256')
    ...
    if OUTPUT.exists() or OUTPUT.is_symlink(): raise ValueError('exclusive chained-anchor native attempt; no retry/resume')
    sources = prepared_sources(); resources = hardware(); cohort_resources(resources, 1)
    if args.source_preflight_only:
        print('CHAINED_ANCHOR_NATIVE_SOURCE_PREFLIGHT', json.dumps(dict(source_count=len(sources), hardware=resources,
            native_execution=False, output_created=False, complete_input_admission_performed=False)), flush=True); return
    if not args.controller_completion_sha256 or not args.flow_wait_result_sha256:
        raise ValueError('exact completed controller verification and contact-plus-flow waiter identities required')
    print('CHAINED_ANCHOR_NATIVE_INPUT_ADMISSION_STARTED', len(sources), flush=True)
    admission = inputs.admit(args.controller_completion_sha256, args.flow_wait_result_sha256, sources)
    ...
    launch.update(source_sha256=sources, protocol=PROTOCOL, output_root=str(OUTPUT), input_admission=admission,
        planned_case=list(CASE), implementation_class='ChainedAnchorResidualController',
    ...
    if args.preflight_only:
        print('CHAINED_ANCHOR_NATIVE_PREFLIGHT', json.dumps(dict(source_count=len(sources), hardware=resources,
    ...
    print('CHAINED_ANCHOR_NATIVE_LAUNCHED', launch_sha, flush=True); started = time.perf_counter()
    try:
        ...
        print('CHAINED_ANCHOR_NATIVE_TERMINAL', record['status'], record.get('verified_round_trip'), record.get('failure'), flush=True)
        if record['status'] != WORKER_STATUS:
            raise ValueError('chained-anchor collection/audit/prefix incomplete: '+str(record.get('failure')))
        ...
        write_json(OUTPUT/'result.json', dict(status='CHAINED_ANCHOR_MAZE02_PILOT_V1_COMPLETE',
            source_sha256=sources, artifact_sha256=ids, conditions=[record], wall_s=time.perf_counter()-started,
            prospective_prefix_result_sha256=admission['controller_result_sha256'],
            controller_completion_sha256=args.controller_completion_sha256, flow_wait_result_sha256=args.flow_wait_result_sha256,
            measured_round_trip_successes=int(record['verified_round_trip']), reused_layout_executions=1,
            new_independent_layout_executions=0, chained_retained_anchor_reacquisition_enabled=True, direct_corner_flow_missingness_fallback_enabled=True, model_training=False,
            navigation_qualified=False, real_time_qualified=False, hardware_qualified=False, goal_achieved=False))
        print('CHAINED_ANCHOR_NATIVE_COMPLETE', digest(OUTPUT/'result.json'), flush=True)
    except Exception as error:
        write_json(OUTPUT/'failure.json', dict(status='TERMINAL_CHAINED_ANCHOR_NATIVE_FAILURE', reason=repr(error), automatic_retry=False)); raise
