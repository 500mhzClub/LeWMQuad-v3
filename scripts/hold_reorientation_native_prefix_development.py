"""Admit a raw hold-reorientation prefix and pair its prospective execution."""
...
def boundary(report):
    ...
    if (type(frames) is not int or frames != 406 or type(changed) is not int or changed != 405
        ...
        raise ValueError('exact original hold-to-left-turn raw prefix required')
...
def reconstruct(report, originals, prospective, tape, expected_boundary_selection):
    ...
    for i, (old, saved) in enumerate(zip(originals, prospective, strict=True)):
        ...
        check = compare_step(old['decision'], saved['decision'], command['requested_command'],
            frame=i, expected_selection=expected)
...
def admit_prefix(root, result):
    if result['status'] != 'HOLD_REORIENTATION_MAZE02_RAW_PREFIX_V1_COMPLETE' or result['native_execution'] is not False:
    ...
    expected = dict(original_complete_decisions_reconstructed=True, candidate_matches_saved_selection_boundary=True,
        observed_map_contact_and_executed_residual_state_exact=True, model_state_sha256=replay.MODEL_SHA,
    ...
    reconstruct(report, originals, prospective, read_json(prior, 'command_tape.json'), saved_boundary['candidate_selection'])
...
def compare(prior, current, prefix_root, report):
    ...
    reconstructed = reconstruct(report, originals, prospective, tapes[0], saved_boundary['candidate_selection'])
