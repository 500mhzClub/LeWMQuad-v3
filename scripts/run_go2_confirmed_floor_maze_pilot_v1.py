"""Prospective current-primary floor confirmation on reused development maze 0."""
import argparse
import contextlib
import json
...
from scripts.confirmed_floor_maze_episode_development import collect, artifacts
from scripts.confirmed_floor_maze_audit_development import audit
from scripts.confirmed_floor_native_prefix_comparison_development import compare
from scripts.training_translation_bias_model_admission_development import load_assigned
from scripts.replay_go2_confirmed_floor_maze_prefix_v1 import (
...
OUTPUT = BASE/'go2_confirmed_floor_maze_pilot_v1_attempt_001'
...
def worker(launch_sha):
    ...
    terminal = dict(case=name, layout_index=index, model_name=model_name,
        status='CONFIRMED_FLOOR_MAZE_WORKER_FAILED', artifact_sha256={}); started = time.perf_counter()
    with (OUTPUT/(name+'_worker.log')).open('x') as log, contextlib.redirect_stdout(log), contextlib.redirect_stderr(log):
        try:
            ...
            terminal.update(status='CONFIRMED_FLOOR_MAZE_COLLECTED_AND_RAW_AUDITED', artifact_sha256=bindings,
...
def main():
    ...
    if OUTPUT.exists() or OUTPUT.is_symlink(): raise ValueError('exclusive prospective floor-confirmation experiment; no retry/resume')
    verify_artifacts(INTEGRATION, {'result.json': args.prefix_result_sha256}); integration = read_json(INTEGRATION, 'result.json')
    assert integration['status'] == 'CONFIRMED_FLOOR_MAZE_PREFIX_COMPLETE'
    ...
    assert prefix['exact_other_decision_fields_before_intervention'] and prefix['causal_observations_raw_maps_mission_and_forecasts_exact']
    ...
    assert prefix['original_complete_surface_checks_and_selection_reconstructed']
    assert prefix['original_nominal_paths_and_primary_checks_exact']
    assert prefix['auxiliary_foot_floor_classification_is_the_declared_change']
    assert prefix['visibility_failure_not_relabeled'] and prefix['input_strict_visibility_failed_frames'] == [909]
    ...
    sources = discover_sources((PROTOCOL, 'scripts/run_go2_confirmed_floor_maze_pilot_v1.py',
        'lewm/tests/test_confirmed_floor_native_scope_development.py',
        'lewm/tests/test_confirmed_floor_native_prefix_comparison_development.py',
        'docs/go2_confirmed_floor_maze_prefix_result_2026-09-08.md'), integration['source_sha256'])
    resources = hardware()
    if args.preflight_only:
        print('CONFIRMED_FLOOR_MAZE_PREFLIGHT', json.dumps(dict(
    ...
    if resources['artifact_free_bytes'] < RESERVE_BYTES+COLLECTION_ALLOWANCE_BYTES+PERSISTENCE_HEADROOM_BYTES:
        raise ValueError('declared floor-confirmation storage/persistence envelope unavailable')
    ...
    launch.update(protocol=PROTOCOL, source_sha256=sources, output_root=str(OUTPUT), planned_case=CASE,
        ...
        concurrency_reason='one existing-maze floor-confirmation intervention; storage envelope excludes parallel scenes',
        ...
        strict_visibility_gate_unchanged=True, visibility_failure_expected_if_prefix_includes_909=True,
    ...
    print('CONFIRMED_FLOOR_MAZE_LAUNCHED', launch_sha, flush=True)
    try:
        ...
        print('CONFIRMED_FLOOR_MAZE_TERMINAL', record['status'], record.get('verified_round_trip'), record.get('failure'), flush=True)
        if record['status'] != 'CONFIRMED_FLOOR_MAZE_COLLECTED_AND_RAW_AUDITED':
            raise ValueError('native floor-confirmation infrastructure/raw audit/prefix comparison failure; evidence retained')
        ...
        verify_artifacts(FITS, admission['base_admission']['fit_artifact_sha256'])
        verify_artifacts(CORRECTION, admission['correction_artifact_sha256'])
        write_json(OUTPUT/'result.json', dict(status='CONFIRMED_FLOOR_MAZE_PILOT_COMPLETE', conditions=[record],
        ...
        print('CONFIRMED_FLOOR_MAZE_COMPLETE', digest(OUTPUT/'result.json'), flush=True)
    except Exception as error:
        write_json(OUTPUT/'failure.json', dict(status='TERMINAL_CONFIRMED_FLOOR_MAZE_PILOT_FAILURE', reason=repr(error)))
        raise
