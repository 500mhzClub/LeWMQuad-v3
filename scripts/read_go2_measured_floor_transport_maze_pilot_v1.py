...
from scripts.run_go2_measured_floor_transport_maze_pilot_v1 import OUTPUT as INPUT, CASE
...
OUTPUT = BASE/'go2_measured_floor_transport_maze_readout_v1_attempt_001'
PROTOCOL = 'docs/go2_measured_floor_transport_maze_readout_v1_2026-09-09.md'
...
def main():
    ...
    assert result['status'] == 'MEASURED_FLOOR_TRANSPORT_MAZE_PILOT_V1_COMPLETE' and len(result['conditions']) == 1
    assert result['conditions'][0]['case'] == CASE[0]
    assert result['conditions'][0]['status']=='MEASURED_FLOOR_TRANSPORT_MAZE_COLLECTED_AND_RAW_AUDITED'
    assert result['conditions'][0]['prefix_comparison']['physical_and_public_prefix_exact']
    assert result['conditions'][0]['prefix_comparison']['complete_candidate_decisions_match_prospective_prefix']
    ...
    assert old['additional_auxiliary_rgb_for_motion'] is True and old['renderer_capture_witnesses_enabled'] is True
    sources = discover_sources((PROTOCOL, 'scripts/read_go2_measured_floor_transport_maze_pilot_v1.py',
        'lewm/tests/test_view_reentry_readout_development.py',
        'lewm/tests/test_dual_camera_native_execution_scope_development.py',
        'lewm/tests/test_later_floor_resolution_execution_readout_development.py',
        'lewm/tests/test_measured_floor_transport_readout_development.py'), old['source_sha256'])
    resources = hardware()
    ...
    try:
        ...
        write_json(OUTPUT/'result.json', dict(status='MEASURED_FLOOR_TRANSPORT_MAZE_READOUT_V1_COMPLETE',
        ...
        print('MEASURED_FLOOR_TRANSPORT_MAZE_READOUT_V1_COMPLETE', digest(OUTPUT/'result.json'), flush=True)
    except Exception as error:
        write_json(OUTPUT/'failure.json', dict(status='TERMINAL_MEASURED_FLOOR_TRANSPORT_MAZE_READOUT_FAILURE', reason=repr(error)))
        raise
