...
from scripts.run_go2_settled_boundary_maze_pilot_v2 import OUTPUT as INPUT, CASE
...
OUTPUT = BASE/'go2_settled_boundary_maze_readout_v2_attempt_001'
PROTOCOL = 'docs/go2_settled_boundary_maze_readout_v2_2026-09-09.md'
...
def main():
    ...
    assert result['status'] == 'SETTLED_BOUNDARY_MAZE_PILOT_V2_COMPLETE' and len(result['conditions']) == 1
    ...
    sources = discover_sources((PROTOCOL, 'scripts/read_go2_settled_boundary_maze_pilot_v2.py',
    ...
    try:
        ...
        write_json(OUTPUT/'result.json', dict(status='SETTLED_BOUNDARY_MAZE_READOUT_V2_COMPLETE',
        ...
        print('SETTLED_BOUNDARY_MAZE_READOUT_V2_COMPLETE', digest(OUTPUT/'result.json'), flush=True)
    except Exception as error:
        write_json(OUTPUT/'failure.json', dict(status='TERMINAL_SETTLED_BOUNDARY_MAZE_READOUT_FAILURE', reason=repr(error)))
        raise
