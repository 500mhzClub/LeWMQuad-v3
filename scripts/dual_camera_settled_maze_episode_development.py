...
def collect(layout_index, definition, *, output, model, geometry, episode_name, condition, variant):
    ...
    try:
        session = DualCameraNovelMazeSession(spec, directory); session.install_contact_identity()
        ...
        result = dict(status='DUAL_CAMERA_SETTLED_MAZE_TERMINAL_AUDIT_REQUIRED', layout_index=layout_index,
        ...
        print('DUAL_CAMERA_SETTLED_MAZE_COLLECTED', result, flush=True)
        return result
    finally:
