#!/usr/bin/env python3
"""Build the causal boundary-aligned recurrent-H4 V2 train/val index."""
...
from lewm.datasets.go2_recurrent_h4_rgb_sequences_v2 import (  # noqa: E402
    SCHEMA,
    build_index,
    canonical_row_bytes,
)
...
PUBLIC_SOURCE_PATHS = (
    Path("lewm/datasets/go2_recurrent_h4_rgb_sequences.py"),
    Path("lewm/datasets/go2_recurrent_h4_rgb_sequences_v2.py"),
    Path("scripts/build_go2_recurrent_h4_rgb_index_v2.py"),
)
...
def _public_source_bindings(repo_root: Path) -> dict[str, Any]:
    ...
    for relative in PUBLIC_SOURCE_PATHS:
        ...
        try:
            ...
            if not stat.S_ISREG(initial.st_mode):
                raise RuntimeError(f"public source is not a regular file: {relative}")
        ...
        if (
            ...
            or byte_count != initial.st_size
        ):
            raise RuntimeError(f"public source changed while hashing: {relative}")
        entries.append(
            {
    ...
    return {
        ...
        "ordered_binding_sha256": _sha256(_canonical_json(entries)),
    }
...
def _load_census_source_binding(repo_root: Path) -> dict[str, Any]:
    ...
    if (
        ...
        or receipt.get("identity", {}).get("ordered_source_content_binding_sha256")
        != CENSUS_SOURCE_BINDING_SHA256
    ):
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    frozen_census_binding = _load_census_source_binding(repo_root)
    source_bindings_before = _public_source_bindings(repo_root)
    ...
    source_bindings_after = _public_source_bindings(repo_root)
    if source_bindings_after != source_bindings_before:
        raise RuntimeError("public index-builder source changed during the build")
    manifest["public_source_bindings"] = source_bindings_after
    manifest["source"]["frozen_census_v2"] = frozen_census_binding
    index_payloads = {
    ...
    receipt = {
        ...
        "discarded_initial_group_count": manifest["source"][
            "discarded_initial_group_count"
        ],
        "emitted_causal_group_count": manifest["source"][
            "emitted_causal_group_count"
        ],
