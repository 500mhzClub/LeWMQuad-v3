...
from scripts import replay_go2_visibility_batched_footprint_late_history_v1 as run
...
SOURCE = 'scripts/verify_go2_visibility_batched_footprint_completion_v1.py'
TEST = 'lewm/tests/test_visibility_batched_footprint_completion_development.py'
EXECUTION = 'docs/go2_visibility_batched_footprint_replay_execution_2026-09-11.json'
EXECUTION_SHA = '3464fa3b80c30cea85b133da2404700c2e3b3bc7ae96b8657a591b64dbd453c0'
RESULT_SHA = '3aae2867faf3a127b3a42acf13c69514607ce15272d5a98ad4a87da8d32fddb4'
OUTPUT = ROOT/'docs/go2_visibility_batched_footprint_completion_verification_2026-09-11.json'
...
def expected_report(prior_report, timing):
    ...
    for key in ('incremental_footprint_evidence_copy_comparison', 'both_controllers_use_packed_fused_scoped_queries'):
        if expected.pop(key) is not True: raise ValueError('original receipt-copy comparison scope required')
    expected.update(baseline='ReceiptCopiedFootprintController', candidate='VisibilityBatchedFootprintController',
        timing_windows=timing, normalized_state_type_paths=run.STATE_TYPE_PATHS,
        incremental_reuse_comparison=False, incremental_empty_patch_visibility_skip_comparison=True,
        persistent_memory_type_unchanged=True, both_controllers_use_receipt_copied_packed_fused_queries=True,
...
def main():
    ...
    _, prior, prior_launch, prior_rows, raw_launch = run.profile.admit_completed(sources)
    ...
    expected_launch = dict(copied_result_sha256=run.profile.COPIED_SHA,
        copied_launch_sha256=run.profile.COPIED_LAUNCH_SHA, frames=1428,
        state_frames=list(run.original.STATE_FRAMES), normalized_state_type_paths=run.STATE_TYPE_PATHS,
        baseline='ReceiptCopiedFootprintController', candidate='VisibilityBatchedFootprintController',
    ...
    write_json(OUTPUT, dict(status='VISIBILITY_BATCHED_FOOTPRINT_COMPLETION_VERIFIED',
        ...
        preceding_receipt_copy_completion_reauthenticated=True, actual_raw_and_model_bindings_rehashed=True,
    ...
    print('VISIBILITY_BATCHED_FOOTPRINT_COMPLETION_VERIFIED', digest(OUTPUT), len(sources), reduction, flush=True)
