#!/usr/bin/env python3
"""Exact trainer for the single V4 N5 hierarchical-first-hit V9 attempt.
...
def train_v9_fit(
    *,
    ...
    """Run the frozen 4,000-update full-panel, final-only V9 schedule."""
    ...
    if (
        ...
        raise PermissionError("V9 frozen optimizer or schedule request changed")
    if not math.isfinite(float(learning_rate)) or not math.isfinite(
        float(weight_decay)
    ):
        raise ValueError("V9 optimizer values must be finite")
    ...
    if schedule_sha256 != policy.EXPECTED_SCHEDULE_SHA256:
        raise ValueError("V9 deterministic schedule hash changed")
    if any(len(indices) != 5 or set(indices) != set(range(5)) for indices in schedule):
        raise ValueError("V9 schedule is not one full panel per update")
    ...
    for step_index, indices in enumerate(schedule, start=1):
        ...
        if not bool(torch.isfinite(total).item()):
            raise FloatingPointError("V9 fit loss became non-finite")
        total.backward()
        gradient_norm = torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0)
        if not bool(torch.isfinite(gradient_norm).item()):
            raise FloatingPointError("V9 fit gradient norm became non-finite")
        optimizer.step()
    ...
    if [row["step"] for row in trace] != diagnostic_updates:
        raise RuntimeError("V9 diagnostic schedule changed during training")
    return {
        "steps": 4000,
        "batch_size": 5,
        "learning_rate": 1e-4,
...
def build_checkpoint_metadata_v9(
    *,
    ...
    """Construct the frozen final-update V9 checkpoint metadata without I/O."""

    if training_schedule_sha256 != policy.EXPECTED_SCHEDULE_SHA256:
        raise ValueError("V9 checkpoint schedule binding changed")
    return {
        "experiment": policy.experiment_contract(),
        "authority_bindings": policy.authority_bindings(),
