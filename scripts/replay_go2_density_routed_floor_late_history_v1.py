"""Compare density-routed floors against the completed visibility-batched controller."""
import argparse
import builtins
import json
import os
...
from lewm.density_routed_floor_controller_development import DensityRoutedFloorController, normalize_to_visibility_batched
from lewm.visibility_batched_footprint_controller_development import VisibilityBatchedFootprintController
from scripts import replay_go2_visibility_batched_footprint_late_history_v1 as previous
from scripts import verify_go2_visibility_batched_footprint_completion_v1 as previous_check
...
SOURCE='scripts/replay_go2_density_routed_floor_late_history_v1.py'
TEST='lewm/tests/test_density_routed_floor_replay_development.py'
PROTOCOL='docs/go2_density_routed_floor_late_history_v1_2026-09-11.md'
WITNESS='docs/go2_visibility_batched_footprint_completion_verification_2026-09-11.json'
WITNESS_SHA='8ed19a7e3de6b0542f912f35f6b3f3de38e812a51a7dcc7748ee091ada4f44ed'
MICROBENCHMARK='docs/go2_density_routed_floor_registration_completion_verification_2026-09-11.json'
MICROBENCHMARK_SHA='278f99ca25615e525a55ae944ed69b461aa2e54e20c395cf3716733ca57de61b'
OUTPUT=BASE/'go2_density_routed_floor_late_history_v1_attempt_001'
PREVIOUS_LAUNCH_SHA='6945b551f09de3153378df50fb4e5b018ae695595c5788ccd194123571b7f02a'
...
def prepared_sources():
    ...
    if witness['status'] != 'VISIBILITY_BATCHED_FOOTPRINT_COMPLETION_VERIFIED':
    ...
    if benchmark['status']!='DENSITY_ROUTED_FLOOR_REGISTRATION_COMPLETION_VERIFIED':
        raise ValueError('completed exact density-routed registration replay required')
    ...
    sources=discover_sources((SOURCE,TEST,PROTOCOL,WITNESS,MICROBENCHMARK,
        'lewm/tests/test_density_routed_floor_controller_development.py',REGISTRATION_EXECUTION),inherited)
    verify(sources)
    return sources
...
def admit_completed(sources):
    ...
    fixed={'launch.json':PREVIOUS_LAUNCH_SHA,'result.json':previous_check.RESULT_SHA}
    ...
    if (result['status']!='VISIBILITY_BATCHED_FOOTPRINT_LATE_HISTORY_V1_COMPLETE'
    ...
    prior=profile.admit_completed(sources)
...
def normalize_candidate(decision):
    return previous.normalize_candidate(normalize_to_visibility_batched(decision))


def progress(*args,**kwargs):
    if args and args[0]=='SCOPED_FOOTPRINT_PAIRED_FRAME':
        args=('DENSITY_ROUTED_FLOOR_PAIRED_FRAME',*args[1:])
...
def isolated_replay():
    ...
    namespace=dict(function.__globals__,profile=view,
        FrozenFootprintAnchoredController=VisibilityBatchedFootprintController,
        ScopedFootprintAnchoredController=DensityRoutedFloorController,
...
def replay(rows,prior_report):
    ...
    if report['observed_state_checks'] != prior_report['observed_state_checks']:
        raise ValueError('all seven completed visibility-batched retained-state identities required')
    return report | dict(baseline='VisibilityBatchedFootprintController',candidate='DensityRoutedFloorController',
        normalized_state_type_paths=STATE_TYPE_PATHS,incremental_reuse_comparison=False,
        incremental_density_routed_floor_comparison=True,persistent_memory_type_unchanged=True,
        both_controllers_use_visibility_batched_receipt_copied_packed_fused_queries=True,imported_module_globals_mutated=False)
...
def main():
    ...
    if args.source_preflight_only:
        print('DENSITY_ROUTED_FLOOR_PREFLIGHT_PASS',len(sources),json.dumps(resources),flush=True);return
    ...
    write_json(OUTPUT/'launch.json',dict(source_sha256=sources,previous_result_sha256=previous_check.RESULT_SHA,
        previous_launch_sha256=PREVIOUS_LAUNCH_SHA,density_routed_registration_completion_sha256=MICROBENCHMARK_SHA,input_admission=admission,
        hardware=resources,environment=env,frames=1428,state_frames=original.STATE_FRAMES,
        normalized_state_type_paths=STATE_TYPE_PATHS,baseline='VisibilityBatchedFootprintController',
        candidate='DensityRoutedFloorController',native_execution=False,model_training=False))
    print('DENSITY_ROUTED_FLOOR_LAUNCHED',digest(OUTPUT/'launch.json'),flush=True)
    ...
    try:
        ...
        write_json(OUTPUT/'result.json',dict(status='DENSITY_ROUTED_FLOOR_LATE_HISTORY_V1_COMPLETE',
        ...
        print('DENSITY_ROUTED_FLOOR_COMPLETE',digest(OUTPUT/'result.json'),flush=True)
    except BaseException as error:
        write_json(OUTPUT/'failure.json',dict(status='TERMINAL_DENSITY_ROUTED_FLOOR_FAILURE',
