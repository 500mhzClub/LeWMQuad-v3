...
def admit_prefix(root,result):
    if (result['status'] != 'DIRECT_FLOW_MAZE01_PREFIX_V2_COMPLETE'
    ...
    expected=dict(case='full_jepa_novel_maze_01',frames=FRAMES,maximum_frames=FRAMES,boundary_frame=CHANGED,
    ...
    for i,row in enumerate(read_rows(root)):
        if i>=FRAMES: raise ValueError('only215 saved decisions admitted')
        ...
        count+=1; last=row
    if count!=FRAMES: raise ValueError('all215 saved decisions required')
...
def compare(prior,current,prefix_root,report):
    if report['frames']!=FRAMES or report['boundary_frame']!=CHANGED:
        raise ValueError('fixed215-observation intervention required')
    ...
    for i,(old,new,bound) in enumerate(zip(islice(read_rows(prior),FRAMES),
        ...
        checked+=1
    if checked!=FRAMES or forecasts!=211: raise ValueError('complete215-observation/211-forecast prefix required')
