"""Join authenticated inputs, all five diagnostics and a bound policy review.
...
SOURCE='scripts/independent_round_trip_final_admission_development.py'
TEST='lewm/tests/test_independent_round_trip_final_admission_development.py'
PROTOCOL='docs/go2_independent_round_trip_final_admission_v1_2026-09-11.md'
REVIEW='docs/go2_independent_round_trip_final_policy_review_v1.json'
REASONS=('policy_and_models','navigation_budget','sensing_failures',
         'causal_comparison_scope','timing_and_hardware_scope')
...
def require_links(input_admission,queue_admission):
    four=queue_admission['original_four_stage_admission']
    three=four['original_queue_admission']
    expected=dict(batch=input_admission['adapter_batch_result_sha256'],
        **three['ordered_waiter_result_sha256'],tracking=four['tracking_wait_result_sha256'])
    if (queue_admission['budget_inputs']!=expected
            or three['adapter_batch_result_sha256']!=expected['batch']
            or set(expected)!= {'batch','frontier','hold','contact','tracking'}):
        raise ValueError('same completed batch and all five ordered diagnostic identities required')
    return three,four


def reconstruct_full_queue(admission,sources):
    four=admission['original_four_stage_admission'];three=four['original_queue_admission']
    first=queue.original.original.admit(three['ordered_waiter_result_sha256'],
        adapter_batch_result_sha256=three['adapter_batch_result_sha256'],sources=sources,full=True)
    second=queue.original.admit(first,four['tracking_wait_result_sha256'],sources=sources,full=True)
    return queue.admit(second,admission['budget_wait_result_sha256'],sources=sources,full=True)


def diagnostic_readout(native,completion):
    sha=completion['native_result_sha256']
    verify_artifacts(native.OUTPUT,{'result.json':sha})
    result=read_json(native.OUTPUT,'result.json')
    # The caller has reauthenticated the complete queue before this bounded
    # extraction. Bind the exact worker/readout again to prevent substitution.
    name=native.CASE[0];bindings=result['artifact_sha256']
    selected={n:bindings[n] for n in (name+'_worker_terminal.json',name+'_readout.json')}
    verify_artifacts(native.OUTPUT,selected)
    worker=read_json(native.OUTPUT,name+'_worker_terminal.json')
    readout=read_json(native.OUTPUT,name+'_readout.json')
    if result['conditions']!=[worker] or readout!=worker['readout']:
        raise ValueError('same single completed diagnostic and readout required')
    verify_artifacts(native.OUTPUT,selected|{'result.json':sha})
    return dict(native_result_sha256=sha,case=name,collection=worker['collection'],
        readout=readout,prefix_comparison=worker['prefix_comparison'])


def review_evidence(input_admission,queue_admission):
    three,four=require_links(input_admission,queue_admission)
    first=[{k:deepcopy(r[k]) for k in ('stage','waiter_result_sha256','native_result_sha256',
        'case','collection','readout','measured_round_trip_successes')} for r in three['completed']]
    if [r['stage'] for r in first]!=['frontier','hold','contact']:
        raise ValueError('original ordered three-stage diagnostic outcomes required')
    return dict(adapter_batch_result_sha256=input_admission['adapter_batch_result_sha256'],
        six_case_development_summary=deepcopy(input_admission['development_summary']),
        original_three_diagnostics=first,
        tracking_wait_result_sha256=four['tracking_wait_result_sha256'],
        tracking=diagnostic_readout(queue.original.tracking.native,four['tracking_completion']),
        budget_wait_result_sha256=queue_admission['budget_wait_result_sha256'],
        extended_budget=diagnostic_readout(queue.budget.native,queue_admission['budget_completion']))


def require_review(review,input_admission,queue_admission,evidence):
    expected=dict(schema='independent_round_trip_final_policy_review.v1',
        status='COMPLETE_FIVE_DIAGNOSTIC_POLICY_REVIEW',
        ...
        five_stage_queue_admission_sha256=fingerprint(queue_admission),
...
def verify_population(launch,*,full=False):
    ...
    input_admission=launch['input_admission'];queue_admission=launch['five_stage_queue_admission']
    ...
    if full:
        ...
        if fingerprint(reconstruct_full_queue(queue_admission,sources))!=fingerprint(queue_admission):
            raise ValueError('full original five-stage queue admission changed')
    ...
    expected=dict(final_policy_review_completed=True,complete_input_admission_performed=True,
        native_queue_completion_verified=True,ordered_cases=inputs.study.manifest()['ordered_cases'],
