...
await_topics() {
  ...
  local required=(/clock /rgb_image /cmd_vel /lewm/go2/command_block /lewm/go2/executed_command_block)
  while true; do
    local listing
...
echo "Recording benchmark bag to $BAG_DIR for ${DURATION_SECS}s..."
...
"$SCRIPT_DIR/convert_smoke_bag_to_raw_rollout.sh" \
  "$BAG_DIR" --out "$RAW_DIR" --quality-profile pilot --no-strict

python3 - "$RAW_DIR/summary.json" "$BAG_DIR" "$LOG_PATH" "$REPORT_PATH" <<'PY'
