...
from scripts.run_go2_joint_room_return_v1 import OUTPUT,PREVIOUS
...
def audit_condition(c,result):
    ...
    return dict(raw_sensor_audit_pass=True,replayed_decisions=len(rows),raw_depth_checks=len(sensors['depth_checks']),
                ...
                native_home_hold=home,full_room_return_success=home is not None and home['passed'] and all(h['native_hold']['passed'] and h['native_winding_hold']['passed'] for h in holds) and all(v['within1mm'] for v in sensors['depth_checks']),
...
def main():
    ...
    sources=discover_sources(('scripts/audit_go2_joint_room_return_v1.py',),launch['source_sha256'])
