...
def compare(prior, current, prefix_root, prefix_report):
    ...
    if hashes[0] != hashes[1]: raise ValueError('physics changed before the later-floor-resolution command')
    ...
    for i, (old, new, bound) in enumerate(zip(islice(read_rows(prior), frames),
        ...
        change = compare_current(old['decision'], new['decision'])
        if (new['decision']['terminal'] is not None
                or change['terminal_changed'] != (i == prefix_report.get('first_terminal_policy_difference'))
                or change['command_changed'] != (i == changed)):
            raise ValueError('exact nonterminal first command intervention required')
    ...
    return dict(common_prefix_frames=frames, first_changed_command=changed, raw_physics_prefix_sha256=hashes[0],
        ...
        later_measured_floor_contact_interpretation_is_the_declared_change=True,
        original_pose_map_mission_forecasts_contact_queries_exact=True,
