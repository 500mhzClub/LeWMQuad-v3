...
from scripts.observed_floor_contact_goal_episode_development import collect,artifacts
from scripts.observed_floor_contact_goal_audit_development import audit
...
from scripts.replay_go2_observed_floor_contact_prefix_v1 import OUTPUT as INTEGRATION
...
PREVIOUS=BASE/'go2_auxiliary_downward45_goal_probe_v1_attempt_001'
DIAGNOSTIC=BASE/'go2_auxiliary_downward45_goal_readout_v1_attempt_001'
PREVIOUS_SHA='5007693d6e2b178d13c42164a08d561bf3ad2a60801b9290fb8033db811ce7fe'
DIAGNOSTIC_SHA='5d2f6df4c9762ac6d7f28c8546cea93e2d61356cda7a567561c32894c7667d6f'
OUTPUT=BASE/'go2_observed_floor_contact_goal_probe_v1_attempt_001'
...
PROTOCOL='docs/go2_observed_floor_contact_goal_probe_v1_2026-09-08.md'
...
def worker(case,launch_sha):
    ...
    terminal=dict(case=name,trial=trial,variant=variant,condition=condition,model_name=model_name,
        status='OBSERVED_FLOOR_CONTACT_GOAL_WORKER_FAILED',artifact_sha256={});started=time.perf_counter()
    with (OUTPUT/(name+'_worker.log')).open('x') as log,contextlib.redirect_stdout(log),contextlib.redirect_stderr(log):
        try:
            ...
            terminal.update(status='OBSERVED_FLOOR_CONTACT_GOAL_COLLECTED_AND_RAW_AUDITED',artifact_sha256=bindings,
...
def main():
    ...
    assert previous['status']=='AUXILIARY_DOWNWARD45_GOAL_PROBE_COMPLETE' and previous['cases']==[list(c) for c in CASES]
    ...
    assert diagnostic['status']=='AUXILIARY_DOWNWARD45_GOAL_READOUT_COMPLETE'
    ...
    assert integration['status']=='OBSERVED_FLOOR_CONTACT_PREFIX_COMPLETE' and len(integration['conditions'])==2
    assert all(r['causal_observations_maps_forecasts_and_nominal_checks_exact'] and r['only_declared_contact_policy_changed'] and not r['controller_failures'] and r['model_state_unchanged'] and r['exact_fresh_replay_passes']==2 for r in integration['conditions'])
    ...
    launch=preflight(output=OUTPUT,protocol=PROTOCOL,
        seed_paths=('scripts/run_go2_observed_floor_contact_goal_probe_v1.py',
            'lewm/tests/test_observed_floor_contact_goal_scope_development.py',
            'docs/go2_observed_floor_contact_prefix_result_2026-09-08.md',
    ...
    launch.update(integration_artifact_sha256=integration_ids,sensor_prefix_artifact_sha256=sensor_ids,
        ...
        experiment='classified floor contacts permitted for four nominal foot spheres with support unknown; both sensor streams, other/nonfoot constraints, corrected planner and bounded waits unchanged',
    ...
    print('OBSERVED_FLOOR_CONTACT_GOAL_LAUNCHED',digest(OUTPUT/'launch.json'),flush=True)
    ...
    try:
            ...
            with ProcessPoolExecutor(max_workers=2,mp_context=multiprocessing.get_context('spawn'),max_tasks_per_child=1) as pool:
                ...
                while pending:
                    ...
                    for future in done:
                        ...
                        except Exception as error:row=dict(case=case[0],status='OBSERVED_FLOOR_CONTACT_GOAL_PROCESS_FAILURE',failure=repr(error))
                        records.append(row);print('OBSERVED_FLOOR_CONTACT_GOAL_TERMINAL',case[0],row['status'],row.get('goal'),flush=True)
        if len(records)!=2 or any(r['status']!='OBSERVED_FLOOR_CONTACT_GOAL_COLLECTED_AND_RAW_AUDITED' for r in records):
        ...
        result=dict(status='OBSERVED_FLOOR_CONTACT_GOAL_PROBE_COMPLETE',cases=CASES,conditions=records,
        ...
        write_json(OUTPUT/'result.json',result);print('OBSERVED_FLOOR_CONTACT_GOAL_COMPLETE',digest(OUTPUT/'result.json'),flush=True)
    except Exception as error:
        write_json(OUTPUT/'failure.json',dict(status='TERMINAL_OBSERVED_FLOOR_CONTACT_GOAL_FAILURE',reason=repr(error),
