"""Read actual later-floor-resolution navigation and executed contact interventions."""
import argparse
...
from scripts.run_go2_later_floor_resolution_maze_pilot_v1 import OUTPUT as INPUT, CASE
...
OUTPUT = BASE/'go2_later_floor_resolution_maze_readout_v1_attempt_001'
PROTOCOL = 'docs/go2_later_floor_resolution_maze_readout_v1_2026-09-09.md'
...
def main():
    ...
    assert result['status'] == 'LATER_FLOOR_RESOLUTION_MAZE_PILOT_COMPLETE' and len(result['conditions']) == 1
    ...
    sources = discover_sources((PROTOCOL, 'scripts/read_go2_later_floor_resolution_maze_pilot_v1.py',
    ...
    try:
        ...
        write_json(OUTPUT/'result.json', dict(status='LATER_FLOOR_RESOLUTION_MAZE_READOUT_COMPLETE',
        ...
        print('LATER_FLOOR_RESOLUTION_MAZE_READOUT_COMPLETE', digest(OUTPUT/'result.json'), flush=True)
    except Exception as error:
        write_json(OUTPUT/'failure.json', dict(status='TERMINAL_LATER_FLOOR_RESOLUTION_MAZE_READOUT_FAILURE', reason=repr(error)))
        raise
