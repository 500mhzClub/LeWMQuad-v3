"""Read completed measured floor confirmation without outcome relabeling."""
import argparse
...
from scripts.run_go2_confirmed_floor_maze_pilot_v1 import OUTPUT as INPUT, CASE
...
OUTPUT = BASE/'go2_confirmed_floor_maze_readout_v1_attempt_001'
PROTOCOL = 'docs/go2_confirmed_floor_maze_readout_v1_2026-09-08.md'
...
def main():
    ...
    assert result['status'] == 'CONFIRMED_FLOOR_MAZE_PILOT_COMPLETE' and len(result['conditions']) == 1
    ...
    sources = discover_sources((PROTOCOL, 'scripts/read_go2_confirmed_floor_maze_pilot_v1.py',
        'lewm/tests/test_view_reentry_readout_development.py',
        'lewm/tests/test_confirmed_floor_execution_readout_development.py'), old['source_sha256'])
    resources = hardware()
    ...
    try:
        ...
        write_json(OUTPUT/'result.json', dict(status='CONFIRMED_FLOOR_MAZE_READOUT_COMPLETE',
        ...
        print('CONFIRMED_FLOOR_MAZE_READOUT_COMPLETE', digest(OUTPUT/'result.json'), flush=True)
    except Exception as error:
        write_json(OUTPUT/'failure.json', dict(status='TERMINAL_CONFIRMED_FLOOR_MAZE_READOUT_FAILURE', reason=repr(error)))
        raise
