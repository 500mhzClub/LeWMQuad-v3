...
from scripts.ordered_dynamic_pilot_development import RUNS,RESERVE,BUDGET,ALLOWANCE,PROTOCOL,definitions,commit_episode
...
OUTPUT=BASE/'go2_ordered_union_dynamic_sensor_pilot_v1_attempt_001'
...
def preflight():
    ...
    sources=discover_sources((PROTOCOL,'scripts/run_go2_ordered_union_dynamic_sensor_pilot_v1.py',
        'scripts/audit_go2_ordered_union_dynamic_sensor_pilot_v1.py',
        'lewm/tests/test_ordered_dynamic_sensor_pilot_development.py'),old['source_sha256'])
...
def collect(inventory,output,run,trial,definition):
    ...
    try:
        session=OrderedDynamicSession(inventory,spec,directory);session.install_contact_identity();build=session.ctx.build
