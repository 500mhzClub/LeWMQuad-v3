...
from scripts.run_go2_room_return_pulse_v1 import OUTPUT
...
def audit_condition(c,result):
    ...
    old=read_npz(ROOT/'.generated/go2_goal_region_pulse_servo_v1_attempt_001'/('nominal_b' if c=='nominal_right' else 'lower_friction_a' if c.startswith('lower') else 'nominal_a'),'physics_trace.npz')
    ...
    model=RoomReturnPulse(spec['turn_sign']);desired=[]
    ...
    for leg in model.snapshot()['executor']['legs']:
        ...
        holds.append(dict(leg_index=leg['leg_index'],goal=g,native_hold=h))
    home=None
    ...
    return dict(raw_sensor_audit_pass=True,replayed_decisions=len(rows),raw_depth_checks=len(sensors['depth_checks']),
                ...
                native_home_hold=home,full_room_return_success=home is not None and home['passed'] and all(h['native_hold']['passed'] for h in holds),
...
def main():
    ...
    sources=discover_sources(('scripts/audit_go2_room_return_pulse_v1.py',),launch['source_sha256'])
