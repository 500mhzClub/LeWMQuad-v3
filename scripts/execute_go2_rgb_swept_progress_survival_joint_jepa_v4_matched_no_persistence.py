#!/usr/bin/env python3
"""Run the one-shot V4 matched control with persistence excluded from backward.

The control reconstructs the reviewed V4 model from the accepted N320 encoder
input and changes only the membership of the persistence objective in the
backward scalar.  Persistence is still computed and traced.  No prior
experiment runtime artifact is named, opened, loaded, or used for
initialization.
"""
...
CHECKPOINT_SCHEMA = (
    ...
    "persistence_checkpoint_v1"
)
TRACE_SCHEMA = (
    "lewm_go2_rgb_swept_progress_survival_joint_jepa_v4_matched_no_"
    "persistence_trace_v1"
)
RESULT_SCHEMA = (
    "lewm_go2_rgb_swept_progress_survival_joint_jepa_v4_matched_no_"
    "persistence_result_v1"
)
FAILURE_SCHEMA = (
    "lewm_go2_rgb_swept_progress_survival_joint_jepa_v4_matched_no_"
    "persistence_failure_v1"
)
...
PREREGISTRATION_COMMIT = "3dd4ca0680347f0a7f35d42d387781ecf53b1685"
...
def _fresh_output_root_v4_matched_no_persistence(repository_root: Path) -> Path:
    ...
    if output.exists() or output.is_symlink():
        raise FileExistsError("fresh matched-no-persistence attempt_v1 already exists")
...
def _validate_terminal_training_receipt_v1(
    ...
    for index, row in enumerate(trace, start=1):
        ...
        full_error = abs(
            float(losses["L_full_diagnostic"])
            - sum(float(losses[name]) for name in ("S", "P_diagnostic", "U", "R", "O"))
        )
        backward_error = abs(
            float(losses["L_backward"])
            - sum(float(losses[name]) for name in ("S", "U", "R", "O"))
        )
        if full_error > 1.0e-6 or backward_error > 1.0e-6:
            raise RuntimeError("matched control loss identity changed")
