...
import cv2
import torch
from lewm.observed_floor_contact_development import ObservedFloorContactGoalProbe,separate_ground_contact
...
from scripts.run_go2_auxiliary_downward45_goal_probe_v1 import OUTPUT as INPUT,CASES,CORRECTION,FITS
from scripts.read_go2_auxiliary_downward45_goal_probe_v1 import OUTPUT as READOUT
from scripts.diagnose_go2_downward45_paired_floor_coverage_v1 import OUTPUT as DIAGNOSTIC,INPUT_SHA,READOUT_SHA
...
OUTPUT=BASE/'go2_observed_floor_contact_prefix_v1_attempt_001'
PROTOCOL='docs/go2_observed_floor_contact_prefix_v1_2026-09-08.md'
DIAGNOSTIC_SHA='cbb2e078b0d4c6473640f2dbeaa633ab5f4d241a3e394c0d917ed280118dafb0'
...
def replay(admission,case):
    ...
    controller=ObservedFloorContactGoalProbe(model,ArticulatedCollisionGeometry(URDF),condition=c,variant=v,persistent=True)
    ...
    for i in range(len(reader.frames)):
        ...
        if a is not None and 'prediction' in a:
            ...
            for old_check,new_check in zip(a['surface_checks'],b['surface_checks'],strict=True):
                assert json.loads(json.dumps(separate_ground_contact(old_check)))==new_check,('only declared contact-policy change',i)
    ...
    return dict(decisions=records,model_state_sha256=before,first_terminal_policy_difference=first_terminal,
        ...
        causal_observations_maps_forecasts_and_nominal_checks_exact=True,only_declared_contact_policy_changed=True,
...
def main():
    ...
    bound=[]
    for root,sha,status in ((INPUT,INPUT_SHA,'AUXILIARY_DOWNWARD45_GOAL_PROBE_COMPLETE'),
            (READOUT,READOUT_SHA,'AUXILIARY_DOWNWARD45_GOAL_READOUT_COMPLETE'),
            (DIAGNOSTIC,DIAGNOSTIC_SHA,'DOWNWARD45_PAIRED_FLOOR_COVERAGE_COMPLETE')):
    ...
    sources=discover_sources((PROTOCOL,'scripts/replay_go2_observed_floor_contact_prefix_v1.py',
        'lewm/tests/test_observed_floor_contact_development.py',
        'docs/go2_downward45_paired_floor_coverage_result_2026-09-08.md'),bound[-1][2]['source_sha256'])
    resources=hardware()
    ...
    print('OBSERVED_FLOOR_CONTACT_PREFIX_LAUNCHED',digest(OUTPUT/'launch.json'),flush=True)
    try:
        ...
        for case in CASES:
            ...
            reports.append(report);print('OBSERVED_FLOOR_CONTACT_MODEL',report,flush=True)
        verify(launch)
        ...
        write_json(OUTPUT/'result.json',dict(status='OBSERVED_FLOOR_CONTACT_PREFIX_COMPLETE',source_sha256=sources,
        ...
        print('OBSERVED_FLOOR_CONTACT_PREFIX_COMPLETE',digest(OUTPUT/'result.json'),flush=True)
    except Exception as error:
        write_json(OUTPUT/'failure.json',dict(status='TERMINAL_OBSERVED_FLOOR_CONTACT_PREFIX_FAILURE',reason=repr(error)));raise
