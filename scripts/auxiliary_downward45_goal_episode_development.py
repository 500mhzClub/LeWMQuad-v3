...
def collect(trial, definition, *, output, model, geometry, persistent, episode_name, condition, variant):
    ...
    try:
        ...
        session._runtime['high_level'] = 'auxiliary_downward45_goal_probe_v1; online model plus measured surfaces'
        ...
        controller = AuxiliaryDownward45GoalProbe(model, geometry, persistent=persistent, condition=condition, variant=variant)
        try:
        ...
        result = dict(status='AUXILIARY_DOWNWARD45_GOAL_PROBE_TERMINAL_AUDIT_REQUIRED', trial=trial,
