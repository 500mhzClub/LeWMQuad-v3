...
No independent execution is possible without the completed eight-stage input
...
from scripts import independent_round_trip_eight_diagnostic_final_admission_development as admission
from scripts import run_go2_independent_round_trip_population_v1 as original
...
SOURCE='scripts/run_go2_independent_round_trip_population_eight_diagnostics_v1.py'
TEST='lewm/tests/test_independent_round_trip_eight_diagnostic_final_launcher_development.py'
PROTOCOL='docs/go2_independent_round_trip_population_eight_diagnostics_execution_v1_2026-09-11.md'
BUNDLE='docs/go2_independent_round_trip_eight_diagnostic_completed_input_bundle_v1.json'
OUTPUT=BASE/'go2_independent_round_trip_population_eight_diagnostics_v1_attempt_001'
...
def prepare_input_bundle(batch_sha,waiter_sha,*,sources):
    """Authenticate all eight outcomes before a separately written review."""
    admission.queue.owners_ended()
    old_names={'frontier','hold','contact','tracking','budget'}
    later_names={s.name for s in admission.queue.STAGES}
    if (type(waiter_sha) is not dict or set(waiter_sha)!=old_names|later_names
            or any(type(v) is not str or re.fullmatch('[0-9a-f]{64}',v) is None
                   for v in [batch_sha,*waiter_sha.values()])):
        raise ValueError('exact completed batch and all eight waiter identities required')
    previous=original.prepare_input_bundle(batch_sha,{k:waiter_sha[k] for k in old_names},sources=sources)
    joined=admission.queue.admit(previous['five_stage_queue_admission'],
        {k:waiter_sha[k] for k in later_names},sources=sources)
    admission.require_links(previous['input_admission'],joined)
    return dict(schema='independent_round_trip_eight_diagnostic_completed_input_bundle.v1',
        source_sha256=deepcopy(sources),input_admission=previous['input_admission'],
        eight_stage_queue_admission=joined,final_policy_review_completed=False,population_execution_permitted=False)


def make_launch(bundle,sources,review_sha,bundle_sha):
    if (set(bundle)!= {'schema','source_sha256','input_admission','eight_stage_queue_admission',
                     'final_policy_review_completed','population_execution_permitted'}
            or bundle['schema']!='independent_round_trip_eight_diagnostic_completed_input_bundle.v1'
    ...
    inputs=bundle['input_admission'];queue=bundle['eight_stage_queue_admission']
    ...
    launch.update(source_sha256=deepcopy(sources),protocol=PROTOCOL,output_root=str(OUTPUT),
        input_admission=deepcopy(inputs),eight_stage_queue_admission=deepcopy(queue),
...
def main():
    ...
    if args.source_preflight_only:
        print('INDEPENDENT_EIGHT_DIAGNOSTIC_POPULATION_SOURCE_PREFLIGHT_PASS',len(sources),json.dumps(measured),flush=True);return
    admission.queue.owners_ended()
    ...
    if args.preflight_only:
        print('INDEPENDENT_EIGHT_DIAGNOSTIC_POPULATION_COMPLETE_PREFLIGHT_PASS',len(sources),flush=True);return
    admission.runtime.require_native_idle()
    create_output(OUTPUT)
    try:
        write_json(OUTPUT/'launch.json',launch);sha=digest(OUTPUT/'launch.json')
        print('INDEPENDENT_EIGHT_DIAGNOSTIC_POPULATION_LAUNCHED',sha,flush=True)
        result=admission.overlap.run_population(OUTPUT,sha,admission.verify_population)
        print('INDEPENDENT_EIGHT_DIAGNOSTIC_POPULATION_RETURNED',result['status'],digest(OUTPUT/'result.json'),flush=True)
    except BaseException as error:
        path=OUTPUT/'failure.json'
        if not path.exists() and not path.is_symlink():
            write_json(path,dict(status='TERMINAL_INDEPENDENT_EIGHT_DIAGNOSTIC_POPULATION_LAUNCH_FAILURE',
