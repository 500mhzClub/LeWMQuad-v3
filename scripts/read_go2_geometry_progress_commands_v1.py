"""Separate raw readout with exact float64 requests and float32 actuator representation."""
import argparse
import json
import cv2
import numpy as np
...
OUTPUT=BASE/'go2_geometry_progress_command_readout_v1_attempt_001'
READOUT_PROTOCOL='docs/go2_geometry_progress_command_readout_v1_2026-09-07.md'
IDENTITIES={
...
def main():
    ...
    if OUTPUT.exists():raise ValueError('exclusive separate command-representation readout; no retry/resume')
    cv2.setNumThreads(1);verify_artifacts(INPUT,IDENTITIES)
    ...
    sources=discover_sources((READOUT_PROTOCOL,'scripts/read_go2_geometry_progress_commands_v1.py',
        'lewm/tests/test_geometry_progress_command_representation_development.py'),launch['source_sha256'])
    definition=launch|dict(source_sha256=sources);verify(definition);create_output(OUTPUT)
    write_json(OUTPUT/'launch.json',dict(status='SEPARATE_REPRESENTATION_READOUT_LAUNCHED',
        input_sha256=bindings,source_sha256=sources,protocol=READOUT_PROTOCOL,
        original_audit_remains_failed=True,changes='exact float64 request and float32-clipped applied-command identity',
    ...
    try:
        for c in TRIALS:
            ...
            print('RAW_COMMAND_REPRESENTATION_READOUT',c,report['outcome'],flush=True)
        verify(definition);verify_artifacts(INPUT,bindings)
        products=['launch.json']+[c+'_geometry_progress_audit.json' for c in TRIALS]
        write_json(OUTPUT/'result.json',dict(status='GEOMETRY_PROGRESS_COMMAND_REPRESENTATION_READOUT_COMPLETE',
            ...
            original_audit_remains_failed=True,
        ...
        write_json(OUTPUT/'failure.json',dict(status='TERMINAL_COMMAND_REPRESENTATION_READOUT_FAILURE',
