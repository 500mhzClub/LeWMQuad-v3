...
def compare(prior, current, prefix_root, prefix_report):
    ...
    checked=0; previous_candidate=None
    for i,(old,new,bound) in enumerate(zip(islice(read_rows(prior),frames),
        ...
        comparison=compare_current(old['decision'],new['decision'],previous_candidate=previous_candidate)
        ...
        previous_candidate=new['decision']; checked+=1
    ...
    return dict(common_prefix_frames=frames,first_changed_mission_behavior=changed,
        ...
        only_declared_settling_mission_and_target_reset_state_differs=True,
