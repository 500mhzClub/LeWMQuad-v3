#!/usr/bin/env python3
"""Compute-only exact verifier imported by the reviewed V13 executor.
...
def derived_raster_cell_nll_for_verification_v13(
    class_probabilities: Any,
    target_raster_labels: Any,
) -> Any:
    """Independent exact verifier implementation of the additive V13 term."""
    ...
    if (
        ...
        raise ValueError("V13 verifier raster NLL inputs changed")
    labels = target_raster_labels.to(dtype=torch.long)
    if bool((labels < 0).any().item()) or bool((labels >= 3).any().item()):
        raise ValueError("V13 verifier raster targets are invalid")
    ...
    if not bool(torch.isfinite(result).item()):
        raise FloatingPointError("V13 verifier raster NLL is non-finite")
    return result


def compute_gate_aligned_v13_losses_for_verification(
    model: Any,
    batch: Any,
) -> tuple[Any, Any, dict[str, Any], Any, Any, Any]:
    """Independently reconstruct the reviewed V13 objective and V4 branches."""
    ...
    cell_nll = derived_raster_cell_nll_for_verification_v13(
...
def recompute_evaluation(
    *,
    ...
    with torch.no_grad():
        for index in range(5):
            ...
            _batch_total, _v11_base, components, raw, targets, raster = (
                compute_gate_aligned_v13_losses_for_verification(model, batch)
            )
    ...
    if not torch.isclose(
        ...
        raise ValueError("V13 verifier raster NLL and metric accumulator disagree")
    return {
        "control": (
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    raise PermissionError(
        "V13 verifier is compute-only; use the reviewed lifecycle-owning executor"
    )
