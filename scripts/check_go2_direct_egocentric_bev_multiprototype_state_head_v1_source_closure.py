#!/usr/bin/env python3
"""Build or verify the Direct-BEV multiprototype V1 source closure."""
...
contract = _source_only_module(
    "_lewm_direct_bev_multiprototype_v1_closure_contract",
    CONTRACT_RELATIVE_PATH,
)
_V12 = _source_only_module(
    "_lewm_direct_bev_multiprototype_v1_frozen_v12_closure",
...
if (
    ...
    raise PermissionError(
        "Direct-BEV multiprototype V1 source-closure identity changed"
    )

MULTIPROTOTYPE_ADDITIVE_SOURCE_PATHS = frozenset({
...
if (
    ...
    raise PermissionError(
        "Direct-BEV multiprototype V1 source entrypoints changed"
    )
...
if not REQUIRED_DYNAMIC_SOURCE_PATHS.issubset(
    ...
    raise PermissionError(
        "Direct-BEV multiprototype V1 dynamic source roots are incomplete: "
        f"{missing}"
    )

# Rebind every inherited checker layer to the new 149-source identities.
...
def main() -> int:
    ...
    print("Go2 Direct BEV multiprototype state-head V1 closure: PASS")
    return 0
