...
import numpy as np
from scripts.run_go2_training_bias_goal_probe_v1 import OUTPUT as INPUT,PREVIOUS as PRIOR,PREVIOUS_SHA as PRIOR_SHA,CASES,TRIAL
...
OUTPUT=BASE/'go2_training_bias_goal_readout_v1_attempt_001'
PROTOCOL='docs/go2_training_bias_goal_readout_v1_2026-09-08.md'
...
def compare(a,b,*,translation_bias=None):
    ...
    if translation_bias is not None:
        bias=np.asarray(translation_bias,dtype=np.float32)
        if bias.shape!=(8,2) or not np.isfinite(bias).all():raise ValueError('invalid bound translation bias')
        for before,after in zip(*predictions,strict=True):
            if before is None or after is None:
                if before!=after:raise ValueError('common-prefix forecast availability changed')
                continue
            raw=np.asarray(before,dtype=np.float32);corrected=np.asarray(after,dtype=np.float32)
            if raw.shape!=(6,8,5) or corrected.shape!=raw.shape:raise ValueError('incomplete six-action forecast bank')
            expected=raw.copy();expected[...,:2]-=bias
            if not np.isfinite(expected).all() or not np.array_equal(expected,corrected):
                raise ValueError('common-prefix forecast differs from exact float32 training correction')
            checked+=1
        if checked==0:raise ValueError('no common-prefix correction witness')
    return dict(first=str(a),second=str(b),first_requested_command_difference=command,
        ...
        training_correction_expected=translation_bias is not None,
        exact_training_correction_verified=translation_bias is not None and checked>0,
        corrected_common_prefix_banks_verified=checked,
...
def main():
    ...
    assert result['status']=='TRAINING_BIAS_GOAL_PROBE_COMPLETE' and result['cases']==[list(c) for c in CASES]
    ...
    sources=discover_sources((PROTOCOL,'scripts/read_go2_training_bias_goal_probe_v1.py',
        'lewm/tests/test_training_bias_goal_readout_development.py'),old['source_sha256'])
    ...
    try:
        ...
        comparisons=[compare(PRIOR/c[0],INPUT/c[0],translation_bias=
            old['correction_admission']['coefficients'][c[4]]['heads'][
                'direct_outcomes' if c[3]=='direct' else 'rollout_outcomes']['applied_bias_xy_m']) for c in CASES]
        ...
        write_json(OUTPUT/'result.json',dict(status='TRAINING_BIAS_GOAL_READOUT_COMPLETE',
        ...
        print('TRAINING_BIAS_GOAL_READOUT_COMPLETE',digest(OUTPUT/'result.json'),flush=True)
    except Exception as error:
        write_json(OUTPUT/'failure.json',dict(status='TERMINAL_TRAINING_BIAS_GOAL_READOUT_FAILURE',reason=repr(error)));raise
