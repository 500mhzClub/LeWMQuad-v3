"""Four fresh native cases with nominal candidate-action geometry constraints."""
import argparse
import contextlib
import json
...
from scripts.nominal_action_goal_episode_development import collect, artifacts
from scripts.nominal_action_goal_audit_development import audit
...
from scripts.run_go2_continuous_connector_goal_probe_v1 import OUTPUT as DIAGNOSTIC
...
OUTPUT=BASE/'go2_nominal_action_goal_probe_v1_attempt_001'
READOUT=BASE/'go2_continuous_connector_goal_readout_v1_attempt_001'
DIAGNOSTIC_IDS={'launch.json':'a6ee9729c8943d3d20b485f7ca5d941f29ac675f2bd74b99833d65612fd47e4f'}
...
PROTOCOL='docs/go2_nominal_action_goal_probe_v1_2026-09-08.md'
...
def worker(case,launch_sha):
    ...
    terminal=dict(case=name,trial=trial,variant=variant,condition=condition,model_name=model_name,
        status='NOMINAL_ACTION_GOAL_WORKER_FAILED',artifact_sha256={})
    ...
    with (OUTPUT/(name+'_worker.log')).open('x') as log,contextlib.redirect_stdout(log),contextlib.redirect_stderr(log):
        try:
            ...
            terminal.update(status='NOMINAL_ACTION_GOAL_COLLECTED_AND_RAW_AUDITED',
...
def main():
    ...
    assert diagnostic['status']=='CONTINUOUS_CONNECTOR_GOAL_PROBE_COMPLETE' and len(diagnostic['conditions'])==4
    ...
    assert readout['status']=='CONTINUOUS_CONNECTOR_GOAL_READOUT_COMPLETE' and readout['probe_result_sha256']==args.prior_result_sha256
    ...
    launch=preflight(output=OUTPUT,protocol=PROTOCOL,
        seed_paths=('scripts/run_go2_nominal_action_goal_probe_v1.py',
            'lewm/tests/test_nominal_action_goal_probe_development.py',
            'docs/go2_continuous_connector_goal_probe_result_2026-09-08.md',*readout['source_sha256'],*refinement['source_sha256']),
    ...
    launch.update(all_six_admission=receipt,fit_artifact_sha256=fit_ids,snapshots=snapshots,
        ...
        experiment='nominal candidate-action constraint V1',
    ...
    print('NOMINAL_ACTION_GOAL_LAUNCHED',digest(OUTPUT/'launch.json'),flush=True)
    ...
    try:
                ...
                while pending:
                    ...
                    for future in done:
                        ...
                        try:record=future.result()
                            ...
                            record=dict(case=case[0],status='NOMINAL_ACTION_GOAL_PROCESS_FAILURE',failure=repr(exc))
                        records.append(record)
                        print('NOMINAL_ACTION_GOAL_TERMINAL',case[0],record['status'],record.get('goal'),flush=True)
            if len(records)!=4 or any(r['status']!='NOMINAL_ACTION_GOAL_COLLECTED_AND_RAW_AUDITED' for r in records):
        ...
        for root,ids in scaling['phase_result_sha256'].items():verify_artifacts(Path(root),ids)
        write_json(OUTPUT/'result.json',dict(status='NOMINAL_ACTION_GOAL_PROBE_COMPLETE',
        ...
        print('NOMINAL_ACTION_GOAL_COMPLETE',digest(OUTPUT/'result.json'),flush=True)
    except Exception as exc:
        write_json(OUTPUT/'failure.json',dict(status='TERMINAL_NOMINAL_ACTION_GOAL_FAILURE',reason=repr(exc),
