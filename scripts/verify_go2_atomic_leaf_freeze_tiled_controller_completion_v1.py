...
SOURCE = 'scripts/verify_go2_atomic_leaf_freeze_tiled_controller_completion_v1.py'
TEST = 'lewm/tests/test_atomic_leaf_freeze_tiled_controller_completion_development.py'
EXECUTION = 'docs/go2_atomic_leaf_freeze_tiled_controller_replay_execution_2026-09-11.json'
OUTPUT = ROOT/'docs/go2_atomic_leaf_freeze_tiled_controller_completion_verification_2026-09-11.json'
...
def expected_report(prior_report, timing):
    ...
    expected.update(baseline='TiledDensityProgressiveFloorController', candidate='AtomicLeafFreezeTiledController',
        timing_windows=timing, normalized_state_type_paths=run.STATE_TYPE_PATHS,
        incremental_reuse_comparison=False, incremental_atomic_leaf_freeze_comparison=True,
        persistent_memory_type_unchanged=True, both_controllers_use_tiled_geometry_and_original_cached_clone=True,
...
def main():
    ...
    if (result['status'] != 'ATOMIC_LEAF_FREEZE_TILED_LATE_HISTORY_V1_COMPLETE'
    ...
    expected_launch = dict(previous_result_sha256=run.PREVIOUS_RESULT_SHA,
        ...
        atomic_leaf_freeze_component_benchmark_sha256=run.MICROBENCHMARK_SHA,frames=1428,
        state_frames=list(run.original.STATE_FRAMES), normalized_state_type_paths=run.STATE_TYPE_PATHS,
        baseline='TiledDensityProgressiveFloorController', candidate='AtomicLeafFreezeTiledController',
    ...
    write_json(OUTPUT, dict(status='ATOMIC_LEAF_FREEZE_TILED_COMPLETION_VERIFIED',
    ...
    print('ATOMIC_LEAF_FREEZE_TILED_COMPLETION_VERIFIED', digest(OUTPUT), len(sources), reduction, flush=True)
