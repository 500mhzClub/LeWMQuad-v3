...
from lewm.course_aware_visual_servo_development import CourseAwareVisualServo,RULES
from lewm.course_aware_visual_servo_scene_development import specification
from lewm.support_friction_challenge_development import CONDITIONS,native_friction
...
from scripts.course_aware_servo_session_development import CourseServoSession
...
PREVIOUS=ROOT/'.generated/go2_visual_course_response_v1_attempt_001'
OUTPUT=ROOT/'.generated/go2_course_aware_visual_servo_v1_attempt_001'
PROTOCOL='docs/go2_course_aware_visual_servo_v1_2026-09-06.md'
IDENTITIES={'launch.json':'7453054ac7af37ef13c5b7bcbf35128bf41fca7ec79b06925bbcd5a7843d69e8',
    'result.json':'aa7ebf98e145a65cb4564c9bf318c0bc05ea733a12810e88d781e08edf767eed'}
...
def preflight():
    ...
    sources=discover_sources((PROTOCOL,'scripts/run_go2_course_aware_visual_servo_v1.py',
        'lewm/tests/test_visual_course_response_development.py',
        'lewm/tests/test_course_aware_servo_scene_development.py'),old['source_sha256'])
    ...
    launch|=dict(source_sha256=sources,input_sha256=inputs,protocol=PROTOCOL,controller_rules=RULES,
        conditions={c:specification(c) for c in CONDITIONS},minimum_free_bytes=10*1024**3,
        maximum_control_ticks=350,maximum_terminal_zero_ticks=10,model='gyro',
...
def collect(condition,definition):
    ...
    try:
        session=CourseServoSession(spec,directory); session.install_contact_identity(); build=session.ctx.build
        ...
        motion=VisualLedMotion('gyro',identity=(0,0,0)); controller=CourseAwareVisualServo()
...
def main():
    ...
    try:
        ...
        write_json(OUTPUT/'result.json',dict(status='COURSE_AWARE_VISUAL_SERVO_COLLECTION_TERMINAL',conditions=results,
