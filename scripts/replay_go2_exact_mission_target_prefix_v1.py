...
from scripts.run_go2_observed_floor_contact_goal_probe_v1 import OUTPUT as INPUT,CASES,CORRECTION,FITS
from scripts.read_go2_observed_floor_contact_goal_probe_v1 import OUTPUT as READOUT
...
OUTPUT=BASE/'go2_exact_mission_target_prefix_v1_attempt_001'
PROTOCOL='docs/go2_exact_mission_target_prefix_v1_2026-09-08.md'
INPUT_SHA='aab193a96edcee6fa2e15a62aab333821f6688995ca8f9e68cdc8887eaf403d9'
READOUT_SHA='aaa0b6202883a22b46acdfe9873414cf76c4b159767bebea1e20263ad9f476dc'
...
def replay(admission,case):
    ...
    controller=ExactMissionTargetGoalProbe(model,ArticulatedCollisionGeometry(URDF),condition=c,variant=v,persistent=True)
    ...
    return dict(decisions=records,model_state_sha256=before,first_terminal_policy_difference=first_terminal,
        ...
        causal_observations_maps_forecasts_and_nominal_checks_exact=True,surface_checks_exact=True,only_final_goal_target_and_associated_scores_changed=True,
...
def main():
    ...
    bound=[]
    for root,sha,status in ((INPUT,INPUT_SHA,'OBSERVED_FLOOR_CONTACT_GOAL_PROBE_COMPLETE'),
            (READOUT,READOUT_SHA,'OBSERVED_FLOOR_CONTACT_GOAL_READOUT_COMPLETE')):
    ...
    sources=discover_sources((PROTOCOL,'scripts/replay_go2_exact_mission_target_prefix_v1.py',
        'lewm/tests/test_exact_mission_target_development.py','lewm/tests/test_exact_mission_target_goal_scope_development.py',
        'docs/go2_observed_floor_contact_goal_probe_result_2026-09-08.md'),bound[-1][2]['source_sha256'])
    resources=hardware()
    ...
    print('EXACT_MISSION_TARGET_PREFIX_LAUNCHED',digest(OUTPUT/'launch.json'),flush=True)
    try:
        ...
        for case in CASES:
            ...
            report={k:v for k,v in first.items() if k!='decisions'}|dict(case=case[0],model=case[4],
                ...
                exact_goal_target_ticks=[dict(frame=i,**r['new_selection']['terminal_goal_target_evidence'])
                    for i,r in enumerate(rows) if (r['new_selection'] or {}).get('intermediate_target_is_mission_goal')],
                controller_failures=[dict(frame=i,failure=r['failure']) for i,r in enumerate(rows) if r['failure']])
            reports.append(report);print('EXACT_MISSION_TARGET_MODEL',report,flush=True)
        verify(launch)
        ...
        write_json(OUTPUT/'result.json',dict(status='EXACT_MISSION_TARGET_PREFIX_COMPLETE',source_sha256=sources,
        ...
        print('EXACT_MISSION_TARGET_PREFIX_COMPLETE',digest(OUTPUT/'result.json'),flush=True)
    except Exception as error:
        write_json(OUTPUT/'failure.json',dict(status='TERMINAL_EXACT_MISSION_TARGET_PREFIX_FAILURE',reason=repr(error)));raise
