"""Two fixed short-horizon models with eight-step nominal path planning."""
import argparse
import contextlib
import json
...
from scripts.eight_step_planning_goal_episode_development import collect,artifacts
from scripts.eight_step_planning_goal_audit_development import audit
from scripts.observation_horizon_model_admission_development import admit,load_assigned
from scripts.run_go2_observation_horizon_fits_v1 import OUTPUT as FITS
from scripts.read_go2_observation_horizon_fits_v1 import OUTPUT as READOUT
...
PREVIOUS=BASE/'go2_observation_horizon_goal_probe_v1_attempt_001'
DIAGNOSTIC=BASE/'go2_observation_horizon_goal_readout_v1_attempt_001'
PREVIOUS_SHA='f484a0c3b95cb2f3d0868dca1f1f40d11ba0d29c42e12cc9604061870a0d5608'
DIAGNOSTIC_SHA='8bce2ae56ed150872b28add57a694ddc685ee6ff92ba77a16aa8aaf54c9a3cb7'
OUTPUT=BASE/'go2_eight_step_planning_goal_probe_v1_attempt_001'
...
PROTOCOL='docs/go2_eight_step_planning_goal_probe_v1_2026-09-08.md'
...
def causal_prefix(directory):
    ...
    for i in range(4):values['packet/'+str(i)]=fingerprint(reader.packet(i))
    return values
...
def worker(case,launch_sha):
    ...
    terminal=dict(case=name,trial=trial,variant=variant,condition=condition,model_name=model_name,
        status='EIGHT_STEP_PLANNING_GOAL_WORKER_FAILED',artifact_sha256={});started=time.perf_counter()
    with (OUTPUT/(name+'_worker.log')).open('x') as log,contextlib.redirect_stdout(log),contextlib.redirect_stderr(log):
        try:
            ...
            model,c,v=load_assigned(launch['all_eighteen_admission'],model_name);assert (c,v)==(condition,variant)
            ...
            replay_model,c,v=load_assigned(launch['all_eighteen_admission'],model_name);assert (c,v)==(condition,variant)
            ...
            verify_artifacts(FITS,{'result.json':launch['fit_result_sha256'],**launch['all_eighteen_admission']['fit_artifact_sha256']})
            assert digest(URDF)==launch['robot_urdf_sha256']
            terminal.update(status='EIGHT_STEP_PLANNING_GOAL_COLLECTED_AND_RAW_AUDITED',artifact_sha256=bindings,
...
def main():
    parser=argparse.ArgumentParser();parser.add_argument('--fit-result-sha256',required=True)
    parser.add_argument('--fit-readout-sha256',required=True);args=parser.parse_args()
    ...
    assert previous['status']=='OBSERVATION_HORIZON_GOAL_PROBE_COMPLETE' and previous['cases']==[list(c) for c in CASES]
    ...
    assert diagnostic['status']=='OBSERVATION_HORIZON_GOAL_READOUT_COMPLETE'
    ...
    admitted=admit(args.fit_result_sha256)
    verify_artifacts(READOUT,{'result.json':args.fit_readout_sha256});readout=read_json(READOUT,'result.json')
    if readout['status']!='OBSERVATION_HORIZON_FIT_READOUT_COMPLETE' or readout['final_models_admitted']!=18:
        raise ValueError('complete full-fit readout required')
    readout_ids={'result.json':args.fit_readout_sha256,**readout['artifact_sha256']};verify_artifacts(READOUT,readout_ids)
    fit_readout_launch=read_json(READOUT,'launch.json');verify(fit_readout_launch)
    if fit_readout_launch['all_eighteen_admission']!=admitted:raise ValueError('unchanged complete fit admission required')
    ...
    launch=preflight(output=OUTPUT,protocol=PROTOCOL,
        seed_paths=('scripts/run_go2_eight_step_planning_goal_probe_v1.py',
            'lewm/tests/test_eight_step_planning_goal_scope_development.py',
            'docs/go2_observation_horizon_goal_probe_result_2026-09-08.md',
            *previous['source_sha256'],*diagnostic['source_sha256'],
            'docs/go2_observation_horizon_fits_result_2026-09-08.md',
    ...
    for source in (prior,prior_readout,readout,previous,diagnostic):
        for name,h in source['source_sha256'].items():assert launch['source_sha256'].get(name)==h,name
    launch.update(previous_probe_artifact_sha256=previous_ids,previous_readout_artifact_sha256=diagnostic_ids,
        all_eighteen_admission=admitted,fit_result_sha256=args.fit_result_sha256,fit_readout_sha256=readout_ids,
        ...
        prior_causal_prefix=causal_prefix(PRIOR/('full_direct_'+TRIAL)),planned_cases=CASES,
        ...
        experiment='fixed short-horizon models; score waypoint progress at 800 ms, check all eight nominal forecast segments, and execute one 100-ms command',
        concurrency_reason='two independent scenes; bounded by the passing four-process native benchmark and completed short-horizon two-scene probe',
    ...
    print('EIGHT_STEP_PLANNING_GOAL_LAUNCHED',digest(OUTPUT/'launch.json'),flush=True)
    ...
    try:
            ...
            with ProcessPoolExecutor(max_workers=2,mp_context=multiprocessing.get_context('spawn'),max_tasks_per_child=1) as pool:
                ...
                while pending:
                    ...
                    for future in done:
                        ...
                        except Exception as error:row=dict(case=case[0],status='EIGHT_STEP_PLANNING_GOAL_PROCESS_FAILURE',failure=repr(error))
                        records.append(row);print('EIGHT_STEP_PLANNING_GOAL_TERMINAL',case[0],row['status'],row.get('goal'),flush=True)
        if len(records)!=2 or any(r['status']!='EIGHT_STEP_PLANNING_GOAL_COLLECTED_AND_RAW_AUDITED' for r in records):
        ...
        verify_artifacts(FITS,{'result.json':args.fit_result_sha256,**admitted['fit_artifact_sha256']})
        result=dict(status='EIGHT_STEP_PLANNING_GOAL_PROBE_COMPLETE',cases=CASES,conditions=records,
        ...
        write_json(OUTPUT/'result.json',result);print('EIGHT_STEP_PLANNING_GOAL_COMPLETE',digest(OUTPUT/'result.json'),flush=True)
    except Exception as error:
        write_json(OUTPUT/'failure.json',dict(status='TERMINAL_EIGHT_STEP_PLANNING_GOAL_FAILURE',reason=repr(error),
