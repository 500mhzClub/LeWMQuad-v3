#!/usr/bin/env python3
"""Build or verify the semantic-anchor V3 gate-timing source closure."""
...
contract = _source_only_module(
    "_lewm_direct_bev_semantic_anchor_v3_timing_closure_contract",
    CONTRACT_RELATIVE_PATH,
)
_V2 = _source_only_module(
    "_lewm_direct_bev_semantic_anchor_v3_timing_frozen_v2_closure",
    FROZEN_V2_SOURCE_CLOSURE_CHECKER_RELATIVE_PATH,
)
_FROZEN_V2_CONTRACT = _V2.contract
if (
    contract.CONTRACT_RELATIVE_PATH != CONTRACT_RELATIVE_PATH
    or _V2.CONTRACT_RELATIVE_PATH != FROZEN_V2_CONTRACT_RELATIVE_PATH
    or _FROZEN_V2_CONTRACT.CONTRACT_RELATIVE_PATH
    != FROZEN_V2_CONTRACT_RELATIVE_PATH
    or _FROZEN_V2_CONTRACT.SOURCE_CLOSURE_CHECKER_RELATIVE_PATH
    != FROZEN_V2_SOURCE_CLOSURE_CHECKER_RELATIVE_PATH
    or tuple(contract.REUSED_SOURCE_PATHS)
    != tuple(_FROZEN_V2_CONTRACT.SOURCE_PATHS)
):
    raise PermissionError("semantic-anchor V3 frozen-V2 closure identity changed")

V3_ADDITIVE_SOURCE_PATHS = frozenset({
...
if (
    set(contract.ADDITIVE_SOURCE_PATHS) != V3_ADDITIVE_SOURCE_PATHS
    or len(contract.ADDITIVE_SOURCE_PATHS) != 5
    or len(contract.REUSED_SOURCE_PATHS) != 160
    or len(contract.SOURCE_PATHS) != 165
    or contract.MODEL_RELATIVE_PATH
    != _FROZEN_V2_CONTRACT.MODEL_RELATIVE_PATH
    or contract.MODEL_RELATIVE_PATH in V3_ADDITIVE_SOURCE_PATHS
    or set(contract.REUSED_SOURCE_PATHS) & V3_ADDITIVE_SOURCE_PATHS
    or set(contract.SOURCE_PATHS)
    != set(contract.REUSED_SOURCE_PATHS) | V3_ADDITIVE_SOURCE_PATHS
):
    raise PermissionError("semantic-anchor V3 five-file source surface changed")
if (
    ...
    raise PermissionError("semantic-anchor V3 source entrypoints changed")

REQUIRED_DYNAMIC_SOURCE_PATHS = frozenset({
    *_V2.REQUIRED_DYNAMIC_SOURCE_PATHS,
    *V3_ADDITIVE_SOURCE_PATHS,
    FROZEN_V2_CONTRACT_RELATIVE_PATH,
    FROZEN_V2_SOURCE_CLOSURE_CHECKER_RELATIVE_PATH,
...
if not REQUIRED_DYNAMIC_SOURCE_PATHS.issubset(
    ...
    raise PermissionError(
        f"semantic-anchor V3 dynamic source roots are incomplete: {missing}"
    )

# Rebind every inherited discovery layer to the exact V3 165-source family.
layers = [_V2, *_V2.layers]
for layer in layers:
...
def main() -> int:
    ...
    mode.add_argument(
        "--emit",
        action="store_true",
        help="build a candidate without requiring a frozen V3 manifest",
    )
    mode.add_argument(
        "--write",
        action="store_true",
    ...
    print(
        "Go2 Direct BEV signed-boundary semantic-anchor state V3 update-100 "
        "trend-gate timing closure: PASS"
    )
    return 0
