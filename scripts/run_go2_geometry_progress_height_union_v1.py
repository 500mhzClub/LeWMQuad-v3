"""Twenty-four randomized training-only ordered-near-field geometry/progress excitation episodes."""
import shutil
import time
import cv2
from lewm.geometry_progress_near_field_development import TRIALS,specification,schedule,decision,WARMUP_TICKS,assignments
...
from scripts.geometry_progress_height_union_session_development import GeometryProgressHeightUnionSession
...
PREVIOUS=BASE/'go2_native_raster_edge_crossing_v1_attempt_001'
OLD_PILOT=BASE/'go2_geometry_progress_pilot_v1_attempt_001'
OLD_SCIENCE=BASE/'go2_geometry_progress_science_v1_attempt_001'
OLD_VISIBILITY=BASE/'go2_geometry_progress_physical_visibility_v1_attempt_001'
OUTPUT=BASE/'go2_geometry_progress_height_union_v1_attempt_001'
PROTOCOL='docs/go2_geometry_progress_height_union_v1_2026-09-07.md'
BENCH=BASE/'go2_geometry_progress_variable_height_render_bench_v1_attempt_001'
BENCH_IDS={'launch.json':'3d2a15124095c632e7b5d64c5e7663377c0d2f1258d01dbf9946a3e69d6849f4',
    'result.json':'8cfb8b30f67514a4f64490eea32eb3f188e57a32f57894d29dc29f0b20231738'}
RESERVE=40*1024**3


def preflight():
    verify_artifacts(PREVIOUS,{'launch.json':'b19ab3392287f2eda9b3d51e043b14a4454cbd66d07a6cb2eff3863ae61ffb6b'})
    ...
    visibility_ids={'result.json':'9bb467653bc6e733c185af103180b28191723d2e57cd98db0e3377a99ecce85b'}
    verify_artifacts(OLD_SCIENCE,science_ids);verify_artifacts(OLD_VISIBILITY,visibility_ids)
    science=read_json(OLD_SCIENCE,'result.json');visibility=read_json(OLD_VISIBILITY,'result.json')
    if not science['panel']['informative_for_next_dataset'] or science['design_and_measurement_gate_pass']:
        raise ValueError('informative but measurement-failed predecessor required')
    if visibility['failed_future_target_frames']!=4:raise ValueError('exact predecessor physical-visibility failure witness required')
    inherited=dict(old['source_sha256'])
    for n,h in visibility['source_sha256'].items():
        if n in inherited and inherited[n]!=h:raise ValueError('conflicting source identities')
        inherited[n]=h
    verify_artifacts(BENCH,BENCH_IDS);bench=read_json(BENCH,'result.json')
    if bench['status']!='VARIABLE_HEIGHT_NATIVE_RENDER_BENCH_COMPLETE' or not bench['passed'] or bench['frames']!=20 or bench['physics_steps']!=0:
        raise ValueError('completed variable-height native rendering bench required')
    verify_artifacts(BENCH,bench['artifact_sha256'])
    for n,h in bench['source_sha256'].items():
        if n in inherited and inherited[n]!=h:raise ValueError('conflicting bench source identities')
        inherited[n]=h
    sources=discover_sources((PROTOCOL,'scripts/run_go2_geometry_progress_height_union_v1.py',
        'scripts/audit_go2_geometry_progress_height_union_v1.py',
        'lewm/tests/test_geometry_progress_height_union_development.py'),inherited)
    resources=hardware()
    if resources['memory_available_bytes']<24*1024**3:raise ValueError('24GiB available RAM required')
    launch={k:old[k] for k in ('native_sha256','native_scene_sha256','native_geometry_sha256','opencv_binary_sha256','opencv_version','rules')}
    launch.update(source_sha256=sources,input_sha256=old['input_sha256'],conditions={c:specification(c) for c in TRIALS},
        hardware=resources,native_scene_workers=1,opencv_threads=1,
        variable_height_render_bench_sha256=BENCH_IDS,visual_surface_provider='variable_height_union_surface_v1',
        predecessor_science_sha256=science_ids,predecessor_visibility_sha256=visibility_ids,
        render_near_m=.005,raster_order='floor_first',eligibility_contract='PROSPECTIVE_RGB_BODY_STABLE_FOOTPRINT_NO_NEAR_OCCLUSION',
        concurrency_reason='small native pilot; no established parallel render determinism or scaling',
        protocol=PROTOCOL,output_root=str(OUTPUT),maximum_command_ticks=43,minimum_free_bytes=RESERVE,
        planned_storage_bytes=3*1024**3,tracker_required_for_commands=False,model_training=False,
        data_role='train',independent_layouts=0,geometry_conditions=2,appearance_seeds=2,
        randomized_assignment=assignments(),same_observation_counterfactual_claim=False,
        physics_paused_during_compute=True,evaluation_layouts=0,real_time_qualified=False,
...
def collect(trial,definition):
    ...
    try:
        session=GeometryProgressHeightUnionSession(spec,directory);session.install_contact_identity();build=session.ctx.build
        ...
        print('GEOMETRY_PROGRESS_HEIGHT_UNION',trial,result,flush=True);return result
    finally:
...
def main():
    if OUTPUT.exists() or OUTPUT.is_symlink():raise ValueError('exclusive fixed geometry-progress pilot; no retry/resume')
    ...
    try:
        ...
        write_json(OUTPUT/'result.json',dict(status='GEOMETRY_PROGRESS_HEIGHT_UNION_COLLECTION_TERMINAL',conditions=results,
            planned_trials=list(TRIALS),absent_expected_artifacts=sorted(set(names)-set(present)),artifact_sha256=bindings,
            independent_layouts=0,data_role='train',evaluation_layouts=0,model_trained=False,navigation_qualified=False,goal_achieved=False))
