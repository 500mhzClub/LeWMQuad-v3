...
"""Fixed source-capturing entrypoint for Shared JEPA V5 finalizers."""
...
CANONICAL_G2_FINALIZER_AUTHORITY_FILE_SHA256: str | None = None
CANONICAL_G3_FINALIZER_AUTHORITY_FILE_SHA256: str | None = None
...
def _main() -> int:
    if not sys.flags.isolated or not sys.flags.no_user_site:
        raise PermissionError("V5 finalizer wrapper requires isolated Python")
    if Path(__file__).resolve() != CANONICAL_WRAPPER_PATH:
        raise PermissionError("V5 finalizer wrapper was executed from a copied path")
    wrapper_bytes, wrapper_hash = _read_source(
        CANONICAL_WRAPPER_PATH,
        name="V5 finalizer wrapper",
    )
    del wrapper_bytes
    ...
    launcher = ModuleType("_lewm_go2_shared_jepa_v5_captured_finalizer_launcher")
