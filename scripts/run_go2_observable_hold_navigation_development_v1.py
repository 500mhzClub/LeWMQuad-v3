...
from lewm.observable_hold_navigation_development import ObservableHoldNavigation as WholeTaskNavigation
...
from scripts.run_go2_release_aware_navigation_development_v1 import preflight as prior_preflight, OUTPUT as PRIOR, verify_native

OUTPUT = ROOT/'.generated/go2_observable_hold_navigation_development_v1_attempt_001'
SCHEMA = 'observable_hold_navigation_development.v1'
NEW_SOURCES = (
    'lewm/observable_approach_development.py',
    'lewm/observable_hold_navigation_development.py',
    'lewm/tests/test_observable_approach_development.py',
    'lewm/tests/test_observable_hold_navigation_development.py',
    'scripts/run_go2_observable_hold_navigation_development_v1.py',
    'scripts/audit_go2_observable_hold_navigation_development_v1.py',
    'scripts/audit_observable_hold_rgb_core_development.py',
    'lewm/tests/test_observable_hold_execution_development.py',
    'docs/go2_observable_hold_navigation_development_v1_2026-09-05.md',
    'docs/go2_release_aware_navigation_development_v1_result_2026-09-05.md')
...
def trial_specs():
    ...
    for spec in original_specs():
        ...
        spec['scene_id'] = 'go2-observable-hold-navigation-development-v1-'+spec['layout_name']
        spec['method'] = 'observable_hold'
        result.append(spec)
    return result
...
def preflight():
    ...
    if (inherited != launch['source_sha256'] or audit['status'] != 'PASS' or audit['audited_trials'] != 2):
        raise ValueError('source-bound completed release-aware audit required; all scientific failures retained')
...
def main():
    ...
    write_json(OUTPUT/'launch.json', {'schema': SCHEMA, 'trial_specs': specs,
        ...
        'scope': 'two continuous blocker/view-constrained approach and active-heading-hold missions; sampled nominal volume and unqualified future gait, no JEPA/hardware qualification'})
    rows, prefixes = [], {}
    try:
        for spec in specs:
