#!/usr/bin/env python3
"""One-shot controller for the preregistered V28 joint JEPA probe.
...
route, its validation controls, and the V28 terminal gate.
"""
...
PREREGISTRATION_PATH = (
    ...
    "joint_jepa_v28_preregistration_2026-07-30.md"
)
PREREGISTRATION_COMMIT = "5d315ad67d2ab3c55152978a167d63eee4fa9cb2"
SOURCE_MANIFEST_RELATIVE_PATH = (
    "docs/lewm_go2_rgb_object_space_explicit_plan_terminal_successor_state_"
    "joint_jepa_v28_source_manifest_2026-07-30.json"
)
SOURCE_REVIEW_RELATIVE_PATH = (
    "docs/lewm_go2_rgb_object_space_explicit_plan_terminal_successor_state_"
    "joint_jepa_v28_source_review_2026-07-30.json"
)
...
TRAINING_MODULE_NAME = (
    "scripts.run_go2_rgb_object_space_explicit_plan_terminal_successor_state_"
    "joint_jepa_v28"
)
...
def _binding(value: Any, *, name: str) -> dict[str, Any]:
    ...
    if (
        ...
        raise TypeError(f"V28 {name} binding changed")
    return result


def validate_future_execution_prerequisites_v28(
    authority: Any,
) -> dict[str, Any]:
    value = validate_content_bound_v28(authority)
    required = {
    ...
    if any(value.get(name) != expected for name, expected in required.items()):
        raise PermissionError("V28 authority identity or terminal bounds changed")
    if value.get("preregistration_commit") != PREREGISTRATION_COMMIT:
        raise PermissionError("V28 authority is not bound to the final preregistration")
    ...
    if selectors != {
        ...
        raise PermissionError("V28 runtime selectors changed")
    if value.get("runtime_data_root") != str(Path(__file__).resolve().parents[1]):
        ...
        if value.get("runtime_data_root") != expected_data_root:
            raise PermissionError("V28 runtime-data root changed")
    ...
    if not isinstance(certification, Mapping) or set(certification) != {
        ...
        raise PermissionError("V28 authority lacks its clean-export certification")
    ...
    if not isinstance(value.get("runtime_inputs"), Mapping) or any(
        ...
        raise PermissionError("V28 inherited runtime bindings are incomplete")
    ...
    if value.get("rgb_root_relative_path") != (
        ...
        raise PermissionError("V28 H6 RGB root changed")
    return value


validate_content_bound_v13 = validate_content_bound_v28
validate_future_execution_prerequisites_v13 = (
    validate_future_execution_prerequisites_v28
)
...
def _read_certification(repository_root: Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise PermissionError("V28 clean-export certification is invalid") from error
    value = validate_content_bound_v28(value)
    if (
...
def validate_bound_sources_v28(repository_root: Path) -> dict[str, Any]:
    ...
    if canonical != certification.get("bindings_sha256"):
        raise PermissionError("V28 source inventory binding changed")
    ...
    for raw_binding in bindings:
        ...
        if (
            ...
            raise PermissionError("V28 certified source path escaped or changed type")
...
validate_bound_sources_v13 = validate_bound_sources_v28


def reserve_attempt_v28(
    ...
    *,
    ...
    root = Path(repository_root).resolve(strict=True)
    validated = validate_future_execution_prerequisites_v28(dict(authority))
    ...
    if (
        ...
        raise PermissionError("V28 reserved attempt root mode changed")
    core = {
    ...
    if (
        ...
        raise PermissionError("V28 reservation file mode changed")
    return reservation


def validate_attempt_reservation_v28(value: Any) -> dict[str, Any]:
    reservation = validate_content_bound_v28(value)
    if (
        ...
        raise PermissionError("V28 reservation identity changed")
    return reservation


reserve_attempt_v13 = reserve_attempt_v28
validate_attempt_reservation_v13 = validate_attempt_reservation_v28


def terminalize_failure_v28(
    output_root: Path,
    ...
    *,
    ...
    validate_attempt_reservation_v28(dict(reservation))
    ...
    if checkpoint_path.exists() or checkpoint_path.is_symlink():
        ...
        if checkpoint_path.is_symlink() or not stat.S_ISREG(info.st_mode):
            raise PermissionError("V28 partial checkpoint changed type")
        ...
        if partial_checkpoint_binding is not None:
            ...
            if supplied_binding != observed_binding:
                raise PermissionError("V28 partial checkpoint binding changed")
        ...
        if stat.S_IMODE(os.lstat(checkpoint_path).st_mode) != 0o000:
            raise PermissionError("V28 partial checkpoint quarantine failed")
        checkpoint_quarantined = True
    elif partial_checkpoint_binding is not None:
        raise PermissionError("V28 bound partial checkpoint is absent")
    core = {
...
terminalize_failure_v13 = terminalize_failure_v28
...
def _mapping(value: Any, *, name: str) -> dict[str, Any]:
    if is_dataclass(value) and not isinstance(value, type):
        ...
        raise TypeError(f"V28 {name} must be a dataclass or mapping")
    return result


def _finite(value: Any, *, name: str) -> float:
    if isinstance(value, bool) or not isinstance(value, (int, float)):
        raise TypeError(f"V28 {name} must be numeric")
    result = float(value)
    if not math.isfinite(result):
        raise FloatingPointError(f"V28 {name} is nonfinite")
    return result


def validate_update_integrity_v28(
    runtime: Any,
    model: Any,
    result: Any,
    *,
    ...
    if accounting != expected:
        raise RuntimeError("V28 update accounting changed")
    route_names = (
        "camera_shared",
        "joint_shared",
        "representation",
    ...
    if tuple(routes) != route_names:
        raise RuntimeError("V28 gradient route order or membership changed")
    for name, route in routes.items():
        ...
        if route.get("absent_tensor_gradient_count") != 0 or preclip < 0.0:
            raise RuntimeError(f"V28 route {name} has an absent or invalid gradient")
        if name != "representation" and not preclip > 0.0:
            raise RuntimeError(f"V28 required route {name} is zero")
        if not 0.0 < scale <= 1.0:
            raise RuntimeError(f"V28 route {name} clipping scale changed")
    losses = {
        name: _finite(value, name=f"loss {name}")
        for name, value in result.mean_losses.items()
    }
    if not {"C", "N28", "J24", "P28", "L28"}.issubset(losses):
        raise RuntimeError("V28 required loss receipt is incomplete")
    ...
    if (
        ...
        raise RuntimeError("V28 plan diagnostics changed")
    if (
        ...
        raise RuntimeError("V28 target, optimizer, or EMA integrity failed")
    for value in model.state_dict().values():
        if value.is_floating_point() and not bool(runtime.torch.isfinite(value).all()):
            raise FloatingPointError("V28 model state became nonfinite")
    return {
...
def evaluate_gate_v28(
    *,
    ...
    if not isinstance(advantages, Mapping) or tuple(advantages) != PLAN_METRIC_NAMES:
        raise RuntimeError("V28 registered plan metrics changed")
...
def _publish_json(
    ...
    if type(result) is not dict or set(result) != {"value", "binding"}:
        raise RuntimeError("V28 publisher JSON result changed")
    return validate_content_bound_v28(result["value"]), dict(result["binding"])


def _serialize_checkpoint_v28(
    runtime: Any,
    model: Any,
    optimizer: Any,
    accounting: Any,
...
def run_future_authorized_engine_v28(
    *,
    authority: Mapping[str, Any],
    reservation: Mapping[str, Any],
    runtime: Any,
    ...
    validated_authority = validate_future_execution_prerequisites_v28(
        dict(authority)
    )
    validated_reservation = validate_attempt_reservation_v28(dict(reservation))
    if validated_reservation["authority_sha256"] != hashlib.sha256(
        _canonical_json_bytes(validated_authority)
    ).hexdigest():
        raise PermissionError("V28 reservation does not bind supplied authority")

    evaluation = __import__(EVALUATION_MODULE_NAME, fromlist=["*"])
    h6_runtime = evaluation.load_v28_h6_runtime(runtime.runtime_data_root)
    ...
    try:
        ...
        for update in range(0, MAXIMUM_UPDATES + 1):
            if update:
                ...
                result = runtime.training_module.joint_training_update_v28(
                    model,
                    optimizer,
                    physical_batches,
                    plan_batches,
                ...
                integrity = validate_update_integrity_v28(
        ...
        if accounting is None or set(observations) != set(OBSERVATION_UPDATES):
            raise RuntimeError("V28 controller did not complete its exact schedule")
        stage = "classify_update400"
        gate = evaluate_gate_v28(
        ...
        checkpoint_raw, checkpoint_core = _serialize_checkpoint_v28(
        ...
        return terminalize_failure_v28(
...
run_future_authorized_engine_v13 = run_future_authorized_engine_v28
...
__all__ = [
    ...
    "evaluate_gate_v28",
    "reserve_attempt_v28",
    "run_future_authorized_engine_v28",
    "terminalize_failure_v28",
    "validate_bound_sources_v28",
    "validate_content_bound_v28",
    "validate_future_execution_prerequisites_v28",
    "validate_update_integrity_v28",
]
