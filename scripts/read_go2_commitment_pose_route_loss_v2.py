...
OUTPUT=BASE/'go2_commitment_pose_route_loss_v2_attempt_001'
PROTOCOL='docs/go2_commitment_pose_route_loss_v2_2026-09-08.md'
...
FAILED=BASE/'go2_commitment_pose_route_loss_v1_attempt_001'
FAILED_IDS={'launch.json':'6df147aafb8fa5f2d20f75e286a41bebeb2afe3fc2b2242a7c9f67afcc60db04',
    'failure.json':'427d8c17bdc982aa8124c9082a3a6c629ec6cb01566432fc03d831a3de432e4d'}
...
def main():
    ...
    sources=discover_sources((PROTOCOL,'scripts/read_go2_commitment_pose_route_loss_v2.py',
        'lewm/tests/test_recorded_visual_evidence_identity_development.py'),failed['source_sha256'])
    launch=original|dict(source_sha256=sources,protocol=PROTOCOL,output_root=str(OUTPUT),
        ...
        diagnostic_cases=CASES,failed_diagnostic_sha256=FAILED_IDS,hardware=h,native_execution=False,maximum_output_bytes=1024**3)
    verify(launch);assert digest(URDF)==launch['robot_urdf_sha256']
    smoke=[replay(c,8) for c in CASES]
    launch['bounded_decode_preflight']=[dict(case=r['case'],receipts=r['available_map_receipts_reconstructed'],
        reconstruction_sha256=hashlib.sha256(json.dumps(r,sort_keys=True).encode()).hexdigest()) for r in smoke]
    ...
    try:
        ...
        verify(launch);verify_artifacts(INPUT,inputs);verify_artifacts(READOUT,readout_ids);verify_artifacts(FAILED,FAILED_IDS)
