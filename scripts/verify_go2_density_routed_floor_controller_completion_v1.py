...
from scripts import replay_go2_density_routed_floor_late_history_v1 as run
...
SOURCE = 'scripts/verify_go2_density_routed_floor_controller_completion_v1.py'
TEST = 'lewm/tests/test_density_routed_floor_controller_completion_development.py'
EXECUTION = 'docs/go2_density_routed_floor_controller_replay_execution_2026-09-11.json'
EXECUTION_SHA = '6a53578c31871e48ccaebeb17a80f0e0093ec2bb1cc01dae29714301b11dd8f8'
OUTPUT = ROOT/'docs/go2_density_routed_floor_controller_completion_verification_2026-09-11.json'
...
def expected_report(prior_report, timing):
    ...
    for key in ('incremental_empty_patch_visibility_skip_comparison', 'both_controllers_use_receipt_copied_packed_fused_queries'):
        if expected.pop(key) is not True: raise ValueError('original visibility-batched comparison scope required')
    expected.update(baseline='VisibilityBatchedFootprintController', candidate='DensityRoutedFloorController',
        timing_windows=timing, normalized_state_type_paths=run.STATE_TYPE_PATHS,
        incremental_reuse_comparison=False, incremental_density_routed_floor_comparison=True,
        persistent_memory_type_unchanged=True, both_controllers_use_visibility_batched_receipt_copied_packed_fused_queries=True,
...
def main():
    ...
    expected_launch = dict(previous_result_sha256=run.previous_check.RESULT_SHA,
        previous_launch_sha256=run.PREVIOUS_LAUNCH_SHA,
        density_routed_registration_completion_sha256=run.MICROBENCHMARK_SHA,frames=1428,
        state_frames=list(run.original.STATE_FRAMES), normalized_state_type_paths=run.STATE_TYPE_PATHS,
        baseline='VisibilityBatchedFootprintController', candidate='DensityRoutedFloorController',
    ...
    write_json(OUTPUT, dict(status='DENSITY_ROUTED_FLOOR_COMPLETION_VERIFIED',
        ...
        preceding_visibility_batched_completion_reauthenticated=True, actual_raw_and_model_bindings_rehashed=True,
    ...
    print('DENSITY_ROUTED_FLOOR_COMPLETION_VERIFIED', digest(OUTPUT), len(sources), reduction, flush=True)
