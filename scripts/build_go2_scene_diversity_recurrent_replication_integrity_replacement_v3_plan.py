#!/usr/bin/env python3
"""Build the final science-identical identity-only replacement V3 plan.

The original frozen V1 exact plan remains the sole scientific plan input.
V3 changes only the prospective attempt identifier and output root; every
scene, state, action, seed, runtime/render contract, count, cap, model, metric
and gate remains byte-canonically identical.
"""
...
FROZEN_V1_ATTEMPT_ID = frozen_builder.FROZEN_V1_ATTEMPT_ID
FROZEN_V1_COLLECTION_ROOT = frozen_builder.FROZEN_V1_COLLECTION_ROOT
...
DEFAULT_ATTEMPT_ID = (
    "go2-scene-diversity-recurrent-replication-integrity-replacement-v3"
)
...
DEFAULT_PLAN_OUTPUT = REPO_ROOT / (
    ...
    "integrity_replacement_v3_exact_plan_2026-08-04.json"
)


class SceneDiversityReplacementV3PlanError(RuntimeError):
...
def _validate_output_root_v3(path: Path) -> Path:
    selected = Path(path)
    ...
    if (
        ...
        raise SceneDiversityReplacementV3PlanError(
            "V3 output_root must be the exact fresh replacement collection path"
        )
    return resolved
...
def build_plan_v3(
    *,
    ...
    """Return frozen V1 with only the prospective V3 identity replaced."""

    if attempt_id != DEFAULT_ATTEMPT_ID:
        raise SceneDiversityReplacementV3PlanError("V3 attempt identifier changed")
    try:
        ...
        raise SceneDiversityReplacementV3PlanError(str(exc)) from exc
    selected_root = _validate_output_root_v3(output_root)
    ...
    if validated != replacement:
        raise SceneDiversityReplacementV3PlanError(
            "V3 plan normalization changed the plan"
        )
    if set(replacement) != set(original) or any(
        ...
        raise SceneDiversityReplacementV3PlanError(
            "V3 changed a frozen scientific or infrastructure field"
        )
    return replacement


build_plan_v1 = build_plan_v3
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.plan_output.exists() or args.plan_output.is_symlink():
        raise SceneDiversityReplacementV3PlanError("V3 plan output must be fresh")
    try:
        frozen_plan, frozen_binding = frozen_builder._load_frozen_json_v1(  # noqa: SLF001
        ...
        _panel, panel_binding = frozen_builder._load_frozen_json_v1(  # noqa: SLF001
            FROZEN_SCENE_PANEL,
...
__all__ = [
    ...
    "FROZEN_SCENE_PANEL",
    "FROZEN_V1_EXACT_PLAN",
    "SceneDiversityReplacementV3PlanError",
    "build_plan_v3",
]
