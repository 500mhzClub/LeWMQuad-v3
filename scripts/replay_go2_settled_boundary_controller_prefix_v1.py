...
OUTPUT = BASE/'go2_settled_boundary_controller_prefix_v1_attempt_001'
PROTOCOL = 'docs/go2_settled_boundary_controller_prefix_v1_2026-09-09.md'
...
def main():
    ...
    sources=discover_sources((PROTOCOL,'scripts/replay_go2_settled_boundary_controller_prefix_v1.py',
        'lewm/tests/test_settled_boundary_round_trip_development.py',
        'lewm/tests/test_settled_boundary_prefix_comparison_development.py'),result['source_sha256'])
    resources=hardware()
    launch=old|dict(protocol=PROTOCOL,output_root=str(OUTPUT),source_sha256=sources,prefix_input_sha256=ids,
        mission_artifact_sha256=mids,hardware=resources,maximum_frames=1867,
        native_audit_pending_at_launch=True,completed_native_audit_required_before_next_native=True,
        ...
        concurrency_reason='one independent raw-controller replay beside the existing native audit')
    verify_all(launch)
    ...
    try:
        ...
        with writer(OUTPUT) as append:
            for i,saved in enumerate(read_rows(directory)):
                ...
                try:comparison=compare_current(original,candidate)
                except Exception:
        ...
        verify_all(launch)
        write_json(OUTPUT/'result.json',dict(status='SETTLED_BOUNDARY_CONTROLLER_PREFIX_COMPLETE',
