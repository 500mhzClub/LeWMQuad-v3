#!/usr/bin/env python3
"""Run the one authorized science-identical V3 operational recovery.
...
_CONTRACT_PATH = (
    ROOT / "lewm/benchmarks/go2_shared_jepa_v5_multires_probe_v3.py"
)
_CONTRACT_SPEC = importlib.util.spec_from_file_location(
    "_lewm_go2_shared_jepa_v5_multires_probe_v3_contract",
    _CONTRACT_PATH,
)
...
PREFLIGHT_ENVIRONMENT_KEY = "LEWM_MULTIRES_PROBE_V3_PREFLIGHT_JSON"
THREAD_ENVIRONMENT = (
    "OMP_NUM_THREADS",
    "MKL_NUM_THREADS",
...
def _failure_boundary(name: str) -> None:
    ...
    if name not in SYNTHETIC_FAILURE_BOUNDARIES:
        raise ValueError(f"unknown V3 failure boundary: {name}")
...
def _read_pre_ledger_prefix(path: Path) -> bytes | None:
    ...
    if not hasattr(os, "O_NOFOLLOW"):
        raise PermissionError("O_NOFOLLOW is required for V3 prefix custody")
    try:
        before = path.stat(follow_symlinks=False)
...
class OperationProgress:
    """Exact attempted/completed operations for terminal V3 receipts."""
...
class PartialAccessLedger:
    ...
    def __init__(
        ...
        *,
        ...
        if not hasattr(os, "O_NOFOLLOW"):
            raise PermissionError("O_NOFOLLOW is required for V3 input custody")
    ...
    def read_regular(
        ...
        *,
        ...
        if (
            ...
            raise PermissionError("prior multires runtime output access is forbidden")
...
def _finalize_schedule_train_identity(
    ...
    if (
        ...
        raise PermissionError("V3 schedule adapter return contract changed")
