"""Own one chained-anchor native child after the original replay and full queue."""
import json
import os
import re
import subprocess
...
SOURCE = 'scripts/await_go2_chained_anchor_maze02_native_v1.py'
TEST = 'lewm/tests/test_chained_anchor_native_wait_development.py'
PROTOCOL = 'docs/go2_chained_anchor_maze02_native_wait_v1_2026-09-11.md'
PREPARATION = 'docs/go2_chained_anchor_native_launcher_preparation_2026-09-11.json'
PREPARATION_SHA = '634822ede9d6d9841184fca4cc4835d3e58cf41548d32d1f9a085046d62e559e'
OUTPUT = BASE/'go2_chained_anchor_maze02_native_wait_v1_attempt_001'
WAIT_SECONDS = 48*3600
RAW_RESULT_SHA = native.inputs.prefix.completed.RESULT_SHA
PREREQUISITES = (
    ('raw', native.inputs.prefix.completed.OWNER, native.inputs.replay.OUTPUT, native.inputs.prefix.completed.LAUNCH_SHA,
        'CHAINED_ANCHOR_CONTROLLER_PREFIX_V1_COMPLETE'),
    ('flow', native.inputs.FLOW_OWNER, native.inputs.flow.OUTPUT, native.inputs.FLOW_LAUNCH,
        'DIRECT_FLOW_COMMITMENT_CONTACT_MAZE02_NATIVE_WAIT_V1_COMPLETE'))
...
def prepared_sources():
    ...
    if prep['status'] != 'CHAINED_ANCHOR_NATIVE_LAUNCHER_PREPARED':
        raise ValueError('tested exact chained-anchor native launcher required')
...
def completion_identity(spec, sources):
    ...
    if key == 'raw':
        if sha != RAW_RESULT_SHA: raise ValueError('exact verified completed chained-anchor controller replay required')
        ...
        raise ValueError('original contact-plus-flow waiter completion without retry required')
    return sha
...
def wait_for_inputs(sources, event, *, sleep=time.sleep, clock=time.monotonic):
    ...
    while True:
        ...
        event('WAITING_FOR_CHAINED_ANCHOR_RAW_AND_ORIGINAL_FLOW_QUEUE', original_processes_live=live,
            completed_result_sha256=dict(completed))
        sleep(30)


def command_for(ids):
    if (type(ids) is not dict or set(ids) != {'raw', 'flow'}
            or any(type(v) is not str or re.fullmatch('[0-9a-f]{64}', v) is None for v in ids.values())
            or ids['raw'] != RAW_RESULT_SHA):
        raise ValueError('exact completed raw replay and original contact-plus-flow waiter SHA-256 identities required')
    return [sys.executable, '-B', native.SOURCE, '--controller-completion-sha256', native.inputs.COMPLETION_SHA,
        '--flow-wait-result-sha256', ids['flow']]


def authenticate_completed(sources, ids):
    root = native.OUTPUT
    if (root/'failure.json').exists() or (root/'failure.json').is_symlink():
        raise ValueError('original chained-anchor native failure must be preserved')
    ...
    if (result['status'] != 'CHAINED_ANCHOR_MAZE02_PILOT_V1_COMPLETE'
            or result['source_sha256'] != launch['source_sha256']
            or any(sources.get(n) != h for n, h in result['source_sha256'].items())
            or len(result['conditions']) != 1
            or result['controller_completion_sha256'] != native.inputs.COMPLETION_SHA
            or admission['controller_completion_sha256'] != native.inputs.COMPLETION_SHA
            or result['prospective_prefix_result_sha256'] != ids['raw']
            or result['flow_wait_result_sha256'] != ids['flow']
            or admission['flow_wait_result_sha256'] != ids['flow']):
        raise ValueError('complete chained-anchor native result tied to both original completions required')
    ...
    if (record != read_json(root, name+'_worker_terminal.json')
        ...
        raise ValueError('complete actual chained-anchor worker receipts and outcome accounting required')
    ...
    if receipt != record['prefix_comparison']:
        raise ValueError('actual chained-anchor physical and public prefix must reconstruct')
...
def main():
    for root in (OUTPUT, native.OUTPUT):
        ...
        if root.exists() or root.is_symlink(): raise ValueError('exclusive waiter and unlaunched chained-anchor child required')
    ...
    print('CHAINED_ANCHOR_NATIVE_WAITER_LAUNCHED', digest(OUTPUT/'launch.json'), flush=True)
    ...
    try:
        with (OUTPUT/'events.jsonl').open('x') as events:
            ...
            with (OUTPUT/'native_stdout.log').open('xb') as log:
                ...
                event('CHAINED_ANCHOR_NATIVE_CHILD_STARTED', pid=child.pid, command=command)
                while child.poll() is None:
                    event('CHAINED_ANCHOR_NATIVE_CHILD_LIVE', pid=child.pid); time.sleep(30)
            event('CHAINED_ANCHOR_NATIVE_CHILD_EXITED', pid=child.pid, returncode=child.returncode)
            if child.returncode != 0: raise ValueError('original chained-anchor child failed; no retry')
        ...
        write_json(OUTPUT/'result.json', dict(status='CHAINED_ANCHOR_MAZE02_NATIVE_WAIT_V1_COMPLETE',
        ...
        print('CHAINED_ANCHOR_NATIVE_WAITER_COMPLETE', digest(OUTPUT/'result.json'), flush=True)
    except Exception as error:
        write_json(OUTPUT/'failure.json', dict(status='TERMINAL_CHAINED_ANCHOR_NATIVE_WAIT_FAILURE',
