...
from lewm.recorded_joint_pose_admission_development import recorded_joint_pose as current_joint_pose
...
OUTPUT = BASE/'go2_causal_executed_residual_diagnosis_integrity_v1_attempt_001'
PROTOCOL = 'docs/go2_causal_executed_residual_diagnosis_integrity_v1_2026-09-08.md'
...
def main():
    ...
    failed = BASE/'go2_causal_executed_residual_diagnosis_v1_attempt_001'
    failure_ids = {'launch.json': 'b53475869e52057e6e806cd6fd19b58efc384035262be97c1a32d51b68f5708b',
        'failure.json': '5218c6ae580b38e2e0f4076bfc7be39110f88a5bab668756910fde65af505957'}
    verify_artifacts(failed, failure_ids); failed_launch = read_json(failed, 'launch.json'); verify(failed_launch)
    bound = []
    ...
    sources = discover_sources((PROTOCOL, 'scripts/diagnose_go2_causal_executed_residual_integrity_v1.py',
        'lewm/tests/test_recorded_joint_pose_admission_development.py',
        'docs/go2_causal_executed_residual_diagnosis_failure_2026-09-08.md',
        ...
        'docs/go2_executed_horizon_final_goal_probe_result_2026-09-08.md'), failed_launch['source_sha256'])
    resources = hardware()
    ...
    launch = old | dict(source_sha256=sources, output_root=str(OUTPUT), protocol=PROTOCOL, hardware=resources,
        ...
        predecessor_failure_bindings={str(failed): failure_ids},
    ...
    print('CAUSAL_EXECUTED_RESIDUAL_INTEGRITY_LAUNCHED', digest(OUTPUT/'launch.json'), flush=True)
    try:
        reports = []
        ...
        verify_artifacts(failed, failure_ids); verify(failed_launch)
        bindings = {n: digest(OUTPUT/n) for n in ('launch.json', 'motion.json')}; verify_artifacts(OUTPUT, bindings)
        write_json(OUTPUT/'result.json', dict(status='CAUSAL_EXECUTED_RESIDUAL_INTEGRITY_COMPLETE',
        ...
        print('CAUSAL_EXECUTED_RESIDUAL_INTEGRITY_COMPLETE', digest(OUTPUT/'result.json'), flush=True)
    except Exception as error:
        write_json(OUTPUT/'failure.json', dict(status='TERMINAL_CAUSAL_RESIDUAL_INTEGRITY_FAILURE', reason=repr(error))); raise
