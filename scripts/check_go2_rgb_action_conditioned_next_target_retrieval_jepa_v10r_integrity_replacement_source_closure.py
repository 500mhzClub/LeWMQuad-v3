#!/usr/bin/env python3
"""Build or verify the source-only V10R recursive source closure."""
...
contract = _source_only_module(
    "_lewm_retrieval_jepa_v10r_source_closure_contract",
    CONTRACT_RELATIVE_PATH,
)
_V10 = _source_only_module(
    "_lewm_retrieval_jepa_v10r_frozen_v10_source_closure",
    V10_CHECKER_RELATIVE_PATH,
)
...
if not REQUIRED_DYNAMIC_SOURCE_PATHS.issubset(
    ...
    raise PermissionError(f"V10R dynamic source roots are incomplete: {missing}")

# The V10 checker is already the reviewed safe AST walker and canonical
# manifest implementation.  Replace only its contract and recursive roots.
...
def main() -> int:
    ...
    print("Go2 Next-Target Retrieval JEPA V10R source closure: PASS")
    return 0
