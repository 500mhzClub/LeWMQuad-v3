...
def audit(layout_index, result, definition, *, input_root, model, robot_geometry, episode_name, condition, variant):
    ...
    assert result['planning_map_variant'] == 'current_paired_observation'
    assert result['persistent_contact_history_retained'] is True and result['selector_scan_state_retained'] is True
    ...
    controller = CurrentObservationPlanningController(model, robot_geometry, public_mission=mission,
