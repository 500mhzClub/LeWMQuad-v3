"""Prospective nominal-reentry intervention on the preserved first maze failure."""
import argparse
import contextlib
import json
...
from scripts.nominal_reentry_maze_episode_development import collect, artifacts
from scripts.nominal_reentry_maze_audit_development import audit
from scripts.nominal_reentry_native_prefix_comparison_development import compare
from scripts.training_translation_bias_model_admission_development import load_assigned
from scripts.replay_go2_nominal_reentry_maze_prefix_v1 import (
    OUTPUT as INTEGRATION, INPUT as PREVIOUS, INPUT_SHA as PREVIOUS_SHA,
    READOUT as PREVIOUS_READOUT, READOUT_SHA as PREVIOUS_READOUT_SHA, CASE, CORRECTION, FITS)
...
OUTPUT = BASE/'go2_nominal_reentry_maze_pilot_v1_attempt_001'
PROTOCOL = 'docs/go2_nominal_reentry_maze_pilot_v1_2026-09-08.md'
...
def worker(launch_sha):
    ...
    terminal = dict(case=name, layout_index=index, model_name=model_name,
        status='NOMINAL_REENTRY_MAZE_WORKER_FAILED', artifact_sha256={}); started = time.perf_counter()
    with (OUTPUT/(name+'_worker.log')).open('x') as log, contextlib.redirect_stdout(log), contextlib.redirect_stderr(log):
        try:
            ...
            prefix = compare(PREVIOUS/name, OUTPUT/name, launch['prefix_report'])
            ...
            terminal.update(status='NOMINAL_REENTRY_MAZE_COLLECTED_AND_RAW_AUDITED', artifact_sha256=bindings,
...
def main():
    parser = argparse.ArgumentParser(); parser.add_argument('--prefix-result-sha256', required=True); args = parser.parse_args()
    if not __debug__: raise ValueError('assertions required')
    validate_root(OUTPUT, must_exist=False)
    if OUTPUT.exists() or OUTPUT.is_symlink(): raise ValueError('exclusive prospective recovery; no retry/resume')
    verify_artifacts(INTEGRATION, {'result.json': args.prefix_result_sha256}); integration = read_json(INTEGRATION, 'result.json')
    assert integration['status'] == 'NOMINAL_REENTRY_MAZE_PREFIX_COMPLETE'
    prefix = integration['report']
    assert prefix['first_requested_command_difference'] is not None and prefix['first_terminal_policy_difference'] is None
    assert prefix['frames'] == prefix['first_requested_command_difference']+1
    assert prefix['exact_original_decisions_before_intervention'] and prefix['causal_observations_maps_mission_forecasts_and_constraints_exact']
    assert prefix['model_state_unchanged'] and prefix['final_terminal'] is None and prefix['prior_requested_command'] == [0., 0., 0.]
    assert prefix['final_requested_command'] != [0., 0., 0.] and prefix['reentry_candidates']
    ...
    assert set(previous_bindings) == {str(PREVIOUS), str(PREVIOUS_READOUT)}
    assert previous_bindings[str(PREVIOUS)]['result.json'] == PREVIOUS_SHA
    assert previous_bindings[str(PREVIOUS_READOUT)]['result.json'] == PREVIOUS_READOUT_SHA
    for root in (PREVIOUS, PREVIOUS_READOUT): verify_artifacts(root, previous_bindings[str(root)])
    ...
    sources = discover_sources((PROTOCOL, 'scripts/run_go2_nominal_reentry_maze_pilot_v1.py',
        'lewm/tests/test_nominal_reentry_native_scope_development.py',
        'lewm/tests/test_nominal_reentry_native_prefix_comparison_development.py',
        'docs/go2_nominal_reentry_maze_prefix_result_2026-09-08.md'), integration['source_sha256'])
    resources = hardware()
    ...
    if resources['artifact_free_bytes'] < RESERVE_BYTES+COLLECTION_ALLOWANCE_BYTES+PERSISTENCE_HEADROOM_BYTES:
        raise ValueError('declared recovery storage/persistence envelope unavailable')
    ...
    launch.update(protocol=PROTOCOL, source_sha256=sources, output_root=str(OUTPUT), planned_case=CASE,
        ...
        previous_native_and_readout_bindings=previous_bindings,
        ...
        predecessor_collection_allowance_bytes=12*1024**3, changed_storage_censoring_explicit=True,
        memory_admission_bytes=32*1024**3, os_resource_limits_enforced=False,
        concurrency_reason='one existing-maze recovery intervention; storage envelope excludes parallel scenes',
    ...
    print('NOMINAL_REENTRY_MAZE_LAUNCHED', launch_sha, flush=True)
    try:
        ...
        print('NOMINAL_REENTRY_MAZE_TERMINAL', record['status'], record.get('verified_round_trip'), record.get('failure'), flush=True)
        if record['status'] != 'NOMINAL_REENTRY_MAZE_COLLECTED_AND_RAW_AUDITED':
            raise ValueError('native recovery infrastructure/raw audit/prefix comparison failure; evidence retained')
        ...
        verify_artifacts(FITS, admission['base_admission']['fit_artifact_sha256'])
        verify_artifacts(CORRECTION, admission['correction_artifact_sha256'])
        write_json(OUTPUT/'result.json', dict(status='NOMINAL_REENTRY_MAZE_PILOT_COMPLETE', conditions=[record],
        ...
        print('NOMINAL_REENTRY_MAZE_COMPLETE', digest(OUTPUT/'result.json'), flush=True)
    except Exception as error:
        write_json(OUTPUT/'failure.json', dict(status='TERMINAL_NOMINAL_REENTRY_MAZE_PILOT_FAILURE', reason=repr(error)))
        raise
