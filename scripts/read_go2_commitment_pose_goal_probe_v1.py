...
import math
import numpy as np
from scripts.run_go2_commitment_pose_goal_probe_v1 import OUTPUT as INPUT, CASES
...
from scripts.run_go2_matched_model_active_view_probe_v1 import OUTPUT as ORIGINAL

OUTPUT=BASE/'go2_commitment_pose_goal_readout_v1_attempt_001'
PROTOCOL='docs/go2_commitment_pose_goal_readout_v1_2026-09-08.md'
ORIGINAL_SHA='4a921e6856db9dc0e8033fe9e4768126ceaf90b402cf195f91bcb3e3c43384e1'
...
def main():
    ...
    assert result['status']=='COMMITMENT_POSE_GOAL_PROBE_COMPLETE' and result['cases']==[list(c) for c in CASES]
    ...
    assert old['status']=='MATCHED_MODEL_ACTIVE_VIEW_PROBE_COMPLETE'
    ...
    sources=discover_sources((PROTOCOL,'scripts/read_go2_commitment_pose_goal_probe_v1.py'),original['source_sha256'])
    ...
    try:
        ...
        write_json(OUTPUT/'result.json',dict(status='COMMITMENT_POSE_GOAL_READOUT_COMPLETE',conditions=reports,models=grouped,
        ...
        print('COMMITMENT_POSE_GOAL_READOUT_COMPLETE',digest(OUTPUT/'result.json'),flush=True)
    except Exception as error:
        write_json(OUTPUT/'failure.json',dict(status='TERMINAL_COMMITMENT_POSE_GOAL_READOUT_FAILURE',reason=repr(error)));raise
