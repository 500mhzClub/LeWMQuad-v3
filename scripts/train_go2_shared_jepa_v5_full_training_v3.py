#!/usr/bin/env python3
"""New-process exact trainer/publisher for Shared JEPA V5 full training V3.
...
if __name__ == "__main__":
    ...
    def _validate_raw_v13_authority_chain(
        ...
        """Deep-check the terminal source/review/authorization/witness chain."""
        ...
        def candidate_binding(
            ...
            *,
            ...
            return isinstance(candidate, list) and any(
                ...
                and row.get("sha256") == file_sha256
                for row in candidate
            )

        builder_authority = builder_review.get("authority")
        if (
            builder_review.get("schema")
            != "lewm_go2_shared_jepa_v5_raw_supervision_builder_v9_independent_review_v1"
            or builder_review.get("verdict") != "PASS"
            or not isinstance(builder_authority, Mapping)
            or builder_authority.get("builder_source_approved") is not True
            or any(
                value is not False
                for name, value in builder_authority.items()
                if name != "builder_source_approved"
            )
            or not candidate_binding(
                builder_review,
                role="builder_source",
                path=policy.RAW_SUPERVISION_BUILDER_RELATIVE_PATH,
                file_sha256=policy.RAW_CHAIN_SOURCE_BINDINGS[
                    policy.RAW_SUPERVISION_BUILDER_RELATIVE_PATH
                ],
            )
        ):
            raise PermissionError("Builder V9 review chain changed")

        auditor_authority = auditor_review.get("authority")
        if (
            auditor_review.get("schema")
            != "lewm_go2_shared_jepa_v5_raw_supervision_auditor_v13_independent_review_v1"
            or auditor_review.get("verdict") != "PASS"
            or not isinstance(auditor_authority, Mapping)
            or auditor_authority.get("auditor_source_approved") is not True
            or any(
                value is not False
                for name, value in auditor_authority.items()
                if name != "auditor_source_approved"
            )
            or not candidate_binding(
                auditor_review,
                role="auditor_source",
                path=policy.RAW_SUPERVISION_AUDITOR_RELATIVE_PATH,
                file_sha256=policy.RAW_CHAIN_SOURCE_BINDINGS[
                    policy.RAW_SUPERVISION_AUDITOR_RELATIVE_PATH
                ],
            )
        ):
            raise PermissionError("Auditor V13 review chain changed")
        ...
        source_by_role = {
            ...
            if type(row) is dict
        } if isinstance(source_map, list) else {}
        forbidden_authority = (
        ...
        if (
            ...
            != "4b179c33de00399652f4f915285ca99a4d47cfa95d31878d1f91ca7e8fd9d0e8"
            or authorization.get("exact_audit_v13_authorized") is not True
            or any(authorization.get(name) is not False for name in forbidden_authority)
            or authorization.get("input_dataset_path")
            != policy.RAW_SUPERVISION_ROOT_RELATIVE_PATH
            or authorization.get("success_report_path")
            != policy.RAW_SUPERVISION_AUDIT_RELATIVE_PATH
            or authorization.get("v9_dataset_manifest_file_sha256")
            != policy.RAW_V9_MANIFEST_FILE_SHA256
            or authorization.get("v9_dataset_manifest_content_sha256")
            != policy.RAW_V9_MANIFEST_CONTENT_SHA256
            or source_by_role.get("auditor_source", {}).get("sha256")
            != policy.RAW_CHAIN_SOURCE_BINDINGS[
                policy.RAW_SUPERVISION_AUDITOR_RELATIVE_PATH
            ]
            or source_by_role.get("auditor_review", {}).get("sha256")
            != policy.RAW_CHAIN_SOURCE_BINDINGS[
                policy.RAW_AUDITOR_V13_REVIEW_RELATIVE_PATH
            ]
        ):
            raise PermissionError("Auditor V13 authorization chain changed")

        witness_authorization = fingerprint.get("authorization")
        verification = fingerprint.get("verification")
        witness_authority = fingerprint.get("authority")
        if (
            fingerprint.get("schema")
            != "lewm_go2_shared_jepa_v5_raw_supervision_audit_v13_authorization_fingerprint_v1"
            or fingerprint.get("content_sha256")
            != "f5c8f84478929b0ee4753bf7f4531ebaebc88eeb6ba2cc9d9718945f44107e2c"
            or not isinstance(witness_authorization, Mapping)
            or witness_authorization.get("path")
            != policy.RAW_AUDITOR_V13_AUTHORIZATION_RELATIVE_PATH
            or witness_authorization.get("file_sha256")
            != policy.RAW_CHAIN_SOURCE_BINDINGS[
                policy.RAW_AUDITOR_V13_AUTHORIZATION_RELATIVE_PATH
            ]
            or witness_authorization.get("content_sha256")
            != authorization.get("content_sha256")
            or not isinstance(verification, Mapping)
            or verification.get("authorization_actual_matches_independent_expected_bytes")
            is not True
            or verification.get("transitive_parent_commitments_exact") is not True
            or verification.get("generated_mapped_target_opened") is not False
            or verification.get("canonical_dataset_opened") is not False
            or verification.get("gpu_used") is not False
            or not isinstance(witness_authority, Mapping)
            or any(value is not False for value in witness_authority.values())
        ):
            raise PermissionError("Auditor V13 authorization witness changed")
        return {
    ...
    def _load_authority_after_preflight(
        ...
        manifest_raw = _read_bound_repo_input(
            events,
            relative_path=policy.EXACT_EXECUTION_MANIFEST_RELATIVE_PATH,
        ...
        manifest = policy.validate_execution_manifest(
            _parse_content_json(manifest_raw, name="exact manifest in trainer"),
            require_ready=True,
        )
        if (
        ...
        if any(
            implementation_review["reviewed_sources"].get(relative)
        ...
        for relative, expected in {
            ...
            **dict(implementation_review["reviewed_sources"]),
        ...
        raw_manifest = _parse_content_json(raw_manifest_raw, name="raw manifest")
        if (
            raw_manifest.get("schema") != policy.RAW_SUPERVISION_MANIFEST_SCHEMA
            or raw_manifest.get("status") != "complete_pending_independent_audit"
            or raw_manifest.get("content_sha256")
            != bindings["development_raw_supervision_manifest_content_sha256"]
            or raw_manifest.get("roles") != list(policy.DEVELOPMENT_ROLES)
            or raw_manifest.get("pair_counts")
            != {role: policy.ROLE_COUNTS[role]["pairs"] for role in policy.DEVELOPMENT_ROLES}
            or raw_manifest.get("unique_endpoint_counts")
            != {
                role: policy.ROLE_COUNTS[role]["unique_endpoints"]
                for role in policy.DEVELOPMENT_ROLES
            }
        ):
            raise PermissionError("raw-supervision manifest contract changed")
        ...
        raw_audit = _parse_content_json(raw_audit_raw, name="raw audit")
        if (
            raw_audit.get("schema") != policy.RAW_SUPERVISION_AUDIT_SCHEMA
            or raw_audit.get("verdict") != "PASS"
            or raw_audit.get("content_sha256")
            != bindings["development_raw_supervision_audit_content_sha256"]
            or raw_audit.get("dataset_manifest_file_sha256")
            != bindings["development_raw_supervision_manifest_file_sha256"]
            or raw_audit.get("dataset_manifest_content_sha256")
            != bindings["development_raw_supervision_manifest_content_sha256"]
            or raw_audit.get("pair_count") != 5172
            or raw_audit.get("unique_endpoint_count") != 9460
            or raw_audit.get("scene_shard_count") != 88
            or raw_audit.get("sample_count") != 24
            or raw_audit.get("sample_results_sha256")
            != policy.RAW_V13_SAMPLE_RESULTS_SHA256
            or raw_audit.get("dataset_use_authorized") is not False
            or raw_audit.get("training_authorized") is not False
            or raw_audit.get("selection_authorized") is not False
            or raw_audit.get("calibration_authorized") is not False
            or raw_audit.get("g2_authorized") is not False
            or raw_audit.get("heldout_authorized") is not False
            or raw_audit.get("runtime_authorized") is not False
            or raw_audit.get("navigation_authorized") is not False
            or raw_audit.get("hardware_authorized") is not False
            or raw_audit.get("production_authorized") is not False
            or raw_audit.get("promotion_authorized") is not False
            or raw_audit.get("deployment_authorized") is not False
            or manifest.get("raw_v13_dataset_use_grant")
            != policy.RAW_DATASET_USE_GRANT
            or manifest.get("dataset_use_authorized_for_exact_attempt") is not True
        ):
            raise PermissionError("raw-supervision independent audit did not pass")

        camera_source_review_raw = _read_bound_repo_input(
            events,
            relative_path=policy.CAMERA_V13_SOURCE_REVIEW_RELATIVE_PATH,
            expected_sha256=bindings["camera_v13_source_review_file_sha256"],
            stage="exact_source_closure",
            role="source_closure",
        )
        camera_source_review = _parse_content_json(
            camera_source_review_raw,
            name="Camera V13 source review",
        )
        if (
            camera_source_review.get("schema")
            != "lewm_go2_observable_camera_ray_fit_v4_n5_gate_aligned_raster_nll_v13_source_review_v1"
            or camera_source_review.get("content_sha256")
            != bindings["camera_v13_source_review_content_sha256"]
            or camera_source_review.get("review_completed") is not True
            or camera_source_review.get("source_closure_approved") is not True
            or camera_source_review.get("exact_attempt_authorized") is not True
            or camera_source_review.get("scientific_retry_authorized") is not False
        ):
            raise PermissionError("Camera V13 source review did not pass")

        camera_n5_gate_raw = _read_bound_repo_input(
            events,
            relative_path=policy.CAMERA_V13_N5_GATE_RELATIVE_PATH,
            expected_sha256=bindings["camera_v13_n5_gate_pass_file_sha256"],
            stage="exact_input",
            role="v4_two_seed_ladder",
        )
        camera_n5_gate = _parse_content_json(
            camera_n5_gate_raw,
            name="Camera V13 seed-20260710 N5 gate",
        )
        numeric_n5 = camera_n5_gate.get("numeric_gate")
        source_review_binding = camera_n5_gate.get("source_review")
        if (
            camera_n5_gate.get("schema")
            != "lewm_go2_observable_camera_ray_fit_v4_n5_gate_aligned_raster_nll_v13_gate_v1"
            or camera_n5_gate.get("content_sha256")
            != bindings["camera_v13_n5_gate_pass_content_sha256"]
            or camera_n5_gate.get("status") != "passed"
            or camera_n5_gate.get("passes") is not True
            or camera_n5_gate.get("seed") != 20260710
            or camera_n5_gate.get("fit_size") != 5
            or camera_n5_gate.get("failure_count") != 0
            or camera_n5_gate.get("failed_checks") != []
            or not isinstance(numeric_n5, Mapping)
            or numeric_n5.get("check_count") != 26
            or numeric_n5.get("failure_count") != 0
            or numeric_n5.get("passes") is not True
            or not isinstance(source_review_binding, Mapping)
            or source_review_binding.get("path")
            != policy.CAMERA_V13_SOURCE_REVIEW_RELATIVE_PATH
            or source_review_binding.get("file_sha256")
            != bindings["camera_v13_source_review_file_sha256"]
            or source_review_binding.get("content_sha256")
            != bindings["camera_v13_source_review_content_sha256"]
        ):
            raise PermissionError("Camera V13 N5 gate did not pass exactly once")

        ladder_preregistration_raw = _read_bound_repo_input(
            events,
            relative_path=policy.CAMERA_V13_LADDER_PREREGISTRATION_RELATIVE_PATH,
            expected_sha256=bindings[
        ...
        ladder_review_raw = _read_bound_repo_input(
            events,
            relative_path=policy.CAMERA_V13_LADDER_REVIEW_RELATIVE_PATH,
            expected_sha256=bindings[
        ...
        ladder_review = _parse_content_json(
            ladder_review_raw,
            name="Camera V13 ladder independent review",
        )
        if (
            ladder_review.get("schema")
            != "lewm_go2_observable_camera_ray_fit_v4_gate_aligned_raster_nll_v13_two_seed_ladder_independent_review_v1"
            or ladder_review.get("verdict") != "PASS"
            or ladder_review.get("preregistration_path")
            ...
            raise PermissionError("Camera V13 ladder preregistration review changed")

        v4_ladder_raw = _read_bound_repo_input(
            events,
            relative_path=policy.V4_TWO_SEED_LADDER_RELATIVE_PATH,
            expected_sha256=bindings["v4_two_seed_ladder_pass_file_sha256"],
            stage="exact_input",
            role="v4_two_seed_ladder",
        )
        v4_ladder = _parse_content_json(v4_ladder_raw, name="V4 two-seed ladder")
        if (
            v4_ladder.get("schema")
            != "lewm_go2_observable_camera_ray_fit_v4_gate_aligned_raster_nll_v13_two_seed_ladder_v1"
            or v4_ladder.get("status") != "passed"
            or v4_ladder.get("content_sha256")
            != bindings["v4_two_seed_ladder_pass_content_sha256"]
            or v4_ladder.get("seeds") != [20260710, 20260711]
            or v4_ladder.get("fit_sizes") != [5, 16, 32, 320]
            ...
            or v4_ladder.get("all_rungs_fresh_initialization") is not True
            or v4_ladder.get("warm_start_used") is not False
            or v4_ladder.get("both_seed_ladders_pass") is not True
            or v4_ladder.get("existing_seed_20260710_n5_gate_file_sha256")
            != bindings["camera_v13_n5_gate_pass_file_sha256"]
            or v4_ladder.get("existing_seed_20260710_n5_gate_content_sha256")
            != bindings["camera_v13_n5_gate_pass_content_sha256"]
            or v4_ladder.get("primary_migratable_rung")
            != {"seed": 20260710, "fit_size": 320}
        ):
            raise PermissionError("V4 two-seed ladder did not pass")
        return {
            "manifest": manifest,
        ...
        class FixedProductionTrainingBackend:
            ...
            def _load_v4_and_initialize(self) -> tuple[dict[str, Any], bytes, str]:
                ...
                checkpoint_raw = _read_bound_repo_input(
                    self.context.events,
                    relative_path=policy.V4_PRIMARY_CHECKPOINT_RELATIVE_PATH,
                    expected_sha256=bindings[
                        "v4_primary_seed_20260710_n320_checkpoint_file_sha256"
                    ],
                    stage="exact_input",
                    role="v4_primary_checkpoint",
            ...
            def _checkpoint_bytes(
                ...
                *,
                ...
                semantic = {
                    "schema": "lewm_go2_shared_jepa_v5_full_training_v3_checkpoint_v1",
                    "arm": arm,
                    "update": update,
            ...
            def _training_step_loss(self, model: Any, batch: Mapping[str, Any], arm: str) -> tuple[Any, Mapping[str, Any]]:
                ...
                joint = self.loss_adapter.combine_joint_losses_v3(
                    model,
                    pair,
            ...
            def _train_arm(
                ...
                *,
                ...
                for update in range(1, policy.UPDATE_COUNT + 1):
                    ...
                    trace_rows.append(
                        {
                            "schema": "lewm_go2_shared_jepa_v5_full_training_v3_trace_row_v1",
                            "arm": arm,
                            "update": update,
                    ...
                    for start in range(0, len(frames), policy.MICROBATCH_SIZE):
                        ...
                        for source, accumulator in ((target, correct_acc), (wrong, wrong_acc)):
                            ...
                            if accumulator is correct_acc:
                                loss = self.loss_adapter.observable_camera_ray_v4_loss_v3(
                                    model,
    ...
    def _build_context(
        ...
        source_review = {
            ...
            "camera_v13_dynamic_authority_bindings": {
                policy.CAMERA_V13_SOURCE_REVIEW_RELATIVE_PATH: reservation[
                ...
                policy.CAMERA_V13_LADDER_REVIEW_RELATIVE_PATH: reservation[
                    "required_exact_bindings"
                ]["camera_v13_ladder_independent_review_file_sha256"],
            },
        ...
        input_bindings = {
            ...
            "camera_v13_source_review_file_sha256": reservation[
                "required_exact_bindings"
            ]["camera_v13_source_review_file_sha256"],
            "camera_v13_source_review_content_sha256": authority[
                "camera_source_review"
            ]["content_sha256"],
            "camera_v13_n5_gate_file_sha256": reservation[
                "required_exact_bindings"
            ]["camera_v13_n5_gate_pass_file_sha256"],
            "camera_v13_n5_gate_content_sha256": authority["camera_n5_gate"][
                "content_sha256"
            ],
            "camera_v13_ladder_preregistration_file_sha256": reservation[
                "required_exact_bindings"
            ]["camera_v13_ladder_preregistration_file_sha256"],
            "camera_v13_ladder_independent_review_file_sha256": reservation[
                "required_exact_bindings"
            ]["camera_v13_ladder_independent_review_file_sha256"],
            "v4_two_seed_ladder_file_sha256": reservation["required_exact_bindings"][
                "v4_two_seed_ladder_pass_file_sha256"
            ],
            "v4_two_seed_ladder_content_sha256": authority["v4_ladder"]["content_sha256"],
    ...
    def _finalize_summary(context: TrainerContext, result: Mapping[str, Any]) -> dict[str, Any]:
        ...
        core = {
            "schema": "lewm_go2_shared_jepa_v5_full_training_v3_trainer_summary_v1",
            "status": "trainer_publication_complete",
            "directory_identity": list(context.directory_identity),
            "execution_manifest_content_sha256": context.manifest["content_sha256"],
