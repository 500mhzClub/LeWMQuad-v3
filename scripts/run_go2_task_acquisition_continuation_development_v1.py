#!/usr/bin/env python3
"""Fixed28 task-directed acquisition factorial and matched learned comparison."""
import ast
import contextlib
import json
...
from lewm.task_acquisition_continuation_development import TaskAcquisitionContinuation as ObservedContinuation
from lewm.task_acquisition_metrics_development import reduce_task_acquisition as reduce_continuation
...
OUTPUT = ROOT / '.generated/go2_task_acquisition_continuation_development_v1_attempt_001'
NEW_SOURCES = (
    'lewm/stop_observe_traversal_development.py',
    'lewm/task_acquisition_continuation_development.py',
    'lewm/task_acquisition_scene_development.py',
    'lewm/task_acquisition_metrics_development.py',
    'lewm/tests/test_task_acquisition_continuation_development.py',
    'scripts/run_go2_task_acquisition_continuation_development_v1.py',
    'scripts/audit_go2_task_acquisition_continuation_development_v1.py',
    'docs/go2_task_acquisition_continuation_development_v1_2026-09-05.md')
LEAVES = (
...
def collect(spec, output, template=None):
    ...
    try:
        ...
        if stop_reason is None:
            controller = ObservedContinuation(spec['method'], geometry, template, acquisition_policy=spec['acquisition_policy'])
            try:
        ...
        return {'scene_id': spec['scene_id'], 'case_index': spec['case_index'], 'method': spec['method'],
                ...
                'initial_heading_rad': spec['initial_heading_rad'], 'acquisition_policy': spec['acquisition_policy'],
...
def preflight():
    predecessor = ROOT / '.generated/go2_persistent_alignment_continuation_development_v1_attempt_001'
    identities = {str((predecessor/leaf).relative_to(ROOT)): sha for leaf, sha in (
        ('launch.json', '9aa05275e42bae0cac32f7c7dd111926af552e4eb5a7b80418358d8d2d77415e'),
        ('result.json', '522326480ad94873b7232858bcd7a8cf3f6a3ca6d86ce4fc2fdc6b2848970344'),
        ('raw_artifact_audit.json', '1d478522b74501b0ed21cce16feaf3e3a12fe0f82b8c06ee9a650e0c145bd112'))}
    ...
    audit = json.loads((predecessor/'raw_artifact_audit.json').read_text())
    if result['status'] != 'COMPLETE' or audit['status'] != 'PASS' or audit['audited_trials'] != 16:
        raise ValueError('completed and fully audited predecessor required')
    inputs = prior['input_sha256'] | identities
    gait = prior['gait_sha256']
    verify_bindings(prior['source_sha256'] | inputs | gait)
    ...
    for template in templates.values():
        for binding in template.bindings:
            ...
            if inputs.get(str(path.relative_to(ROOT))) != binding['checkpoint_sha256']:
                raise ValueError('frozen checkpoint binding changed')
...
def main():
    ...
    specs = trials()
    OUTPUT.mkdir()
    write_json(OUTPUT/'launch.json', {'schema': 'task_acquisition_continuation_development.v1', 'trial_specs': specs,
        'source_sha256': sources, 'input_sha256': inputs, 'gait_sha256': gait,
        'scope': 'twenty-eight task-acquisition factorial and matched learned development trials; no independent-maze or hardware qualification'})
    rows, prefixes = [], {}
    try:
        for spec in specs:
            ...
            print(json.dumps({'event': 'continuation_finished', 'completed': len(rows), 'total': len(specs),
                'method': spec['method'], 'acquisition_policy': spec['acquisition_policy'], 'controller_terminal': response['controller_terminal'],
                'stop_reason': response['stop_reason'], 'two_leg_success': response['task_two_leg_integration_success'],
                'full_scan_two_leg_success': response['two_leg_integration_success'],
