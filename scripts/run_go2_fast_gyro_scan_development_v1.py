#!/usr/bin/env python3
"""Eight fresh paired50/500 Hz actual-sensor scans; no model or gate fitting."""
import ast
import copy
import contextlib
import json
...
from lewm.active_exit_scan_scene_development import scan_scenes as original_scenes
...
OUTPUT = ROOT / '.generated/go2_fast_gyro_scan_development_v1_attempt_001'
NEW_SOURCES = (
    ...
    'lewm/fast_gyro_development.py', 'lewm/fast_gyro_scan_development.py', 'lewm/simulated_fast_gyro_development.py',
    'scripts/fast_gyro_scan_session_development.py', 'scripts/run_go2_fast_gyro_scan_development_v1.py',
    'scripts/audit_go2_fast_gyro_scan_development_v1.py', 'lewm/tests/test_fast_gyro_development.py',
    'lewm/tests/test_fast_gyro_scan_evidence_development.py', 'docs/go2_fast_gyro_scan_development_v1_2026-09-05.md')
LEAVES = ('physics_trace.npz', 'native_contacts.npz', 'contact_events.json', 'contact_topology.json',
          ...
          'actuator_identity.json', 'terminal_actuator_gains.json', 'scan_decisions.json', 'command_tape.json', 'process.log', 'fast_gyro_samples.npz', 'fast_gyro_histories.npz')


def scan_scenes():
    result = []
    for case, old_index in enumerate((0, 2, 12, 14)):
        for rate in (50, 500):
            spec = copy.deepcopy(original_scenes()[old_index])
            spec.update(scene_id=f'fast-gyro-scan-development-v1-{case:02d}-{rate}',
                        case_index=case, procedural_seed=2026092600 + case, rate_hz=rate, initial_heading_rad=0.)
            spec['geometry']['spawn_se2_world'] = [0., 0., 0.]
            result.append(spec)
    return result
...
def sensor_decision(controller, ground, packet, *, tick, observation_id, fast_packet):
    ...
    if isinstance(controller, FastGyroScan):
        decision = controller.begin(packet, fast_packet, now_ns=now) if tick == 0 else controller.step(packet, fast_packet, now_ns=now)
    else:
...
def collect(spec, output):
    ...
    try:
                ...
                for tick in range(301):
                    ...
                    try:
                        decision, state, observation = sensor_decision(controller, ground, packet, tick=tick,
                                                                     ...
                                                                     fast_packet=session.fast_buffer.packet(now_ns=packet['sensor_state']['decision_ns']))
        ...
        return {'scene_id': spec['scene_id'], 'case_index': spec['case_index'], 'motif': spec['motif'],
                'width_m': spec['width_m'], 'initial_heading_rad': spec['initial_heading_rad'], 'rate_hz': spec['rate_hz'],
...
def main():
    ...
    predecessor = ROOT / '.generated/go2_active_exit_scan_development_v1_attempt_001'
    inputs = {str((predecessor / 'launch.json').relative_to(ROOT)): '2c484b07ee94d1366c6c744aa725a6b5da503ca1558f84b9339b1b60bd85a70d',
              str((predecessor / 'result.json').relative_to(ROOT)): 'c77c12ac1a690fe984840b342e0dbf5b6b57bfd84320efbbc120f0dba65af46e'}
    ...
    specs = scan_scenes()
    OUTPUT.mkdir()
    write_json(OUTPUT / 'launch.json', {'schema': 'fast_gyro_scan_development.v1', 'trial_specs': specs,
               'source_sha256': sources, 'input_sha256': inputs, 'gait_sha256': gait,
               'scope': 'eight fresh paired sensor-rate scans; no learned controller, traversal, maze or hardware qualification'})
    rows = []
    prefixes = {}
    try:
        for spec in specs:
            ...
            directory.mkdir()
            print(json.dumps({'event': 'scan_started', 'scene_id': spec['scene_id'], 'completed': len(rows), 'total': 8}), flush=True)
            ...
            print(json.dumps({'event': 'scan_finished', 'completed': len(rows), 'total': 8,
                              'rate_hz': spec['rate_hz'], 'physical_task_success': response['physical_task_success'], 'stop_reason': response['stop_reason'],
        ...
        verify_bindings(sources | inputs | gait)
        write_json(OUTPUT / 'result.json', {'status': 'COMPLETE', 'completed_trials': 8, 'planned_trials': 8,
                   'trials': rows, 'launch_sha256': digest(OUTPUT / 'launch.json')})
        print(json.dumps({'status': 'COMPLETE', 'trials': 8}), flush=True)
    except Exception as error:
        write_json(OUTPUT / 'result.json', {'status': 'INFRASTRUCTURE_FAILURE', 'error': repr(error),
                   'completed_trials': len(rows), 'planned_trials': 8, 'trials': rows,
