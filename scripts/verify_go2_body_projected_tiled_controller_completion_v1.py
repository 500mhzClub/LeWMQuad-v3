...
from scripts import replay_go2_body_projected_tiled_late_history_v1 as run
...
SOURCE = 'scripts/verify_go2_body_projected_tiled_controller_completion_v1.py'
TEST = 'lewm/tests/test_body_projected_tiled_controller_completion_development.py'
EXECUTION = 'docs/go2_body_projected_tiled_controller_replay_execution_2026-09-11.json'
OUTPUT = ROOT/'docs/go2_body_projected_tiled_controller_completion_verification_2026-09-11.json'
...
def expected_report(prior_report, timing):
    ...
    for key in ('incremental_tiled_dense_floor_comparison', 'both_controllers_use_progressive_retained_floor_patch_batching'):
        if expected.pop(key) is not True: raise ValueError('original tiled comparison scope required')
    expected.update(baseline='TiledDensityProgressiveFloorController', candidate='BodyProjectedTiledController',
        timing_windows=timing, normalized_state_type_paths=run.STATE_TYPE_PATHS,
        incremental_reuse_comparison=False, incremental_body_projection_comparison=True,
        persistent_memory_type_unchanged=True, both_controllers_use_original_tiled_classification_and_receipt_handling=True,
...
def main():
    ...
    execution_sha=args.execution_sha256
    if re.fullmatch('[0-9a-f]{64}',execution_sha) is None:raise ValueError('exact execution SHA256 required')
    ...
    if (result['status'] != 'BODY_PROJECTED_TILED_LATE_HISTORY_V1_COMPLETE'
    ...
    expected_launch = dict(previous_result_sha256=run.PREVIOUS_RESULT_SHA,
        ...
        body_projection_component_benchmark_sha256=run.MICROBENCHMARK_SHA,frames=1428,
        state_frames=list(run.original.STATE_FRAMES), normalized_state_type_paths=run.STATE_TYPE_PATHS,
        baseline='TiledDensityProgressiveFloorController', candidate='BodyProjectedTiledController',
    ...
    write_json(OUTPUT, dict(status='BODY_PROJECTED_TILED_COMPLETION_VERIFIED',
        utc=datetime.now(timezone.utc).isoformat(), source_sha256=sources, source_count=len(sources),
        original_source_count=len(result['source_sha256']), result_sha256=result_sha, execution_sha256=execution_sha,
        ...
        preceding_tiled_completion_reauthenticated=True, actual_raw_and_model_bindings_rehashed=True,
    ...
    print('BODY_PROJECTED_TILED_COMPLETION_VERIFIED', digest(OUTPUT), len(sources), reduction, flush=True)
