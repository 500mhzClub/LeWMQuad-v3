"""Bounded source-state collection with unchanged dense navigation controllers.

Audit-only orchestration. No comparative selection, scoring or fitting occurs.
Execution requires the Phase 1 admission record and completed Stage A.
"""
import argparse
...
ASSIGNMENTS = ((0, 'command_history'), (0, 'reactive_feedback'), (0, 'action'),
               (2, 'command_history'), (2, 'reactive_feedback'), (2, 'action'))
...
def run(case):
    ...
    try:
        ...
        spec = source.prospective.specification(layout)
        mission = source.prospective.public_mission(layout)
        ...
        with contextlib.ExitStack() as stack:
            ...
            session = source.ProspectiveRGBOnlySession(spec, directory, noise_layout_index=layout, noise_sigma_mm=2)
            ...
            controller.audit_snapshot_frames = tuple(caps['collection_caps']['snapshot_frames'])
            ...
            for tick in range(caps['collection_caps']['source_policy_steps_per_rollout']):
                ...
                if tick % 5 == 0:
                    ...
                    if packet.frame in controller.audit_snapshot_frames:
                        state_dir = output / f'state_{packet.frame:04d}'
                        state_dir.mkdir()
                        if packet.frame not in getattr(controller, 'audit_packets', {}):
                            save(state_dir / 'unavailable.json', dict(reason='NO_SELECTOR_CALL_AT_FIXED_FRAME', frame=packet.frame))
                        else:
                            budget.reserve_snapshot(case, packet.frame)
                            capture_started = time.monotonic()
                            physical = capture(session)
                            decision = controller.audit_packets.pop(packet.frame)
                            physical_binding = binary(state_dir / 'physical.pkl', physical, budget)
                            packet_binding = binary(state_dir / 'decision.pkl', decision, budget)
                            metadata = dict(frame=packet.frame, measured_ns=measured,
                                source_physical_sample_index=len(session.samples)-1,
                                physical=physical_binding, decision=packet_binding,
                                capture_and_write_wall_s=time.monotonic()-capture_started,
                                source_controller=arm, layout=layout, pilot_only=True,
                                restoration_qualified=False, comparative_rows_computed=False)
                            save(state_dir / 'snapshot.json', metadata)
                            snapshots.append(metadata)
                            del physical, decision
        ...
        try:
            if session is not None:
                ...
                rgb_bytes = sum(pair[0].nbytes for row in session.captured_pairs for pair in row['images'])
                trace_bytes = sum(np.asarray(value).nbytes for row in session.samples for value in row.values())
                budget.admit_write(2 * rgb_bytes + 4 * trace_bytes + 32 * 1024**2)
                session.persist(directory)
                session.persist_observations(directory)
                budget.check('source_persisted', force=True)
                if error is None:
                    from scripts.run_go2_decision_headroom_branches_development import run as run_branches
                    run_branches(session, output, budget, tolerances=admission['restoration_tolerances'])
                    from scripts.time_go2_decision_headroom_components_development import run as time_components
                    time_components(model, output, budget, case)
        except BaseException as exc:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--case', type=int, choices=range(6), required=True)
