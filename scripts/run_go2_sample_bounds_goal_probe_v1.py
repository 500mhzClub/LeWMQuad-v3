"""Four fresh native cases with all-return sample bounds and exact foot spheres."""
import argparse
import contextlib
import json
...
from scripts.sample_bounds_goal_episode_development import collect, artifacts
from scripts.sample_bounds_goal_audit_development import audit
...
from scripts.run_go2_nominal_action_goal_probe_v1 import OUTPUT as DIAGNOSTIC
from scripts.read_go2_measured_sample_bounds_v1 import OUTPUT as REFINEMENT
from scripts.navigation_artifact_root_development import BASE, create_output, validate_root, verify_artifacts
from scripts.run_go2_successive_choice_maze_development_v1 import digest, write_json
from scripts.startup_raw_sensor_audit_development import read_json
from scripts.read_go2_tracking_posthoc_raw_accuracy_v1 import hardware

OUTPUT=BASE/'go2_sample_bounds_goal_probe_v1_attempt_001'
READOUT=BASE/'go2_nominal_action_goal_readout_v1_attempt_001'
DIAGNOSTIC_IDS={'launch.json':'2cf7994a176e64c5d74318bdcb65330f4626a38a2caa25ccd8936b06e03d3145'}
...
PROTOCOL='docs/go2_sample_bounds_goal_probe_v1_2026-09-08.md'
REFINEMENT_SHA='9bd03cc52dfd19fa08faf9f15bab739cd2a4a681cdf491fe842b2590f09d48df'
...
def worker(case,launch_sha):
    ...
    terminal=dict(case=name,trial=trial,variant=variant,condition=condition,model_name=model_name,
        status='SAMPLE_BOUNDS_GOAL_WORKER_FAILED',artifact_sha256={})
    ...
    with (OUTPUT/(name+'_worker.log')).open('x') as log,contextlib.redirect_stdout(log),contextlib.redirect_stderr(log):
        try:
            ...
            terminal.update(status='SAMPLE_BOUNDS_GOAL_COLLECTED_AND_RAW_AUDITED',
...
def main():
    ...
    assert diagnostic['status']=='NOMINAL_ACTION_GOAL_PROBE_COMPLETE' and len(diagnostic['conditions'])==4
    ...
    assert readout['status']=='NOMINAL_ACTION_GOAL_READOUT_COMPLETE' and readout['probe_result_sha256']==args.prior_result_sha256
    ...
    assert refinement['status']=='MEASURED_SAMPLE_BOUNDS_DIAGNOSTIC_COMPLETE'
    ...
    launch=preflight(output=OUTPUT,protocol=PROTOCOL,
        seed_paths=('scripts/run_go2_sample_bounds_goal_probe_v1.py',
            'lewm/tests/test_sample_bounds_goal_probe_development.py',
            'docs/go2_nominal_action_goal_probe_result_2026-09-08.md',*readout['source_sha256'],*refinement['source_sha256']),
    ...
    launch.update(all_six_admission=receipt,fit_artifact_sha256=fit_ids,snapshots=snapshots,
        ...
        experiment='measured sample bounds and exact foot spheres V1',
    ...
    print('SAMPLE_BOUNDS_GOAL_LAUNCHED',digest(OUTPUT/'launch.json'),flush=True)
    ...
    try:
                ...
                while pending:
                    ...
                    for future in done:
                        ...
                        try:record=future.result()
                            ...
                            record=dict(case=case[0],status='SAMPLE_BOUNDS_GOAL_PROCESS_FAILURE',failure=repr(exc))
                        records.append(record)
                        print('SAMPLE_BOUNDS_GOAL_TERMINAL',case[0],record['status'],record.get('goal'),flush=True)
            if len(records)!=4 or any(r['status']!='SAMPLE_BOUNDS_GOAL_COLLECTED_AND_RAW_AUDITED' for r in records):
        ...
        for root,ids in scaling['phase_result_sha256'].items():verify_artifacts(Path(root),ids)
        write_json(OUTPUT/'result.json',dict(status='SAMPLE_BOUNDS_GOAL_PROBE_COMPLETE',
        ...
        print('SAMPLE_BOUNDS_GOAL_COMPLETE',digest(OUTPUT/'result.json'),flush=True)
    except Exception as exc:
        write_json(OUTPUT/'failure.json',dict(status='TERMINAL_SAMPLE_BOUNDS_GOAL_FAILURE',reason=repr(exc),
