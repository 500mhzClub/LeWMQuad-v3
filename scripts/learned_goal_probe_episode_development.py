...
from lewm.learned_goal_probe_development import LearnedGoalProbe, WARMUP_TICKS, NAVIGATION_TICKS, DRAIN_TICKS
...
from scripts.learned_goal_probe_checkpoint_development import load_model
...
def collect(trial, definition, *, output):
    ...
    try:
        ...
        session._runtime['high_level'] = 'learned_goal_bootstrap_probe_v1; online model and measured joint RGBD pose'
        ...
        controller = LearnedGoalProbe(model)
        try:
        ...
        result = dict(status='LEARNED_GOAL_PROBE_TERMINAL_AUDIT_REQUIRED', trial=trial,
        ...
        print('LEARNED_GOAL_PROBE', trial, result, flush=True)
        return result
    finally:
