...
SCHEMA = 'receipt_bound_observation_horizon_snapshot.v1'
MAX_BYTES = 64 * 1024**2
RESERVE = 40 * 1024**3
...
class EvaluationOnlyTrainer(ObservationHorizonTrainer):
...
def validate_payload(payload, expected_binding, expected_config):
    ...
    fields = {'schema', 'contact_semantics', 'position_scale_m', 'target_cadence_ns', 'maximum_horizon_ns', 'action_ticks_per_block', *CONFIG_KEYS, 'failed', 'initial_sha256',
    ...
    require(saved['schema'] == 'observation_horizon_training_development.v1'
        and saved['contact_semantics'] == 'integrated_softplus_hazard_per_second'
        and saved['position_scale_m'] == POSITION_SCALE_M
        and saved['target_cadence_ns'] == 100_000_000 and saved['maximum_horizon_ns'] == 800_000_000
        and saved['action_ticks_per_block'] == 1
...
def save_snapshot(directory, name, trainer, binding):
    ...
    require(isinstance(trainer, ObservationHorizonTrainer) and not isinstance(trainer, EvaluationOnlyTrainer)
