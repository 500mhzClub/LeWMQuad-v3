#!/usr/bin/env python3
"""Collect the fresh V2 ``ld.lld``-driver Genesis ROCm successor.

This is a scoped identity adapter over the frozen V1 collector source.  It
owns the V2 plan role/root routing so a fresh child process cannot fall back
to V1 identities.  No invocation is self-authorizing.
"""
...
from scripts import collect_go2_scene_diversity_recurrent_replication_genesis_rocm_backend_v1 as predecessor  # noqa: E402


AUTHORITY_SCHEMA = (
    "lewm_go2_scene_diversity_recurrent_replication_"
    "genesis_rocm_backend_v2_execution_authority_v1"
)
AUTHORITY_STATUS = (
    "AUTHORIZED_SCENE_DIVERSITY_RECURRENT_REPLICATION_"
    "GENESIS_ROCM_BACKEND_V2"
)
...
RESERVATION_SCHEMA = (
    "lewm_go2_scene_diversity_recurrent_replication_"
    "genesis_rocm_backend_v2_collection_reservation_v1"
)
SCENE_RESULT_SCHEMA = (
    "lewm_go2_scene_diversity_recurrent_replication_"
    "genesis_rocm_backend_v2_scene_physics_result_v1"
)
SCENE_EVIDENCE_SCHEMA = (
    "lewm_go2_scene_diversity_recurrent_replication_"
    "genesis_rocm_backend_v2_scene_process_evidence_v1"
)
...
ROCM_ADDITIONAL_SANITIZED_KEYS = predecessor.ROCM_ADDITIONAL_SANITIZED_KEYS

CONTACT_FORCE_ROUTE_AUDIT = {
    **copy.deepcopy(predecessor.CONTACT_FORCE_ROUTE_AUDIT),
    "schema": (
        "lewm_go2_scene_diversity_recurrent_replication_"
        "genesis_rocm_backend_v2_contact_force_route_source_audit_v1"
    ),
}
PROCESS_RESET_EQUIVALENCE_AUDIT_ROCM = {
    **copy.deepcopy(predecessor.PROCESS_RESET_EQUIVALENCE_AUDIT_ROCM),
    "schema": (
        "lewm_go2_scene_diversity_recurrent_replication_"
        "genesis_rocm_backend_v2_process_reset_equivalence_source_audit_v1"
    ),
    "v1_runtime_payload_reuse_authorized": False,
    "v2_ld_lld_driver_entrypoint": str(
...
_ORIGINAL_VALIDATE_AUTHORITY_V2 = (
    predecessor.predecessor._validate_authority_v2  # noqa: SLF001
)
_CONFIGURATION_LOCK = threading.RLock()


def _standard_v1_review_binding() -> dict[str, Any]:
    value = plan_builder.V1_QUALIFICATION_TERMINAL_REVIEW_BINDING
    return {
        "path": str(value["path"]),
...
def _require_v1_review_binding(authority: Mapping[str, Any]) -> None:
    if authority.get(
        "predecessor_v1_qualification_terminal_review_binding"
    ) != _standard_v1_review_binding():
        raise SceneProcessCollectionError(
            "V2 authority V1 terminal-review binding changed"
        )


def _validate_authority_v2_review_bound(*args: Any, **kwargs: Any):
...
def _expected_output_root_for_identity() -> Path:
    ...
    raise SceneProcessCollectionError("V2 collector attempt identity changed")
...
def _plan_role_for_identity() -> str:
    ...
    raise SceneProcessCollectionError("V2 collector attempt role changed")
...
def require_exact_orchestrator_python() -> str:
    """Fail before reservation unless V2 runs from its lexical ROCm venv."""

    expected_python = str(plan_builder.ROCM_PYTHON.absolute())
    if sys.executable != expected_python:
        raise SceneProcessCollectionError(
            "V2 orchestrator Python launcher is not the exact lexical ROCm venv path"
        )
...
def require_exact_orchestrator_environment(
    ...
    """Require the complete sanitized environment before V2 reservation."""
    ...
    for key in sorted(selector_keys):
        if key in expected:
            if os.environ.get(key) != expected[key]:
                raise SceneProcessCollectionError(
                    f"V2 {plan_role} orchestrator environment changed at {key}"
                )
        elif key in os.environ:
            raise SceneProcessCollectionError(
                f"V2 {plan_role} orchestrator environment retained forbidden {key}"
            )
...
def _worker_argv_rocm(**kwargs: Any) -> list[str]:
    ...
    if (
        ...
        raise SceneProcessCollectionError(
            "V1 worker argv changed from the exact V2 ROCm launcher contract"
        )
...
def _configured_predecessor_collector_rocm() -> Iterator[None]:
    """Install V2 identity before entering the inherited runtime adapter."""
    ...
    with _CONFIGURATION_LOCK:
        ...
        original_validate_authority = lower._validate_authority_v2  # noqa: SLF001
        try:
            for name, value in overrides.items():
                setattr(predecessor, name, value)
            lower._validate_authority_v2 = (  # noqa: SLF001
                _validate_authority_v2_review_bound
            )
            with predecessor._configured_predecessor_collector_rocm():  # noqa: SLF001
                yield
        finally:
            lower._validate_authority_v2 = original_validate_authority  # noqa: SLF001
...
def load_and_validate_rocm(*args: Any, **kwargs: Any):
    with _configured_predecessor_collector_rocm():
        return predecessor.predecessor.load_and_validate_v2(*args, **kwargs)
...
def validate_scene_process_evidence_rocm(*args: Any, **kwargs: Any):
    with _configured_predecessor_collector_rocm():
        return predecessor.predecessor.validate_scene_process_evidence_v2(
            *args, **kwargs
        )
...
def validate_scene_process_closure_rocm(*args: Any, **kwargs: Any):
    with _configured_predecessor_collector_rocm():
        return predecessor.predecessor.validate_scene_process_closure_v2(
            *args, **kwargs
        )
...
def collect_rocm(
    *,
    ...
    with _configured_predecessor_collector_rocm():
        return predecessor.predecessor.collect_v2(
            plan_path=plan_path,
...
def build_parser():
    parser = predecessor.predecessor.build_parser()
    parser.description = (
        "Collect the separately authorized fresh Genesis ROCm V2 successor."
    )
    return parser
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    require_exact_orchestrator_environment(_plan_role_for_identity())
    with _configured_predecessor_collector_rocm():
        return predecessor.predecessor.main(argv)
