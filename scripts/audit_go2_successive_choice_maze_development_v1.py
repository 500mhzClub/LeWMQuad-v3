#!/usr/bin/env python3
"""Read-only physical/causal/policy replay; writes a separately bound audit."""
import argparse
import hashlib
import json
import math
...
def audit_trial(directory,spec,row,template):
    ...
    if eligible:
        expected_ingested=[i for i,r in enumerate(camera) if r['timestamp_s']<=float(raw['timestamp_s'][start])]
        expected_ingested+=sorted({next(i for i,r in enumerate(camera) if r['physical_sample_index']==e['post_sample_index'])
            for e in tape if e['stage']=='control' and e['post_sample_index']-e['pre_sample_index']==50})
        check(observed==expected_ingested,'continuous live history coverage')
...
def main():
    ...
    target=OUTPUT/('raw_artifact_audit.json' if count==144 else f'raw_artifact_audit_interim_{count:03d}.json')
    ...
    try:
        ...
        write_json(target,audit); print(json.dumps({k:v for k,v in audit.items() if k!='trials'},indent=2))
