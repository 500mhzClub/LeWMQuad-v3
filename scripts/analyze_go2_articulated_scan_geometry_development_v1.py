...
OUTPUT = ROOT / '.generated/go2_articulated_scan_geometry_development_v1_attempt_001'
POPULATIONS = (
...
def contact_geometry(spec, pose, joint_position, contact, model):
    ...
    group = [r for r in shapes if r['urdf_rigid_group'] == contact['robot_link_name']]
    if not group:
...
def main():
    ...
    try:
        for group, directory, spec, member in work:
            ...
            native_names = set(topology['link_names'].values())
            frames, maximum_fk = [], 0.
            for decision in decisions:
                ...
                if any(s['urdf_rigid_group'] not in native_names for s in state['shapes']):
            ...
            for event in events:
                for contact in event['disallowed_contacts']:
                    ...
                    contacts.append({'sample_index': i, 'timestamp_s': event['timestamp_s'],
                                     **contact_geometry(spec, poses[i], joints[i], contact, model)})
