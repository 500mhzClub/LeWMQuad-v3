#!/usr/bin/env python3
"""Authority-free, one-process V7 executor for the frozen N5 experiment.
...
if __name__ == "__main__":
    ...
    STAGING_PREFIX = ".n5.reservation-v7-"
    LOCK_NAME = ".n5.reservation-v7.lock"
    STAGING_SCHEMA = "lewm_go2_n5_full_panel_v7_preclaim_staging_v1"
    RECOVERY_POLICY = {
    ...
    def _directory_flags() -> int:
        ...
        if not nofollow or not directory:
            raise PermissionError("V7 canonical directory walks require no-follow opens")
        return os.O_RDONLY | nofollow | directory | getattr(os, "O_CLOEXEC", 0)


    @dataclass
    class DirectoryEntryV7:
        parent_fd: int
        name: str
        child_fd: int
    ...
    class DirectorySnapshotV7:
        fingerprint: tuple[int, ...]
        inventory: tuple[tuple[str, tuple[int, ...]], ...]


    @dataclass(frozen=True)
    class JournalEventV7:
        watch: int
        mask: int
        cookie: int
        name: str
    ...
    @dataclass
    class DirectoryWatchV7:
        watch: int
        generation: int
        directory_fd: int
        identity: tuple[int, int]
        label: str
        snapshot: DirectorySnapshotV7


    class OwnedDirectoryJournalV7:
        ...
        def __init__(self) -> None:
            ...
            if renameat2 is None:
                ...
                raise PermissionError(
                    "V7 requires Linux renameat2 for no-replace publication"
                )
            renameat2.argtypes = [
                ctypes.c_int,
                ctypes.c_char_p,
            ...
            self._fd = descriptor
            self._by_watch: dict[int, DirectoryWatchV7] = {}
            self._by_identity: dict[tuple[int, int], DirectoryWatchV7] = {}
            self._generation = 0
        ...
        def _fail(self, reason: str) -> None:
            if self._poison is None:
                self._poison = reason
            raise PermissionError(f"V7 owned-directory journal rejected: {reason}")

        @staticmethod
        def _identity(directory_fd: int) -> tuple[int, int]:
            metadata = os.fstat(directory_fd)
            if not stat.S_ISDIR(metadata.st_mode):
                raise PermissionError("V7 journal target is not a directory")
        ...
        @staticmethod
        def _snapshot(directory_fd: int) -> DirectorySnapshotV7:
            opened = os.fstat(directory_fd)
            if not stat.S_ISDIR(opened.st_mode):
                raise PermissionError("V7 journal directory descriptor changed")
            rows: list[tuple[str, tuple[int, ...]]] = []
            for name in sorted(os.listdir(directory_fd)):
                if Path(name).name != name or name in {"", ".", ".."}:
                    raise PermissionError("V7 journal observed a malformed child name")
                metadata = os.stat(
                    name,
                    dir_fd=directory_fd,
            ...
            return DirectorySnapshotV7(
                fingerprint=_stable_fingerprint(opened),
                inventory=tuple(rows),
            )

        def _events(self) -> list[JournalEventV7]:
            events: list[JournalEventV7] = []
            while True:
                try:
        ...
        def _reject_special(self, events: Sequence[JournalEventV7]) -> None:
            for event in events:
        ...
        def _shared_parent_events(
            self,
            events: Sequence[JournalEventV7],
            *,
            watch: int,
            reserved_name: str,
            allow_ignored: bool = False,
        ) -> tuple[list[JournalEventV7], list[JournalEventV7]]:
            ...
            reserved: list[JournalEventV7] = []
            ignored: list[JournalEventV7] = []
            for event in events:
                if event.mask & self.IN_Q_OVERFLOW:
                ...
                if event == JournalEventV7(watch, self.IN_IGNORED, 0, ""):
        ...
        def create_exclusive_root(
            ...
            *,
            ...
            unrelated child names are tolerated, while the V7 name must have
            ...
            try:
                ...
                if ignored != [
                    JournalEventV7(watch, self.IN_IGNORED, 0, "")
                ]:
        ...
        def watch_directory(
            ...
            *,
            label: str,
            expected_snapshot: DirectorySnapshotV7 | None = None,
        ) -> None:
            self.assert_clean()
        ...
        def _state(self, directory_fd: int) -> DirectoryWatchV7:
            identity = self._identity(directory_fd)
        ...
        def baseline(self, directory_fd: int) -> DirectorySnapshotV7:
            return self._state(directory_fd).snapshot

        def _snapshots(self) -> dict[int, DirectorySnapshotV7]:
            return {
        ...
        def _validate_snapshots(
            self,
            before: Mapping[int, DirectorySnapshotV7],
            after: Mapping[int, DirectorySnapshotV7],
            *,
        ...
        def _commit_snapshots(
            self,
            snapshots: Mapping[int, DirectorySnapshotV7],
        ) -> None:
        ...
        def _begin(
            self,
            directory_fd: int,
        ) -> tuple[DirectoryWatchV7, dict[int, DirectorySnapshotV7]]:
            self.assert_clean()
        ...
        def _finish(
            ...
            before: Mapping[int, DirectorySnapshotV7],
            *,
            expected_events: Sequence[JournalEventV7],
        ...
        def mkdir(self, parent_fd: int, name: str, *, mode: int = 0o700) -> None:
            ...
            self._finish(
                ...
                before,
                expected_events=(
                    JournalEventV7(state.watch, self.IN_CREATE | self.IN_ISDIR, 0, name),
                ),
        ...
        def create_file(
            ...
            *,
            ...
            expected_events = [
                JournalEventV7(state.watch, self.IN_CREATE, 0, name),
            ]
            if payload:
                expected_events.append(
                    JournalEventV7(state.watch, self.IN_MODIFY, 0, name)
                )
            expected_events.append(
                JournalEventV7(state.watch, self.IN_CLOSE_WRITE, 0, name)
            )
            expected = self._replace_inventory(
        ...
        def unlink(
            ...
            *,
            ...
            self._finish(
                ...
                before,
                expected_events=(
                    JournalEventV7(state.watch, self.IN_DELETE, 0, name),
                ),
                expected_inventory=expected,
            )
        ...
        def rmdir(
            ...
            *,
            ...
            if retired != [
                JournalEventV7(child.watch, self.IN_DELETE_SELF, 0, ""),
                JournalEventV7(child.watch, self.IN_IGNORED, 0, ""),
            ]:
    ...
    @dataclass
    class CanonicalDirectoryChainV7:
        anchor_fd: int
        anchor_identity_security: tuple[int, ...]
        descriptors: list[int]
        entries: list[DirectoryEntryV7]
        path_fds: dict[Path, int]
        journal: OwnedDirectoryJournalV7
        output_root_created: bool = False
        closed: bool = False


    @dataclass(frozen=True)
    class OwnedArtifactV7:
        role: str
        parent_fd: int
        name: str
        fingerprint: tuple[int, ...]
    ...
    def _assert_directory_chain(chain: CanonicalDirectoryChainV7) -> None:
        if chain.closed:
            raise PermissionError("V7 canonical directory chain is closed")
        chain.journal.assert_clean()
        if (
            _identity_security_fingerprint(os.fstat(chain.anchor_fd))
            != chain.anchor_identity_security
        ):
            raise PermissionError("V7 filesystem-root descriptor changed")
        for entry in chain.entries:
            ...
            if (
                ...
                raise PermissionError("V7 canonical directory component changed")


    def _open_canonical_directory_chain(final_path: Path) -> CanonicalDirectoryChainV7:
        ...
        if (
            ...
            raise PermissionError("V7 canonical directory target escaped the repository")
        ...
        journal = OwnedDirectoryJournalV7()
        chain = CanonicalDirectoryChainV7(
            anchor_fd=anchor_fd,
        ...
        try:
            if (
                ...
                raise PermissionError("V7 filesystem root changed during open")
            parent_fd = anchor_fd
            ...
            for index, component in enumerate(final_path.parts[1:]):
                ...
                try:
                    ...
                    if index < repository_depth:
                        raise PermissionError("V7 repository root component is missing")
                    if current_path / component == policy.CANONICAL_OUTPUT_ROOT:
                        ...
                        if not chain.output_root_created:
                            raise PermissionError(
                                "existing V7 recovery tree is missing a structural directory"
                            )
                    ...
                    if (
                        ...
                        raise PermissionError(
                            "V7 exclusive root changed before retained open"
                        )
                if stat.S_ISLNK(before.st_mode) or not stat.S_ISDIR(before.st_mode):
                    raise PermissionError("V7 canonical component is not a real directory")
                ...
                if (
                    ...
                    raise PermissionError("V7 canonical component changed during open")
                entry = DirectoryEntryV7(
                    parent_fd,
                    component,
                    child_fd,
                    identity_security,
    ...
    def _open_chain_child(
        ...
        try:
            ...
            if not chain.output_root_created:
                raise PermissionError(
                    "existing V7 recovery tree is missing a derived directory"
                )
            chain.journal.mkdir(parent_fd, name, mode=0o700)
            before = _entry_metadata(parent_fd, name)
        if stat.S_ISLNK(before.st_mode) or not stat.S_ISDIR(before.st_mode):
            raise PermissionError("V7 derived parent is not a real directory")
        identity_security = _identity_security_fingerprint(before)
        full_fingerprint = _stable_fingerprint(before)
        ...
        if (
            ...
            raise PermissionError("V7 derived parent changed during open")
    ...
    def _close_directory_chain(chain: CanonicalDirectoryChainV7) -> None:
        if chain.closed:
            return
        chain.closed = True
        chain.journal.close()
    ...
    class AttemptReservationV7:
        ...
        directory_chain: CanonicalDirectoryChainV7 | None = None
        owned_claim_artifacts: dict[str, OwnedArtifactV7] = field(
            default_factory=dict,
            compare=False,
        )
        owned_derived_artifacts: dict[str, OwnedArtifactV7] = field(
            default_factory=dict,
            compare=False,
        )
        ...
        @property
        def journal(self) -> OwnedDirectoryJournalV7:
            if self.directory_chain is None:
                raise PermissionError("N5 full-panel V7 lacks its transaction journal")
            return self.directory_chain.journal

    def _assert_claim_fd_owned(reservation: AttemptReservationV7) -> None:
        if reservation.directory_fd < 0 or reservation.directory_identity is None:
            raise PermissionError("N5 full-panel V7 lacks an open claimed directory")
        opened = os.fstat(reservation.directory_fd)
        if not stat.S_ISDIR(opened.st_mode):
            raise PermissionError("N5 full-panel V7 claimed descriptor is not a directory")
        if (opened.st_dev, opened.st_ino) != reservation.directory_identity:
            raise PermissionError("N5 full-panel V7 claimed descriptor identity changed")


    def _assert_owned_claim(reservation: AttemptReservationV7) -> None:
        ...
        if (
            ...
            raise PermissionError("N5 full-panel V7 claimed directory changed")
        if reservation.directory_chain is None or reservation.seed_root_fd < 0:
            raise PermissionError("N5 full-panel V7 lacks its canonical directory chain")
        ...
        if (
            ...
            raise PermissionError("N5 full-panel V7 canonical claim identity changed")
        ...
        if set(os.listdir(reservation.directory_fd)) != expected_names:
            raise PermissionError("N5 full-panel V7 claimed directory inventory changed")


    def _write_claim_file_exclusive(
        reservation: AttemptReservationV7,
        name: str,
        payload: bytes,
        *,
        mode: int = 0o600,
        ...
        if Path(name).name != name or name in {"", ".", ".."}:
            raise PermissionError("N5 full-panel V7 claim filename escaped")
        ...
        reservation.owned_claim_artifacts[name] = OwnedArtifactV7(
            role=role or name,
        ...
        if _stable_fingerprint(committed) != committed_fingerprint:
            raise PermissionError("N5 full-panel V7 committed claim leaf changed")
        if require_canonical:
            _assert_owned_claim(reservation)
        else:
            _assert_claim_fd_owned(reservation)
        ...
        *,
        ...
        try:
            ...
            if (
                ...
                raise PermissionError("N5 full-panel V7 claim leaf is not singly linked")
            chunks: list[bytes] = []
            ...
            if fingerprint(before) != fingerprint(after):
                raise RuntimeError("N5 full-panel V7 claim leaf changed while read")
        finally:
            os.close(descriptor)
        if require_canonical:
            _assert_owned_claim(reservation)
    ...
    def _classify_staging(
        ...
        *,
        attempt_path: Path,
        review_binding: Mapping[str, str],
    ) -> tuple[dict[str, Any], AttemptReservationV7 | None]:
        ...
        journal.assert_clean()
        return evidence, AttemptReservationV7(
            directory=attempt_path,
    ...
    def _complete_recoveries_are_equivalent(
        first: AttemptReservationV7,
        second: AttemptReservationV7,
    ) -> bool:
    ...
    def _validate_recovery_tree_baseline(
        ...
        """Reject historical bytes outside the sole recoverable V7 shape."""

        chain.journal.assert_clean()
        relative_seed = seed_root.relative_to(policy.CANONICAL_OUTPUT_ROOT)
        if len(relative_seed.parts) != 2:
            chain.journal._fail("V7 recovery seed-root depth changed")
        ...
        if not required_paths <= set(chain.path_fds):
            chain.journal._fail("V7 recovery tree lacks retained descriptors")
    ...
    def _reserve_exact_attempt(source_review_file_sha256: str) -> AttemptReservationV7:
        ...
        active_staging: OwnedStagingV7 | None = None
        owned_claim_identity: tuple[int, int] | None = None
        claimed_directory_fd: int | None = None
        claimed_chain: CanonicalDirectoryChainV7 | None = None
        claimed_reservation: AttemptReservationV7 | None = None
        ...
        try:
            ...
            with _locked_seed_root(
                ...
                complete: list[
                    tuple[OwnedStagingV7, AttemptReservationV7, dict[str, Any]]
                ] = []
                ...
                if (
                    ...
                    raise PermissionError("N5 full-panel V7 staging identity changed")
                owned_claim_identity = (staging_metadata.st_dev, staging_metadata.st_ino)
                claimed_reservation = AttemptReservationV7(
                    directory=attempt_path,
    ...
    def _cleanup_owned_artifacts(
        registry: dict[str, OwnedArtifactV7],
        *,
        journal: OwnedDirectoryJournalV7,
    ...
    def _publish_success(
        ...
        *,
        ...
        if not policy.is_sha256(checkpoint_content_sha256):
            raise ValueError("N5 full-panel V7 checkpoint content hash is malformed")
    ...
    def _write_canonical_json(
        ...
        if path not in {policy.CANONICAL_METRIC_RECEIPT_PATH, policy.CANONICAL_GATE_PATH}:
            raise PermissionError("N5 full-panel V7 publication path is not canonical")
        expected_schema = (
        ...
        if (
            ...
            raise ValueError("N5 full-panel V7 derived artifact is malformed")
        raw = policy.canonical_json_bytes(value) + b"\n"
        relative = path.relative_to(policy.CANONICAL_OUTPUT_ROOT)
        if len(relative.parts) != 2:
            raise PermissionError("N5 full-panel V7 publication depth changed")
        if reservation.directory_chain is None or reservation.output_root_fd < 0:
            raise PermissionError("N5 full-panel V7 lacks its retained output root")
        ...
        if parent_fd < 0:
            raise PermissionError("N5 full-panel V7 lacks its derived parent descriptor")
        ...
        reservation.owned_derived_artifacts[relative_text] = OwnedArtifactV7(
        ...
        if _stable_fingerprint(committed) != committed_fingerprint:
            raise PermissionError("N5 full-panel V7 committed derived leaf changed")
        _assert_owned_claim(reservation)
        return policy.artifact_binding(
            relative_text,
            raw,
    ...
    def _read_owned_derived_artifact(
        ...
        if artifact is None:
            raise PermissionError("N5 full-panel V7 derived artifact is not owned")
        raw = _read_regular_at(
            artifact.parent_fd,
            artifact.name,
            expected_fingerprint=artifact.fingerprint,
        ...
        if hashlib.sha256(raw).hexdigest() != artifact.payload_sha256:
            raise PermissionError("N5 full-panel V7 derived payload changed")
        _assert_owned_claim(reservation)
        return raw
    ...
    def _run_frozen_training(
        ...
        *,
        rgb_workers: int,
    ) -> tuple[dict[str, Any], AttemptReservationV7]:
        ...
        def require(value: object) -> object:
            if value is not token:
                raise PermissionError("retained science lacks its local V7 execution token")
        ...
        def reserve(value: object) -> AttemptReservationV7:
            nonlocal claimed_reservation
            require(value)
            if claimed_reservation is not None:
                raise FileExistsError("the sole V7 attempt was already reserved")
        ...
        try:
            ...
            if claimed_reservation is None:
                raise RuntimeError(
                    "frozen V7 training returned without claiming its attempt"
                )
            _assert_owned_claim(claimed_reservation)
            launch_summary = {
                **summary,
                "schema": "lewm_go2_observable_camera_ray_fit_v4_n5_full_panel_v7_launch_summary_v1",
            }
    ...
    def _run_independent_verification(
        ...
        def require(value: object) -> object:
            if value is not token:
                raise PermissionError("retained verifier lacks its local V7 execution token")
            policy.preflight_static_authority()
            policy.preflight_source_review(
                policy.CANONICAL_SOURCE_REVIEW_PATH,
                source_review_file_sha256,
        ...
        def write_metric(path: Path, value: Mapping[str, Any]) -> dict[str, Any]:
            if Path(path) != policy.CANONICAL_METRIC_RECEIPT_PATH:
                raise PermissionError("retained verifier changed its V7 output path")
    ...
    def _run_finalization(
        ...
        def require(value: object) -> object:
            if value is not token:
                raise PermissionError("retained finalizer lacks its local V7 execution token")
            policy.preflight_static_authority()
            policy.preflight_source_review(
                policy.CANONICAL_SOURCE_REVIEW_PATH,
                source_review_file_sha256,
        ...
        def write_gate(path: Path, value: Mapping[str, Any]) -> dict[str, Any]:
            if Path(path) != policy.CANONICAL_GATE_PATH:
                raise PermissionError("retained finalizer changed its V7 output path")
            return _write_canonical_json(reservation, Path(path), value)
    ...
    def execute_exact(
        ...
        *,
        ...
        if not sys.flags.isolated:
            raise PermissionError("N5 full-panel V7 exact execution requires isolation")
        ...
        if (
            ...
            raise FileExistsError(
                "the sole N5 full-panel V7 recovery attempt is already claimed"
            )
        policy.preflight_static_authority()
        policy.preflight_source_review(
            policy.CANONICAL_SOURCE_REVIEW_PATH,
        ...
        try:
            ...
            _assert_owned_claim(reservation)
            return {
                "schema": "lewm_go2_observable_camera_ray_fit_v4_n5_full_panel_v7_end_to_end_summary_v1",
                "seed": 20260710,
                "fit_size": 5,
                "training": training,
            ...
            try:
                ...
                raise RuntimeError(
                    f"N5 full-panel V7 {stage} failed and terminal receipt "
                    "could not be written"
                ) from terminal_error
            raise
        finally:
    ...
    def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
        ...
        if not policy.is_sha256(args.source_review_sha256):
            raise ValueError("N5 full-panel V7 source review SHA-256 is malformed")
        return args
