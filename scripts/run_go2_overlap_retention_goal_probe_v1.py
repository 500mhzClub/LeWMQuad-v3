"""One fresh direct-039 native case with qualified-overlap anchor retention."""
import argparse
import contextlib
import json
...
from scripts.run_go2_retained_patch_contact_goal_probe_v1 import OUTPUT as DIAGNOSTIC
from scripts.replay_go2_overlap_retention_observer_v1 import OUTPUT as REFINEMENT
from scripts.navigation_artifact_root_development import BASE, create_output, validate_root, verify_artifacts
from scripts.run_go2_successive_choice_maze_development_v1 import digest, write_json
from scripts.startup_raw_sensor_audit_development import read_json
from scripts.read_go2_tracking_posthoc_raw_accuracy_v1 import hardware

OUTPUT=BASE/'go2_overlap_retention_goal_probe_v1_attempt_001'
READOUT=BASE/'go2_retained_patch_contact_goal_readout_v1_attempt_001'
DIAGNOSTIC_IDS={'launch.json':'9a17318bde6998f7f0c74ad1a123c79862255994f5d382ab62ae0840d4e3207a'}
...
PROTOCOL='docs/go2_overlap_retention_goal_probe_v1_2026-09-08.md'
REFINEMENT_SHA='bf51ce8ecdde75dcc9ba6a4e82e56a9e96694046561af43a4f0008382b1762d8'
TRIALS=('family_episode_039',)
CASES=[(f'{v}_{c}_{t}',t,v,c,f'seed_2026091001_{v}_{c}') for v in ('full',)
    for c in ('direct',) for t in TRIALS]
...
def worker(case,launch_sha):
    ...
    terminal=dict(case=name,trial=trial,variant=variant,condition=condition,model_name=model_name,
        status='OVERLAP_RETENTION_GOAL_WORKER_FAILED',artifact_sha256={})
    ...
    with (OUTPUT/(name+'_worker.log')).open('x') as log,contextlib.redirect_stdout(log),contextlib.redirect_stderr(log):
        try:
            ...
            terminal.update(status='OVERLAP_RETENTION_GOAL_COLLECTED_AND_RAW_AUDITED',
...
def main():
    ...
    assert diagnostic['status']=='RETAINED_PATCH_CONTACT_GOAL_PROBE_COMPLETE' and len(diagnostic['conditions'])==1
    ...
    assert readout['status']=='RETAINED_PATCH_CONTACT_GOAL_READOUT_COMPLETE' and readout['probe_result_sha256']==args.prior_result_sha256
    ...
    assert refinement['status']=='OVERLAP_RETENTION_OBSERVER_REPLAY_COMPLETE'
    assert refinement['candidate_eligible_for_separate_native_probe']
    assert [r['arm'] for r in refinement['reports']]==['original','overlap_retention']
    assert refinement['reports'][0]['original_available_evidence_exact']
    assert refinement['reports'][1]['current_joint_poses']==147
    ...
    launch=preflight(output=OUTPUT,protocol=PROTOCOL,
        seed_paths=('scripts/run_go2_overlap_retention_goal_probe_v1.py',
            'lewm/tests/test_overlap_retention_goal_probe_development.py',
            'docs/go2_retained_patch_contact_goal_probe_result_2026-09-08.md',
            'docs/go2_overlap_retention_observer_replay_result_2026-09-08.md',*readout['source_sha256'],*refinement['source_sha256']),
    ...
    launch.update(all_six_admission=receipt,fit_artifact_sha256=fit_ids,snapshots=snapshots,
        ...
        experiment='qualified half-feature-overlap anchor retention V1',
        data_scope='one known affected integration layout; not independent mazes',
        concurrency_reason='one chronological native case; no independent episode units; paused-physics single-worker timing',
    ...
    print('OVERLAP_RETENTION_GOAL_LAUNCHED',digest(OUTPUT/'launch.json'),flush=True)
    ...
    try:
                ...
                while pending:
                    ...
                    for future in done:
                        ...
                        try:record=future.result()
                            ...
                            record=dict(case=case[0],status='OVERLAP_RETENTION_GOAL_PROCESS_FAILURE',failure=repr(exc))
                        records.append(record)
                        print('OVERLAP_RETENTION_GOAL_TERMINAL',case[0],record['status'],record.get('goal'),flush=True)
            if len(records)!=1 or any(r['status']!='OVERLAP_RETENTION_GOAL_COLLECTED_AND_RAW_AUDITED' for r in records):
        ...
        for root,ids in scaling['phase_result_sha256'].items():verify_artifacts(Path(root),ids)
        write_json(OUTPUT/'result.json',dict(status='OVERLAP_RETENTION_GOAL_PROBE_COMPLETE',
        ...
        print('OVERLAP_RETENTION_GOAL_COMPLETE',digest(OUTPUT/'result.json'),flush=True)
    except Exception as exc:
        write_json(OUTPUT/'failure.json',dict(status='TERMINAL_OVERLAP_RETENTION_GOAL_FAILURE',reason=repr(exc),
