...
def audit(layout_index, result, definition, *, input_root, model, robot_geometry, episode_name, condition, variant):
    ...
    assert result['recent_qualified_anchor_enabled'] is True
    assert result['reached_frontier_transition_enabled'] is True
    ...
    controller = ReachedFrontierRecentQualifiedController(model, robot_geometry, public_mission=mission,
    ...
    return dict(reached_frontier_transition_enabled=True, layout_index=layout_index, renderer_capture_audit=renderer, measured_floor_transport_enabled=True,
