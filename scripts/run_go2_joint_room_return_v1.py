"""Fresh joint tracking plus unchanged inner-goal room-return execution."""
...
OUTPUT=BASE/'go2_joint_room_return_v1_attempt_001'
PROTOCOL='docs/go2_joint_room_return_v1_2026-09-07.md'
IDENTITIES={'launch.json':'53ec5a2a04831ddbc8ae3932038ce90a5f12c797d38087bb6fb95a3e8778667a',
...
def preflight():
    ...
    witness_name='docs/go2_joint_continuity_comparison_scientific_readout_2026-09-07.json'
    witness_ids={witness_name:'5945d0fbf985dfd45f62202a59d546c1375ab6d7439316d653cd4a0c53b8cda3'}
    verify_bindings(witness_ids);inputs|=witness_ids
    witness=read_json(ROOT,witness_name)
    if not witness['criteria']['development_continuation_criterion_met']:
        raise ValueError('joint development continuation criterion required')
    verify_bindings(witness['source_sha256'])
    verify_artifacts(BASE/'go2_joint_continuity_comparison_v1_attempt_001',
        witness['artifact_sha256']|{'result.json':witness['experiment_result_sha256']})
    inherited=old['source_sha256']|witness['source_sha256']
    sources=discover_sources((PROTOCOL,'scripts/run_go2_joint_room_return_v1.py',
        'scripts/audit_go2_joint_room_return_v1.py',
        'lewm/tests/test_joint_pulse_execution_development.py',
        'lewm/tests/test_joint_room_return_collection_development.py'),inherited)
    resources=hardware()
    if resources['memory_available_bytes']<24*1024**3:
        raise ValueError('native collection needs 24 GiB available RAM')
    ...
    launch|=dict(source_sha256=sources,input_sha256=inputs,protocol=PROTOCOL,controller_stages=STAGES,
        ...
        paired_factor='multi_reference_gyro_to_joint_measured_continuity_observer_family',
        hardware=resources,native_scene_workers=1,opencv_threads=1,
        concurrency_reason='uncontended full-loop timing; native parallel scaling not established',
        internal_position_tolerance_m=.04,external_position_tolerance_m=.06,
        maximum_control_ticks=3600,maximum_terminal_zero_ticks=10,model='joint',
...
def collect(condition,definition):
    ...
    try:
        ...
        controller=JointInnerGoalRoomReturn(spec['turn_sign'],load_fixed_table())
        def execute(command,phase,role):
            ...
            try: session.command_tick(command); item['completed']=True
                ...
                item['wall_ms']=(time.perf_counter_ns()-command_start)/1e6
        try:
            ...
            for tick in range(3601):
                ...
                acquisition_ms=(time.perf_counter_ns()-start)/1e6
                ...
                control_ms=(time.perf_counter_ns()-control_start)/1e6
                decisions.append(dict(tick=tick,observation_index=len(session.model_manifest)-1,evidence=evidence,decision=decision,
                    ...
                    acquisition_wall_ms=acquisition_ms,controller_wall_ms=control_ms,
                    decision_elapsed_wall_ms=(time.perf_counter_ns()-loop_start)/1e6,
                    iteration_wall_ms=None,command_ticks_before=len(tape),command_ticks_after=None,
                ...
                if tick%50==0 or decision['terminal']:
                    ...
                    print('JOINT_ROOM_SERVO',condition,tick,decision['stage'],decision['terminal'],
                ...
                if decision['terminal']:
                    ...
                    decisions[-1].update(iteration_wall_ms=(time.perf_counter_ns()-loop_start)/1e6,command_ticks_after=len(tape))
                    break
                local=decision['execution']['local_decision']
                try: execute(decision['requested_command'],local['phase'] if local else 2,'visual_feedback')
                finally:
                    decisions[-1].update(iteration_wall_ms=(time.perf_counter_ns()-loop_start)/1e6,command_ticks_after=len(tape))
            if final_control is None: raise ValueError('controller must be terminal by fixed maximum')
        except PhysicalStop as error:
            if decisions and decisions[-1]['iteration_wall_ms'] is None:
                decisions[-1].update(iteration_wall_ms=(time.perf_counter_ns()-loop_start)/1e6,command_ticks_after=len(tape))
