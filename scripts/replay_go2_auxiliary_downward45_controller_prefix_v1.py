"""Fixed corrected-model replay with the explicitly calibrated 45-degree camera."""
import json
import time
import cv2
import numpy as np
...
from scripts.capture_go2_auxiliary_downward45_depth_prefix_v1 import OUTPUT as INPUT
from scripts.read_go2_auxiliary_depth_reobserve_goal_probe_v1 import OUTPUT as READOUT
from scripts.run_go2_auxiliary_depth_reobserve_goal_probe_v1 import OUTPUT as PRIOR,CORRECTION,FITS,CASES
...
OUTPUT=BASE/'go2_auxiliary_downward45_controller_prefix_v1_attempt_001'
PROTOCOL='docs/go2_auxiliary_downward45_controller_prefix_v1_2026-09-08.md'
INPUT_SHA='92f32aad3d2bde466a073df6410bf7060dfc9a1a1cdf5b9520fabcf8eb4268e4'
READOUT_SHA='2fe50e68c57ebec738daebfc70f589752fda1dc7c43de2dfdc6f93a10efd9e8e'
PRIOR_SHA='3675f09b90770ed9345154b8fe926c66fd6029e9c7bc7a6f2d53cba0c6861193'


def replay(admission,model_name,reader,acquisitions):
    model,condition,variant=load_assigned(admission,model_name)
    before=state_digest(model.state_dict());controller=AuxiliaryDownward45GoalProbe(model,ArticulatedCollisionGeometry(URDF),
        condition=condition,variant=variant,persistent=True)
    rows=[]
    for i in range(len(reader.frames)):
        p,d,f,now=reader.packet(i)
        auxiliary=auxiliary_packet(INPUT/'sensor_prefix',i,p,public_acquisition(acquisitions[i]),now_ns=now)
        rows.append(controller.observe(p,d,f,now_ns=now,auxiliary_depth=auxiliary))
...
def main():
    ...
    bound=[]
    for root,sha,status in ((INPUT,INPUT_SHA,'AUXILIARY_DOWNWARD45_DEPTH_PREFIX_COMPLETE'),
            (READOUT,READOUT_SHA,'AUXILIARY_DEPTH_REOBSERVE_GOAL_READOUT_COMPLETE'),
            (PRIOR,PRIOR_SHA,'AUXILIARY_DEPTH_REOBSERVE_GOAL_PROBE_COMPLETE')):
    ...
    assert bound[0][2]['raw_audit_pass']
    ...
    sources=discover_sources((PROTOCOL,'scripts/replay_go2_auxiliary_downward45_controller_prefix_v1.py',
        'lewm/tests/test_recorded_command_prefix_comparison_development.py',
        'docs/go2_auxiliary_downward45_depth_prefix_result_2026-09-08.md',
        'lewm/tests/test_auxiliary_downward45_depth_observation_development.py','lewm/tests/test_auxiliary_downward45_floor_map_development.py',
        'lewm/tests/test_auxiliary_downward45_controller_scope_development.py','lewm/tests/test_auxiliary_downward45_packet_replay_development.py'),bound[0][2]['source_sha256'])
    resources=hardware()
    ...
    launch=old|dict(source_sha256=sources,protocol=PROTOCOL,output_root=str(OUTPUT),hardware=resources,
        ...
        auxiliary_depth_used=True,shadow_replay_only=True,complete_replay_passes_per_model=2)
    verify(launch);create_output(OUTPUT);write_json(OUTPUT/'launch.json',launch)
    print('AUXILIARY_DOWNWARD45_CONTROLLER_PREFIX_LAUNCHED',digest(OUTPUT/'launch.json'),flush=True);start=time.perf_counter()
    try:
        reader=IntentReturnRGBDReplay(INPUT/'sensor_prefix');assert len(reader.frames)==18
        tape=read_json(INPUT/'sensor_prefix','command_tape.json');assert len(tape)==17
        acquisitions=read_json(INPUT/'sensor_prefix','auxiliary_camera_audit.json');assert len(acquisitions)==18
        ...
        for case,trial,variant,condition,model_name in CASES:
            reference=[r['decision'] for r in read_json(PRIOR/case,'context_decisions.json')[:18]]
            ...
            records,state=replay(admission,model_name,reader,acquisitions);again,state2=replay(admission,model_name,reader,acquisitions)
            ...
            for i,r in enumerate(records):
                ...
                pose_rows.append(dict(frame=i,controller_failure=r['failure'],terminal=r['terminal'],
                    ...
                    auxiliary_receipt=r['auxiliary_floor_partition_receipt'],
        ...
        write_json(OUTPUT/'result.json',dict(status='AUXILIARY_DOWNWARD45_CONTROLLER_PREFIX_COMPLETE',
            ...
            auxiliary_depth_used=True,shadow_replay_only=True,prospective_closed_loop=False,
            full_mission_compatibility_established=False,navigation_qualified=False,goal_achieved=False))
        print('AUXILIARY_DOWNWARD45_CONTROLLER_PREFIX_COMPLETE',digest(OUTPUT/'result.json'),flush=True)
    except Exception as error:
        write_json(OUTPUT/'failure.json',dict(status='TERMINAL_AUXILIARY_CONTROLLER_PREFIX_FAILURE',reason=repr(error)));raise
