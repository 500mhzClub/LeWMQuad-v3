#!/usr/bin/env python3
"""Build or verify the 91-file rigid-BEV transport source closure."""
...
contract = _source_module(
    "_lewm_global_rigid_bev_transport_closure_contract",
    CONTRACT_RELATIVE_PATH,
)
if (
    ...
    raise PermissionError("rigid-BEV transport source-closure identity changed")
_V3 = _source_module(
    "_lewm_global_rigid_bev_transport_frozen_v3_closure",
    FROZEN_V3_CHECKER_RELATIVE_PATH,
)
...
if (
    ...
    raise PermissionError("rigid transport closure is not V3 plus seven files")
...
if not REQUIRED_DYNAMIC_SOURCE_PATHS.issubset(
    ...
    raise PermissionError(f"rigid transport dynamic source roots missing: {missing}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    print("Go2 global rigid-BEV transport joint-JEPA V1 closure: PASS")
    return 0
