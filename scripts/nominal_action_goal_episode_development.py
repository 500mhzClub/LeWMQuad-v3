...
def collect(trial, definition, *, output, model, geometry, persistent, episode_name, condition, variant):
    ...
    try:
        ...
        session._runtime['high_level'] = 'nominal_action_goal_probe_v1; online model plus measured surfaces'
        ...
        controller = NominalActionGoalProbe(model, geometry, persistent=persistent, condition=condition, variant=variant)
        try:
        ...
        result = dict(status='NOMINAL_ACTION_GOAL_PROBE_TERMINAL_AUDIT_REQUIRED', trial=trial,
