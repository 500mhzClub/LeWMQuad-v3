#!/usr/bin/env python3
"""Fail-closed launcher for the certified masked-spatial JEPA V1 source."""
...
def validate_certified_source_v1(
    ...
    identity = _binding(
        authority.get("clean_export_certification"), name="certification"
    )
...
def validate_gpu_v1(torch: Any) -> dict[str, Any]:
    if not bool(torch.cuda.is_available()) or int(torch.cuda.device_count()) != 1:
        raise RuntimeError("masked-spatial V1 requires exactly one visible GPU")
    name = str(torch.cuda.get_device_name(0))
    if "R9700" not in name.replace(" ", "").upper():
        raise RuntimeError("visible GPU is not the registered R9700")
    return {
        "status": "PASS_EXACTLY_ONE_VISIBLE_R9700",
...
def main() -> int:
    ...
    result = launch_authorized_v1(args.authority)
