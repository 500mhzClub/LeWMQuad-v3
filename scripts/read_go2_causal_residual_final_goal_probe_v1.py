...
import numpy as np
from scripts.run_go2_causal_residual_final_goal_probe_v1 import OUTPUT as INPUT,PREVIOUS as PRIOR,PREVIOUS_SHA as PRIOR_SHA,CASES,TRIAL
...
OUTPUT=BASE/'go2_causal_residual_final_goal_readout_v1_attempt_001'
PROTOCOL='docs/go2_causal_residual_final_goal_readout_v1_2026-09-08.md'
...
def constraint_summary(selection):
    ...
    for surface,nominal,path in zip(selection['surface_checks'],selection['nominal_action_checks'],
        ...
        records.append(dict(action=action,phase_allowed=action in selection['phase_allowed_actions'],
            ...
            collision_contact_policy=surface['collision_contact_policy'],
            coverage_required_surface_intersection=surface['coverage_required_possible_intersection'],
...
def summarize(record):
    ...
    return dict(case=name,model_name=record['model_name'],condition=record['condition'],
        ...
        exact_goal_target_trace=[dict(tick=r['tick'],**r['decision']['new_selection']['terminal_goal_target_evidence'])
            for r in rows if (r['decision']['new_selection'] or {}).get('terminal_goal_target_evidence') is not None],
        ...
        eight_step_planning_changed_selections=sum(s.get('eight_step_final_goal_action',s['action'])!=s['first_step_action'] for s in forecast),
        final_goal_scoring_changed_selections=sum(s.get('uncorrected_final_goal_action',s['action'])!=s.get('eight_step_final_goal_action',s['action']) for s in forecast),
        causal_score_changed_selections=sum(s['action']!=s.get('uncorrected_final_goal_action',s['action']) for s in forecast),
        final_goal_scoring_trace=[dict(tick=s['tick'],previous_action=s['eight_step_final_goal_action'],
            action=s.get('uncorrected_final_goal_action',s['action']),scored_pose_horizon_ns=s['scored_pose_horizon_ns'],
            scored_contact_horizon_ns=s['scored_contact_horizon_ns'])
            for s in forecast if s.get('final_goal_execution_horizon_scoring')],
        causal_score_trace=[dict(tick=s['tick'],uncorrected_action=s['uncorrected_final_goal_action'],
            action=s['action'],residual=s['causal_score_residual_receipt'])
            for s in forecast if s.get('causal_final_goal_scoring')],
        causal_residual_trace=[dict(tick=r['tick'],receipt=r['decision']['causal_residual_receipt']) for r in rows],
...
def main():
    ...
    assert result['status']=='CAUSAL_RESIDUAL_FINAL_GOAL_PROBE_COMPLETE' and result['cases']==[list(c) for c in CASES]
    ...
    sources=discover_sources((PROTOCOL,'scripts/read_go2_causal_residual_final_goal_probe_v1.py',
        'lewm/tests/test_causal_residual_final_goal_readout_development.py'),old['source_sha256'])
    ...
    try:
        ...
        comparisons=[compare(PRIOR/c[0],INPUT/c[0],auxiliary_pair=True) for c in CASES]
        ...
        write_json(OUTPUT/'result.json',dict(status='CAUSAL_RESIDUAL_FINAL_GOAL_READOUT_COMPLETE',
        ...
        print('CAUSAL_RESIDUAL_FINAL_GOAL_READOUT_COMPLETE',digest(OUTPUT/'result.json'),flush=True)
    except Exception as error:
        write_json(OUTPUT/'failure.json',dict(status='TERMINAL_CAUSAL_RESIDUAL_FINAL_GOAL_READOUT_FAILURE',reason=repr(error)));raise
