...
from scripts.auxiliary_depth_reobserve_goal_episode_development import collect,artifacts
from scripts.auxiliary_depth_reobserve_goal_audit_development import audit
...
from scripts.replay_go2_auxiliary_depth_reobserve_prefix_v1 import OUTPUT as INTEGRATION
from scripts.capture_go2_auxiliary_tilted_depth_prefix_integrity_v2 import OUTPUT as SENSOR_PREFIX
...
PREVIOUS=BASE/'go2_auxiliary_depth_goal_probe_v1_attempt_001'
DIAGNOSTIC=BASE/'go2_auxiliary_depth_goal_readout_v1_attempt_001'
PREVIOUS_SHA='f8bcad37fdf1623c30eb6782c4514b483473e8c6a4f3e404d77ab71cc0f80f21'
DIAGNOSTIC_SHA='db1440338a68b8d8c2a04a7c11fa7b43e6bccd79ee428f607202936e6ddcfacb'
OUTPUT=BASE/'go2_auxiliary_depth_reobserve_goal_probe_v1_attempt_001'
...
SENSOR_PREFIX_SHA='e8de0873c71f50c5a01793c0464ba2014c1a7fe72f77daf47204eaeee1c8c38a'
SCALING_SHA='b593dfe4b8924d9bf8cdd9e6e382a3772575e332444d56c7dbb8aed20031ef79'
PROTOCOL='docs/go2_auxiliary_depth_reobserve_goal_probe_v1_2026-09-08.md'
...
def worker(case,launch_sha):
    ...
    terminal=dict(case=name,trial=trial,variant=variant,condition=condition,model_name=model_name,
        status='AUXILIARY_DEPTH_REOBSERVE_GOAL_WORKER_FAILED',artifact_sha256={});started=time.perf_counter()
    with (OUTPUT/(name+'_worker.log')).open('x') as log,contextlib.redirect_stdout(log),contextlib.redirect_stderr(log):
        try:
            ...
            terminal.update(status='AUXILIARY_DEPTH_REOBSERVE_GOAL_COLLECTED_AND_RAW_AUDITED',artifact_sha256=bindings,
...
def main():
    ...
    assert previous['status']=='AUXILIARY_DEPTH_GOAL_PROBE_COMPLETE' and previous['cases']==[list(c) for c in CASES]
    ...
    assert diagnostic['status']=='AUXILIARY_DEPTH_GOAL_READOUT_COMPLETE'
    ...
    assert integration['status']=='AUXILIARY_DEPTH_REOBSERVE_PREFIX_COMPLETE' and len(integration['conditions'])==2
    assert all(r['original_decisions_exact_before_terminal_intervention'] and not r['controller_failures'] and r['model_state_unchanged'] and r['exact_fresh_replay_passes']==2 for r in integration['conditions'])
    assert any(r['final_recovery_count']>0 for r in integration['conditions'])
    ...
    assert sensor_prefix['status']=='AUXILIARY_TILTED_DEPTH_PREFIX_INTEGRITY_V2_COMPLETE' and sensor_prefix['raw_audit_pass']
    ...
    launch=preflight(output=OUTPUT,protocol=PROTOCOL,
        seed_paths=('scripts/run_go2_auxiliary_depth_reobserve_goal_probe_v1.py',
            'lewm/tests/test_auxiliary_depth_reobserve_goal_scope_development.py',
            'docs/go2_auxiliary_depth_reobserve_prefix_result_2026-09-08.md',
    ...
    launch.update(integration_artifact_sha256=integration_ids,sensor_prefix_artifact_sha256=sensor_ids,
        ...
        experiment='same paired sensing and corrected planner; allow at most ten active zero-command reobservations after valid forecast infeasibility',
    ...
    print('AUXILIARY_DEPTH_REOBSERVE_GOAL_LAUNCHED',digest(OUTPUT/'launch.json'),flush=True)
    ...
    try:
            ...
            with ProcessPoolExecutor(max_workers=2,mp_context=multiprocessing.get_context('spawn'),max_tasks_per_child=1) as pool:
                ...
                while pending:
                    ...
                    for future in done:
                        ...
                        except Exception as error:row=dict(case=case[0],status='AUXILIARY_DEPTH_REOBSERVE_GOAL_PROCESS_FAILURE',failure=repr(error))
                        records.append(row);print('AUXILIARY_DEPTH_REOBSERVE_GOAL_TERMINAL',case[0],row['status'],row.get('goal'),flush=True)
        if len(records)!=2 or any(r['status']!='AUXILIARY_DEPTH_REOBSERVE_GOAL_COLLECTED_AND_RAW_AUDITED' for r in records):
        ...
        result=dict(status='AUXILIARY_DEPTH_REOBSERVE_GOAL_PROBE_COMPLETE',cases=CASES,conditions=records,
        ...
        write_json(OUTPUT/'result.json',result);print('AUXILIARY_DEPTH_REOBSERVE_GOAL_COMPLETE',digest(OUTPUT/'result.json'),flush=True)
    except Exception as error:
        write_json(OUTPUT/'failure.json',dict(status='TERMINAL_AUXILIARY_DEPTH_REOBSERVE_GOAL_FAILURE',reason=repr(error),
