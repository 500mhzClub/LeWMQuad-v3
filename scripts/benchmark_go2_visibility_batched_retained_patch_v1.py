...
SOURCE='scripts/benchmark_go2_visibility_batched_retained_patch_v1.py'
TEST='lewm/tests/test_visibility_batched_retained_floor_patch_development.py'
OUTPUT=ROOT/'docs/go2_visibility_batched_retained_patch_microbenchmark_2026-09-11.json'
REPEATS=30
FRAMES=1428
...
def counted(memory):
    counts=dict(recorder_calls=0,empty_visibility_rows=0,nonempty_visibility_rows=0)
    def record(*args):
    ...
    clone=FunctionType(fn.__code__,fn.__globals__|{'record_coverage':record},fn.__name__,fn.__defaults__,fn.__closure__)
...
def main():
    ...
    verify(sources)
    write_json(OUTPUT,dict(status='VISIBILITY_BATCHED_RETAINED_PATCH_MICROBENCHMARK_COMPLETE',
        utc=datetime.now(timezone.utc).isoformat(),source_sha256=sources,source_count=len(sources),hardware=resources,
        cases=results,synthetic_history=True,shared_read_only_prefix_arrays=True,isolated_benchmark=False,
    ...
    print('VISIBILITY_PATCH_MICROBENCHMARK_COMPLETE',digest(OUTPUT),len(sources),flush=True)
