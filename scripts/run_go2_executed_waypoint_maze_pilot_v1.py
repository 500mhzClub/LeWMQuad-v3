"""Prospective execution-horizon waypoint scoring on reused development maze 0."""
import argparse
import contextlib
import json
...
from scripts.executed_waypoint_maze_episode_development import collect, artifacts
from scripts.executed_waypoint_maze_audit_development import audit
...
from scripts.replay_go2_executed_waypoint_maze_prefix_v1 import (
...
OUTPUT = BASE/'go2_executed_waypoint_maze_pilot_v1_attempt_001'
PROTOCOL = 'docs/go2_executed_waypoint_maze_pilot_v1_2026-09-08.md'
...
def worker(launch_sha):
    ...
    terminal = dict(case=name, layout_index=index, model_name=model_name,
        status='EXECUTED_WAYPOINT_MAZE_WORKER_FAILED', artifact_sha256={}); started = time.perf_counter()
    with (OUTPUT/(name+'_worker.log')).open('x') as log, contextlib.redirect_stdout(log), contextlib.redirect_stderr(log):
        try:
            ...
            terminal.update(status='EXECUTED_WAYPOINT_MAZE_COLLECTED_AND_RAW_AUDITED', artifact_sha256=bindings,
...
def main():
    ...
    if OUTPUT.exists() or OUTPUT.is_symlink(): raise ValueError('exclusive prospective waypoint experiment; no retry/resume')
    verify_artifacts(INTEGRATION, {'result.json': args.prefix_result_sha256}); integration = read_json(INTEGRATION, 'result.json')
    assert integration['status'] == 'EXECUTED_WAYPOINT_MAZE_PREFIX_COMPLETE'
    ...
    assert prefix['exact_other_decision_fields_before_intervention'] and prefix['causal_observations_maps_mission_forecasts_and_constraints_exact']
    ...
    assert prefix['changed_selection']['executed_waypoint_scoring']
    ...
    sources = discover_sources((PROTOCOL, 'scripts/run_go2_executed_waypoint_maze_pilot_v1.py',
        'lewm/tests/test_executed_waypoint_native_scope_development.py',
        'lewm/tests/test_nominal_reentry_native_prefix_comparison_development.py',
        'docs/go2_executed_waypoint_maze_prefix_result_2026-09-08.md'), integration['source_sha256'])
    resources = hardware()
    ...
    if resources['artifact_free_bytes'] < RESERVE_BYTES+COLLECTION_ALLOWANCE_BYTES+PERSISTENCE_HEADROOM_BYTES:
        raise ValueError('declared waypoint storage/persistence envelope unavailable')
    ...
    launch.update(protocol=PROTOCOL, source_sha256=sources, output_root=str(OUTPUT), planned_case=CASE,
        ...
        predecessor_collection_allowance_bytes=11*1024**3, changed_storage_censoring_explicit=True,
        memory_admission_bytes=32*1024**3, os_resource_limits_enforced=False,
        concurrency_reason='one existing-maze waypoint intervention; storage envelope excludes parallel scenes',
    ...
    print('EXECUTED_WAYPOINT_MAZE_LAUNCHED', launch_sha, flush=True)
    try:
        ...
        print('EXECUTED_WAYPOINT_MAZE_TERMINAL', record['status'], record.get('verified_round_trip'), record.get('failure'), flush=True)
        if record['status'] != 'EXECUTED_WAYPOINT_MAZE_COLLECTED_AND_RAW_AUDITED':
            raise ValueError('native waypoint infrastructure/raw audit/prefix comparison failure; evidence retained')
        ...
        verify_artifacts(FITS, admission['base_admission']['fit_artifact_sha256'])
        verify_artifacts(CORRECTION, admission['correction_artifact_sha256'])
        write_json(OUTPUT/'result.json', dict(status='EXECUTED_WAYPOINT_MAZE_PILOT_COMPLETE', conditions=[record],
        ...
        print('EXECUTED_WAYPOINT_MAZE_COMPLETE', digest(OUTPUT/'result.json'), flush=True)
    except Exception as error:
        write_json(OUTPUT/'failure.json', dict(status='TERMINAL_EXECUTED_WAYPOINT_MAZE_PILOT_FAILURE', reason=repr(error)))
        raise
