"""Additional scoped support-query benefit over the verified combined controller."""
...
from scripts.benchmark_go2_single_pass_receipt_copied_v1 import verify_predecessors as verify_optimization_ancestry
from scripts.diagnose_go2_single_pass_receipt_phases_v1 import verify_all as verify_phase

OUTPUT=BASE/'go2_scoped_support_cache_benchmark_v1_attempt_001'
PROTOCOL='docs/go2_scoped_support_cache_benchmark_v1_2026-09-09.md'
PREDECESSORS={
    'go2_single_pass_receipt_copied_benchmark_v1_attempt_001':'d688f2ed9d30177d2e55fb98e9c9f25d2b035f2258d86449ac8e86615cd13c72',
    'go2_single_pass_receipt_phases_v1_attempt_001':'6ad04101046cdcb9e855a100a446bb0206397354614608f89a03c38d80ecdf76',
}
FRAMES=514
LABELS=('single_pass_receipt_copied','support_cached_single_pass')
...
def aggregate(rows):
    ...
    def values(group):
        ...
        old=np.asarray([r['controller_wall_ms']['single_pass_receipt_copied'] for r in group])
        new=np.asarray([r['controller_wall_ms']['support_cached_single_pass'] for r in group])
        return dict(observations=len(group),single_pass_receipt_copied_median_ms=float(np.median(old)),
            support_cached_single_pass_median_ms=float(np.median(new)),paired_median_reduction_ms=float(np.median(old-new)),
            single_pass_receipt_copied_mean_ms=float(old.mean()),support_cached_single_pass_mean_ms=float(new.mean()),
            single_pass_receipt_copied_over_100ms=int((old>100).sum()),support_cached_single_pass_over_100ms=int((new>100).sum()))
    return dict(active=values(selected),by_first_controller={label:values([r for r in selected if r['order'][0]==label])
        ...
        alternating_order_per_observation=True,concurrent_cpu_jobs_possible=True,
        end_to_end_speedup_established=False,real_time_qualified=False)


def replay(launch):
    models=[]; controllers=[]
    for cls in (SinglePassReceiptCopiedController,SupportCachedSinglePassController):
    ...
    with (OUTPUT/'paired_timings.jsonl').open('x') as stream,(OUTPUT/'resource_monitor.jsonl').open('x') as monitor:
        for i,original in enumerate(islice(read_rows(directory),FRAMES)):
            ...
            if i%64==0:
                ...
                print('SCOPED_SUPPORT_CACHE_BENCHMARK_FRAME',i,flush=True)
...
def verify_predecessors():
    sources={}
    for name,sha in PREDECESSORS.items():
        root=BASE/name
        ...
        if name=='go2_single_pass_receipt_copied_benchmark_v1_attempt_001':
            verify_inputs(launch); verify_optimization_ancestry()
        else: verify_phase(launch)
        for path,binding in result['source_sha256'].items():
            if path in sources and sources[path]!=binding: raise ValueError('conflicting optimization predecessor source: '+path)
            sources[path]=binding
    return sources
...
def main():
    ...
    if OUTPUT.exists() or OUTPUT.is_symlink(): raise ValueError('exclusive paired support-cache benchmark required')
    ...
    inherited=verify_predecessors()
    for name,sha in old['source_sha256'].items():
        if name in inherited and inherited[name]!=sha: raise ValueError('conflicting current source: '+name)
        inherited[name]=sha
    sources=discover_sources((PROTOCOL,'scripts/benchmark_go2_scoped_support_cache_v1.py',
        ...
        'docs/go2_single_pass_receipt_phases_result_2026-09-09.md'),inherited)
    launch=old|dict(protocol=PROTOCOL,source_sha256=sources,output_root=str(OUTPUT),replay_input_bindings=ids,
        ...
        optimization_predecessor_result_sha256=dict(PREDECESSORS),
        ...
        order='combined first on even observations, support-cached first on odd observations',
    ...
    print('SCOPED_SUPPORT_CACHE_BENCHMARK_LAUNCHED',digest(OUTPUT/'launch.json'),flush=True)
    try:
        report=replay(launch); verify_inputs(launch); verify_predecessors()
        ...
        write_json(OUTPUT/'result.json',dict(status='SCOPED_SUPPORT_CACHE_BENCHMARK_V1_COMPLETE',
        ...
        print('SCOPED_SUPPORT_CACHE_BENCHMARK_COMPLETE',digest(OUTPUT/'result.json'),flush=True)
    except Exception as error:
        write_json(OUTPUT/'failure.json',dict(status='TERMINAL_SCOPED_SUPPORT_CACHE_BENCHMARK_FAILURE',reason=repr(error))); raise
