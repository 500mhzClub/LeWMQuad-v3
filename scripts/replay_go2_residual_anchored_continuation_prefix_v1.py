"""Prospective anchored-continuation replay on the completed original trajectory."""
import argparse
...
OUTPUT = BASE/'go2_residual_anchored_continuation_prefix_v1_attempt_001'
PROTOCOL = 'docs/go2_residual_anchored_continuation_prefix_v1_2026-09-09.md'
...
PREDECESSOR = BASE/'go2_residual_hold_prefix_v2_attempt_001'
PREDECESSOR_SHA = '47f8ab41c12d91d9f46a1def1308218a5eead9ca6c18651683051477001db89a'
DIAGNOSIS = BASE/'go2_residual_hold_veto_readout_v1_attempt_001'
DIAGNOSIS_SHA = '14a6b2cc7bcc889823426ab9961444754a849cde6c3d8a60514480db82ee68c8'
NATIVE_SHA = '55a7d5071f39337b3c9ea329e5b48320f11c8a5a9ba6e34296926006768ce466'


def verify_predecessor():
    sources = {}
    for root, sha in ((PREDECESSOR,PREDECESSOR_SHA),(DIAGNOSIS,DIAGNOSIS_SHA)):
        verify_artifacts(root, {'result.json':sha}); result = read_json(root,'result.json')
        verify_artifacts(root,result['artifact_sha256']); verify_sources(result['source_sha256'])
        if result['native_result_sha256'] != NATIVE_SHA or result['native_execution'] is not False:
            raise ValueError('same completed native input and nonphysical predecessor required')
        report = result['report']
        if root == PREDECESSOR:
            if (result['status'] != 'RESIDUAL_HOLD_PREFIX_V2_COMPLETE' or report['frames'] != 3004
                    or report['first_requested_command_difference'] is not None
                    or report['hold_reconsideration_interventions'] != 0
                    or report['final_terminal'] != 'MISSION_TICK_BUDGET_EXHAUSTED'
                    or report['raw_model_forecast_comparisons'] != 3000
                    or report['complete_original_selection_preserved'] is not True
                    or report['unchanged_observed_mission_and_residual_state_exact'] is not True
                    or report['model_state_sha256'] != MODEL_STATE or report['model_state_unchanged'] is not True):
                raise ValueError('completed unchanged full hold replay required')
        elif (result['status'] != 'RESIDUAL_HOLD_VETO_READOUT_V1_COMPLETE'
                or report['observations'] != 3014 or report['selected_holds'] != 2643
                or report['hold_reason_counts'] != {'no_strictly_better_allowed_nonhold':24,
                    'every_better_nonhold_has_a_first_point_invariant_veto':2619}):
            raise ValueError('completed causal hold veto diagnosis required')
        for name, binding in result['source_sha256'].items():
            if name in sources and sources[name] != binding: raise ValueError('predecessor source conflict: '+name)
...
def verify_inputs(launch):
    if (launch['verification_benchmark_result_sha256'] != BENCHMARK_SHA
            or launch['native_result_sha256'] != NATIVE_SHA
            or launch['predecessor_result_sha256'] != PREDECESSOR_SHA
            or launch['diagnosis_result_sha256'] != DIAGNOSIS_SHA):
        raise ValueError('exact completed input identities required')
    ...
    if result is not None or fingerprint(launch) != before:
        raise ValueError('unchanged original verifier context required')
    print('ANCHORED_CONTINUATION_INPUTS_VERIFIED', counters, flush=True)
...
def replay(launch):
    ...
    controller = ResidualAnchoredContinuationController(model, ArticulatedCollisionGeometry(URDF),
    ...
    with writer(OUTPUT) as append:
        for i, original in enumerate(islice(read_rows(directory), MAX_FRAMES)):
            ...
            if i%32 == 0: print('RESIDUAL_ANCHORED_CONTINUATION_PREFIX_FRAME', i, flush=True)
    ...
    return dict(case=CASE[0], frames=frames, maximum_frames=MAX_FRAMES,
        ...
        anchored_continuation_interventions=int(check['anchored_continuation_changed_action']),
...
def main():
    ...
    if OUTPUT.exists() or OUTPUT.is_symlink(): raise ValueError('exclusive anchored continuation prefix required')
    ...
    sources = discover_sources((PROTOCOL, 'scripts/replay_go2_residual_anchored_continuation_prefix_v1.py',
        'lewm/tests/test_residual_anchored_continuation_development.py',
        'lewm/tests/test_residual_anchored_continuation_prefix_development.py'), inherited)
    launch = old|dict(protocol=PROTOCOL, source_sha256=sources, output_root=str(OUTPUT), replay_input_bindings=ids,
        ...
        model_state_sha256=MODEL_STATE, implementation_class='ResidualAnchoredContinuationController',
        ...
        verification_benchmark_result_sha256=BENCHMARK_SHA,
        scientific_policy_and_replay_unchanged=False, fresh_replay_from_observation_zero=True,
        ...
        memory_admission_bytes=16*1024**3, concurrent_native_allowance_bytes=32*1024**3, minimum_free_bytes=RESERVE_BYTES,
    ...
    if resources['memory_available_bytes'] < 48*1024**3 or resources['artifact_free_bytes'] < RESERVE_BYTES+MAX_OUTPUT_BYTES:
        raise ValueError('anchored replay and conservative native resource allowances unavailable')
    if args.preflight_only:
        print('RESIDUAL_ANCHORED_CONTINUATION_PREFIX_PREFLIGHT', json.dumps(dict(source_count=len(sources),
    ...
    print('RESIDUAL_ANCHORED_CONTINUATION_PREFIX_V1_LAUNCHED', digest(OUTPUT/'launch.json'), flush=True)
    try:
        ...
        write_json(OUTPUT/'result.json', dict(status='RESIDUAL_ANCHORED_CONTINUATION_PREFIX_V1_COMPLETE',
        ...
        print('RESIDUAL_ANCHORED_CONTINUATION_PREFIX_V1_COMPLETE', digest(OUTPUT/'result.json'),
            {k:v for k,v in report.items() if k != 'changed_selection'}, flush=True)
    except Exception as error:
        write_json(OUTPUT/'failure.json', dict(status='TERMINAL_RESIDUAL_ANCHORED_CONTINUATION_PREFIX_V1_FAILURE', reason=repr(error)))
        raise
