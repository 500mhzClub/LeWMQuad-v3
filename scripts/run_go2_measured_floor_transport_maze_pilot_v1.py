"""Prospective measured floor transport with recorded camera provenance on maze0."""
import argparse
import contextlib
import json
...
from scripts.measured_floor_transport_native_prefix_development import compare
from scripts.training_translation_bias_model_admission_development import load_assigned
from scripts.replay_go2_measured_floor_transport_prefix_v1 import OUTPUT as INTEGRATION, verify_all as verify_prefix
from scripts.run_go2_dual_camera_settled_maze_pilot_v1 import OUTPUT as PREVIOUS, CASE, CORRECTION, FITS, verify_inputs as verify_predecessor
from lewm.measured_floor_transport_prefix_development import admit_native
from scripts.probe_go2_maze_renderer_witness_v1 import OUTPUT as RENDERER, verify_all as verify_renderer
from scripts.measured_floor_transport_intervention_development import admit_intervention
...
OUTPUT = BASE/'go2_measured_floor_transport_maze_pilot_v1_attempt_001'
PROTOCOL = 'docs/go2_measured_floor_transport_maze_pilot_v1_2026-09-09.md'
...
def worker(launch_sha):
    ...
    terminal = dict(case=name, layout_index=index, model_name=model_name,
        status='MEASURED_FLOOR_TRANSPORT_MAZE_WORKER_FAILED', artifact_sha256={}); started = time.perf_counter()
    with (OUTPUT/(name+'_worker.log')).open('x') as log, contextlib.redirect_stdout(log), contextlib.redirect_stderr(log):
        try:
            ...
            terminal.update(status='MEASURED_FLOOR_TRANSPORT_MAZE_COLLECTED_AND_RAW_AUDITED', artifact_sha256=bindings,
...
PREVIOUS_READOUT = BASE/'go2_dual_camera_settled_maze_readout_v1_attempt_001'
NATIVE_RESULT = '44710966178a57b31f7da3bec10ad4f21710bcff3701b0a1038750f1ef6d747c'
READOUT_RESULT = '6fbbf9a63ca770317fc929c49796993ff3cf9b268cb4572d2d39a7bfd660d5db'
RENDERER_RESULT = '5107b9a48c9767149b0f1a217875065f6e98489fe97885626cca9a0ace547f88'
ENVIRONMENT_KEYS = ('PYOPENGL_PLATFORM', 'LIBGL_ALWAYS_SOFTWARE', 'EGL_DEVICE_ID',
    'MESA_GL_VERSION_OVERRIDE', 'MESA_LOADER_DRIVER_OVERRIDE')
...
def verify_inputs(launch):
    ...
    verify_renderer(read_json(RENDERER, 'launch.json'))
    if {k:os.environ.get(k) for k in ENVIRONMENT_KEYS} != launch['renderer_environment']:
        raise ValueError('bound actual-camera execution environment changed')
...
def admit_renderer(result):
    if (result['status'] != 'MAZE_RENDERER_WITNESS_INTEGRATION_COMPLETE'
            or result['native_result_sha256'] != NATIVE_RESULT
            or result['actual_maze_camera_endpoints_queried'] is not True
            or result['predecessor_outcome_unchanged'] is not True
            or result['new_navigation_episodes'] != 0):
        raise ValueError('completed actual maze-camera recorder integration required')
    for key in ('raw_sensor_reconstruction_pass', 'raw_model_command_replay_pass',
            'raw_command_audit_pass', 'model_state_unchanged'):
        if result[key] is not True: raise ValueError('renderer probe raw invariant required: '+key)
    comparison = result['comparison']; witnesses = comparison['renderer_witnesses']
    if (comparison['frames'] != 3 or comparison['physics_samples'] != 900
            or witnesses['frames'] != 3 or witnesses['capture_endpoints'] != 6):
        raise ValueError('three complete original warmup observations and six endpoints required')
    for key in ('complete_controller_decisions_exact', 'raw_captures_and_public_packets_exact',
            'actual_zero_commands_exact'):
        if comparison[key] is not True: raise ValueError('unchanged actual-camera startup required: '+key)
    if witnesses['paired_context_readbacks_equal'] is not True or witnesses['all_witnesses_match_raw_acquisitions'] is not True:
        raise ValueError('actual context and raw acquisition witnesses must match')
...
def main():
    ...
    if OUTPUT.exists() or OUTPUT.is_symlink(): raise ValueError('exclusive fresh floor-transport native attempt required')
    verify_artifacts(INTEGRATION, {'result.json': args.prefix_result_sha256})
    integration = read_json(INTEGRATION, 'result.json')
    ids = {'result.json': args.prefix_result_sha256, **integration['artifact_sha256']}
    verify_artifacts(INTEGRATION, ids)
    ...
    verify_artifacts(PREVIOUS, previous['artifact_sha256']); predecessor_launch = read_json(PREVIOUS, 'launch.json')
    admit_native(previous, read_json(PREVIOUS, CASE[0]+'_audit.json'), case=CASE[0])
    ...
    verify_artifacts(PREVIOUS_READOUT, {'result.json': READOUT_RESULT})
    readout = read_json(PREVIOUS_READOUT, 'result.json')
    if (readout['status'] != 'DUAL_CAMERA_SETTLED_MAZE_READOUT_V1_COMPLETE'
            or readout['native_result_sha256'] != NATIVE_RESULT or readout['original_outcome_unchanged'] is not True):
        raise ValueError('completed unchanged eleventh execution readout required')
    verify_artifacts(RENDERER, {'result.json': RENDERER_RESULT})
    renderer = read_json(RENDERER, 'result.json'); admit_renderer(renderer)
    renderer_launch = read_json(RENDERER, 'launch.json')
    previous_bindings = {str(PREVIOUS): {'result.json': NATIVE_RESULT, **previous['artifact_sha256']},
        ...
        str(RENDERER): {'result.json': RENDERER_RESULT, **renderer['artifact_sha256']}}
    inherited = dict(integration['source_sha256'])
    for report in (readout, renderer):
        for name, h in report['source_sha256'].items():
            ...
            inherited[name] = h
    sources = discover_sources((PROTOCOL, 'scripts/run_go2_measured_floor_transport_maze_pilot_v1.py',
        'lewm/tests/test_measured_floor_transport_native_development.py',
        'lewm/tests/test_measured_floor_transport_native_prefix_development.py',
        'docs/go2_dual_camera_settled_maze_pilot_result_2026-09-09.md',
        'docs/go2_maze_renderer_witness_probe_result_2026-09-09.md'), inherited)
    ...
    launch.update(protocol=PROTOCOL, source_sha256=sources, output_root=str(OUTPUT), planned_case=CASE,
        ...
        previous_evidence_bindings=previous_bindings, implementation_class='MeasuredFloorTransportController',
        intervention_admission=intervention, renderer_environment=renderer_launch['render_environment'],
        renderer_capture_witnesses_enabled=True, robot_urdf_path=str(URDF), robot_urdf_sha256=digest(URDF),
        hardware=resources, navigation_ticks=NAVIGATION_TICKS, shared_outbound_return_budget=True,
        ...
        concurrency_reason='one native scene after completed transport prefix and renderer startup audit; no parallel native scenes',
        native_execution=True, model_training=False, checkpoint_selection_performed=False,
        model_map_contact_selector_algorithms_unchanged=True, pose_admission_changed_explicitly=True,
        measured_quiet_boundary_required_before_dwell=True, additional_auxiliary_rgb_for_motion=True,
        primary_first_tracking=True, continuous_speed_bound_from_visual_displacement=False,
        strict_visibility_gate_unchanged=True, predecessor_strict_visibility_failed_frames=[909],
        predecessor_failure_in_unchanged_prefix=True, pose_uncertainty_calibrated=False,
        data_scope='prospective measured-floor-transport intervention on reused development maze 0',
    ...
    if args.preflight_only:
        print('MEASURED_FLOOR_TRANSPORT_MAZE_PREFLIGHT', json.dumps(dict(source_count=len(sources), hardware=resources,
    ...
    if not memory_ok or not storage_ok: raise ValueError('native floor-transport resources unavailable')
    create_output(OUTPUT); write_json(OUTPUT/'launch.json', launch); launch_sha = digest(OUTPUT/'launch.json')
    print('MEASURED_FLOOR_TRANSPORT_MAZE_LAUNCHED', launch_sha, flush=True); started = time.perf_counter()
    try:
        ...
        print('MEASURED_FLOOR_TRANSPORT_MAZE_TERMINAL', record['status'], record.get('verified_round_trip'), record.get('failure'), flush=True)
        if record['status'] != 'MEASURED_FLOOR_TRANSPORT_MAZE_COLLECTED_AND_RAW_AUDITED':
        ...
        verify_inputs(launch); verify_artifacts(OUTPUT, bindings)
        write_json(OUTPUT/'result.json', dict(status='MEASURED_FLOOR_TRANSPORT_MAZE_PILOT_V1_COMPLETE', conditions=[record],
        ...
        print('MEASURED_FLOOR_TRANSPORT_MAZE_COMPLETE', digest(OUTPUT/'result.json'), flush=True)
    except Exception as error:
        write_json(OUTPUT/'failure.json', dict(status='TERMINAL_MEASURED_FLOOR_TRANSPORT_MAZE_PILOT_FAILURE', reason=repr(error)))
        raise
