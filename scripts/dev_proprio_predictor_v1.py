...
2. **Proprioception** (optional) enters as ONE token per context slot, formed
   from the five trailing 10 Hz samples of that slot.  A slot whose frame is a
   *prediction* rather than an observation receives a learned ``absent`` token
   and is marked invalid.  Proprioception is an input only: the target stays
   visual and no proprioceptive loss exists.
...
class ProprioActionPredictor(nn.Module):
    ...
    def forward(self, context, action, mask, proprio=None, proprio_valid=None):
        """context (B,3,N,D); action (B,15); mask (B,N) True=predict.

        ``proprio``       (B, 3, 5, 33) trailing samples per slot, or None
...
@torch.no_grad()
def unroll(model, context, action_blocks, proprio=None, max_h: int = 4,
    ...
    previous = T.normalise(model(window, action_blocks[0], mask, p_window, valid))
    ...
    for step in range(1, max_h):
        ...
        previous = T.normalise(model(window, action_blocks[step], mask, p_window, valid))
        outputs.append(previous)
...
def parameter_report(width=384, depth=6, heads=6) -> dict:
    ...
    return {
        "frozen_predictor_12d_action": total(frozen),
        "corrected_action_rgb": total(rgb),
        "corrected_action_proprio": total(prop),
