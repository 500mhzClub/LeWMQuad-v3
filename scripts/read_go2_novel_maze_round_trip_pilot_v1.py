"""Read completed first-maze execution without outcome relabeling or selection."""
import argparse
...
from scripts.run_go2_novel_maze_round_trip_pilot_v1 import OUTPUT as INPUT, CASE
...
OUTPUT = BASE/'go2_novel_maze_round_trip_readout_v1_attempt_001'
PROTOCOL = 'docs/go2_novel_maze_round_trip_readout_v1_2026-09-08.md'
...
def main():
    ...
    assert result['status'] == 'NOVEL_MAZE_ROUND_TRIP_PILOT_COMPLETE' and len(result['conditions']) == 1
    ...
    sources = discover_sources((PROTOCOL, 'scripts/read_go2_novel_maze_round_trip_pilot_v1.py',
        'lewm/tests/test_novel_maze_round_trip_readout_development.py'), old['source_sha256'])
    resources = hardware()
    ...
    try:
        ...
        write_json(OUTPUT/'result.json', dict(status='NOVEL_MAZE_ROUND_TRIP_READOUT_COMPLETE',
            ...
            original_outcome_unchanged=True, measured_round_trip_successes=int(report['verified_round_trip']),
            matched_baselines_completed=False, navigation_qualified=False, goal_achieved=False))
        print('NOVEL_MAZE_ROUND_TRIP_READOUT_COMPLETE', digest(OUTPUT/'result.json'), flush=True)
    except Exception as error:
        write_json(OUTPUT/'failure.json', dict(status='TERMINAL_NOVEL_MAZE_ROUND_TRIP_READOUT_FAILURE', reason=repr(error)))
        raise
