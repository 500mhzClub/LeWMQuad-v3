"""Full raw sensor/reactive-command verification and independent maze evaluation."""
import json
...
def audit(layout_index, result, definition, *, input_root, robot_geometry, episode_name):
    ...
    assert result['learned_model_used'] is False and result['candidate_future_outcomes_evaluated'] is False
    controller = ReactiveFloorTransportController(robot_geometry, public_mission=mission,
    ...
    return dict(layout_index=layout_index, renderer_capture_audit=renderer, measured_floor_transport_enabled=True,
        raw_sensor_reconstruction_pass=True, additional_auxiliary_rgb_reconstructed=True, raw_controller_command_replay_pass=True,
        raw_command_audit_pass=True, high_level_world_model_used=False, physical_stop=stops,
