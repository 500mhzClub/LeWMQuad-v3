...
def capture(session,directory,frame):
    ...
    try:
        ...
        mapping={int(k):tuple(map(int,v)) for k,v in context.seg_idxc_map.items()}
        ...
        return dict(frame=frame,physical_sample_index=before[1]-1,measured_ns=before[0],
            ...
            diagnostic_segmentation_map={str(k):list(v) for k,v in mapping.items()},robot_segmentation_ids=robot_ids,
