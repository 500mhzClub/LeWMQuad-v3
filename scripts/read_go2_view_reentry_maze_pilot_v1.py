"""Read completed translating view recovery without outcome relabeling."""
import argparse
...
from scripts.view_reentry_floor_plane_registration_diagnosis_development import diagnose as floor_registration_diagnosis

OUTPUT = BASE/'go2_view_reentry_maze_readout_v1_attempt_001'
PROTOCOL = 'docs/go2_view_reentry_maze_readout_v1_2026-09-08.md'
...
def main():
    ...
    assert result['status'] == 'VIEW_REENTRY_MAZE_PILOT_COMPLETE' and len(result['conditions']) == 1
    ...
    sources = discover_sources((PROTOCOL, 'scripts/read_go2_view_reentry_maze_pilot_v1.py',
        'lewm/tests/test_view_reentry_readout_development.py',
        'lewm/tests/test_floor_plane_registration_diagnosis_development.py',
        'docs/go2_view_reentry_auxiliary_contact_provisional_diagnosis_2026-09-08.md'), old['source_sha256'])
    resources = hardware()
    ...
    try:
        ...
        diagnosis = floor_registration_diagnosis(INPUT/CASE[0])
        verify(launch); verify_artifacts(INPUT, bindings)
        write_json(OUTPUT/'result.json', dict(status='VIEW_REENTRY_MAZE_READOUT_COMPLETE',
            ...
            auxiliary_floor_registration_diagnosis=diagnosis,
        ...
        print('VIEW_REENTRY_MAZE_READOUT_COMPLETE', digest(OUTPUT/'result.json'), flush=True)
    except Exception as error:
        write_json(OUTPUT/'failure.json', dict(status='TERMINAL_VIEW_REENTRY_MAZE_READOUT_FAILURE', reason=repr(error)))
        raise
