#!/usr/bin/env python3
"""Stdlib-only preauthorization launcher for exact V4 development fitting.

No repository, NumPy, Torch, PIL, dataset, audit, or RGB module is imported or
opened until the canonical trainer authorization and its complete source/review
closure pass. The currently frozen authorization is false, so exact execution
remains disabled pending a second independent review.
"""
...
TRAINER_PATH = (ROOT / "scripts/train_go2_observable_camera_ray_fit_v4.py").resolve()
...
def _rgb_worker_terminal(
    ...
    job, authorization_file_sha256, review_record_file_sha256 = payload
    ...
    try:
        captured_launcher = load("scripts.launch_go2_observable_camera_ray_fit_v4")
        ...
        trainer = load("scripts.train_go2_observable_camera_ray_fit_v4")
...
def _execute_captured_trainer(
    ...
    try:
        ...
        trainer = load("scripts.train_go2_observable_camera_ray_fit_v4")
        result = int(trainer._captured_exact_cli(args, dict(receipt)))
        for logical in tuple(fingerprints):
            load(logical)
        return result
    finally:
...
def validate_execution_args(args: argparse.Namespace) -> argparse.Namespace:
    if (
        args.steps != DEFAULT_STEPS[args.fit_size]
