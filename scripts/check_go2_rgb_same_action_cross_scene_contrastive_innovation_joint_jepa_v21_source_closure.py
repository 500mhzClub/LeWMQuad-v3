#!/usr/bin/env python3
"""Build or verify the V21 scene-innovation recursive source closure."""
...
SCHEMA = (
    ...
    "joint_jepa_v21_source_manifest"
)
PASS_STATUS_TEXT = "Go2 RGB V21 scene-innovation source closure: PASS"
...
_V20 = _source_only_module(
    "_lewm_v21_scene_innovation_source_closure_base",
    BASE_CHECKER_PATH,
)
if Path(_V20.ROOT).resolve(strict=True) != ROOT.resolve(strict=True):
    raise PermissionError("V21 source-closure base escaped the repository")
...
FORCED_DYNAMIC_SOURCES = tuple(
    dict.fromkeys(
        (
            *_V20.FORCED_DYNAMIC_SOURCES,
            (
                "scripts/run_go2_rgb_object_space_height_volume_executed_"
...
EXCLUDED_RUNTIME_CATEGORIES = _V20.EXCLUDED_RUNTIME_CATEGORIES
FORBIDDEN_PATH_PARTS = _V20.FORBIDDEN_PATH_PARTS
FORBIDDEN_RUNNER_PREFIX = _V20.FORBIDDEN_RUNNER_PREFIX
...
def _configure_private_base() -> None:
    ...
    _V20._V18._V13._BASE.build_manifest = _V20._V18._V13.build_manifest


_configure_private_base()
build_manifest = _V20.build_manifest
discover_source_closure = _V20.discover_source_closure
verify_manifest = _V20.verify_manifest
_safe_source_path = _V20._safe_source_path
_read_regular_source = _V20._read_regular_source
_verify_tracked = _V20._verify_tracked
_write_manifest_exclusive = _V20._write_manifest_exclusive
