"""New controlled-floor visual-servo initialization, unchanged gait."""
import copy
import numpy as np
from lewm.command_pulse_response_development import pack,specification
...
class CommandPulsePhysicalInit(_GenesisPhysicalSession):
    def __init__(self,spec,*,backend):
        ...
        if self.backend!='cpu' or spec!=specification(spec['trial']): raise ExperimentError('exact CPU visual-servo experiment required')
        ...
        self._runtime=dict(backend='cpu',n_envs=1,physics_dt_s=.002,policy_dt_s=.02,command_dt_s=.1,
            ...
            ppo_observation_contact_inputs='none_in_frozen45elementcontract',high_level='fixed development command excitation with visual acquisition stop',
