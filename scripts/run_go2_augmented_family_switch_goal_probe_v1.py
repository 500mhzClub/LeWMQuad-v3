"""Two fixed augmented models in the unchanged overlap-retention native probe."""
import argparse
import contextlib
import json
...
from scripts.overlap_retention_goal_episode_development import collect,artifacts
...
OUTPUT=BASE/'go2_augmented_family_switch_goal_probe_v1_attempt_001'
...
PROTOCOL='docs/go2_augmented_family_switch_goal_probe_v1_2026-09-08.md'
...
def worker(case,launch_sha):
    ...
    terminal=dict(case=name,trial=trial,variant=variant,condition=condition,model_name=model_name,
        status='AUGMENTED_FAMILY_SWITCH_GOAL_WORKER_FAILED',artifact_sha256={});started=time.perf_counter()
    with (OUTPUT/(name+'_worker.log')).open('x') as log,contextlib.redirect_stdout(log),contextlib.redirect_stderr(log):
        try:
            ...
            terminal.update(status='AUGMENTED_FAMILY_SWITCH_GOAL_COLLECTED_AND_RAW_AUDITED',artifact_sha256=bindings,
...
def main():
    ...
    launch=preflight(output=OUTPUT,protocol=PROTOCOL,
        seed_paths=('scripts/run_go2_augmented_family_switch_goal_probe_v1.py',
            'lewm/tests/test_augmented_family_switch_goal_scope_development.py',
    ...
    for source in (prior,prior_readout,readout):
        for name,h in source['source_sha256'].items():assert launch['source_sha256'].get(name)==h,name
    launch.update(all_eighteen_admission=admitted,fit_result_sha256=args.fit_result_sha256,fit_readout_sha256=readout_ids,
        ...
        experiment='fixed augmented full-JEPA and full-direct first-seed models; unchanged overlap-retention controller',
    ...
    print('AUGMENTED_FAMILY_SWITCH_GOAL_LAUNCHED',digest(OUTPUT/'launch.json'),flush=True)
    ...
    try:
            ...
            with ProcessPoolExecutor(max_workers=2,mp_context=multiprocessing.get_context('spawn'),max_tasks_per_child=1) as pool:
                ...
                while pending:
                    ...
                    for future in done:
                        ...
                        except Exception as error:row=dict(case=case[0],status='AUGMENTED_FAMILY_SWITCH_GOAL_PROCESS_FAILURE',failure=repr(error))
                        records.append(row);print('AUGMENTED_FAMILY_SWITCH_GOAL_TERMINAL',case[0],row['status'],row.get('goal'),flush=True)
        if len(records)!=2 or any(r['status']!='AUGMENTED_FAMILY_SWITCH_GOAL_COLLECTED_AND_RAW_AUDITED' for r in records):
        ...
        result=dict(status='AUGMENTED_FAMILY_SWITCH_GOAL_PROBE_COMPLETE',cases=CASES,conditions=records,
        ...
        write_json(OUTPUT/'result.json',result);print('AUGMENTED_FAMILY_SWITCH_GOAL_COMPLETE',digest(OUTPUT/'result.json'),flush=True)
    except Exception as error:
        write_json(OUTPUT/'failure.json',dict(status='TERMINAL_AUGMENTED_FAMILY_SWITCH_GOAL_FAILURE',reason=repr(error),
