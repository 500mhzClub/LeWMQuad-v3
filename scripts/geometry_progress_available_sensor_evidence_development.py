"""Available-evidence readout: insufficient depth rays remain a failed measurement gate.

Public sensor validity stays unchanged. Native surfaces >5.5cm are compared,
including intentionally near obstacles; no rejected public depth is filled.
All inherited contact, body, camera and native-geometry checks are retained.
"""
import hashlib
import json
...
def audit_sensors(directory, spec, result):
    ...
    require(n == result['physics_samples'] and n > 0, 'nonempty complete recorded physics required')
    ...
    static = read_json(directory, 'static_objects.json') if result['setup_checked'] else []
    require(not result['setup_checked'] or len(static) == len(walls), 'complete wall inventory')
    for row, expected in zip(static, walls if result['setup_checked'] else [], strict=True):
    ...
    require(len(cameras) == len(dcameras) == result['rgbd_frames'], 'complete paired sensor frames')
    if reader is None:
        same(read_json(directory, 'policy_observations.json')['frames'], [])
        same(read_json(directory, 'depth_observations.json')['frames'], [])
        for name in ('policy_histories.npz', 'fast_gyro_histories.npz'):
            require(all(len(v) == 0 for v in read_npz(directory, name).values()), 'no fabricated empty-episode histories')
    ...
    require(indices == sorted(set(indices)) and (not indices or indices[0] == 749 and indices[-1] < n), 'ordered camera samples')
    ...
    for i, stamp in enumerate(ns):
        ...
        policy, depth, recorded_fast, decision_ns = reader.packet(frame)
        same(decision_ns, int(stamp))
        same(buffer.packet(policy['image']['rgb'], int(stamp)), policy)
        packet = fbuffer.packet(now_ns=int(stamp)); same(packet, recorded_fast)
        ...
        expected = ref['expected_depth_m']; use = native_depth_comparison_mask(expected, ref['surface_interior'])
        measured = native[np.ix_(ref['rows'], ref['columns'])]; errors = np.abs(measured[use]-expected[use])
        require(np.isfinite(errors).all(), 'finite interior depth errors required')
        row = dict(frame=frame, **depth_evidence(errors),
            public_depth_valid_pixels=int(depth['valid'].sum()), native_near_surface_rays=int((use & (expected < .2)).sum()))
    ...
    report = dict(sensor_contact_reconstruction_exact=True, raw_depth_comparator_recomputed=True, depth_checks=depth_results,
        complete_settling=n >= 750, native_wall_inventory_checked=result['setup_checked'],
        paired_frames_reconstructed=len(cameras), partial_setup=not result['setup_admitted'],
...
def native_depth_comparison_mask(expected, interior):
    expected=np.asarray(expected);interior=np.asarray(interior)
    if expected.shape!=interior.shape or interior.dtype!=bool:
        raise ValueError('native optical depth and boolean interior shapes required')
    return interior & np.isfinite(expected) & (expected>.055) & (expected<4.98)



def depth_evidence(errors):
    errors=np.asarray(errors,float)
    if errors.ndim!=1 or not np.isfinite(errors).all() or (errors<0).any():
        raise ValueError('finite nonnegative per-ray absolute errors required')
    maximum=float(errors.max()) if len(errors) else None
    sufficient=len(errors)>=1000
    passed=bool(sufficient and maximum<=.001)
    return dict(rays=len(errors),maximum_error_m=maximum,within1mm=passed,
        sufficient_native_depth_rays=sufficient,
        measurement_status='INSUFFICIENT_NATIVE_DEPTH_RAYS' if not sufficient else 'PASS' if passed else 'DEPTH_GEOMETRY_ERROR')
