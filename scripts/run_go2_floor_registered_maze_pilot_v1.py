"""Prospective observed floor-pose registration on reused development maze 0."""
import argparse
import contextlib
import json
...
from scripts.floor_registered_maze_episode_development import collect, artifacts
from scripts.floor_registered_maze_audit_development import audit
from scripts.floor_registered_native_prefix_comparison_development import compare
from scripts.training_translation_bias_model_admission_development import load_assigned
from scripts.replay_go2_floor_registered_maze_prefix_v1 import (
    OUTPUT as INTEGRATION, INPUT as PREVIOUS,
    READOUT as PREVIOUS_READOUT, CASE, CORRECTION, FITS)
...
OUTPUT = BASE/'go2_floor_registered_maze_pilot_v1_attempt_001'
PROTOCOL = 'docs/go2_floor_registered_maze_pilot_v1_2026-09-08.md'
...
def worker(launch_sha):
    ...
    terminal = dict(case=name, layout_index=index, model_name=model_name,
        status='FLOOR_REGISTERED_MAZE_WORKER_FAILED', artifact_sha256={}); started = time.perf_counter()
    with (OUTPUT/(name+'_worker.log')).open('x') as log, contextlib.redirect_stdout(log), contextlib.redirect_stderr(log):
        try:
            ...
            terminal.update(status='FLOOR_REGISTERED_MAZE_COLLECTED_AND_RAW_AUDITED', artifact_sha256=bindings,
...
def main():
    ...
    if OUTPUT.exists() or OUTPUT.is_symlink(): raise ValueError('exclusive prospective floor-registration experiment; no retry/resume')
    verify_artifacts(INTEGRATION, {'result.json': args.prefix_result_sha256}); integration = read_json(INTEGRATION, 'result.json')
    assert integration['status'] == 'FLOOR_REGISTERED_MAZE_PREFIX_COMPLETE'
    ...
    assert prefix['complete_predecessor_sensor_model_map_mission_command_reconstructed']
    assert prefix['original_visual_evidence_exact'] and prefix['identical_raw_forecasts_when_both_controllers_plan']
    assert prefix['map_contact_residual_mission_pose_correction_is_the_declared_change']
    ...
    assert prefix['original_visibility_outcome_unchanged'] and prefix['stopped_before_unexecuted_outcome']
    ...
    assert previous_bindings[str(PREVIOUS)]['result.json'] == '5ee4ef051e1a506f205aae51610deece18f755fb5440c40b1113e22e5ba317ee'
    assert read_json(PREVIOUS, 'result.json')['status'] == 'CONFIRMED_FLOOR_MAZE_PILOT_COMPLETE'
    ...
    sources = discover_sources((PROTOCOL, 'scripts/run_go2_floor_registered_maze_pilot_v1.py',
        'lewm/tests/test_floor_registered_native_scope_development.py',
        'lewm/tests/test_floor_registered_native_prefix_comparison_development.py',
        'docs/go2_floor_registered_maze_prefix_result_2026-09-08.md'), integration['source_sha256'])
    resources = hardware()
    if args.preflight_only:
        print('FLOOR_REGISTERED_MAZE_PREFLIGHT', json.dumps(dict(
    ...
    if resources['artifact_free_bytes'] < RESERVE_BYTES+COLLECTION_ALLOWANCE_BYTES+PERSISTENCE_HEADROOM_BYTES:
        raise ValueError('declared floor-registration storage/persistence envelope unavailable')
    ...
    launch.update(protocol=PROTOCOL, source_sha256=sources, output_root=str(OUTPUT), planned_case=CASE,
        ...
        concurrency_reason='one existing-maze floor-registration intervention; storage envelope excludes parallel scenes',
    ...
    print('FLOOR_REGISTERED_MAZE_LAUNCHED', launch_sha, flush=True)
    try:
        ...
        print('FLOOR_REGISTERED_MAZE_TERMINAL', record['status'], record.get('verified_round_trip'), record.get('failure'), flush=True)
        if record['status'] != 'FLOOR_REGISTERED_MAZE_COLLECTED_AND_RAW_AUDITED':
            raise ValueError('native floor-registration infrastructure/raw audit/prefix comparison failure; evidence retained')
        ...
        verify_artifacts(FITS, admission['base_admission']['fit_artifact_sha256'])
        verify_artifacts(CORRECTION, admission['correction_artifact_sha256'])
        write_json(OUTPUT/'result.json', dict(status='FLOOR_REGISTERED_MAZE_PILOT_COMPLETE', conditions=[record],
        ...
        print('FLOOR_REGISTERED_MAZE_COMPLETE', digest(OUTPUT/'result.json'), flush=True)
    except Exception as error:
        write_json(OUTPUT/'failure.json', dict(status='TERMINAL_FLOOR_REGISTERED_MAZE_PILOT_FAILURE', reason=repr(error)))
        raise
