"""Fixed noisy/control sequence study of local inverse-depth floor estimation."""
import argparse
import hashlib
import json
...
from lewm.local_inverse_depth_floor_tracking_development import (
    LocalInverseDepthFloorMotion, LocalInverseDepthFloorRegistration)
...
def main():
    ...
    output = root/f'depth_noise_{args.sigma_mm}mm_local_floor_tracking_601_v1'
    output.mkdir()
    config = dict(layout_index=i, sigma_mm=args.sigma_mm, seed=SEED,
        frames=FRAME_COUNT, tracker='LocalInverseDepthFloorMotion',
        ...
        sensor_noise_calibrated=False, source_sha256={str(p):hashlib.sha256(p.read_bytes()).hexdigest()
            for p in map(Path, (__file__, 'lewm/local_inverse_depth_floor_tracking_development.py',
    ...
    reader = PublicReplay(root/'native'); motion = LocalInverseDepthFloorMotion()
    ...
    with (output/'frames.jsonl').open('x') as sink:
        for frame in range(FRAME_COUNT):
            ...
            if frame % 100 == 0: print('LOCAL_FLOOR_FRAME', i, args.sigma_mm, frame, flush=True)
