...
from scripts.replay_go2_packed_owned_maze_controller_v1 import (
    OUTPUT as REPLAY, INPUT, CASE, verify_all as verify_replay)

OUTPUT=BASE/'go2_packed_owned_controller_pair_v1_attempt_001'
PROTOCOL='docs/go2_packed_owned_controller_pair_v1_2026-09-09.md'
FRAMES=256
IMPLEMENTATIONS=('original','packed_owned')
ALLOWANCE=1024**3
...
def admit(result):
    if (result['status']!='PACKED_OWNED_MAZE_CONTROLLER_REPLAY_COMPLETE'
...
def main():
    ...
    sources=discover_sources((PROTOCOL,'scripts/benchmark_go2_packed_owned_controller_pair_v1.py',
        'lewm/tests/test_packed_owned_controller_pair_development.py'),result['source_sha256'])
    resources=hardware()
    launch=old|dict(protocol=PROTOCOL,output_root=str(OUTPUT),source_sha256=sources,
        ...
        implementation_classes=dict(original='LaterFloorResolutionRoundTripController',
            packed_owned='PackedOwnedLaterFloorController'),
        ...
        concurrency_reason='one paired CPU benchmark beside independent settling replay; shared-machine timing')
    ...
    if args.preflight_only:
        print('PACKED_PAIR_PREFLIGHT',json.dumps(dict(source_count=len(sources),hardware=resources,
    ...
    print('PACKED_PAIR_LAUNCHED',digest(OUTPUT/'launch.json'),flush=True);started=time.perf_counter()
    try:
        name,index,variant,condition,model_name=CASE
        controllers={};models={}
        for implementation,cls in zip(IMPLEMENTATIONS,
                (LaterFloorResolutionRoundTripController,PackedOwnedLaterFloorController),strict=True):
        ...
        with ExitStack() as stack:
            ...
            for i,saved in enumerate(islice(read_rows(INPUT/name),FRAMES)):
                ...
                if i%32==0:print('PACKED_PAIR_FRAME',i,flush=True)
        ...
        assert artifacts['original/context_decisions.jsonl.gz']==artifacts['packed_owned/context_decisions.jsonl.gz']
        ...
        ratios={metric:float(np.median([r['runs']['original'][metric]/r['runs']['packed_owned'][metric]
        ...
        write_json(OUTPUT/'result.json',dict(status='PACKED_OWNED_CONTROLLER_PAIR_COMPLETE',
        ...
        print('PACKED_PAIR_COMPLETE',digest(OUTPUT/'result.json'),flush=True)
