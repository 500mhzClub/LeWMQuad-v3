...
def run(session,source_root,budget,*,model,case):
    config=json.loads(Path('docs/go2_decision_headroom_protocol_v4_2026-09-23.json').read_text())
    ...
    for state in sorted(states,key=lambda s:s['frame']):
        ...
        try:
            ...
            repeat_cost_ranges=[]
            if reference['status']=='available' and repeat==3:
                for i,action in enumerate(v4.ACTIONS):
                    ...
                    for j in range(3):
                        ...
                        rc=v4.scalar_reference(refgeo,repeated,arrival_settling=objective['mode']=='positional_terminal',parameters=config['reference_cost'])
                        if rc['cost_s'] is not None:values.append(rc['cost_s'])
                    spread=max(values)-min(values) if len(values)==3 else None
                    repeat_cost_ranges.append(dict(action=action,range_s=spread))
                    if i in reference.get('physical',[]) and (spread is None or spread>.025):reference=dict(status='unresolved',reason='SPOT_CHECK_REFERENCE_REPEAT_VARIABILITY')
            ...
            physics.save(root/'audit_v4.json',dict(objective=objective,sampling=state,source_input_valid=source_valid,physics_valid=physics_valid,rgb_valid=rgb_valid,safety=safety,reference=reference,costs=costs,repeat_cost_ranges=repeat_cost_ranges,rows=panel,filter_audit=filters,phase2=True))
