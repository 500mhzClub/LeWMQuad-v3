"""Identical fresh height intervention with admitted scoped input verification."""
import argparse
import contextlib
import json
...
from scripts.partial_floor_height_scoped_verification_admission_development import (
    BENCHMARK_SHA, admit_benchmark, verify_inputs)

OUTPUT = BASE/'go2_partial_floor_height_maze01_scoped_verification_pilot_v1_attempt_001'
CASE = ('full_jepa_partial_floor_height_maze_01', *PRIOR_CASE[1:])
PROTOCOL = 'docs/go2_partial_floor_height_maze01_scoped_verification_pilot_v1_2026-09-09.md'
...
def worker(launch_sha):
    ...
    terminal = dict(case=name,layout_index=index,model_name=model_name,
        status='PARTIAL_FLOOR_HEIGHT_MAZE01_SCOPED_VERIFICATION_WORKER_FAILED',artifact_sha256={}); started = time.perf_counter()
    with (OUTPUT/(name+'_worker.log')).open('x') as log, contextlib.redirect_stdout(log), contextlib.redirect_stderr(log):
        try:
            ...
            terminal.update(status='PARTIAL_FLOOR_HEIGHT_MAZE01_SCOPED_VERIFICATION_COLLECTED_AND_RAW_AUDITED',artifact_sha256=bindings,
...
def main():
    ...
    for name, sha in benchmark['source_sha256'].items():
        ...
        inherited[name] = sha
    sources = discover_sources((PROTOCOL, 'scripts/run_go2_partial_floor_height_maze01_scoped_verification_pilot_v1.py',
        'lewm/tests/test_partial_floor_height_maze01_native_development.py',
        'lewm/tests/test_partial_floor_height_maze01_native_prefix_development.py',
        'lewm/tests/test_partial_floor_height_scoped_verification_development.py',
        'docs/go2_scoped_verification_digest_result_2026-09-09.md'), inherited)
    ...
    if args.preflight_only:
        print('PARTIAL_FLOOR_HEIGHT_MAZE01_SCOPED_VERIFICATION_NATIVE_PREFLIGHT',json.dumps(dict(source_count=len(sources),hardware=resources,
    ...
    print('PARTIAL_FLOOR_HEIGHT_MAZE01_SCOPED_VERIFICATION_NATIVE_LAUNCHED',launch_sha,flush=True); started = time.perf_counter()
    try:
        ...
        print('PARTIAL_FLOOR_HEIGHT_MAZE01_SCOPED_VERIFICATION_NATIVE_TERMINAL',record['status'],record.get('verified_round_trip'),record.get('failure'),flush=True)
        if record['status'] != 'PARTIAL_FLOOR_HEIGHT_MAZE01_SCOPED_VERIFICATION_COLLECTED_AND_RAW_AUDITED':
        ...
        verify_inputs(launch); verify_artifacts(OUTPUT,bindings)
        write_json(OUTPUT/'result.json',dict(verification_benchmark_result_sha256=BENCHMARK_SHA,status='PARTIAL_FLOOR_HEIGHT_MAZE01_SCOPED_VERIFICATION_PILOT_V1_COMPLETE',
        ...
        print('PARTIAL_FLOOR_HEIGHT_MAZE01_SCOPED_VERIFICATION_NATIVE_COMPLETE',digest(OUTPUT/'result.json'),flush=True)
    except Exception as error:
        write_json(OUTPUT/'failure.json',dict(status='TERMINAL_PARTIAL_FLOOR_HEIGHT_MAZE01_SCOPED_VERIFICATION_NATIVE_FAILURE',reason=repr(error)))
        raise
