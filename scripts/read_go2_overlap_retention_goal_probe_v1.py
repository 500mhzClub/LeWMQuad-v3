...
from scripts.run_go2_overlap_retention_goal_probe_v1 import OUTPUT as INPUT, CASES
...
from scripts.run_go2_retained_patch_contact_goal_probe_v1 import OUTPUT as ORIGINAL

OUTPUT=BASE/'go2_overlap_retention_goal_readout_v1_attempt_001'
PROTOCOL='docs/go2_overlap_retention_goal_readout_v1_2026-09-08.md'
ORIGINAL_SHA='9842921b16b0fb2dffc501c6d958fbfd6ffb72e57a9c56855b2d5f9b54e61685'
...
def summarize(record):
    ...
    return dict(case=name,model_name=record['model_name'],condition=record['condition'],variant=record['variant'],
        ...
        geometry_refinement_trace=[dict(tick=s['tick'],checks=[dict(action=ACTIONS[i],
            whole_voxel_possible=c['whole_voxel_possible_intersection'],
            tight_aabb_possible=c['sample_bounds_aabb_possible_intersection'],
            refined_possible=c['possible_intersection'],
            surviving_shape_ids=[r['shape_id'] for r in c['shapes'] if r['intersecting_voxels']])
            for i,c in enumerate(s['surface_checks'])]) for s in forecasts],
        last_floor_partition_receipt=next((r['decision']['floor_partition_receipt'] for r in reversed(rows)
            if r['decision']['floor_partition_receipt'] is not None),None),
        contact_rule_trace=[dict(tick=s['tick'],checks=[dict(action=ACTIONS[i],
            all_return_possible=c['all_return_possible_intersection'],after_contact_rule_possible=c['possible_intersection'],
            eligible_floor_overlaps=[f['shape_id'] for f in c['foot_floor_contacts']
                if f['measured_floor_contact_rule_eligible'] and f['floor']['intersecting_voxels']],
            unknown_or_nonfloor_foot_hits=[f['shape_id'] for f in c['foot_floor_contacts'] if f['other_or_unknown']['intersecting_voxels']],
            incomplete_projection_feet=[f['shape_id'] for f in c['foot_floor_contacts'] if not f['measured_floor_contact_rule_eligible']])
            for i,c in enumerate(s['surface_checks'])]) for s in forecasts],
        retained_patch_trace=[dict(tick=s['tick'],checks=[dict(action=ACTIONS[i],
            grid_rule_possible=c['grid_contact_possible_intersection'],patch_rule_possible=c['possible_intersection'],
            retained_frames=c['retained_patch_frames'],prefix_bytes=c['retained_prefix_bytes'],
            new_coverage=[dict(shape_id=f['shape_id'],witness=f['retained_patch']['coverage_witness']) for f in c['foot_floor_contacts']
                if f['retained_patch'] is not None and f['retained_patch']['complete_nominal_foot_patch']])
            for i,c in enumerate(s['surface_checks'])]) for s in forecasts],
        observer_retention_trace=[dict(tick=r['tick'],
            overlap=r['decision']['evidence'].get('overlap_retention'),
...
def compare_prior(case):
    ...
    def pose(row):
        p=(row['decision']['evidence'] or {}).get('current_pose')
        return None if p is None else [p[k] for k in ('position_initial_body_m','rotation_initial_body_from_current_body')]
    first_pose=next((i for i,(a,b) in enumerate(zip(old_rows,new_rows)) if pose(a)!=pose(b)),None)
    ...
    terminal_first=next((i for i,(a,b) in enumerate(zip(old_rows,new_rows))
        if a['decision']['terminal']!=b['decision']['terminal']),None)
    changes=[i for i in (first,terminal_first) if i is not None]
    cutoff=min(changes) if changes else None
    frames=min(len(old_rows),len(new_rows),cutoff+1 if cutoff is not None else max(len(old_rows),len(new_rows)))
    end=749+50*(frames-1)
    ...
    return dict(case=case,first_requested_command_difference=first,difference_is_tape_length_only=length_only,
        first_observed_numeric_pose_difference=first_pose,observer_evidence_schema_intentionally_extended=True,
        first_terminal_state_difference=terminal_first,
        comparison_stops_at_first_command_or_terminal_difference=True,
...
def main():
    ...
    assert result['status']=='OVERLAP_RETENTION_GOAL_PROBE_COMPLETE' and result['cases']==[list(c) for c in CASES]
    ...
    assert old['status']=='RETAINED_PATCH_CONTACT_GOAL_PROBE_COMPLETE'
    ...
    sources=discover_sources((PROTOCOL,'scripts/read_go2_overlap_retention_goal_probe_v1.py'),original['source_sha256'])
    ...
    try:
        ...
        write_json(OUTPUT/'result.json',dict(status='OVERLAP_RETENTION_GOAL_READOUT_COMPLETE',conditions=reports,models=grouped,
        ...
        print('OVERLAP_RETENTION_GOAL_READOUT_COMPLETE',digest(OUTPUT/'result.json'),flush=True)
    except Exception as error:
        write_json(OUTPUT/'failure.json',dict(status='TERMINAL_OVERLAP_RETENTION_GOAL_READOUT_FAILURE',reason=repr(error)));raise
