...
import json
import math
from scripts import replay_go2_shared_surface_anchored_prefix_v1 as replay
...
SOURCE = 'scripts/verify_go2_shared_surface_anchored_prefix_v1.py'
TEST = 'lewm/tests/test_shared_surface_anchored_verification_development.py'
PROTOCOL = 'docs/go2_shared_surface_anchored_verification_v1_2026-09-10.md'
OUTPUT = ROOT/'docs/go2_shared_surface_anchored_prefix_verification_2026-09-10.json'
LAUNCH_SHA = '3b2c3d3650ebefc2e314dab4de107877cc2dc805bed43bf5b190422bc6f4d97d'
...
def require_report(result, launch, reference):
    ...
    if any(type(launch.get(k)) is not type(v) or launch[k] != v for k, v in expected_launch.items()):
        raise ValueError('exact receipt-sharing-only prospective launch required')
...
def verified_result(result_sha):
    root = replay.OUTPUT
...
def main():
    ...
    result, checked = verified_result(args.result_sha256)
    ...
    print('SHARED_SURFACE_ANCHORED_RECORDED_EVIDENCE_VERIFIED', digest(OUTPUT), flush=True)
