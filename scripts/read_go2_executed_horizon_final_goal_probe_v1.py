...
import numpy as np
from scripts.run_go2_executed_horizon_final_goal_probe_v1 import OUTPUT as INPUT,PREVIOUS as PRIOR,PREVIOUS_SHA as PRIOR_SHA,CASES,TRIAL
...
OUTPUT=BASE/'go2_executed_horizon_final_goal_readout_v1_attempt_001'
PROTOCOL='docs/go2_executed_horizon_final_goal_readout_v1_2026-09-08.md'
...
def summarize(record):
    ...
    return dict(case=name,model_name=record['model_name'],condition=record['condition'],
        ...
        final_goal_scoring_changed_selections=sum(s['action']!=s.get('eight_step_final_goal_action',s['action']) for s in forecast),
        final_goal_scoring_trace=[dict(tick=s['tick'],previous_action=s['eight_step_final_goal_action'],
            action=s['action'],scored_pose_horizon_ns=s['scored_pose_horizon_ns'],
...
def main():
    ...
    assert result['status']=='EXECUTED_HORIZON_FINAL_GOAL_PROBE_COMPLETE' and result['cases']==[list(c) for c in CASES]
    ...
    sources=discover_sources((PROTOCOL,'scripts/read_go2_executed_horizon_final_goal_probe_v1.py',
        'lewm/tests/test_executed_horizon_final_goal_readout_development.py'),old['source_sha256'])
    ...
    try:
        ...
        write_json(OUTPUT/'result.json',dict(status='EXECUTED_HORIZON_FINAL_GOAL_READOUT_COMPLETE',
        ...
        print('EXECUTED_HORIZON_FINAL_GOAL_READOUT_COMPLETE',digest(OUTPUT/'result.json'),flush=True)
    except Exception as error:
        write_json(OUTPUT/'failure.json',dict(status='TERMINAL_EXECUTED_HORIZON_FINAL_GOAL_READOUT_FAILURE',reason=repr(error)));raise
