...
def main():
    ...
    caps=config['historical_rerender_proposal'];root=BASE/'go2_headroom_historical_rerender_v41_attempt_001'
    assert caps['physics_steps']==0 and caps['examples']==16 and caps['primary_images']==32
    root.mkdir(exist_ok=False);start=time.monotonic();cpu_start=time.process_time();owner=psutil.Process()
