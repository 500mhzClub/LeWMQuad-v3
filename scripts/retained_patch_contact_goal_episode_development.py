...
def collect(trial, definition, *, output, model, geometry, persistent, episode_name, condition, variant):
    ...
    try:
        ...
        session._runtime['high_level'] = 'retained_patch_contact_goal_probe_v1; online model plus measured surfaces'
        ...
        controller = RetainedPatchContactGoalProbe(model, geometry, persistent=persistent, condition=condition, variant=variant)
        try:
        ...
        result = dict(status='RETAINED_PATCH_CONTACT_GOAL_PROBE_TERMINAL_AUDIT_REQUIRED', trial=trial,
