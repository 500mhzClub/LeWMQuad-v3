...
ROCM_BACKEND_SOURCE_AUDIT = {
    "audit_passed": True,
    "material_fresh_v3_host_identity_successor_not_v2_retry": True,
    ...
    "fresh_v3_role_local_quadrants_cache_required": True,
...
class GenesisRocmBackendV3AuthorityError(RuntimeError):
...
def file_binding(path: Path) -> dict[str, object]:
    try:
        ...
        raise GenesisRocmBackendV3AuthorityError(str(exc)) from exc
...
def _v1_terminal_review_binding() -> dict[str, Any]:
    return runner._standard_binding(  # noqa: SLF001
        plan_builder.predecessor.V1_QUALIFICATION_TERMINAL_REVIEW_BINDING
    )
