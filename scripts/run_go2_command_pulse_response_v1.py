...
from lewm.command_pulse_response_development import PulseSequence,specification,TRIALS,MAXIMUM_TICKS,events
...
from scripts.command_pulse_session_development import CommandPulseSession
...
PREVIOUS=ROOT/'.generated/go2_goal_hold_visual_servo_v1_attempt_001'
OUTPUT=ROOT/'.generated/go2_command_pulse_response_v1_attempt_001'
PROTOCOL='docs/go2_command_pulse_response_v1_2026-09-06.md'
IDENTITIES={'launch.json':'86740c721e427575a124cc6f2eb201daf916ac7819b3dd9299ed7437c2565952',
...
def preflight():
    ...
    sources=discover_sources((PROTOCOL,'scripts/run_go2_command_pulse_response_v1.py',
        'scripts/audit_go2_command_pulse_response_v1.py',
        'lewm/tests/test_command_pulse_response_development.py'),old['source_sha256'])
    launch={k:old[k] for k in ('native_sha256','native_geometry_sha256','opencv_binary_sha256','opencv_version','rules')}
    launch|=dict(source_sha256=sources,input_sha256=inputs,protocol=PROTOCOL,events={order:events(order) for order in ('a','b')},
        conditions={c:specification(c) for c in TRIALS},minimum_free_bytes=10*1024**3,
        maximum_control_ticks=MAXIMUM_TICKS,maximum_terminal_zero_ticks=10,model='gyro',
...
def collect(condition,definition):
    ...
    try:
        session=CommandPulseSession(spec,directory); session.install_contact_identity(); build=session.ctx.build
        ...
        motion=VisualLedMotion('gyro',identity=(0,0,0)); controller=PulseSequence(spec['order'])
        ...
        try:
            ...
            for tick in range(MAXIMUM_TICKS+1):
                ...
                if tick%20==0 or decision['terminal']:
                    print('VISUAL_SERVO',condition,tick,decision['stage'],decision['terminal'],{k:decision[k] for k in ('event_index','command_name','event_offset')},flush=True)
                if decision['terminal']:
                    final_control=decision
                    if decision['terminal']!='PULSE_SCHEDULE_COMPLETE':
                        for _ in range(10): execute([0.,0.,0.],9,'terminal_zero_command_drain')
                        session.capture_current()
                    break
                execute(decision['requested_command'],decision['phase'],'fixed_pulse_schedule')
        ...
        result=dict(status='PULSE_TERMINAL_AUDIT_REQUIRED',condition=condition,physical_stop=physical_stop,
            schedule_terminal=final_control,command_ticks=len(tape),completed_ticks=sum(t['completed'] for t in tape),
        ...
        if session is not None:
            try:
                ...
                write_json(directory/'pulse_decisions.json',decisions); write_json(directory/'command_tape.json',tape)
...
def artifacts(result):
    names=['specification.json','actuator_identity.json','floor_roles.json','terminal_actuator_gains.json',
        ...
        'pulse_decisions.json','command_tape.json','native_guard_rows.json','friction_checks.json',
...
def main():
    ...
    try:
        ...
        write_json(OUTPUT/'result.json',dict(status='COMMAND_PULSE_COLLECTION_TERMINAL',conditions=results,
