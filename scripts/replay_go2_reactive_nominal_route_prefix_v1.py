"""Observed-route reactive baseline replay, stopped before a changed action outcome."""
import json
import time
import cv2
import torch
...
from scripts.run_go2_nominal_reentry_maze_pilot_v1 import OUTPUT as INPUT, CASE
...
OUTPUT = BASE/'go2_reactive_nominal_route_prefix_v1_attempt_001'
PROTOCOL = 'docs/go2_reactive_nominal_route_prefix_v1_2026-09-08.md'
...
PRIOR_PREFIX = BASE/'go2_reactive_observed_route_prefix_v1_attempt_001'
PRIOR_PREFIX_SHA = '2ceee2441c3600eff6b2c1176e0c70d93e8d535b419957db8cbcd3326bcc3cdc'


def replay():
    name, index, _, _, _ = CASE; directory = INPUT/name
    controller = ReactiveNominalRoundTripController(ArticulatedCollisionGeometry(URDF),
    ...
    with writer(OUTPUT) as append:
        for i, original in enumerate(read_rows(directory)):
            ...
            if first_command is None and first_terminal is None:
                for key in ('tick', 'requested_command', 'quiet_intervals', 'selected_action', 'terminal'):
                    assert result[key] == old[key], ('same actual command/mission output before intervention', i, key)
            append(dict(tick=i, decision=result)); frames += 1; last = result
            if i % 100 == 0: print('REACTIVE_NOMINAL_ROUTE_PREFIX_FRAME', i, flush=True)
...
def main():
    ...
    for root, sha, status in ((INPUT, INPUT_SHA, 'NOMINAL_REENTRY_MAZE_PILOT_COMPLETE'),
            ...
            (PRIOR_PREFIX, PRIOR_PREFIX_SHA, 'REACTIVE_OBSERVED_ROUTE_PREFIX_COMPLETE')):
    ...
    sources = discover_sources((PROTOCOL, 'scripts/replay_go2_reactive_nominal_route_prefix_v1.py',
        'lewm/tests/test_reactive_nominal_route_development.py',
        'docs/go2_reactive_observed_route_prefix_result_2026-09-08.md'), bound[-1][2]['source_sha256'])
    resources = hardware()
    ...
    print('REACTIVE_NOMINAL_ROUTE_PREFIX_LAUNCHED', digest(OUTPUT/'launch.json'), flush=True)
    try:
        ...
        write_json(OUTPUT/'result.json', dict(status='REACTIVE_NOMINAL_ROUTE_PREFIX_COMPLETE',
        ...
        print('REACTIVE_NOMINAL_ROUTE_PREFIX_COMPLETE', digest(OUTPUT/'result.json'),
            {k: v for k, v in report.items() if k != 'changed_selection'}, flush=True)
    except Exception as error:
        write_json(OUTPUT/'failure.json', dict(status='TERMINAL_REACTIVE_NOMINAL_ROUTE_PREFIX_FAILURE', reason=repr(error)))
        raise
