#!/usr/bin/env python3
"""Run fresh V2 science only after exact V2 qualification PASS."""
...
from scripts import build_go2_scene_diversity_recurrent_replication_genesis_rocm_backend_v2_plan as plan_builder  # noqa: E402
from scripts import collect_go2_scene_diversity_recurrent_replication_genesis_rocm_backend_v2 as collector  # noqa: E402
from scripts import qualify_go2_scene_diversity_recurrent_replication_genesis_rocm_backend_v2 as qualifier  # noqa: E402
from scripts import run_go2_scene_diversity_recurrent_replication_genesis_rocm_backend_v1 as predecessor  # noqa: E402
...
SOURCE_REVIEW_SCHEMA = (
    ...
    "genesis_rocm_backend_v2_source_review_v1"
)
SOURCE_REVIEW_STATUS = "PASS_INDEPENDENT_SOURCE_REVIEW"
RESULT_SCHEMA = (
    "lewm_go2_scene_diversity_recurrent_replication_"
    "genesis_rocm_backend_v2_result_v1"
)
TERMINAL_SCHEMA = (
    "lewm_go2_scene_diversity_recurrent_replication_"
    "genesis_rocm_backend_v2_terminal_v1"
)
RESERVATION_SCHEMA = (
    "lewm_go2_scene_diversity_recurrent_replication_"
    "genesis_rocm_backend_v2_attempt_reservation_v1"
)
...
_V1_SOURCE_ONLY_PATHS = {
    "v1_rocm_plan_builder_source": Path(
        plan_builder.predecessor.__file__
    ).resolve(),
    "v1_rocm_collector_source": Path(collector.predecessor.__file__).resolve(),
    "v1_rocm_qualifier_source": Path(qualifier.predecessor.__file__).resolve(),
    "v1_rocm_runner_source": Path(predecessor.__file__).resolve(),
}
...
SOURCE_PATHS = {
    ...
    **_V1_SOURCE_ONLY_PATHS,
    ...
    "predecessor_v1_qualification_terminal_review": (
        plan_builder.V1_QUALIFICATION_TERMINAL_REVIEW
    ),
}
...
ROCM_IDENTITY_FIELDS = predecessor.ROCM_IDENTITY_FIELDS
...
class _V2QualifierReplayFacade:
    """Expose V2 qualification APIs with the inherited scene implementation.

    The V1 runner historically treats ``qualifier.predecessor`` as the scene
    collector implementation.  V2's wrapper uses that name for its V1
    qualifier adapter, so the replay seam must make the distinction explicit.
    """
...
_QUALIFIER_REPLAY_FACADE = _V2QualifierReplayFacade()
...
def predecessor_failure_bindings_rocm() -> dict[str, dict[str, object]]:
    """Add only the permitted V1 terminal-review document to prior evidence."""

    evidence = dict(_ORIGINAL_PREDECESSOR_FAILURE_BINDINGS())
    expected = _standard_binding(
        plan_builder.V1_QUALIFICATION_TERMINAL_REVIEW_BINDING
    )
    if file_binding_v1(plan_builder.V1_QUALIFICATION_TERMINAL_REVIEW) != expected:
        raise SceneDiversityRunnerError(
            "V1 qualification terminal-review document changed"
        )
    evidence["predecessor_v1_qualification_terminal_review"] = expected
    return evidence


def _validate_v2_preflight_evidence(result: Mapping[str, Any]) -> None:
    preflight = result.get("rocm_egl_preflight")
    if not isinstance(preflight, Mapping):
        raise SceneDiversityRunnerError("V2 linker evidence is absent")
    ...
    if (
        ...
        raise SceneDiversityRunnerError(
            "V2 exact ld.lld driver evidence changed"
        )
...
def validate_qualification_result_binding(value: object):
    ...
    _validate_v2_preflight_evidence(result)
    try:
        authority = json.loads(qualifier.QUALIFICATION_AUTHORITY.read_bytes())
    except (OSError, UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise SceneDiversityRunnerError(
            "V2 qualification authority is not strict JSON"
        ) from exc
    collector._require_v1_review_binding(authority)  # noqa: SLF001
...
def _validate_plan_rocm(
    ...
    if authority.get("attempt_id") != plan_builder.DEFAULT_ATTEMPT_ID:
        raise SceneDiversityRunnerError(
            "V2 scientific authority identity changed"
        )
    try:
    ...
    except plan_builder.SceneDiversityGenesisRocmV2PlanError as exc:
...
def _validate_authority_rocm(
    ...
    *,
    ...
    collector._require_v1_review_binding(validated[0])  # noqa: SLF001
    validate_qualification_result_binding(
...
def build_parser():
    ...
    parser.description = (
        "Run fresh Genesis ROCm V2 science after exact V2 qualification."
    )
    return parser
