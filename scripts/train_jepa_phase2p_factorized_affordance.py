#!/usr/bin/env python3
"""Train the bounded Phase 2P factorized primitive affordance diagnostic."""
...
def _assert_finite_metrics(metrics: dict[str, float], *, step: int, phase: str) -> None:
    ...
    if nonfinite:
        raise RuntimeError(
            json.dumps(
                {
                    "error": "nonfinite_phase2p_metrics",
                    "metrics": nonfinite,
                    "phase": phase,
                    "step": int(step),
                },
...
def _forward_batch(
    ...
    *,
    ...
    factor_logits = model(batch.start_vision)
...
def evaluate(
    ...
    *,
    ...
    for indices in batches:
        batch = materialize_phase2p_factorized_batch(examples, indices).to(device)
        ...
        contracts.append(phase2p_batch_contract_audit(batch))
        selection_records.extend(
            factorized_affordance_selection_records(
                ...
                scorer_name="factorized_affordance_model",
                **selection_kwargs,
            )
        )
    return {
...
def main() -> int:
    ...
    while completed_steps < args.optimization_steps:
        ...
        for indices in batches:
            batch = materialize_phase2p_factorized_batch(
                train_examples,
                indices,
            ).to(device)
            output = _forward_batch(
            ...
            if args.max_grad_norm > 0.0:
                ...
                if not torch.isfinite(gradient_norm_tensor):
                    raise RuntimeError(
                        json.dumps(
                            {
                                "error": "nonfinite_phase2p_gradient_norm",
                                "gradient_norm": float(
            ...
            record = {
                ...
                "batch_contract": phase2p_batch_contract_audit(batch),
            }
    ...
    report = {
        "schema": "jepa_phase2p_factorized_affordance_training_v0",
        "run_class": args.run_class,
        "confirmatory_result": False,
        "target_version": FACTORIZED_AFFORDANCE_TARGET_VERSION,
        "model": {
            "name": "FactorizedPrimitiveAffordanceModel",
