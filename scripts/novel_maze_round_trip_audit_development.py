...
def audit(layout_index, result, definition, *, input_root, model, robot_geometry, episode_name, condition, variant):
    ...
    controller = ObservedRoundTripController(model, robot_geometry, public_mission=mission,
    ...
    for tick, row in enumerate(read_rows(directory)):
        ...
        auxiliary = packet(directory, tick, p, public_acquisition(auxiliary_rows[tick]), now_ns=now)
    ...
    return dict(layout_index=layout_index, raw_sensor_reconstruction_pass=True, raw_model_command_replay_pass=True,
        ...
        independent_layout_development_execution=True, navigation_qualified=False, hardware_qualified=False,
