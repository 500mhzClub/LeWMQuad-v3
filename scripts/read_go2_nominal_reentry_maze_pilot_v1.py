"""Read completed same-maze recovery execution without outcome relabeling."""
import argparse
...
OUTPUT = BASE/'go2_nominal_reentry_maze_readout_v1_attempt_001'
PROTOCOL = 'docs/go2_nominal_reentry_maze_readout_v1_2026-09-08.md'
...
def summarize(record):
    ...
    return dict(case=name, layout_index=record['layout_index'], collection=collection,
        ...
        prefix_comparison=record.get('prefix_comparison'), reentry_execution=motion,
...
def main():
    ...
    assert result['status'] == 'NOMINAL_REENTRY_MAZE_PILOT_COMPLETE' and len(result['conditions']) == 1
    ...
    sources = discover_sources((PROTOCOL, 'scripts/read_go2_nominal_reentry_maze_pilot_v1.py',
        'lewm/tests/test_nominal_reentry_execution_readout_development.py'), old['source_sha256'])
    resources = hardware()
    ...
    try:
        ...
        write_json(OUTPUT/'result.json', dict(status='NOMINAL_REENTRY_MAZE_READOUT_COMPLETE',
        ...
        print('NOMINAL_REENTRY_MAZE_READOUT_COMPLETE', digest(OUTPUT/'result.json'), flush=True)
    except Exception as error:
        write_json(OUTPUT/'failure.json', dict(status='TERMINAL_NOMINAL_REENTRY_MAZE_READOUT_FAILURE', reason=repr(error)))
        raise
