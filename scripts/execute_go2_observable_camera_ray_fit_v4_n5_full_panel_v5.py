#!/usr/bin/env python3
"""Authority-free, one-process V5 executor for the frozen N5 experiment.
...
if __name__ == "__main__":
    ...
    STAGING_PREFIX = ".n5.reservation-v5-"
    ...
    LOCK_NAME = ".n5.reservation-v5.lock"
    STAGING_SCHEMA = "lewm_go2_n5_full_panel_v5_preclaim_staging_v1"
    ...
    def _directory_flags() -> int:
        ...
        if not nofollow or not directory:
            raise PermissionError("V5 canonical directory walks require no-follow opens")
        return os.O_RDONLY | nofollow | directory | getattr(os, "O_CLOEXEC", 0)


    @dataclass
    class DirectoryEntryV5:
        parent_fd: int
        name: str
        child_fd: int
    ...
    @dataclass
    class CanonicalDirectoryChainV5:
        anchor_fd: int
        anchor_fingerprint: tuple[int, ...]
        descriptors: list[int]
        entries: list[DirectoryEntryV5]
        path_fds: dict[Path, int]
        closed: bool = False


    @dataclass(frozen=True)
    class OwnedArtifactV5:
        role: str
        parent_fd: int
        name: str
        fingerprint: tuple[int, ...]
    ...
    def _assert_directory_chain(chain: CanonicalDirectoryChainV5) -> None:
        if chain.closed:
            raise PermissionError("V5 canonical directory chain is closed")
        if _stable_fingerprint(os.fstat(chain.anchor_fd)) != chain.anchor_fingerprint:
            raise PermissionError("V5 filesystem-root descriptor changed")
        for entry in chain.entries:
            ...
            if (
                ...
                or _stable_fingerprint(named) != entry.fingerprint
                or _stable_fingerprint(opened) != entry.fingerprint
            ):
                raise PermissionError("V5 canonical directory component changed")


    def _refresh_directory_chain(
        chain: CanonicalDirectoryChainV5,
        *,
        mutable_fds: set[int],
    ) -> None:
        if chain.closed:
        ...
        for entry in chain.entries:
            ...
            if (
                ...
                or not stat.S_ISDIR(named.st_mode)
                or not stat.S_ISDIR(opened.st_mode)
                or named_fingerprint[:2] != entry.fingerprint[:2]
                or opened_fingerprint[:2] != entry.fingerprint[:2]
                or named_fingerprint != opened_fingerprint
            ):
                raise PermissionError("V5 canonical directory identity changed")
            if entry.child_fd in mutable_fds:
                entry.fingerprint = opened_fingerprint
            elif opened_fingerprint != entry.fingerprint:
                raise PermissionError("V5 unexpected canonical directory mutation")


    def _open_canonical_directory_chain(final_path: Path) -> CanonicalDirectoryChainV5:
        ...
        if (
            ...
            raise PermissionError("V5 canonical directory target escaped the repository")
        ...
        try:
            if _stable_fingerprint(os.fstat(anchor_fd)) != anchor_fingerprint:
                raise PermissionError("V5 filesystem root changed during open")
            parent_fd = anchor_fd
            ...
            for index, component in enumerate(final_path.parts[1:]):
                ...
                try:
                    ...
                    if index < repository_depth:
                        raise PermissionError("V5 repository root component is missing")
                    os.mkdir(component, 0o700, dir_fd=parent_fd)
                    os.fsync(parent_fd)
                    created = True
                    before = _entry_metadata(parent_fd, component)
                if stat.S_ISLNK(before.st_mode) or not stat.S_ISDIR(before.st_mode):
                    raise PermissionError("V5 canonical component is not a real directory")
                ...
                if (
                    ...
                    raise PermissionError("V5 canonical component changed during open")
                if created and entries:
                    _refresh_directory_chain(chain, mutable_fds={parent_fd})
                entry = DirectoryEntryV5(parent_fd, component, child_fd, fingerprint)
    ...
    def _open_chain_child(
        ...
        try:
            ...
            before = _entry_metadata(parent_fd, name)
        if stat.S_ISLNK(before.st_mode) or not stat.S_ISDIR(before.st_mode):
            raise PermissionError("V5 derived parent is not a real directory")
        fingerprint = _stable_fingerprint(before)
        child_fd = os.open(name, _directory_flags(), dir_fd=parent_fd)
        chain.descriptors.append(child_fd)
        opened = os.fstat(child_fd)
        if _stable_fingerprint(opened) != fingerprint:
            raise PermissionError("V5 derived parent changed during open")
        child_path = parent_path / name
        chain.entries.append(DirectoryEntryV5(parent_fd, name, child_fd, fingerprint))
        chain.path_fds[child_path] = child_fd
        _assert_directory_chain(chain)
        return child_fd


    def _close_directory_chain(chain: CanonicalDirectoryChainV5) -> None:
        if chain.closed:
            return
        chain.closed = True
    ...
    class AttemptReservationV5:
        ...
        directory_chain: CanonicalDirectoryChainV5 | None = None
        owned_claim_artifacts: dict[str, OwnedArtifactV5] = field(
            default_factory=dict,
            compare=False,
        )
        owned_derived_artifacts: dict[str, OwnedArtifactV5] = field(
            default_factory=dict,
            compare=False,
        )
    ...
    def _assert_claim_fd_owned(reservation: AttemptReservationV5) -> None:
        if reservation.directory_fd < 0 or reservation.directory_identity is None:
            raise PermissionError("N5 full-panel V5 lacks an open claimed directory")
        opened = os.fstat(reservation.directory_fd)
        if not stat.S_ISDIR(opened.st_mode):
            raise PermissionError("N5 full-panel V5 claimed descriptor is not a directory")
        if (opened.st_dev, opened.st_ino) != reservation.directory_identity:
            raise PermissionError("N5 full-panel V5 claimed descriptor identity changed")


    def _assert_owned_claim(reservation: AttemptReservationV5) -> None:
        ...
        if reservation.directory_chain is None or reservation.seed_root_fd < 0:
            raise PermissionError("N5 full-panel V5 lacks its canonical directory chain")
        ...
        if (
            ...
            raise PermissionError("N5 full-panel V5 canonical claim identity changed")
    ...
    def _write_claim_file_exclusive(
        reservation: AttemptReservationV5,
        name: str,
        payload: bytes,
        *,
        mode: int = 0o600,
        ...
        if Path(name).name != name or name in {"", ".", ".."}:
            raise PermissionError("N5 full-panel V5 claim filename escaped")
        ...
        try:
            ...
            if not stat.S_ISREG(metadata.st_mode) or metadata.st_nlink != 1:
                raise PermissionError("N5 full-panel V5 claim leaf is not singly linked")
        ...
        try:
            ...
            if (
                ...
                raise PermissionError("N5 full-panel V5 committed claim leaf changed")
        except BaseException:
            _remove_created_leaf_if_owned(
                reservation.directory_fd,
                name,
            ...
            raise
        reservation.owned_claim_artifacts[name] = OwnedArtifactV5(
            role=role or name,
        ...
        *,
        ...
        try:
            ...
            if fingerprint(before) != fingerprint(after):
                raise RuntimeError("N5 full-panel V5 claim leaf changed while read")
        finally:
            os.close(descriptor)
        if require_canonical:
            _assert_owned_claim(reservation)
    ...
    def _classify_staging(
        ...
        *,
        attempt_path: Path,
        review_binding: Mapping[str, str],
    ) -> tuple[dict[str, Any], AttemptReservationV5 | None]:
        ...
        return evidence, AttemptReservationV5(
            directory=attempt_path,
    ...
    def _reserve_exact_attempt(source_review_file_sha256: str) -> AttemptReservationV5:
        ...
        claimed_chain: CanonicalDirectoryChainV5 | None = None
        claimed_reservation: AttemptReservationV5 | None = None
        try:
            with _locked_seed_root(seed_root):
                ...
                complete: list[tuple[Path, AttemptReservationV5, dict[str, Any]]] = []
                ...
                if (
                    ...
                    raise PermissionError("N5 full-panel V5 staging identity changed")
                owned_claim_identity = (staging_metadata.st_dev, staging_metadata.st_ino)
                claimed_reservation = AttemptReservationV5(
                    directory=attempt_path,
    ...
    def _cleanup_owned_artifacts(
        registry: dict[str, OwnedArtifactV5],
        *,
    ...
    def _publish_success(
        ...
        *,
        ...
        if not policy.is_sha256(checkpoint_content_sha256):
            raise ValueError("N5 full-panel V5 checkpoint content hash is malformed")
    ...
    def _write_canonical_json(
        ...
        if path not in {policy.CANONICAL_METRIC_RECEIPT_PATH, policy.CANONICAL_GATE_PATH}:
            raise PermissionError("N5 full-panel V5 publication path is not canonical")
        expected_schema = (
        ...
        if (
            ...
            raise ValueError("N5 full-panel V5 derived artifact is malformed")
        raw = policy.canonical_json_bytes(value) + b"\n"
        relative = path.relative_to(policy.CANONICAL_OUTPUT_ROOT)
        if len(relative.parts) != 2:
            raise PermissionError("N5 full-panel V5 publication depth changed")
        if reservation.directory_chain is None or reservation.output_root_fd < 0:
            raise PermissionError("N5 full-panel V5 lacks its retained output root")
        ...
        if parent_fd < 0:
            raise PermissionError("N5 full-panel V5 lacks its derived parent descriptor")
        leaf_fd = -1
        created_identity: tuple[int, int] | None = None
        try:
            leaf_fd = os.open(
            ...
            if not stat.S_ISREG(metadata.st_mode) or metadata.st_nlink != 1:
                raise PermissionError("N5 full-panel V5 derived leaf is not singly linked")
            created_identity = (metadata.st_dev, metadata.st_ino)
        ...
        try:
            ...
            if (
                ...
                raise PermissionError("N5 full-panel V5 committed derived leaf changed")
        except BaseException:
            _remove_created_leaf_if_owned(
                parent_fd,
                relative.parts[1],
        ...
        reservation.owned_derived_artifacts[relative_text] = OwnedArtifactV5(
    ...
    def _run_frozen_training(
        ...
        *,
        rgb_workers: int,
    ) -> tuple[dict[str, Any], AttemptReservationV5]:
        ...
        def require(value: object) -> object:
            if value is not token:
                raise PermissionError("retained science lacks its local V5 execution token")
        ...
        def reserve(value: object) -> AttemptReservationV5:
            nonlocal claimed_reservation
            require(value)
            if claimed_reservation is not None:
                raise FileExistsError("the sole V5 attempt was already reserved")
        ...
        if claimed_reservation is None:
            raise RuntimeError("frozen V5 training returned without claiming its attempt")
        _assert_owned_claim(claimed_reservation)
        return {
            **summary,
            "schema": "lewm_go2_observable_camera_ray_fit_v4_n5_full_panel_v5_launch_summary_v1",
        }, claimed_reservation
    ...
    def _run_independent_verification(
        ...
        def require(value: object) -> object:
            if value is not token:
                raise PermissionError("retained verifier lacks its local V5 execution token")
            policy.preflight_static_authority()
            policy.preflight_source_review(
                policy.CANONICAL_SOURCE_REVIEW_PATH,
                source_review_file_sha256,
        ...
        def write_metric(path: Path, value: Mapping[str, Any]) -> dict[str, Any]:
            if Path(path) != policy.CANONICAL_METRIC_RECEIPT_PATH:
                raise PermissionError("retained verifier changed its V5 output path")
    ...
    def _run_finalization(
        ...
        def require(value: object) -> object:
            if value is not token:
                raise PermissionError("retained finalizer lacks its local V5 execution token")
            policy.preflight_static_authority()
            policy.preflight_source_review(
                policy.CANONICAL_SOURCE_REVIEW_PATH,
                source_review_file_sha256,
        ...
        def write_gate(path: Path, value: Mapping[str, Any]) -> dict[str, Any]:
            if Path(path) != policy.CANONICAL_GATE_PATH:
                raise PermissionError("retained finalizer changed its V5 output path")
            return _write_canonical_json(reservation, Path(path), value)
    ...
    def execute_exact(
        ...
        *,
        ...
        if not sys.flags.isolated:
            raise PermissionError("N5 full-panel V5 exact execution requires isolation")
        ...
        try:
            ...
            _assert_owned_claim(reservation)
            return {
                "schema": "lewm_go2_observable_camera_ray_fit_v4_n5_full_panel_v5_end_to_end_summary_v1",
                "seed": 20260710,
                "fit_size": 5,
                "training": training,
            ...
            try:
                ...
                raise RuntimeError(
                    f"N5 full-panel V5 {stage} failed and terminal receipt "
    ...
    def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
        ...
        if not policy.is_sha256(args.source_review_sha256):
            raise ValueError("N5 full-panel V5 source review SHA-256 is malformed")
        return args
