...
OUTPUT = BASE / 'go2_measured_settling_mission_json_prefix_v1_attempt_001'
PROTOCOL = 'docs/go2_measured_settling_mission_json_prefix_v1_2026-09-09.md'
...
def main():
    ...
    sources = discover_sources((PROTOCOL, 'scripts/replay_go2_measured_settling_mission_json_prefix_v1.py',
        'lewm/measured_settling_round_trip_controller_development.py',
        'lewm/tests/test_measured_settling_round_trip_development.py',
        'lewm/tests/test_registered_json_pose_development.py',
        'docs/go2_measured_settling_mission_prefix_v1_2026-09-09.md'), native['source_sha256'])
    ...
    print('MEASURED_SETTLING_MISSION_JSON_PREFIX_LAUNCHED', digest(OUTPUT/'launch.json'), flush=True)
    try:
        ...
        with (OUTPUT/'mission_receipts.jsonl').open('x') as stream:
            for row in read_rows(INPUT/CASE):
                ...
                p, _, pose = registered_json_pose(d['evidence'], now_ns=now)
                assert pose['frame'] == i
                ...
                if i%200 == 0:
                    print('MEASURED_SETTLING_MISSION_JSON_PREFIX_FRAME', i, flush=True)
                if differences:
                    first_behavior = i; break
                previous_command = d['requested_command']
        verify_artifacts(INPUT, BINDINGS); source_check(sources)
        write_json(OUTPUT/'result.json', dict(status='MEASURED_SETTLING_MISSION_JSON_PREFIX_COMPLETE',
        ...
        print('MEASURED_SETTLING_MISSION_JSON_PREFIX_COMPLETE', digest(OUTPUT/'result.json'), flush=True)
