"""Targeted live layout-2 navigation with current-plane raw floor coverage."""
...
def annotate_write(name,value):
    if name=='launch.json':
        value=value|dict(experiment='current_paired_plane_floor_coverage_noisy_layout2_development_v1',
            comparison_condition='current_plane_raw_floor_coverage_with_retained_depth_cache',
            reference_root_name='go2_live_gyro_height_floor_noise_2mm_native_layout02_4800_v1_attempt_001',
...
def finish_write(name,value):
    ...
    bind(previous.finish_write,LAYOUT_INDEX=2,RAW_WRITE=annotated)(name,value)
...
def main():
    ...
    output=stable.source.BASE/'go2_current_plane_coverage_noise_2mm_native_layout02_4800_v1_attempt_001'
    stable.source.validate_root(output,must_exist=False)
    if output.exists():raise ValueError('preserve prior plane-coverage trial')
    writer=bind(transfer.make_writer,finish_write=finish_write)(output,2,'learned')
    ...
    with ProcessPoolExecutor(max_workers=1,mp_context=get_context('spawn'),
        ...
        bind(stable.source.main,OUTPUT=output,COUNT=4814,LAYOUT_INDEX=2,
            ...
            PacedNativeSession=partial(original.NoisyTransferSession,noise_layout_index=2,noise_sigma_mm=2),
