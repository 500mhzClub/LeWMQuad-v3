"""Own one native hold-reorientation experiment after both exact predecessors."""
import json
import os
import subprocess
import sys
...
OUTPUT = BASE/'go2_hold_reorientation_maze02_native_wait_v1_attempt_001'
SOURCE = 'scripts/await_go2_hold_reorientation_maze02_native_v1.py'
TEST = 'lewm/tests/test_hold_reorientation_native_wait_development.py'
PROTOCOL = 'docs/go2_hold_reorientation_maze02_native_wait_v1_2026-09-10.md'
WAIT_SECONDS = 48*3600
RAW_OWNER = dict(pid=2669739, created=1789035532.54, command=[
    '.generated/venvs/genesis_rocm_0_4_6_v1/bin/python', inputs.raw_wait.SOURCE])
FRONTIER_OWNER = dict(pid=2663938, created=1789032169.81, command=[
    '.generated/venvs/genesis_rocm_0_4_6_v1/bin/python', inputs.frontier_wait.SOURCE])
PREREQUISITES = (
    ('raw_prefix_wait_result_sha256', RAW_OWNER, inputs.raw_wait.OUTPUT, inputs.RAW_WAIT_LAUNCH,
        'HOLD_REORIENTATION_RAW_PREFIX_WAIT_V1_COMPLETE'),
    ('frontier_wait_result_sha256', FRONTIER_OWNER, inputs.frontier_wait.OUTPUT, inputs.FRONTIER_WAIT_LAUNCH,
        'REACHED_FRONTIER_MAZE03_NATIVE_WAIT_COMPLETE'))
...
def wait_for_inputs(sources, event, *, sleep=time.sleep, clock=time.monotonic):
    ...
    while True:
        ...
        event('WAITING_FOR_RAW_REPLAY_AND_SCHEDULED_FRONTIER', original_processes_live=live)
        sleep(30)


def authenticate_completed(sources, receipt):
    root = native.OUTPUT
    if (root/'failure.json').exists(): raise ValueError('native hold-reorientation failure retained')
    ...
    if (result['status'] != 'HOLD_REORIENTATION_MAZE02_PILOT_V1_COMPLETE'
            ...
            or result['frontier_wait_result_sha256'] != receipt['frontier_wait_result_sha256']
            or result['prospective_prefix_result_sha256'] != launch['input_admission']['prefix_result_sha256']
            or len(result['conditions']) != 1):
        raise ValueError('complete exact native hold-reorientation result required')
...
def main():
    ...
    write_json(OUTPUT/'launch.json', dict(source_sha256=sources, boot_id=BOOT,
        original_raw_replay_owner=RAW_OWNER, original_frontier_owner=FRONTIER_OWNER,
        original_raw_wait_launch_sha256=inputs.RAW_WAIT_LAUNCH,
        original_frontier_wait_launch_sha256=inputs.FRONTIER_WAIT_LAUNCH,
    ...
    print('HOLD_REORIENTATION_NATIVE_WAITER_LAUNCHED', digest(OUTPUT/'launch.json'), flush=True)
    ...
    try:
        with (OUTPUT/'events.jsonl').open('x') as events:
            ...
            command = [sys.executable, native.SOURCE,
                ...
                '--frontier-wait-result-sha256', receipt['frontier_wait_result_sha256']]
            with (OUTPUT/'native_stdout.log').open('xb') as log:
                child = subprocess.Popen(command, stdout=log, stderr=subprocess.STDOUT)
                event('HOLD_REORIENTATION_NATIVE_CHILD_STARTED', pid=child.pid, command=command)
                while child.poll() is None:
                    event('HOLD_REORIENTATION_NATIVE_CHILD_LIVE', pid=child.pid); time.sleep(30)
            event('HOLD_REORIENTATION_NATIVE_CHILD_EXITED', pid=child.pid, returncode=child.returncode)
        ...
        write_json(OUTPUT/'result.json', dict(status='HOLD_REORIENTATION_MAZE02_NATIVE_WAIT_V1_COMPLETE',
        ...
        print('HOLD_REORIENTATION_NATIVE_WAITER_COMPLETE', digest(OUTPUT/'result.json'), flush=True)
    except Exception as error:
        write_json(OUTPUT/'failure.json', dict(status='TERMINAL_HOLD_REORIENTATION_NATIVE_WAIT_FAILURE',
