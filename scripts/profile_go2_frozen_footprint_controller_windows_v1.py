...
from scripts import replay_go2_frozen_footprint_anchored_prefix_v1 as completed_replay
from scripts.replay_go2_frozen_footprint_anchored_prefix_v1 import normalize_candidate
...
OUTPUT = BASE/'go2_frozen_footprint_controller_windows_profile_v1_attempt_001'
SOURCE = 'scripts/profile_go2_frozen_footprint_controller_windows_v1.py'
PROTOCOL = 'docs/go2_frozen_footprint_controller_windows_profile_v1_2026-09-10.md'
TEST = 'lewm/tests/test_frozen_footprint_controller_profile_development.py'
...
def replay():
    ...
    with (OUTPUT/'comparison.jsonl').open('x') as stream:
        for row in islice(read_rows(directory), FRAMES):
            ...
            if frame % 50 == 0: print('FROZEN_FOOTPRINT_CONTROLLER_PROFILE_FRAME',frame,flush=True)
...
COMPLETED_SHA = '4fb9363938101682473115563f4aca2f48a68a8a6d8e9ead2cf3a8ea2ddae772'
VERIFICATION = Path('docs/go2_frozen_footprint_anchored_prefix_verification_2026-09-10.json')
VERIFICATION_SHA = '4b574cf6a46943b7135ec11c472f7c1c73a01af48411178a9a4917e0dd5681c8'
SEEDS = (SOURCE, PROTOCOL, TEST, str(VERIFICATION),
    'docs/go2_frozen_footprint_anchored_prefix_result_2026-09-10.md')


def completed_inputs():
    root = completed_replay.OUTPUT
    verify_artifacts(root, {'result.json': COMPLETED_SHA})
    result = read_json(root, 'result.json')
    verify(result['source_sha256']); verify_artifacts(root, result['artifact_sha256'])
    if digest(VERIFICATION) != VERIFICATION_SHA:
        raise ValueError('exact completed independent verification required')
    verification = json.loads(VERIFICATION.read_text())
    verify(verification['verification_source_sha256'])
    if (result['status'] != 'FROZEN_FOOTPRINT_ANCHORED_PREFIX_V1_COMPLETE'
            or verification['result_sha256'] != COMPLETED_SHA
            or verification['artifact_sha256'] != result['artifact_sha256']
            or verification['source_count'] != len(result['source_sha256'])
            or any(verification['verification_source_sha256'].get(n) != h
                for n,h in result['source_sha256'].items())):
        raise ValueError('complete verified original frozen-footprint replay required')
    completed_replay.preceding_inputs()
...
def main():
    ...
    if OUTPUT.exists() or OUTPUT.is_symlink(): raise ValueError('exclusive optimized-controller profile required')
    completed, verification = completed_inputs()
    verify_artifacts(OWNER_ROOT, {'launch.json': OWNER_SHA})
    sources = discover_sources(SEEDS, verification['verification_source_sha256']); verify(sources)
    resources = reference.hardware(); resources_for(resources)
    if args.source_preflight_only:
        print('FROZEN_FOOTPRINT_CONTROLLER_PROFILE_SOURCE_PREFLIGHT_PASS', len(sources), flush=True); return
    print('FROZEN_FOOTPRINT_CONTROLLER_PROFILE_INPUT_ADMISSION_STARTED', flush=True)
    ...
    write_json(OUTPUT/'launch.json', dict(source_sha256=sources, input_admission=admission,
        completed_replay_result_sha256=COMPLETED_SHA, independent_verification_sha256=VERIFICATION_SHA,
        ...
        invocation_frozen_footprint_receipts=True, normalization_outside_profiled_region=True))
    print('FROZEN_FOOTPRINT_CONTROLLER_PROFILE_LAUNCHED', digest(OUTPUT/'launch.json'), flush=True)
    cv2.setNumThreads(1); torch.set_num_threads(1); torch.use_deterministic_algorithms(True)
    start = time.perf_counter()
    try:
        report = replay(); verify(sources)
        if reference.admit_worker(WORKER_SHA, sources) != admission:
            raise ValueError('complete original admission changed')
        if completed_inputs() != (completed, verification):
            raise ValueError('completed paired replay evidence changed')
        ...
        write_json(OUTPUT/'result.json', dict(status='FROZEN_FOOTPRINT_CONTROLLER_WINDOWS_PROFILE_V1_COMPLETE',
        ...
        print('FROZEN_FOOTPRINT_CONTROLLER_PROFILE_COMPLETE', digest(OUTPUT/'result.json'), flush=True)
    except Exception as error:
        write_json(OUTPUT/'failure.json', dict(status='TERMINAL_FROZEN_FOOTPRINT_CONTROLLER_PROFILE_FAILURE', reason=repr(error)))
        raise
