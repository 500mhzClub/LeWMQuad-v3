#!/usr/bin/env python3
"""Authority-free V10 executor for the frozen N5 experiment.
...
from lewm.benchmarks import (
    go2_observable_camera_ray_fit_v4_n5_hierarchical_first_hit_v10 as policy,
)
...
if __name__ == "__main__":
    ...
    STAGING_PREFIX = ".n5.reservation-v10-"
    LOCK_NAME = ".n5.reservation-v10.lock"
    STAGING_SCHEMA = "lewm_go2_n5_hierarchical_first_hit_v10_preclaim_staging_v1"
    RECOVERY_POLICY = {
    ...
    def _directory_flags() -> int:
        ...
        if not nofollow or not directory:
            raise PermissionError("V10 canonical directory walks require no-follow opens")
        return os.O_RDONLY | nofollow | directory | getattr(os, "O_CLOEXEC", 0)


    @dataclass
    class DirectoryEntryV10:
        parent_fd: int
        name: str
        child_fd: int
    ...
    class DirectorySnapshotV10:
        fingerprint: tuple[int, ...]
        inventory: tuple[tuple[str, tuple[int, ...]], ...]


    @dataclass(frozen=True)
    class JournalEventV10:
        watch: int
        mask: int
        cookie: int
        name: str
    ...
    @dataclass
    class DirectoryWatchV10:
        watch: int
        generation: int
        directory_fd: int
        identity: tuple[int, int]
        label: str
        snapshot: DirectorySnapshotV10


    class OwnedDirectoryJournalV10:
        ...
        def __init__(self) -> None:
            ...
            if renameat2 is None:
                ...
                raise PermissionError(
                    "V10 requires Linux renameat2 for no-replace publication"
                )
            renameat2.argtypes = [
                ctypes.c_int,
                ctypes.c_char_p,
            ...
            self._fd = descriptor
            self._by_watch: dict[int, DirectoryWatchV10] = {}
            self._by_identity: dict[tuple[int, int], DirectoryWatchV10] = {}
            self._generation = 0
        ...
        def _fail(self, reason: str) -> None:
            if self._poison is None:
                self._poison = reason
            raise PermissionError(f"V10 owned-directory journal rejected: {reason}")

        @staticmethod
        def _identity(directory_fd: int) -> tuple[int, int]:
            metadata = os.fstat(directory_fd)
            if not stat.S_ISDIR(metadata.st_mode):
                raise PermissionError("V10 journal target is not a directory")
        ...
        @staticmethod
        def _snapshot(directory_fd: int) -> DirectorySnapshotV10:
            opened = os.fstat(directory_fd)
            if not stat.S_ISDIR(opened.st_mode):
                raise PermissionError("V10 journal directory descriptor changed")
            rows: list[tuple[str, tuple[int, ...]]] = []
            for name in sorted(os.listdir(directory_fd)):
                if Path(name).name != name or name in {"", ".", ".."}:
                    raise PermissionError("V10 journal observed a malformed child name")
                metadata = os.stat(
                    name,
                    dir_fd=directory_fd,
            ...
            return DirectorySnapshotV10(
                fingerprint=_stable_fingerprint(opened),
                inventory=tuple(rows),
            )

        def _events(self) -> list[JournalEventV10]:
            events: list[JournalEventV10] = []
            while True:
                try:
        ...
        def _reject_special(self, events: Sequence[JournalEventV10]) -> None:
            for event in events:
        ...
        def _shared_parent_events(
            self,
            events: Sequence[JournalEventV10],
            *,
            watch: int,
            reserved_name: str,
            allow_ignored: bool = False,
        ) -> tuple[list[JournalEventV10], list[JournalEventV10]]:
            ...
            reserved: list[JournalEventV10] = []
            ignored: list[JournalEventV10] = []
            for event in events:
                if event.mask & self.IN_Q_OVERFLOW:
                ...
                if event == JournalEventV10(watch, self.IN_IGNORED, 0, ""):
        ...
        def create_exclusive_root(
            ...
            *,
            ...
            unrelated child names are tolerated, while the V10 name must have
            ...
            try:
                ...
                if ignored != [
                    JournalEventV10(watch, self.IN_IGNORED, 0, "")
                ]:
        ...
        def watch_directory(
            ...
            *,
            label: str,
            expected_snapshot: DirectorySnapshotV10 | None = None,
        ) -> None:
            self.assert_clean()
        ...
        def _state(self, directory_fd: int) -> DirectoryWatchV10:
            identity = self._identity(directory_fd)
        ...
        def baseline(self, directory_fd: int) -> DirectorySnapshotV10:
            return self._state(directory_fd).snapshot

        def _snapshots(self) -> dict[int, DirectorySnapshotV10]:
            return {
        ...
        def _validate_snapshots(
            self,
            before: Mapping[int, DirectorySnapshotV10],
            after: Mapping[int, DirectorySnapshotV10],
            *,
        ...
        def _commit_snapshots(
            self,
            snapshots: Mapping[int, DirectorySnapshotV10],
        ) -> None:
        ...
        def _begin(
            self,
            directory_fd: int,
        ) -> tuple[DirectoryWatchV10, dict[int, DirectorySnapshotV10]]:
            self.assert_clean()
        ...
        def _finish(
            ...
            before: Mapping[int, DirectorySnapshotV10],
            *,
            expected_events: Sequence[JournalEventV10],
            expected_inventory: tuple[tuple[str, tuple[int, ...]], ...],
        ) -> DirectorySnapshotV10:
        ...
        def mkdir(self, parent_fd: int, name: str, *, mode: int = 0o700) -> None:
            ...
            self._finish(
                ...
                before,
                expected_events=(
                    JournalEventV10(state.watch, self.IN_CREATE | self.IN_ISDIR, 0, name),
                ),
        ...
        def create_file(
            ...
            *,
            ...
            expected_events = [
                JournalEventV10(state.watch, self.IN_CREATE, 0, name),
            ]
            if payload:
                expected_events.append(
                    JournalEventV10(state.watch, self.IN_MODIFY, 0, name)
                )
            expected_events.append(
                JournalEventV10(state.watch, self.IN_CLOSE_WRITE, 0, name)
            )
            expected = self._replace_inventory(
        ...
        def unlink(
            ...
            *,
            ...
            self._finish(
                ...
                before,
                expected_events=(
                    JournalEventV10(state.watch, self.IN_DELETE, 0, name),
                ),
                expected_inventory=expected,
            )
        ...
        def rmdir(
            ...
            *,
            ...
            if retired != [
                JournalEventV10(child.watch, self.IN_DELETE_SELF, 0, ""),
                JournalEventV10(child.watch, self.IN_IGNORED, 0, ""),
            ]:
    ...
    @dataclass
    class CanonicalDirectoryChainV10:
        anchor_fd: int
        anchor_identity_security: tuple[int, ...]
        descriptors: list[int]
        entries: list[DirectoryEntryV10]
        path_fds: dict[Path, int]
        journal: OwnedDirectoryJournalV10
        output_root_created: bool = False
        closed: bool = False


    @dataclass(frozen=True)
    class OwnedArtifactV10:
        role: str
        parent_fd: int
        name: str
        fingerprint: tuple[int, ...]
    ...
    def _assert_directory_chain(chain: CanonicalDirectoryChainV10) -> None:
        if chain.closed:
            raise PermissionError("V10 canonical directory chain is closed")
        chain.journal.assert_clean()
        if (
            _identity_security_fingerprint(os.fstat(chain.anchor_fd))
            != chain.anchor_identity_security
        ):
            raise PermissionError("V10 filesystem-root descriptor changed")
        for entry in chain.entries:
            ...
            if (
                ...
                raise PermissionError("V10 canonical directory component changed")


    def _open_canonical_directory_chain(final_path: Path) -> CanonicalDirectoryChainV10:
        ...
        if (
            ...
            raise PermissionError("V10 canonical directory target escaped the repository")
        ...
        journal = OwnedDirectoryJournalV10()
        chain = CanonicalDirectoryChainV10(
            anchor_fd=anchor_fd,
        ...
        try:
            if (
                ...
                raise PermissionError("V10 filesystem root changed during open")
            parent_fd = anchor_fd
            ...
            for index, component in enumerate(final_path.parts[1:]):
                ...
                try:
                    ...
                    if index < repository_depth:
                        raise PermissionError("V10 repository root component is missing")
                    if current_path / component == policy.CANONICAL_OUTPUT_ROOT:
                        ...
                        if not chain.output_root_created:
                            raise PermissionError(
                                "existing V10 recovery tree is missing a structural directory"
                            )
                    ...
                    if (
                        ...
                        raise PermissionError(
                            "V10 exclusive root changed before retained open"
                        )
                if stat.S_ISLNK(before.st_mode) or not stat.S_ISDIR(before.st_mode):
                    raise PermissionError("V10 canonical component is not a real directory")
                ...
                if (
                    ...
                    raise PermissionError("V10 canonical component changed during open")
                entry = DirectoryEntryV10(
                    parent_fd,
                    component,
                    child_fd,
                    identity_security,
    ...
    def _open_chain_child(
        ...
        try:
            ...
            if not chain.output_root_created:
                raise PermissionError(
                    "existing V10 recovery tree is missing a derived directory"
                )
            chain.journal.mkdir(parent_fd, name, mode=0o700)
            before = _entry_metadata(parent_fd, name)
        if stat.S_ISLNK(before.st_mode) or not stat.S_ISDIR(before.st_mode):
            raise PermissionError("V10 derived parent is not a real directory")
        identity_security = _identity_security_fingerprint(before)
        full_fingerprint = _stable_fingerprint(before)
        ...
        if (
            ...
            raise PermissionError("V10 derived parent changed during open")
    ...
    def _close_directory_chain(chain: CanonicalDirectoryChainV10) -> None:
        if chain.closed:
            return
        chain.closed = True
        chain.journal.close()
    ...
    class AttemptReservationV10:
        ...
        directory_chain: CanonicalDirectoryChainV10 | None = None
        owned_claim_artifacts: dict[str, OwnedArtifactV10] = field(
            default_factory=dict,
            compare=False,
        )
        owned_derived_artifacts: dict[str, OwnedArtifactV10] = field(
            default_factory=dict,
            compare=False,
        )
        ...
        @property
        def journal(self) -> OwnedDirectoryJournalV10:
            if self.directory_chain is None:
                raise PermissionError("N5 full-panel V10 lacks its transaction journal")
            return self.directory_chain.journal

    def _assert_claim_fd_owned(reservation: AttemptReservationV10) -> None:
        if reservation.directory_fd < 0 or reservation.directory_identity is None:
            raise PermissionError("N5 full-panel V10 lacks an open claimed directory")
        opened = os.fstat(reservation.directory_fd)
        if not stat.S_ISDIR(opened.st_mode):
            raise PermissionError("N5 full-panel V10 claimed descriptor is not a directory")
        if (opened.st_dev, opened.st_ino) != reservation.directory_identity:
            raise PermissionError("N5 full-panel V10 claimed descriptor identity changed")


    def _assert_owned_claim(reservation: AttemptReservationV10) -> None:
        ...
        if (
            ...
            raise PermissionError("N5 full-panel V10 claimed directory changed")
        if reservation.directory_chain is None or reservation.seed_root_fd < 0:
            raise PermissionError("N5 full-panel V10 lacks its canonical directory chain")
        ...
        if (
            ...
            raise PermissionError("N5 full-panel V10 canonical claim identity changed")
        ...
        if set(os.listdir(reservation.directory_fd)) != expected_names:
            raise PermissionError("N5 full-panel V10 claimed directory inventory changed")


    def _write_claim_file_exclusive(
        reservation: AttemptReservationV10,
        name: str,
        payload: bytes,
        *,
        mode: int = 0o600,
        ...
        if Path(name).name != name or name in {"", ".", ".."}:
            raise PermissionError("N5 full-panel V10 claim filename escaped")
        ...
        reservation.owned_claim_artifacts[name] = OwnedArtifactV10(
            role=role or name,
        ...
        if _stable_fingerprint(committed) != committed_fingerprint:
            raise PermissionError("N5 full-panel V10 committed claim leaf changed")
        if require_canonical:
            _assert_owned_claim(reservation)
        else:
            _assert_claim_fd_owned(reservation)
        ...
        *,
        ...
        try:
            ...
            if (
                ...
                raise PermissionError("N5 full-panel V10 claim leaf is not singly linked")
            chunks: list[bytes] = []
            ...
            if fingerprint(before) != fingerprint(after):
                raise RuntimeError("N5 full-panel V10 claim leaf changed while read")
        finally:
            os.close(descriptor)
        if require_canonical:
            _assert_owned_claim(reservation)
    ...
    def _classify_staging(
        ...
        *,
        attempt_path: Path,
        review_binding: Mapping[str, str],
    ) -> tuple[dict[str, Any], AttemptReservationV10 | None]:
        ...
        journal.assert_clean()
        return evidence, AttemptReservationV10(
            directory=attempt_path,
    ...
    def _complete_recoveries_are_equivalent(
        first: AttemptReservationV10,
        second: AttemptReservationV10,
    ) -> bool:
    ...
    def _validate_recovery_tree_baseline(
        ...
        """Reject historical bytes outside the sole recoverable V10 shape."""

        chain.journal.assert_clean()
        relative_seed = seed_root.relative_to(policy.CANONICAL_OUTPUT_ROOT)
        if len(relative_seed.parts) != 2:
            chain.journal._fail("V10 recovery seed-root depth changed")
        ...
        if not required_paths <= set(chain.path_fds):
            chain.journal._fail("V10 recovery tree lacks retained descriptors")
    ...
    def _reserve_exact_attempt(source_review_file_sha256: str) -> AttemptReservationV10:
        ...
        active_staging: OwnedStagingV10 | None = None
        owned_claim_identity: tuple[int, int] | None = None
        claimed_directory_fd: int | None = None
        claimed_chain: CanonicalDirectoryChainV10 | None = None
        claimed_reservation: AttemptReservationV10 | None = None
        ...
        try:
            ...
            with _locked_seed_root(
                ...
                complete: list[
                    tuple[OwnedStagingV10, AttemptReservationV10, dict[str, Any]]
                ] = []
                ...
                if (
                    ...
                    raise PermissionError("N5 full-panel V10 staging identity changed")
                owned_claim_identity = (staging_metadata.st_dev, staging_metadata.st_ino)
                claimed_reservation = AttemptReservationV10(
                    directory=attempt_path,
    ...
    def _cleanup_owned_artifacts(
        registry: dict[str, OwnedArtifactV10],
        *,
        journal: OwnedDirectoryJournalV10,
    ...
    def _terminate_failure(
        ...
        *,
        ...
        if isinstance(error, VerifierChildFailureV10):
            try:
    ...
    def _publish_success(
        ...
        *,
        ...
        if not policy.is_sha256(checkpoint_content_sha256):
            raise ValueError("N5 full-panel V10 checkpoint content hash is malformed")
    ...
    def _write_canonical_json(
        ...
        if path not in {policy.CANONICAL_METRIC_RECEIPT_PATH, policy.CANONICAL_GATE_PATH}:
            raise PermissionError("N5 full-panel V10 publication path is not canonical")
        expected_schema = (
        ...
        if (
            ...
            raise ValueError("N5 full-panel V10 derived artifact is malformed")
        raw = policy.canonical_json_bytes(value) + b"\n"
        relative = path.relative_to(policy.CANONICAL_OUTPUT_ROOT)
        if len(relative.parts) != 2:
            raise PermissionError("N5 full-panel V10 publication depth changed")
        if reservation.directory_chain is None or reservation.output_root_fd < 0:
            raise PermissionError("N5 full-panel V10 lacks its retained output root")
        ...
        if parent_fd < 0:
            raise PermissionError("N5 full-panel V10 lacks its derived parent descriptor")
        ...
        reservation.owned_derived_artifacts[relative_text] = OwnedArtifactV10(
        ...
        if _stable_fingerprint(committed) != committed_fingerprint:
            raise PermissionError("N5 full-panel V10 committed derived leaf changed")
        _assert_owned_claim(reservation)
        return policy.artifact_binding(
            relative_text,
            raw,
    ...
    def _read_owned_derived_artifact(
        ...
        if artifact is None:
            raise PermissionError("N5 full-panel V10 derived artifact is not owned")
        raw = _read_regular_at(
            artifact.parent_fd,
            artifact.name,
            expected_fingerprint=artifact.fingerprint,
        ...
        if hashlib.sha256(raw).hexdigest() != artifact.payload_sha256:
            raise PermissionError("N5 full-panel V10 derived payload changed")
        _assert_owned_claim(reservation)
        return raw
    ...
    def _run_frozen_training(
        ...
        *,
        rgb_workers: int,
    ) -> tuple[dict[str, Any], AttemptReservationV10]:
        """Run the reviewed V10 science behind an ephemeral local adapter."""
        ...
        def require(value: object) -> object:
            if value is not token:
                raise PermissionError("retained science lacks its local V10 execution token")
        ...
        def reserve(value: object) -> AttemptReservationV10:
            nonlocal claimed_reservation
            require(value)
            if claimed_reservation is not None:
                raise FileExistsError("the sole V10 attempt was already reserved")
        ...
        from scripts import (
            train_go2_observable_camera_ray_fit_v4_n5_hierarchical_first_hit_v10
            as retained,
        )
        ...
        try:
            ...
            if claimed_reservation is None:
                raise RuntimeError(
                    "frozen V10 training returned without claiming its attempt"
                )
            _assert_owned_claim(claimed_reservation)
            launch_summary = {
                **summary,
                "schema": "lewm_go2_observable_camera_ray_fit_v4_n5_hierarchical_first_hit_v10_launch_summary_v1",
            }
    ...
    @dataclass(frozen=True)
    class ChildProcessObservationV10:
        returncode: int
        signal: int | None
        timed_out: bool
        stdout_raw: bytes
    ...
    class VerifierChildFailureV10(RuntimeError):
        def __init__(
            self,
            reason: str,
            observation: ChildProcessObservationV10,
    ...
    class SmokeInjectedFailureV10(RuntimeError):
        pass
    ...
    def _parse_failure_envelope(
        ...
        try:
            value = policy.parse_json(raw, name="V10 verifier failure envelope")
        ...
        try:
            checked = _self_hashed_value(value, name="V10 verifier failure envelope")
        except (TypeError, ValueError):
            return None, None, "malformed"
        if (
    ...
    def _observe_child_process(
        ...
        *,
        ...
        return ChildProcessObservationV10(
    ...
    def _verification_child_environment() -> dict[str, str]:
        ...
        if _child_environment_projection(environment) != _expected_child_environment():
            raise PermissionError("V10 verifier child environment sanitization failed")
        return environment
    ...
    def _verification_request(
        ...
        if not sys.flags.isolated:
            raise PermissionError("V10 verifier parent must already be isolated")
        _assert_owned_claim(reservation)
        review, _ = policy.preflight_source_review(
            policy.CANONICAL_SOURCE_REVIEW_PATH,
            source_review_file_sha256,
        ...
        if len(raw) > policy.VERIFICATION_MAX_REQUEST_BYTES:
            raise ValueError("V10 verifier request exceeds its frozen size bound")
        _assert_owned_claim(reservation)
        return request, raw
    ...
    def _validate_verification_request(
        ...
        value = _self_hashed_value(request, name="V10 verifier request")
        expected_fields = {
            "schema",
            "nonce",
            "source_review",
        ...
        if (
            ...
            raise PermissionError("V10 verifier request contract changed")
        ...
        if (
            ...
            raise PermissionError("V10 verifier request process binding changed")
        source_review = value.get("source_review")
        if not isinstance(source_review, Mapping):
            raise PermissionError("V10 verifier request source review is malformed")
        review, _ = policy.preflight_source_review(
            policy.CANONICAL_SOURCE_REVIEW_PATH,
        ...
        if (
            ...
            raise PermissionError("V10 verifier request source binding changed")
        ...
        if (
            ...
            raise PermissionError("V10 verifier request artifact map changed")
        for role, expected_path in expected_paths.items():
            bound = artifacts.get(role)
            if not isinstance(bound, str):
                raise PermissionError("V10 verifier artifact binding is malformed")
            path, digest = policy.parse_bound_path(bound)
            if path != expected_path.resolve(strict=True) or not policy.is_sha256(digest):
                raise PermissionError("V10 verifier artifact binding changed")
            ...
            if (
                ...
                raise PermissionError("V10 verifier artifact content binding changed")
        policy.preflight_static_authority()
        return value
    ...
    def _compute_verification_receipt_child(
        ...
        if (
            ...
            raise PermissionError("V10 verifier child process boundary changed")
        ...
        def require(candidate: object) -> object:
            if candidate is not token:
                raise PermissionError(
                    "retained verifier lacks its local V10 child token"
                )
            policy.preflight_static_authority()
            policy.preflight_source_review(
                policy.CANONICAL_SOURCE_REVIEW_PATH,
        ...
        def forbid_publication(*_args: Any, **_kwargs: Any) -> None:
            raise PermissionError("V10 verifier child cannot publish")
        ...
        from scripts import (
            verify_go2_observable_camera_ray_fit_v4_n5_hierarchical_first_hit_v10
            as verifier,
        )
    ...
    def _load_claim_json(
        ...
        raw = _read_claim_file(reservation, name)
        value = policy.parse_json(raw, name=f"V10 parent {name}")
        if raw != policy.canonical_json_bytes(value) + b"\n":
            raise ValueError(f"V10 parent {name} is not canonical JSON")
        return _self_hashed_value(value, name=f"V10 parent {name}"), raw
    ...
    def _validate_child_metric_receipt(
        ...
        value = _self_hashed_value(receipt, name="V10 child metric receipt")
        expected_fields = {
            "schema",
            "authoritative",
        ...
        for role, binding in expected_artifacts.items():
            ...
            if digest != binding["file_sha256"]:
                raise PermissionError("V10 child request artifact changed")
        ...
        if (
            ...
            raise PermissionError("V10 child metric receipt provenance changed")
        from lewm.benchmarks import (
        ...
        if (
            ...
            raise ValueError("V10 child frozen metric decision changed")
        _assert_owned_claim(reservation)
        return value
    ...
    def _validate_verification_response(
        ...
        value = _self_hashed_value(response, name="V10 verifier response")
        expected_fields = {
            "schema",
            "status",
            "nonce",
        ...
        if (
            ...
            raise PermissionError("V10 verifier response binding changed")
        receipt = value.get("receipt")
        if not isinstance(receipt, Mapping):
            raise ValueError("V10 verifier response lacks a receipt")
        ...
        if value.get("receipt_content_sha256") != checked["content_sha256"]:
            raise PermissionError("V10 verifier response receipt binding changed")
        return checked
    ...
    def _run_independent_verification(
        ...
        try:
            ...
            response = policy.parse_json(
                response_raw,
                name="V10 verifier child response",
            )
            if response_raw != policy.canonical_json_bytes(response) + b"\n":
                raise ValueError(
                    "V10 verifier child response is not one canonical JSON value"
                )
            ...
            raise VerifierChildFailureV10(
                "V10 verifier boundary failed; no fallback",
                observation,
                request,
            ) from error
        _assert_owned_claim(reservation)
    ...
    def _verification_child_main() -> int:
        ...
        try:
            ...
            if not sys.flags.isolated or not sys.flags.dont_write_bytecode:
                raise PermissionError("V10 verifier child requires -I -B")
            ...
            if not raw or len(raw) > policy.VERIFICATION_MAX_REQUEST_BYTES:
                raise ValueError("V10 verifier child request size changed")
            phase_state.enter("request_parse")
            request = policy.parse_json(raw, name="V10 verifier child request")
            if raw != policy.canonical_json_bytes(request) + b"\n":
                raise ValueError("V10 verifier child request is not canonical JSON")
            ...
            if len(response_raw) > policy.VERIFICATION_MAX_RESPONSE_BYTES:
                raise ValueError("V10 verifier child response exceeds its size bound")
    ...
    def _smoke_child_environment(
        *,
        ...
        for name in (
            ...
            "PYTHONUSERBASE",
            "HSA_OVERRIDE_GFX_VERSION",
            "LEWM_V10_SMOKE_INJECTION_PHASE",
            "LEWM_V10_SMOKE_PROCESS_CASE",
            "LEWM_V10_VERIFIER_MODE",
        ):
            environment.pop(name, None)
        environment["PYTHONNOUSERSITE"] = "1"
        for name in (
        ...
        for name in policy.THREAD_ENVIRONMENT:
            environment[name] = "1"
        if smoke_mode:
            environment["LEWM_V10_VERIFIER_MODE"] = "cpu_contract_smoke_v1"
        if injection_phase is not None:
            ...
            environment["LEWM_V10_SMOKE_INJECTION_PHASE"] = injection_phase
        if process_case is not None:
            ...
            environment["LEWM_V10_SMOKE_PROCESS_CASE"] = process_case
    ...
    def _smoke_request(temporary_directory: Path) -> tuple[dict[str, Any], bytes]:
        ...
        core = {
            ...
            "bundle": {
                "schema": "lewm_v10_cpu_verifier_smoke_bundle_v1",
                "production_eligible": False,
                "frame_count": 5,
                "frames": frames,
    ...
    def _smoke_enter(phase_state: Any, phase: str) -> None:
        ...
        if os.environ.get("LEWM_V10_SMOKE_INJECTION_PHASE") == phase:
            raise SmokeInjectedFailureV10("injected verifier smoke phase failure")
    ...
    def _smoke_child_response(
        ...
        *,
        ...
        core = {
            ...
            "gate_reconstruction_sha256": gate_sha256,
            "publication_performed": False,
        }
        return {**core, "content_sha256": policy.canonical_json_sha256(core)}
    ...
    def _cpu_verifier_smoke_child_main() -> int:
        ...
        process_case = os.environ.get("LEWM_V10_SMOKE_PROCESS_CASE")
        ...
        try:
            ...
            if (
                ...
                or bundle.get("schema") != "lewm_v10_cpu_verifier_smoke_bundle_v1"
            ...
            gate_sha256 = policy.canonical_json_sha256(
                {
    ...
    def run_cpu_verifier_contract_smoke() -> dict[str, Any]:
        ...
        temporary_path: Path
        with tempfile.TemporaryDirectory(prefix="lewm-v10-verifier-smoke-") as name:
            ...
            if (
                ...
                raise RuntimeError("CPU verifier contract smoke success path failed")
            ...
            cases: dict[str, ChildProcessObservationV10] = {}
            for process_case in (
                "timeout",
                "signal",
                "nonzero",
        ...
        return {
            "schema": "lewm_v10_cpu_verifier_contract_smoke_summary_v1",
            "production_eligible": False,
    ...
    def _run_finalization(
        ...
        def require(value: object) -> object:
            if value is not token:
                raise PermissionError("retained finalizer lacks its local V10 execution token")
            policy.preflight_static_authority()
            policy.preflight_source_review(
                policy.CANONICAL_SOURCE_REVIEW_PATH,
                source_review_file_sha256,
        ...
        def write_gate(path: Path, value: Mapping[str, Any]) -> dict[str, Any]:
            if Path(path) != policy.CANONICAL_GATE_PATH:
                raise PermissionError("retained finalizer changed its V10 output path")
    ...
    def execute_exact(
        ...
        *,
        ...
        if not sys.flags.isolated:
            raise PermissionError("N5 full-panel V10 exact execution requires isolation")
        ...
        if (
            ...
            raise FileExistsError(
                "the sole N5 full-panel V10 recovery attempt is already claimed"
            )
        policy.preflight_static_authority()
        policy.preflight_source_review(
            policy.CANONICAL_SOURCE_REVIEW_PATH,
        ...
        try:
            ...
            _assert_owned_claim(reservation)
            return {
                "schema": "lewm_go2_observable_camera_ray_fit_v4_n5_hierarchical_first_hit_v10_end_to_end_summary_v1",
                "seed": 20260710,
                "fit_size": 5,
                "training": training,
            ...
            try:
                ...
                raise RuntimeError(
                    f"N5 full-panel V10 {stage} failed and terminal receipt "
                    "could not be written"
                ) from terminal_error
            raise
        finally:
    ...
    def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
        ...
        if not policy.is_sha256(args.source_review_sha256):
            raise ValueError("N5 full-panel V10 source review SHA-256 is malformed")
        return args
    ...
    def dispatch(argv: Sequence[str] | None = None) -> int:
        ...
        if raw_argv == ["--verification-child"]:
            if not sys.flags.isolated:
                raise PermissionError(
                    "V10 verifier child cannot self-authorize isolation"
                )
            if (
                os.environ.get("LEWM_V10_VERIFIER_MODE")
