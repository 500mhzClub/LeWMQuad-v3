...
from scripts.exact_mission_target_goal_episode_development import collect,artifacts
from scripts.exact_mission_target_goal_audit_development import audit
...
from scripts.replay_go2_exact_mission_target_prefix_v1 import OUTPUT as INTEGRATION
...
PREVIOUS=BASE/'go2_observed_floor_contact_goal_probe_v1_attempt_001'
DIAGNOSTIC=BASE/'go2_observed_floor_contact_goal_readout_v1_attempt_001'
PREVIOUS_SHA='aab193a96edcee6fa2e15a62aab333821f6688995ca8f9e68cdc8887eaf403d9'
DIAGNOSTIC_SHA='aaa0b6202883a22b46acdfe9873414cf76c4b159767bebea1e20263ad9f476dc'
OUTPUT=BASE/'go2_exact_mission_target_goal_probe_v1_attempt_001'
...
PROTOCOL='docs/go2_exact_mission_target_goal_probe_v1_2026-09-08.md'
...
def worker(case,launch_sha):
    ...
    terminal=dict(case=name,trial=trial,variant=variant,condition=condition,model_name=model_name,
        status='EXACT_MISSION_TARGET_GOAL_WORKER_FAILED',artifact_sha256={});started=time.perf_counter()
    with (OUTPUT/(name+'_worker.log')).open('x') as log,contextlib.redirect_stdout(log),contextlib.redirect_stderr(log):
        try:
            ...
            terminal.update(status='EXACT_MISSION_TARGET_GOAL_COLLECTED_AND_RAW_AUDITED',artifact_sha256=bindings,
...
def main():
    ...
    assert previous['status']=='OBSERVED_FLOOR_CONTACT_GOAL_PROBE_COMPLETE' and previous['cases']==[list(c) for c in CASES]
    ...
    assert diagnostic['status']=='OBSERVED_FLOOR_CONTACT_GOAL_READOUT_COMPLETE'
    ...
    assert integration['status']=='EXACT_MISSION_TARGET_PREFIX_COMPLETE' and len(integration['conditions'])==2
    assert all(r['causal_observations_maps_forecasts_and_nominal_checks_exact'] and r['surface_checks_exact'] and r['only_final_goal_target_and_associated_scores_changed'] and not r['controller_failures'] and r['model_state_unchanged'] and r['exact_fresh_replay_passes']==2 for r in integration['conditions'])
    ...
    launch=preflight(output=OUTPUT,protocol=PROTOCOL,
        seed_paths=('scripts/run_go2_exact_mission_target_goal_probe_v1.py',
            'lewm/tests/test_exact_mission_target_native_scope_development.py',
            'docs/go2_exact_mission_target_prefix_result_2026-09-08.md',
    ...
    launch.update(integration_artifact_sha256=integration_ids,sensor_prefix_artifact_sha256=sensor_ids,
        ...
        experiment='exact instructed final goal replaces goal-cell centre only with fully observed and nominally clear connector; models, scoring formula, contact and nominal constraints, arrival gates and budgets unchanged',
    ...
    print('EXACT_MISSION_TARGET_GOAL_LAUNCHED',digest(OUTPUT/'launch.json'),flush=True)
    ...
    try:
            ...
            with ProcessPoolExecutor(max_workers=2,mp_context=multiprocessing.get_context('spawn'),max_tasks_per_child=1) as pool:
                ...
                while pending:
                    ...
                    for future in done:
                        ...
                        except Exception as error:row=dict(case=case[0],status='EXACT_MISSION_TARGET_GOAL_PROCESS_FAILURE',failure=repr(error))
                        records.append(row);print('EXACT_MISSION_TARGET_GOAL_TERMINAL',case[0],row['status'],row.get('goal'),flush=True)
        if len(records)!=2 or any(r['status']!='EXACT_MISSION_TARGET_GOAL_COLLECTED_AND_RAW_AUDITED' for r in records):
        ...
        result=dict(status='EXACT_MISSION_TARGET_GOAL_PROBE_COMPLETE',cases=CASES,conditions=records,
        ...
        write_json(OUTPUT/'result.json',result);print('EXACT_MISSION_TARGET_GOAL_COMPLETE',digest(OUTPUT/'result.json'),flush=True)
    except Exception as error:
        write_json(OUTPUT/'failure.json',dict(status='TERMINAL_EXACT_MISSION_TARGET_GOAL_FAILURE',reason=repr(error),
