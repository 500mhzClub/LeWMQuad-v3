...
from lewm.release_aware_navigation_development import ReleaseAwareNavigation as WholeTaskNavigation
...
from scripts.run_go2_measured_line_integral_navigation_development_v1 import preflight as prior_preflight, OUTPUT as PRIOR, verify_native

OUTPUT = ROOT/'.generated/go2_release_aware_navigation_development_v1_attempt_001'
SCHEMA = 'release_aware_navigation_development.v1'
NEW_SOURCES = (
    'lewm/release_aware_navigation_development.py',
    'lewm/tests/test_release_aware_navigation_development.py',
    'scripts/run_go2_release_aware_navigation_development_v1.py',
    'scripts/audit_go2_release_aware_navigation_development_v1.py',
    'scripts/audit_release_aware_rgb_core_development.py',
    'lewm/tests/test_release_aware_execution_development.py',
    'docs/go2_release_aware_navigation_development_v1_2026-09-05.md',
    'docs/go2_measured_line_integral_navigation_development_v1_result_2026-09-05.md')
...
def trial_specs():
    ...
    for spec in original_specs():
        ...
        spec['scene_id'] = 'go2-release-aware-navigation-development-v1-'+spec['layout_name']
        spec['method'] = 'release_aware'
        result.append(spec)
    return result
...
def preflight():
    ...
    if (inherited != launch['source_sha256'] or audit['status'] != 'PASS' or audit['audited_trials'] != 2):
        raise ValueError('source-bound completed line/integral audit required; all scientific failures retained')
...
def main():
    ...
    write_json(OUTPUT/'launch.json', {'schema': SCHEMA, 'trial_specs': specs,
        ...
        'scope': 'two continuous release-aware alignment control missions; sampled nominal volume and unqualified future gait, no JEPA/hardware qualification'})
    rows, prefixes = [], {}
    try:
        for spec in specs:
