...
import numpy as np
from scripts.run_go2_exact_mission_target_goal_probe_v1 import OUTPUT as INPUT,PREVIOUS as PRIOR,PREVIOUS_SHA as PRIOR_SHA,CASES,TRIAL
...
OUTPUT=BASE/'go2_exact_mission_target_goal_readout_v1_attempt_001'
PROTOCOL='docs/go2_exact_mission_target_goal_readout_v1_2026-09-08.md'
...
def main():
    ...
    sources=discover_sources((PROTOCOL,'scripts/read_go2_exact_mission_target_goal_probe_v1.py',
        'lewm/tests/test_exact_mission_target_goal_readout_development.py'),old['source_sha256'])
    ...
    try:
        ...
        write_json(OUTPUT/'result.json',dict(status='EXACT_MISSION_TARGET_GOAL_READOUT_COMPLETE',
        ...
        print('EXACT_MISSION_TARGET_GOAL_READOUT_COMPLETE',digest(OUTPUT/'result.json'),flush=True)
    except Exception as error:
        write_json(OUTPUT/'failure.json',dict(status='TERMINAL_EXACT_MISSION_TARGET_GOAL_READOUT_FAILURE',reason=repr(error)));raise
