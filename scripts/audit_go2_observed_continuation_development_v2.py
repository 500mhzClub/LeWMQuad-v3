#!/usr/bin/env python3
"""Reader-population-only correction; same physical evidence and scientific checks."""
import hashlib
import json
...
def audit_trial(directory, spec, row, template=None):
    ...
    for tick, decision in enumerate(decisions):
        ...
        packet = load_continuation_observation(directory, decision['observation_index'])
    ...
    if fault is not None:
        ...
        packet = load_continuation_observation(directory, fault['observation_index'])
        try:
    ...
    for index, camera in enumerate(cameras):
        packet = load_continuation_observation(directory, index)
...
CORRECTION_SOURCES = (
    'lewm/continuation_rgb_dataset_development.py',
    'scripts/audit_go2_observed_continuation_development_v2.py',
    'lewm/tests/test_continuation_reader_correction_development.py',
    'lewm/tests/test_causal_rgb_dataset_development.py',
    'lewm/tests/test_simulated_body_observation_development.py',
    'docs/go2_observed_continuation_reader_correction_2026-09-05.md')


def bind_correction():
    inputs = {str((OUTPUT/leaf).relative_to(ROOT)): sha for leaf, sha in (
        ...
        ('raw_artifact_audit.json', 'df3ed99d49c66e4161e6054358f4f4dd05309b041f95e54f8d29ee3e02dccd2c'))}
    verify_bindings(inputs)
    failure = json.loads((OUTPUT/'raw_artifact_audit.json').read_text())
    check(failure['status'] == 'FAIL' and len(failure['trials']) == 4
          and failure['error'] == "SensorContractError('invalid route observation population')",
          'preserved original reader-population failure')
    sources = {name: digest(ROOT/name) for name in CORRECTION_SOURCES}
    verify_bindings(sources)
    witness = {'source_sha256': sources, 'input_sha256': inputs,
               'scope': 'reader806 instead of341 population bound only; no changed packets, controller or scientific endpoints'}
    write_json(OUTPUT/'audit_population_v2_binding.json', witness)
    return sources | inputs


def main():
    if len(sys.argv) != 1 or (OUTPUT / 'raw_artifact_audit_population_v2.json').exists() or (OUTPUT/'audit_population_v2_binding.json').exists():
    ...
    correction_bindings = bind_correction()
    ...
    try:
        ...
        verify_bindings(launch['source_sha256'] | launch['input_sha256'] | launch['gait_sha256'])
        verify_bindings(correction_bindings)
        write_json(OUTPUT / 'raw_artifact_audit_population_v2.json', {'status': 'PASS', 'audited_trials': len(rows), 'trials': rows,
        ...
        write_json(OUTPUT / 'raw_artifact_audit_population_v2.json', {'status': 'FAIL', 'error': repr(error), 'trials': rows,
                   'study_result_sha256': digest(OUTPUT / 'result.json'), 'audit_source_sha256': digest(Path(__file__))})
        raise
