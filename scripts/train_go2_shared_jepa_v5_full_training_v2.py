#!/usr/bin/env python3
"""New-process exact trainer/publisher for Shared JEPA V5 full training V2.
...
if __name__ == "__main__":
    ...
    def _publish_bootstrap_artifacts(context: TrainerContext) -> None:
        ...
        _publish_json(
            ...
            {
                "schema": "lewm_go2_shared_jepa_v5_full_training_v2_preflight_binding_v1",
        ...
        class FixedProductionTrainingBackend:
            ...
            def _checkpoint_bytes(
                ...
                *,
                ...
                semantic = {
                    "schema": "lewm_go2_shared_jepa_v5_full_training_v2_checkpoint_v1",
                    "arm": arm,
                    "update": update,
                ...
                for update in range(1, policy.UPDATE_COUNT + 1):
                    ...
                    for micro in range(policy.ACCUMULATION_STEPS):
                        ...
                        for name, value in {
                            ...
                            "current_v4_ordered_first_hit_nll": current_v4.ordered_first_hit_nll,
                            ...
                            "next_v4_ordered_first_hit_nll": next_v4.ordered_first_hit_nll,
                    ...
                    trace_rows.append(
                        {
                            "schema": "lewm_go2_shared_jepa_v5_full_training_v2_trace_row_v1",
                            "arm": arm,
                            "update": update,
                    ...
                    for start in range(0, len(frames), policy.MICROBATCH_SIZE):
                        ...
                        for source, accumulator in ((target, correct_acc), (wrong, wrong_acc)):
                            ...
                            if accumulator is correct_acc:
                                loss = model.observable_camera_ray_v4_loss(
            ...
            def _qualified_checkpoint(self, checkpoint_raw: bytes, selection: Mapping[str, Any], calibration: Mapping[str, Any]) -> tuple[bytes, str, str]:
                checkpoint = torch.load(io.BytesIO(checkpoint_raw), map_location="cpu", weights_only=False)
                ...
                core = {
                    "schema": model_module.CHECKPOINT_V5_SCHEMA,
                    "model_config": model.model_config.to_dict(),
            ...
            def run(self) -> dict[str, Any]:
                ...
                checkpoint_metrics = _publish_json(
                    ...
                    {
                        "schema": "lewm_go2_shared_jepa_v5_full_training_v2_checkpoint_metrics_v1",
                ...
                _publish_json(
                    ...
                    {
                        "schema": "lewm_go2_shared_jepa_v5_full_training_v2_matched_metrics_v1",
                ...
                qualified_raw, qualified_content, qualified_state = self._qualified_checkpoint(
                ...
                _publish_bytes(
                    ...
                    qualified_raw,
                    content_sha256=qualified_content,
                    state_sha256=qualified_state,
                )
                ...
                training_record = _publish_json(
                    ...
                    "training_record.json",
                    {
                        ...
                        "status": "development_checkpoint_qualified_pending_independent_verification",
                ...
                return {
                    ...
                    "qualified_state_sha256": qualified_state,
                }
    ...
    def _finalize_summary(context: TrainerContext, result: Mapping[str, Any]) -> dict[str, Any]:
        ...
        core = {
            "schema": "lewm_go2_shared_jepa_v5_full_training_v2_trainer_summary_v1",
            ...
            "qualified_state_sha256": result["qualified_state_sha256"],
