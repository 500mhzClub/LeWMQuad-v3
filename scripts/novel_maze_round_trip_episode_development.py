...
from lewm.novel_maze_round_trip_contract_development import (
    ...
    RESERVE_BYTES, PERSISTENCE_HEADROOM_BYTES, COLLECTION_ALLOWANCE_BYTES)
...
def collect(layout_index, definition, *, output, model, geometry, episode_name, condition, variant):
    ...
    try:
        ...
        controller = ObservedRoundTripController(model, geometry, public_mission=mission,
        ...
        result = dict(status='NOVEL_MAZE_ROUND_TRIP_TERMINAL_AUDIT_REQUIRED', layout_index=layout_index,
        ...
        print('NOVEL_MAZE_ROUND_TRIP_COLLECTED', result, flush=True)
        return result
    finally:
