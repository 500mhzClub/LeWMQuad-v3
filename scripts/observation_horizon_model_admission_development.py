...
from scripts.run_go2_observation_horizon_fits_v1 import OUTPUT,ROSTER,OPTIMIZATION_SEEDS,canonical
from scripts.observation_horizon_fit_inputs_development import authenticate,stream,TARGET_SHA
...
def admit(result_sha256):
    ...
    if (result['status']!='OBSERVATION_HORIZON_EIGHTEEN_FITS_COMPLETE' or result['optimizer_updates']!=21600
    ...
    if any(launch['science'].get(k)!=v for k,v in dict(target_cadence_ns=100_000_000,maximum_horizon_ns=800_000_000,action_ticks_per_block=1).items()):
        raise ValueError('exact short-horizon scientific contract required')
    check_sha=launch['science']['input_result_sha256'];_,_,schedules=authenticate(check_sha)
    if read_json(OUTPUT,'training_schedules.json')!=schedules:raise ValueError('all three exact mixed schedules required')
    pair=dict(targets=TARGET_SHA,inputs=check_sha)
    ...
    for record in result['records']:
        ...
        if (record['status']!='OBSERVATION_HORIZON_WORKER_COMPLETE' or record['actual_updates']!=1200
