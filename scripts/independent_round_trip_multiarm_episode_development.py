"""Fixed four-arm collection with the original sensing and physics loop.

No entry point: a future fully admitted population launcher owns execution.
"""
import shutil
import time
import json
...
from lewm.independent_round_trip_multiarm_contract_development import (
    COLLECTION_STATUS, treatment, verify_model, command_role)
from scripts.independent_round_trip_controller_factory_development import create
...
from scripts.renderer_witness_dual_camera_maze_session_development import ARTIFACT as RENDERER_WITNESSES
...
def collect(case, definition, *, output, geometry, correction_admission=None):
    require_case(case); layout_index = case.layout_index
    output = validate_root(output); directory = output/case.name
    if directory.exists() or directory.is_symlink():
        raise ValueError('exclusive assigned episode directory required')
    controller, model = create(case, geometry, correction_admission=correction_admission)
    ...
    try:
        session = IndependentRoundTripSession(spec, directory); session.install_contact_identity()
            ...
            try:
                ...
                for tick in range(MAX_OBSERVATIONS):
                    ...
                    if selected['terminal'] is not None:
                        ...
                        phase, role = 2, command_role(case)
                    session.phase = phase
        ...
        verify_model(case, model)
        result = dict(status=COLLECTION_STATUS, **treatment(case),
            ...
            measured_floor_transport_enabled=True, renderer_witnesses_recorded=True,
        ...
        print('INDEPENDENT_ROUND_TRIP_MULTIARM_COLLECTED', result, flush=True)
        return result
    finally:
...
def artifacts(case, result):
    ...
    require_collection(case, result)
