...
OUTPUT = ROOT/'.generated/go2_depth_proposal_navigation_interface_development_v1_attempt_001'
PROTOCOL = 'docs/go2_depth_proposal_navigation_interface_development_v1_2026-09-06.md'
SEEDS = (PROTOCOL, 'scripts/probe_go2_depth_proposal_navigation_interface_development_v1.py',
         'lewm/tests/test_rgbd_fused_navigation_development.py',
         'lewm/tests/test_rgbd_fused_navigation_interface_serialization_development.py',
         'lewm/tests/test_depth_proposal_navigation_development.py')
IDENTITIES = {
...
def assert_serialized_equal(actual, saved):
    # Exact JSON representation, no float tolerance or scientific-field omission.
    if json.loads(json.dumps(actual, allow_nan=False)) != saved:
        raise AssertionError('serialized scientific state differs')


def preflight():
    bindings = {str((INPUT/n).relative_to(ROOT)): h for n, h in IDENTITIES.items()}
    failed = ROOT/'.generated/go2_rgbd_fused_navigation_interface_development_v1_attempt_001'
    bindings |= {str((failed/n).relative_to(ROOT)): h for n, h in {
        'launch.json': '296460553bf2900eb9760b5d2e9a01307041a38a0039f1a0e89bc9b84059279e',
        'failure.json': 'a80c2454ddf83339e725d8c0a8d7375087b7283a70aae42fdfeb25d1c0be817a'}.items()}
    verify_bindings(bindings)
    failed_launch = read_json(failed, 'launch.json')
    verify(failed_launch)
    ...
    verify_bindings(bindings)
    predecessor = ROOT/'.generated/go2_rgbd_fused_navigation_interface_development_v2_attempt_001'
    witness = {str((predecessor/n).relative_to(ROOT)): h for n, h in {
        'launch.json': '12b77128bf89af2c8190ea0185ae9688f479c765d3122b16f24584b01f88f19e',
        'result.json': '9abf0a59b7ab184a808680adda2d62fbd139e2aaed31959fdc93ca1d61a69eec'}.items()}
    verify_bindings(witness)
    predecessor_launch = read_json(predecessor, 'launch.json')
    verify(predecessor_launch)
    predecessor_result = read_json(predecessor, 'result.json')
    witness |= {str((predecessor/n).relative_to(ROOT)): h for n, h in predecessor_result['artifact_sha256'].items()}
    bindings |= witness
    verify_bindings(bindings)
    sources = discover_sources(SEEDS, predecessor_launch['source_sha256'])
    launch = old | dict(source_sha256=sources, input_sha256=old['input_sha256'] | bindings,
        ...
        scope='measured-depth proposal successor on already-seen sensors; no closed-loop outcome')
    verify(launch)
    return launch
...
def run_arm(arm, launch):
    ...
    controller = DepthProposalNavigation(ArticulatedCollisionGeometry(URDF), memory_arm='local_only',
    ...
    for index, shadow in enumerate(saved):
        ...
        try:
            ...
            assert_serialized_equal(decision['sensor_fusion'], expected['fusion'])
            assert_serialized_equal(decision['raw_depth_motion'], expected['depth_state']['motion'])
