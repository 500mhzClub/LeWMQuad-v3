...
from lewm.sustained_motion_collection_development import TRIALS, specification, schedule, TAIL_TICKS
...
from scripts.run_go2_sustained_observed_floor_motion_development_v1 import OUTPUT, PROTOCOL, artifact_names
...
def audit_tape(raw, tape):
    ...
    if len(tape) != len(expected) or len(raw['timestamp_s']) != 17500:
    ...
    np.testing.assert_array_equal(np.rint(raw['timestamp_s']*1e9), np.arange(1,17501)*2_000_000)
    previous = None
    for i, (item, wanted) in enumerate(zip(tape, expected, strict=True)):
        ...
        previous = finish
    return dict(complete=True, motion_ticks=330, zero_tail_ticks=5, all_commands_and_clocks_reconstructed=True)
...
def audit_trial(trial, result, definition):
    ...
    assert [c['physical_sample_index'] for c in cameras] == list(range(749,17500,50))
    ...
    assert result['completed_motion_ticks']==330 and result['completed_zero_tail_ticks']==5
    ...
    assert len(saved)==len(cameras)==336
...
def main():
    ...
    sources = discover_sources(('scripts/audit_go2_sustained_observed_floor_motion_development_v1.py',
        'lewm/tests/test_sustained_motion_raw_audit_development.py'),launch['source_sha256'])
