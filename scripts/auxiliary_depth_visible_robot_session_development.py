...
import copy
import numpy as np
from lewm.geometry_progress_layout_family_development import specification,pack
...
class VisibleRobotFamilyPhysicalInit(_GenesisPhysicalSession):
    def __init__(self,spec,*,backend):
        ...
        self._runtime=dict(backend='cpu',n_envs=1,physics_dt_s=.002,policy_dt_s=.02,command_dt_s=.1,
            ...
            high_level='fixed recorded command prefix for sensor characterization; no navigation policy',
...
class VisibleRobotFamilySession(RGBDSession,VisibleRobotFamilyPhysicalInit):
    ...
    def capture_fixed_rgb(self,output,name):
        ...
        write_json(output/(name.replace('rgb_','raster_')+'.json'),dict(
            physical_sample_index=len(self.samples)-1,order=verify_order(self.ctx.build,self.raster_order),
            precision=precision_readback(self.ctx.build.camera)))
        return row
