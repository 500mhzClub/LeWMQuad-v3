"""Two fresh native cases with learned view acquisition and observed-floor waypoints."""
import argparse
import contextlib
import json
...
from scripts.active_view_goal_episode_development import collect, artifacts
from scripts.active_view_goal_audit_development import audit
...
OUTPUT = BASE/'go2_active_view_goal_probe_v1_attempt_001'
MEMORY = BASE/'go2_observed_floor_waypoint_v1_attempt_001'
MEMORY_IDS = {'launch.json': 'ff9159b290db08aa3e636058cb9bcf22b8be1b13fc4baacc6cc6a5198da88cf7',
    'result.json': '1c482b9a875092c9866d8085832426e95c802f60ca094bc4b52c4261428f1182'}
PROTOCOL = 'docs/go2_active_view_goal_probe_v1_2026-09-08.md'
TRIALS = ('family_episode_052', 'family_episode_039')
CASES = [('active_view_'+t, t, True) for t in TRIALS]
...
def worker(case, launch_sha):
    ...
    terminal = dict(case=name, trial=trial, persistent=persistent,
        status='ACTIVE_VIEW_GOAL_WORKER_FAILED', artifact_sha256={})
    ...
    with (OUTPUT/(name+'_worker.log')).open('x') as log, contextlib.redirect_stdout(log), contextlib.redirect_stderr(log):
        try:
            ...
            terminal.update(status='ACTIVE_VIEW_GOAL_COLLECTED_AND_RAW_AUDITED',
...
def main():
    ...
    assert memory['status'] == 'OBSERVED_FLOOR_WAYPOINT_REPLAY_COMPLETE'
    ...
    launch = preflight(output=OUTPUT, protocol=PROTOCOL,
        seed_paths=('scripts/run_go2_active_view_goal_probe_v1.py',
            'lewm/tests/test_active_view_goal_probe_development.py', *memory['source_sha256']),
    ...
    launch.update(checkpoint=checkpoint, memory_sha256=MEMORY_IDS, planned_cases=CASES,
        ...
        experiment='active side-view acquisition and observed-floor waypoints V1',
    ...
    print('ACTIVE_VIEW_GOAL_LAUNCHED', digest(OUTPUT/'launch.json'), flush=True)
    ...
    try:
        with (OUTPUT/'resource_monitor.jsonl').open('x') as monitor:
            with ProcessPoolExecutor(max_workers=1, mp_context=multiprocessing.get_context('spawn'), max_tasks_per_child=1) as executor:
                for case in CASES:
                    ...
                    print('ACTIVE_VIEW_GOAL_TERMINAL', case[0], record['status'], record.get('goal'), flush=True)
                    if record['status'] != 'ACTIVE_VIEW_GOAL_COLLECTED_AND_RAW_AUDITED':
        ...
        verify(launch); verify_artifacts(OUTPUT, bindings)
        write_json(OUTPUT/'result.json', dict(status='ACTIVE_VIEW_GOAL_PROBE_COMPLETE',
        ...
        print('ACTIVE_VIEW_GOAL_COMPLETE', digest(OUTPUT/'result.json'), flush=True)
    except Exception as error:
        write_json(OUTPUT/'failure.json', dict(status='TERMINAL_ACTIVE_VIEW_GOAL_FAILURE',
