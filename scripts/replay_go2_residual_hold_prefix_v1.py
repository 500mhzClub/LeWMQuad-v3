...
OUTPUT = BASE/'go2_residual_hold_prefix_v1_attempt_001'
PROTOCOL = 'docs/go2_residual_hold_prefix_v1_2026-09-09.md'
...
def main():
    ...
    sources = discover_sources((PROTOCOL, 'scripts/replay_go2_residual_hold_prefix_v1.py',
        'lewm/tests/test_residual_hold_feasibility_development.py',
        'lewm/tests/test_residual_hold_prefix_development.py'), result['source_sha256'])
    ...
    print('RESIDUAL_HOLD_PREFIX_LAUNCHED', digest(OUTPUT/'launch.json'), flush=True)
    try:
        ...
        write_json(OUTPUT/'result.json', dict(status='RESIDUAL_HOLD_PREFIX_V1_COMPLETE',
        ...
        print('RESIDUAL_HOLD_PREFIX_COMPLETE', digest(OUTPUT/'result.json'),
            {k:v for k,v in report.items() if k != 'changed_selection'}, flush=True)
    except Exception as error:
        write_json(OUTPUT/'failure.json', dict(status='TERMINAL_RESIDUAL_HOLD_PREFIX_FAILURE', reason=repr(error)))
        raise
