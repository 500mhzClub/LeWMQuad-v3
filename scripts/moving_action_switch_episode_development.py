"""One fixed moving-prefix/suffix case at a fresh owned native output path."""
import shutil
import time
import cv2
from lewm.moving_action_switch_family_development import TRIALS,specification,schedule,decision,BRANCH_TICK,assignments,branch_specification
...
def collect(trial,definition,*,output):
    ...
    write_json(directory/'branch_specification.json',branch_specification(trial))
    ...
    try:
        ...
        try:
            ...
            for tick in range(len(schedule(trial))+1):
                ...
                try:
                    p,d,f,now=session.sensor_packets();selected=decision(trial,tick,p)
                ...
                if selected['terminal']:terminal='FIXED_MOVING_ACTION_SWITCH_COMPLETE';break
        ...
        result=dict(status='MOVING_ACTION_SWITCH_TERMINAL_AUDIT_REQUIRED',trial=trial,physical_stop=stop,acquisition_stop=acquisition_stop,
            schedule_terminal=terminal,setup_admitted=admitted,setup_checked=(directory/'setup_checks.json').is_file(),
            departure_present=any(r['tick']==BRANCH_TICK for r in rows),command_ticks=len(tape),completed_ticks=sum(t['completed'] for t in tape),
        ...
        print('MOVING_ACTION_SWITCH_FAMILY',trial,result,flush=True);return result
    finally:
...
def artifacts(trial,result):
    names=['branch_specification.json','specification.json','actuator_identity.json','floor_roles.json','terminal_actuator_gains.json',
