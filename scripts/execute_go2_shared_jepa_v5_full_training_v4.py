#!/usr/bin/env python3
"""Fail-closed exact reserver for Shared JEPA V5 full training V4.

This entry point is standard-library only.  The fixed final exact authorization must
...
if __name__ == "__main__":
    ...
    def _load_ready_manifest(expected_file_sha256: str) -> tuple[dict[str, Any], bytes]:
        if not policy.is_sha256(expected_file_sha256):
            raise ValueError("final-exact-authorization SHA-256 is malformed")
        raw = _read_repo_authority(policy.FINAL_EXACT_EXECUTION_AUTHORIZATION_RELATIVE_PATH)
        if hashlib.sha256(raw).hexdigest() != expected_file_sha256:
            raise PermissionError("final-exact-authorization file hash changed")
        value = policy.parse_canonical_json(raw, name="exact final exact authorization")
        return policy.validate_final_exact_execution_authorization(value), raw
    ...
    def _reserve_exact(
        ...
        try:
            ...
            core = {
                ...
                "final_exact_authorization_content_sha256": manifest["content_sha256"],
    ...
    def _run_fixed_child(
        ...
        completed = subprocess.run(
            [
                ...
                reservation.directory_name,
                "--final-exact-authorization-sha256",
                reservation.manifest_file_sha256,
                "--final-exact-authorization-content-sha256",
    ...
    def _publish_completion(
        ...
        if (
            ...
            != "lewm_go2_shared_jepa_v5_full_training_v4_trainer_summary_v1"
            or trainer_summary.get("status") != "trainer_publication_complete"
            or verification.get("schema")
            != "lewm_go2_shared_jepa_v5_full_training_v4_verification_v1"
            ...
            or trainer_summary.get("final_exact_authorization_content_sha256")
            != reservation.manifest["content_sha256"]
            or verification.get("claim_identity")
            != list(reservation.directory_identity)
            or verification.get("final_exact_authorization_file_sha256")
            != reservation.manifest_file_sha256
            or verification.get("final_exact_authorization_content_sha256")
            != reservation.manifest["content_sha256"]
        ...
        core = {
            ...
            "final_exact_authorization_file_sha256": reservation.manifest_file_sha256,
            "final_exact_authorization_content_sha256": reservation.manifest["content_sha256"],
    ...
    def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
        ...
        if not policy.is_sha256(args.final_exact_authorization_sha256):
            raise ValueError("final-exact-authorization SHA-256 is malformed")
        return args
    ...
    completion = _orchestrate_exact(
        lambda: _reserve_exact(exact_manifest, arguments.final_exact_authorization_sha256),
