"""Three-observation renderer-witness integration using the original warmup controller."""
import shutil
import time
import json
...
from scripts.renderer_witness_dual_camera_maze_session_development import RendererWitnessDualCameraMazeSession, ARTIFACT as RENDERER_WITNESSES
...
def collect(layout_index, definition, *, output, model, geometry, episode_name, condition, variant):
    ...
    try:
        session = RendererWitnessDualCameraMazeSession(spec, directory); session.install_contact_identity()
        ...
        controller = DualCameraSettledController(model, geometry, public_mission=mission,
        ...
        with writer(directory) as raw_append, (directory/'decision_stream_timing.jsonl').open('x') as stream_timing:
            ...
            try:
                ...
                for tick in range(3):
                    ...
                    try:
                        p, d, f, auxiliary, image, now = session.sensor_packets(); packet_ns = time.perf_counter_ns()
                        selected = controller.observe(p, d, f, now_ns=now, auxiliary_depth=auxiliary, auxiliary_rgb=image)
                        if selected['requested_command'] != [0., 0., 0.] or selected['terminal'] is not None:
                            raise ValueError('three unchanged zero-command warmup decisions required')
                    ...
                    if selected['terminal'] is not None:
                        ...
                        phase, role = 2, 'online_learned_round_trip_command'
                    session.phase = phase
                ...
                stop = str(error)
        if decisions == 3 and stop is None and acquisition_stop is None and terminal is None:
            acquisition_stop = 'DECLARED_THREE_OBSERVATION_PROBE_BOUNDARY'
        ...
        result = dict(status='RENDERER_WITNESS_MAZE_PROBE_TERMINAL_AUDIT_REQUIRED', layout_index=layout_index,
            ...
            navigation_ticks=NAVIGATION_TICKS, tracker_required_for_commands=True, additional_auxiliary_rgb_for_motion=True,
            renderer_witnesses_recorded=True, declared_probe_observations=3, navigation_experiment=False,
        ...
        print('RENDERER_WITNESS_MAZE_PROBE_COLLECTED', result, flush=True)
        return result
    finally:
...
def artifacts(layout_index, result):
    ...
    return names+[RENDERER_WITNESSES, 'public_mission.json', 'auxiliary_camera_audit.json', 'decision_stream_timing.jsonl']+[
