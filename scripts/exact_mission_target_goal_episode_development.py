...
def collect(trial, definition, *, output, model, geometry, persistent, episode_name, condition, variant):
    ...
    try:
        ...
        session._runtime['high_level'] = 'exact_mission_target_goal_probe_v1; online model plus measured surfaces'
        ...
        controller = ExactMissionTargetGoalProbe(model, geometry, persistent=persistent, condition=condition, variant=variant)
        try:
        ...
        result = dict(status='EXACT_MISSION_TARGET_GOAL_PROBE_TERMINAL_AUDIT_REQUIRED', trial=trial,
