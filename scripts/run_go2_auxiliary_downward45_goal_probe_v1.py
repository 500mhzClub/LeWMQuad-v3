...
from scripts.auxiliary_downward45_goal_episode_development import collect,artifacts
from scripts.auxiliary_downward45_goal_audit_development import audit
...
from scripts.replay_go2_auxiliary_downward45_controller_prefix_v1 import OUTPUT as INTEGRATION
...
PREVIOUS=BASE/'go2_auxiliary_depth_reobserve_goal_probe_v1_attempt_001'
DIAGNOSTIC=BASE/'go2_auxiliary_depth_reobserve_goal_readout_v1_attempt_001'
PREVIOUS_SHA='3675f09b90770ed9345154b8fe926c66fd6029e9c7bc7a6f2d53cba0c6861193'
DIAGNOSTIC_SHA='2fe50e68c57ebec738daebfc70f589752fda1dc7c43de2dfdc6f93a10efd9e8e'
OUTPUT=BASE/'go2_auxiliary_downward45_goal_probe_v1_attempt_001'
...
PROTOCOL='docs/go2_auxiliary_downward45_goal_probe_v1_2026-09-08.md'
...
def worker(case,launch_sha):
    ...
    terminal=dict(case=name,trial=trial,variant=variant,condition=condition,model_name=model_name,
        status='AUXILIARY_DOWNWARD45_GOAL_WORKER_FAILED',artifact_sha256={});started=time.perf_counter()
    with (OUTPUT/(name+'_worker.log')).open('x') as log,contextlib.redirect_stdout(log),contextlib.redirect_stderr(log):
        try:
            ...
            terminal.update(status='AUXILIARY_DOWNWARD45_GOAL_COLLECTED_AND_RAW_AUDITED',artifact_sha256=bindings,
...
def main():
    ...
    assert previous['status']=='AUXILIARY_DEPTH_REOBSERVE_GOAL_PROBE_COMPLETE' and previous['cases']==[list(c) for c in CASES]
    ...
    assert diagnostic['status']=='AUXILIARY_DEPTH_REOBSERVE_GOAL_READOUT_COMPLETE'
    ...
    assert integration['status']=='AUXILIARY_DOWNWARD45_CONTROLLER_PREFIX_COMPLETE' and len(integration['conditions'])==2
    ...
    launch=preflight(output=OUTPUT,protocol=PROTOCOL,
        seed_paths=('scripts/run_go2_auxiliary_downward45_goal_probe_v1.py',
            'lewm/tests/test_auxiliary_downward45_goal_scope_development.py',
            'docs/go2_auxiliary_downward45_controller_prefix_result_2026-09-08.md',
    ...
    launch.update(integration_artifact_sha256=integration_ids,sensor_prefix_artifact_sha256=sensor_ids,
        ...
        experiment='explicit 45-degree auxiliary camera and transform; same corrected planner, constraints and bounded active reobservation',
    ...
    print('AUXILIARY_DOWNWARD45_GOAL_LAUNCHED',digest(OUTPUT/'launch.json'),flush=True)
    ...
    try:
            ...
            with ProcessPoolExecutor(max_workers=2,mp_context=multiprocessing.get_context('spawn'),max_tasks_per_child=1) as pool:
                ...
                while pending:
                    ...
                    for future in done:
                        ...
                        except Exception as error:row=dict(case=case[0],status='AUXILIARY_DOWNWARD45_GOAL_PROCESS_FAILURE',failure=repr(error))
                        records.append(row);print('AUXILIARY_DOWNWARD45_GOAL_TERMINAL',case[0],row['status'],row.get('goal'),flush=True)
        if len(records)!=2 or any(r['status']!='AUXILIARY_DOWNWARD45_GOAL_COLLECTED_AND_RAW_AUDITED' for r in records):
        ...
        result=dict(status='AUXILIARY_DOWNWARD45_GOAL_PROBE_COMPLETE',cases=CASES,conditions=records,
        ...
        write_json(OUTPUT/'result.json',result);print('AUXILIARY_DOWNWARD45_GOAL_COMPLETE',digest(OUTPUT/'result.json'),flush=True)
    except Exception as error:
        write_json(OUTPUT/'failure.json',dict(status='TERMINAL_AUXILIARY_DOWNWARD45_GOAL_FAILURE',reason=repr(error),
