"""Full-recording test of local correspondence endpoint depth estimation."""
import argparse
import hashlib
import json
import os
...
from lewm.local_feature_depth_consensus_development import (
    LocalFeatureDepthRegistration, LocalFeatureDepthConsensusMotion)


FRAME_COUNTS = (3106, 3525, 3105, 3027)
...
def main():
    ...
    output = root/f'depth_noise_{args.sigma_mm}mm_local_feature_depth_full_journey_v1'
    output.mkdir()
    config = dict(layout_index=i, sigma_mm=args.sigma_mm, seed=SEED,
        frames=frame_count, tracker='LocalFeatureDepthConsensusMotion',
        ...
        floor_depth_source="local_inverse_depth_5x5", image_feature_depth_changed=True,
        image_correspondence_depth_source="local_inverse_depth_5x5", original_packets_preserved=True,
        feature_detection_depth_changed=False, intermediate_chain_depth_changed=False, floor_depth_filtered_twice=False,
        ...
        sensor_noise_calibrated=False, source_sha256={str(p):hashlib.sha256(p.read_bytes()).hexdigest()
            for p in map(Path, (__file__, 'lewm/local_feature_depth_consensus_development.py',
    ...
    reader = PublicReplay(root/'native'); motion = LocalFeatureDepthConsensusMotion()
    ...
    with (output/'frames.jsonl').open('x') as sink:
        for frame in range(frame_count):
            ...
            if frame % 250 == 0: print('FULL_LOCAL_FEATURE_DEPTH_FRAME', i, args.sigma_mm, frame, flush=True)
    # Compare against the completed short replay only after estimation ends.
    short_root = root/f"depth_noise_{args.sigma_mm}mm_local_feature_depth_tracking_601_v1"
    short_rows = [json.loads(line) for line in (short_root/"frames.jsonl").read_text().splitlines()]
    prefix_count = min(len(rows), len(short_rows))
    prefix_equal = all(rows[j] == short_rows[j] for j in range(prefix_count))
    ...
    result=dict(config=config, accepted_frames=len(rows), failure=failure,
        short_prefix_compared_frames=prefix_count, short_prefix_exactly_reproduced=prefix_equal,
        entire_short_prefix_completed=prefix_count==len(short_rows),
        full_recording_completed=len(rows)==frame_count,
