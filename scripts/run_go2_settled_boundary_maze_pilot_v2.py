...
from scripts.settled_target_reset_native_prefix_comparison_development import compare
from scripts.training_translation_bias_model_admission_development import load_assigned
from scripts.replay_go2_settled_boundary_controller_prefix_v2 import (
...
OUTPUT = BASE/'go2_settled_boundary_maze_pilot_v2_attempt_001'
PROTOCOL = 'docs/go2_settled_boundary_maze_pilot_v2_2026-09-09.md'
...
def admit_prefix(integration):
    if (integration['status']!='SETTLED_BOUNDARY_CONTROLLER_PREFIX_V2_COMPLETE'
    ...
    for key in ('complete_decisions_exact_outside_declared_mission_and_target_reset_fields',
            'actual_requested_commands_exact','stopped_before_later_decisions','model_state_unchanged',
            'final_target_reset_mode_difference','original_failed_attempt_preserved','controller_implementation_unchanged'):
...
def main():
    ...
    for name,h in readout['source_sha256'].items():
        ...
        inherited[name]=h
    sources=discover_sources((PROTOCOL,'scripts/run_go2_settled_boundary_maze_pilot_v2.py',
        'lewm/tests/test_settled_target_reset_native_scope_development.py',
        'lewm/tests/test_settled_target_reset_native_prefix_comparison_development.py',
        'lewm/tests/test_settled_target_reset_native_admission_development.py',
    ...
    launch.update(protocol=PROTOCOL,source_sha256=sources,output_root=str(OUTPUT),planned_case=CASE,
        ...
        model_map_contact_selector_memory_unchanged=True,measured_quiet_boundary_required_before_dwell=True,declared_target_reset_mode_effect=True,
    ...
    try:
        ...
        write_json(OUTPUT/'result.json',dict(status='SETTLED_BOUNDARY_MAZE_PILOT_V2_COMPLETE',conditions=[record],
