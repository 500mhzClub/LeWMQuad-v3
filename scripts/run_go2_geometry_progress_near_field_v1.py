...
from scripts.geometry_progress_near_field_session_development import GeometryProgressNearFieldSession
...
OUTPUT=BASE/'go2_geometry_progress_near_field_v1_attempt_001'
PROTOCOL='docs/go2_geometry_progress_near_field_v1_2026-09-07.md'
RESERVE=40*1024**3
...
def preflight():
    ...
    for n,h in visibility['source_sha256'].items():
        ...
        inherited[n]=h
    sources=discover_sources((PROTOCOL,'scripts/run_go2_geometry_progress_near_field_v1.py',
        'scripts/audit_go2_geometry_progress_near_field_v1.py',
        'lewm/tests/test_geometry_progress_near_field_development.py'),inherited)
    resources=hardware()
...
def collect(trial,definition):
    ...
    try:
        session=GeometryProgressNearFieldSession(spec,directory);session.install_contact_identity();build=session.ctx.build
        ...
        print('GEOMETRY_PROGRESS_NEAR_FIELD',trial,result,flush=True);return result
    finally:
...
def main():
    ...
    try:
        ...
        write_json(OUTPUT/'result.json',dict(status='GEOMETRY_PROGRESS_NEAR_FIELD_COLLECTION_TERMINAL',conditions=results,
