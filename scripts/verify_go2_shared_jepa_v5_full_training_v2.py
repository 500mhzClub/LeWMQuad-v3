#!/usr/bin/env python3
"""Independent reconstruction verifier for Shared JEPA V5 training V2.
...
if __name__ == "__main__":
    ...
    def _validate_traces(values: Mapping[str, Any]) -> None:
        loss_names = {
            ...
            "pair_v4_total",
            "current_v4_ordered_first_hit_nll",
        ...
        for arm in policy.ARMS:
            ...
            for index, row in enumerate(rows, start=1):
                if (
                    row.get("schema")
                    != "lewm_go2_shared_jepa_v5_full_training_v2_trace_row_v1"
                    or row.get("arm") != arm
        ...
        class IndependentReconstructor:
            ...
            def _checkpoint(self, arm: str, update: int) -> tuple[Any, Mapping[str, Any]]:
                ...
                if (
                    ...
                    != "lewm_go2_shared_jepa_v5_full_training_v2_checkpoint_v1"
                    ...
                    for start in range(0, len(frames), policy.MICROBATCH_SIZE):
                        ...
                        for source, accumulator in ((target, correct), ([frames[wrong_map[index]] for index in indices], wrong)):
                            ...
                            if accumulator is correct:
                                ...
                                complete_loss += float(model.observable_camera_ray_v4_loss(fake_pair, supervision, supervision).total.cpu()) * len(indices)
            ...
            def _independent_artifact_bindings(self) -> dict[str, Any]:
                ...
                for relative in policy.EXACT_INVENTORY[:-1]:
                    ...
                    elif relative == "qualified_checkpoint.pt":
                        payload = torch.load(
                            io.BytesIO(raw),
                            map_location="cpu",
                            weights_only=False,
            ...
            def reconstruct(self) -> dict[str, Any]:
                ...
                qualified = torch.load(io.BytesIO(self.artifact_raw["qualified_checkpoint.pt"]), map_location="cpu", weights_only=False)
                deployment = promoted_model.deployment_state_dict()
                deployment_sha = model_module.tensor_state_dict_sha256(deployment)
                qualified_state_sha = model_module.tensor_state_dict_sha256(
                    qualified["deployment_state_dict"]
                )
                qualified_core = {
                    name: value
                ...
                if (
                    ...
                    or qualified_core != expected_qualified_core
                    or qualified.get("content_sha256")
                    != policy.canonical_json_sha256(qualified_core)
                    or qualified_state_sha != deployment_sha
                ):
                    raise PermissionError("independent qualified deployment filtering differs")
                ...
                return {
                    ...
                    "qualified_state_sha256": deployment_sha,
                    "schedule_content_sha256": schedule["content_sha256"],
    ...
    def verify(
        ...
        core = {
            "schema": "lewm_go2_shared_jepa_v5_full_training_v2_verification_v1",
