...
from scripts.run_go2_continuous_connector_goal_probe_v1 import OUTPUT as INPUT, CASES
...
from scripts.run_go2_commitment_pose_goal_probe_v1 import OUTPUT as ORIGINAL

OUTPUT=BASE/'go2_continuous_connector_goal_readout_v1_attempt_001'
PROTOCOL='docs/go2_continuous_connector_goal_readout_v1_2026-09-08.md'
ORIGINAL_SHA='485c7d2081cacc2a5176d72bc4d52a34fd795ea0f4c4bc8a73cb0a29091afd6e'
...
def main():
    ...
    assert result['status']=='CONTINUOUS_CONNECTOR_GOAL_PROBE_COMPLETE' and result['cases']==[list(c) for c in CASES]
    ...
    assert old['status']=='COMMITMENT_POSE_GOAL_PROBE_COMPLETE'
    ...
    sources=discover_sources((PROTOCOL,'scripts/read_go2_continuous_connector_goal_probe_v1.py'),original['source_sha256'])
    ...
    try:
        ...
        write_json(OUTPUT/'result.json',dict(status='CONTINUOUS_CONNECTOR_GOAL_READOUT_COMPLETE',conditions=reports,models=grouped,
        ...
        print('CONTINUOUS_CONNECTOR_GOAL_READOUT_COMPLETE',digest(OUTPUT/'result.json'),flush=True)
    except Exception as error:
        write_json(OUTPUT/'failure.json',dict(status='TERMINAL_CONTINUOUS_CONNECTOR_GOAL_READOUT_FAILURE',reason=repr(error)));raise
