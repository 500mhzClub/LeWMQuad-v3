"""Distinct fixed training-context construction; inherited gait remains frozen."""
import copy
import numpy as np
from lewm.independent_pulse_context_development import specification,pack
...
class PulseContextPhysicalInit(_GenesisPhysicalSession):
    def __init__(self,spec,*,backend):
        ...
        if self.backend!='cpu' or spec!=specification(spec['trial']):raise ExperimentError('exact CPU training-context experiment required')
