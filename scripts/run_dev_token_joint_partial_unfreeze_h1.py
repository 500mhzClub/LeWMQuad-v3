#!/usr/bin/env python3
"""DEVELOPMENT-ONLY staged partial-unfreeze joint run (h=1).
...
OUT = ROOT / ".generated/dev/DEVELOPMENT_ONLY_token_joint_partial_unfreeze_h1"
CKPT = ROOT / ".generated/dev/DEVELOPMENT_ONLY_token_predictor_h1/predictor_only_designated_role_epoch40.pt"
...
def main() -> int:
    ...
    record = {
        ...
        "bev_branch": {"parameters_frozen": True, "forward_active": True,
    ...
    for epoch in range(EPOCHS):
        ...
        for s in range(0, len(order), BATCH):
            ...
            # 1) online forward (grad flows in the partial arm)
            cur_tok = enc.forward_tokens(cur_img)[:, 1:, :]
            # 2) EMA target forward (never grads)
            with torch.no_grad():
                tgt_tok = target_enc.forward_tokens(nxt_img)[:, 1:, :]

            pred = predictor(cur_tok, oh_all[sel].to(device), cmd_all[sel].to(device))
            jepa = F.mse_loss(pred, tgt_tok)

            # BEV auxiliary: frozen params, live differentiable forward
