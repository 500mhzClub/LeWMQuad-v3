...
def collect(trial, definition, *, output, model, geometry, persistent, episode_name, condition, variant):
    ...
    try:
        ...
        session._runtime['high_level'] = 'observed_floor_contact_goal_probe_v1; online model plus measured surfaces'
        ...
        controller = ObservedFloorContactGoalProbe(model, geometry, persistent=persistent, condition=condition, variant=variant)
        try:
        ...
        result = dict(status='OBSERVED_FLOOR_CONTACT_GOAL_PROBE_TERMINAL_AUDIT_REQUIRED', trial=trial,
