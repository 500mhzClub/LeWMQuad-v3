...
import cv2
import torch
from lewm.causal_residual_final_goal_development import CausalResidualFinalGoalProbe,correct_final_goal_score
...
from scripts.run_go2_executed_horizon_final_goal_probe_v1 import OUTPUT as INPUT,CASES,CORRECTION,FITS
...
OUTPUT=BASE/'go2_causal_residual_final_goal_prefix_v1_attempt_001'
PROTOCOL='docs/go2_causal_residual_final_goal_prefix_v1_2026-09-08.md'
...
DIAGNOSTIC=BASE/'go2_causal_executed_residual_diagnosis_integrity_v1_attempt_001'
DIAGNOSTIC_SHA='c07af36277147654ee0c9b651ab46c280833b128d4b657adee1225bfa2cf4dae'


def replay(admission,case,diagnostic):
    ...
    controller=CausalResidualFinalGoalProbe(model,ArticulatedCollisionGeometry(URDF),condition=c,variant=v,persistent=True)
    ...
    reference=next(r for r in diagnostic['conditions'] if r['input_root']==str(INPUT) and r['case']==name)
    expected={r['tick']:r['causal_diagnostic'] for r in reference['records']}
    records=[];first_terminal=None;first_command=None;checked_corrections=[]
    for i in range(len(reader.frames)):
        ...
        receipt=r['causal_residual_receipt']
        if i in expected:
            for key in ('correction_xy_m','residual_source_ticks','residual_available_ticks','observed_residual_samples'):
                assert receipt[key]==expected[i][key],('exact prequential observed residual',i,key)
            checked_corrections.append(i)
        if a is not None:
            before_request=receipt|dict(pending_forecast_tick=None)
            assert b==correct_final_goal_score(a,before_request),('exact causal final-goal score transformation',i)
    ...
    return dict(decisions=records,model_state_sha256=before,first_terminal_policy_difference=first_terminal,
        ...
        causal_observations_maps_forecasts_and_nominal_checks_exact=True,surface_checks_exact=True,only_final_goal_scoring_pose_corrected=True,
        exact_prequential_residual_comparison_frames=checked_corrections,
...
def main():
    ...
    bound=[]
    for root,sha,status in ((INPUT,INPUT_SHA,'EXECUTED_HORIZON_FINAL_GOAL_PROBE_COMPLETE'),
            (READOUT,READOUT_SHA,'EXECUTED_HORIZON_FINAL_GOAL_READOUT_COMPLETE'),
            (DIAGNOSTIC,DIAGNOSTIC_SHA,'CAUSAL_EXECUTED_RESIDUAL_INTEGRITY_COMPLETE')):
    ...
    sources=discover_sources((PROTOCOL,'scripts/replay_go2_causal_residual_final_goal_prefix_v1.py',
        'lewm/tests/test_causal_residual_final_goal_development.py',
        'lewm/tests/test_online_executed_residual_development.py',
        'docs/go2_causal_executed_residual_result_2026-09-08.md'),bound[-1][2]['source_sha256'])
    resources=hardware()
    ...
    print('CAUSAL_RESIDUAL_FINAL_GOAL_PREFIX_LAUNCHED',digest(OUTPUT/'launch.json'),flush=True)
    try:
        diagnostic=read_json(DIAGNOSTIC,'motion.json')
        reports=[];names=['launch.json']
        for case in CASES:
            first=replay(admission,case,diagnostic);second=replay(admission,case,diagnostic);assert first==second,'exact fresh replay required'
            ...
            report={k:v for k,v in first.items() if k!='decisions'}|dict(case=case[0],model=case[4],
                ...
                changed_score_ticks=[i for i,r in enumerate(rows)
                    if (r['new_selection'] or {}).get('causal_final_goal_scoring')],
                controller_failures=[dict(frame=i,failure=r['failure']) for i,r in enumerate(rows) if r['failure']])
            reports.append(report);print('CAUSAL_RESIDUAL_FINAL_GOAL_MODEL',report,flush=True)
        verify(launch)
        ...
        write_json(OUTPUT/'result.json',dict(status='CAUSAL_RESIDUAL_FINAL_GOAL_PREFIX_COMPLETE',source_sha256=sources,
        ...
        print('CAUSAL_RESIDUAL_FINAL_GOAL_PREFIX_COMPLETE',digest(OUTPUT/'result.json'),flush=True)
    except Exception as error:
        write_json(OUTPUT/'failure.json',dict(status='TERMINAL_CAUSAL_RESIDUAL_FINAL_GOAL_PREFIX_FAILURE',reason=repr(error)));raise
