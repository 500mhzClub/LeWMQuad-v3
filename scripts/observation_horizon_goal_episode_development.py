...
def collect(trial, definition, *, output, model, geometry, persistent, episode_name, condition, variant):
    ...
    try:
        ...
        session._runtime['high_level'] = 'observation_horizon_goal_probe_v1; online model plus measured surfaces'
        ...
        controller = ObservationHorizonGoalProbe(model, geometry, persistent=persistent, condition=condition, variant=variant)
        try:
        ...
        result = dict(status='OBSERVATION_HORIZON_GOAL_PROBE_TERMINAL_AUDIT_REQUIRED', trial=trial,
