"""Training-only, balanced native starts; RGB and physical evidence retained."""
import argparse
import json
import os
...
OUTPUT = Path('/home/andrewknowles/Workspace/LeWMQuad-v3/.generated/navigation_development_artifacts_v1/go2_balanced_start_actions_v1_attempt_001')
PLAN = Path('docs/go2_balanced_start_actions_plan_2026-09-17.json')
RESULT = Path('docs/go2_balanced_start_actions_result_2026-09-17.json')
...
def prepare():
    ...
    assert free > (512 + 220) * 1024**2, free
    plan = dict(cases=CASES, specifications=[specification(t) for t, _ in CASES],
                ...
                context_ticks=[0, 5, 10], future_tick=15, quiet_prefix_ticks=10,
                command_suffix_ticks=5, actions_per_environment=6, environments=8,
                ...
                resources=dict(free_bytes=free, reserve_bytes=512*1024**2,
                               expected_collection_bytes=220*1024**2, workers=2,
    ...
    print('BALANCED_START_COLLECTION_PREPARED', len(CASES), flush=True)
...
def run(case):
    ...
    try:
        ...
        try:
            ...
            tape=[[0.,0.,0.]]*10+[candidate_commands(action)[0]]*5
        ...
        result=dict(status='COMPLETE',case=case,trial=trial,action=action,data_role='train',
                    ...
                    complete_500ms=stop is None and len(session.image_audit)==16,
    ...
    print('BALANCED_START_CASE_COMPLETE',json.dumps(result),flush=True)
...
def summarize():
    ...
    for i,(trial,action) in enumerate(CASES):
        ...
        if r['complete_500ms'] and not r['disallowed_contact']:
    ...
    result=dict(status='COMPLETE',cases=rows,training_only=True,
                eligible=sum(r['complete_500ms'] and not r['disallowed_contact'] for r in rows),
    ...
    print('BALANCED_START_COLLECTION_COMPLETE',result['eligible'],result['contacts'],flush=True)
