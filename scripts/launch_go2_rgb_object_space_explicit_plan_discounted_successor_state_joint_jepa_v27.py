#!/usr/bin/env python3
"""Denied-by-default one-shot launcher for the preregistered V27 probe.
...
then hands the reserved attempt to the V27 executor.  Import and the no-arg
path do not import the V27 scientific modules or open runtime payloads.
"""
...
PRIVATE_BASE_MODULE_NAME = "_lewm_v27_explicit_plan_private_v25_launcher"
...
REQUIRED_CERTIFIED_SOURCE_PATHS = frozenset(
    {
        "scripts/launch_go2_rgb_object_space_explicit_plan_discounted_"
        "successor_state_joint_jepa_v27.py",
...
def _canonical_json_bytes_v27(value: Any) -> bytes:
    return json.dumps(
        value,
        sort_keys=True,
        separators=(",", ":"),
...
def _strict_json_object_v27(raw: bytes, *, name: str) -> dict[str, Any]:
...
def _validate_content_bound_v27(
    ...
    if (
        ...
        or len(observed) != 64
        or hashlib.sha256(_canonical_json_bytes_v27(core)).hexdigest() != observed
    ):
        raise PermissionError(f"{name} content binding changed")
    return dict(value)


def _sha256_canonical_v27(value: Any) -> str:
    return hashlib.sha256(_canonical_json_bytes_v27(value)).hexdigest()


def _is_lower_sha256_v27(value: Any) -> bool:
    return (
        type(value) is str
        and len(value) == 64
...
def _is_commit_v27(value: Any) -> bool:
    return (
        type(value) is str
        and len(value) == 40
        and all(character in "0123456789abcdef" for character in value)
...
def _assert_runtime_adapter_v27() -> None:
    if any(
        getattr(_BASE_LAUNCHER, name, None) != value
        for name, value in _BASE_OVERRIDES.items()
    ):
        raise PermissionError("V27 inherited runtime selectors changed after import")
    if (
        _BASE_LAUNCHER._build_one_microbatch_v13
        is not _V25_LAUNCHER._build_one_microbatch_v25
    ):
        raise PermissionError("V27 physical microbatch builder is not frozen V25")
    if (
        _BASE_LAUNCHER._validate_certified_source_binding_v13
        is not _validate_certified_source_binding_for_base_v27
    ):
        raise PermissionError("V27 terminal source validator changed after import")


def _load_authority_file_v27(path: Path) -> dict[str, Any]:
    candidate = Path(path)
    ...
    try:
        ...
        raise PermissionError("fixed V27 execution authority is absent") from error
    if (
        ...
        raise PermissionError("V27 authority must be its fixed regular file")
    raw = candidate.read_bytes()
    value = _strict_json_object_v27(raw, name="V27 execution authority")
    if raw != _canonical_json_bytes_v27(value) + b"\n":
        raise PermissionError("V27 authority must be canonical JSON")
    return value


def _validate_binding_shape_v27(value: Any, *, name: str) -> dict[str, Any]:
    if type(value) is not dict or set(value) not in (
        {"path", "file_sha256", "byte_count"},
        {"path", "file_sha256", "byte_count", "content_sha256"},
    ):
        raise PermissionError(f"V27 {name} binding shape changed")
    if (
        type(value.get("path")) is not str
        or not value["path"]
        or not _is_lower_sha256_v27(value.get("file_sha256"))
        or type(value.get("byte_count")) is not int
        or value["byte_count"] <= 0
        or (
            "content_sha256" in value
            and not _is_lower_sha256_v27(value["content_sha256"])
        )
    ):
        raise PermissionError(f"V27 {name} binding is malformed")
    return dict(value)


def validate_authority_v27(value: Any) -> dict[str, Any]:
    authority = _validate_content_bound_v27(
        value, name="V27 execution authority", exact_keys=AUTHORITY_KEYS
    )
    fixed = {
    ...
    if any(authority.get(name) != expected for name, expected in fixed.items()):
        raise PermissionError("V27 authority identity or scientific cap changed")
    if not _is_commit_v27(authority.get("pinned_source_and_review_commit")):
        raise PermissionError("V27 frozen source-and-review commit is malformed")
    if not _is_commit_v27(authority.get("implementation_commit")):
        raise PermissionError("V27 implementation commit is malformed")
    selectors = {
    ...
    if authority.get("selectors") != selectors:
        raise PermissionError("V27 authority runtime selectors changed")
    if (
        _sha256_canonical_v27(authority.get("hardware")) != HARDWARE_SHA256
        or _sha256_canonical_v27(authority.get("runtime")) != RUNTIME_SHA256
        or _sha256_canonical_v27(authority.get("authorized_roles"))
        != AUTHORIZED_ROLES_SHA256
    ):
        raise PermissionError("V27 inherited physical runtime identity changed")
    ...
    if type(runtime_inputs) is not dict or set(runtime_inputs) != expected_input_names:
        raise PermissionError("V27 runtime input inventory changed")
    for name, binding in runtime_inputs.items():
        _validate_binding_shape_v27(binding, name=name)
    physical = {name: runtime_inputs[name] for name in PHYSICAL_RUNTIME_INPUT_NAMES}
    if _sha256_canonical_v27(physical) != PHYSICAL_RUNTIME_INPUTS_SHA256:
        raise PermissionError("V27 inherited physical runtime bindings changed")
    if (
        runtime_inputs["h6_train_index"] != H6_TRAIN_BINDING
        or runtime_inputs["h6_validation_index"] != H6_VALIDATION_BINDING
    ):
        raise PermissionError("V27 corrected H6 V2 bindings changed")
    ...
    if type(certification) is not dict or set(certification) != {
        ...
        raise PermissionError("V27 certification authority binding changed")
    _validate_binding_shape_v27(certification, name="certification")
    if certification["path"] != CLEAN_EXPORT_CERTIFICATION_RELATIVE_PATH:
        raise PermissionError("V27 certification path changed")
    return authority


def _validate_certified_path_v27(
    source_root: Path, binding: Any
) -> tuple[str, bytes]:
    value = _validate_binding_shape_v27(binding, name="certified source")
    if set(value) != {"path", "file_sha256", "byte_count"}:
        raise PermissionError("V27 certified source binding gained metadata")
    relative = value["path"]
    ...
    if (
        ...
        raise PermissionError("V27 certification contains a protected path")
    path = source_root.joinpath(*pure.parts)
    try:
        resolved = path.resolve(strict=True)
    except (FileNotFoundError, OSError) as error:
        raise PermissionError("V27 certified source is absent") from error
    if (
        resolved != path.absolute()
        ...
        raise PermissionError("V27 certified source escaped or is not regular")
    raw = path.read_bytes()
    if (
...
_BASE_LAUNCHER._validate_certified_source_binding_v13 = (
    _validate_certified_source_binding_for_base_v27
)


def validate_source_certification_v27(
    ...
    raw = path.read_bytes()
    certification = _strict_json_object_v27(
        raw, name="V27 clean-export certification"
    )
    if raw != _canonical_json_bytes_v27(certification) + b"\n":
        raise PermissionError("V27 certification must be canonical JSON")
    certification = _validate_content_bound_v27(
        certification,
        name="V27 clean-export certification",
    ...
    if (
        ...
        raise PermissionError("V27 authority does not bind this certification")
    if (
    ...
    if _sha256_canonical_v27(bindings) != certification["bindings_sha256"]:
        raise PermissionError("V27 certified source inventory binding changed")
    paths: list[str] = []
    for binding in bindings:
        source_path, _ = _validate_certified_path_v27(root, binding)
        paths.append(source_path)
    if paths != sorted(paths) or len(paths) != len(set(paths)):
        raise PermissionError("V27 certified source inventory order changed")
    if not REQUIRED_CERTIFIED_SOURCE_PATHS.issubset(paths):
        raise PermissionError("V27 certification omits a required runtime source")
    return {
...
def _terminal_exists_v27(output_root: Path) -> bool:
    return any(
...
def execute_future_authorized_v27(
    *, repository_root: Path, authority: Mapping[str, Any]
) -> Mapping[str, Any]:
    """Validate, reserve exactly once, compose, execute, and close."""

    _assert_runtime_adapter_v27()
    repository = Path(repository_root).resolve(strict=True)
    if repository != ROOT.resolve(strict=True):
        raise PermissionError("V27 execution must run from its certified export")
    fixed_authority = _load_authority_file_v27(ROOT / AUTHORITY_RELATIVE_PATH)
    ...
    source_evidence = validate_source_certification_v27(
        repository, validated_authority
    )
    executor = importlib.import_module(EXECUTOR_MODULE_NAME)
    executor.validate_future_execution_prerequisites_v27(validated_authority)

    def validate_bound_sources_for_runtime_v27(
        source_root: Path,
    ) -> dict[str, Any]:
        return validate_source_certification_v27(
            source_root, validated_authority
        )

    # V13's terminal rehash uses this compatibility name.  Keep the strong
    # V27 protected-path filter in front of every listed file at both ends of
    # the run rather than delegating to the executor's weaker generic reader.
    executor.validate_bound_sources_v13 = validate_bound_sources_for_runtime_v27
    ...
    try:
        ...
        stage = "future_authorized_engine_v27"
        result = executor.run_future_authorized_engine_v27(
            authority=validated_authority,
            reservation=reservation,
            runtime=runtime,
            publisher=publisher,
        ...
        if not isinstance(result, Mapping):
            raise RuntimeError("V27 controller omitted its terminal receipt")
        return dict(result)
    except BaseException as error:
        if not _terminal_exists_v27(output_root):
...
def private_launcher_adapter_receipt_v27() -> dict[str, Any]:
    _assert_runtime_adapter_v27()
    return {
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    authority = _load_authority_file_v27(parsed.future_authority)
    result = execute_future_authorized_v27(
    ...
    if isinstance(status, str) and status.startswith("FAIL_"):
        return 2
    raise RuntimeError("V27 controller returned a nonterminal status")
