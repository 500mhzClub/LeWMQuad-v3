#!/usr/bin/env python3
"""Exact trainer for the single V4 N5 gate-aligned raster-NLL V12 attempt.
...
def compute_gate_aligned_v12_losses(
    model: Any,
    batch: Any,
...
def train_v12_fit(
    *,
    ...
    """Run the frozen 4,000-update full-panel, final-only V12 schedule."""
    ...
    if (
        ...
        raise PermissionError("V12 frozen optimizer or schedule request changed")
    if not math.isfinite(float(learning_rate)) or not math.isfinite(
        float(weight_decay)
    ):
        raise ValueError("V12 optimizer values must be finite")
    ...
    if schedule_sha256 != policy.EXPECTED_SCHEDULE_SHA256:
        raise ValueError("V12 deterministic schedule hash changed")
    if any(len(indices) != 5 or set(indices) != set(range(5)) for indices in schedule):
        raise ValueError("V12 schedule is not one full panel per update")
    ...
    for step_index, indices in enumerate(schedule, start=1):
        ...
        total, _v11_base_total, components, _raw, _targets, _raster = (
            compute_gate_aligned_v12_losses(model, batch)
        )
        if not bool(torch.isfinite(total).item()):
            raise FloatingPointError("V12 fit loss became non-finite")
        total.backward()
        gradient_norm = torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0)
        if not bool(torch.isfinite(gradient_norm).item()):
            raise FloatingPointError("V12 fit gradient norm became non-finite")
        optimizer.step()
    ...
    if [row["step"] for row in trace] != diagnostic_updates:
        raise RuntimeError("V12 diagnostic schedule changed during training")
    return {
        "steps": 4000,
        "batch_size": 5,
        "learning_rate": 1e-4,
...
def evaluate_gate_aligned_v12(
    *,
    ...
    if not torch.isclose(
        ...
        raise ValueError("V12 trainer raster NLL and metric accumulator disagree")
    return {
        "control": (
...
def build_checkpoint_metadata_v9(
    *,
    ...
    """Construct the frozen final-update V12 checkpoint metadata without I/O."""

    if training_schedule_sha256 != policy.EXPECTED_SCHEDULE_SHA256:
        raise ValueError("V12 checkpoint schedule binding changed")
    return {
        "experiment": policy.experiment_contract(),
        "authority_bindings": policy.authority_bindings(),
        ...
        "loss_contract": {
            "version": "gate_aligned_raster_nll_v12",
    ...
    *,
    ...
    try:
        ...
        with warnings.catch_warnings(record=True) as caught:
            ...
            matched = evaluate_gate_aligned_v12(
                model=model,
                frames=inputs.frames,
                images=images,
                device=device,
            ...
            wrong = evaluate_gate_aligned_v12(
                model=model,
                frames=inputs.frames,
                images=images,
                device=device,
