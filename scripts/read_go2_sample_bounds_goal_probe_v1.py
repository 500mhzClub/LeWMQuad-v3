...
from scripts.run_go2_sample_bounds_goal_probe_v1 import OUTPUT as INPUT, CASES
...
from scripts.run_go2_nominal_action_goal_probe_v1 import OUTPUT as ORIGINAL

OUTPUT=BASE/'go2_sample_bounds_goal_readout_v1_attempt_001'
PROTOCOL='docs/go2_sample_bounds_goal_readout_v1_2026-09-08.md'
ORIGINAL_SHA='f9708a794b678bec4c61c5aba13dc3a3886ec836de7127f5440178cd5737288b'
...
def main():
    ...
    assert result['status']=='SAMPLE_BOUNDS_GOAL_PROBE_COMPLETE' and result['cases']==[list(c) for c in CASES]
    ...
    assert old['status']=='NOMINAL_ACTION_GOAL_PROBE_COMPLETE'
    ...
    sources=discover_sources((PROTOCOL,'scripts/read_go2_sample_bounds_goal_probe_v1.py'),original['source_sha256'])
    ...
    try:
        ...
        write_json(OUTPUT/'result.json',dict(status='SAMPLE_BOUNDS_GOAL_READOUT_COMPLETE',conditions=reports,models=grouped,
        ...
        print('SAMPLE_BOUNDS_GOAL_READOUT_COMPLETE',digest(OUTPUT/'result.json'),flush=True)
    except Exception as error:
        write_json(OUTPUT/'failure.json',dict(status='TERMINAL_SAMPLE_BOUNDS_GOAL_READOUT_FAILURE',reason=repr(error)));raise
