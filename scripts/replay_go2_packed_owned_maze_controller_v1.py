"""Exact full ninth controller replay with independent packed voxel bounds."""
import argparse
import json
import time
import cv2
...
BENCHMARK=BASE/'go2_packed_owned_maze_bounds_benchmark_v1_attempt_001'
OUTPUT=BASE/'go2_packed_owned_maze_controller_replay_v1_attempt_001'
PROTOCOL='docs/go2_packed_owned_maze_controller_replay_v1_2026-09-09.md'
NATIVE_RESULT='3745c0b7c45265a2fcc38caf732b6f3f4b228487f344d23502dc7395077d5755'
BENCHMARK_RESULT='e3f603f7baed193e23b53116fe14f46a94bc6a98ad4b5eebca0af604dbe9a872'
...
def main():
    ...
    assert benchmark['status']=='PACKED_OWNED_MAZE_BOUNDS_BENCHMARK_COMPLETE'
    bids={'result.json':BENCHMARK_RESULT,**benchmark['artifact_sha256']}
    old=read_json(INPUT,'launch.json')
    sources=discover_sources((PROTOCOL,'scripts/replay_go2_packed_owned_maze_controller_v1.py',
        'lewm/tests/test_packed_owned_later_floor_controller_development.py'),benchmark['source_sha256'])
    resources=hardware()
    launch=old|dict(protocol=PROTOCOL,output_root=str(OUTPUT),source_sha256=sources,native_input_sha256=ids,
        ...
        implementation_class='PackedOwnedLaterFloorController',original_decision_labels_preserved=True,
        ...
        concurrency_reason='one exact-performance replay beside the independent settled-boundary work')
    verify_all(launch)
    memory_ok=resources['memory_available_bytes']>=8*1024**3;storage_ok=resources['artifact_free_bytes']>=RESERVE_BYTES+1024**3
    if args.preflight_only:
        print('PACKED_OWNED_CONTROLLER_PREFLIGHT',json.dumps(dict(source_count=len(sources),hardware=resources,
    ...
    print('PACKED_OWNED_CONTROLLER_LAUNCHED',digest(OUTPUT/'launch.json'),flush=True);start=time.perf_counter()
    try:
        ...
        controller=PackedOwnedLaterFloorController(model,ArticulatedCollisionGeometry(URDF),
        ...
        with writer(OUTPUT) as append:
            for i,saved in enumerate(read_rows(INPUT/name)):
                ...
                if i%100==0:print('PACKED_OWNED_CONTROLLER_FRAME',i,flush=True)
        ...
        verify_all(launch)
        write_json(OUTPUT/'result.json',dict(status='PACKED_OWNED_MAZE_CONTROLLER_REPLAY_COMPLETE',
        ...
        print('PACKED_OWNED_CONTROLLER_COMPLETE',digest(OUTPUT/'result.json'),flush=True)
