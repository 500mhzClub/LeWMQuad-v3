"""Fresh maze3 physics for the exact completed reached-frontier replay."""
import argparse
import contextlib
...
from scripts.reached_frontier_native_prefix_development import compare, boundary
from scripts.reached_frontier_maze03_episode_development import collect, artifacts
from scripts.reached_frontier_maze03_audit_development import audit
...
OUTPUT = BASE/'go2_reached_frontier_maze03_pilot_v1_attempt_001'
SOURCE = 'scripts/run_go2_reached_frontier_maze03_pilot_v1.py'
PROTOCOL = 'docs/go2_reached_frontier_maze03_pilot_v1_2026-09-10.md'
TESTS = ('lewm/tests/test_reached_frontier_native_launcher_development.py',
    'lewm/tests/test_reached_frontier_native_prefix_development.py',
    'lewm/tests/test_reached_frontier_native_calculations_development.py')
CASE = ('full_jepa_reached_frontier_maze_03', *inputs.replay.original.CASE[1:])
WORKER_STATUS = 'REACHED_FRONTIER_MAZE03_COLLECTED_AND_RAW_AUDITED'
...
def verify_inputs(launch, *, full=False):
    ...
    expected = dict(planned_case=list(CASE), output_root=str(OUTPUT),
        implementation_class='ReachedFrontierRecentQualifiedController',
        ...
        reached_frontier_transition_enabled=True, planner_interface_adapter_enabled=False,
    ...
    if any(launch[k] != v or type(launch[k]) is not type(v) for k, v in expected.items()):
        raise ValueError('exact prospective frontier native definition required')
    admission = launch['input_admission']; boundary(admission['prefix_report'])
    inputs.verify_bound(admission, launch['source_sha256'])
    if full and inputs.admit(admission['prefix_result_sha256'], admission['adapter_batch_result_sha256'],
            launch['source_sha256']) != admission:
        raise ValueError('complete original frontier input admission changed')


def assigned_model(launch):
    model, condition, variant = load_assigned(launch['input_admission']['correction_admission'], CASE[4])
    if (condition, variant) != (CASE[3], CASE[2]) or state_digest(model.state_dict()) != inputs.replay.MODEL_SHA:
        raise ValueError('original assigned frontier model state required')
    return model
...
def require_worker(record, report, prefix_report):
    if (record['status'] != WORKER_STATUS or 'failure' in record or record['case'] != CASE[0]
            ...
            or report['reached_frontier_transition_enabled'] is not True
            or record['collection']['reached_frontier_transition_enabled'] is not True):
        raise ValueError('complete exact frontier collection and raw audit required')
    ...
    for key in ('physical_and_public_prefix_exact', 'all_preintervention_requested_commands_exact',
        ...
        if receipt[key] is not True: raise ValueError('complete physical frontier intervention required: '+key)
...
def worker(launch_sha):
    ...
    record = dict(case=name, layout_index=index, variant=variant, condition=condition, model_name=model_name,
        status='REACHED_FRONTIER_MAZE03_WORKER_FAILED', artifact_sha256={}); started = time.perf_counter()
    with (OUTPUT/(name+'_worker.log')).open('x') as log, contextlib.redirect_stdout(log), contextlib.redirect_stderr(log):
        try:
            ...
            if state_digest(model.state_dict()) != before: raise ValueError('collection changed assigned frontier model')
            ...
            record.update(status=WORKER_STATUS, artifact_sha256=ids, prefix_comparison=receipt, readout=readout,
                model_state_sha256=before, model_state_unchanged=True, reached_frontier_transition_enabled=True,
            ...
            import traceback
            traceback.print_exc(); record.update(status='REACHED_FRONTIER_MAZE03_WORKER_FAILED', failure=repr(error))
...
def main():
    ...
    parser.add_argument('--prefix-result-sha256'); parser.add_argument('--adapter-batch-result-sha256')
    ...
    if OUTPUT.exists() or OUTPUT.is_symlink(): raise ValueError('exclusive frontier native attempt; no retry/resume')
    ...
    if args.source_preflight_only:
        print('REACHED_FRONTIER_NATIVE_SOURCE_PREFLIGHT', json.dumps(dict(source_count=len(sources), hardware=resources,
            native_execution=False, output_created=False, complete_input_admission_performed=False)), flush=True); return
    if not args.prefix_result_sha256 or not args.adapter_batch_result_sha256:
        raise ValueError('exact completed frontier prefix and adapter batch identities required')
    print('REACHED_FRONTIER_NATIVE_INPUT_ADMISSION_STARTED', len(sources), flush=True)
    admission = inputs.admit(args.prefix_result_sha256, args.adapter_batch_result_sha256, sources)
    ...
    launch.update(source_sha256=sources, protocol=PROTOCOL, output_root=str(OUTPUT), input_admission=admission,
        planned_case=list(CASE), implementation_class='ReachedFrontierRecentQualifiedController',
        scene_specification=specification(3), public_mission=public_mission(3), robot_urdf_sha256=digest(URDF),
    ...
    if args.preflight_only:
        print('REACHED_FRONTIER_NATIVE_PREFLIGHT', json.dumps(dict(source_count=len(sources), hardware=resources,
    ...
    print('REACHED_FRONTIER_NATIVE_LAUNCHED', launch_sha, flush=True); started = time.perf_counter()
    try:
        ...
        print('REACHED_FRONTIER_NATIVE_TERMINAL', record['status'], record.get('verified_round_trip'), record.get('failure'), flush=True)
        if record['status'] != WORKER_STATUS:
            raise ValueError('frontier collection/audit/prefix incomplete: '+str(record.get('failure')))
        ...
        write_json(OUTPUT/'result.json', dict(status='REACHED_FRONTIER_MAZE03_PILOT_V1_COMPLETE',
            source_sha256=sources, artifact_sha256=ids, conditions=[record], wall_s=time.perf_counter()-started,
            prospective_prefix_result_sha256=args.prefix_result_sha256, adapter_batch_result_sha256=args.adapter_batch_result_sha256,
            measured_round_trip_successes=int(record['verified_round_trip']), reused_layout_executions=1,
            new_independent_layout_executions=0, reached_frontier_transition_enabled=True, model_training=False,
            navigation_qualified=False, real_time_qualified=False, hardware_qualified=False, goal_achieved=False))
        print('REACHED_FRONTIER_NATIVE_COMPLETE', digest(OUTPUT/'result.json'), flush=True)
    except Exception as error:
        write_json(OUTPUT/'failure.json', dict(status='TERMINAL_REACHED_FRONTIER_NATIVE_FAILURE', reason=repr(error), automatic_retry=False)); raise
