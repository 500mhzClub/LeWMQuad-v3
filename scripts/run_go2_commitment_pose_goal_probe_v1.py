"""Four fresh native cases with commitment-horizon waypoint pose utility."""
import argparse
import contextlib
import json
...
from scripts.commitment_pose_goal_episode_development import collect, artifacts
from scripts.commitment_pose_goal_audit_development import audit
...
from scripts.run_go2_matched_model_active_view_probe_v1 import OUTPUT as DIAGNOSTIC
from scripts.navigation_artifact_root_development import BASE, create_output, validate_root, verify_artifacts
from scripts.run_go2_successive_choice_maze_development_v1 import digest, write_json
from scripts.startup_raw_sensor_audit_development import read_json
from scripts.read_go2_tracking_posthoc_raw_accuracy_v1 import hardware

OUTPUT=BASE/'go2_commitment_pose_goal_probe_v1_attempt_001'
READOUT=BASE/'go2_matched_model_active_view_readout_v1_attempt_001'
DIAGNOSTIC_IDS={'launch.json':'bf03f9887ca55a2b874869947b2ebdd4d77589cfdcb7b39696b126833fc30cc2'}
...
PROTOCOL='docs/go2_commitment_pose_goal_probe_v1_2026-09-08.md'
...
def worker(case,launch_sha):
    ...
    terminal=dict(case=name,trial=trial,variant=variant,condition=condition,model_name=model_name,
        status='COMMITMENT_POSE_GOAL_WORKER_FAILED',artifact_sha256={})
    ...
    with (OUTPUT/(name+'_worker.log')).open('x') as log,contextlib.redirect_stdout(log),contextlib.redirect_stderr(log):
        try:
            ...
            terminal.update(status='COMMITMENT_POSE_GOAL_COLLECTED_AND_RAW_AUDITED',
...
def main():
    ...
    assert diagnostic['status']=='MATCHED_MODEL_ACTIVE_VIEW_PROBE_COMPLETE' and len(diagnostic['conditions'])==12
    ...
    assert readout['status']=='MATCHED_MODEL_ACTIVE_VIEW_READOUT_COMPLETE' and readout['probe_result_sha256']==args.prior_result_sha256
    ...
    launch=preflight(output=OUTPUT,protocol=PROTOCOL,
        seed_paths=('scripts/run_go2_commitment_pose_goal_probe_v1.py',
            'lewm/tests/test_commitment_pose_goal_probe_development.py',
            'docs/go2_matched_model_active_view_result_2026-09-08.md',
            'docs/go2_commitment_pose_waypoint_utility_v1_2026-09-08.md',*readout['source_sha256']),
    ...
    launch.update(all_six_admission=receipt,fit_artifact_sha256=fit_ids,snapshots=snapshots,
        ...
        experiment='commitment-horizon waypoint distance and bearing utility V1',
    ...
    print('COMMITMENT_POSE_GOAL_LAUNCHED',digest(OUTPUT/'launch.json'),flush=True)
    ...
    try:
                ...
                while pending:
                    ...
                    for future in done:
                        ...
                        try:record=future.result()
                            ...
                            record=dict(case=case[0],status='COMMITMENT_POSE_GOAL_PROCESS_FAILURE',failure=repr(exc))
                        records.append(record)
                        print('COMMITMENT_POSE_GOAL_TERMINAL',case[0],record['status'],record.get('goal'),flush=True)
            if len(records)!=4 or any(r['status']!='COMMITMENT_POSE_GOAL_COLLECTED_AND_RAW_AUDITED' for r in records):
        ...
        for root,ids in scaling['phase_result_sha256'].items():verify_artifacts(Path(root),ids)
        write_json(OUTPUT/'result.json',dict(status='COMMITMENT_POSE_GOAL_PROBE_COMPLETE',
        ...
        print('COMMITMENT_POSE_GOAL_COMPLETE',digest(OUTPUT/'result.json'),flush=True)
    except Exception as exc:
        write_json(OUTPUT/'failure.json',dict(status='TERMINAL_COMMITMENT_POSE_GOAL_FAILURE',reason=repr(exc),
