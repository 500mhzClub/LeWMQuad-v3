...
OUTPUT=BASE/'go2_auxiliary_return_pair_diagnosis_v2_attempt_001'
PROTOCOL='docs/go2_auxiliary_return_pair_diagnosis_v2_2026-09-09.md'
FAILED=BASE/'go2_auxiliary_return_pair_diagnosis_v1_attempt_001'
FAILED_IDS={'launch.json':'00438ea2df4021ae4c08343dea2aa5a78fe1848065a46e93712a90306f2f3e1e',
    'failure.json':'fc7b88723e3fda3a2dfa302283dd4a6df6c81c898523b71781985b197b7a24e3'}
...
def main():
    ...
    cv2.setNumThreads(1)
    verify_artifacts(FAILED,FAILED_IDS);failed=read_json(FAILED,'launch.json');source_check(failed['source_sha256'])
    ...
    sources=discover_sources((PROTOCOL,'scripts/diagnose_go2_auxiliary_return_pairs_v2.py',
        'lewm/tests/test_auxiliary_reference_pose_adapter_development.py'),failed['source_sha256'])
    resources=hardware()
    launch=dict(protocol=PROTOCOL,output_root=str(OUTPUT),source_sha256=sources,input_sha256=ids,
        diagnosis_artifact_sha256=diagnosis_ids,failed_attempt_artifact_sha256=FAILED_IDS,hardware=resources,frames=list(FRAMES),
    ...
    try:
        ...
        for i in FRAMES:
            ...
            with Image.open(artifact_path(INPUT,CASE+f'/auxiliary_rgb_{i:04d}.png')) as image:
                rgb=np.array(image)
        ...
        verify_all(launch)
        write_json(OUTPUT/'result.json',dict(status='AUXILIARY_RETURN_PAIR_DIAGNOSIS_V2_COMPLETE',
            ...
            failed_path_adapter_attempt_preserved=True,original_primary_stage_counts_exact=True,thresholds_unchanged=True,input_bytes_unchanged=True,
