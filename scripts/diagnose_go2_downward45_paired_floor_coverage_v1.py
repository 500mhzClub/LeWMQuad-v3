...
from scripts.run_go2_auxiliary_downward45_goal_probe_v1 import OUTPUT as INPUT,CORRECTION,FITS
...
INPUT_SHA='5007693d6e2b178d13c42164a08d561bf3ad2a60801b9290fb8033db811ce7fe'
READOUT_SHA='5d2f6df4c9762ac6d7f28c8546cea93e2d61356cda7a567561c32894c7667d6f'
OUTPUT=BASE/'go2_downward45_paired_floor_coverage_v1_attempt_001'
PROTOCOL='docs/go2_downward45_paired_floor_coverage_v1_2026-09-08.md'
...
def main():
    ...
    assert result['status']=='AUXILIARY_DOWNWARD45_GOAL_PROBE_COMPLETE'
    ...
    assert readout['status']=='AUXILIARY_DOWNWARD45_GOAL_READOUT_COMPLETE' and readout['probe_result_sha256']==INPUT_SHA
    ...
    sources=discover_sources((PROTOCOL,'scripts/diagnose_go2_downward45_paired_floor_coverage_v1.py',
        ...
        'docs/go2_auxiliary_downward45_goal_probe_result_2026-09-08.md'),readout['source_sha256'])
    resources=hardware()
    ...
    launch=original|dict(source_sha256=sources,protocol=PROTOCOL,output_root=str(OUTPUT),hardware=resources,
        ...
        controller_changed=False,division_levels=[1,2,4,8],diagnostic_ticks=[42,52],maximum_diagnostic_frames=256)
    verify(launch);create_output(OUTPUT);write_json(OUTPUT/'launch.json',launch)
    print('DOWNWARD45_PAIRED_FLOOR_COVERAGE_LAUNCHED',digest(OUTPUT/'launch.json'),flush=True)
    ...
    try:
        ...
        controller=AuxiliaryDownward45GoalProbe(model,geometry,condition=condition,variant=variant,persistent=True)
        ...
        write_json(OUTPUT/'result.json',dict(status='DOWNWARD45_PAIRED_FLOOR_COVERAGE_COMPLETE',
        ...
        print('DOWNWARD45_PAIRED_FLOOR_COVERAGE_COMPLETE',digest(OUTPUT/'result.json'),flush=True)
