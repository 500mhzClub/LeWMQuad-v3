#!/usr/bin/env python3
"""One-shot launcher for the reviewed V4 N5 full-panel development attempt."""
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Exact different-agent source-review JSON path.",
    )
    ...
    if str(args.source_review.resolve()) != str(policy.CANONICAL_SOURCE_REVIEW_PATH):
        raise PermissionError("N5 full-panel source review path is not canonical")
    if not policy.is_sha256(args.source_review_sha256):
        raise ValueError("N5 full-panel source review SHA-256 is malformed")
    return args
...
def _run_authorized(args: argparse.Namespace) -> int:
    # These three frozen records and every reviewed source are bound before the
    # trainer import can load RGB/model/accelerator code or create an output.
    authority = policy.verify_authority(
        args.source_review,
        args.source_review_sha256,
        require_unclaimed_output=True,
    )
    from scripts import (  # protected import intentionally follows authority
        train_go2_observable_camera_ray_fit_v4_n5_full_panel_v1 as trainer,
    )

    summary = trainer.run_exact(
        authority,
