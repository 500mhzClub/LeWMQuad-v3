...
from scripts.run_go2_course_aware_visual_servo_v1 import OUTPUT,CONDITIONS
...
def audit_condition(condition,result):
    ...
    previous_servo=read_npz(ROOT/'.generated/go2_bounded_visual_servo_v1_attempt_001'/condition,'physics_trace.npz')
    ...
    np.testing.assert_allclose(raw['base_pose_world'][0,:2],[-.55,-.20],rtol=0,atol=.002)
    initial_yaw=float(Rotation.from_quat(raw['base_pose_world'][0,3:]).as_euler('xyz')[2])
    assert abs(wrapped(initial_yaw+.08))<.002
    ...
    motion=VisualLedMotion('gyro',identity=(0,0,0)); controller=CourseAwareVisualServo(); desired=[]
...
def main():
    ...
    sources=discover_sources(('scripts/audit_go2_course_aware_visual_servo_v1.py',),launch['source_sha256'])
