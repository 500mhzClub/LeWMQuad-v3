...
OUTPUT = BASE/'go2_reactive_observed_route_prefix_v1_attempt_001'
PROTOCOL = 'docs/go2_reactive_observed_route_prefix_v1_2026-09-08.md'
...
def replay():
    ...
    controller = ReactiveObservedRoundTripController(ArticulatedCollisionGeometry(URDF),
    ...
    with writer(OUTPUT) as append:
        for i, original in enumerate(read_rows(directory)):
            ...
            if i % 100 == 0: print('REACTIVE_ROUTE_PREFIX_FRAME', i, flush=True)
...
def main():
    ...
    sources = discover_sources((PROTOCOL, 'scripts/replay_go2_reactive_observed_route_prefix_v1.py',
        'lewm/tests/test_reactive_observed_route_development.py',
    ...
    print('REACTIVE_ROUTE_PREFIX_LAUNCHED', digest(OUTPUT/'launch.json'), flush=True)
    try:
        ...
        write_json(OUTPUT/'result.json', dict(status='REACTIVE_OBSERVED_ROUTE_PREFIX_COMPLETE',
        ...
        print('REACTIVE_ROUTE_PREFIX_COMPLETE', digest(OUTPUT/'result.json'),
            {k: v for k, v in report.items() if k != 'changed_selection'}, flush=True)
    except Exception as error:
        write_json(OUTPUT/'failure.json', dict(status='TERMINAL_REACTIVE_ROUTE_PREFIX_FAILURE', reason=repr(error)))
        raise
