...
def collect(trial, definition, *, output, model, geometry, persistent, episode_name):
    ...
    try:
        ...
        session._runtime['high_level'] = 'surface_memory_goal_probe_v1; online model plus measured surfaces'
        ...
        controller = SurfaceMemoryGoalProbe(model, geometry, persistent=persistent)
        try:
        ...
        result = dict(status='SURFACE_MEMORY_GOAL_PROBE_TERMINAL_AUDIT_REQUIRED', trial=trial,
        ...
        print('SURFACE_MEMORY_GOAL_PROBE', trial, result, flush=True)
        return result
    finally:
