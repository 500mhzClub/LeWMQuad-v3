#!/usr/bin/env python3
"""Build or verify the V15 extended-horizon recursive source closure."""
...
_V13 = _source_only_module(
    "_lewm_v15_extended_horizon_source_closure_base",
    BASE_CHECKER_PATH,
)
if Path(_V13.ROOT).resolve(strict=True) != ROOT.resolve(strict=True):
    raise PermissionError("V15 source-closure base escaped the repository")
...
ENTRYPOINTS = (
    ...
    (
        "scripts/run_go2_rgb_unified_ray_survival_joint_jepa_v15_extended_"
        "horizon.py"
    ),
    (
        "scripts/execute_go2_rgb_unified_ray_survival_joint_jepa_v15_"
        "extended_horizon.py"
    ),
    (
        "scripts/launch_go2_rgb_unified_ray_survival_joint_jepa_v15_"
        "extended_horizon.py"
    ),
)
    ...
    dict.fromkeys(
        (
            *_V13.FORCED_DYNAMIC_SOURCES,
            ...
            (
                "scripts/go2_rgb_unified_ray_survival_joint_jepa_v15_"
                "extended_horizon_lifecycle.py"
            ),
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print("Go2 RGB unified-ray-survival V15 extended-horizon source closure: PASS")
    return 0
