#!/usr/bin/env python3
"""Finalizer successor for V4 receipts produced by metric verifier V2."""
...
if __name__.startswith("_lewm_v4_ca_"):
    ...
    from scripts import verify_go2_observable_camera_ray_fit_v4_metrics_v2 as metric_verifier


def _require_captured_private_finalizer() -> None:
    logical_name = "scripts.finalize_go2_observable_camera_ray_fit_v4_ladder_v2"
    if (
...
def _preflight_successor_sources(bound: str) -> tuple[Path, str, dict[str, Any]]:
    """Bind both successor sources before any attempt or gate is opened."""

    path, digest = _parse_bound_path(bound)
    _require_path(path, CANONICAL_SUCCESSOR_REVIEW_PATH, name="V4 V2 successor review")
    review = load_caller_hashed_json(path, digest, name="V4 V2 successor review")
    ...
    if (
        ...
        raise PermissionError("V4 V2 successor lacks a different-agent source review")
    for relative in SUCCESSOR_SOURCE_PATHS:
        ...
        source_sha = binding.get("file_sha256")
        if not _is_sha256(source_sha):
            raise PermissionError("V4 V2 successor source hash is malformed")
        _read_caller_hashed_bytes(
            ROOT / relative,
            str(source_sha),
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--successor-review", required=True, help="PATH:SHA256")
...
def _captured_finalizer_cli(args: argparse.Namespace) -> int:
    _require_captured_private_finalizer()
    successor_review_path, successor_review_sha, _review = (
        _preflight_successor_sources(args.successor_review)
    )
    metric_verifier.preflight_successor_review(
        successor_review_path,
        successor_review_sha,
    )
    receipt = run(args)
...
def _dispatch_captured_finalizer_cli(args: argparse.Namespace) -> int:
    if __name__ != "__main__":
        raise PermissionError("V4 V2 finalizer library execution is unsupported")
    _successor_review_path, _successor_review_sha, successor_review = (
        _preflight_successor_sources(args.successor_review)
    )
    ...
    logical_self = "scripts.finalize_go2_observable_camera_ray_fit_v4_ladder_v2"
    try:
        ...
        if not isinstance(live, types.ModuleType) or (
            ...
            raise PermissionError("V4 live V2 finalizer differs from captured source")
