...
from scripts.run_go2_single_sample_rgbd_observation_development_v1 import (OUTPUT, LEAVES, RGB_LEAVES, trials,
...
def audit_trial(directory, spec, row):
    ...
    floor = check_floor_identity(json.loads((directory/'floor_visual_collision_identity.json').read_text()))
    check(row['floor_identity'] == floor, 'native visual/collision floor offset')
    check(len(row['physical_reference_checks']) == 5, 'physical reference retained separately')
    ...
    for index, camera in enumerate(cameras):
        ...
        check(evaluate_physical_reference(native, spec['geometry']['wall_boxes'], camera['world_from_optical'],
              marker_case=spec['marker_case']) == row['physical_reference_checks'][index], 'unchanged physical-reference outcome')
