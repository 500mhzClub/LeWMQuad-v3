"""Fresh physical maze3 tracking successor after completed causal integration."""
import argparse
import contextlib
import json
...
from scripts.direct_flow_maze03_episode_development import collect, artifacts
from scripts.direct_flow_maze01_audit_development import audit
from scripts.direct_flow_maze03_native_prefix_development import admit_prefix, compare
from scripts.replay_go2_direct_flow_maze03_prefix_v1 import (
...
OUTPUT = BASE/'go2_direct_flow_maze03_pilot_v1_attempt_001'
CASE = ('full_jepa_direct_flow_maze_03', *LEARNED_CASE[1:])
PROTOCOL = 'docs/go2_direct_flow_maze03_pilot_v1_2026-09-09.md'
PREFIX_SHA = '104edd70824e7d4e7b909be924eb68a918542efd3464ff3247f9570ddc28669e'
...
def verify_inputs(launch):
    ...
    if (launch['planned_case'] != list(CASE) or launch['scene_specification'] != specification(3)
        ...
        raise ValueError('fixed direct-flow tracking maze3 definition required')
...
def worker(launch_sha):
    ...
    terminal = dict(case=name,layout_index=index,model_name=model_name,
        status='DIRECT_FLOW_MAZE03_WORKER_FAILED',artifact_sha256={}); started = time.perf_counter()
    with (OUTPUT/(name+'_worker.log')).open('x') as log, contextlib.redirect_stdout(log), contextlib.redirect_stderr(log):
        try:
            ...
            terminal.update(status='DIRECT_FLOW_MAZE03_COLLECTED_AND_RAW_AUDITED',artifact_sha256=bindings,
...
def main():
    ...
    admission = dict(model_state_sha256=old['model_state_sha256'],
        predecessor_verified_round_trip=learned['conditions'][2]['verified_round_trip'])
    ...
    sources = discover_sources((PROTOCOL,'scripts/run_go2_direct_flow_maze03_pilot_v1.py',
        'lewm/tests/test_direct_flow_maze03_native_development.py',
        'lewm/tests/test_direct_flow_maze03_native_prefix_development.py',
        'docs/go2_direct_flow_maze03_prefix_result_2026-09-09.md'),inherited)
    ...
    if args.preflight_only:
        print('DIRECT_FLOW_MAZE03_NATIVE_PREFLIGHT',json.dumps(dict(source_count=len(sources),hardware=resources,
    ...
    print('DIRECT_FLOW_MAZE03_NATIVE_LAUNCHED',launch_sha,flush=True); started = time.perf_counter()
    try:
        ...
        print('DIRECT_FLOW_MAZE03_NATIVE_TERMINAL',record['status'],record.get('verified_round_trip'),record.get('failure'),flush=True)
        if record['status'] != 'DIRECT_FLOW_MAZE03_COLLECTED_AND_RAW_AUDITED':
        ...
        verify_inputs(launch); verify_artifacts(OUTPUT,bindings)
        write_json(OUTPUT/'result.json',dict(status='DIRECT_FLOW_MAZE03_PILOT_V1_COMPLETE',
        ...
        print('DIRECT_FLOW_MAZE03_NATIVE_COMPLETE',digest(OUTPUT/'result.json'),flush=True)
    except Exception as error:
        write_json(OUTPUT/'failure.json',dict(status='TERMINAL_DIRECT_FLOW_MAZE03_NATIVE_FAILURE',reason=repr(error)))
        raise
