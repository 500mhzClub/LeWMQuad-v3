#!/usr/bin/env python3
"""Run the one authorized science-identical V2 operational recovery.
...
_CONTRACT_PATH = (
    ROOT / "lewm/benchmarks/go2_shared_jepa_v5_multires_probe_v2.py"
)
_CONTRACT_SPEC = importlib.util.spec_from_file_location(
    "_lewm_go2_shared_jepa_v5_multires_probe_v2_contract",
    _CONTRACT_PATH,
)
...
PREFLIGHT_ENVIRONMENT_KEY = "LEWM_MULTIRES_PROBE_V2_PREFLIGHT_JSON"
THREAD_ENVIRONMENT = (
    "OMP_NUM_THREADS",
    "MKL_NUM_THREADS",
...
def _failure_boundary(name: str) -> None:
    ...
    if name not in SYNTHETIC_FAILURE_BOUNDARIES:
        raise ValueError(f"unknown V2 failure boundary: {name}")
...
class OperationProgress:
    """Exact attempted/completed operations for terminal V2 receipts."""
...
class PartialAccessLedger:
    ...
    def __init__(
        ...
        *,
        ...
        if not hasattr(os, "O_NOFOLLOW"):
            raise PermissionError("O_NOFOLLOW is required for V2 input custody")
        self.output_root = output_root
        self.repository_root = repository_root.resolve()
        self.path = output_root / self.RELATIVE_PATH
        self.records: list[dict[str, Any]] = []
        self.raw_parts: list[bytes] = []
        ...
        _write_exclusive(self.path, header_raw, mode=0o600)
        self.records.append(header)
        self.raw_parts.append(header_raw)
    ...
    def read_regular(
        ...
        *,
        ...
        if (
            relative == contract.V1_OUTPUT_ROOT_RELATIVE_PATH
            or relative.startswith(contract.V1_OUTPUT_ROOT_RELATIVE_PATH + "/")
        ):
            raise PermissionError("V1 runtime output access is forbidden")
...
def _load_post_reservation_stack() -> tuple[Any, Any, Any, Any]:
    ...
    schedule_adapter = _load_path(
        "_lewm_multires_probe_v2_schedule_adapter",
        ROOT / contract.SCHEDULE_ADAPTER_RELATIVE_PATH,
    )
    return matched, runtime, multires, schedule_adapter
...
def _finalize_schedule_train_identity(
    ...
    if (
        ...
        raise PermissionError("V2 schedule adapter return contract changed")
...
def _execute_after_reservation(
    *,
    ...
    try:
        ...
        matched, runtime, multires, schedule_adapter = (
