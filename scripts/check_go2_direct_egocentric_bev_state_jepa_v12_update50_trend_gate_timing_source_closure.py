#!/usr/bin/env python3
"""Build or verify the Direct-BEV V12 recursive source closure."""
...
contract = _source_only_module(
    "_lewm_direct_bev_v12_update50_trend_gate_closure_contract",
    CONTRACT_RELATIVE_PATH,
)
_V11 = _source_only_module(
    "_lewm_direct_bev_v12_update50_trend_gate_frozen_v11_closure",
    contract.FROZEN_V11_SOURCE_CLOSURE_CHECKER_RELATIVE_PATH,
)
if (
    contract.CONTRACT_RELATIVE_PATH != CONTRACT_RELATIVE_PATH
    or _V11.CONTRACT_RELATIVE_PATH
    != contract.FROZEN_V11_CONTRACT_RELATIVE_PATH
):
    raise PermissionError("Direct-BEV V12 source-closure identity changed")

V12_ADDITIVE_SOURCE_PATHS = frozenset({
...
if (
    set(contract.ADDITIVE_SOURCE_PATHS) != V12_ADDITIVE_SOURCE_PATHS
    or len(contract.ADDITIVE_SOURCE_PATHS) != 5
    or len(contract.REUSED_SOURCE_PATHS) != 138
    or len(contract.SOURCE_PATHS) != 143
):
    raise PermissionError("Direct-BEV V12 five-file source surface changed")
if (
    ...
    raise PermissionError("Direct-BEV V12 source entrypoints changed")

REQUIRED_DYNAMIC_SOURCE_PATHS = frozenset({
    *_V11.REQUIRED_DYNAMIC_SOURCE_PATHS,
    *V12_ADDITIVE_SOURCE_PATHS,
    contract.FROZEN_V11_CONTRACT_RELATIVE_PATH,
    contract.FROZEN_V11_SOURCE_CLOSURE_CHECKER_RELATIVE_PATH,
})
if (
    ...
    raise PermissionError(
        f"Direct-BEV V12 dynamic source roots are incomplete: {missing}"
    )

# Rebind every inherited checker layer to the V12 manifest identities.
layers = [_V11, *_V11.layers]
for layer in layers:
...
def main() -> int:
    ...
    mode.add_argument(
        "--emit",
        action="store_true",
        help="build a candidate without requiring a frozen V12 manifest",
    )
    mode.add_argument(
        "--write",
        action="store_true",
    ...
    print("Go2 Direct BEV-State JEPA V12 update-50 trend gate closure: PASS")
    return 0
