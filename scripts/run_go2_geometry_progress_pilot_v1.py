"""Twenty-four randomized training-only geometry/progress excitation episodes."""
import shutil
import time
import cv2
from lewm.geometry_progress_pilot_development import TRIALS,specification,schedule,decision,WARMUP_TICKS,assignments
...
OUTPUT=BASE/'go2_geometry_progress_pilot_v1_attempt_001'
PROTOCOL='docs/go2_geometry_progress_pilot_v1_2026-09-07.md'
RESERVE=40*1024**3
...
def preflight():
    ...
    sources=discover_sources((PROTOCOL,'scripts/run_go2_geometry_progress_pilot_v1.py',
        'scripts/audit_go2_geometry_progress_pilot_v1.py',
        'lewm/tests/test_geometry_progress_pilot_development.py'),old['source_sha256'])
    resources=hardware()
...
def collect(trial,definition):
    ...
    try:
        session=GeometryProgressSession(spec,directory);session.install_contact_identity();build=session.ctx.build
        ...
        print('GEOMETRY_PROGRESS_PILOT',trial,result,flush=True);return result
    finally:
...
def main():
    ...
    try:
        ...
        write_json(OUTPUT/'result.json',dict(status='GEOMETRY_PROGRESS_COLLECTION_TERMINAL',conditions=results,
