...
CONFIG=Path('docs/go2_decision_headroom_protocol_v41_2026-09-23.json')

class AuditBudget(PilotBudget):
    def check(self,stage,*,force=False):
        super().check(stage,force=force)
        # The wall amendment does not increase V4's GPU-owner wall allowance.
...
def main():
    ...
    try:
        ...
        from scripts.run_go2_headroom_v41_source_development import run,require_stage_a_closed
        ...
        p=root/'resource_result.json';r=json.loads(p.read_text());r['phase2_authorized']=True;r['gpu_owner_wall_s']=getattr(budget,'gpu_owner_seconds',0.);r['approval_sha256']=hashlib.sha256(args.approval.read_bytes()).hexdigest();p.write_text(json.dumps(r,indent=2)+'\n')
