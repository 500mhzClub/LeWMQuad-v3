"""Public-sensor replay until the first unexecuted waypoint-score command."""
import json
import time
import cv2
import torch
...
from scripts.run_go2_nominal_reentry_maze_pilot_v1 import OUTPUT as INPUT, CASE, CORRECTION, FITS
from scripts.read_go2_nominal_reentry_maze_pilot_v1 import OUTPUT as READOUT
...
OUTPUT = BASE/'go2_executed_waypoint_maze_prefix_v1_attempt_001'
PROTOCOL = 'docs/go2_executed_waypoint_maze_prefix_v1_2026-09-08.md'
INPUT_SHA = 'b48f3c79d19889c67438073a3a7305d0eafcbc5f8b7286ea014e2eb1fd711739'
READOUT_SHA = '0ff22595c9a49fb855212952a11f02915a6269c22db3011180c03dbfda0a20f7'
...
def replay(admission):
    ...
    controller = ExecutedWaypointRoundTripController(model, ArticulatedCollisionGeometry(URDF),
    ...
    with writer(OUTPUT) as append:
        for i, original in enumerate(read_rows(directory)):
            ...
            for key in ('evidence', 'memory_receipt', 'observed_goal_distance_m', 'auxiliary_floor_partition_receipt', 'mission_receipt', 'causal_residual_receipt'):
                assert r[key] == old[key], ('unchanged causal observation/map/mission', i, key)
            assert (a is None) == (b is None)
            if a is not None:
                receipt = old['causal_residual_receipt'] | dict(pending_forecast_tick=None)
                expected = score_waypoint_execution(a, receipt)
                assert b == expected, ('exact executed-waypoint transformation', i)
            ...
            if first_command is None and first_terminal is None:
                for key in old:
                    if key not in ('controller', 'new_selection'): assert r[key] == old[key], ('exact pre-intervention decision', i, key)
            append(dict(tick=i, decision=r)); frames += 1; last = r
            if i % 100 == 0: print('EXECUTED_WAYPOINT_PREFIX_FRAME', i, flush=True)
...
def main():
    ...
    if OUTPUT.exists() or OUTPUT.is_symlink(): raise ValueError('exclusive executed-waypoint replay required')
    bound = []
    for root, sha, status in ((INPUT, INPUT_SHA, 'NOMINAL_REENTRY_MAZE_PILOT_COMPLETE'),
            (READOUT, READOUT_SHA, 'NOMINAL_REENTRY_MAZE_READOUT_COMPLETE')):
    ...
    sources = discover_sources((PROTOCOL, 'scripts/replay_go2_executed_waypoint_maze_prefix_v1.py',
        'lewm/tests/test_executed_waypoint_score_development.py',
        'docs/go2_nominal_reentry_maze_pilot_result_2026-09-08.md'), bound[-1][2]['source_sha256'])
    resources = hardware()
    ...
    print('EXECUTED_WAYPOINT_PREFIX_LAUNCHED', digest(OUTPUT/'launch.json'), flush=True)
    try:
        ...
        write_json(OUTPUT/'result.json', dict(status='EXECUTED_WAYPOINT_MAZE_PREFIX_COMPLETE', source_sha256=sources,
        ...
        print('EXECUTED_WAYPOINT_PREFIX_COMPLETE', digest(OUTPUT/'result.json'), {k: v for k, v in report.items() if k != 'changed_selection'}, flush=True)
    except Exception as error:
        write_json(OUTPUT/'failure.json', dict(status='TERMINAL_EXECUTED_WAYPOINT_PREFIX_FAILURE', reason=repr(error)))
        raise
