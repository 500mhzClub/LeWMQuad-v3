#!/usr/bin/env python3
"""Independent reconstruction verifier for Shared JEPA V5 training V4.
...
if __name__ == "__main__":
    ...
    def _validate_static_artifacts(
        ...
        if implementation_review["reviewed_production_sources"] != {
        ...
        expected_source_review = policy.content_value(
            {
                ...
                "camera_v14_dynamic_authority_bindings": {
                    policy.CAMERA_V14_SOURCE_REVIEW_RELATIVE_PATH: bindings[
                    ...
                    policy.CAMERA_V14_LADDER_REVIEW_RELATIVE_PATH: bindings[
        ...
        policy.validate_raw_v13_source_chain(
            builder_review=_parse_json(
                _read_repo(policy.RAW_BUILDER_V9_REVIEW_RELATIVE_PATH),
                name="Builder V9 review reconstruction",
            ),
            auditor_review=_parse_json(
                _read_repo(policy.RAW_AUDITOR_V13_REVIEW_RELATIVE_PATH),
                name="Auditor V13 review reconstruction",
            ),
            authorization=_parse_json(
                _read_repo(policy.RAW_AUDITOR_V13_AUTHORIZATION_RELATIVE_PATH),
                name="Raw V13 authorization reconstruction",
            ),
            fingerprint=_parse_json(
                _read_repo(policy.RAW_AUDITOR_V13_FINGERPRINT_RELATIVE_PATH),
                name="Raw V13 authorization fingerprint reconstruction",
            ),
        )
        dynamic_camera_sources = {
            ...
            policy.CAMERA_V14_LADDER_PREREGISTRATION_RELATIVE_PATH: bindings[
                "camera_v14_ladder_preregistration_file_sha256"
            ],
            policy.CAMERA_V14_LADDER_REVIEW_RELATIVE_PATH: bindings[
                "camera_v14_ladder_independent_review_file_sha256"
        ...
        ladder_review = _parse_json(
            dynamic_camera_raw[policy.CAMERA_V14_LADDER_REVIEW_RELATIVE_PATH],
        ...
        if (
            ...
            != bindings["camera_v14_ladder_preregistration_file_sha256"]
        ):
        ...
        raw_manifest = policy.validate_raw_v13_manifest(
            _parse_json(raw_manifest_raw, name="raw manifest for input binding")
        )
        ...
        raw_audit = policy.validate_raw_v13_terminal_report(
            _parse_json(raw_audit_raw, name="raw audit for input binding")
        )
        camera_source_review_raw = _read_repo(
            policy.CAMERA_V14_SOURCE_REVIEW_RELATIVE_PATH
        )
        _assert_hash(
            camera_source_review_raw,
            bindings["camera_v14_source_review_file_sha256"],
            name="Camera V14 source review for input binding",
        )
        camera_source_review = _parse_json(
            camera_source_review_raw,
            name="Camera V14 source review for input binding",
        )
        camera_n5_gate_raw = _read_repo(policy.CAMERA_V14_N5_GATE_RELATIVE_PATH)
        _assert_hash(
            camera_n5_gate_raw,
            bindings["camera_v14_n5_gate_pass_file_sha256"],
            name="Camera V14 N5 gate for input binding",
        )
        camera_n5_gate = _parse_json(
            camera_n5_gate_raw,
            name="Camera V14 N5 gate for input binding",
        )
        v4_ladder_raw = _read_repo(policy.CAMERA_V14_TWO_SEED_LADDER_RELATIVE_PATH)
        _assert_hash(
            v4_ladder_raw,
            bindings["camera_v14_two_seed_ladder_pass_file_sha256"],
        ...
        ladder_rows = policy.validate_camera_v14_ladder_rows(
            authorization["camera_ladder_rows"],
            reviewed_source_bindings=authorization["camera_v14_source_bindings"],
        )
        for row in ladder_rows:
            for name in ("reservation", "source_review", "gate", "completion", "rung_review"):
                row_binding = row[name]
                row_raw = _read_repo(row_binding["path"])
                _assert_hash(
                    row_raw,
                    row_binding["file_sha256"],
                    name=f"Camera V14 row {row['row_index']} {name}",
                )
                row_value = _parse_json(
                    row_raw,
                    name=f"Camera V14 row {row['row_index']} {name}",
                )
                if row_value.get("content_sha256") != row_binding["content_sha256"]:
                    raise PermissionError("Camera V14 per-rung content binding changed")
            checkpoint_raw = _read_repo(row["checkpoint"]["path"])
            _assert_hash(
                checkpoint_raw,
                row["checkpoint"]["file_sha256"],
                name=f"Camera V14 row {row['row_index']} checkpoint",
            )
        if (
            ...
            != bindings["camera_v14_n5_gate_pass_content_sha256"]
            ...
            or authorization["camera_ladder_aggregate"].get("rows_sha256")
            != policy.canonical_json_sha256(ladder_rows)
        ):
        ...
        expected_input_bindings = policy.content_value(
            {
                ...
                "final_exact_authorization_file_sha256": reservation[
                    "final_exact_authorization_file_sha256"
                ],
                "final_exact_authorization_content_sha256": reservation[
                    "final_exact_authorization_content_sha256"
                ],
                ...
                "camera_v14_source_review_file_sha256": bindings[
                    "camera_v14_source_review_file_sha256"
                ],
                "camera_v14_source_review_content_sha256": (
                    camera_source_review["content_sha256"]
                ),
                "camera_v14_n5_gate_file_sha256": bindings[
                    "camera_v14_n5_gate_pass_file_sha256"
                ],
                "camera_v14_n5_gate_content_sha256": camera_n5_gate[
                    "content_sha256"
                ],
                "camera_v14_ladder_preregistration_file_sha256": bindings[
                    "camera_v14_ladder_preregistration_file_sha256"
                ],
                "camera_v14_ladder_independent_review_file_sha256": bindings[
                    "camera_v14_ladder_independent_review_file_sha256"
                ],
                "camera_v14_two_seed_ladder_file_sha256": bindings[
                    "camera_v14_two_seed_ladder_pass_file_sha256"
                ],
                "camera_v14_two_seed_ladder_content_sha256": v4_ladder[
                    "content_sha256"
                ],
    ...
    def _validate_traces(values: Mapping[str, Any]) -> None:
        ...
        for arm in policy.ARMS:
            ...
            for index, row in enumerate(rows, start=1):
                if (
                    row.get("schema")
                    != "lewm_go2_shared_jepa_v5_full_training_v4_trace_row_v1"
                    or row.get("arm") != arm
    ...
    def _load_fixed_backend() -> Any:
        ...
        class IndependentReconstructor:
            ...
            def _reconstruct_initialization(self) -> tuple[Mapping[str, Any], list[str], Any]:
                bindings = self.reservation["required_exact_bindings"]
                raw = _read_repo(policy.CAMERA_V14_PRIMARY_CHECKPOINT_RELATIVE_PATH)
                _assert_hash(
                    raw,
            ...
            def _checkpoint(self, arm: str, update: int) -> tuple[Any, Mapping[str, Any]]:
                ...
                if (
                    ...
                    != "lewm_go2_shared_jepa_v5_full_training_v4_checkpoint_v1"
    ...
    def verify(
        ...
        manifest_raw = _read_repo(policy.FINAL_EXACT_EXECUTION_AUTHORIZATION_RELATIVE_PATH)
        ...
        manifest = policy.validate_final_exact_execution_authorization(
            _parse_json(manifest_raw, name="final exact authorization in verifier"),
        )
        ...
        core = {
            "schema": "lewm_go2_shared_jepa_v5_full_training_v4_verification_v1",
