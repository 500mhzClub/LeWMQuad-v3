...
SOURCE = 'scripts/profile_go2_tiled_density_progressive_floor_late_history_v2.py'
TEST = 'lewm/tests/test_tiled_density_progressive_floor_profile_v2_development.py'
PROTOCOL = 'docs/go2_tiled_density_progressive_floor_profile_v2_2026-09-11.md'
...
FAILED_PREDECESSOR = 'docs/go2_tiled_density_progressive_floor_profile_v1_admission_failure_2026-09-11.json'
FAILED_PREDECESSOR_SHA = '038ea8fe55b38e7a6d7d68744423a558dbd69e417f7bef5efa6bc85b1673dde7'
LAUNCH_SHA = '8ad6b00ce598b26b2058c9c904a51a908f2f63f4398b6eb5b46e8484b6ca45dc'
OUTPUT = BASE/'go2_tiled_density_progressive_floor_late_history_profile_v2_attempt_001'
...
def prepared_sources():
    ...
    verify({FAILED_PREDECESSOR: FAILED_PREDECESSOR_SHA})
    failure = json.loads((ROOT/FAILED_PREDECESSOR).read_text())
    if failure['status'] != 'TERMINAL_TILED_PROFILE_V1_ADMISSION_CAPTURE_LOG_FAILURE':
        raise ValueError('preserved original admission failure required')
    inherited = merge_sources(preparation['source_sha256'], failure['source_sha256'])
    sources = discover_sources((SOURCE, TEST, PROTOCOL, PREPARATION, FAILED_PREDECESSOR), inherited)
    verify(sources)
    return sources
...
def capture_verification(result_sha):
    ...
    def capture_digest(path):
        if path == destination:
            if len(receipts) != 1:
                raise ValueError('captured receipt must precede its logging digest')
...
def main():
    ...
    try:
        ...
        write_json(OUTPUT/'result.json', dict(status='TILED_DENSITY_PROGRESSIVE_FLOOR_LATE_HISTORY_PROFILE_V2_COMPLETE',
