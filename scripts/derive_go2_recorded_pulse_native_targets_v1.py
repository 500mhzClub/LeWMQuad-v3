...
OUTPUT=ROOT/'.generated/go2_recorded_pulse_native_targets_v1_attempt_001'
PROTOCOL='docs/go2_recorded_pulse_native_targets_v1_2026-09-06.md'
...
def preflight():
    ...
    sources=discover_sources((PROTOCOL,'scripts/derive_go2_recorded_pulse_native_targets_v1.py',
        'lewm/tests/test_pulse_native_targets_development.py',
        'lewm/tests/test_recorded_pulse_native_targets_development.py'),old['source_sha256'])
    predecessor=ROOT/'.generated/go2_pulse_native_targets_v1_attempt_001'
    inputs|={str((predecessor/n).relative_to(ROOT)):h for n,h in {
        'launch.json':'00c2009cfb5480074bcaa0065a8e5f7e4086413ff68de0bd1cb9e61580142d0c',
        'failure.json':'52f055893b4e644b562e211a94b3c9180d8b1392b2ead687faea9fa5cee5be9a'}.items()}
    verify_bindings(sources|inputs)
    for c in TRIALS:RecordedPulseNativeTargets(read_npz(INPUT/c,'physics_trace.npz'))
...
def main():
    ...
    try:
        ...
        for c in TRIALS:
            model=RecordedPulseNativeTargets(read_npz(INPUT/c,'physics_trace.npz'));counts=Counter();rows=[]
            for w in windows:
