...
from lewm.decision_headroom_v41_development import paired_filter, binding_event

MODES=('all','view_seeking','positional_route','positional_terminal',
    'pose_position/le_20mm','pose_position/20_to_100mm','pose_position/gt_100mm','pose_position/unresolved',
    'pose_yaw/le_5deg','pose_yaw/5_to_15deg','pose_yaw/gt_15deg','pose_yaw/unresolved')

def mode_match(row, mode):
    if mode.startswith('pose_'):
        key='position_stratum' if mode.startswith('pose_position') else 'yaw_stratum'
        return row.get('localisation',{}).get(key,'unresolved')==mode.split('/')[1]
    return mode=='all' or row['objective']['mode']==mode
...
def report(root,*,budget=None):
    ...
    for case in range(24):
        ...
        for mode in MODES:
            for phase in ('all','outbound_exploration','goal_approach_settle','return'):
                state_rows=[(r['sampling']['weight'],r) for r in population if mode_match(r,mode) and (phase=='all' or r['objective']['phase']==phase)]
                summaries={}
                for motion in ROWS:
                ...
                for motion in ('R4/old_data','R4/maze_data','R5c'):
                    for quantity in ('excluded_safe','all_excluded_despite_safe'):
                        summaries[f'paired_filter/{motion}-R2/{quantity}']=paired_filter(state_rows,motion,quantity)
                binding_rows=[(w,binding_event(r['filter_audit']['R2']['operating'])) for w,r in state_rows if 'R2' in r['filter_audit']]
                summaries['memo/R2/motion_binding_complete_exclusion']=ratio([(w,v) for w,v in binding_rows if v is not None],lambda v:v,lambda v:1)
                ...
                def regrets(row,names,field='regret_s'):
                    if not row['objective']['reference_regret_applicable']:return None
                    values=[row['rows'].get(n,{}).get(field) for n in names]
                ...
                for name,names in contrasts.items():
                    supported=[(w,regrets(r,names,'secondary_regret_s')) for w,r in state_rows if regrets(r,names,'secondary_regret_s') is not None]
                    summaries['secondary_reference/'+name]=ratio(supported,lambda v:v[0] if len(v)==1 else v[0]-v[1],lambda v:1)
    ...
    clustered={};confidence=1-.05/13
    quantities=primary_names+[f'paired_filter/{m}-R2/{q}' for m in ('R4/old_data','R4/maze_data','R5c') for q in ('excluded_safe','all_excluded_despite_safe')]+[f'filter/R2/operating/{q}' for q in ('excluded_safe','all_excluded_despite_safe')]
    assert len(quantities)==13
    descriptive=[f'filter/{m}/{c}/{q}' for m in ROWS for c in ('operating','hard') for q in ('excluded_safe','admitted_unsafe','all_excluded_despite_safe','unsafe_admission_given_unsafe','safe_among_excluded')]
    descriptive += [f'outcome/{name}/hard' for name in ('R5c','R4/old_data','R4/maze_data')]+['paired_harm/old_data','paired_harm/maze_data']
    descriptive += ['secondary_reference/'+n for n in contrasts]
    secondary_quantities=list(dict.fromkeys(descriptive))
    for exposure,indices in [('exposed',range(4)),('runtime_unexamined_at_freeze',range(4,8))]:
        for mode in MODES:
            for phase in ('all','outbound_exploration','goal_approach_settle','return'):
                for quantity in list(dict.fromkeys(quantities+secondary_quantities+['memo/R2/motion_binding_complete_exclusion'])):
                    values=[]
                    ...
                    clustered[f'{exposure}/{mode}/{phase}/{quantity}']=layout_interval(values,confidence if mode=='all' and phase=='all' and (quantity in quantities or quantity.startswith('memo/')) else .95)
                    if quantity in descriptive:
                        clustered[f'descriptive/{exposure}/{mode}/{phase}/{quantity}']=layout_interval(values,.95)
    reference_coverage=[]
    for case in range(24):
        for stratum in ('inside_046m_inflation','outside_inflation_below_5mm','reference_free'):
            for criterion in ('hard','operating'):
                for status in ('safe','unsafe','unresolved'):
                    candidates=[]
                    for r in records:
                        if r['case']!=case or not r['objective']['reference_regret_applicable']:continue
                        for i,s in enumerate(r.get('endpoint_clearance_strata',[])):
                            value=r['safety'][i][criterion]
                            if s==stratum and (value==status or status=='unresolved' and value.startswith('unresolved')):
                                candidates.append((r['sampling']['weight'],(r,i)))
                    for label,field,ref in [('primary','costs','reference'),('secondary','secondary_costs','secondary_reference')]:
                        endpoint=ratio(candidates,lambda v:len(v[0].get(field,[]))>v[1] and v[0][field][v[1]].get('cost_s') is not None,lambda v:1)
                        optimum=ratio(candidates,lambda v:v[0].get(ref,{}).get('status')=='available',lambda v:1)
                        reference_coverage.append(dict(case=case,endpoint_clearance_stratum=stratum,criterion=criterion,physical_status=status,reference=label,endpoint_cost_coverage=endpoint,reference_regret_coverage=optimum,candidates=len(candidates)))
    if budget:budget.admit_write(16*1024**2)
    save(root/'analysis_v42.json',dict(schema='headroom_v42_analysis.v1',cell_coverage=coverage,cells=cells,layout_clustered=clustered,
        primary_family_size=13,primary_quantities=quantities,reference_coverage_by_clearance=reference_coverage,continuous_regret_unzeroed=True,inconclusive_regret_supports_neither_stopping_nor_continuing=True,training_render_provenance='render_unverified',source_navigation_is_descriptive_only=True,
        no_unresolved_item_blocks_other_quantities=True,decision_memo_requires_joint_harm_coverage_and_precision_review=True))
    (root/'analysis_v42.md').write_text('# V4 fixed audit closure\n\nAll assigned cells, missing quantities, weighted denominators, filter binding rules, paired regret populations and per-layout intervals are in analysis_v42.json. No source-run navigation outcome is a cohort estimate. No automated recommendation is executed. Prepare the one-step decision memo from these fixed results and then stop.\n')
