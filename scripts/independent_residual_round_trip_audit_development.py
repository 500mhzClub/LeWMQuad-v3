"""Complete raw audit for the fixed independent-layout residual collection.

Private geometry remains evaluator-only. A passing episode is not a claim of
population reliability, planning/memory advantage or hardware qualification.
"""
import json
...
def audit(layout_index, result, definition, *, input_root, model, robot_geometry, episode_name, condition, variant):
    ...
    assert result['residual_first_interval_feasibility_fallback_enabled'] is True
    assert result['residual_hold_feasibility_enabled'] is True
    assert result['residual_anchored_continuation_enabled'] is True
    ...
    controller = ResidualAnchoredContinuationController(model, robot_geometry, public_mission=mission,
    ...
    return dict(layout_index=layout_index, renderer_capture_audit=renderer, measured_floor_transport_enabled=True,
        ...
        independent_layout_development_execution=True, reused_development_layout=False, navigation_qualified=False, hardware_qualified=False,
