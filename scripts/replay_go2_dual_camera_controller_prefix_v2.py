...
from lewm.dual_camera_json_prefix_comparison_development import compare_json_primary_decision as compare_primary_decision
...
OUTPUT = BASE/'go2_dual_camera_controller_prefix_v2_attempt_001'
PROTOCOL = 'docs/go2_dual_camera_controller_prefix_v2_2026-09-09.md'
...
FAILED = BASE/'go2_dual_camera_controller_prefix_v1_attempt_001'
FAILED_IDS = {'launch.json':'8b01122596274745c0d2e0de89cf8f51956ebafc316b58977d7339223f571efe',
    'failure.json':'25421b3950be88c00340b586399ec151324bb771509782de6c809c7567637ca2',
    'mismatch.json':'f3e1d3d3894547374ff4dfb4aa4fb4f16d219cca707c9e4cffb2202a7488f79a'}
...
def main():
    ...
    verify_artifacts(FAILED,FAILED_IDS)
    failed=read_json(FAILED,'launch.json');source_check(failed['source_sha256'])
    inherited=dict(failed['source_sha256'])
    for path,h in native['source_sha256'].items():
        if path in inherited and inherited[path]!=h:raise ValueError('native source differs from failed attempt: '+path)
        inherited[path]=h
    for path,h in registered['source_sha256'].items():
        ...
        inherited[path]=h
    sources=discover_sources((PROTOCOL,'scripts/replay_go2_dual_camera_controller_prefix_v2.py',
        'lewm/tests/test_dual_camera_json_prefix_comparison_development.py'),inherited)
    resources=hardware()
    launch=dict(protocol=PROTOCOL,output_root=str(OUTPUT),source_sha256=sources,
        collected_input_sha256=bindings,failed_attempt_artifact_sha256=FAILED_IDS,
    ...
    try:
        ...
        write_json(OUTPUT/'result.json',dict(status='DUAL_CAMERA_CONTROLLER_PREFIX_V2_COMPLETE',
            ...
            completed_native_audit_and_matching_collection_bindings_required_before_next_native=True,
            failed_json_identity_attempt_preserved=True,controller_implementation_unchanged=True,
