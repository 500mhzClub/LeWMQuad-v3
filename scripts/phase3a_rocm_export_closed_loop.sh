#!/usr/bin/env bash
# Run Phase 3A closed-loop export/evaluation through ROCm-capable TinyQuadJEPA Python.
set -euo pipefail
...
cd "$ROOT"
exec "$PYTHON_BIN" scripts/export_jepa_phase3a_closed_loop_demo_mp4.py \
  --device "${PHASE3A_EXPORT_DEVICE:-cuda}" \
  "$@"
