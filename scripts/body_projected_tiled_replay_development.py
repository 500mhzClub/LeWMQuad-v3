"""Original full-history replay body with only body-projection controller bindings.
...
from lewm.body_projected_tiled_controller_development import BodyProjectedTiledController, normalize_to_tiled
...
SOURCE='scripts/body_projected_tiled_replay_development.py'
TEST='lewm/tests/test_body_projected_tiled_replay_development.py'
OUTPUT=BASE/'go2_body_projected_tiled_late_history_v1_attempt_001'
...
def progress(*args,**kwargs):
    if args and args[0]=='SCOPED_FOOTPRINT_PAIRED_FRAME':
        args=('BODY_PROJECTED_TILED_PAIRED_FRAME',*args[1:])
...
def isolated_replay():
    ...
    namespace=dict(function.__globals__,profile=view,
        ...
        ScopedFootprintAnchoredController=BodyProjectedTiledController,
...
def replay(rows,prior_report):
    ...
    return report|dict(baseline='TiledDensityProgressiveFloorController',candidate='BodyProjectedTiledController',
        normalized_state_type_paths=STATE_TYPE_PATHS,incremental_reuse_comparison=False,
        incremental_body_projection_comparison=True,persistent_memory_type_unchanged=True,
        both_controllers_use_original_tiled_classification_and_receipt_handling=True,
