"""Fresh physical maze2 feasibility successor after completed causal integration."""
import argparse
import contextlib
import json
...
from scripts.residual_first_interval_maze_episode_development import collect, artifacts
from scripts.residual_first_interval_maze_audit_development import audit
from scripts.residual_first_interval_native_prefix_development import admit_prefix, compare
from scripts.replay_go2_residual_first_interval_prefix_v1 import (
...
OUTPUT = BASE/'go2_residual_first_interval_maze_pilot_v1_attempt_001'
CASE = ('full_jepa_residual_first_interval_maze_02', *LEARNED_CASE[1:])
PROTOCOL = 'docs/go2_residual_first_interval_maze_pilot_v1_2026-09-09.md'
PREFIX_SHA = '45b9b10e0dc89d4ba477499f817a08df02cc79392c3bf3df9b0363394f7b714b'
...
def verify_inputs(launch):
    ...
    if (launch['planned_case'] != list(CASE) or launch['scene_specification'] != specification(2)
        ...
        raise ValueError('fixed residual feasibility maze2 definition required')
...
def worker(launch_sha):
    ...
    terminal = dict(case=name,layout_index=index,model_name=model_name,
        status='RESIDUAL_FIRST_INTERVAL_WORKER_FAILED',artifact_sha256={}); started = time.perf_counter()
    with (OUTPUT/(name+'_worker.log')).open('x') as log, contextlib.redirect_stdout(log), contextlib.redirect_stderr(log):
        try:
            ...
            terminal.update(status='RESIDUAL_FIRST_INTERVAL_COLLECTED_AND_RAW_AUDITED',artifact_sha256=bindings,
...
def main():
    ...
    if OUTPUT.exists() or OUTPUT.is_symlink(): raise ValueError('exclusive residual feasibility native attempt required')
    ...
    admission = dict(model_state_sha256=old['model_state_sha256'],
        predecessor_verified_round_trip=learned['conditions'][1]['verified_round_trip'])
    ...
    sources = discover_sources((PROTOCOL,'scripts/run_go2_residual_first_interval_maze_pilot_v1.py',
        'lewm/tests/test_residual_first_interval_native_development.py',
        'docs/go2_residual_first_interval_prefix_result_2026-09-09.md'),inherited)
    ...
    launch.update(protocol=PROTOCOL,source_sha256=sources,output_root=str(OUTPUT),planned_case=list(CASE),
        ...
        implementation_class='ResidualFirstIntervalController',robot_urdf_sha256=digest(URDF),
        ...
        residual_first_interval_feasibility_fallback_enabled=True,
        original_forecasts_and_surface_vetoes_preserved=True,
        first_interval_corrected_nominal_geometry_checked=True,
        model_error_bound_applied=False,
    ...
    if args.preflight_only:
        print('RESIDUAL_FIRST_INTERVAL_NATIVE_PREFLIGHT',json.dumps(dict(source_count=len(sources),hardware=resources,
        ...
        return
    if not memory_ok or not storage_ok: raise ValueError('residual feasibility native resources unavailable')
    create_output(OUTPUT); write_json(OUTPUT/'launch.json',launch); launch_sha = digest(OUTPUT/'launch.json')
    print('RESIDUAL_FIRST_INTERVAL_NATIVE_LAUNCHED',launch_sha,flush=True); started = time.perf_counter()
    try:
        ...
        print('RESIDUAL_FIRST_INTERVAL_NATIVE_TERMINAL',record['status'],record.get('verified_round_trip'),record.get('failure'),flush=True)
        if record['status'] != 'RESIDUAL_FIRST_INTERVAL_COLLECTED_AND_RAW_AUDITED':
        ...
        verify_inputs(launch); verify_artifacts(OUTPUT,bindings)
        write_json(OUTPUT/'result.json',dict(status='RESIDUAL_FIRST_INTERVAL_MAZE_PILOT_V1_COMPLETE',
        ...
        print('RESIDUAL_FIRST_INTERVAL_NATIVE_COMPLETE',digest(OUTPUT/'result.json'),flush=True)
    except Exception as error:
        write_json(OUTPUT/'failure.json',dict(status='TERMINAL_RESIDUAL_FIRST_INTERVAL_NATIVE_FAILURE',reason=repr(error)))
        raise
