"""Independent-layout collection with the fixed residual continuation controller.

No entry point or implicit launch. A separately admitted population launcher
must supply its assigned fresh model and exact physics setup definition.
"""
import shutil
import time
import json
...
def collect(layout_index, definition, *, output, model, geometry, episode_name, condition, variant):
    ...
    try:
        ...
        controller = ResidualAnchoredContinuationController(model, geometry, public_mission=mission,
        ...
        result = dict(status='INDEPENDENT_RESIDUAL_ROUND_TRIP_TERMINAL_AUDIT_REQUIRED', layout_index=layout_index,
            ...
            residual_hold_feasibility_enabled=True,residual_anchored_continuation_enabled=True, renderer_witnesses_recorded=True,
        ...
        print('INDEPENDENT_RESIDUAL_ROUND_TRIP_COLLECTED', result, flush=True)
        return result
    finally:
