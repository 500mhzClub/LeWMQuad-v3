"""Fixed sequence test of identical consecutive retained-camera fitting."""
import argparse
import hashlib
import json
...
from lewm.consecutive_retained_camera_pair_development import ConsecutiveRetainedCameraPairMotion
...
def main():
    ...
    output = root/f'depth_noise_{args.sigma_mm}mm_consecutive_retained_tracking_601_v1'
    output.mkdir()
    config = dict(layout_index=i, sigma_mm=args.sigma_mm, seed=SEED,
        frames=FRAME_COUNT, tracker='ConsecutiveRetainedCameraPairMotion',
        ...
        sensor_noise_calibrated=False, source_sha256={str(p):hashlib.sha256(p.read_bytes()).hexdigest()
            for p in map(Path, (__file__, 'lewm/consecutive_retained_camera_pair_development.py',
    ...
    reader = PublicReplay(root/'native'); motion = ConsecutiveRetainedCameraPairMotion()
    ...
    with (output/'frames.jsonl').open('x') as sink:
        for frame in range(FRAME_COUNT):
            ...
            if frame % 100 == 0: print('CONSECUTIVE_RETAINED_FRAME', i, args.sigma_mm, frame, flush=True)
