...
SOURCE='scripts/benchmark_go2_wide_batched_retained_patch_v1.py'
TEST='lewm/tests/test_wide_batched_retained_floor_patch_development.py'
OUTPUT=ROOT/'docs/go2_wide_batched_retained_patch_microbenchmark_2026-09-11.json'
...
def main():
    ...
    verify({PROFILE_WITNESS:PROFILE_WITNESS_SHA})
    inherited=json.loads((ROOT/PROFILE_WITNESS).read_text())['source_sha256']
    sources=discover_sources((SOURCE,TEST,PROFILE_WITNESS),inherited);verify(sources)
    ...
    verify(sources)
    write_json(OUTPUT,dict(status='WIDE_BATCHED_RETAINED_PATCH_MICROBENCHMARK_COMPLETE',
        utc=datetime.now(timezone.utc).isoformat(),source_sha256=sources,source_count=len(sources),hardware=resources,
        cases=results,baseline_frame_batch=32,candidate_frame_batch=128,profile_witness_sha256=PROFILE_WITNESS_SHA,synthetic_history=True,shared_read_only_prefix_arrays=True,isolated_benchmark=False,
    ...
    print('WIDE_PATCH_MICROBENCHMARK_COMPLETE',digest(OUTPUT),len(sources),flush=True)
