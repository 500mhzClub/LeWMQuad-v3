"""Frozen JEPA/direct models on the same four locked development layouts."""
import argparse
...
ROOT = 'go2_persistent_visual_learning_comparison_{arm}_noise_2mm_native_layout{index:02d}_4800_v1_attempt_001'
PLAN = Path('docs/go2_persistent_visual_learning_plan_2026-09-16.json')
...
ASSIGNMENTS = tuple((i, a) for i in range(4) for a in
    (('jepa', 'direct') if i % 2 == 0 else ('direct', 'jepa')))
CONTROLLER_SOURCES = (
...
def prepare():
    ...
    if json.loads(INVENTORY.read_text()) != inventory:
        raise ValueError('same locked four-layout inventory required')
    encoded = INVENTORY.read_text()
    plan = dict(schema='persistent_visual_learning_plan.v1', assignments=ASSIGNMENTS,
        ...
        model_selection_from_new_maze_outcomes=False,
        fixed_before_first_navigation=False,
        fixed_before_first_navigation_of_assigned_models_on_these_layouts=True,
        comparison='same controller, frozen JEPA and direct models; compare with completed supervised/pose-command arms',
        computations_matched='same architecture and forecast consumers; assigned frozen training condition and output head',
        ...
        limitations=['one training seed', 'same procedural maze family', 'ideal gyro',
            'not a prediction-on/off experiment', 'single-seed training comparison',
            'layouts already used by frozen supervised/pose-command arms', 'measured simulation, not real time'])
...
def main():
    ...
    if args.evaluate:
        ...
        return
    predecessor = study.BASE/'go2_persistent_visual_transfer_supervised_rollout_noise_2mm_native_layout03_4800_v1_attempt_001'
    if not (predecessor/'short_pulse_navigation_evaluation_v1.json').is_file():
        raise ValueError('finish the eight fixed supervised/pose-command missions first')
    ...
    def write(name, value):
        if name == 'launch.json':
            value = value | dict(experiment='persistent_visual_learning_comparison_v1',
                ...
                layout_novelty_scope='same locked four layouts, previously executed by frozen supervised and pose-command arms',
                planned_layout_count=4, planned_native_assignments=8,
                neural_reference_is_unused_for_control=False,
                ...
                reference_maximum_observation_age_ns=250_000_000, world_model_changed=True,
                training_condition=arm, controller_changed=False,
    ...
    with ProcessPoolExecutor(max_workers=1, mp_context=get_context('spawn'),
        ...
        def runtime(model, **kwargs):
            return PersistentLocalVisualRuntime(model, prediction_source='neural',
