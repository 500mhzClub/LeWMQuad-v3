#!/usr/bin/env python3
"""Authority-free V15 executor for the frozen N5 experiment.
...
if __name__ == "__main__":
    ...
    STAGING_PREFIX = ".n5.reservation-v15-"
    LOCK_NAME = ".n5.reservation-v15.lock"
    STAGING_SCHEMA = "lewm_go2_n5_gate_aligned_raster_nll_v15_preclaim_staging_v1"
    RECOVERY_POLICY = {
    ...
    def _directory_flags() -> int:
        ...
        if not nofollow or not directory:
            raise PermissionError("V15 canonical directory walks require no-follow opens")
        return os.O_RDONLY | nofollow | directory | getattr(os, "O_CLOEXEC", 0)


    @dataclass
    class DirectoryEntryV15:
        parent_fd: int
        name: str
        child_fd: int
    ...
    class DirectorySnapshotV15:
        fingerprint: tuple[int, ...]
        inventory: tuple[tuple[str, tuple[int, ...]], ...]


    @dataclass(frozen=True)
    class JournalEventV15:
        watch: int
        mask: int
        cookie: int
        name: str
    ...
    @dataclass
    class DirectoryWatchV15:
        watch: int
        generation: int
        directory_fd: int
        identity: tuple[int, int]
        label: str
        snapshot: DirectorySnapshotV15


    class OwnedDirectoryJournalV15:
        ...
        def __init__(self) -> None:
            ...
            if renameat2 is None:
                ...
                raise PermissionError(
                    "V15 requires Linux renameat2 for no-replace publication"
                )
            renameat2.argtypes = [
                ctypes.c_int,
                ctypes.c_char_p,
            ...
            self._fd = descriptor
            self._by_watch: dict[int, DirectoryWatchV15] = {}
            self._by_identity: dict[tuple[int, int], DirectoryWatchV15] = {}
            self._generation = 0
        ...
        def _fail(self, reason: str) -> None:
            if self._poison is None:
                self._poison = reason
            raise PermissionError(f"V15 owned-directory journal rejected: {reason}")

        @staticmethod
        def _identity(directory_fd: int) -> tuple[int, int]:
            metadata = os.fstat(directory_fd)
            if not stat.S_ISDIR(metadata.st_mode):
                raise PermissionError("V15 journal target is not a directory")
        ...
        @staticmethod
        def _snapshot(directory_fd: int) -> DirectorySnapshotV15:
            opened = os.fstat(directory_fd)
            if not stat.S_ISDIR(opened.st_mode):
                raise PermissionError("V15 journal directory descriptor changed")
            rows: list[tuple[str, tuple[int, ...]]] = []
            for name in sorted(os.listdir(directory_fd)):
                if Path(name).name != name or name in {"", ".", ".."}:
                    raise PermissionError("V15 journal observed a malformed child name")
                metadata = os.stat(
                    name,
                    dir_fd=directory_fd,
            ...
            return DirectorySnapshotV15(
                fingerprint=_stable_fingerprint(opened),
                inventory=tuple(rows),
            )

        def _events(self) -> list[JournalEventV15]:
            events: list[JournalEventV15] = []
            while True:
                try:
        ...
        def _reject_special(self, events: Sequence[JournalEventV15]) -> None:
            for event in events:
        ...
        def _shared_parent_events(
            self,
            events: Sequence[JournalEventV15],
            *,
            watch: int,
            reserved_name: str,
            allow_ignored: bool = False,
        ) -> tuple[list[JournalEventV15], list[JournalEventV15]]:
            ...
            reserved: list[JournalEventV15] = []
            ignored: list[JournalEventV15] = []
            for event in events:
                if event.mask & self.IN_Q_OVERFLOW:
                ...
                if event == JournalEventV15(watch, self.IN_IGNORED, 0, ""):
        ...
        def create_exclusive_root(
            ...
            *,
            ...
            unrelated child names are tolerated, while the V15 name must have
            ...
            try:
                ...
                if ignored != [
                    JournalEventV15(watch, self.IN_IGNORED, 0, "")
                ]:
        ...
        def watch_directory(
            ...
            *,
            label: str,
            expected_snapshot: DirectorySnapshotV15 | None = None,
        ) -> None:
            self.assert_clean()
        ...
        def _state(self, directory_fd: int) -> DirectoryWatchV15:
            identity = self._identity(directory_fd)
        ...
        def baseline(self, directory_fd: int) -> DirectorySnapshotV15:
            return self._state(directory_fd).snapshot

        def _snapshots(self) -> dict[int, DirectorySnapshotV15]:
            return {
        ...
        def _validate_snapshots(
            self,
            before: Mapping[int, DirectorySnapshotV15],
            after: Mapping[int, DirectorySnapshotV15],
            *,
        ...
        def _commit_snapshots(
            self,
            snapshots: Mapping[int, DirectorySnapshotV15],
        ) -> None:
        ...
        def _begin(
            self,
            directory_fd: int,
        ) -> tuple[DirectoryWatchV15, dict[int, DirectorySnapshotV15]]:
            self.assert_clean()
        ...
        def _finish(
            ...
            before: Mapping[int, DirectorySnapshotV15],
            *,
            expected_events: Sequence[JournalEventV15],
            expected_inventory: tuple[tuple[str, tuple[int, ...]], ...],
        ) -> DirectorySnapshotV15:
        ...
        def mkdir(self, parent_fd: int, name: str, *, mode: int = 0o700) -> None:
            ...
            self._finish(
                ...
                before,
                expected_events=(
                    JournalEventV15(state.watch, self.IN_CREATE | self.IN_ISDIR, 0, name),
                ),
        ...
        def create_file(
            ...
            *,
            ...
            expected_events = [
                JournalEventV15(state.watch, self.IN_CREATE, 0, name),
            ]
            if payload:
                expected_events.append(
                    JournalEventV15(state.watch, self.IN_MODIFY, 0, name)
                )
            expected_events.append(
                JournalEventV15(state.watch, self.IN_CLOSE_WRITE, 0, name)
            )
            expected = self._replace_inventory(
        ...
        def unlink(
            ...
            *,
            ...
            self._finish(
                ...
                before,
                expected_events=(
                    JournalEventV15(state.watch, self.IN_DELETE, 0, name),
                ),
                expected_inventory=expected,
            )
        ...
        def rmdir(
            ...
            *,
            ...
            if retired != [
                JournalEventV15(child.watch, self.IN_DELETE_SELF, 0, ""),
                JournalEventV15(child.watch, self.IN_IGNORED, 0, ""),
            ]:
    ...
    @dataclass
    class CanonicalDirectoryChainV15:
        anchor_fd: int
        anchor_identity_security: tuple[int, ...]
        descriptors: list[int]
        entries: list[DirectoryEntryV15]
        path_fds: dict[Path, int]
        journal: OwnedDirectoryJournalV15
        output_root_created: bool = False
        closed: bool = False


    @dataclass(frozen=True)
    class OwnedArtifactV15:
        role: str
        parent_fd: int
        name: str
        fingerprint: tuple[int, ...]
    ...
    def _assert_directory_chain(chain: CanonicalDirectoryChainV15) -> None:
        if chain.closed:
            raise PermissionError("V15 canonical directory chain is closed")
        chain.journal.assert_clean()
        if (
            _identity_security_fingerprint(os.fstat(chain.anchor_fd))
            != chain.anchor_identity_security
        ):
            raise PermissionError("V15 filesystem-root descriptor changed")
        for entry in chain.entries:
            ...
            if (
                ...
                raise PermissionError("V15 canonical directory component changed")


    def _open_canonical_directory_chain(final_path: Path) -> CanonicalDirectoryChainV15:
        ...
        if (
            ...
            raise PermissionError("V15 canonical directory target escaped the repository")
        ...
        journal = OwnedDirectoryJournalV15()
        chain = CanonicalDirectoryChainV15(
            anchor_fd=anchor_fd,
        ...
        try:
            if (
                ...
                raise PermissionError("V15 filesystem root changed during open")
            parent_fd = anchor_fd
            ...
            for index, component in enumerate(final_path.parts[1:]):
                ...
                try:
                    ...
                    if index < repository_depth:
                        raise PermissionError("V15 repository root component is missing")
                    if current_path / component == policy.CANONICAL_OUTPUT_ROOT:
                        ...
                        if not chain.output_root_created:
                            raise PermissionError(
                                "existing V15 recovery tree is missing a structural directory"
                            )
                    ...
                    if (
                        ...
                        raise PermissionError(
                            "V15 exclusive root changed before retained open"
                        )
                if stat.S_ISLNK(before.st_mode) or not stat.S_ISDIR(before.st_mode):
                    raise PermissionError("V15 canonical component is not a real directory")
                ...
                if (
                    ...
                    raise PermissionError("V15 canonical component changed during open")
                entry = DirectoryEntryV15(
                    parent_fd,
                    component,
                    child_fd,
                    identity_security,
    ...
    def _open_chain_child(
        ...
        try:
            ...
            if not chain.output_root_created:
                raise PermissionError(
                    "existing V15 recovery tree is missing a derived directory"
                )
            chain.journal.mkdir(parent_fd, name, mode=0o700)
            before = _entry_metadata(parent_fd, name)
        if stat.S_ISLNK(before.st_mode) or not stat.S_ISDIR(before.st_mode):
            raise PermissionError("V15 derived parent is not a real directory")
        identity_security = _identity_security_fingerprint(before)
        full_fingerprint = _stable_fingerprint(before)
        ...
        if (
            ...
            raise PermissionError("V15 derived parent changed during open")
    ...
    def _close_directory_chain(chain: CanonicalDirectoryChainV15) -> None:
        if chain.closed:
            return
        chain.closed = True
        chain.journal.close()
    ...
    class AttemptReservationV15:
        ...
        directory_chain: CanonicalDirectoryChainV15 | None = None
        owned_claim_artifacts: dict[str, OwnedArtifactV15] = field(
            default_factory=dict,
            compare=False,
        )
        owned_derived_artifacts: dict[str, OwnedArtifactV15] = field(
            default_factory=dict,
            compare=False,
        )
        ...
        @property
        def journal(self) -> OwnedDirectoryJournalV15:
            if self.directory_chain is None:
                raise PermissionError("N5 full-panel V15 lacks its transaction journal")
            return self.directory_chain.journal

    def _assert_claim_fd_owned(reservation: AttemptReservationV15) -> None:
        if reservation.directory_fd < 0 or reservation.directory_identity is None:
            raise PermissionError("N5 full-panel V15 lacks an open claimed directory")
        opened = os.fstat(reservation.directory_fd)
        if not stat.S_ISDIR(opened.st_mode):
            raise PermissionError("N5 full-panel V15 claimed descriptor is not a directory")
        if (opened.st_dev, opened.st_ino) != reservation.directory_identity:
            raise PermissionError("N5 full-panel V15 claimed descriptor identity changed")


    def _assert_owned_claim(reservation: AttemptReservationV15) -> None:
        ...
        if (
            ...
            raise PermissionError("N5 full-panel V15 claimed directory changed")
        if reservation.directory_chain is None or reservation.seed_root_fd < 0:
            raise PermissionError("N5 full-panel V15 lacks its canonical directory chain")
        ...
        if (
            ...
            raise PermissionError("N5 full-panel V15 canonical claim identity changed")
        ...
        if set(os.listdir(reservation.directory_fd)) != expected_names:
            raise PermissionError("N5 full-panel V15 claimed directory inventory changed")


    def _write_claim_file_exclusive(
        reservation: AttemptReservationV15,
        name: str,
        payload: bytes,
        *,
        mode: int = 0o600,
        ...
        if Path(name).name != name or name in {"", ".", ".."}:
            raise PermissionError("N5 full-panel V15 claim filename escaped")
        ...
        reservation.owned_claim_artifacts[name] = OwnedArtifactV15(
            role=role or name,
        ...
        if _stable_fingerprint(committed) != committed_fingerprint:
            raise PermissionError("N5 full-panel V15 committed claim leaf changed")
        if require_canonical:
            _assert_owned_claim(reservation)
        else:
            _assert_claim_fd_owned(reservation)
        ...
        *,
        ...
        try:
            ...
            if (
                ...
                raise PermissionError("N5 full-panel V15 claim leaf is not singly linked")
            chunks: list[bytes] = []
            ...
            if fingerprint(before) != fingerprint(after):
                raise RuntimeError("N5 full-panel V15 claim leaf changed while read")
        finally:
            os.close(descriptor)
        if require_canonical:
            _assert_owned_claim(reservation)
    ...
    def _reservation_core(
        review_binding: Mapping[str, str],
        gpu_visibility_binding: Mapping[str, Any],
        *,
        ...
        return {
            ...
            "gpu_visibility": dict(gpu_visibility_binding),
    ...
    def _classify_staging(
        ...
        *,
        ...
        review_binding: Mapping[str, str],
        gpu_visibility_binding: Mapping[str, Any],
    ) -> tuple[dict[str, Any], AttemptReservationV15 | None]:
        ...
        valid = (
            ...
            and _validate_recoverable_reservation(
                ...
                gpu_visibility_binding=gpu_visibility_binding,
            )
        )
        if not valid:
            evidence["classification"] = "mutated"
        ...
        journal.assert_clean()
        return evidence, AttemptReservationV15(
            directory=attempt_path,
    ...
    def _complete_recoveries_are_equivalent(
        first: AttemptReservationV15,
        second: AttemptReservationV15,
    ) -> bool:
    ...
    def _validate_recovery_tree_baseline(
        ...
        """Reject historical bytes outside the sole recoverable V15 shape."""

        chain.journal.assert_clean()
        relative_seed = seed_root.relative_to(policy.CANONICAL_OUTPUT_ROOT)
        if len(relative_seed.parts) != 2:
            chain.journal._fail("V15 recovery seed-root depth changed")
        ...
        if not required_paths <= set(chain.path_fds):
            chain.journal._fail("V15 recovery tree lacks retained descriptors")
    ...
    def _reserve_exact_attempt(
        ...
        active_staging: OwnedStagingV15 | None = None
        owned_claim_identity: tuple[int, int] | None = None
        claimed_directory_fd: int | None = None
        claimed_chain: CanonicalDirectoryChainV15 | None = None
        claimed_reservation: AttemptReservationV15 | None = None
        ...
        try:
            ...
            with _locked_seed_root(
                ...
                complete: list[
                    tuple[OwnedStagingV15, AttemptReservationV15, dict[str, Any]]
                ] = []
                ...
                if (
                    ...
                    raise PermissionError("N5 full-panel V15 staging identity changed")
                owned_claim_identity = (staging_metadata.st_dev, staging_metadata.st_ino)
                claimed_reservation = AttemptReservationV15(
                    directory=attempt_path,
    ...
    def _cleanup_owned_artifacts(
        registry: dict[str, OwnedArtifactV15],
        *,
        journal: OwnedDirectoryJournalV15,
    ...
    def _terminate_failure(
        ...
        *,
        ...
        if isinstance(error, VerifierChildFailureV15):
            try:
    ...
    def _publish_success(
        ...
        *,
        ...
        if not policy.is_sha256(checkpoint_content_sha256):
            raise ValueError("N5 full-panel V15 checkpoint content hash is malformed")
    ...
    def _write_canonical_json(
        ...
        if path not in {policy.CANONICAL_METRIC_RECEIPT_PATH, policy.CANONICAL_GATE_PATH}:
            raise PermissionError("N5 full-panel V15 publication path is not canonical")
        expected_schema = (
        ...
        if (
            ...
            raise ValueError("N5 full-panel V15 derived artifact is malformed")
        raw = policy.canonical_json_bytes(value) + b"\n"
        relative = path.relative_to(policy.CANONICAL_OUTPUT_ROOT)
        if len(relative.parts) != 2:
            raise PermissionError("N5 full-panel V15 publication depth changed")
        if reservation.directory_chain is None or reservation.output_root_fd < 0:
            raise PermissionError("N5 full-panel V15 lacks its retained output root")
        ...
        if parent_fd < 0:
            raise PermissionError("N5 full-panel V15 lacks its derived parent descriptor")
        ...
        reservation.owned_derived_artifacts[relative_text] = OwnedArtifactV15(
        ...
        if _stable_fingerprint(committed) != committed_fingerprint:
            raise PermissionError("N5 full-panel V15 committed derived leaf changed")
        _assert_owned_claim(reservation)
        return policy.artifact_binding(
            relative_text,
            raw,
    ...
    def _read_owned_derived_artifact(
        ...
        if artifact is None:
            raise PermissionError("N5 full-panel V15 derived artifact is not owned")
        raw = _read_regular_at(
            artifact.parent_fd,
            artifact.name,
            expected_fingerprint=artifact.fingerprint,
        ...
        if hashlib.sha256(raw).hexdigest() != artifact.payload_sha256:
            raise PermissionError("N5 full-panel V15 derived payload changed")
        _assert_owned_claim(reservation)
        return raw
    ...
    def _run_frozen_training(
        ...
        *,
        rgb_workers: int,
    ) -> tuple[dict[str, Any], AttemptReservationV15]:
        """Run the reviewed V15 science behind an ephemeral local adapter."""
        ...
        def require(value: object) -> object:
            if value is not token:
                raise PermissionError("retained science lacks its local V15 execution token")
        ...
        def reserve(value: object) -> AttemptReservationV15:
            nonlocal claimed_reservation
            require(value)
            if claimed_reservation is not None:
                raise FileExistsError("the sole V15 attempt was already reserved")
            claimed_reservation = _reserve_exact_attempt(
                source_review_file_sha256,
                gpu_visibility_binding,
            )
            return claimed_reservation
        ...
        try:
            ...
            if claimed_reservation is None:
                raise RuntimeError(
                    "frozen V15 training returned without claiming its attempt"
                )
            _assert_owned_claim(claimed_reservation)
            launch_summary = {
                **summary,
                "schema": "lewm_go2_observable_camera_ray_fit_v4_n5_gate_aligned_raster_nll_v15_launch_summary_v1",
            }
    ...
    @dataclass(frozen=True)
    class ChildProcessObservationV15:
        returncode: int
        signal: int | None
        timed_out: bool
        stdout_raw: bytes
    ...
    class VerifierChildFailureV15(RuntimeError):
        def __init__(
            self,
            reason: str,
            observation: ChildProcessObservationV15,
    ...
    class SmokeInjectedFailureV15(RuntimeError):
        pass
    ...
    def _parse_failure_envelope(
        ...
        try:
            value = policy.parse_json(raw, name="V15 verifier failure envelope")
        ...
        try:
            checked = _self_hashed_value(value, name="V15 verifier failure envelope")
        except (TypeError, ValueError):
            return None, None, "malformed"
        if (
    ...
    def _observe_child_process(
        ...
        *,
        ...
        return ChildProcessObservationV15(
    ...
    def _verification_child_environment() -> dict[str, str]:
        ...
        if _child_environment_projection(environment) != _expected_child_environment():
            raise PermissionError("V15 verifier child environment sanitization failed")
        return environment
    ...
    def _verification_request(
        ...
        if not sys.flags.isolated:
            raise PermissionError("V15 verifier parent must already be isolated")
        _assert_owned_claim(reservation)
        review, _ = policy.preflight_source_review(
            policy.CANONICAL_SOURCE_REVIEW_PATH,
            source_review_file_sha256,
        ...
        if len(raw) > policy.VERIFICATION_MAX_REQUEST_BYTES:
            raise ValueError("V15 verifier request exceeds its frozen size bound")
        _assert_owned_claim(reservation)
        return request, raw
    ...
    def _validate_verification_request(
        ...
        value = _self_hashed_value(request, name="V15 verifier request")
        expected_fields = {
            "schema",
            "nonce",
            "source_review",
        ...
        if (
            ...
            raise PermissionError("V15 verifier request contract changed")
        ...
        if (
            ...
            raise PermissionError("V15 verifier request process binding changed")
        source_review = value.get("source_review")
        if not isinstance(source_review, Mapping):
            raise PermissionError("V15 verifier request source review is malformed")
        review, _ = policy.preflight_source_review(
            policy.CANONICAL_SOURCE_REVIEW_PATH,
        ...
        if (
            ...
            raise PermissionError("V15 verifier request source binding changed")
        ...
        if (
            ...
            raise PermissionError("V15 verifier request artifact map changed")
        for role, expected_path in expected_paths.items():
            bound = artifacts.get(role)
            if not isinstance(bound, str):
                raise PermissionError("V15 verifier artifact binding is malformed")
            path, digest = policy.parse_bound_path(bound)
            if path != expected_path.resolve(strict=True) or not policy.is_sha256(digest):
                raise PermissionError("V15 verifier artifact binding changed")
            ...
            if (
                ...
                raise PermissionError("V15 verifier artifact content binding changed")
        policy.preflight_static_authority()
        return value
    ...
    def _compute_verification_receipt_child(
        ...
        if (
            ...
            raise PermissionError("V15 verifier child process boundary changed")
        ...
        def require(candidate: object) -> object:
            if candidate is not token:
                raise PermissionError(
                    "retained verifier lacks its local V15 child token"
                )
            policy.preflight_static_authority()
            policy.preflight_source_review(
                policy.CANONICAL_SOURCE_REVIEW_PATH,
        ...
        def forbid_publication(*_args: Any, **_kwargs: Any) -> None:
            raise PermissionError("V15 verifier child cannot publish")
    ...
    def _load_claim_json(
        ...
        raw = _read_claim_file(reservation, name)
        value = policy.parse_json(raw, name=f"V15 parent {name}")
        if raw != policy.canonical_json_bytes(value) + b"\n":
            raise ValueError(f"V15 parent {name} is not canonical JSON")
        return _self_hashed_value(value, name=f"V15 parent {name}"), raw
    ...
    def _validate_child_metric_receipt_provenance(
        ...
        value = _self_hashed_value(receipt, name="V15 child metric receipt")
        expected_fields = {
            "schema",
            "authoritative",
        ...
        for role, binding in expected_artifacts.items():
            ...
            if digest != binding["file_sha256"]:
                raise PermissionError("V15 child request artifact changed")
        ...
        if (
            ...
            raise PermissionError("V15 child metric receipt provenance changed")
        _assert_owned_claim(reservation)
    ...
    def _validate_child_metric_receipt(
        ...
        if (
            ...
            raise ValueError("V15 child frozen metric decision changed")
        _assert_owned_claim(reservation)
        return value
    ...
    def _validate_verification_response(
        ...
        value = _self_hashed_value(response, name="V15 verifier response")
        expected_fields = {
            "schema",
            "status",
            "nonce",
        ...
        if (
            ...
            raise PermissionError("V15 verifier response binding changed")
        receipt = value.get("receipt")
        if not isinstance(receipt, Mapping):
            raise ValueError("V15 verifier response lacks a receipt")
        ...
        if value.get("receipt_content_sha256") != checked["content_sha256"]:
            raise PermissionError("V15 verifier response receipt binding changed")
        return checked
    ...
    def _run_independent_verification(
        ...
        try:
            ...
            response = policy.parse_json(
                response_raw,
                name="V15 verifier child response",
            )
            if response_raw != policy.canonical_json_bytes(response) + b"\n":
                raise ValueError(
                    "V15 verifier child response is not one canonical JSON value"
                )
            ...
            raise VerifierChildFailureV15(
                "V15 verifier boundary failed; no fallback",
                observation,
                request,
            ) from error
        _assert_owned_claim(reservation)
    ...
    def _verification_child_main() -> int:
        ...
        try:
            ...
            if not sys.flags.isolated or not sys.flags.dont_write_bytecode:
                raise PermissionError("V15 verifier child requires -I -B")
            ...
            if not raw or len(raw) > policy.VERIFICATION_MAX_REQUEST_BYTES:
                raise ValueError("V15 verifier child request size changed")
            phase_state.enter("request_parse")
            request = policy.parse_json(raw, name="V15 verifier child request")
            if raw != policy.canonical_json_bytes(request) + b"\n":
                raise ValueError("V15 verifier child request is not canonical JSON")
            ...
            if len(response_raw) > policy.VERIFICATION_MAX_RESPONSE_BYTES:
                raise ValueError("V15 verifier child response exceeds its size bound")
    ...
    def _smoke_child_environment(
        *,
        ...
        for name in (
            ...
            "PYTHONUSERBASE",
            "HSA_OVERRIDE_GFX_VERSION",
            "LEWM_V15_SMOKE_INJECTION_PHASE",
            "LEWM_V15_SMOKE_PROCESS_CASE",
        ...
        for name in policy.THREAD_ENVIRONMENT:
            environment[name] = "1"
        if smoke_mode:
            environment["LEWM_V15_VERIFIER_MODE"] = "cpu_contract_smoke_v1"
        if injection_phase is not None:
            ...
            environment["LEWM_V15_SMOKE_INJECTION_PHASE"] = injection_phase
        if process_case is not None:
            ...
            environment["LEWM_V15_SMOKE_PROCESS_CASE"] = process_case
    ...
    def _smoke_request(temporary_directory: Path) -> tuple[dict[str, Any], bytes]:
        ...
        gate_evaluation = complete_retained_gate_evaluation_v15()
        core = {
            ...
            "bundle": {
                "schema": "lewm_v15_cpu_verifier_smoke_bundle_v1",
                "production_eligible": False,
                "frame_count": 5,
                "frames": frames,
    ...
    def _smoke_enter(phase_state: Any, phase: str) -> None:
        ...
        if os.environ.get("LEWM_V15_SMOKE_INJECTION_PHASE") == phase:
            raise SmokeInjectedFailureV15("injected verifier smoke phase failure")
    ...
    def _cpu_verifier_smoke_child_main() -> int:
        ...
        process_case = os.environ.get("LEWM_V15_SMOKE_PROCESS_CASE")
        ...
        try:
            ...
            if (
                ...
                or bundle.get("schema") != "lewm_v15_cpu_verifier_smoke_bundle_v1"
            ...
            independent_cell_nll = (
                verifier.derived_raster_cell_nll_for_verification_v15(
                    smoke_probabilities,
                    smoke_labels,
                )
            )
            ...
            compatibility_view = (
                policy.adapt_native_v15_evaluation_for_retained_v4_gate(
                    gate_evaluation
                )
            )
    ...
    def run_cpu_verifier_contract_smoke() -> dict[str, Any]:
        ...
        temporary_path: Path
        with tempfile.TemporaryDirectory(prefix="lewm-v15-verifier-smoke-") as name:
            ...
            cases: dict[str, ChildProcessObservationV15] = {}
            for process_case in (
                "timeout",
                "signal",
                "nonzero",
        ...
        return {
            "schema": "lewm_v15_cpu_verifier_contract_smoke_summary_v1",
            "production_eligible": False,
            ...
            "independent_v15_raster_nll_recomputed": True,
    ...
    def _run_finalization(
        ...
        metric = policy.parse_json(metric_raw, name="V15 finalizer metric receipt")
        if metric_raw != policy.canonical_json_bytes(metric) + b"\n":
            raise ValueError("V15 finalizer metric receipt is not canonical JSON")
        ...
        if (
            ...
            raise ValueError("V15 finalizer frozen metric decision changed")
    ...
    def _gpu_visibility_reservation_binding(
        *,
        receipt: Mapping[str, Any],
    ...
    def _preflight_exact_output_freshness() -> dict[str, bool]:
        """Reject every pre-existing V15 namespace without creating anything."""

        for path in (policy.CANONICAL_OUTPUT_ROOT, policy.CANONICAL_ATTEMPT_PATH):
            try:
                os.lstat(path)
            except FileNotFoundError:
                continue
            raise FileExistsError(
                "the sole N5 full-panel V15 output namespace is not fresh"
            )
        return {
            "output_root_absent": True,
            "attempt_slot_absent": True,
            "output_mutation_count": 0,
        }


    def _active_generated_mutator_pids(
        *,
        proc_root: Path = Path("/proc"),
        current_pid: int | None = None,
        generated_root: Path | None = None,
    ) -> list[int]:
        """Find other processes with writable descriptors below `.generated`."""
        ...
        try:
            ...
            raise PermissionError("V15 cannot inspect process descriptor state") from error
        for process in processes:
            ...
            for descriptor in descriptors:
                try:
                    ...
                    if target != root_text and not target.startswith(root_text + "/"):
                        continue
    ...
    def _preflight_generated_mutator_quiescence() -> dict[str, Any]:
        active = _active_generated_mutator_pids()
        if active:
            raise PermissionError(
                "another process has writable .generated descriptors: "
                + ",".join(str(pid) for pid in active)
            )
        return {
            "observable_writable_generated_descriptor_pid_count": 0,
            "checked_before_v15_output_mutation": True,
        }
    ...
    def execute_exact(
        source_review_file_sha256: str,
        *,
        gpu_visibility_receipt_file_sha256: str,
        gpu_visibility_receipt_content_sha256: str,
        rgb_workers: int,
    ) -> dict[str, Any]:
        """Own the complete canonical lifecycle; no stage authority escapes."""

        if not sys.flags.isolated:
            raise PermissionError("N5 full-panel V15 exact execution requires isolation")
        ...
        repository_commit = gpu_visibility.current_reviewed_git_commit(review)
        receipt = gpu_visibility.validate_fixed_receipt(
            expected_file_sha256=gpu_visibility_receipt_file_sha256,
            expected_content_sha256=gpu_visibility_receipt_content_sha256,
            expected_source_review=review,
            expected_source_review_file_sha256=source_review_file_sha256,
            expected_repository_commit=repository_commit,
        )
        live_observation = gpu_visibility.observe_visibility()
        gpu_visibility.require_passing_observation(live_observation)
        visibility_binding = _gpu_visibility_reservation_binding(
            receipt=receipt,
            receipt_file_sha256=gpu_visibility_receipt_file_sha256,
            receipt_content_sha256=gpu_visibility_receipt_content_sha256,
            repository_commit=repository_commit,
        )
        ...
        try:
            ...
            _assert_owned_claim(reservation)
            return {
                "schema": "lewm_go2_observable_camera_ray_fit_v4_n5_gate_aligned_raster_nll_v15_end_to_end_summary_v1",
                "seed": 20260710,
                "fit_size": 5,
                "training": training,
                "gpu_visibility": visibility_binding,
            ...
            try:
                ...
                raise RuntimeError(
                    f"N5 full-panel V15 {stage} failed and terminal receipt "
                    "could not be written"
                ) from terminal_error
            raise
        finally:
    ...
    def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
        ...
        parser.add_argument("--gpu-visibility-receipt-sha256")
        parser.add_argument("--gpu-visibility-receipt-content-sha256")
        ...
        if args.cpu_contract_smoke:
            if any(
                ...
                for value in (
                    ...
                    args.gpu_visibility_receipt_sha256,
                    args.gpu_visibility_receipt_content_sha256,
                )
            ):
                raise ValueError("CPU smoke does not accept exact-launch hashes")
            return args
        if not policy.is_sha256(args.source_review_sha256):
            raise ValueError("N5 full-panel V15 source review SHA-256 is malformed")
        if not policy.is_sha256(args.gpu_visibility_receipt_sha256):
            raise ValueError("N5 V15 visibility receipt SHA-256 is malformed")
        if not policy.is_sha256(args.gpu_visibility_receipt_content_sha256):
            raise ValueError("N5 V15 visibility receipt content SHA-256 is malformed")
        return args
    ...
    def dispatch(argv: Sequence[str] | None = None) -> int:
        ...
        if raw_argv == ["--verification-child"]:
            if not sys.flags.isolated:
                raise PermissionError(
                    "V15 verifier child cannot self-authorize isolation"
                )
            if (
                os.environ.get("LEWM_V15_VERIFIER_MODE")
        ...
        if not sys.flags.isolated:
            ...
            raise PermissionError(
                "V15 exact execution must be invoked explicitly with Python -I -B"
            )
        ...
        summary = execute_exact(
            args.source_review_sha256,
            gpu_visibility_receipt_file_sha256=(
                args.gpu_visibility_receipt_sha256
            ),
            gpu_visibility_receipt_content_sha256=(
                args.gpu_visibility_receipt_content_sha256
            ),
