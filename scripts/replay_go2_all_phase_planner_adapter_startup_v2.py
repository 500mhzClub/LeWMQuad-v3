"""V2: unchanged observed state with the failure-distance display distinguished."""
...
OUTPUT = BASE/'go2_all_phase_planner_adapter_startup_v2_attempt_001'
SOURCE = 'scripts/replay_go2_all_phase_planner_adapter_startup_v2.py'
PROTOCOL = 'docs/go2_all_phase_planner_adapter_startup_v2_2026-09-10.md'
...
def replay_case(case, launch):
    ...
    with writer(dest) as append:
        for row in islice(read_rows(directory), 4):
            ...
            observed_check = compare_observed(old, new, frame=frame)
            ...
            receipt = dict(tick=frame, decision=new, original_complete_decision_reconstructed=True,
                public_input_sha256=before, public_input_arrays_unchanged=True, observed_comparison=observed_check,
...
def main():
    ...
    sources = discover_sources((SOURCE, PROTOCOL, TEST,
        'scripts/replay_go2_all_phase_planner_adapter_startup_v1.py',
        'docs/go2_all_phase_planner_adapter_startup_v1_2026-09-10.md',
        'lewm/tests/test_all_phase_adapter_prefix_comparison_development.py'), queued['source_sha256']); verify(sources)
    resources = hardware()
