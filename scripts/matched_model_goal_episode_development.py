...
def collect(trial, definition, *, output, model, geometry, persistent, episode_name, condition, variant):
    ...
    try:
        ...
        session._runtime['high_level'] = 'matched_model_goal_probe_v1; online model plus measured surfaces'
        ...
        controller = MatchedModelGoalProbe(model, geometry, persistent=persistent, condition=condition, variant=variant)
        try:
        ...
        result = dict(status='MATCHED_MODEL_GOAL_PROBE_TERMINAL_AUDIT_REQUIRED', trial=trial,
        ...
        print('MATCHED_MODEL_GOAL_PROBE', trial, result, flush=True)
        return result
    finally:
