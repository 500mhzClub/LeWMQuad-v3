"""Twenty frozen prospective maze missions with the camera scheduling repair."""
import argparse
...
from lewm import nogil_navigation_replication_layouts_development as layouts
...
BASE = study.BASE
ROOT = 'go2_nogil_replication_{arm}_noise_2mm_native_layout{index:02d}_4800_v1_attempt_001'
PLAN = Path('docs/go2_nogil_navigation_replication_plan_2026-09-16.json')
INVENTORY = Path('docs/go2_nogil_navigation_replication_layout_inventory_2026-09-16.json')
...
class InstantaneousRuntime(InstantaneousWaypointScoreMixin, LivePlanningStageProfileRuntime):
    pass


class ReactiveRuntime(ComputedForecastsUnusedMixin, ReactiveFeedbackMixin, LivePlanningStageProfileRuntime):
    pass


RUNTIMES = dict(jepa=LivePlanningStageProfileRuntime,
    supervised_rollout=LivePlanningStageProfileRuntime, pose_command=LivePlanningStageProfileRuntime,
...
def prepare():
    ...
    plan = dict(schema='nogil_navigation_replication_plan.v1', assignments=ASSIGNMENTS,
...
def main():
    ...
    def write(name, value):
        if name == 'launch.json':
            value = value | dict(experiment='nogil_navigation_replication_v1', study_arm=arm,
                ...
                layout_novelty_scope='distinct_from_explicit_88_layout_registry',
