...
from lewm.measured_pose_seeded_rgbd_development import MeasuredPoseSeededVisualLedMotion
...
OUTPUT = BASE / 'go2_measured_pose_correspondence_replay_v1_attempt_001'
SOURCE = 'scripts/replay_go2_measured_pose_correspondence_v1.py'
TEST = 'lewm/tests/test_measured_pose_correspondence_replay_development.py'
PROTOCOL = 'docs/go2_measured_pose_correspondence_replay_v1_2026-09-07.md'
...
ROTATION_ONLY = BASE / 'go2_gyro_seeded_correspondence_replay_v1_attempt_001'
ROTATION_RECEIPTS = {'launch.json': 'e4bfb91e49acf8ce29358966c8671bddfed16206fd49f1b74daff7a7cfa359c4',
    'result.json': 'afe3baba0c13f542d4ea481b1814dd8a30183156bfb24754dc5baa31b08af697'}
MODELS = dict(original=MultiReferenceVisualLedMotion, measured_pose=MeasuredPoseSeededVisualLedMotion)
...
def preflight():
    ...
    verify_artifacts(ROTATION_ONLY, ROTATION_RECEIPTS)
    rotation_launch = read_json(ROTATION_ONLY, 'launch.json'); verify_launch(rotation_launch)
    require(rotation_launch['predecessor_replay_sha256'] == witness, 'same original replay witness required')
    rotation_result = read_json(ROTATION_ONLY, 'result.json')
    require(rotation_result['status'] == 'GYRO_SEEDED_PAIRED_DEVELOPMENT_REPLAY_COMPLETE'
        and not (ROTATION_ONLY / 'failure.json').exists(), 'completed frozen rotation-only baseline required')
    rotation_witness = ROTATION_RECEIPTS | rotation_result['output_sha256']
    verify_artifacts(ROTATION_ONLY, rotation_witness)
    sources = discover_sources((SOURCE, TEST, PROTOCOL,
        'lewm/tests/test_measured_pose_seeded_rgbd_development.py'), rotation_launch['source_sha256'])
    ...
    return old | dict(source_sha256=sources, predecessor_replay_sha256=witness,
        rotation_only_replay_sha256=rotation_witness, rotation_only_rerun=False,
        streams={c: list(MODELS) for c in COHORTS}, output_root=str(OUTPUT),
        correspondence_method='previous_measured_pose_seeded_bidirectional_flow_v1', correspondence_checks_changed=True,
...
def verify(launch):
    ...
    verify_artifacts(ROTATION_ONLY, launch['rotation_only_replay_sha256'])
...
def replay_trial(cohort, trial, store):
    ...
    with artifact_path(PREDECESSOR, old_name).open() as witness, store.path(name).open('xb') as target:
        for frame in range(len(reader.frames)):
            ...
            if frame % 250 == 0:
                print('MEASURED_POSE_REPLAY', stem, frame, {a: c['available'] for a, c in counts.items()}, flush=True)
...
def evaluate_trial(cohort, trial, report, store):
    ...
    availability = {k: 0 for k in ('both', 'original_only', 'measured_pose_only', 'neither')}
    ...
    with artifact_path(store.output, report['estimates_file']).open() as source, store.path(name).open('xb') as target:
        for line in source:
            ...
            both = all(have.values()); category = ('both' if both else 'original_only' if have['original']
                else 'measured_pose_only' if have['measured_pose'] else 'neither')
...
def compare_frozen(reports, previous):
    ...
    require(previous['status'] == 'GYRO_SEEDED_PAIRED_DEVELOPMENT_REPLAY_COMPLETE'
        and set(reports) == set(previous['reports']) == set(previous['evaluations']),
        'complete frozen rotation-only population required')
    ...
    return dict(result_sha256=ROTATION_RECEIPTS['result.json'],
        reports=previous['reports'], evaluations=previous['evaluations'],
        original_streams_independently_matched_to_same_witness=True,
        rotation_only_rerun=False, cross_run_timing_comparison_qualified=False,
        independent_validation=False, navigation_qualified=False)


def frozen_comparison(reports):
    binding = {'result.json': ROTATION_RECEIPTS['result.json']}
    verify_artifacts(ROTATION_ONLY, binding)
    result = compare_frozen(reports, read_json(ROTATION_ONLY, 'result.json'))
    verify_artifacts(ROTATION_ONLY, binding)
    return result
    ...
    try:
        ...
        for cohort in COHORTS:
            for trial in TRIALS:
                ...
                print('MEASURED_POSE_EVALUATION', active, evaluations[active], flush=True)
        verify(launch); verify_artifacts(OUTPUT, store.hashes)
        store.save('frozen_rotation_only_comparison.json', frozen_comparison(reports))
        verify_artifacts(OUTPUT, store.hashes)
        store.save('result.json', dict(status='MEASURED_POSE_PAIRED_DEVELOPMENT_REPLAY_COMPLETE', reports=reports,
        ...
        write_json(OUTPUT / 'failure.json', dict(status='TERMINAL_MEASURED_POSE_REPLAY_FAILURE',
