"""Short prospective maze run with asynchronous perception and wall deadlines.

Native physics and rendering remain on the scene-owning thread. Their measured
wall-clock delay is retained; no deadline is retimed to make a slow simulator
appear real time. This prefix is a timing/integration experiment, not a mission.
"""
...
def main():
    ...
    if MODEL_ASSIGNMENT=='reactive':
        ...
        model,condition,variant=MODEL_LOADER(MODEL_ASSIGNMENT)
    else:
    ...
    write('launch.json',dict(owner=dict(pid=owner.pid,created=owner.create_time()),observations=COUNT,
        ...
        physics_waits_for_high_level_result=False,physics_paused_during_rendering=True,
        acquisition_retains_original_ideal_simulation_stamps=True,
        maximum_motion_dispatch_simulator_lag_ns=20_000_000,
        simulator_lag_is_not_retimed=True,shared_host=True,native_scene_workers=1,
    ...
    try:
        with (ProcessPoolExecutor(max_workers=1,mp_context=get_context('spawn'),initializer=initialize_mapping) as mapping,
            ...
            try:
                for tick in range(COUNT*5):
                    ...
                    if tick%5==0:
                        began=clock();capture_start=time.perf_counter_ns()
                        p,d,f,a,rgb,measured=session.sensor_packets()
                        history.append(p)
                        packet=AcquiredFrame(tick//5,measured,p,d,f,rgb,a,tuple(history))
                        capture_cost=time.perf_counter_ns()-capture_start
                        if simulation_clock is None:controller.submit(packet)
                        else:pending.append((measured+capture_cost,packet))
                        acquisitions.append(dict(frame=tick//5,measured_ns=measured,
                            acquisition_started_ns=began,packet_available_ns=clock() if simulation_clock is None else measured+capture_cost,
                            measured_acquisition_wall_ns=capture_cost))
                    while pending and pending[0][0]<=sim_ns:
                        _,packet=pending.popleft();controller.submit(packet)
                    now=clock();request=controller.request(now_ns=now)
        ...
        if controller is not None:
            ...
            if simulation_clock is not None and not (OUTPUT/'measured_latency_releases.json').exists():
                write('measured_latency_releases.json',simulation_clock.releases)
            ...
            if hasattr(controller,'visual_support_rows'):
                write('visual_support_recovery.json',controller.visual_support_rows)
