...
def report(root,*,budget=None):
    ...
    save(root/'analysis_v41.json',dict(schema='headroom_v41_analysis.v1',cell_coverage=coverage,cells=cells,layout_clustered=clustered,
    ...
    (root/'analysis_v41.md').write_text('# V4 fixed audit closure\n\nAll assigned cells, missing quantities, weighted denominators, filter binding rules, paired regret populations and per-layout intervals are in analysis_v41.json. No source-run navigation outcome is a cohort estimate. No automated recommendation is executed. Prepare the one-step decision memo from these fixed results and then stop.\n')
