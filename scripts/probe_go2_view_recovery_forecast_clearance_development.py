...
def main():
    root=run.BASE/run.root_name(2);output=root/'view_recovery_forecast_clearance_probe_v1.json'
    ...
    wanted=(504,508,512,516,520,524,528)
    ...
    mapper=RecordedCurrentPlaneMap();reader=NoisyPublicReplay(root/'native');snapshots={};started=time.monotonic()
    for frame in frames:
        ...
        if frame%200==0:print('CLEARANCE_MAP_REPLAY',frame,flush=True)
    rows=[]
    for frame in wanted:
    ...
    result=dict(schema='view_recovery_forecast_clearance_probe.v1',rows=rows,
