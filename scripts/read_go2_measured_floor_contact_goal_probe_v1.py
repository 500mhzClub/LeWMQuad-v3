...
from scripts.run_go2_measured_floor_contact_goal_probe_v1 import OUTPUT as INPUT, CASES
...
from scripts.run_go2_sample_bounds_goal_probe_v1 import OUTPUT as ORIGINAL

OUTPUT=BASE/'go2_measured_floor_contact_goal_readout_v1_attempt_001'
PROTOCOL='docs/go2_measured_floor_contact_goal_readout_v1_2026-09-08.md'
ORIGINAL_SHA='76bcd4079e7271a2e8ecd6fde9c551cfc1a83918b84239f4449e138345a36dc4'
...
def main():
    ...
    assert result['status']=='MEASURED_FLOOR_CONTACT_GOAL_PROBE_COMPLETE' and result['cases']==[list(c) for c in CASES]
    ...
    assert old['status']=='SAMPLE_BOUNDS_GOAL_PROBE_COMPLETE'
    ...
    sources=discover_sources((PROTOCOL,'scripts/read_go2_measured_floor_contact_goal_probe_v1.py'),original['source_sha256'])
    ...
    try:
        ...
        write_json(OUTPUT/'result.json',dict(status='MEASURED_FLOOR_CONTACT_GOAL_READOUT_COMPLETE',conditions=reports,models=grouped,
        ...
        print('MEASURED_FLOOR_CONTACT_GOAL_READOUT_COMPLETE',digest(OUTPUT/'result.json'),flush=True)
    except Exception as error:
        write_json(OUTPUT/'failure.json',dict(status='TERMINAL_MEASURED_FLOOR_CONTACT_GOAL_READOUT_FAILURE',reason=repr(error)));raise
