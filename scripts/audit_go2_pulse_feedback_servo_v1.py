...
from lewm.pulse_feedback_servo_development import PulseFeedbackServo
from lewm.bounded_visual_servo_development import wrapped
from lewm.pulse_feedback_scene_development import specification
...
from scripts.run_go2_pulse_feedback_servo_v1 import OUTPUT,TRIALS
...
def audit_condition(condition,result):
    ...
    n=len(raw['timestamp_s']); assert n<=51250
    ...
    previous_servo=read_npz(ROOT/'.generated/go2_command_pulse_response_v1_attempt_001'/(spec['condition']+'_a'),'physics_trace.npz')
    ...
    motion=VisualLedMotion('gyro',identity=(0,0,0)); controller=PulseFeedbackServo(); desired=[]
    ...
    return dict(raw_sensor_audit_pass=True,raw_depth_checks_within1mm=all(r['within1mm'] for r in sensors['depth_checks']),
        ...
        pulse_count=terminal['pulse_count'] if terminal else None,
        native_path_length_m=float(np.linalg.norm(np.diff(poses[749:,:3],axis=0),axis=1).sum()),
        absolute_joint_travel_rad=float(np.abs(np.diff(raw['joint_position'][749:],axis=0)).sum()),
        mechanical_energy_j=None,energy_unavailable_reason='no torque/power sensor recorded; joint travel is not energy',
...
def main():
    ...
    sources=discover_sources(('scripts/audit_go2_pulse_feedback_servo_v1.py',),launch['source_sha256'])
    ...
    try:
        for c in TRIALS:
            ...
            print('PULSE_FEEDBACK_AUDIT',c,{k:v for k,v in reports[c].items() if k not in ('brake_segments','terminal_controller')},flush=True)
        ...
        write_json(OUTPUT/'raw_servo_audit.json',dict(status='RAW_SERVO_AUDIT_PASS',conditions=reports,
            audit_launch_sha256=digest(target),evaluation_sha256={c:digest(OUTPUT/(c+'_servo_evaluation.json')) for c in TRIALS},
