"""Two fixed exposed-maze ablations of early recovery-heading release."""
import argparse
...
PLAN = Path('docs/go2_no_early_heading_release_plan_2026-09-16.json')
CASES = (
    dict(name='replication_original', reference='go2_nogil_replication_jepa_noise_2mm_native_layout01_4800_v1_attempt_001', repaired=False),
    dict(name='survey_transfer_repaired', reference='go2_recovery_survey_transfer_limited_survey_jepa_noise_2mm_native_layout01_4800_v1_attempt_001', repaired=True),
)
ROOT = 'go2_no_early_heading_release_{name}_jepa_noise_2mm_native_layout01_4800_v1_attempt_001'


class OriginalNoEarlyReleaseRuntime(previous.LivePlanningStageProfileRuntime,
        NoEarlyHeadingReleaseMixin, FullReserveHeadingReleaseRuntime):
    pass


class RepairedNoEarlyReleaseRuntime(transfer.RecoveryLimitedSurveyRuntime,
        NoEarlyHeadingReleaseMixin, FullReserveHeadingReleaseRuntime):
    pass
...
def main():
    ...
    if args.prepare:
        ...
        for cls in (OriginalNoEarlyReleaseRuntime, RepairedNoEarlyReleaseRuntime):
            mro = cls.__mro__
        ...
        plan = dict(schema='no_early_heading_release_plan.v1', cases=CASES,
            source_sha256=source_hashes(), references=references,
            fixed_before_first_ablation=True, exposed_development_layouts=True,
            final_evaluation=False, hardware_validated=False, candidate_count=6,
            navigation_ticks=4800, planning_extra_ns=20_000_000,
            only_intervention='suppress early preferred-heading reversal of a recovery turn',
        ...
        print('PREPARED two exposed-maze ablations', flush=True)
        return
    ...
    if args.assignment == 2 and not (study.BASE/ROOT.format(name=CASES[0]['name'])/'short_pulse_navigation_evaluation_v1.json').is_file():
    ...
    runtime_class = RepairedNoEarlyReleaseRuntime if case['repaired'] else OriginalNoEarlyReleaseRuntime
    runtimes = []

    def write(name, value):
        if name == 'launch.json':
            value = reference | value | dict(experiment='no_early_heading_release_v1',
                ...
                fixed_sequential_assignments=[c['name'] for c in CASES], planned_native_assignments=2,
