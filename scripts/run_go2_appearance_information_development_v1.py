...
from scripts.probe_go2_rgbd_correspondence_motion_development_v1 import OUTPUT as PREVIOUS,B,verify as verify_previous
from scripts.run_go2_successive_choice_maze_development_v1 import ROOT,digest,write_json
from scripts.single_sample_rgbd_session_development import sampling_readback
from scripts.startup_raw_sensor_audit_development import read_json
from scripts.startup_source_inventory_development import discover_sources

OUTPUT=ROOT/'.generated/go2_appearance_information_development_v1_attempt_001'
PROTOCOL='docs/go2_appearance_information_development_v1_2026-09-06.md'
SEEDS=('scripts/run_go2_appearance_information_development_v1.py',
       'lewm/tests/test_appearance_surface_development.py',PROTOCOL)
IDENTITIES={'launch.json':'8a725bfe6fbdbd7220962d5195282660c479ed16350ea09c32c4be74064111a4',
    ...
    'raw_artifact_audit.json':'3888530bcbf246a4e1b6f0135c6f8cc8881001e2687f9dd037875b7329aa6f2f'}
...
def preflight():
    ...
    old=read_json(PREVIOUS,'launch.json');result=read_json(PREVIOUS,'result.json');verify_previous(old)
...
def build(gs,arm,directory,boxes):
    ...
    for name,mesh in surfaces(boxes,arm):
        ...
        if path.exists():raise ValueError('fresh visual artifact required')
        mesh.export(path,file_type='ply')
        expected[name]=triangle_identity(mesh.vertices.astype(np.float32),mesh.faces)
        visual.append(scene.add_entity(gs.morphs.Mesh(file=str(path),fixed=True,collision=False,
            visualization=True,decimate=False,convexify=False,align=False,file_meshes_are_zup=True),name=name))
