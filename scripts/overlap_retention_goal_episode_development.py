...
def collect(trial, definition, *, output, model, geometry, persistent, episode_name, condition, variant):
    ...
    try:
        ...
        session._runtime['high_level'] = 'overlap_retention_goal_probe_v1; online model plus measured surfaces'
        ...
        controller = OverlapRetentionGoalProbe(model, geometry, persistent=persistent, condition=condition, variant=variant)
        try:
        ...
        result = dict(status='OVERLAP_RETENTION_GOAL_PROBE_TERMINAL_AUDIT_REQUIRED', trial=trial,
