#!/usr/bin/env python3
"""Exact trainer for the single V4 N5 hierarchical-first-hit V11 attempt.
...
def compute_four_equal_v9_losses(
    model: Any,
    batch: Any,
) -> tuple[Any, dict[str, Any], Any, Any, Any]:
    """Preserve the V4 pipeline while replacing only its first-hit objective."""
...
def train_v9_fit(
    *,
    ...
    """Run the frozen 4,000-update full-panel, final-only V11 schedule."""
    ...
    if (
        ...
        raise PermissionError("V11 frozen optimizer or schedule request changed")
    if not math.isfinite(float(learning_rate)) or not math.isfinite(
        float(weight_decay)
    ):
        raise ValueError("V11 optimizer values must be finite")
    ...
    if schedule_sha256 != policy.EXPECTED_SCHEDULE_SHA256:
        raise ValueError("V11 deterministic schedule hash changed")
    if any(len(indices) != 5 or set(indices) != set(range(5)) for indices in schedule):
        raise ValueError("V11 schedule is not one full panel per update")
    ...
    for step_index, indices in enumerate(schedule, start=1):
        ...
        total, components, _raw, _targets, _raster = compute_four_equal_v9_losses(
            model,
            batch,
        )
        if not bool(torch.isfinite(total).item()):
            raise FloatingPointError("V11 fit loss became non-finite")
        total.backward()
        gradient_norm = torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0)
        if not bool(torch.isfinite(gradient_norm).item()):
            raise FloatingPointError("V11 fit gradient norm became non-finite")
        optimizer.step()
        if step_index == 1 or step_index % 100 == 0:
            ...
            trace.append(
                {
                    "step": step_index,
                    "total": 0.25 * sum(component_values.values()),
    ...
    if [row["step"] for row in trace] != diagnostic_updates:
        raise RuntimeError("V11 diagnostic schedule changed during training")
    return {
        "steps": 4000,
        "batch_size": 5,
        "learning_rate": 1e-4,
...
def evaluate_hierarchical_first_hit_v9(
    *,
    model: Any,
    frames: Sequence[Any],
    images: Any,
    ...
    with torch.no_grad():
        for target_index in range(len(frames)):
            ...
            _total, components, raw, targets, soft_raster = (
                compute_four_equal_v9_losses(model, batch)
            )
...
def build_checkpoint_metadata_v9(
    *,
    ...
    """Construct the frozen final-update V11 checkpoint metadata without I/O."""

    if training_schedule_sha256 != policy.EXPECTED_SCHEDULE_SHA256:
        raise ValueError("V11 checkpoint schedule binding changed")
    return {
        "experiment": policy.experiment_contract(),
        "authority_bindings": policy.authority_bindings(),
    ...
    *,
    ...
    try:
        ...
        with warnings.catch_warnings(record=True) as caught:
            ...
            matched = evaluate_hierarchical_first_hit_v9(
                model=model,
                frames=inputs.frames,
                images=images,
                device=device,
            ...
            wrong = evaluate_hierarchical_first_hit_v9(
                model=model,
                frames=inputs.frames,
                images=images,
                device=device,
