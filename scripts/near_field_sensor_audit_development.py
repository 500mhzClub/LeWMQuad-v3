"""Distinct physical-first-surface audit; preserve frozen clipping-blind predecessor."""
import hashlib
import json
...
def audit_sensors(directory, spec, result):
    ...
    for i, stamp in enumerate(ns):
        ...
        require(dc['native_near_m'] == RENDER_NEAR_M and dc['native_far_m'] == 200., 'fixed camera clipping')
        ...
        visibility = evaluate_visibility(native, walls, T, render_near_m=dc['native_near_m'])
        ...
        row = dict(physical_visibility=visibility, frame=frame, rays=len(errors), maximum_error_m=float(errors.max()), within1mm=bool(errors.max() <= .001),
...
def native_depth_comparison_mask(expected, interior):
    ...
    return interior & np.isfinite(expected) & (expected>0.) & (expected<4.98)
