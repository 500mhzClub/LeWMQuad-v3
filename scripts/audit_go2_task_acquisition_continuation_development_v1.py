#!/usr/bin/env python3
"""Full28 task-acquisition replay, preserving strict full-scan and physical metrics."""
import hashlib
import json
...
from lewm.continuation_rgb_dataset_development import load_continuation_observation as load_route_observation
...
from scripts.run_go2_task_acquisition_continuation_development_v1 import (
...
def audit_trial(directory, spec, row, template=None):
    check(all(row[k] == spec[k] for k in ('scene_id', 'case_index', 'method', 'destination_motif', 'initial_offset_m', 'initial_heading_rad', 'acquisition_policy')), 'scan identity')
    ...
    return {'scene_id': spec['scene_id'], 'status': 'PASS', 'rgb_packets': len(cameras),
            ...
            'two_leg_integration_success': row['response']['two_leg_integration_success'],
            'task_two_leg_integration_success': row['response']['task_two_leg_integration_success'],
            'acquisition_policy': spec['acquisition_policy'], 'sensor_fault': fault}
...
def main():
    ...
    check(result['status'] == 'COMPLETE' and result['completed_trials'] == result['planned_trials'] == 28 and len(result['trials']) == 28, 'traversal population incomplete')
    ...
    try:
        for spec, row in zip(trials(), result['trials'], strict=True):
            ...
            print(json.dumps({'event': 'continuation_audited', 'completed': len(rows), 'total': 28}), flush=True)
        verify_bindings(launch['source_sha256'] | launch['input_sha256'] | launch['gait_sha256'])
