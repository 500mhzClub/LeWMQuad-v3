...
from scripts.bounded_visual_servo_session_development import ServoSession
...
PREVIOUS=ROOT/'.generated/go2_visual_led_motion_integration_v1_attempt_001'
OUTPUT=ROOT/'.generated/go2_bounded_visual_servo_v1_attempt_001'
PROTOCOL='docs/go2_bounded_visual_servo_v1_2026-09-06.md'
IDENTITIES={'launch.json':'3e7903e671ded08b0b9725d6b2cafde9c53e29469dd8452e197de412d2916e8a',
...
def preflight():
    ...
    sources=discover_sources((PROTOCOL,'scripts/run_go2_bounded_visual_servo_v1.py',
        'lewm/tests/test_bounded_visual_servo_development.py'),old['source_sha256'])
    ...
    launch|=dict(source_sha256=sources,input_sha256=inputs,protocol=PROTOCOL,controller_rules=RULES,
        ...
        maximum_control_ticks=200,maximum_terminal_zero_ticks=10,model='gyro',
...
def collect(condition,definition):
    ...
    try:
        session=ServoSession(spec,directory); session.install_contact_identity(); build=session.ctx.build
        ...
        motion=VisualLedMotion('gyro',identity=(0,0,0)); controller=BoundedVisualServo()
...
def main():
    ...
    try:
        ...
        write_json(OUTPUT/'result.json',dict(status='BOUNDED_VISUAL_SERVO_COLLECTION_TERMINAL',conditions=results,
