#!/usr/bin/env python3
"""Bounded H6 loading and endpoint evaluation for joint-JEPA V28.
...
current-and-terminal-only normalized-RGB cache avoids repeating Pillow and disk
work across observations; training RGB is never cached here.
"""
...
RGB_ROOT_RELATIVE_PATH_V28 = Path(
    ".generated/datagen_full/render_textured_v03"
)
TRAIN_MICROBATCH_SIZE_V28 = 4
TRAIN_MICROBATCHES_PER_UPDATE_V28 = 4
VALIDATION_BATCH_SIZE_V28 = 16
MAXIMUM_RGB_FILE_BYTES_V28 = 4 * 1024 * 1024
LATENT_SHAPE_V28 = (64, 64, 64)
VALIDATION_RGB_TENSOR_BYTES_V28 = 3 * 112 * 112 * 4
MAXIMUM_VALIDATION_CACHE_ENTRIES_V28 = data.INDEX_BINDINGS["val"].row_count * 2
MAXIMUM_VALIDATION_CACHE_BYTES_V28 = (
    MAXIMUM_VALIDATION_CACHE_ENTRIES_V28 * VALIDATION_RGB_TENSOR_BYTES_V28
)
...
class V28EvaluationContractError(RuntimeError):
    """A V28 RGB-loading, batching, or evaluation invariant failed."""


@dataclass(frozen=True, slots=True)
class ValidationPlanEnergyVectorsV28:
...
def _open_absolute_directory(path: Path) -> int:
    ...
    if (
        ...
        raise V28EvaluationContractError(
...
class SafeV28RGBLoader:
    ...
    def __init__(
        ...
        if not root.is_absolute():
            raise V28EvaluationContractError("runtime data root must be absolute")
        ...
        for row in rows:
            ...
            if key in registered:
                raise V28EvaluationContractError("registered RGB row identity repeats")
            registered[key] = row
        if not registered:
            raise V28EvaluationContractError("RGB loader requires registered rows")
        try:
            descriptor = _open_absolute_directory(
                root / RGB_ROOT_RELATIVE_PATH_V28
            )
        except OSError as error:
            raise V28EvaluationContractError("frozen RGB root open failed") from error
        self._root_fd: int | None = descriptor
        self._rows = registered
        self._endpoint_leaf_allowlist = frozenset(
            leaf
            for row in registered.values()
            for leaf in (row.current_rgb, row.rgb[6])
        )
        ...
        if len(self._validation_cache_allowlist) > MAXIMUM_VALIDATION_CACHE_ENTRIES_V28:
            self.close()
            raise V28EvaluationContractError("validation RGB cache allowlist is too large")
    ...
    def __enter__(self) -> SafeV28RGBLoader:
        self._require_open()
        return self
    ...
    def _require_open(self) -> int:
        if self._root_fd is None:
            raise V28EvaluationContractError("V28 RGB loader is closed")
    ...
    def _registered(self, row: data.H6V2Row) -> data.H6V2Row:
        ...
        if registered is None or registered != row:
            raise V28EvaluationContractError("RGB row is not the registered index row")
        return registered

    def _read_leaf(self, row: data.H6V2Row, leaf: str) -> bytes:
        registered = self._registered(row)
        if leaf not in self._endpoint_leaf_allowlist or leaf not in (
            registered.current_rgb,
            registered.rgb[6],
        ):
            raise V28EvaluationContractError(
                "RGB leaf is not this row's current or terminal endpoint"
            )
        try:
            ...
            raise V28EvaluationContractError("registered RGB leaf is invalid") from error
        parts = PurePosixPath(canonical).parts
        if len(parts) != 3:
            raise V28EvaluationContractError("registered RGB leaf shape changed")
        ...
        try:
            ...
            if not stat.S_ISREG(before.st_mode):
                raise V28EvaluationContractError("RGB leaf is not a regular file")
            if not 0 < before.st_size <= MAXIMUM_RGB_FILE_BYTES_V28:
                raise V28EvaluationContractError("RGB leaf byte count is unsafe")
            chunks: list[bytes] = []
            consumed = 0
            while True:
                chunk = os.read(image_fd, min(1024 * 1024, MAXIMUM_RGB_FILE_BYTES_V28))
                if not chunk:
                    break
                consumed += len(chunk)
                if consumed > MAXIMUM_RGB_FILE_BYTES_V28:
                    raise V28EvaluationContractError("RGB leaf exceeded its byte cap")
                chunks.append(chunk)
            ...
            if (
                ...
                raise V28EvaluationContractError("RGB leaf changed while being read")
            self._access["rgb_open_success_count"] += 1
            self._access["rgb_byte_count"] += len(raw)
            return raw
        except V28EvaluationContractError:
            raise
        except OSError as error:
            raise V28EvaluationContractError("no-follow RGB leaf open failed") from error
        finally:
    ...
    def _decode_leaf(self, row: data.H6V2Row, leaf: str) -> torch.Tensor:
        ...
        if row.role == "val" and not cacheable:
            raise V28EvaluationContractError("validation RGB leaf left cache allowlist")
        ...
        try:
            ...
            raise V28EvaluationContractError("registered RGB decode failed") from error
        self._access["rgb_decode_success_count"] += 1
        if cacheable:
            if len(self._validation_cache) >= MAXIMUM_VALIDATION_CACHE_ENTRIES_V28:
                raise V28EvaluationContractError("validation RGB cache capacity exceeded")
    ...
    def load_terminal(self, row: data.H6V2Row) -> torch.Tensor:
        registered = self._registered(row)
        result = self._decode_leaf(registered, registered.rgb[6])
        if tuple(result.shape) != (3, 112, 112) or result.dtype != torch.float32:
            raise V28EvaluationContractError("terminal RGB tensor contract changed")
        return result
        ...
        return {
            **self._access,
            ...
            "validation_cache_bytes": (
                len(self._validation_cache) * VALIDATION_RGB_TENSOR_BYTES_V28
            ),
        }
...
def _access_delta(
    ...
    if set(before) != set(after):
        raise V28EvaluationContractError("RGB access counter schema changed")
    result = {name: int(after[name]) - int(before[name]) for name in before}
    if any(value < 0 for value in result.values()):
        raise V28EvaluationContractError("RGB access counters moved backwards")
    return result


def _stack_current(
    rows: Sequence[data.H6V2Row],
    loader: SafeV28RGBLoader,
    device: Any,
) -> torch.Tensor:
    ...
    if (
        ...
        raise V28EvaluationContractError("current RGB batch is invalid")
    return value
...
def _stack_terminal(
    ...
    value = torch.stack(tuple(loader.load_terminal(row) for row in rows), dim=0)
    value = value.to(device=torch.device(device), non_blocking=False)
    if (
        tuple(value.shape) != (len(rows), 3, 112, 112)
        or value.dtype != torch.float32
        ...
        raise V28EvaluationContractError("terminal RGB batch is invalid")
    return value
...
def build_train_h6_microbatch(
    ...
    *,
    ...
    if (
        ...
        or row_start < 0
        or row_start % TRAIN_MICROBATCH_SIZE_V28
        or row_start + TRAIN_MICROBATCH_SIZE_V28 > data.TRAIN_PREFIX_ROWS
        or row_start + TRAIN_MICROBATCH_SIZE_V28 > len(train_rows)
    ):
        raise V28EvaluationContractError("train H6 microbatch offset is invalid")
    rows = tuple(train_rows[row_start : row_start + TRAIN_MICROBATCH_SIZE_V28])
    if any(
        row.role != "train" or row.index != row_start + offset
        for offset, row in enumerate(rows)
    ):
        raise V28EvaluationContractError("train H6 rows left frozen consecutive order")
    current = _stack_current(rows, loader, device)
    terminal = _stack_terminal(rows, loader, device)
    plans = torch.tensor(
    ...
    result = {
        training.H6_CURRENT_RGB_KEY_V28: current,
        training.H6_TERMINAL_RGB_KEY_V28: terminal,
        training.H6_FUTURE_ACTIONS_KEY_V28: plans,
    }
    if tuple(result) != training.REQUIRED_H6_BATCH_KEYS_V28:
        raise V28EvaluationContractError("train H6 microbatch key order changed")
    return result
...
def build_train_h6_microbatches_for_update(
    ...
    *,
    ...
    """Return the four exact B4 H6 microbatches for one V28 update."""

    if type(update) is not int or not 1 <= update <= training.MAXIMUM_UPDATES_V28:
        raise V28EvaluationContractError("V28 update must be in the range 1 through 400")
    if len(train_rows) != data.INDEX_BINDINGS["train"].row_count:
        raise V28EvaluationContractError("complete frozen train index is required")
    first = 16 * (update - 1)
    result = tuple(
        build_train_h6_microbatch(
            train_rows,
            row_start=first + microbatch * TRAIN_MICROBATCH_SIZE_V28,
            loader=loader,
            device=device,
        )
        for microbatch in range(TRAIN_MICROBATCHES_PER_UPDATE_V28)
    )
    if len(result) != TRAIN_MICROBATCHES_PER_UPDATE_V28:
        raise V28EvaluationContractError("V28 update lost an H6 microbatch")
    return result
...
def _validate_latent(value: Any, *, batch: int, name: str) -> torch.Tensor:
    if (
        ...
        or tuple(value.shape) != (batch, *LATENT_SHAPE_V28)
        or value.dtype != torch.float32
        or not bool(torch.isfinite(value).all())
    ):
        raise V28EvaluationContractError(f"{name} is not finite Bx64x64x64 float32")
    return value
...
def _target_batch(
    ...
    terminal = _stack_terminal(rows, loader, device)
    with torch.no_grad():
        target = model.encode_target(terminal)
    return _validate_latent(
        target,
        batch=len(rows),
        name="terminal EMA target",
    )
...
def _energy_values(prediction: torch.Tensor, target: torch.Tensor) -> list[float]:
    ...
    if (
        ...
        raise V28EvaluationContractError("V28 latent energy vector is invalid")
...
def _validate_validation_panel(
    ...
    if count <= 0 or any(
        ...
        raise V28EvaluationContractError("validation rows must be ordered from zero")
    if (
        ...
        raise V28EvaluationContractError("validation donor vector length changed")
    for index, row in enumerate(rows):
        ...
        if not 0 <= tail < count or not 0 <= wrong < count:
            raise V28EvaluationContractError("mandatory donor index is out of range")
        if (
            ...
            raise V28EvaluationContractError("same-a0 tail donor changed")
        if (
            rows[wrong].scene_id == row.scene_id
            or rows[wrong].family != row.family
            or rows[wrong].plan == row.plan
        ):
            raise V28EvaluationContractError("full wrong-plan donor changed")
        if exact is not None and (
            ...
            raise V28EvaluationContractError("exact-plan wrong-scene donor changed")
...
def stream_validation_plan_energy_vectors(
    ...
    *,
    loader: SafeV28RGBLoader,
    device: Any,
) -> ValidationPlanEnergyVectorsV28:
    ...
    if not parameters or any(parameter.device != requested_device for parameter in parameters):
        raise V28EvaluationContractError("model parameters do not share evaluation device")
    target_modules_method = getattr(model, "target_modules", None)
    if not callable(target_modules_method):
        raise V28EvaluationContractError("model does not expose EMA target modules")
    target_modules = tuple(target_modules_method())
    if not target_modules:
        raise V28EvaluationContractError("model EMA target module inventory is empty")
    ...
    wrong_scene_donor_terminal_target_row_request_count = 0
    ...
    try:
        with torch.no_grad():
            for start in range(0, count, VALIDATION_BATCH_SIZE_V28):
                batch_rows = ordered_rows[start : start + VALIDATION_BATCH_SIZE_V28]
                ...
                if eligible:
                    ...
                    wrong_scene_donor_terminal_target_row_request_count += len(
                        unique_indices
                    )
            ...
            for scene_key in sorted(scene_rows):
                ...
                for start in range(0, len(selected_rows), VALIDATION_BATCH_SIZE_V28):
                    batch_rows = selected_rows[start : start + VALIDATION_BATCH_SIZE_V28]
                    target = _target_batch(model, batch_rows, loader, requested_device)
                    target_cpu = target.detach().to(device="cpu", dtype=torch.float32)
                    for local, row in enumerate(batch_rows):
                        key = (row.family, row.first_plan_action)
    ...
    if (
        ...
        raise V28EvaluationContractError("streamed validation energies are incomplete")
    after_access = loader.access_snapshot()
    access_delta = _access_delta(before_access, after_access)
    expected_cache_entry_count = len(
        {
            leaf
            for row in ordered_rows
            for leaf in (row.current_rgb, row.rgb[6])
    ...
    cached_before = before_access["validation_cache_entry_count"]
    if not 0 <= cached_before <= expected_cache_entry_count:
        raise V28EvaluationContractError("validation endpoint cache state is invalid")
    expected_cache_miss_count = expected_cache_entry_count - cached_before
    expected_rgb_tensor_request_count = (
        4 * count + wrong_scene_donor_terminal_target_row_request_count
    )
    expected_cache_hit_count = (
        expected_rgb_tensor_request_count - expected_cache_miss_count
    )
    expected_access = {
        "rgb_tensor_request_count": expected_rgb_tensor_request_count,
        "rgb_open_attempt_count": expected_cache_miss_count,
        "rgb_open_success_count": expected_cache_miss_count,
        "rgb_decode_success_count": expected_cache_miss_count,
        "validation_cache_hit_count": expected_cache_hit_count,
        "validation_cache_miss_count": expected_cache_miss_count,
        "validation_cache_insert_count": expected_cache_miss_count,
        "validation_cache_entry_count": expected_cache_miss_count,
        "validation_cache_bytes": (
            expected_cache_miss_count * VALIDATION_RGB_TENSOR_BYTES_V28
        ),
    }
    if (
        any(access_delta[name] != value for name, value in expected_access.items())
        or after_access["validation_cache_entry_count"]
        != expected_cache_entry_count
        or after_access["validation_cache_bytes"]
        != expected_cache_entry_count * VALIDATION_RGB_TENSOR_BYTES_V28
    ):
        raise V28EvaluationContractError(
            "validation endpoint RGB access formula changed"
        )
    ...
    return ValidationPlanEnergyVectorsV28(
        ...
        memory_receipt={
            ...
            "validation_batch_size": VALIDATION_BATCH_SIZE_V28,
            "complete_terminal_target_pass_count": 2,
            ...
            "expected_validation_cache_miss_count": expected_cache_miss_count,
            "expected_validation_cache_hit_count": expected_cache_hit_count,
            ...
            "validation_rgb_cache_maximum_entries": MAXIMUM_VALIDATION_CACHE_ENTRIES_V28,
            "validation_rgb_cache_maximum_bytes": MAXIMUM_VALIDATION_CACHE_BYTES_V28,
            "validation_rgb_cache_scope": (
                "validation_only_current_e2_and_terminal_e6_normalized_float32"
            ),
        },
        integrity={
            "schema": "lewm_go2_v28_plan_observation_integrity_v1",
...
class V28H6Runtime:
    ...
    def __init__(
        ...
        *,
        ...
        if (
            ...
            raise V28EvaluationContractError("V28 runtime requires both exact indexes")
        self._loader: SafeV28RGBLoader | None = SafeV28RGBLoader(
            self.runtime_data_root,
            (*self.train_rows, *self.validation_rows),
        )

    def __enter__(self) -> V28H6Runtime:
    ...
    def _require_loader(self) -> SafeV28RGBLoader:
        if self._loader is None:
            raise V28EvaluationContractError("V28 H6 runtime is closed")
        return self._loader
    ...
    def evaluate_plan_metrics(
        ...
        if type(update) is not int or update not in (0, 100, 400):
            raise V28EvaluationContractError("V28 observation update must be 0, 100, or 400")
    ...
    def preflight_receipt(self) -> dict[str, Any]:
        ...
        if any(access.values()):
            raise V28EvaluationContractError(
                "V28 H6 preflight receipt must precede every RGB access"
            )
        return {
            "schema": "lewm_go2_v28_h6_runtime_preflight_v1",
    ...
    def terminal_access_receipt(self) -> dict[str, Any]:
        ...
        if (
            ...
            raise V28EvaluationContractError("terminal H6 index rehash changed")
        access = loader.access_snapshot()
        return {
            "schema": "lewm_go2_v28_h6_terminal_access_v1",
...
def load_v28_h6_runtime(runtime_data_root: Path) -> V28H6Runtime:
    ...
    if (
        ...
        raise V28EvaluationContractError("V28 train and validation indexes overlap")
...
__all__ = [
    "LATENT_SHAPE_V28",
    "RGB_ROOT_RELATIVE_PATH_V28",
    "SafeV28RGBLoader",
    "TRAIN_MICROBATCHES_PER_UPDATE_V28",
    "TRAIN_MICROBATCH_SIZE_V28",
    "V28EvaluationContractError",
    "V28H6Runtime",
    "VALIDATION_BATCH_SIZE_V28",
    "ValidationPlanEnergyVectorsV28",
    "build_train_h6_microbatch",
