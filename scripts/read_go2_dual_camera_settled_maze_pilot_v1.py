...
OUTPUT = BASE/'go2_dual_camera_settled_maze_readout_v1_attempt_001'
PROTOCOL = 'docs/go2_dual_camera_settled_maze_readout_v1_2026-09-09.md'
...
def main():
    ...
    assert result['status'] == 'DUAL_CAMERA_SETTLED_MAZE_PILOT_V1_COMPLETE' and len(result['conditions']) == 1
    assert result['conditions'][0]['case'] == CASE[0]
    assert result['conditions'][0]['status']=='DUAL_CAMERA_SETTLED_MAZE_COLLECTED_AND_RAW_AUDITED'
    ...
    assert old['additional_auxiliary_rgb_for_motion'] is True
    sources = discover_sources((PROTOCOL, 'scripts/read_go2_dual_camera_settled_maze_pilot_v1.py',
    ...
    try:
        ...
        write_json(OUTPUT/'result.json', dict(status='DUAL_CAMERA_SETTLED_MAZE_READOUT_V1_COMPLETE',
        ...
        print('DUAL_CAMERA_SETTLED_MAZE_READOUT_V1_COMPLETE', digest(OUTPUT/'result.json'), flush=True)
    except Exception as error:
        write_json(OUTPUT/'failure.json', dict(status='TERMINAL_DUAL_CAMERA_SETTLED_MAZE_READOUT_FAILURE', reason=repr(error)))
        raise
