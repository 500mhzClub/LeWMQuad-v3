#!/usr/bin/env python3
"""Actual traversal decision replay, sensor reconstruction and physical outcome audit."""
import hashlib
import json
...
from scripts.run_go2_observed_traversal_development_v1 import (
...
def scientific_decision(value):
    # Wall-clock inference timings cannot replay; no prediction, command,
    # observation or scientific state field is excluded.
    result = json.loads(json.dumps(value, allow_nan=False))
    if result.get('selection') is not None:
        for key in ('inference_ms', 'adapter_ms'):
            elapsed = result['selection'].pop(key)
...
def audit_trial(directory, spec, row, template=None):
    ...
    check(1 <= row['rgb_packets'] <= 150, 'scan RGB budget')
    ...
    check(count == row['physics_samples'] and 0 < count <= 8150 and start == min(count, 750) - 1, 'physical population')
    ...
    decisions = json.loads((directory / 'traversal_decisions.json').read_text())
    ...
    for tick, decision in enumerate(decisions):
        check(tick <= 143 and decision['tick'] == tick and decision['pre_sample_index'] == start + 50 * tick, 'decision sequence')
    ...
    if stop_reason is not None and controller.ledger.record is not None and controller.ledger.record['status'] == 'PENDING':
        controller.ledger.finish(status='PHYSICAL_STOP')
    check(controller.ledger.snapshot() == json.loads((directory/'provisional_ledger.json').read_text()), 'exact provisional ledger')
    check(reduce_traversal(spec, raw, start, decisions, terminal, stop_reason, fault, geometry) == row['response'],
    ...
    return {'scene_id': spec['scene_id'], 'status': 'PASS', 'rgb_packets': len(cameras),
            ...
            'integration_success': row['response']['integration_success'], 'sensor_fault': fault}
...
def main():
    ...
    check(result['status'] == 'COMPLETE' and result['completed_trials'] == result['planned_trials'] == 20 and len(result['trials']) == 20, 'traversal population incomplete')
    ...
    try:
        for spec, row in zip(trials(), result['trials'], strict=True):
            ...
            print(json.dumps({'event': 'traversal_audited', 'completed': len(rows), 'total': 20}), flush=True)
        verify_bindings(launch['source_sha256'] | launch['input_sha256'] | launch['gait_sha256'])
        write_json(OUTPUT / 'raw_artifact_audit.json', {'status': 'PASS', 'audited_trials': len(rows), 'trials': rows,
                   ...
                   'scope': 'exact sensor/model-driven replay and raw one-transition simulation evidence; no maze or hardware qualification'})
