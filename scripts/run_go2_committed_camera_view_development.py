"""One exposed-layout follow-up after the fixed eight-run comparison finishes."""
...
ROOT='go2_committed_camera_view_pose_command_noise_2mm_native_layout01_4800_v1_attempt_001'
REFERENCE='go2_combined_perception_motion_pose_command_noise_2mm_native_layout01_4800_v1_attempt_001'


class CommittedViewRuntime(CombinedMotionSourceMixin,CommittedCameraFrontierRuntimeMixin,
                           combined.contact.ContactScoreRuntime):
    pass


def annotate_followup(name,value):
    if name=='launch.json':
        value=value|dict(experiment='committed_camera_view_followup_v1',
            comparison='complete_started_view_turn_before_reassessing_camera_evidence',
            comparison_condition='committed_view',reference_root_name=REFERENCE,
            planned_conditions=['committed_view'],planned_layout_indices=[1],
            ...
            layout_novelty_scope='previously_exposed_combined_comparison_layout_1',
...
def main():
    if sorted(os.sched_getaffinity(0))!=cohort.transfer.CPU_GROUPS[1]:
        raise ValueError('original layout-1 CPU group required')
    ...
    output=cohort.stable.source.BASE/ROOT
    ...
    with ProcessPoolExecutor(max_workers=1,mp_context=get_context('spawn'),
        ...
        def runtime(*args,**kwargs):
            ...
            return CommittedViewRuntime(*args,motion_prediction_source='pose_command',
        ...
        bind(cohort.stable.source.main,OUTPUT=output,COUNT=4814,LAYOUT_INDEX=1,
            ...
            PacedNativeSession=partial(combined.FreshCameraSession,noise_layout_index=1,noise_sigma_mm=2),
