"""Execute all six preassigned expanded-data models with the fixed controller."""
import argparse
import contextlib
import json
...
from scripts.all_phase_residual_maze02_native_inputs_development import (
    prepared_sources, admit_inputs, verify_bound_inputs, reference, corrected_wait, NATIVE_OWNER, CORRECTION_OWNER)
...
OUTPUT=BASE/'go2_all_phase_residual_maze02_matched_native_v1_attempt_001'
SOURCE='scripts/run_go2_all_phase_residual_maze02_matched_native_v1.py'
PROTOCOL='docs/go2_all_phase_residual_maze02_matched_native_v1_2026-09-10.md'
TESTS=('lewm/tests/test_all_phase_residual_maze02_study_development.py',
...
def assigned_model(launch,case):
    ...
    if (condition,variant)!=(case[3],case[2]) or state_digest(model.state_dict())!=launch['assigned_model_states'][case[4]]:
...
def worker(case,launch_sha):
    ...
    with (OUTPUT/(name+'_worker.log')).open('x') as log,contextlib.redirect_stdout(log),contextlib.redirect_stderr(log):
        try:
            ...
            startup=compare_startup(reference.OUTPUT/reference.CASE[0],OUTPUT/name)
...
def main():
    ...
    try:
        ...
        write_json(OUTPUT/'result.json',dict(status='ALL_PHASE_RESIDUAL_MAZE02_MATCHED_NATIVE_V1_COMPLETE',
            ...
            predecessor_native_result_sha256=args.native_result_sha256,**summary))
        print('ALL_PHASE_RESIDUAL_MAZE02_COMPLETE',digest(OUTPUT/'result.json'),flush=True)
    except Exception as error:
        write_json(OUTPUT/'failure.json',dict(status='TERMINAL_ALL_PHASE_RESIDUAL_MAZE02_MATCHED_NATIVE_FAILURE',
