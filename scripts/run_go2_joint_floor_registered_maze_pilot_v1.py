...
from scripts.joint_floor_registered_maze_episode_development import collect, artifacts
from scripts.joint_floor_registered_maze_audit_development import audit
...
OUTPUT = BASE/'go2_joint_floor_registered_maze_pilot_v1_attempt_001'
PROTOCOL = 'docs/go2_joint_floor_registered_maze_pilot_v1_2026-09-08.md'
...
def worker(launch_sha):
    ...
    terminal = dict(case=name, layout_index=index, model_name=model_name,
        status='JOINT_FLOOR_REGISTERED_MAZE_WORKER_FAILED', artifact_sha256={}); started = time.perf_counter()
    with (OUTPUT/(name+'_worker.log')).open('x') as log, contextlib.redirect_stdout(log), contextlib.redirect_stderr(log):
        try:
            ...
            terminal.update(status='JOINT_FLOOR_REGISTERED_MAZE_COLLECTED_AND_RAW_AUDITED', artifact_sha256=bindings,
...
def main():
    ...
    assert integration['status'] == 'JOINT_FLOOR_REGISTERED_MAZE_PREFIX_COMPLETE'
    ...
    sources = discover_sources((PROTOCOL, 'scripts/run_go2_joint_floor_registered_maze_pilot_v1.py',
        'lewm/tests/test_joint_floor_registered_native_scope_development.py',
        'lewm/tests/test_floor_registered_native_prefix_comparison_development.py',
        'docs/go2_joint_floor_registered_maze_prefix_result_2026-09-08.md'), integration['source_sha256'])
    resources = hardware()
    if args.preflight_only:
        print('JOINT_FLOOR_REGISTERED_MAZE_PREFLIGHT', json.dumps(dict(
    ...
    print('JOINT_FLOOR_REGISTERED_MAZE_LAUNCHED', launch_sha, flush=True)
    try:
        ...
        print('JOINT_FLOOR_REGISTERED_MAZE_TERMINAL', record['status'], record.get('verified_round_trip'), record.get('failure'), flush=True)
        if record['status'] != 'JOINT_FLOOR_REGISTERED_MAZE_COLLECTED_AND_RAW_AUDITED':
        ...
        verify_artifacts(FITS, admission['base_admission']['fit_artifact_sha256'])
        verify_artifacts(CORRECTION, admission['correction_artifact_sha256'])
        write_json(OUTPUT/'result.json', dict(status='JOINT_FLOOR_REGISTERED_MAZE_PILOT_COMPLETE', conditions=[record],
        ...
        print('JOINT_FLOOR_REGISTERED_MAZE_COMPLETE', digest(OUTPUT/'result.json'), flush=True)
    except Exception as error:
        write_json(OUTPUT/'failure.json', dict(status='TERMINAL_JOINT_FLOOR_REGISTERED_MAZE_PILOT_FAILURE', reason=repr(error)))
        raise
