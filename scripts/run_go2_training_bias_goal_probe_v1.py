"""Fixed base models with training-only intercepts and unchanged path planning."""
import argparse
import contextlib
import json
...
from scripts.training_bias_goal_episode_development import collect,artifacts
from scripts.training_bias_goal_audit_development import audit
...
PREVIOUS=BASE/'go2_eight_step_planning_goal_probe_v1_attempt_001'
DIAGNOSTIC=BASE/'go2_eight_step_planning_goal_readout_v1_attempt_001'
PREVIOUS_SHA='a42774f8a7ebd98a46dcf8c6a4c90c49e8551a6bcb0ccd3031810abd039b53c5'
DIAGNOSTIC_SHA='9b80085ac93a16e7793f75cdae09971b7e4f6b4023167b25817cfecd8ee28660'
OUTPUT=BASE/'go2_training_bias_goal_probe_v1_attempt_001'
...
PROTOCOL='docs/go2_training_bias_goal_probe_v1_2026-09-08.md'
...
def worker(case,launch_sha):
    ...
    terminal=dict(case=name,trial=trial,variant=variant,condition=condition,model_name=model_name,
        status='TRAINING_BIAS_GOAL_WORKER_FAILED',artifact_sha256={});started=time.perf_counter()
    with (OUTPUT/(name+'_worker.log')).open('x') as log,contextlib.redirect_stdout(log),contextlib.redirect_stderr(log):
        try:
            ...
            terminal.update(status='TRAINING_BIAS_GOAL_COLLECTED_AND_RAW_AUDITED',artifact_sha256=bindings,
...
def main():
    ...
    parser.add_argument('--correction-readout-sha256',required=True);args=parser.parse_args()
    ...
    assert previous['status']=='EIGHT_STEP_PLANNING_GOAL_PROBE_COMPLETE' and previous['cases']==[list(c) for c in CASES]
    ...
    assert diagnostic['status']=='EIGHT_STEP_PLANNING_GOAL_READOUT_COMPLETE'
    ...
    launch=preflight(output=OUTPUT,protocol=PROTOCOL,
        seed_paths=('scripts/run_go2_training_bias_goal_probe_v1.py',
            'lewm/tests/test_training_bias_goal_scope_development.py',
    ...
    launch.update(previous_probe_artifact_sha256=previous_ids,previous_readout_artifact_sha256=diagnostic_ids,
        ...
        experiment='same eight-step planner and frozen base models; apply only the complete training-derived XY intercept correction',
        concurrency_reason='two independent scenes; bounded by the passing four-process native benchmark and completed eight-step two-scene probe',
    ...
    print('TRAINING_BIAS_GOAL_LAUNCHED',digest(OUTPUT/'launch.json'),flush=True)
    ...
    try:
            ...
            with ProcessPoolExecutor(max_workers=2,mp_context=multiprocessing.get_context('spawn'),max_tasks_per_child=1) as pool:
                ...
                while pending:
                    ...
                    for future in done:
                        ...
                        except Exception as error:row=dict(case=case[0],status='TRAINING_BIAS_GOAL_PROCESS_FAILURE',failure=repr(error))
                        records.append(row);print('TRAINING_BIAS_GOAL_TERMINAL',case[0],row['status'],row.get('goal'),flush=True)
        if len(records)!=2 or any(r['status']!='TRAINING_BIAS_GOAL_COLLECTED_AND_RAW_AUDITED' for r in records):
        ...
        result=dict(status='TRAINING_BIAS_GOAL_PROBE_COMPLETE',cases=CASES,conditions=records,
        ...
        write_json(OUTPUT/'result.json',result);print('TRAINING_BIAS_GOAL_COMPLETE',digest(OUTPUT/'result.json'),flush=True)
    except Exception as error:
        write_json(OUTPUT/'failure.json',dict(status='TERMINAL_TRAINING_BIAS_GOAL_FAILURE',reason=repr(error),
