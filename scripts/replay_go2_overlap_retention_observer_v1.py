"""Full-trace qualified-overlap retention comparison; no new commands or physics."""
import json
import time
import cv2
import numpy as np
...
from scripts.run_go2_retained_patch_contact_goal_probe_v1 import OUTPUT as INPUT
TRIALS = ('full_direct_family_episode_039',)
...
OUTPUT = BASE/'go2_overlap_retention_observer_replay_v1_attempt_001'
PROTOCOL = 'docs/go2_overlap_retention_observer_replay_v1_2026-09-08.md'
IDS = {'launch.json': '9a17318bde6998f7f0c74ad1a123c79862255994f5d382ab62ae0840d4e3207a',
    'result.json': '9842921b16b0fb2dffc501c6d958fbfd6ffb72e57a9c56855b2d5f9b54e61685'}
ARMS = ('original', 'overlap_retention')


def replay(trial, arm):
    reader = IntentReturnRGBDReplay(INPUT/trial)
    motion = (CornerSupportVisualLedMotion if arm == 'original' else OverlapRetentionVisualLedMotion)()
    rows = []
    ...
    if arm == 'original':
        original_rows = read_json(INPUT/trial, 'context_decisions.json')
        compared = 0
        for row, original_row in zip(rows, original_rows, strict=True):
            expected = original_row['decision']['evidence']
            if expected is not None:
                assert json.loads(json.dumps(row['evidence'])) == expected
                compared += 1
        assert compared == 137
    ...
    report = dict(trial=trial, arm=arm, frames=len(rows), current_joint_poses=accepted,
        ...
        original_available_evidence_exact=arm == 'original',
        promotions=sum(bool((r['evidence']['current_pose'] or {}).get('promoted_keyframe')) for r in rows),
        overlap_promotions=sum((r['evidence']['current_pose'] or {}).get('promotion_reason') == 'accepted_half_feature_overlap' for r in rows),
    ...
    print('OVERLAP_RETENTION_REPLAY', json.dumps(report, sort_keys=True), flush=True)
    return report
...
def main():
    ...
    assert result['status'] == 'RETAINED_PATCH_CONTACT_GOAL_PROBE_COMPLETE'
    assert [c[0] for c in result['cases']] == list(TRIALS)
    bindings = IDS | result['artifact_sha256']
    verify_artifacts(INPUT, bindings)
    sources = discover_sources((PROTOCOL, 'scripts/replay_go2_overlap_retention_observer_v1.py',
        'lewm/tests/test_overlap_retention_observer_development.py',
        'docs/go2_retained_patch_contact_goal_probe_result_2026-09-08.md'), original['source_sha256'])
    resources = hardware()
    ...
    print('OVERLAP_RETENTION_REPLAY_LAUNCHED', digest(OUTPUT/'launch.json'), flush=True)
    reports = []
    try:
        ...
        assert reports[0]['first_failure_frame'] == 136
        assert [r['frames'] for r in reports] == [147, 147]
        ...
        write_json(OUTPUT/'result.json', dict(status='OVERLAP_RETENTION_OBSERVER_REPLAY_COMPLETE',
            reports=reports, source_sha256=sources, artifact_sha256=products,
            candidate_eligible_for_separate_native_probe=all(r['diagnostic_thresholds_pass'] for r in reports if r['arm']=='overlap_retention'),
        ...
        print('OVERLAP_RETENTION_REPLAY_COMPLETE', digest(OUTPUT/'result.json'), flush=True)
    except Exception as error:
        write_json(OUTPUT/'failure.json', dict(status='TERMINAL_OVERLAP_RETENTION_REPLAY_FAILURE', reason=repr(error)))
        raise
