#!/usr/bin/env python3
"""Collect the science-identical V3 integrity replacement V1.

This is a scoped identity adapter over the reviewed V3 collector.  It owns the
fresh plan role/root routing and exact predecessor-review witness.  No
invocation is self-authorizing.
"""
...
from scripts import collect_go2_scene_diversity_recurrent_replication_genesis_rocm_backend_v3 as predecessor  # noqa: E402


AUTHORITY_SCHEMA = (
    "lewm_go2_scene_diversity_recurrent_replication_"
    "genesis_rocm_backend_v3_integrity_replacement_v1_execution_authority_v1"
)
AUTHORITY_STATUS = (
    "AUTHORIZED_SCENE_DIVERSITY_RECURRENT_REPLICATION_"
    "GENESIS_ROCM_BACKEND_V3_INTEGRITY_REPLACEMENT_V1"
)
...
RESERVATION_SCHEMA = (
    "lewm_go2_scene_diversity_recurrent_replication_"
    "genesis_rocm_backend_v3_integrity_replacement_v1_collection_reservation_v1"
)
SCENE_RESULT_SCHEMA = (
    "lewm_go2_scene_diversity_recurrent_replication_"
    "genesis_rocm_backend_v3_integrity_replacement_v1_scene_physics_result_v1"
)
SCENE_EVIDENCE_SCHEMA = (
    "lewm_go2_scene_diversity_recurrent_replication_"
    "genesis_rocm_backend_v3_integrity_replacement_v1_scene_process_evidence_v1"
)
...
CONTACT_FORCE_ROUTE_AUDIT = {
    **copy.deepcopy(predecessor.CONTACT_FORCE_ROUTE_AUDIT),
    "schema": (
        "lewm_go2_scene_diversity_recurrent_replication_"
        "genesis_rocm_backend_v3_integrity_replacement_v1_contact_force_route_source_audit_v1"
    ),
}
PROCESS_RESET_EQUIVALENCE_AUDIT_ROCM = {
    **copy.deepcopy(predecessor.PROCESS_RESET_EQUIVALENCE_AUDIT_ROCM),
    "schema": (
        "lewm_go2_scene_diversity_recurrent_replication_"
        "genesis_rocm_backend_v3_integrity_replacement_v1_process_reset_equivalence_source_audit_v1"
    ),
...
_ORIGINAL_VALIDATE_AUTHORITY_V3 = (
    predecessor._validate_authority_v3_review_bound  # noqa: SLF001
)
_CONFIGURATION_LOCK = threading.RLock()


def _standard_v3_review_binding() -> dict[str, Any]:
    value = plan_builder.v3_qualification_terminal_review_binding()
    return {
        "path": str(value["path"]),
...
def _require_v3_review_binding(authority: Mapping[str, Any]) -> None:
    if authority.get(
        "predecessor_v3_qualification_terminal_review_binding"
    ) != _standard_v3_review_binding():
        raise SceneProcessCollectionError(
            "replacement authority V3 terminal-review binding changed"
        )


def _validate_authority_replacement_review_bound(*args: Any, **kwargs: Any):
    authority = _ORIGINAL_VALIDATE_AUTHORITY_V3(*args, **kwargs)
