"""Raw-only sensor/contact replay for the fresh startup trial (no simulation)."""
import hashlib
import json
...
def audit_sensors(directory, spec, result):
    ...
    relatives = read_json(directory, 'relative_state_observations.json')
    require(len(cameras) == len(dcameras) == len(relatives) == result['rgbd_frames'] > 0, 'complete paired observer frames')
    same(len(relatives), result['relative_observer_frames'])
    check_extent_identity(read_json(directory, 'floor_visual_collision_identity.json'), 32.)
    ...
    for i, stamp in enumerate(ns):
        ...
        same(relatives[frame], dict(observation_index=frame, observer=observed))
    ...
    report = dict(sensor_contact_relative_reconstruction_exact=True, depth_checks=depth_results,
