...
OUTPUT = BASE/'go2_commitment_contact_anchored_raw_prefix_wait_v1_attempt_001'
SOURCE = 'scripts/await_go2_commitment_contact_anchored_raw_prefix_v1.py'
PROTOCOL = 'docs/go2_commitment_contact_anchored_raw_prefix_wait_v1_2026-09-10.md'
TEST = 'lewm/tests/test_commitment_contact_anchored_raw_wait_development.py'
WAIT_SECONDS = 48*3600
WORKER = dict(pid=2672443, created=1789037034.85, command=[
...
def wait_for_worker(event, *, sleep=time.sleep, clock=time.monotonic):
    ...
    while owner_live(WORKER):
        ...
        event('WAITING_FOR_ORIGINAL_ADAPTER_SUPERVISED_WORKER', original_worker_pid=WORKER['pid'])
        sleep(30)
...
def authenticate_replay(worker_sha, sources):
    ...
    if (result['status'] != 'COMMITMENT_CONTACT_ANCHORED_RAW_PREFIX_V1_COMPLETE'
            ...
            or report['frames'] != 4 or report['first_changed_command_frame'] != 3
            ...
            or report['complete_retained_observed_state_exact'] is not True
            or report['selected_pending_forecasts_checked'] is not True
    ...
    return dict(raw_replay_result_sha256=sha, frames=4, first_changed_command_frame=3,
...
def main():
    ...
    sources = discover_sources((SOURCE, PROTOCOL, TEST, *replay.SEEDS), saved_result['source_sha256'])
    ...
    print('COMMITMENT_CONTACT_ANCHORED_RAW_WAITER_LAUNCHED', digest(OUTPUT/'launch.json'), flush=True)
    ...
    try:
        with (OUTPUT/'events.jsonl').open('x') as events:
            ...
            with (OUTPUT/'replay_stdout.log').open('xb') as log:
                ...
                event('COMMITMENT_CONTACT_ANCHORED_RAW_CHILD_STARTED', pid=child.pid, command=command)
                while child.poll() is None:
                    event('COMMITMENT_CONTACT_ANCHORED_RAW_CHILD_LIVE', pid=child.pid); time.sleep(30)
            event('COMMITMENT_CONTACT_ANCHORED_RAW_CHILD_EXITED', pid=child.pid, returncode=child.returncode)
        ...
        write_json(OUTPUT/'result.json', dict(status='COMMITMENT_CONTACT_ANCHORED_RAW_PREFIX_WAIT_V1_COMPLETE',
            source_sha256=sources, artifact_sha256=ids, report=report, automatic_retry=False,
            native_execution=False, goal_achieved=False))
        print('COMMITMENT_CONTACT_ANCHORED_RAW_WAITER_COMPLETE', digest(OUTPUT/'result.json'), flush=True)
    except Exception as error:
        write_json(OUTPUT/'failure.json', dict(status='TERMINAL_COMMITMENT_CONTACT_ANCHORED_RAW_WAITER_FAILURE',
