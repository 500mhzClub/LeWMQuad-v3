...
import numpy as np
from scripts import replay_go2_no_rgb_jepa_direct_flow_controller_prefix_v1 as replay
...
def boundary(report):
    expected = dict(frames=FRAMES, exact_original_decisions=INTERVENTION, original_forecasts_compared=856,
    ...
    if any(report.get(k) != v or type(report.get(k)) is not type(v) for k,v in expected.items()):
        raise ValueError('complete recovered fixed no-RGB JEPA controller prefix required')
...
def admit_prefix(result_sha, sources):
    root = replay.OUTPUT
    if (root/'failure.json').exists(): raise ValueError('failed controller prefix cannot authorize a native test')
    verify_artifacts(root,{'result.json':result_sha})
    ...
    if (result['status'] != 'NO_RGB_JEPA_DIRECT_FLOW_CONTROLLER_PREFIX_V1_COMPLETE'
            ...
            or launch['implementation_class'] != 'DirectFlowResidualAnchoredController'
            or launch['full_original_input_admission_reexecuted'] is not True):
    ...
    verify_artifacts(replay.batch.OUTPUT,expected_inputs)
    prior = replay.batch.OUTPUT/replay.CASE[0]
...
def compare(prior,current,prefix_root,report):
    ...
    for directory in (prior,current):
        with np.load(artifact_path(directory.parent,directory.name+'/physics_trace.npz'),allow_pickle=False) as raw:
            if not raw.files or any(len(raw[k]) < PHYSICS_SAMPLES for k in raw.files):
