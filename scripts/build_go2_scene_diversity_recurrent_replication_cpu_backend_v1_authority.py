#!/usr/bin/env python3
"""Build scientific CPU authority only after exact PASS qualification."""
...
from scripts import build_go2_scene_diversity_recurrent_replication_cpu_backend_v1_qualification_authority as qualification_authority  # noqa: E402
...
AUTHORITY_OUTPUT = REPO_ROOT / (
    ...
    "cpu_backend_v1_execution_authority_2026-08-04.json"
)
CpuBackendAuthorityError = qualification_authority.CpuBackendAuthorityError
...
def build_scientific_authority(
    *, preregistration_binding: Mapping[str, Any],
    ...
    scientific_plan_binding: Mapping[str, Any], qualification_plan_binding: Mapping[str, Any],
    source_review: Mapping[str, Any], source_review_binding: Mapping[str, Any],
    ...
    _result, exact_result_binding = runner.validate_qualification_result_binding(
        qualification_result_binding
    )
    if plan_builder.DEFAULT_ATTEMPT_ROOT.exists() or plan_builder.DEFAULT_ATTEMPT_ROOT.is_symlink():
        raise CpuBackendAuthorityError("scientific CPU attempt root is not fresh")
    authority = {
    ...
    if set(authority) != runner.collector.AUTHORITY_FIELDS:
        raise CpuBackendAuthorityError("scientific authority fields changed")
    return authority
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print(json.dumps({"authority": qualification_authority._write_json_exclusive(  # noqa: SLF001
...
__all__ = [
    "AUTHORITY_OUTPUT", "CpuBackendAuthorityError", "build_scientific_authority",
]
