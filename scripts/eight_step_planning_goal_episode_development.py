...
def collect(trial, definition, *, output, model, geometry, persistent, episode_name, condition, variant):
    ...
    try:
        ...
        session._runtime['high_level'] = 'eight_step_planning_goal_probe_v1; online model plus measured surfaces'
        ...
        controller = EightStepPlanningGoalProbe(model, geometry, persistent=persistent, condition=condition, variant=variant)
        try:
        ...
        result = dict(status='EIGHT_STEP_PLANNING_GOAL_PROBE_TERMINAL_AUDIT_REQUIRED', trial=trial,
