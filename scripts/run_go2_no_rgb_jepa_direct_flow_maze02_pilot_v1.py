"""Fresh no-RGB JEPA maze2 physics after controller recovery and the original native queue."""
import argparse
import contextlib
...
from scripts.no_rgb_jepa_direct_flow_native_prefix_development import compare, boundary
from scripts.no_rgb_jepa_direct_flow_maze02_episode_development import collect, artifacts
from scripts.no_rgb_jepa_direct_flow_maze02_audit_development import audit
...
OUTPUT = BASE/'go2_no_rgb_jepa_direct_flow_maze02_pilot_v1_attempt_001'
SOURCE = 'scripts/run_go2_no_rgb_jepa_direct_flow_maze02_pilot_v1.py'
PROTOCOL = 'docs/go2_no_rgb_jepa_direct_flow_maze02_pilot_v1_2026-09-10.md'
TESTS = ('lewm/tests/test_no_rgb_jepa_direct_flow_native_launcher_development.py',
    'lewm/tests/test_no_rgb_jepa_direct_flow_native_prefix_development.py',
    'lewm/tests/test_no_rgb_jepa_direct_flow_native_source_development.py')
CASE = ('no_rgb_jepa_direct_flow_anchored_maze_02', *inputs.replay.CASE[1:])
WORKER_STATUS = 'NO_RGB_JEPA_DIRECT_FLOW_MAZE02_COLLECTED_AND_RAW_AUDITED'
...
def verify_inputs(launch, *, full=False):
    ...
    expected = dict(planned_case=list(CASE), output_root=str(OUTPUT),
        implementation_class='DirectFlowResidualAnchoredController',
        ...
        direct_corner_flow_missingness_fallback_enabled=True, planner_interface_adapter_enabled=True,
    ...
    if any(launch[k] != v or type(launch[k]) is not type(v) for k, v in expected.items()):
        raise ValueError('exact prospective tracking-recovery native definition required')
    ...
    if full and inputs.admit(admission['prefix_result_sha256'], completed['ordered_waiter_result_sha256'],
            completed['adapter_batch_result_sha256'], launch['source_sha256']) != admission:
        raise ValueError('complete original tracking-recovery input admission changed')
...
def assigned_model(launch):
    ...
    if (type(model) is not AllPhasePlannerModel or model.training
        ...
        raise ValueError('original assigned tracking-recovery model state required')
    return model
...
def require_worker(record, report, prefix_report):
    if (record['status'] != WORKER_STATUS or 'failure' in record or record['case'] != CASE[0]
            ...
            or record['collection']['status'] != 'NO_RGB_JEPA_DIRECT_FLOW_MAZE02_TERMINAL_AUDIT_REQUIRED'
            or record['collection']['direct_corner_flow_missingness_fallback_enabled'] is not True):
        raise ValueError('complete exact tracking-recovery collection and raw audit required')
    ...
    for key in ('physical_and_public_prefix_exact', 'all_preintervention_requested_commands_exact',
        ...
        if receipt[key] is not True: raise ValueError('complete physical tracking-recovery intervention required: '+key)
    if (receipt['common_prefix_frames'] != frames or receipt['first_intervention_frame'] != changed
            ...
            or receipt['original_forecasts_compared'] != prefix_report['original_forecasts_compared']
            or receipt['candidate_intervention_command'] != prefix_report['boundary_requested_command']
            or receipt['intervention_command_changed'] is not (prefix_report['boundary_requested_command'] != [0.,0.,0.])
            or receipt['observer_and_full_controller_intervention'] is not True
            or receipt['navigation_verified'] is not False
...
def worker(launch_sha):
    ...
    record = dict(case=name, layout_index=index, variant=variant, condition=condition, model_name=model_name,
        status='NO_RGB_JEPA_DIRECT_FLOW_MAZE02_WORKER_FAILED', artifact_sha256={}); started = time.perf_counter()
    with (OUTPUT/(name+'_worker.log')).open('x') as log, contextlib.redirect_stdout(log), contextlib.redirect_stderr(log):
        try:
            ...
            if state_digest(model.state_dict()) != before: raise ValueError('collection changed assigned tracking-recovery model')
            ...
            receipt = compare(inputs.replay.batch.OUTPUT/inputs.replay.CASE[0], OUTPUT/name,
            ...
            record.update(status=WORKER_STATUS, artifact_sha256=ids, prefix_comparison=receipt, readout=readout,
                model_state_sha256=before, model_state_unchanged=True, direct_corner_flow_missingness_fallback_enabled=True,
            ...
            import traceback
            traceback.print_exc(); record.update(status='NO_RGB_JEPA_DIRECT_FLOW_MAZE02_WORKER_FAILED', failure=repr(error))
...
def main():
    ...
    parser.add_argument('--controller-prefix-result-sha256'); parser.add_argument('--adapter-batch-result-sha256')
    parser.add_argument('--frontier-wait-result-sha256'); parser.add_argument('--hold-wait-result-sha256')
    parser.add_argument('--contact-wait-result-sha256')
    ...
    if OUTPUT.exists() or OUTPUT.is_symlink(): raise ValueError('exclusive tracking-recovery native attempt; no retry/resume')
    ...
    if args.source_preflight_only:
        print('NO_RGB_JEPA_DIRECT_FLOW_NATIVE_SOURCE_PREFLIGHT', json.dumps(dict(source_count=len(sources), hardware=resources,
            native_execution=False, output_created=False, complete_input_admission_performed=False)), flush=True); return
    identities = dict(frontier=args.frontier_wait_result_sha256, hold=args.hold_wait_result_sha256,
        contact=args.contact_wait_result_sha256)
    if not args.controller_prefix_result_sha256 or not args.adapter_batch_result_sha256 or not all(identities.values()):
        raise ValueError('exact completed controller prefix, batch and all three native waiter identities required')
    print('NO_RGB_JEPA_DIRECT_FLOW_NATIVE_INPUT_ADMISSION_STARTED', len(sources), flush=True)
    admission = inputs.admit(args.controller_prefix_result_sha256, identities, args.adapter_batch_result_sha256, sources)
    old = read_json(inputs.replay.batch.OUTPUT, 'launch.json')
    ...
    launch.update(source_sha256=sources, protocol=PROTOCOL, output_root=str(OUTPUT), input_admission=admission,
        planned_case=list(CASE), implementation_class='DirectFlowResidualAnchoredController',
    ...
    if args.preflight_only:
        print('NO_RGB_JEPA_DIRECT_FLOW_NATIVE_PREFLIGHT', json.dumps(dict(source_count=len(sources), hardware=resources,
    ...
    print('NO_RGB_JEPA_DIRECT_FLOW_NATIVE_LAUNCHED', launch_sha, flush=True); started = time.perf_counter()
    try:
        ...
        print('NO_RGB_JEPA_DIRECT_FLOW_NATIVE_TERMINAL', record['status'], record.get('verified_round_trip'), record.get('failure'), flush=True)
        if record['status'] != WORKER_STATUS:
            raise ValueError('tracking-recovery collection/audit/prefix incomplete: '+str(record.get('failure')))
        ...
        write_json(OUTPUT/'result.json', dict(status='NO_RGB_JEPA_DIRECT_FLOW_MAZE02_PILOT_V1_COMPLETE',
            ...
            original_adapter_batch_result_sha256=args.adapter_batch_result_sha256,
            original_ordered_waiter_result_sha256=identities,
            measured_round_trip_successes=int(record['verified_round_trip']), reused_layout_executions=1,
            new_independent_layout_executions=0, direct_corner_flow_missingness_fallback_enabled=True, model_training=False,
            navigation_qualified=False, real_time_qualified=False, hardware_qualified=False, goal_achieved=False))
        print('NO_RGB_JEPA_DIRECT_FLOW_NATIVE_COMPLETE', digest(OUTPUT/'result.json'), flush=True)
    except Exception as error:
        write_json(OUTPUT/'failure.json', dict(status='TERMINAL_NO_RGB_JEPA_DIRECT_FLOW_NATIVE_FAILURE', reason=repr(error), automatic_retry=False)); raise
