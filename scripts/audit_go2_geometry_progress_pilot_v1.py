"""Complete raw replay and predeclared local-progress gate; no paired-image claim."""
import argparse
import json
import cv2
import numpy as np
...
from scripts.pulse_context_sensor_audit_development import audit_sensors, read_json
from scripts.audit_go2_independent_pulse_context_pilot_v1 import audit_setup, audit_stops, fingerprint
from scripts.run_go2_geometry_progress_pilot_v1 import OUTPUT, PROTOCOL, RESERVE, artifacts
...
def audit_condition(trial,result,definition):
    directory=OUTPUT/trial;spec=specification(trial);cell=assignments()[trial]
...
def main():
    ...
    parser.add_argument('--collection-launch-sha256',required=True)
    parser.add_argument('--collection-result-sha256',required=True)
    args=parser.parse_args();validate_root(OUTPUT)
    for name in ('geometry_progress_audit_launch.json','geometry_progress_audit.json','geometry_progress_audit_failure.json'):
        if (OUTPUT/name).exists():raise ValueError('exclusive frozen raw audit; no retry/resume')
    cv2.setNumThreads(1)
    ids={'launch.json':args.collection_launch_sha256,'result.json':args.collection_result_sha256}
    verify_artifacts(OUTPUT,ids);launch=read_json(OUTPUT,'launch.json');result=read_json(OUTPUT,'result.json');verify(launch)
    ...
    bindings=ids|result['artifact_sha256'];verify_artifacts(OUTPUT,bindings)
    write_json(OUTPUT/'geometry_progress_audit_launch.json',dict(status='RAW_AUDIT_LAUNCHED',input_sha256=bindings,
        source_sha256=launch['source_sha256'],same_observation_counterfactual_claim=False,model_training=False))
    reports=[]
    try:
        for c in TRIALS:
            ...
            print('RAW_GEOMETRY_PROGRESS',c,report['outcome'],flush=True)
        verify(launch);verify_artifacts(OUTPUT,bindings)
        products=['geometry_progress_audit_launch.json']+[c+'_geometry_progress_audit.json' for c in TRIALS]
        write_json(OUTPUT/'geometry_progress_audit.json',dict(status='GEOMETRY_PROGRESS_RAW_AUDIT_COMPLETE',
            collection_sha256=ids,output_sha256={n:digest(OUTPUT/n) for n in products},
        ...
        write_json(OUTPUT/'geometry_progress_audit_failure.json',dict(status='TERMINAL_RAW_AUDIT_FAILURE',
