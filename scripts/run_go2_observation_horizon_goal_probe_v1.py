...
from scripts.observation_horizon_goal_episode_development import collect,artifacts
from scripts.observation_horizon_goal_audit_development import audit
...
PREVIOUS=BASE/'go2_observation_replan_goal_probe_v1_attempt_001'
DIAGNOSTIC=BASE/'go2_observation_replan_goal_readout_v1_attempt_001'
PREVIOUS_SHA='8cdfd800eda961de5b1a58a3b64fe8fd471c0f8c8165aae9c61f699c500010eb'
DIAGNOSTIC_SHA='be2c24798bca35111c62b3795c42c8eb14783405b7b1e6afe3b2f7a1c59f554d'
OUTPUT=BASE/'go2_observation_horizon_goal_probe_v1_attempt_001'
...
PROTOCOL='docs/go2_observation_horizon_goal_probe_v1_2026-09-08.md'
...
def worker(case,launch_sha):
    ...
    terminal=dict(case=name,trial=trial,variant=variant,condition=condition,model_name=model_name,
        status='OBSERVATION_HORIZON_GOAL_WORKER_FAILED',artifact_sha256={});started=time.perf_counter()
    with (OUTPUT/(name+'_worker.log')).open('x') as log,contextlib.redirect_stdout(log),contextlib.redirect_stderr(log):
        try:
            ...
            terminal.update(status='OBSERVATION_HORIZON_GOAL_COLLECTED_AND_RAW_AUDITED',artifact_sha256=bindings,
...
def main():
    ...
    assert previous['status']=='OBSERVATION_REPLAN_GOAL_PROBE_COMPLETE' and previous['cases']==[list(c) for c in CASES]
    ...
    launch=preflight(output=OUTPUT,protocol=PROTOCOL,
        seed_paths=('scripts/run_go2_observation_horizon_goal_probe_v1.py',
            'lewm/tests/test_observation_horizon_goal_scope_development.py',
    ...
    launch.update(previous_probe_artifact_sha256=previous_ids,cadence_readout_artifact_sha256=diagnostic_ids,
        ...
        experiment='fixed short-horizon full-JEPA and full-direct first-seed models; replan at each 100-ms observation with trained 100-ms first forecast',
        concurrency_reason='two independent scenes; bounded by the passing four-process native benchmark and completed two-scene cadence probe',
    ...
    print('OBSERVATION_HORIZON_GOAL_LAUNCHED',digest(OUTPUT/'launch.json'),flush=True)
    ...
    try:
            ...
            with ProcessPoolExecutor(max_workers=2,mp_context=multiprocessing.get_context('spawn'),max_tasks_per_child=1) as pool:
                ...
                while pending:
                    ...
                    for future in done:
                        ...
                        except Exception as error:row=dict(case=case[0],status='OBSERVATION_HORIZON_GOAL_PROCESS_FAILURE',failure=repr(error))
                        records.append(row);print('OBSERVATION_HORIZON_GOAL_TERMINAL',case[0],row['status'],row.get('goal'),flush=True)
        if len(records)!=2 or any(r['status']!='OBSERVATION_HORIZON_GOAL_COLLECTED_AND_RAW_AUDITED' for r in records):
        ...
        result=dict(status='OBSERVATION_HORIZON_GOAL_PROBE_COMPLETE',cases=CASES,conditions=records,
        ...
        write_json(OUTPUT/'result.json',result);print('OBSERVATION_HORIZON_GOAL_COMPLETE',digest(OUTPUT/'result.json'),flush=True)
    except Exception as error:
        write_json(OUTPUT/'failure.json',dict(status='TERMINAL_OBSERVATION_HORIZON_GOAL_FAILURE',reason=repr(error),
