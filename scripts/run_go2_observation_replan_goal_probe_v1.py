"""Two fixed augmented models with observation-cadence receding execution."""
import argparse
import contextlib
import json
...
from scripts.observation_replan_goal_episode_development import collect,artifacts
from scripts.observation_replan_goal_audit_development import audit
from scripts.augmented_family_switch_model_admission_development import admit,load_assigned
from scripts.run_go2_augmented_family_switch_fits_v1 import OUTPUT as FITS
from scripts.read_go2_augmented_family_switch_fits_v1 import OUTPUT as READOUT
...
PREVIOUS=BASE/'go2_augmented_family_switch_goal_probe_v1_attempt_001'
DIAGNOSTIC=BASE/'go2_augmented_commitment_clearance_v1_attempt_001'
PREVIOUS_SHA='f2e0a45a77f27b7a23ca3f621d40de68ad9fcb10c40515abcefb11e7c6e85dab'
DIAGNOSTIC_SHA='991feea7a987e9c75027453f5f2d9a8dcd7034e2d7a14bb722841a53eb8c292c'
OUTPUT=BASE/'go2_observation_replan_goal_probe_v1_attempt_001'
...
PROTOCOL='docs/go2_observation_replan_goal_probe_v1_2026-09-08.md'
...
def worker(case,launch_sha):
    ...
    terminal=dict(case=name,trial=trial,variant=variant,condition=condition,model_name=model_name,
        status='OBSERVATION_REPLAN_GOAL_WORKER_FAILED',artifact_sha256={});started=time.perf_counter()
    with (OUTPUT/(name+'_worker.log')).open('x') as log,contextlib.redirect_stdout(log),contextlib.redirect_stderr(log):
        try:
            ...
            terminal.update(status='OBSERVATION_REPLAN_GOAL_COLLECTED_AND_RAW_AUDITED',artifact_sha256=bindings,
...
def main():
    ...
    assert previous['status']=='AUGMENTED_FAMILY_SWITCH_GOAL_PROBE_COMPLETE' and previous['cases']==[list(c) for c in CASES]
    ...
    assert diagnostic['status']=='AUGMENTED_COMMITMENT_CLEARANCE_DIAGNOSTIC_COMPLETE'
    ...
    if readout['status']!='AUGMENTED_FAMILY_SWITCH_FIT_READOUT_COMPLETE' or readout['final_models_admitted']!=18:
    ...
    launch=preflight(output=OUTPUT,protocol=PROTOCOL,
        seed_paths=('scripts/run_go2_observation_replan_goal_probe_v1.py',
            'lewm/tests/test_observation_replan_goal_probe_development.py',
            'docs/go2_augmented_commitment_clearance_result_2026-09-08.md',
            *previous['source_sha256'],*diagnostic['source_sha256'],
            'docs/go2_augmented_family_switch_fits_result_2026-09-08.md',
    ...
    launch.update(previous_probe_artifact_sha256=previous_ids,clearance_diagnostic_artifact_sha256=diagnostic_ids,
        ...
        experiment='fixed augmented full-JEPA and full-direct first-seed models; replan at each 100-ms observation with unchanged 500-ms forecast',
        concurrency_reason='two independent scenes; bounded by passing four-process native benchmark and four-worker fitting benchmark',
    ...
    print('OBSERVATION_REPLAN_GOAL_LAUNCHED',digest(OUTPUT/'launch.json'),flush=True)
    ...
    try:
            ...
            with ProcessPoolExecutor(max_workers=2,mp_context=multiprocessing.get_context('spawn'),max_tasks_per_child=1) as pool:
                ...
                while pending:
                    ...
                    for future in done:
                        ...
                        except Exception as error:row=dict(case=case[0],status='OBSERVATION_REPLAN_GOAL_PROCESS_FAILURE',failure=repr(error))
                        records.append(row);print('OBSERVATION_REPLAN_GOAL_TERMINAL',case[0],row['status'],row.get('goal'),flush=True)
        if len(records)!=2 or any(r['status']!='OBSERVATION_REPLAN_GOAL_COLLECTED_AND_RAW_AUDITED' for r in records):
        ...
        result=dict(status='OBSERVATION_REPLAN_GOAL_PROBE_COMPLETE',cases=CASES,conditions=records,
        ...
        write_json(OUTPUT/'result.json',result);print('OBSERVATION_REPLAN_GOAL_COMPLETE',digest(OUTPUT/'result.json'),flush=True)
    except Exception as error:
        write_json(OUTPUT/'failure.json',dict(status='TERMINAL_OBSERVATION_REPLAN_GOAL_FAILURE',reason=repr(error),
