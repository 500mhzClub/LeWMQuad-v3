"""Public-sensor replay until the first unexecuted translating-recovery command."""
import json
import time
import cv2
import torch
...
from scripts.run_go2_executed_waypoint_maze_pilot_v1 import OUTPUT as INPUT, CASE, CORRECTION, FITS
from scripts.read_go2_executed_waypoint_maze_pilot_v1 import OUTPUT as READOUT
...
OUTPUT = BASE/'go2_view_reentry_maze_prefix_v1_attempt_001'
PROTOCOL = 'docs/go2_view_reentry_maze_prefix_v1_2026-09-08.md'
INPUT_SHA = 'd5ba1136c969cd6591028f435a93f0e35a4dae7b9cbf217524887b459271ace8'
READOUT_SHA = '1bb9ae412452fd68a99fa827ad304d9201f5dfdc2b7bb24f57e003390ba7799c'
...
def replay(admission):
    ...
    controller = ViewReentryRoundTripController(model, ArticulatedCollisionGeometry(URDF),
    ...
    with writer(OUTPUT) as append:
        for i, original in enumerate(read_rows(directory)):
            ...
            assert set(r) == set(old) | {'view_reentry_translation_policy_enabled'}
            assert r['view_reentry_translation_policy_enabled'] is True
            assert (a is None) == (b is None)
            if a is not None:
                B = controller.mapper.map_from_initial
                compare_selection(a, b, B@controller.mapper.surface.position,
                    B@controller.mapper.surface.rotation, controller.mapper.occupied)
            ...
            if i % 100 == 0: print('VIEW_REENTRY_PREFIX_FRAME', i, flush=True)
...
def main():
    ...
    bound = []
    for root, sha, status in ((INPUT, INPUT_SHA, 'EXECUTED_WAYPOINT_MAZE_PILOT_COMPLETE'),
            (READOUT, READOUT_SHA, 'EXECUTED_WAYPOINT_MAZE_READOUT_COMPLETE')):
    ...
    sources = discover_sources((PROTOCOL, 'scripts/replay_go2_view_reentry_maze_prefix_v1.py',
        'lewm/tests/test_view_reentry_selection_development.py',
        'docs/go2_executed_waypoint_maze_pilot_result_2026-09-08.md'), bound[-1][2]['source_sha256'])
    resources = hardware()
    ...
    print('VIEW_REENTRY_PREFIX_LAUNCHED', digest(OUTPUT/'launch.json'), flush=True)
    try:
        ...
        write_json(OUTPUT/'result.json', dict(status='VIEW_REENTRY_MAZE_PREFIX_COMPLETE', source_sha256=sources,
        ...
        print('VIEW_REENTRY_PREFIX_COMPLETE', digest(OUTPUT/'result.json'), {k: v for k, v in report.items() if k != 'changed_selection'}, flush=True)
    except Exception as error:
        write_json(OUTPUT/'failure.json', dict(status='TERMINAL_VIEW_REENTRY_PREFIX_FAILURE', reason=repr(error)))
        raise
