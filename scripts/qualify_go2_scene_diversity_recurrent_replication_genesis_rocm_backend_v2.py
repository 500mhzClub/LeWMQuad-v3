#!/usr/bin/env python3
"""Qualify the fresh V2 exact-``ld.lld`` Genesis ROCm successor.

The fresh V2 root is consumed before this module invokes any preflight.  The
only material runtime change from V1 is preserving the Unix-driver symlink as
``argv[0]``; HIP, EGL, and both full-scene probes remain unchanged.
"""
...
from scripts import qualify_go2_scene_diversity_recurrent_replication_genesis_rocm_backend_v1 as predecessor  # noqa: E402


pilot = collector.pilot
kernel = collector.kernel
# The inherited scene implementation, not the V1 identity adapter.
scene_predecessor = collector.predecessor.predecessor

QUALIFICATION_AUTHORITY_SCHEMA = (
    "lewm_go2_scene_diversity_recurrent_replication_"
    "genesis_rocm_backend_v2_qualification_authority_v1"
)
QUALIFICATION_AUTHORITY_STATUS = (
    "AUTHORIZED_GENESIS_ROCM_BACKEND_V2_QUALIFICATION_ONLY"
)
...
QUALIFICATION_RESERVATION_SCHEMA = (
    "lewm_go2_scene_diversity_recurrent_replication_"
    "genesis_rocm_backend_v2_qualification_reservation_v1"
)
QUALIFICATION_SCENE_RESULT_SCHEMA = (
    "lewm_go2_scene_diversity_recurrent_replication_"
    "genesis_rocm_backend_v2_qualification_scene_result_v1"
)
QUALIFICATION_RESULT_SCHEMA = (
    "lewm_go2_scene_diversity_recurrent_replication_"
    "genesis_rocm_backend_v2_qualification_result_v1"
)
QUALIFICATION_TERMINAL_SCHEMA = (
    "lewm_go2_scene_diversity_recurrent_replication_"
    "genesis_rocm_backend_v2_qualification_terminal_v1"
)
QUALIFICATION_RESULT_STATUS = "PASS_GENESIS_ROCM_BACKEND_V2_QUALIFICATION"
...
SOURCE_REVIEW = REPO_ROOT / (
    "docs/lewm_go2_scene_diversity_recurrent_replication_"
    "genesis_rocm_backend_v2_source_review_2026-08-04.json"
)
QUALIFICATION_AUTHORITY = REPO_ROOT / (
    "docs/lewm_go2_scene_diversity_recurrent_replication_"
    "genesis_rocm_backend_v2_qualification_authority_2026-08-04.json"
)
...
class GenesisRocmBackendV2QualificationError(RuntimeError):
    """Raised when fresh V2 qualification fails closed."""


GenesisRocmBackendQualificationError = GenesisRocmBackendV2QualificationError


def _source_paths() -> Mapping[str, Path]:
    from scripts import run_go2_scene_diversity_recurrent_replication_genesis_rocm_backend_v2 as runner
...
@contextmanager
def _configured_predecessor_qualifier_v2() -> Iterator[None]:
...
def _configured_qualification_collector() -> Iterator[None]:
    with _configured_predecessor_qualifier_v2():
...
def _run_rocm_egl_preflight(
    ...
    if expectation != plan_builder.ROCM_GRAPHICS_PREFLIGHT_EXPECTATION:
        raise GenesisRocmBackendV2QualificationError(
            "V2 ROCm preflight expectation changed"
        )
    for name in (
        "eglinfo_executable",
        "rocminfo_executable",
        ...
        pilot.require_binding(runtime[name], label=f"V2 preflight {name}")
    ...
    try:
        ...
        raise GenesisRocmBackendV2QualificationError(
            "V2 ld.lld driver entrypoint is unavailable"
        ) from exc
    if (
        ...
        raise GenesisRocmBackendV2QualificationError(
            "V2 PATH/ROCM_PATH ld.lld driver identity changed"
        )
...
"""
    ...
    return {
        ...
        # New V2 evidence keeps lexical drivers separate from their target.
        # Retain the inherited resolved-target fields for the full frozen
        # qualification validator; V2 adds and independently validates the
...
def _worker_argv_qualification(**kwargs: Any) -> list[str]:
    ...
    if (
        ...
        raise GenesisRocmBackendV2QualificationError(
            "V2 ROCm worker entry point changed"
        )
...
def _child_environment(plan: Mapping[str, Any]) -> dict[str, str]:
    with _configured_predecessor_qualifier_v2():
        return predecessor._child_environment(plan)  # noqa: SLF001
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if "--worker-scene-index" in raw:
        with (
            _configured_predecessor_qualifier_v2(),
    ...
    try:
        ...
        if (
            ...
            raise GenesisRocmBackendV2QualificationError(
                "V2 qualification CLI plan pins differ from authority"
            )
