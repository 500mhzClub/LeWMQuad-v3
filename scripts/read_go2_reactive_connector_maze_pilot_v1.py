...
from scripts.run_go2_reactive_connector_maze_pilot_v1 import OUTPUT as INPUT, CASE
from scripts.run_go2_reactive_nominal_maze_pilot_v1 import WAYPOINT_READOUT, WAYPOINT_READOUT_SHA
...
OUTPUT = BASE/'go2_reactive_connector_maze_readout_v1_attempt_001'
PROTOCOL = 'docs/go2_reactive_connector_maze_readout_v1_2026-09-08.md'
...
def main():
    ...
    assert result['status'] == 'REACTIVE_CONNECTOR_MAZE_PILOT_COMPLETE' and len(result['conditions']) == 1
    ...
    sources = discover_sources((PROTOCOL, 'scripts/read_go2_reactive_connector_maze_pilot_v1.py',
        'lewm/tests/test_reactive_connector_execution_readout_development.py'), old['source_sha256'])
    resources = hardware()
    ...
    try:
        ...
        verify_artifacts(WAYPOINT_READOUT, {'result.json': WAYPOINT_READOUT_SHA})
        write_json(OUTPUT/'result.json', dict(status='REACTIVE_CONNECTOR_MAZE_READOUT_COMPLETE',
        ...
        print('REACTIVE_CONNECTOR_MAZE_READOUT_COMPLETE', digest(OUTPUT/'result.json'), flush=True)
    except Exception as error:
        write_json(OUTPUT/'failure.json', dict(status='TERMINAL_REACTIVE_CONNECTOR_MAZE_READOUT_FAILURE', reason=repr(error)))
        raise
