#!/usr/bin/env python3
"""Authority-first launcher for Action-Query Spatial Successor JEPA V1."""
...
contract = _source_module(
    "_lewm_action_query_spatial_successor_launcher_contract",
    ROOT / CONTRACT_RELATIVE_PATH,
)
if ROOT / contract.LAUNCHER_RELATIVE_PATH != LAUNCHER_PATH:
    raise PermissionError("Action-Query Spatial Successor launcher path changed")

_V3 = _source_module(
    "_lewm_action_query_spatial_successor_frozen_v3_launcher",
...
def _rebind_inherited_launcher_once() -> None:
    """Install the new authority identity directly on the deepest launcher."""
...
_rebind_inherited_launcher_once()
...
def _assert_final_launcher_bindings() -> None:
    if (
        ...
        or _BASE.LAUNCHER_PATH != LAUNCHER_PATH
        or _BASE.RUNNER_PATH != ROOT / contract.RUNNER_RELATIVE_PATH
        or _BASE.OUTPUT_ROOT != ROOT / contract.OUTPUT_ROOT_RELATIVE_PATH
    ):
        raise PermissionError("final Action-Query launcher bindings changed")
