...
from lewm.observation_horizon_fit_development import train, predict, score
from scripts.observation_horizon_fit_inputs_development import authenticate, stream, TARGET_SHA, OPTIMIZATION_SEEDS
...
BENCH = BASE/'go2_observation_horizon_fit_benchmark_v1_attempt_001'
OUTPUT = BASE/'go2_observation_horizon_fits_v1_attempt_001'
PROTOCOL = 'docs/go2_observation_horizon_fits_v1_2026-09-08.md'
...
def worker(request):
    ...
    terminal = dict(name=name, status='OBSERVATION_HORIZON_WORKER_FAILED', actual_updates=0, artifact_sha256={})
    ...
    with (output/(name+'_worker.log')).open('x') as log, contextlib.redirect_stdout(log), contextlib.redirect_stderr(log):
        try:
            ...
            check_sha=launch['science']['input_result_sha256']
            ...
            if benchmark and (request['phase'] not in ('serial', 'parallel') or request['case'] not in range(4)
                    or request['seed'] != 2026091610+request['case'] or request['variant'] != 'full' or request['condition'] != 'jepa'):
                raise ValueError('exact separate benchmark case required')
            write_json(output/(name+'_request.json'), request)
            trainer = ObservationHorizonTrainer(request['condition'], seed=request['seed'], latent_dim=32)
            ...
            with (output/ledger_name).open('xb') as ledger:
                def record(row):
                    ...
                    if count == 1 or count % 100 == 0:
                        print('OBSERVATION_HORIZON_UPDATE', name, count, flush=True)
            ...
            terminal.update(status='OBSERVATION_HORIZON_WORKER_COMPLETE', actual_updates=count,
        ...
        for at in range(0, len(requests), workers):
            ...
            while pending:
                ...
                for future in done:
                    ...
                    print('OBSERVATION_HORIZON_TERMINAL', name, result['status'], result.get('failure'), flush=True)
            records += batch
            if any(r['status'] != 'OBSERVATION_HORIZON_WORKER_COMPLETE' for r in batch):
...
def benchmark_decision(serial, parallel):
    ...
    for i, (x, y) in enumerate(zip(a, b, strict=True)):
        if (x['name'] != f'serial_{i}' or y['name'] != f'parallel_{i}'
                or x['status'] != y['status'] or x['status'] != 'OBSERVATION_HORIZON_WORKER_COMPLETE'
...
def main():
    ...
    parser.add_argument('--benchmark-result-sha256'); parser.add_argument('--input-result-sha256',required=True); args = parser.parse_args()
    ...
    definition, checked, schedules = authenticate(args.input_result_sha256)
    sources = discover_sources((PROTOCOL, 'scripts/run_go2_observation_horizon_fits_v1.py',
        'docs/go2_observation_horizon_family_design_v1_2026-09-08.md',
        'docs/go2_observation_horizon_inputs_result_2026-09-08.md',
        'lewm/tests/test_observation_horizon_fit_development.py',
        'lewm/tests/test_observation_horizon_stream_development.py',
        'scripts/observation_horizon_model_admission_development.py',
        'lewm/tests/test_observation_horizon_model_admission_development.py',
        'lewm/tests/test_observation_horizon_fit_benchmark_development.py'), definition['source_sha256'])
    resources = hardware()
    ...
    pair=dict(targets=TARGET_SHA,inputs=args.input_result_sha256)
    ...
    science = dict(optimization_seeds=list(OPTIMIZATION_SEEDS), variants=list(VARIANTS), conditions=list(CONDITIONS), updates=1200,
        ...
        target_cadence_ns=100_000_000,maximum_horizon_ns=800_000_000,action_ticks_per_block=1,
        input_result_sha256=args.input_result_sha256,dataset_pair_sha256=pair_sha,
        source_batches=dict(family=600,switch=600),schedule_sha256={seed:row['schedule_sha256'] for seed,row in schedules.items()},
        benchmark_seeds=list(range(2026091610, 2026091614)), benchmark_updates=20,
    ...
    if args.phase == 'fits':
        ...
        if bench['status'] != 'OBSERVATION_HORIZON_FIT_BENCHMARK_COMPLETE' or bench['science'] != science:
    ...
    launch_sha = digest(output/'launch.json'); print('OBSERVATION_HORIZON_PHASE_LAUNCHED', args.phase, launch_sha, flush=True)
    ...
    try:
        with (output/'resource_monitor.jsonl').open('x') as monitor:
            if args.phase == 'benchmark':
                ...
                for phase, n in (('serial', 1), ('parallel', 4)):
                    jobs = [request(f'{phase}_{i}', seed=2026091610+i, variant='full', condition='jepa', phase=phase, case=i) for i in range(4)]
                    phases[phase] = dispatch(jobs, workers=n, monitor=monitor)
                decision = benchmark_decision(phases['serial'], phases['parallel'])
                result = dict(status='OBSERVATION_HORIZON_FIT_BENCHMARK_COMPLETE', **phases, decision=decision)
                ...
                result = dict(status='OBSERVATION_HORIZON_EIGHTEEN_FITS_COMPLETE', **phase, selected_workers=workers, optimizer_updates=21600)
        ...
        authenticate(args.input_result_sha256); verify(launch); verify_artifacts(output, bindings)
        ...
        write_json(output/'result.json', result); print('OBSERVATION_HORIZON_PHASE_COMPLETE', args.phase, digest(output/'result.json'), flush=True)
    except Exception as error:
        write_json(output/'failure.json', dict(status='TERMINAL_OBSERVATION_HORIZON_PHASE_FAILURE', phase=args.phase, reason=repr(error)))
        raise
