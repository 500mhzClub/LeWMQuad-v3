...
import numpy as np
from scripts.run_go2_eight_step_planning_goal_probe_v1 import OUTPUT as INPUT,PREVIOUS as PRIOR,PREVIOUS_SHA as PRIOR_SHA,CASES,TRIAL
...
OUTPUT=BASE/'go2_eight_step_planning_goal_readout_v1_attempt_001'
PROTOCOL='docs/go2_eight_step_planning_goal_readout_v1_2026-09-08.md'
...
def compare(a,b,*,same_model=False):
    ...
    if same_model and predictions[0]!=predictions[1]:raise ValueError('unchanged model must reproduce every common-prefix forecast')
    return dict(first=str(a),second=str(b),first_requested_command_difference=command,
        ...
        model_predictions_expected_identical=same_model,common_prefix_model_forecasts_exact=predictions[0]==predictions[1],
...
def main():
    ...
    assert result['status']=='EIGHT_STEP_PLANNING_GOAL_PROBE_COMPLETE' and result['cases']==[list(c) for c in CASES]
    ...
    sources=discover_sources((PROTOCOL,'scripts/read_go2_eight_step_planning_goal_probe_v1.py',
        'lewm/tests/test_eight_step_planning_goal_readout_development.py'),old['source_sha256'])
    ...
    try:
        ...
        comparisons=[compare(PRIOR/c[0],INPUT/c[0],same_model=True) for c in CASES]
        ...
        write_json(OUTPUT/'result.json',dict(status='EIGHT_STEP_PLANNING_GOAL_READOUT_COMPLETE',
        ...
        print('EIGHT_STEP_PLANNING_GOAL_READOUT_COMPLETE',digest(OUTPUT/'result.json'),flush=True)
    except Exception as error:
        write_json(OUTPUT/'failure.json',dict(status='TERMINAL_EIGHT_STEP_PLANNING_GOAL_READOUT_FAILURE',reason=repr(error)));raise
