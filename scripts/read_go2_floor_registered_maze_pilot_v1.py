...
from scripts.run_go2_floor_registered_maze_pilot_v1 import OUTPUT as INPUT, CASE
...
OUTPUT = BASE/'go2_floor_registered_maze_readout_v1_attempt_001'
PROTOCOL = 'docs/go2_floor_registered_maze_readout_v1_2026-09-08.md'
...
def main():
    ...
    assert result['status'] == 'FLOOR_REGISTERED_MAZE_PILOT_COMPLETE' and len(result['conditions']) == 1
    ...
    sources = discover_sources((PROTOCOL, 'scripts/read_go2_floor_registered_maze_pilot_v1.py',
    ...
    try:
        ...
        write_json(OUTPUT/'result.json', dict(status='FLOOR_REGISTERED_MAZE_READOUT_COMPLETE',
        ...
        print('FLOOR_REGISTERED_MAZE_READOUT_COMPLETE', digest(OUTPUT/'result.json'), flush=True)
    except Exception as error:
        write_json(OUTPUT/'failure.json', dict(status='TERMINAL_FLOOR_REGISTERED_MAZE_READOUT_FAILURE', reason=repr(error)))
        raise
