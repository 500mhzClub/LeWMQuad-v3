"""Fresh sustained-turn physics after the completed raw prefix and original five-stage queue."""
import argparse
import contextlib
...
from scripts.sustained_hold_reorientation_native_prefix_development import compare, boundary
from scripts.sustained_hold_reorientation_maze02_episode_development import collect, artifacts
from scripts.sustained_hold_reorientation_maze02_audit_development import audit
...
OUTPUT = BASE/'go2_sustained_hold_reorientation_maze02_pilot_v1_attempt_001'
SOURCE = 'scripts/run_go2_sustained_hold_reorientation_maze02_pilot_v1.py'
PROTOCOL = 'docs/go2_sustained_hold_reorientation_maze02_pilot_v1_2026-09-11.md'
TESTS = ('lewm/tests/test_sustained_hold_reorientation_native_launcher_development.py',
    'lewm/tests/test_sustained_hold_reorientation_native_prefix_development.py',
    'lewm/tests/test_sustained_hold_reorientation_native_source_development.py')
CASE = ('full_jepa_sustained_hold_reorientation_maze_02', *inputs.replay.original.CASE[1:])
WORKER_STATUS = 'SUSTAINED_HOLD_REORIENTATION_MAZE02_COLLECTED_AND_RAW_AUDITED'
...
def verify_inputs(launch, *, full=False):
    ...
    expected = dict(planned_case=list(CASE), output_root=str(OUTPUT),
        ...
        hold_reorientation_enabled=True, sustained_hold_reorientation_enabled=True, planner_interface_adapter_enabled=True,
    ...
    if any(launch[k] != v or type(launch[k]) is not type(v) for k, v in expected.items()):
        raise ValueError('exact prospective sustained hold-reorientation native definition required')
    admission = launch['input_admission']; boundary(admission['prefix_report'])
    inputs.verify_bound(admission, launch['source_sha256'])
    if full and inputs.admit(admission['raw_prefix_result_sha256'], admission['budget_wait_result_sha256'],
            launch['source_sha256']) != admission:
        raise ValueError('complete original sustained hold-reorientation input admission changed')
...
def assigned_model(launch):
    ...
    if (type(model) is not AllPhasePlannerModel or model.training
        ...
        raise ValueError('original assigned sustained hold-reorientation model state required')
    return model
...
def require_worker(record, report, prefix_report):
    if (record['status'] != WORKER_STATUS or 'failure' in record or record['case'] != CASE[0]
            ...
            or report['sustained_hold_reorientation_enabled'] is not True
            or record['collection']['status'] != 'SUSTAINED_HOLD_REORIENTATION_MAZE02_TERMINAL_AUDIT_REQUIRED'
            or record['collection']['hold_reorientation_enabled'] is not True
            or record['collection']['sustained_hold_reorientation_enabled'] is not True):
        raise ValueError('complete exact sustained hold-reorientation collection and raw audit required')
    ...
    for key in ('physical_and_public_prefix_exact', 'all_preintervention_requested_commands_exact',
        ...
        if receipt[key] is not True: raise ValueError('complete physical sustained hold-reorientation intervention required: '+key)
...
def worker(launch_sha):
    ...
    record = dict(case=name, layout_index=index, variant=variant, condition=condition, model_name=model_name,
        status='SUSTAINED_HOLD_REORIENTATION_MAZE02_WORKER_FAILED', artifact_sha256={}); started = time.perf_counter()
    with (OUTPUT/(name+'_worker.log')).open('x') as log, contextlib.redirect_stdout(log), contextlib.redirect_stderr(log):
        try:
            ...
            if state_digest(model.state_dict()) != before: raise ValueError('collection changed assigned sustained hold-reorientation model')
            ...
            receipt = compare(inputs.replay.original.OUTPUT/inputs.replay.original.CASE[0], OUTPUT/name,
            ...
            record.update(status=WORKER_STATUS, artifact_sha256=ids, prefix_comparison=receipt, readout=readout,
                model_state_sha256=before, model_state_unchanged=True, hold_reorientation_enabled=True, sustained_hold_reorientation_enabled=True,
            ...
            import traceback
            traceback.print_exc(); record.update(status='SUSTAINED_HOLD_REORIENTATION_MAZE02_WORKER_FAILED', failure=repr(error))
...
def main():
    ...
    parser.add_argument('--raw-prefix-result-sha256'); parser.add_argument('--budget-wait-result-sha256')
    ...
    if OUTPUT.exists() or OUTPUT.is_symlink(): raise ValueError('exclusive sustained hold-reorientation native attempt; no retry/resume')
    ...
    if args.source_preflight_only:
        print('SUSTAINED_HOLD_REORIENTATION_NATIVE_SOURCE_PREFLIGHT', json.dumps(dict(source_count=len(sources), hardware=resources,
            native_execution=False, output_created=False, complete_input_admission_performed=False)), flush=True); return
    if not args.raw_prefix_result_sha256 or not args.budget_wait_result_sha256:
        raise ValueError('exact completed raw-prefix result and final original budget waiter identities required')
    print('SUSTAINED_HOLD_REORIENTATION_NATIVE_INPUT_ADMISSION_STARTED', len(sources), flush=True)
    admission = inputs.admit(args.raw_prefix_result_sha256, args.budget_wait_result_sha256, sources)
    ...
    launch.update(source_sha256=sources, protocol=PROTOCOL, output_root=str(OUTPUT), input_admission=admission,
        planned_case=list(CASE), implementation_class='SustainedHoldReorientationController',
    ...
    if args.preflight_only:
        print('SUSTAINED_HOLD_REORIENTATION_NATIVE_PREFLIGHT', json.dumps(dict(source_count=len(sources), hardware=resources,
    ...
    print('SUSTAINED_HOLD_REORIENTATION_NATIVE_LAUNCHED', launch_sha, flush=True); started = time.perf_counter()
    try:
        ...
        print('SUSTAINED_HOLD_REORIENTATION_NATIVE_TERMINAL', record['status'], record.get('verified_round_trip'), record.get('failure'), flush=True)
        if record['status'] != WORKER_STATUS:
            raise ValueError('sustained hold-reorientation collection/audit/prefix incomplete: '+str(record.get('failure')))
        ...
        write_json(OUTPUT/'result.json', dict(status='SUSTAINED_HOLD_REORIENTATION_MAZE02_PILOT_V1_COMPLETE',
            ...
            raw_prefix_result_sha256=args.raw_prefix_result_sha256, budget_wait_result_sha256=args.budget_wait_result_sha256,
            measured_round_trip_successes=int(record['verified_round_trip']), reused_layout_executions=1,
            new_independent_layout_executions=0, hold_reorientation_enabled=True, sustained_hold_reorientation_enabled=True, model_training=False,
            navigation_qualified=False, real_time_qualified=False, hardware_qualified=False, goal_achieved=False))
        print('SUSTAINED_HOLD_REORIENTATION_NATIVE_COMPLETE', digest(OUTPUT/'result.json'), flush=True)
    except Exception as error:
        write_json(OUTPUT/'failure.json', dict(status='TERMINAL_SUSTAINED_HOLD_REORIENTATION_NATIVE_FAILURE', reason=repr(error), automatic_retry=False)); raise
