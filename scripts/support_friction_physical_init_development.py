"""Fresh matched-condition scene init; unchanged gait and native contact logger."""
import copy
import numpy as np
from lewm.support_friction_challenge_development import pack,specification,native_friction
...
class SupportFrictionPhysicalInit(_GenesisPhysicalSession):
    def __init__(self,spec,*,backend):
        ...
        if self.backend!='cpu' or spec!=specification(spec['trial']): raise ExperimentError('exact CPU friction challenge required')
        ...
        self._runtime=dict(backend='cpu',n_envs=1,physics_dt_s=.002,policy_dt_s=.02,command_dt_s=.1,
            ...
            ppo_observation_contact_inputs='none_in_frozen45elementcontract',foot_sensor_role='new shadow observation only')
