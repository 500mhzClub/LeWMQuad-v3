...
from lewm.auxiliary_downward45_depth_geometry_development import reference_pose,body_from_optical
...
from scripts.auxiliary_downward45_frame_acquisition_development import capture_frame
...
from scripts.characterize_go2_auxiliary_downward45_depth_geometry_v1 import OUTPUT as GEOMETRY
...
from scripts.near_field_sensor_audit_development import audit_sensors
...
OUTPUT=BASE/'go2_auxiliary_downward45_depth_prefix_v1_attempt_001'
PROTOCOL='docs/go2_auxiliary_downward45_depth_prefix_v1_2026-09-08.md'
GEOMETRY_SHA='e0a61165564fd3acec667c58e525c33fd6ba626228e25eef0a51a501704b12c6'
...
COMMAND_TICKS=17
PREVIOUS_CAPTURE=BASE/'go2_auxiliary_tilted_depth_prefix_integrity_v2_attempt_001'
PREVIOUS_CAPTURE_IDS={'launch.json':'a01f0975b8d80cbd9633672db9b0cb397d08372bccb5a89cf222ee651308fcf7',
    'result.json':'e8de0873c71f50c5a01793c0464ba2014c1a7fe72f77daf47204eaeee1c8c38a'}
...
def collect(launch,tape):
    ...
    try:
        ...
        for tick in range(COMMAND_TICKS+1):
            ...
            aux.append(capture_frame(session,directory,tick))
...
def audit(collection):
    ...
    raw,contacts,topology,roles,cameras,relatives,geometry,sensors=audit_sensors(directory,spec,collection)
    primary_checks=audit_rasters_and_footprints(directory,spec,cameras,sensors)
    auxiliary_checks=audit_auxiliary(directory,spec,raw,cameras,topology,collection)
    ...
    for i,row in enumerate(auxiliary):
        ...
        coverage_rows.append(dict(frame=i,candidates=[dict(source_tick=c['source_tick'],action=c['action'],shape_id=c['shape_id'],**w) for c,w in zip(target['candidates'],covered,strict=True)]))
    ...
    return dict(exact_physical_and_public_prefix_sha256=matches,auxiliary_frames_reconstructed=len(auxiliary),
        primary_footprint_checks=primary_checks,auxiliary_sensor_audit=auxiliary_checks,
        all_measurement_gates_pass=all(f['stable_interior_metric_pass'] and not f['near_occlusion_failure']
            and f['original_strict_score']['passes_sampled_physical_visibility'] for f in primary_checks)
            and all(f['auxiliary_visibility_pass'] for f in auxiliary_checks),
...
def main():
    ...
    assert prior['status']=='AUXILIARY_DEPTH_REOBSERVE_GOAL_PROBE_COMPLETE' and prior['all_measurement_gates_pass']
    ...
    assert geometry['status']=='AUXILIARY_DOWNWARD45_DEPTH_GEOMETRY_COMPLETE'
    geometry_ids={'result.json':GEOMETRY_SHA,**geometry['artifact_sha256']};verify_artifacts(GEOMETRY,geometry_ids)
    projection=read_json(GEOMETRY,'projection.json')
    assert projection['first_translating_command_tick']==COMMAND_TICKS and len(projection['candidates'])==12
    assert all(c['complete_frustum_frames_before_first_translation'] for c in projection['candidates'])
    verify_artifacts(PREVIOUS_CAPTURE,PREVIOUS_CAPTURE_IDS);previous_capture_launch=read_json(PREVIOUS_CAPTURE,'launch.json');verify(previous_capture_launch)
    seeds=('scripts/capture_go2_auxiliary_downward45_depth_prefix_v1.py','lewm/tests/test_visible_robot_sensor_scope_development.py',
        'lewm/tests/test_native_link_segmentation_map_development.py','lewm/tests/test_auxiliary_depth_integrity_scope_development.py',
        'docs/go2_auxiliary_downward45_depth_geometry_result_2026-09-08.md',
        'lewm/tests/test_auxiliary_downward45_capture_scope_development.py')
    ...
    for name,sha in previous_capture_launch['source_sha256'].items():
        if name in launch['source_sha256']:assert launch['source_sha256'][name]==sha
    sources=discover_sources((PROTOCOL,*seeds),launch['source_sha256']|geometry['source_sha256']|previous_capture_launch['source_sha256'])
    ...
    launch.update(source_sha256=sources,input_artifact_sha256=input_ids,geometry_artifact_sha256=geometry_ids,
        ...
        robot_visualization_enabled=True,model_inference=False,fixed_command_replay=True,previous_capture_sha256=PREVIOUS_CAPTURE_IDS,
    ...
    print('AUXILIARY_DOWNWARD45_DEPTH_PREFIX_LAUNCHED',digest(OUTPUT/'launch.json'),flush=True);start=time.perf_counter()
    try:
        ...
        verify(launch);verify_artifacts(INPUT,input_ids);verify_artifacts(GEOMETRY,geometry_ids);verify_artifacts(PREVIOUS_CAPTURE,PREVIOUS_CAPTURE_IDS)
        write_json(OUTPUT/'result.json',dict(status='AUXILIARY_DOWNWARD45_DEPTH_PREFIX_COMPLETE',source_sha256=sources,
            artifact_sha256=ids,collection=result,raw_audit_pass=report['all_measurement_gates_pass'],wall_s=time.perf_counter()-start,
        ...
        print('AUXILIARY_DOWNWARD45_DEPTH_PREFIX_COMPLETE',digest(OUTPUT/'result.json'),flush=True)
