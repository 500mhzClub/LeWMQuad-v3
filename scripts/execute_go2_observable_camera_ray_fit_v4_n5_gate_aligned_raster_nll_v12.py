#!/usr/bin/env python3
"""Authority-free V12 executor for the frozen N5 experiment.
...
if __name__ == "__main__":
    ...
    STAGING_PREFIX = ".n5.reservation-v12-"
    LOCK_NAME = ".n5.reservation-v12.lock"
    STAGING_SCHEMA = "lewm_go2_n5_gate_aligned_raster_nll_v12_preclaim_staging_v1"
    RECOVERY_POLICY = {
    ...
    def _directory_flags() -> int:
        ...
        if not nofollow or not directory:
            raise PermissionError("V12 canonical directory walks require no-follow opens")
        return os.O_RDONLY | nofollow | directory | getattr(os, "O_CLOEXEC", 0)


    @dataclass
    class DirectoryEntryV12:
        parent_fd: int
        name: str
        child_fd: int
    ...
    class DirectorySnapshotV12:
        fingerprint: tuple[int, ...]
        inventory: tuple[tuple[str, tuple[int, ...]], ...]


    @dataclass(frozen=True)
    class JournalEventV12:
        watch: int
        mask: int
        cookie: int
        name: str
    ...
    @dataclass
    class DirectoryWatchV12:
        watch: int
        generation: int
        directory_fd: int
        identity: tuple[int, int]
        label: str
        snapshot: DirectorySnapshotV12


    class OwnedDirectoryJournalV12:
        ...
        def __init__(self) -> None:
            ...
            if renameat2 is None:
                ...
                raise PermissionError(
                    "V12 requires Linux renameat2 for no-replace publication"
                )
            renameat2.argtypes = [
                ctypes.c_int,
                ctypes.c_char_p,
            ...
            self._fd = descriptor
            self._by_watch: dict[int, DirectoryWatchV12] = {}
            self._by_identity: dict[tuple[int, int], DirectoryWatchV12] = {}
            self._generation = 0
        ...
        def _fail(self, reason: str) -> None:
            if self._poison is None:
                self._poison = reason
            raise PermissionError(f"V12 owned-directory journal rejected: {reason}")

        @staticmethod
        def _identity(directory_fd: int) -> tuple[int, int]:
            metadata = os.fstat(directory_fd)
            if not stat.S_ISDIR(metadata.st_mode):
                raise PermissionError("V12 journal target is not a directory")
        ...
        @staticmethod
        def _snapshot(directory_fd: int) -> DirectorySnapshotV12:
            opened = os.fstat(directory_fd)
            if not stat.S_ISDIR(opened.st_mode):
                raise PermissionError("V12 journal directory descriptor changed")
            rows: list[tuple[str, tuple[int, ...]]] = []
            for name in sorted(os.listdir(directory_fd)):
                if Path(name).name != name or name in {"", ".", ".."}:
                    raise PermissionError("V12 journal observed a malformed child name")
                metadata = os.stat(
                    name,
                    dir_fd=directory_fd,
            ...
            return DirectorySnapshotV12(
                fingerprint=_stable_fingerprint(opened),
                inventory=tuple(rows),
            )

        def _events(self) -> list[JournalEventV12]:
            events: list[JournalEventV12] = []
            while True:
                try:
        ...
        def _reject_special(self, events: Sequence[JournalEventV12]) -> None:
            for event in events:
        ...
        def _shared_parent_events(
            self,
            events: Sequence[JournalEventV12],
            *,
            watch: int,
            reserved_name: str,
            allow_ignored: bool = False,
        ) -> tuple[list[JournalEventV12], list[JournalEventV12]]:
            ...
            reserved: list[JournalEventV12] = []
            ignored: list[JournalEventV12] = []
            for event in events:
                if event.mask & self.IN_Q_OVERFLOW:
                ...
                if event == JournalEventV12(watch, self.IN_IGNORED, 0, ""):
        ...
        def create_exclusive_root(
            ...
            *,
            ...
            unrelated child names are tolerated, while the V12 name must have
            ...
            try:
                ...
                if ignored != [
                    JournalEventV12(watch, self.IN_IGNORED, 0, "")
                ]:
        ...
        def watch_directory(
            ...
            *,
            label: str,
            expected_snapshot: DirectorySnapshotV12 | None = None,
        ) -> None:
            self.assert_clean()
        ...
        def _state(self, directory_fd: int) -> DirectoryWatchV12:
            identity = self._identity(directory_fd)
        ...
        def baseline(self, directory_fd: int) -> DirectorySnapshotV12:
            return self._state(directory_fd).snapshot

        def _snapshots(self) -> dict[int, DirectorySnapshotV12]:
            return {
        ...
        def _validate_snapshots(
            self,
            before: Mapping[int, DirectorySnapshotV12],
            after: Mapping[int, DirectorySnapshotV12],
            *,
        ...
        def _commit_snapshots(
            self,
            snapshots: Mapping[int, DirectorySnapshotV12],
        ) -> None:
        ...
        def _begin(
            self,
            directory_fd: int,
        ) -> tuple[DirectoryWatchV12, dict[int, DirectorySnapshotV12]]:
            self.assert_clean()
        ...
        def _finish(
            ...
            before: Mapping[int, DirectorySnapshotV12],
            *,
            expected_events: Sequence[JournalEventV12],
            expected_inventory: tuple[tuple[str, tuple[int, ...]], ...],
        ) -> DirectorySnapshotV12:
        ...
        def mkdir(self, parent_fd: int, name: str, *, mode: int = 0o700) -> None:
            ...
            self._finish(
                ...
                before,
                expected_events=(
                    JournalEventV12(state.watch, self.IN_CREATE | self.IN_ISDIR, 0, name),
                ),
        ...
        def create_file(
            ...
            *,
            ...
            expected_events = [
                JournalEventV12(state.watch, self.IN_CREATE, 0, name),
            ]
            if payload:
                expected_events.append(
                    JournalEventV12(state.watch, self.IN_MODIFY, 0, name)
                )
            expected_events.append(
                JournalEventV12(state.watch, self.IN_CLOSE_WRITE, 0, name)
            )
            expected = self._replace_inventory(
        ...
        def unlink(
            ...
            *,
            ...
            self._finish(
                ...
                before,
                expected_events=(
                    JournalEventV12(state.watch, self.IN_DELETE, 0, name),
                ),
                expected_inventory=expected,
            )
        ...
        def rmdir(
            ...
            *,
            ...
            if retired != [
                JournalEventV12(child.watch, self.IN_DELETE_SELF, 0, ""),
                JournalEventV12(child.watch, self.IN_IGNORED, 0, ""),
            ]:
    ...
    @dataclass
    class CanonicalDirectoryChainV12:
        anchor_fd: int
        anchor_identity_security: tuple[int, ...]
        descriptors: list[int]
        entries: list[DirectoryEntryV12]
        path_fds: dict[Path, int]
        journal: OwnedDirectoryJournalV12
        output_root_created: bool = False
        closed: bool = False


    @dataclass(frozen=True)
    class OwnedArtifactV12:
        role: str
        parent_fd: int
        name: str
        fingerprint: tuple[int, ...]
    ...
    def _assert_directory_chain(chain: CanonicalDirectoryChainV12) -> None:
        if chain.closed:
            raise PermissionError("V12 canonical directory chain is closed")
        chain.journal.assert_clean()
        if (
            _identity_security_fingerprint(os.fstat(chain.anchor_fd))
            != chain.anchor_identity_security
        ):
            raise PermissionError("V12 filesystem-root descriptor changed")
        for entry in chain.entries:
            ...
            if (
                ...
                raise PermissionError("V12 canonical directory component changed")


    def _open_canonical_directory_chain(final_path: Path) -> CanonicalDirectoryChainV12:
        ...
        if (
            ...
            raise PermissionError("V12 canonical directory target escaped the repository")
        ...
        journal = OwnedDirectoryJournalV12()
        chain = CanonicalDirectoryChainV12(
            anchor_fd=anchor_fd,
        ...
        try:
            if (
                ...
                raise PermissionError("V12 filesystem root changed during open")
            parent_fd = anchor_fd
            ...
            for index, component in enumerate(final_path.parts[1:]):
                ...
                try:
                    ...
                    if index < repository_depth:
                        raise PermissionError("V12 repository root component is missing")
                    if current_path / component == policy.CANONICAL_OUTPUT_ROOT:
                        ...
                        if not chain.output_root_created:
                            raise PermissionError(
                                "existing V12 recovery tree is missing a structural directory"
                            )
                    ...
                    if (
                        ...
                        raise PermissionError(
                            "V12 exclusive root changed before retained open"
                        )
                if stat.S_ISLNK(before.st_mode) or not stat.S_ISDIR(before.st_mode):
                    raise PermissionError("V12 canonical component is not a real directory")
                ...
                if (
                    ...
                    raise PermissionError("V12 canonical component changed during open")
                entry = DirectoryEntryV12(
                    parent_fd,
                    component,
                    child_fd,
                    identity_security,
    ...
    def _open_chain_child(
        ...
        try:
            ...
            if not chain.output_root_created:
                raise PermissionError(
                    "existing V12 recovery tree is missing a derived directory"
                )
            chain.journal.mkdir(parent_fd, name, mode=0o700)
            before = _entry_metadata(parent_fd, name)
        if stat.S_ISLNK(before.st_mode) or not stat.S_ISDIR(before.st_mode):
            raise PermissionError("V12 derived parent is not a real directory")
        identity_security = _identity_security_fingerprint(before)
        full_fingerprint = _stable_fingerprint(before)
        ...
        if (
            ...
            raise PermissionError("V12 derived parent changed during open")
    ...
    def _close_directory_chain(chain: CanonicalDirectoryChainV12) -> None:
        if chain.closed:
            return
        chain.closed = True
        chain.journal.close()
    ...
    class AttemptReservationV12:
        ...
        directory_chain: CanonicalDirectoryChainV12 | None = None
        owned_claim_artifacts: dict[str, OwnedArtifactV12] = field(
            default_factory=dict,
            compare=False,
        )
        owned_derived_artifacts: dict[str, OwnedArtifactV12] = field(
            default_factory=dict,
            compare=False,
        )
        ...
        @property
        def journal(self) -> OwnedDirectoryJournalV12:
            if self.directory_chain is None:
                raise PermissionError("N5 full-panel V12 lacks its transaction journal")
            return self.directory_chain.journal

    def _assert_claim_fd_owned(reservation: AttemptReservationV12) -> None:
        if reservation.directory_fd < 0 or reservation.directory_identity is None:
            raise PermissionError("N5 full-panel V12 lacks an open claimed directory")
        opened = os.fstat(reservation.directory_fd)
        if not stat.S_ISDIR(opened.st_mode):
            raise PermissionError("N5 full-panel V12 claimed descriptor is not a directory")
        if (opened.st_dev, opened.st_ino) != reservation.directory_identity:
            raise PermissionError("N5 full-panel V12 claimed descriptor identity changed")


    def _assert_owned_claim(reservation: AttemptReservationV12) -> None:
        ...
        if (
            ...
            raise PermissionError("N5 full-panel V12 claimed directory changed")
        if reservation.directory_chain is None or reservation.seed_root_fd < 0:
            raise PermissionError("N5 full-panel V12 lacks its canonical directory chain")
        ...
        if (
            ...
            raise PermissionError("N5 full-panel V12 canonical claim identity changed")
        ...
        if set(os.listdir(reservation.directory_fd)) != expected_names:
            raise PermissionError("N5 full-panel V12 claimed directory inventory changed")


    def _write_claim_file_exclusive(
        reservation: AttemptReservationV12,
        name: str,
        payload: bytes,
        *,
        mode: int = 0o600,
        ...
        if Path(name).name != name or name in {"", ".", ".."}:
            raise PermissionError("N5 full-panel V12 claim filename escaped")
        ...
        reservation.owned_claim_artifacts[name] = OwnedArtifactV12(
            role=role or name,
        ...
        if _stable_fingerprint(committed) != committed_fingerprint:
            raise PermissionError("N5 full-panel V12 committed claim leaf changed")
        if require_canonical:
            _assert_owned_claim(reservation)
        else:
            _assert_claim_fd_owned(reservation)
        ...
        *,
        ...
        try:
            ...
            if (
                ...
                raise PermissionError("N5 full-panel V12 claim leaf is not singly linked")
            chunks: list[bytes] = []
            ...
            if fingerprint(before) != fingerprint(after):
                raise RuntimeError("N5 full-panel V12 claim leaf changed while read")
        finally:
            os.close(descriptor)
        if require_canonical:
            _assert_owned_claim(reservation)
    ...
    def _classify_staging(
        ...
        *,
        attempt_path: Path,
        review_binding: Mapping[str, str],
    ) -> tuple[dict[str, Any], AttemptReservationV12 | None]:
        ...
        journal.assert_clean()
        return evidence, AttemptReservationV12(
            directory=attempt_path,
    ...
    def _complete_recoveries_are_equivalent(
        first: AttemptReservationV12,
        second: AttemptReservationV12,
    ) -> bool:
    ...
    def _validate_recovery_tree_baseline(
        ...
        """Reject historical bytes outside the sole recoverable V12 shape."""

        chain.journal.assert_clean()
        relative_seed = seed_root.relative_to(policy.CANONICAL_OUTPUT_ROOT)
        if len(relative_seed.parts) != 2:
            chain.journal._fail("V12 recovery seed-root depth changed")
        ...
        if not required_paths <= set(chain.path_fds):
            chain.journal._fail("V12 recovery tree lacks retained descriptors")
    ...
    def _reserve_exact_attempt(source_review_file_sha256: str) -> AttemptReservationV12:
        ...
        active_staging: OwnedStagingV12 | None = None
        owned_claim_identity: tuple[int, int] | None = None
        claimed_directory_fd: int | None = None
        claimed_chain: CanonicalDirectoryChainV12 | None = None
        claimed_reservation: AttemptReservationV12 | None = None
        ...
        try:
            ...
            with _locked_seed_root(
                ...
                complete: list[
                    tuple[OwnedStagingV12, AttemptReservationV12, dict[str, Any]]
                ] = []
                ...
                if (
                    ...
                    raise PermissionError("N5 full-panel V12 staging identity changed")
                owned_claim_identity = (staging_metadata.st_dev, staging_metadata.st_ino)
                claimed_reservation = AttemptReservationV12(
                    directory=attempt_path,
    ...
    def _cleanup_owned_artifacts(
        registry: dict[str, OwnedArtifactV12],
        *,
        journal: OwnedDirectoryJournalV12,
    ...
    def _terminate_failure(
        ...
        *,
        ...
        if isinstance(error, VerifierChildFailureV12):
            try:
    ...
    def _publish_success(
        ...
        *,
        ...
        if not policy.is_sha256(checkpoint_content_sha256):
            raise ValueError("N5 full-panel V12 checkpoint content hash is malformed")
    ...
    def _write_canonical_json(
        ...
        if path not in {policy.CANONICAL_METRIC_RECEIPT_PATH, policy.CANONICAL_GATE_PATH}:
            raise PermissionError("N5 full-panel V12 publication path is not canonical")
        expected_schema = (
        ...
        if (
            ...
            raise ValueError("N5 full-panel V12 derived artifact is malformed")
        raw = policy.canonical_json_bytes(value) + b"\n"
        relative = path.relative_to(policy.CANONICAL_OUTPUT_ROOT)
        if len(relative.parts) != 2:
            raise PermissionError("N5 full-panel V12 publication depth changed")
        if reservation.directory_chain is None or reservation.output_root_fd < 0:
            raise PermissionError("N5 full-panel V12 lacks its retained output root")
        ...
        if parent_fd < 0:
            raise PermissionError("N5 full-panel V12 lacks its derived parent descriptor")
        ...
        reservation.owned_derived_artifacts[relative_text] = OwnedArtifactV12(
        ...
        if _stable_fingerprint(committed) != committed_fingerprint:
            raise PermissionError("N5 full-panel V12 committed derived leaf changed")
        _assert_owned_claim(reservation)
        return policy.artifact_binding(
            relative_text,
            raw,
    ...
    def _read_owned_derived_artifact(
        ...
        if artifact is None:
            raise PermissionError("N5 full-panel V12 derived artifact is not owned")
        raw = _read_regular_at(
            artifact.parent_fd,
            artifact.name,
            expected_fingerprint=artifact.fingerprint,
        ...
        if hashlib.sha256(raw).hexdigest() != artifact.payload_sha256:
            raise PermissionError("N5 full-panel V12 derived payload changed")
        _assert_owned_claim(reservation)
        return raw
    ...
    def _run_frozen_training(
        ...
        *,
        rgb_workers: int,
    ) -> tuple[dict[str, Any], AttemptReservationV12]:
        """Run the reviewed V12 science behind an ephemeral local adapter."""
        ...
        def require(value: object) -> object:
            if value is not token:
                raise PermissionError("retained science lacks its local V12 execution token")
        ...
        def reserve(value: object) -> AttemptReservationV12:
            nonlocal claimed_reservation
            require(value)
            if claimed_reservation is not None:
                raise FileExistsError("the sole V12 attempt was already reserved")
        ...
        try:
            ...
            if claimed_reservation is None:
                raise RuntimeError(
                    "frozen V12 training returned without claiming its attempt"
                )
            _assert_owned_claim(claimed_reservation)
            launch_summary = {
                **summary,
                "schema": "lewm_go2_observable_camera_ray_fit_v4_n5_gate_aligned_raster_nll_v12_launch_summary_v1",
            }
    ...
    @dataclass(frozen=True)
    class ChildProcessObservationV12:
        returncode: int
        signal: int | None
        timed_out: bool
        stdout_raw: bytes
    ...
    class VerifierChildFailureV12(RuntimeError):
        def __init__(
            self,
            reason: str,
            observation: ChildProcessObservationV12,
    ...
    class SmokeInjectedFailureV12(RuntimeError):
        pass
    ...
    def _parse_failure_envelope(
        ...
        try:
            value = policy.parse_json(raw, name="V12 verifier failure envelope")
        ...
        try:
            checked = _self_hashed_value(value, name="V12 verifier failure envelope")
        except (TypeError, ValueError):
            return None, None, "malformed"
        if (
    ...
    def _observe_child_process(
        ...
        *,
        ...
        return ChildProcessObservationV12(
    ...
    def _verification_child_environment() -> dict[str, str]:
        ...
        if _child_environment_projection(environment) != _expected_child_environment():
            raise PermissionError("V12 verifier child environment sanitization failed")
        return environment
    ...
    def _verification_request(
        ...
        if not sys.flags.isolated:
            raise PermissionError("V12 verifier parent must already be isolated")
        _assert_owned_claim(reservation)
        review, _ = policy.preflight_source_review(
            policy.CANONICAL_SOURCE_REVIEW_PATH,
            source_review_file_sha256,
        ...
        if len(raw) > policy.VERIFICATION_MAX_REQUEST_BYTES:
            raise ValueError("V12 verifier request exceeds its frozen size bound")
        _assert_owned_claim(reservation)
        return request, raw
    ...
    def _validate_verification_request(
        ...
        value = _self_hashed_value(request, name="V12 verifier request")
        expected_fields = {
            "schema",
            "nonce",
            "source_review",
        ...
        if (
            ...
            raise PermissionError("V12 verifier request contract changed")
        ...
        if (
            ...
            raise PermissionError("V12 verifier request process binding changed")
        source_review = value.get("source_review")
        if not isinstance(source_review, Mapping):
            raise PermissionError("V12 verifier request source review is malformed")
        review, _ = policy.preflight_source_review(
            policy.CANONICAL_SOURCE_REVIEW_PATH,
        ...
        if (
            ...
            raise PermissionError("V12 verifier request source binding changed")
        ...
        if (
            ...
            raise PermissionError("V12 verifier request artifact map changed")
        for role, expected_path in expected_paths.items():
            bound = artifacts.get(role)
            if not isinstance(bound, str):
                raise PermissionError("V12 verifier artifact binding is malformed")
            path, digest = policy.parse_bound_path(bound)
            if path != expected_path.resolve(strict=True) or not policy.is_sha256(digest):
                raise PermissionError("V12 verifier artifact binding changed")
            ...
            if (
                ...
                raise PermissionError("V12 verifier artifact content binding changed")
        policy.preflight_static_authority()
        return value
    ...
    def _compute_verification_receipt_child(
        ...
        if (
            ...
            raise PermissionError("V12 verifier child process boundary changed")
        ...
        def require(candidate: object) -> object:
            if candidate is not token:
                raise PermissionError(
                    "retained verifier lacks its local V12 child token"
                )
            policy.preflight_static_authority()
            policy.preflight_source_review(
                policy.CANONICAL_SOURCE_REVIEW_PATH,
        ...
        def forbid_publication(*_args: Any, **_kwargs: Any) -> None:
            raise PermissionError("V12 verifier child cannot publish")
    ...
    def _load_claim_json(
        ...
        raw = _read_claim_file(reservation, name)
        value = policy.parse_json(raw, name=f"V12 parent {name}")
        if raw != policy.canonical_json_bytes(value) + b"\n":
            raise ValueError(f"V12 parent {name} is not canonical JSON")
        return _self_hashed_value(value, name=f"V12 parent {name}"), raw
    ...
    def _validate_child_metric_receipt_provenance(
        ...
        value = _self_hashed_value(receipt, name="V12 child metric receipt")
        expected_fields = {
            "schema",
            "authoritative",
        ...
        for role, binding in expected_artifacts.items():
            ...
            if digest != binding["file_sha256"]:
                raise PermissionError("V12 child request artifact changed")
        ...
        if (
            ...
            raise PermissionError("V12 child metric receipt provenance changed")
        _assert_owned_claim(reservation)
    ...
    def _validate_child_metric_receipt(
        ...
        if (
            ...
            raise ValueError("V12 child frozen metric decision changed")
        _assert_owned_claim(reservation)
        return value
    ...
    def _validate_verification_response(
        ...
        value = _self_hashed_value(response, name="V12 verifier response")
        expected_fields = {
            "schema",
            "status",
            "nonce",
        ...
        if (
            ...
            raise PermissionError("V12 verifier response binding changed")
        receipt = value.get("receipt")
        if not isinstance(receipt, Mapping):
            raise ValueError("V12 verifier response lacks a receipt")
        ...
        if value.get("receipt_content_sha256") != checked["content_sha256"]:
            raise PermissionError("V12 verifier response receipt binding changed")
        return checked
    ...
    def _run_independent_verification(
        ...
        try:
            ...
            response = policy.parse_json(
                response_raw,
                name="V12 verifier child response",
            )
            if response_raw != policy.canonical_json_bytes(response) + b"\n":
                raise ValueError(
                    "V12 verifier child response is not one canonical JSON value"
                )
            ...
            raise VerifierChildFailureV12(
                "V12 verifier boundary failed; no fallback",
                observation,
                request,
            ) from error
        _assert_owned_claim(reservation)
    ...
    def _verification_child_main() -> int:
        ...
        try:
            ...
            if not sys.flags.isolated or not sys.flags.dont_write_bytecode:
                raise PermissionError("V12 verifier child requires -I -B")
            ...
            if not raw or len(raw) > policy.VERIFICATION_MAX_REQUEST_BYTES:
                raise ValueError("V12 verifier child request size changed")
            phase_state.enter("request_parse")
            request = policy.parse_json(raw, name="V12 verifier child request")
            if raw != policy.canonical_json_bytes(request) + b"\n":
                raise ValueError("V12 verifier child request is not canonical JSON")
            ...
            if len(response_raw) > policy.VERIFICATION_MAX_RESPONSE_BYTES:
                raise ValueError("V12 verifier child response exceeds its size bound")
    ...
    def _smoke_child_environment(
        *,
        ...
        for name in (
            ...
            "PYTHONUSERBASE",
            "HSA_OVERRIDE_GFX_VERSION",
            "LEWM_V12_SMOKE_INJECTION_PHASE",
            "LEWM_V12_SMOKE_PROCESS_CASE",
            "LEWM_V12_VERIFIER_MODE",
        ):
            environment.pop(name, None)
        environment["PYTHONNOUSERSITE"] = "1"
        for name in (
        ...
        for name in policy.THREAD_ENVIRONMENT:
            environment[name] = "1"
        if smoke_mode:
            environment["LEWM_V12_VERIFIER_MODE"] = "cpu_contract_smoke_v1"
        if injection_phase is not None:
            ...
            environment["LEWM_V12_SMOKE_INJECTION_PHASE"] = injection_phase
        if process_case is not None:
            ...
            environment["LEWM_V12_SMOKE_PROCESS_CASE"] = process_case
    ...
    def _smoke_request(temporary_directory: Path) -> tuple[dict[str, Any], bytes]:
        ...
        gate_evaluation = complete_retained_gate_evaluation_v12()
        core = {
            ...
            "bundle": {
                "schema": "lewm_v12_cpu_verifier_smoke_bundle_v1",
                "production_eligible": False,
                "frame_count": 5,
                "frames": frames,
    ...
    def _smoke_enter(phase_state: Any, phase: str) -> None:
        ...
        if os.environ.get("LEWM_V12_SMOKE_INJECTION_PHASE") == phase:
            raise SmokeInjectedFailureV12("injected verifier smoke phase failure")
    ...
    def _cpu_verifier_smoke_child_main() -> int:
        ...
        process_case = os.environ.get("LEWM_V12_SMOKE_PROCESS_CASE")
        ...
        try:
            ...
            if (
                ...
                or bundle.get("schema") != "lewm_v12_cpu_verifier_smoke_bundle_v1"
            ...
            independent_cell_nll = (
                verifier.derived_raster_cell_nll_for_verification_v12(
                    smoke_probabilities,
                    smoke_labels,
                )
            )
            ...
            compatibility_view = (
                policy.adapt_native_v12_evaluation_for_retained_v4_gate(
                    gate_evaluation
                )
            )
    ...
    def run_cpu_verifier_contract_smoke() -> dict[str, Any]:
        ...
        temporary_path: Path
        with tempfile.TemporaryDirectory(prefix="lewm-v12-verifier-smoke-") as name:
            ...
            cases: dict[str, ChildProcessObservationV12] = {}
            for process_case in (
                "timeout",
                "signal",
                "nonzero",
        ...
        return {
            "schema": "lewm_v12_cpu_verifier_contract_smoke_summary_v1",
            "production_eligible": False,
    ...
    def _run_finalization(
        ...
        metric = policy.parse_json(metric_raw, name="V12 finalizer metric receipt")
        if metric_raw != policy.canonical_json_bytes(metric) + b"\n":
            raise ValueError("V12 finalizer metric receipt is not canonical JSON")
        ...
        if (
            ...
            raise ValueError("V12 finalizer frozen metric decision changed")
    ...
    def execute_exact(
        ...
        *,
        ...
        if not sys.flags.isolated:
            raise PermissionError("N5 full-panel V12 exact execution requires isolation")
        ...
        if (
            ...
            raise FileExistsError(
                "the sole N5 full-panel V12 recovery attempt is already claimed"
            )
        policy.preflight_static_authority()
        policy.preflight_source_review(
            policy.CANONICAL_SOURCE_REVIEW_PATH,
        ...
        try:
            ...
            _assert_owned_claim(reservation)
            return {
                "schema": "lewm_go2_observable_camera_ray_fit_v4_n5_gate_aligned_raster_nll_v12_end_to_end_summary_v1",
                "seed": 20260710,
                "fit_size": 5,
                "training": training,
            ...
            try:
                ...
                raise RuntimeError(
                    f"N5 full-panel V12 {stage} failed and terminal receipt "
                    "could not be written"
                ) from terminal_error
            raise
        finally:
    ...
    def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
        ...
        if not policy.is_sha256(args.source_review_sha256):
            raise ValueError("N5 full-panel V12 source review SHA-256 is malformed")
        return args
    ...
    def dispatch(argv: Sequence[str] | None = None) -> int:
        ...
        if raw_argv == ["--verification-child"]:
            if not sys.flags.isolated:
                raise PermissionError(
                    "V12 verifier child cannot self-authorize isolation"
                )
            if (
                os.environ.get("LEWM_V12_VERIFIER_MODE")
