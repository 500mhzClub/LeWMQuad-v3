"""Prospective full-length reactive comparison, with independent raw auditing."""
import argparse
import contextlib
import json
...
from scripts.reactive_nominal_maze_episode_development import collect, artifacts
from scripts.reactive_nominal_maze_audit_development import audit
from scripts.reactive_nominal_native_prefix_comparison_development import compare
from scripts.replay_go2_reactive_nominal_route_prefix_v1 import OUTPUT as PREFIX, INPUT as PREVIOUS, CASE as PRIOR_CASE
from scripts.run_go2_executed_waypoint_maze_pilot_v1 import OUTPUT as WAYPOINT
from scripts.read_go2_executed_waypoint_maze_pilot_v1 import OUTPUT as WAYPOINT_READOUT
...
OUTPUT = BASE/'go2_reactive_nominal_maze_pilot_v1_attempt_001'
PROTOCOL = 'docs/go2_reactive_nominal_maze_pilot_v1_2026-09-08.md'
CASE = 'reactive_nominal_maze_00'
WAYPOINT_SHA = 'd5ba1136c969cd6591028f435a93f0e35a4dae7b9cbf217524887b459271ace8'
WAYPOINT_READOUT_SHA = '1bb9ae412452fd68a99fa827ad304d9201f5dfdc2b7bb24f57e003390ba7799c'
WAYPOINT_PREFIX = dict(frames=4, first_requested_command_difference=3,
    prior_requested_command=[.16, 0., .45], final_requested_command=[.2, 0., 0.])


def worker(launch_sha):
    cv2.setNumThreads(1); torch.set_num_threads(1); torch.use_deterministic_algorithms(True)
    terminal = dict(case=CASE, layout_index=0, controller='reactive_nominal_round_trip_controller_v1',
        status='REACTIVE_NOMINAL_MAZE_WORKER_FAILED', artifact_sha256={}); started = time.perf_counter()
    with (OUTPUT/(CASE+'_worker.log')).open('x') as log, contextlib.redirect_stdout(log), contextlib.redirect_stderr(log):
        try:
            ...
            comparisons = dict(
                original_recovery=compare(PREVIOUS/PRIOR_CASE[0], OUTPUT/CASE, launch['prefix_report']),
                executed_waypoint=compare(WAYPOINT/PRIOR_CASE[0], OUTPUT/CASE, launch['waypoint_comparison_prefix']))
            ...
            terminal.update(status='REACTIVE_NOMINAL_MAZE_COLLECTED_AND_RAW_AUDITED', artifact_sha256=bindings,
...
def main():
    ...
    bound = []
    for root, sha, status in ((PREFIX, args.prefix_result_sha256, 'REACTIVE_NOMINAL_ROUTE_PREFIX_COMPLETE'),
            (WAYPOINT, WAYPOINT_SHA, 'EXECUTED_WAYPOINT_MAZE_PILOT_COMPLETE'),
            (WAYPOINT_READOUT, WAYPOINT_READOUT_SHA, 'EXECUTED_WAYPOINT_MAZE_READOUT_COMPLETE')):
    ...
    assert prefix['first_requested_command_difference'] == 7 and prefix['frames'] == 8
    ...
    baseline_three = next(islice(read_rows(PREFIX), 3, 4))
    assert baseline_three['tick'] == 3 and baseline_three['decision']['requested_command'] == WAYPOINT_PREFIX['final_requested_command']
    waypoint_tape = read_json(WAYPOINT/PRIOR_CASE[0], 'command_tape.json')
    assert waypoint_tape[3]['requested_command'] == WAYPOINT_PREFIX['prior_requested_command']
    ...
    for _, _, result in bound:
        for name, sha in result['source_sha256'].items():
            ...
            merged[name] = sha
    sources = discover_sources((PROTOCOL, 'scripts/run_go2_reactive_nominal_maze_pilot_v1.py',
        'lewm/tests/test_reactive_nominal_native_scope_development.py',
        'lewm/tests/test_reactive_nominal_native_prefix_comparison_development.py',
        'docs/go2_reactive_nominal_route_prefix_result_2026-09-08.md',
        'docs/go2_executed_waypoint_maze_pilot_result_2026-09-08.md'), merged)
    resources = hardware()
    if args.preflight_only:
        print('REACTIVE_NOMINAL_PREFLIGHT', json.dumps(dict(
    ...
    launch.update(protocol=PROTOCOL, source_sha256=sources, output_root=str(OUTPUT), planned_case=CASE,
        ...
        waypoint_comparison_prefix=WAYPOINT_PREFIX, previous_replay_input_bindings=previous_bindings,
        ...
        data_scope='reactive nominal-route baseline on reused development maze 0',
    ...
    print('REACTIVE_NOMINAL_MAZE_LAUNCHED', launch_sha, flush=True)
    try:
        ...
        print('REACTIVE_NOMINAL_MAZE_TERMINAL', record['status'], record.get('verified_round_trip'), record.get('failure'), flush=True)
        if record['status'] != 'REACTIVE_NOMINAL_MAZE_COLLECTED_AND_RAW_AUDITED':
        ...
        for name, ids in previous_bindings.items(): verify_artifacts(Path(name), ids)
        write_json(OUTPUT/'result.json', dict(status='REACTIVE_NOMINAL_MAZE_PILOT_COMPLETE', conditions=[record],
        ...
        print('REACTIVE_NOMINAL_MAZE_COMPLETE', digest(OUTPUT/'result.json'), flush=True)
    except Exception as error:
        write_json(OUTPUT/'failure.json', dict(status='TERMINAL_REACTIVE_NOMINAL_MAZE_PILOT_FAILURE', reason=repr(error)))
        raise
