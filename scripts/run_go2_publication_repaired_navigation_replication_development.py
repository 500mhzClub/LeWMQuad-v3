"""Fifteen prospective missions with nonblocking recovery publication on unused mazes."""
import argparse
...
BASE = study.BASE
ROOT = 'go2_publication_repaired_replication_{arm}_noise_2mm_native_layout{index:02d}_4800_v1_attempt_001'
PLAN = Path('docs/go2_publication_repaired_navigation_replication_plan_2026-09-16.json')
...
ASSIGNMENTS = tuple((i, a) for i in (1, 2, 3) for a in ARMS[i:]+ARMS[:i])
SOURCES = tuple(dict.fromkeys((*baseline.SOURCES, __file__,
    ...
    'lewm/measured_recovery_publication_development.py')))
...
class PublicationRepairedRuntime(MeasuredRecoveryPublicationMixin, RecoveryLimitedSurveyRuntime):
    pass


class InstantaneousRuntime(InstantaneousWaypointScoreMixin, PublicationRepairedRuntime):
    pass


class ReactiveRuntime(ComputedForecastsUnusedMixin, ReactiveFeedbackMixin, PublicationRepairedRuntime):
    pass


RUNTIMES = dict(jepa=PublicationRepairedRuntime,
    supervised_rollout=PublicationRepairedRuntime, pose_command=PublicationRepairedRuntime,
...
def prepare():
    ...
    if json.loads(INVENTORY.read_text()) != inventory:
        raise ValueError('reuse the previously generated inventory unchanged')
    for index, arm in ASSIGNMENTS:
        old = BASE/f'go2_recovery_repaired_replication_{arm}_noise_2mm_native_layout{index:02d}_4800_v1_attempt_001'
        if old.exists() or (BASE/ROOT.format(index=index, arm=arm)).exists():
            raise ValueError('only previously unexecuted layouts 1-3 are eligible')
    plan = dict(schema='publication_repaired_navigation_replication_plan.v1', assignments=ASSIGNMENTS,
        ...
        executed_layout_indices=[1, 2, 3], excluded_exposed_layout_indices=[0],
        predecessor_cohort_interrupted=True, predecessor_outcomes_preserved_separately=True,
        ...
        limitations=['three previously unexecuted mazes from the existing maze family', 'one execution per arm/layout',
...
def main():
    ...
    def write(name, value):
        if name == 'launch.json':
            value = value | dict(experiment='publication_repaired_navigation_replication_v1', study_arm=arm,
                ...
                layout_novelty_scope='unused_layouts_1_2_3_distinct_from_94_prior_layouts_and_exposed_layout_0',
                planned_layout_count=3, planned_native_assignments=15,
