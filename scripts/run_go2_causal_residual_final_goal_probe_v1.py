...
from scripts.causal_residual_final_goal_episode_development import collect,artifacts
from scripts.causal_residual_final_goal_audit_development import audit
...
from scripts.replay_go2_causal_residual_final_goal_prefix_v1 import OUTPUT as INTEGRATION
...
PREVIOUS=BASE/'go2_executed_horizon_final_goal_probe_v1_attempt_001'
DIAGNOSTIC=BASE/'go2_executed_horizon_final_goal_readout_v1_attempt_001'
PREVIOUS_SHA='dabc90b19f1d8285d21fd81d265efae504fbe75b1868357f6bcd17f2e77fe062'
DIAGNOSTIC_SHA='edd050780b55f45956cb685d57b989cb748ed90227bc8937bec088589d755559'
OUTPUT=BASE/'go2_causal_residual_final_goal_probe_v1_attempt_001'
...
PROTOCOL='docs/go2_causal_residual_final_goal_probe_v1_2026-09-08.md'
...
def worker(case,launch_sha):
    ...
    terminal=dict(case=name,trial=trial,variant=variant,condition=condition,model_name=model_name,
        status='CAUSAL_RESIDUAL_FINAL_GOAL_WORKER_FAILED',artifact_sha256={});started=time.perf_counter()
    with (OUTPUT/(name+'_worker.log')).open('x') as log,contextlib.redirect_stdout(log),contextlib.redirect_stderr(log):
        try:
            ...
            terminal.update(status='CAUSAL_RESIDUAL_FINAL_GOAL_COLLECTED_AND_RAW_AUDITED',artifact_sha256=bindings,
...
def main():
    ...
    assert previous['status']=='EXECUTED_HORIZON_FINAL_GOAL_PROBE_COMPLETE' and previous['cases']==[list(c) for c in CASES]
    ...
    assert diagnostic['status']=='EXECUTED_HORIZON_FINAL_GOAL_READOUT_COMPLETE'
    ...
    assert integration['status']=='CAUSAL_RESIDUAL_FINAL_GOAL_PREFIX_COMPLETE' and len(integration['conditions'])==2
    assert all(r['causal_observations_maps_forecasts_and_nominal_checks_exact'] and r['surface_checks_exact'] and r['only_final_goal_scoring_pose_corrected'] and r['exact_prequential_residual_comparison_frames'] and not r['controller_failures'] and r['model_state_unchanged'] and r['exact_fresh_replay_passes']==2 for r in integration['conditions'])
    ...
    launch=preflight(output=OUTPUT,protocol=PROTOCOL,
        seed_paths=('scripts/run_go2_causal_residual_final_goal_probe_v1.py',
            'lewm/tests/test_causal_residual_final_goal_native_scope_development.py',
            'docs/go2_causal_residual_final_goal_prefix_result_2026-09-08.md',
    ...
    launch.update(integration_artifact_sha256=integration_ids,sensor_prefix_artifact_sha256=sensor_ids,
        ...
        experiment='causal preceding-eight-tick public observed XY residual adjusts final-goal 100-ms scoring poses only; original forecasts, yaw/contact, surface/path checks, intermediate/view scores, weights, arrivals and budgets retained',
    ...
    print('CAUSAL_RESIDUAL_FINAL_GOAL_LAUNCHED',digest(OUTPUT/'launch.json'),flush=True)
    ...
    try:
            ...
            with ProcessPoolExecutor(max_workers=2,mp_context=multiprocessing.get_context('spawn'),max_tasks_per_child=1) as pool:
                ...
                while pending:
                    ...
                    for future in done:
                        ...
                        except Exception as error:row=dict(case=case[0],status='CAUSAL_RESIDUAL_FINAL_GOAL_PROCESS_FAILURE',failure=repr(error))
                        records.append(row);print('CAUSAL_RESIDUAL_FINAL_GOAL_TERMINAL',case[0],row['status'],row.get('goal'),flush=True)
        if len(records)!=2 or any(r['status']!='CAUSAL_RESIDUAL_FINAL_GOAL_COLLECTED_AND_RAW_AUDITED' for r in records):
        ...
        result=dict(status='CAUSAL_RESIDUAL_FINAL_GOAL_PROBE_COMPLETE',cases=CASES,conditions=records,
        ...
        write_json(OUTPUT/'result.json',result);print('CAUSAL_RESIDUAL_FINAL_GOAL_COMPLETE',digest(OUTPUT/'result.json'),flush=True)
    except Exception as error:
        write_json(OUTPUT/'failure.json',dict(status='TERMINAL_CAUSAL_RESIDUAL_FINAL_GOAL_FAILURE',reason=repr(error),
