...
import shutil
import time
from lewm.executed_horizon_final_goal_development import ExecutedHorizonFinalGoalProbe
...
from scripts.auxiliary_downward45_goal_session_development import AuxiliaryDownward45GoalSession
from scripts.geometry_progress_family_episode_development import artifacts as primary_artifacts, RESERVE
...
def collect(trial, definition, *, output, model, geometry, persistent, episode_name, condition, variant):
    ...
    try:
        session = AuxiliaryDownward45GoalSession(spec, directory)
        session._runtime['high_level'] = 'executed_horizon_final_goal_probe_v1; online model plus measured surfaces'
        ...
        controller = ExecutedHorizonFinalGoalProbe(model, geometry, persistent=persistent, condition=condition, variant=variant)
        try:
            ...
            for tick in range(WARMUP_TICKS + NAVIGATION_TICKS + DRAIN_TICKS + 1):
                ...
                try:
                    p, d, f, auxiliary, now = session.sensor_packets()
        ...
        result = dict(status='EXECUTED_HORIZON_FINAL_GOAL_PROBE_TERMINAL_AUDIT_REQUIRED', trial=trial,
            ...
            physics_samples=len(session.samples), rgbd_frames=len(session.model_manifest), auxiliary_frames=len(session.auxiliary_audit), decisions=len(rows),
...
def artifacts(trial,result):
    return primary_artifacts(trial,result)+['auxiliary_camera_audit.json']+[
