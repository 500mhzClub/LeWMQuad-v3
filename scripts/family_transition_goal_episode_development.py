...
def collect(trial, definition, *, output, model):
    ...
    try:
        ...
        session._runtime['high_level'] = 'family_transition_corner_goal_probe_v1; online model and measured joint RGBD pose'
        ...
        controller = FamilyTransitionGoalProbe(model)
        try:
        ...
        result = dict(status='FAMILY_TRANSITION_GOAL_PROBE_TERMINAL_AUDIT_REQUIRED', trial=trial,
        ...
        print('FAMILY_TRANSITION_GOAL_PROBE', trial, result, flush=True)
        return result
    finally:
