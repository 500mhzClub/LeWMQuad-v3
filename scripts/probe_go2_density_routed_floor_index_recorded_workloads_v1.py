"""Benchmark density routing on fixed original camera inputs."""
...
SOURCE='scripts/probe_go2_density_routed_floor_index_recorded_workloads_v1.py'
TEST='lewm/tests/test_density_routed_floor_cell_index_development.py'
PRIOR='docs/go2_eligible_floor_index_recorded_workload_probe_2026-09-11.json'
PRIOR_SHA='4c36fa2efc4cc1661a4704d32bf8be522bdf1701f98001085ab08daee81f068e'
OUTPUT=Path('docs/go2_density_routed_floor_index_recorded_workload_probe_2026-09-11.json')
...
def compare(depth,valid,up):
    ...
    sparse=FunctionType(function.__code__,function.__globals__|dict(dense_index=dense),
        function.__name__,function.__defaults__)
    def choose(*args):
        selected=routed.prefer_dense(*args);routes.append('dense' if selected else 'eligible')
        return selected
    function=routed.observed_floor_cell_index
    candidate=FunctionType(function.__code__,function.__globals__|dict(prefer_dense=choose,eligible_index=sparse),
        function.__name__,function.__defaults__)
    ...
    for pair in range(PAIRS):
        ...
        rows.append(dict(pair=pair,order=list(order),wall_ms=times,dense_fallback=len(fallbacks)!=n,selected_kernel=routes[-1]))
    ...
    return dict(input_sha256=before,eligible_cells=eligible_count(depth,valid,up),
        ...
        dense_fallback_calls=len(fallbacks),selected_kernels=routes,pairs=rows,total_wall_ms=totals,
...
def main():
    ...
    write_json(OUTPUT,dict(status='DENSITY_ROUTED_FLOOR_INDEX_RECORDED_WORKLOADS_CHECKED',source_sha256=sources,
        ...
        private_route_and_fallback_counters_only=True,imported_module_globals_mutated=False,
