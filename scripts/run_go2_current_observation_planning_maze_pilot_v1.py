"""Fresh physical planning-map comparison after completed causal integration."""
import argparse
import contextlib
import json
...
from scripts.current_observation_planning_maze_episode_development import collect, artifacts
from scripts.current_observation_planning_maze_audit_development import audit
from scripts.current_observation_planning_native_prefix_development import admit_prefix, compare
from scripts.replay_go2_current_observation_planning_prefix_v1 import OUTPUT as PREFIX, verify_inputs as verify_prefix
...
OUTPUT = BASE/'go2_current_observation_planning_maze_pilot_v1_attempt_001'
CASE = ('full_jepa_current_observation_planning_maze_00', *LEARNED_CASE[1:])
PROTOCOL = 'docs/go2_current_observation_planning_maze_pilot_v1_2026-09-09.md'
PREFIX_SHA = '8d0e1391f95c71cd71394356c902a9367a1d37fad75571197456dd0b0a6ca90a'
...
def verify_inputs(launch):
    ...
    if (launch['planned_case'] != list(CASE) or launch['scene_specification'] != specification(0)
            or launch['public_mission'] != public_mission(0)):
        raise ValueError('fixed planning-map maze0 definition required')
...
def worker(launch_sha):
    ...
    terminal = dict(case=name,layout_index=index,model_name=model_name,
        status='CURRENT_OBSERVATION_PLANNING_WORKER_FAILED',artifact_sha256={}); started = time.perf_counter()
    with (OUTPUT/(name+'_worker.log')).open('x') as log, contextlib.redirect_stdout(log), contextlib.redirect_stderr(log):
        try:
            ...
            terminal.update(status='CURRENT_OBSERVATION_PLANNING_COLLECTED_AND_RAW_AUDITED',artifact_sha256=bindings,
                ...
                planning_map_variant='current_paired_observation',persistent_contact_history_retained=True)
...
def main():
    ...
    if OUTPUT.exists() or OUTPUT.is_symlink(): raise ValueError('exclusive planning-map native attempt required')
    ...
    verify_artifacts(LEARNED,{'result.json':args.learned_result_sha256}); learned = read_json(LEARNED,'result.json')
    learned_ids = dict(learned['artifact_sha256']); learned_ids['result.json'] = args.learned_result_sha256
    ...
    if prefix_report['model_state_sha256'] != old['prefix_report']['model_state_sha256']:
    ...
    sources = discover_sources((PROTOCOL,'scripts/run_go2_current_observation_planning_maze_pilot_v1.py',
        'lewm/tests/test_current_observation_planning_native_development.py',
        'docs/go2_current_observation_planning_prefix_result_2026-09-09.md'),inherited)
    ...
    launch.update(protocol=PROTOCOL,source_sha256=sources,output_root=str(OUTPUT),planned_case=list(CASE),
        ...
        implementation_class='CurrentObservationPlanningController',robot_urdf_sha256=digest(URDF),
    ...
    if args.preflight_only:
        print('CURRENT_OBSERVATION_PLANNING_NATIVE_PREFLIGHT',json.dumps(dict(source_count=len(sources),hardware=resources,
        ...
        return
    if not memory_ok or not storage_ok: raise ValueError('planning-map native resources unavailable')
    create_output(OUTPUT); write_json(OUTPUT/'launch.json',launch); launch_sha = digest(OUTPUT/'launch.json')
    print('CURRENT_OBSERVATION_PLANNING_NATIVE_LAUNCHED',launch_sha,flush=True); started = time.perf_counter()
    try:
        ...
        print('CURRENT_OBSERVATION_PLANNING_NATIVE_TERMINAL',record['status'],record.get('verified_round_trip'),record.get('failure'),flush=True)
        if record['status'] != 'CURRENT_OBSERVATION_PLANNING_COLLECTED_AND_RAW_AUDITED':
        ...
        verify_inputs(launch); verify_artifacts(OUTPUT,bindings)
        write_json(OUTPUT/'result.json',dict(status='CURRENT_OBSERVATION_PLANNING_MAZE_PILOT_V1_COMPLETE',
            ...
            planning_map_variant='current_paired_observation',persistent_contact_history_retained=True,
            memory_advantage_established=False,memoryless_controller=False,
            navigation_qualified=False,hardware_qualified=False,real_time_qualified=False,goal_achieved=False))
        print('CURRENT_OBSERVATION_PLANNING_NATIVE_COMPLETE',digest(OUTPUT/'result.json'),flush=True)
    except Exception as error:
        write_json(OUTPUT/'failure.json',dict(status='TERMINAL_CURRENT_OBSERVATION_PLANNING_NATIVE_FAILURE',reason=repr(error)))
        raise
