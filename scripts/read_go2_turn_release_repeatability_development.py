"""Read the four balanced heading-release repetitions without dropping outcomes."""
import json
from pathlib import Path
import sys

BASE = Path('/home/andrewknowles/RecoveryStorage/LeWMQuad-v3/.generated/navigation_development_artifacts_v1')
plan = json.loads(Path('docs/go2_turn_release_repeatability_plan_2026-09-16.json').read_text())
assignment = int(sys.argv[1])
case = plan['cases'][assignment-1]
root = BASE/f"go2_turn_release_repeatability_{case['name']}_jepa_noise_2mm_native_layout01_4800_v1_attempt_001"
...
eligible = [r for r in plans if 'heading_release_comparison' in r['selection']]
assert all(r['selection']['heading_release_comparison']['suppressed'] == case['suppress'] for r in eligible)
suppressed = [r for r in eligible if r['selection']['heading_release_comparison']['suppressed']]
...
result = dict(schema='heading_release_repeatability_readout.v1', assignment=assignment,
    case=case['name'], reference_root=reference.name, root=root.name,
    condition='disabled' if case['suppress'] else 'enabled',
    repetition=1 if case['name'].endswith('rep1') else 2,
    eligible_release_decisions=len(eligible), release_decisions_applied=len(eligible)-len(suppressed),
...
with (root/'heading_release_repeatability_readout_v1.json').open('x') as f:
    json.dump(result, f, indent=2); f.write('\n')
records=[]
for c in plan['cases']:
    p=BASE/f"go2_turn_release_repeatability_{c['name']}_jepa_noise_2mm_native_layout01_4800_v1_attempt_001"/'heading_release_repeatability_readout_v1.json'
...
(output/'result.json').write_text(json.dumps(dict(planned=4, evaluated=len(records),
    complete=len(records)==4, records=records, exposed_development_only=True,
    independent_layouts=1, executions_per_condition=2), indent=2)+'\n')
