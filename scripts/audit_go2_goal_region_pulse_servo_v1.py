...
from lewm.goal_region_pulse_servo_development import GoalRegionPulseServo
from lewm.bounded_visual_servo_development import wrapped
from lewm.goal_region_pulse_scene_development import specification
...
def audit_condition(condition,result):
    ...
    previous_servo=read_npz(ROOT/'.generated/go2_pulse_feedback_servo_v1_attempt_001'/condition,'physics_trace.npz')
    ...
    motion=VisualLedMotion('gyro',identity=(0,0,0)); controller=GoalRegionPulseServo(); desired=[]
...
def main():
    ...
    sources=discover_sources(('scripts/audit_go2_goal_region_pulse_servo_v1.py',),launch['source_sha256'])
    ...
    try:
        for c in TRIALS:
            ...
            print('GOAL_REGION_PULSE_AUDIT',c,{k:v for k,v in reports[c].items() if k not in ('brake_segments','terminal_controller')},flush=True)
