...
import numpy as np
from scripts.run_go2_observed_floor_contact_goal_probe_v1 import OUTPUT as INPUT,PREVIOUS as PRIOR,PREVIOUS_SHA as PRIOR_SHA,CASES,TRIAL
...
OUTPUT=BASE/'go2_observed_floor_contact_goal_readout_v1_attempt_001'
PROTOCOL='docs/go2_observed_floor_contact_goal_readout_v1_2026-09-08.md'
...
def main():
    ...
    assert result['status']=='OBSERVED_FLOOR_CONTACT_GOAL_PROBE_COMPLETE' and result['cases']==[list(c) for c in CASES]
    ...
    sources=discover_sources((PROTOCOL,'scripts/read_go2_observed_floor_contact_goal_probe_v1.py',
        'lewm/tests/test_observed_floor_contact_goal_readout_development.py'),old['source_sha256'])
    ...
    try:
        ...
        write_json(OUTPUT/'result.json',dict(status='OBSERVED_FLOOR_CONTACT_GOAL_READOUT_COMPLETE',
        ...
        print('OBSERVED_FLOOR_CONTACT_GOAL_READOUT_COMPLETE',digest(OUTPUT/'result.json'),flush=True)
    except Exception as error:
        write_json(OUTPUT/'failure.json',dict(status='TERMINAL_OBSERVED_FLOOR_CONTACT_GOAL_READOUT_FAILURE',reason=repr(error)));raise
