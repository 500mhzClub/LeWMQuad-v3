"""Fresh maze1 execution of the prospectively admitted recent qualified reference."""
import argparse
import contextlib
import json
...
from scripts.recent_qualified_anchor_maze01_episode_development import collect, artifacts
from scripts.recent_qualified_anchor_maze01_audit_development import audit
from scripts.recent_qualified_anchor_native_prefix_development import admit_prefix, compare
from scripts.replay_go2_recent_qualified_anchor_prefix_v1 import (
    OUTPUT as PREFIX, INPUT as PRIOR, CASE as PRIOR_CASE, INPUT_SHA as PRIOR_SHA,
...
OUTPUT = BASE/'go2_recent_qualified_anchor_maze01_pilot_v1_attempt_001'
CASE = ('full_jepa_recent_qualified_anchor_maze_01', *PRIOR_CASE[1:])
PROTOCOL = 'docs/go2_recent_qualified_anchor_maze01_pilot_v1_2026-09-09.md'
PREFIX_SHA = '6e25c6c561473b60966a1a47388a01b48ab3e547da0c8ab13aa1e267b9fad302'


def fixed_prefix_report(report):
    expected=dict(frames=646,maximum_frames=646,first_requested_command_difference=645,
...
def verify_input_context(launch):
    ...
    if (launch['planned_case'] != list(CASE) or launch['scene_specification'] != specification(1)
            ...
            or launch['implementation_class'] != 'RecentQualifiedAnchorController'
            or launch['recent_qualified_anchor_enabled'] is not True
            or launch['partial_floor_height_constraint_enabled'] is not True):
        raise ValueError('fixed recent qualified reference maze1 definition required')
...
def verify_inputs(launch):
    ...
    print('RECENT_QUALIFIED_ANCHOR_MAZE01_NATIVE_INPUTS_VERIFIED',counters,flush=True)
...
def worker(launch_sha):
    ...
    terminal = dict(case=name,layout_index=index,model_name=model_name,
        status='RECENT_QUALIFIED_ANCHOR_MAZE01_WORKER_FAILED',artifact_sha256={}); started = time.perf_counter()
    with (OUTPUT/(name+'_worker.log')).open('x') as log, contextlib.redirect_stdout(log), contextlib.redirect_stderr(log):
        try:
            ...
            terminal.update(status='RECENT_QUALIFIED_ANCHOR_MAZE01_COLLECTED_AND_RAW_AUDITED',artifact_sha256=bindings,
...
def main():
    ...
    for name, sha in benchmark['source_sha256'].items():
        ...
        inherited[name] = sha
    sources = discover_sources((PROTOCOL, 'scripts/run_go2_recent_qualified_anchor_maze01_pilot_v1.py',
        'lewm/tests/test_recent_qualified_anchor_native_calculations_development.py',
        'lewm/tests/test_recent_qualified_anchor_native_prefix_development.py',
        'lewm/tests/test_recent_qualified_anchor_native_launcher_development.py'), inherited)
    ...
    launch.update(verification_benchmark_result_sha256=BENCHMARK_SHA,protocol=PROTOCOL,source_sha256=sources,output_root=str(OUTPUT),planned_case=list(CASE),
        ...
        implementation_class='RecentQualifiedAnchorController',robot_urdf_sha256=digest(URDF),
    ...
    if args.preflight_only:
        print('RECENT_QUALIFIED_ANCHOR_MAZE01_NATIVE_PREFLIGHT',json.dumps(dict(source_count=len(sources),hardware=resources,
    ...
    print('RECENT_QUALIFIED_ANCHOR_MAZE01_NATIVE_LAUNCHED',launch_sha,flush=True); started = time.perf_counter()
    try:
        ...
        print('RECENT_QUALIFIED_ANCHOR_MAZE01_NATIVE_TERMINAL',record['status'],record.get('verified_round_trip'),record.get('failure'),flush=True)
        if record['status'] != 'RECENT_QUALIFIED_ANCHOR_MAZE01_COLLECTED_AND_RAW_AUDITED':
        ...
        verify_inputs(launch); verify_artifacts(OUTPUT,bindings)
        write_json(OUTPUT/'result.json',dict(verification_benchmark_result_sha256=BENCHMARK_SHA,status='RECENT_QUALIFIED_ANCHOR_MAZE01_PILOT_V1_COMPLETE',
        ...
        print('RECENT_QUALIFIED_ANCHOR_MAZE01_NATIVE_COMPLETE',digest(OUTPUT/'result.json'),flush=True)
    except Exception as error:
        write_json(OUTPUT/'failure.json',dict(status='TERMINAL_RECENT_QUALIFIED_ANCHOR_MAZE01_NATIVE_FAILURE',reason=repr(error)))
        raise
