...
OUTPUT=BASE/'go2_training_bias_floor_coverage_v1_attempt_001'
PROTOCOL='docs/go2_training_bias_floor_coverage_v1_2026-09-08.md'
    ...
    for candidate,prediction,check in zip(selection['candidates'],selection['prediction'],selection['surface_checks'],strict=True):
        ...
        for foot in pending:
            ...
            feet.append(dict(shape_id=foot['shape_id'],centre_map_xy_m=centre.tolist(),
                ...
                partitions=[coverage(memory.patches,centre,divisions=n) for n in (1,2,4,8)]))
...
def main():
    ...
    sources=discover_sources((PROTOCOL,'scripts/diagnose_go2_training_bias_floor_coverage_v1.py',
        'lewm/tests/test_partitioned_floor_patch_coverage_development.py',
        'docs/go2_training_bias_goal_probe_result_2026-09-08.md'),readout['source_sha256'])
    resources=hardware()
    ...
    launch=original|dict(source_sha256=sources,protocol=PROTOCOL,output_root=str(OUTPUT),hardware=resources,
        ...
        controller_changed=False,division_levels=[1,2,4,8],maximum_diagnostic_frames=256)
    verify(launch);create_output(OUTPUT);write_json(OUTPUT/'launch.json',launch)
    print('TRAINING_BIAS_FLOOR_COVERAGE_LAUNCHED',digest(OUTPUT/'launch.json'),flush=True)
    ...
    try:
        ...
        write_json(OUTPUT/'result.json',dict(status='TRAINING_BIAS_FLOOR_COVERAGE_COMPLETE',
        ...
        print('TRAINING_BIAS_FLOOR_COVERAGE_COMPLETE',digest(OUTPUT/'result.json'),flush=True)
