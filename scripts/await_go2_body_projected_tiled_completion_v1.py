...
from scripts import verify_go2_body_projected_tiled_controller_completion_v1 as check
...
SOURCE = 'scripts/await_go2_body_projected_tiled_completion_v1.py'
TEST = 'lewm/tests/test_body_projected_tiled_completion_watch_development.py'
INVOCATION = 'docs/go2_body_projected_tiled_replay_invocation_2026-09-11.json'
PREPARATION = 'docs/go2_body_projected_tiled_completion_preparation_2026-09-11.json'
PREPARATION_SHA = '35244eabdb1cd9c5fd5e6ec75956dc4457472436b6a40c4f5c16792f6cdb23c9'
WATCH = ROOT/'docs/go2_body_projected_tiled_completion_watch_execution_2026-09-11.json'
RESULT = ROOT/'docs/go2_body_projected_tiled_completion_watch_result_2026-09-11.json'
FAILURE = ROOT/'docs/go2_body_projected_tiled_completion_watch_failure_2026-09-11.json'
...
def main():
    ...
    INVOCATION_SHA = args.invocation_sha256
    if re.fullmatch('[0-9a-f]{64}', INVOCATION_SHA) is None:
        raise ValueError('actual invocation SHA-256 required')
    ...
    sources = discover_sources((SOURCE, TEST, INVOCATION, PREPARATION), preparation['source_sha256'])
    verify(sources)
    ...
    write_json(WATCH, dict(status='BODY_PROJECTED_TILED_COMPLETION_WATCH_ACTIVE',
    ...
    print('BODY_PROJECTED_TILED_COMPLETION_WATCH_ACTIVE', digest(WATCH), flush=True)
    try:
        ...
        if (launch['source_sha256'] != invocation['source_sha256']
                ...
                or launch['body_projection_component_benchmark_sha256'] != invocation['component_benchmark_sha256']
        ...
        print('BODY_PROJECTED_TILED_EXECUTION_RECORDED', execution_sha, launch_sha, flush=True)
        ...
        print('BODY_PROJECTED_TILED_OWNER_ENDED_VERIFYING', result_sha, flush=True)
        subprocess.run([sys.executable, '-B', check.SOURCE, '--result-sha256', result_sha,
            '--execution-sha256', execution_sha], cwd=ROOT, check=True)
        verify(sources)
        write_json(RESULT, dict(status='BODY_PROJECTED_TILED_COMPLETION_WATCH_COMPLETE',
        ...
        print('BODY_PROJECTED_TILED_COMPLETION_WATCH_COMPLETE', digest(RESULT), digest(check.OUTPUT), flush=True)
    except BaseException as error:
        write_json(FAILURE, dict(status='TERMINAL_BODY_PROJECTED_TILED_COMPLETION_WATCH_FAILURE',
