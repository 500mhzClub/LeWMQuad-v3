...
def collect(trial, definition, *, output, model, geometry, persistent, episode_name):
    ...
    try:
        ...
        session._runtime['high_level'] = 'active_view_goal_probe_v1; online model plus measured surfaces'
        ...
        controller = ActiveViewGoalProbe(model, geometry, persistent=persistent)
        try:
        ...
        result = dict(status='ACTIVE_VIEW_GOAL_PROBE_TERMINAL_AUDIT_REQUIRED', trial=trial,
        ...
        print('ACTIVE_VIEW_GOAL_PROBE', trial, result, flush=True)
        return result
    finally:
