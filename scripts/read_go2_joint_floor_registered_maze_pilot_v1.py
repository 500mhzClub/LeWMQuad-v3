"""Read actual floor-registered navigation and paired pose accuracy without outcome relabeling."""
import argparse
...
from scripts.run_go2_joint_floor_registered_maze_pilot_v1 import OUTPUT as INPUT, CASE
...
OUTPUT = BASE/'go2_joint_floor_registered_maze_readout_v1_attempt_001'
PROTOCOL = 'docs/go2_joint_floor_registered_maze_readout_v1_2026-09-08.md'
...
def summarize(record):
    ...
    return dict(case=name, layout_index=record['layout_index'], collection=collection,
        ...
        confirmed_floor_execution=confirmed_floor_execution(poses, read_json(directory, 'command_tape.json'), read_rows(directory)),
...
def main():
    ...
    assert result['status'] == 'JOINT_FLOOR_REGISTERED_MAZE_PILOT_COMPLETE' and len(result['conditions']) == 1
    ...
    sources = discover_sources((PROTOCOL, 'scripts/read_go2_joint_floor_registered_maze_pilot_v1.py',
        'lewm/tests/test_view_reentry_readout_development.py',
        'lewm/tests/test_confirmed_floor_execution_readout_development.py',
        'lewm/tests/test_floor_registered_pose_readout_development.py'), old['source_sha256'])
    resources = hardware()
    ...
    try:
        ...
        write_json(OUTPUT/'result.json', dict(status='JOINT_FLOOR_REGISTERED_MAZE_READOUT_COMPLETE',
        ...
        print('JOINT_FLOOR_REGISTERED_MAZE_READOUT_COMPLETE', digest(OUTPUT/'result.json'), flush=True)
    except Exception as error:
        write_json(OUTPUT/'failure.json', dict(status='TERMINAL_JOINT_FLOOR_REGISTERED_MAZE_READOUT_FAILURE', reason=repr(error)))
        raise
