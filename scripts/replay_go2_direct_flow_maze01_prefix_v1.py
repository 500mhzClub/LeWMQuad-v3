"""Fresh full controller through the fixed maze1 failed observation; no following tape."""
import argparse
...
from lewm.direct_flow_prefix_development import compare_step, MAX_FRAMES, BOUNDARY_FRAME
...
OUTPUT=BASE/'go2_direct_flow_maze01_prefix_v1_attempt_001'
PROTOCOL='docs/go2_direct_flow_maze01_prefix_v1_2026-09-09.md'
INPUT_SHA='a0ac72330a6c34fbae7812a360b22189086f16bd83f586d71e769539ddc2e720'
CASE=('full_jepa_novel_maze_01',1,'full','jepa','seed_2026091001_full_jepa')
...
def admit(result,audit,old):
    ...
    if (record['case'] != CASE[0] or record['status'] != 'INDEPENDENT_FLOOR_TRANSPORT_COLLECTED_AND_RAW_AUDITED'
            or audit['layout_index'] != 1 or audit['verified_round_trip'] != record['verified_round_trip']):
        raise ValueError('completed maze1 raw-audited population required')
...
def replay(launch):
    ...
    with writer(OUTPUT) as append:
        for i,original in enumerate(islice(read_rows(directory),MAX_FRAMES)):
            ...
            last=json.loads(json.dumps(controller.observe(policy,depth,fast,now_ns=now,
            ...
            try:
                ...
                if check['controller_recovered']:
                    current_dual_camera_pose(last['original_visual_evidence'],policy,image,auxiliary,
                        identity=(0,0,0),now_ns=now)
                    current_measured_floor_pose(last['evidence'],identity=(0,0,0),now_ns=now)
            ...
            if i%32==0: print('DIRECT_FLOW_MAZE01_PREFIX_FRAME',i,flush=True)
            if check['stop']: break
    ...
    return dict(case=CASE[0],frames=frames,maximum_frames=MAX_FRAMES,boundary_frame=BOUNDARY_FRAME,
...
def main():
    ...
    sources=discover_sources((PROTOCOL,'scripts/replay_go2_direct_flow_maze01_prefix_v1.py',
    ...
    launch.update(protocol=PROTOCOL,source_sha256=sources,output_root=str(OUTPUT),replay_input_bindings=ids,
        ...
        minimum_free_bytes=RESERVE_BYTES,concurrency_reason='one CPU replay alongside the fixed reactive native cohort',
        input_scope='completed development maze1 through its original tracking failure; no following observation')
    ...
    if args.preflight_only:
        print('DIRECT_FLOW_MAZE01_PREFIX_PREFLIGHT',json.dumps(dict(source_count=len(sources),hardware=resources,
    ...
    print('DIRECT_FLOW_MAZE01_PREFIX_LAUNCHED',digest(OUTPUT/'launch.json'),flush=True)
    try:
        ...
        write_json(OUTPUT/'result.json',dict(status='DIRECT_FLOW_MAZE01_PREFIX_V1_COMPLETE',source_sha256=sources,
        ...
        print('DIRECT_FLOW_MAZE01_PREFIX_COMPLETE',digest(OUTPUT/'result.json'),
            {k:v for k,v in report.items() if k != 'fallback_receipt'},flush=True)
    except Exception as error:
        write_json(OUTPUT/'failure.json',dict(status='TERMINAL_DIRECT_FLOW_MAZE01_PREFIX_FAILURE',reason=repr(error)))
        raise
