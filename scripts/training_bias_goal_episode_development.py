...
def collect(trial, definition, *, output, model, geometry, persistent, episode_name, condition, variant):
    ...
    try:
        ...
        session._runtime['high_level'] = 'training_bias_goal_probe_v1; online model plus measured surfaces'
        ...
        controller = TrainingBiasGoalProbe(model, geometry, persistent=persistent, condition=condition, variant=variant)
        try:
        ...
        result = dict(status='TRAINING_BIAS_GOAL_PROBE_TERMINAL_AUDIT_REQUIRED', trial=trial,
