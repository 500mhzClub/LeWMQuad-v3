...
from scripts import replay_go2_progressive_batched_floor_late_history_v1 as run
...
SOURCE = 'scripts/verify_go2_progressive_batched_floor_controller_completion_v1.py'
TEST = 'lewm/tests/test_progressive_batched_floor_controller_completion_development.py'
EXECUTION = 'docs/go2_progressive_batched_floor_controller_replay_execution_2026-09-11.json'
EXECUTION_SHA = 'c7b73ff683ba557843418450deafae7e841f099c3095b3135ea824a0065d9da4'
OUTPUT = ROOT/'docs/go2_progressive_batched_floor_controller_completion_verification_2026-09-11.json'
...
def expected_report(prior_report, timing):
    ...
    for key in ('incremental_density_routed_floor_comparison', 'both_controllers_use_visibility_batched_receipt_copied_packed_fused_queries'):
        if expected.pop(key) is not True: raise ValueError('original density-routed comparison scope required')
    expected.update(baseline='DensityRoutedFloorController', candidate='ProgressiveBatchedFloorController',
        timing_windows=timing, normalized_state_type_paths=run.STATE_TYPE_PATHS,
        incremental_reuse_comparison=False, incremental_progressive_patch_batching_comparison=True,
        persistent_memory_type_unchanged=True, both_controllers_use_density_routed_floor_registration_and_mapping=True,
...
def main():
    ...
    expected_launch = dict(previous_result_sha256=run.PREVIOUS_RESULT_SHA,
        ...
        progressive_patch_microbenchmark_sha256=run.MICROBENCHMARK_SHA,frames=1428,
        state_frames=list(run.original.STATE_FRAMES), normalized_state_type_paths=run.STATE_TYPE_PATHS,
        baseline='DensityRoutedFloorController', candidate='ProgressiveBatchedFloorController',
    ...
    write_json(OUTPUT, dict(status='PROGRESSIVE_BATCHED_FLOOR_COMPLETION_VERIFIED',
        ...
        preceding_density_routed_completion_reauthenticated=True, actual_raw_and_model_bindings_rehashed=True,
    ...
    print('PROGRESSIVE_BATCHED_FLOOR_COMPLETION_VERIFIED', digest(OUTPUT), len(sources), reduction, flush=True)
