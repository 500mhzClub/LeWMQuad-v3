"""Additional map-index benefit over the receipt-copied current controller."""
...
from scripts.replay_go2_single_pass_dual_camera_prefix_v1 import verify_all as verify_single_pass

OUTPUT=BASE/'go2_single_pass_receipt_copied_benchmark_v1_attempt_001'
PROTOCOL='docs/go2_single_pass_receipt_copied_benchmark_v1_2026-09-09.md'
PREDECESSORS={
    'go2_receipt_copied_controller_benchmark_v1_attempt_001':'60849db8aa7a2b09e5e6de977115cb953f6c1b893ed62b0533ccd19b31f1ff95',
    'go2_single_pass_dual_camera_prefix_v1_attempt_001':'18b79c77516ef738366298ee3e093952a250cfd083938818004d232e71a7ca96',
}
FRAMES=514
LABELS=('receipt_copied','single_pass_receipt_copied')
...
def aggregate(rows):
    ...
    def values(group):
        ...
        old=np.asarray([r['controller_wall_ms']['receipt_copied'] for r in group])
        new=np.asarray([r['controller_wall_ms']['single_pass_receipt_copied'] for r in group])
        return dict(observations=len(group),receipt_copied_median_ms=float(np.median(old)),
            single_pass_receipt_copied_median_ms=float(np.median(new)),paired_median_reduction_ms=float(np.median(old-new)),
            receipt_copied_mean_ms=float(old.mean()),single_pass_receipt_copied_mean_ms=float(new.mean()),
            receipt_copied_over_100ms=int((old>100).sum()),single_pass_receipt_copied_over_100ms=int((new>100).sum()))
...
def replay(launch):
    models=[]; controllers=[]
    for cls in (ReceiptCopiedMeasuredFloorController,SinglePassReceiptCopiedController):
    ...
    with (OUTPUT/'paired_timings.jsonl').open('x') as stream,(OUTPUT/'resource_monitor.jsonl').open('x') as monitor:
        for i,original in enumerate(islice(read_rows(directory),FRAMES)):
            ...
            if i%64==0:
                ...
                print('SINGLE_PASS_RECEIPT_COPIED_BENCHMARK_FRAME',i,flush=True)
...
def verify_predecessors():
    ...
    for name,sha in PREDECESSORS.items():
        ...
        if name=='go2_receipt_copied_controller_benchmark_v1_attempt_001': verify_inputs(launch)
        else: verify_single_pass(launch)
...
def main():
    ...
    for name,sha in old['source_sha256'].items():
        ...
        inherited[name]=sha
    sources=discover_sources((PROTOCOL,'scripts/benchmark_go2_single_pass_receipt_copied_v1.py',
        'lewm/tests/test_single_pass_receipt_copied_controller_development.py',
        'lewm/tests/test_single_pass_receipt_copied_benchmark_development.py',
        'docs/go2_receipt_copied_controller_benchmark_result_2026-09-09.md',
        'docs/go2_single_pass_dual_camera_prefix_result_2026-09-09.md'),inherited)
    launch=old|dict(protocol=PROTOCOL,source_sha256=sources,output_root=str(OUTPUT),replay_input_bindings=ids,
        ...
        order='receipt-copied first on even observations, combined first on odd observations',
    ...
    print('SINGLE_PASS_RECEIPT_COPIED_BENCHMARK_LAUNCHED',digest(OUTPUT/'launch.json'),flush=True)
    try:
        ...
        write_json(OUTPUT/'result.json',dict(status='SINGLE_PASS_RECEIPT_COPIED_BENCHMARK_V1_COMPLETE',
        ...
        print('SINGLE_PASS_RECEIPT_COPIED_BENCHMARK_COMPLETE',digest(OUTPUT/'result.json'),flush=True)
    except Exception as error:
        write_json(OUTPUT/'failure.json',dict(status='TERMINAL_SINGLE_PASS_RECEIPT_COPIED_BENCHMARK_FAILURE',reason=repr(error))); raise
