#!/usr/bin/env python3
"""Two moving RGBD whole-mission observation runs with unchanged RGB control."""
import ast
import contextlib
import json
...
from scripts.run_go2_single_sample_rgbd_observation_development_v1 import preflight as prior_preflight, OUTPUT as PRIOR, verify_native

OUTPUT = ROOT/'.generated/go2_moving_rgbd_whole_task_development_v1_attempt_001'
SCHEMA = 'moving_rgbd_whole_task_development.v1'
NEW_SOURCES = (
    'lewm/depth_local_surfaces_development.py', 'lewm/depth_relative_motion_development.py',
    'lewm/depth_motion_evaluation_development.py', 'scripts/moving_rgbd_session_development.py',
    'scripts/run_go2_moving_rgbd_whole_task_development_v1.py',
    'scripts/audit_go2_moving_rgbd_whole_task_development_v1.py',
    'lewm/tests/test_depth_local_surfaces_development.py',
    'lewm/tests/test_depth_relative_motion_development.py',
    'lewm/tests/test_moving_rgbd_whole_task_development.py',
    'docs/go2_moving_rgbd_whole_task_development_v1_2026-09-05.md',
    'docs/go2_single_sample_rgbd_observation_development_v1_result_2026-09-05.md',
    'docs/go2_depth_local_surfaces_development_v1_2026-09-05.md')
...
def trial_specs():
    ...
    for spec in original_specs():
        ...
        spec['scene_id'] = 'go2-moving-rgbd-whole-task-development-v1-'+spec['layout_name']
        result.append(spec)
    return result
...
def preflight():
    ...
    if inherited != launch['source_sha256'] or audit['status'] != 'PASS' or not audit['all_depth_checks_pass']:
        raise ValueError('actual source-bound visual-depth interface required')
...
def main():
    ...
    write_json(OUTPUT/'launch.json', {'schema': SCHEMA, 'trial_specs': specs,
        ...
        'scope': 'two continuous development missions with causal non-actuating depth/gyro state observer; unchanged RGB policy, no JEPA/hardware qualification'})
    rows, prefixes = [], {}
    try:
        for spec in specs:
