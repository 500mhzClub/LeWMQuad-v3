"""Full-trace spatial feature observer comparison; no new commands or physics."""
import json
import time
import cv2
import numpy as np
...
from lewm.spatial_support_joint_observer_development import SpatialSupportVisualLedMotion
...
OUTPUT = BASE/'go2_spatial_support_observer_replay_v1_attempt_001'
PROTOCOL = 'docs/go2_spatial_support_observer_replay_v1_2026-09-08.md'
...
ARMS = ('original', 'spatial_support')


def replay(trial, arm):
    reader = IntentReturnRGBDReplay(INPUT/trial)
    motion = (JointTemporalAnchorVisualLedMotion if arm == 'original' else SpatialSupportVisualLedMotion)()
    rows = []
    ...
    print('SPATIAL_SUPPORT_REPLAY', json.dumps(report, sort_keys=True), flush=True)
    return report
...
def main():
    ...
    sources = discover_sources((PROTOCOL, 'scripts/replay_go2_spatial_support_observer_v1.py',
        'lewm/tests/test_spatial_support_observer_development.py'), original['source_sha256'])
    resources = hardware()
    ...
    print('SPATIAL_SUPPORT_REPLAY_LAUNCHED', digest(OUTPUT/'launch.json'), flush=True)
    reports = []
    try:
        ...
        write_json(OUTPUT/'result.json', dict(status='SPATIAL_SUPPORT_OBSERVER_REPLAY_COMPLETE',
            reports=reports, source_sha256=sources, artifact_sha256=products,
            candidate_eligible_for_separate_native_probe=all(r['diagnostic_thresholds_pass'] for r in reports if r['arm']=='spatial_support'),
        ...
        print('SPATIAL_SUPPORT_REPLAY_COMPLETE', digest(OUTPUT/'result.json'), flush=True)
    except Exception as error:
        write_json(OUTPUT/'failure.json', dict(status='TERMINAL_SPATIAL_SUPPORT_REPLAY_FAILURE', reason=repr(error)))
        raise
