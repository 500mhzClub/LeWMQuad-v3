...
from scripts.run_go2_retained_patch_contact_goal_probe_v1 import OUTPUT as INPUT, CASES
...
from scripts.run_go2_measured_floor_contact_goal_probe_v1 import OUTPUT as ORIGINAL

OUTPUT=BASE/'go2_retained_patch_contact_goal_readout_v1_attempt_001'
PROTOCOL='docs/go2_retained_patch_contact_goal_readout_v1_2026-09-08.md'
ORIGINAL_SHA='607524c7b6cc86356bc0bdceac1dd310723c6012e1dc377fe80c890f5057f17e'
...
def main():
    ...
    assert result['status']=='RETAINED_PATCH_CONTACT_GOAL_PROBE_COMPLETE' and result['cases']==[list(c) for c in CASES]
    ...
    assert old['status']=='MEASURED_FLOOR_CONTACT_GOAL_PROBE_COMPLETE'
    ...
    sources=discover_sources((PROTOCOL,'scripts/read_go2_retained_patch_contact_goal_probe_v1.py'),original['source_sha256'])
    ...
    try:
        ...
        write_json(OUTPUT/'result.json',dict(status='RETAINED_PATCH_CONTACT_GOAL_READOUT_COMPLETE',conditions=reports,models=grouped,
        ...
        print('RETAINED_PATCH_CONTACT_GOAL_READOUT_COMPLETE',digest(OUTPUT/'result.json'),flush=True)
    except Exception as error:
        write_json(OUTPUT/'failure.json',dict(status='TERMINAL_RETAINED_PATCH_CONTACT_GOAL_READOUT_FAILURE',reason=repr(error)));raise
