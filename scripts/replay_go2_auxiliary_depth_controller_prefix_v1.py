...
COMPATIBILITY=BASE/'go2_visible_robot_primary_compatibility_v1_attempt_001'
COMPATIBILITY_SHA='7b524b6b1408db532662f2fd7a190ed7a47c726f7636daa40e3651eff188d186'
OUTPUT=BASE/'go2_auxiliary_depth_controller_prefix_v1_attempt_001'
PROTOCOL='docs/go2_auxiliary_depth_controller_prefix_v1_2026-09-08.md'
...
def replay(admission,model_name,reader,acquisitions):
    ...
    before=state_digest(model.state_dict());controller=AuxiliaryDepthGoalProbe(model,ArticulatedCollisionGeometry(URDF),
...
def main():
    ...
    verify_artifacts(COMPATIBILITY,{'result.json':COMPATIBILITY_SHA});compat=read_json(COMPATIBILITY,'result.json')
    assert compat['status']=='VISIBLE_ROBOT_PRIMARY_COMPATIBILITY_COMPLETE'
    compat_ids={'result.json':COMPATIBILITY_SHA,**compat['artifact_sha256']};verify_artifacts(COMPATIBILITY,compat_ids)
    bound.append((COMPATIBILITY,compat_ids,compat))
    ...
    sources=discover_sources((PROTOCOL,'scripts/replay_go2_auxiliary_depth_controller_prefix_v1.py',
        'lewm/tests/test_recorded_command_prefix_comparison_development.py',
        'docs/go2_visible_robot_primary_compatibility_result_2026-09-08.md',
        'lewm/tests/test_auxiliary_tilted_depth_observation_development.py','lewm/tests/test_auxiliary_depth_floor_map_development.py',
        'lewm/tests/test_auxiliary_depth_goal_probe_development.py','lewm/tests/test_auxiliary_depth_packet_replay_development.py'),compat['source_sha256'])
    resources=hardware()
    ...
    print('AUXILIARY_DEPTH_CONTROLLER_PREFIX_LAUNCHED',digest(OUTPUT/'launch.json'),flush=True);start=time.perf_counter()
    try:
        ...
        acquisitions=read_json(INPUT/'sensor_prefix','auxiliary_camera_audit.json');assert len(acquisitions)==20
        ...
        write_json(OUTPUT/'result.json',dict(status='AUXILIARY_DEPTH_CONTROLLER_PREFIX_COMPLETE',
        ...
        print('AUXILIARY_DEPTH_CONTROLLER_PREFIX_COMPLETE',digest(OUTPUT/'result.json'),flush=True)
