"""Marker-preserving appearance adapter; same primitive collision construction."""
...
def mission_surfaces(boxes, arm, seed):
    surfaces = independently_seeded_surfaces(boxes, arm, seed)
    expected = {'marker_red_panel': ('landmark_red', (230, 15, 15, 255)),
                'marker_blue_panel': ('landmark_blue', (15, 15, 230, 255))}
    found = {b['wall_id']: b for b in boxes if b['wall_id'] in expected}
    if set(found) != set(expected):
        raise ValueError('exact two declared marker panels required')
    for name, (material, rgba) in expected.items():
        if found[name]['material_id'] != material:
            raise ValueError('marker visual identity must agree with declared material')
        mesh = dict(surfaces)[name+'_visual']
        # Color only: primitive collision and triangle coordinates stay intact.
        mesh.visual.vertex_colors = np.tile(np.asarray(rgba, np.uint8), (len(mesh.vertices), 1))
    return surfaces
...
def build_scene_from_pack(pack, *, output, appearance_arm, appearance_seed,
    ...
    boxes=[dict(wall_id=o.object_id,centre_xyz=o.center_xyz_m,size_xyz=o.size_xyz_m,yaw_rad=o.yaw_rad,material_id=o.material_id) for o in pack.static_objects]
    # Construct/validate procedural meshes before native allocation.
    meshes=mission_surfaces(boxes,appearance_arm,appearance_seed)
