"""Variable-height union visuals; existing native collision boxes and5mm camera."""
from dataclasses import replace
from lewm_genesis.variable_height_union_surface_development import independently_seeded_union_surfaces
import math
import numpy as np
...
def build_scene_from_pack(pack, *, output, appearance_arm, appearance_seed,
    ...
    if n_envs!=1 or backend!='cpu' or show_viewer or not render_robot:
        raise ValueError('one CPU offscreen robot-visible sensor characterization scene required')
    ...
    try:
        ...
        robot=scene.add_entity(gs.morphs.URDF(file=str(pack.robot.urdf_path),pos=pack.robot.spawn_xyz_m,
            quat=pack.robot.spawn_quat_wxyz,fixed=False,visualization=True,collision=True),name='go2')
        visual=[]; expected={}
        ...
        if not robot_geometry or int(scene.t)!=0 or not robot.morph.collision or not robot.morph.visualization:
            raise ValueError('actual collision-and-visual Go2 geometry and zero-step construction required')
